...
Motivation: 调查自动评分系统对ELLs的评分偏见及差异，尤其是训练数据不平衡的影响。

Method: 使用BERT模型在四种数据集上微调，分析21项评估题目，计算评分准确性和平均分数差距（MSG）。

Result: 训练数据量大（ELL=30,000和1,000）时未发现明显偏见，样本量小（ELL=200）时可能存在偏见。

Conclusion: 自动评分系统在数据量充足时对ELLs公平，但需注意小样本情况下的潜在偏见。
...
<details>
  <summary>Details</summary>
Motivation: 地理空间数据具有密集数值、时空依赖性和多模态表示等独特挑战，需评估基础模型在此领域的适用性。

Method: 基于16个气候变量和150个地点的数据，通过专家设计的8个模板生成问题-答案对，涵盖从基础查询到复杂时空比较的任务。

Result: 视觉语言模型表现最佳，分析揭示了不同基础模型在地理空间任务中的优缺点。

Conclusion: GeoGrid-Bench为地理空间数据分析和科学研究提供了基础模型应用的有效参考。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（如GPT-4）在临床部署中面临高计算成本和延迟问题，而小型语言模型（SLMs）虽成本低但能力有限，且临床数据敏感且稀缺。

Method: 提出MediPhi框架，包括预指令调优、模型合并和临床任务对齐；扩展CLUE基准为CLUE+；构建MediFlow合成数据集。

Result: MediPhi在多个临床任务上表现优异，如医学实体识别（提升64.3%）、放射报告分析（提升49.5%）和ICD-10编码（超越GPT-4 14%）。

Conclusion: MediPhi框架成功解决了SLMs在临床领域的适应问题，并通过合成数据和任务对齐进一步提升了性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法在概念/特征覆盖范围与质量验证之间存在权衡，因为规范研究需要大量人力。

Method: 结合人类生成的特征规范和LLMs的响应，同时通过可靠的人类判断验证规范质量。

Result: NOVA数据集在特征密度和概念重叠方面表现更优，且在预测人类语义相似性判断上优于纯人类数据集和词嵌入模型。

Conclusion: 人类概念知识比以往规范数据集所捕获的更丰富，经过适当验证，LLMs可作为认知科学研究的强大工具。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型的兴起促使研究者探索其符号能力，但RASP编程语言实现的任务难以通过无监督学习掌握，理论与实际存在差距。

Method: 提出tracr-injection方法，将RASP算法直接注入预训练模型，并验证其效果。

Result: 实验展示了三种算法的成功注入，模型内部形成可解释的子空间，且分布外性能有所提升。

Conclusion: tracr-injection方法有效，为模型内部符号机制提供了新视角，并开源了实验代码。
...
Method: IPOMP采用语义聚类和边界分析选择样本，并通过实时性能数据迭代替换冗余样本。

Result: 在BIG-bench数据集上，IPOMP效果提升1.6%至5.3%，稳定性提升至少57%，计算开销低于1%。

Conclusion: IPOMP能有效提升提示优化效果，且其实时性能引导方法可通用化增强现有核心集选择方法。
...
<details>
  <summary>Details</summary>
Motivation: 在线虚假信息的快速传播是一个全球性问题，但多语言和低资源语言的研究常被忽视。

Method: 通过共享任务设计两个子任务：单语言和跨语言声明检索，收集了179名参与者的52份测试提交。

Result: 23个团队提交了系统论文，论文总结了最佳表现系统和最有效的方法。

Conclusion: 共享任务及其数据集为多语言声明检索和自动事实核查提供了宝贵见解，支持未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 传统链式思维推理方法通常只生成单一答案，忽略了其他潜在答案的可能性，导致投票过程不够可靠。

Method: 使用三种排名投票方法（即时决选投票、博尔达计数投票和平均倒数排名投票）对多个排名答案进行投票。

Result: 在六个数据集上的实验表明，该方法优于基线，验证了排名答案信息和排名投票对推理性能的提升。

Conclusion: 排名投票方法能有效提高链式思维推理的可靠性，展示了利用排名答案信息的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型（LLMs）的快速发展，选择合适的基础模型对训练高质量奖励模型（RMs）至关重要，但这一因素常被忽视。

Method: 系统分析了基础模型选择对奖励建模性能的影响，结合小规模基准测试结果提升模型选择效果，并探讨了后训练步骤的影响。

Result: 性能提升达14%，基准测试结果与下游性能强相关，结合小规模基准测试可平均提升18%的模型选择效果。

Conclusion: 基础模型选择对奖励建模性能有显著影响，结合基准测试和优化后训练步骤可进一步提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决RAG框架中因检索不完美导致LLMs输出幻觉的问题。

Method: 提出Finetune-RAG微调方法，构建模拟现实不完美的RAG训练数据集；设计Bench-RAG评估管道。
...
<details>
  <summary>Details</summary>
Motivation: 现有标注数据集无法有效评估自动提取KGs的准确性，因为它们过于分散、规模小或复杂。AffilKG旨在解决这一问题。

Method: AffilKG提供六个数据集，包含完整书籍扫描和标注的KGs，涵盖简单的成员关系图和扩展的关系类型图。

Result: 初步实验显示模型性能在不同数据集间差异显著，验证了AffilKG在评估提取错误传播和验证KG提取方法方面的潜力。

Conclusion: AffilKG为KG提取的准确性评估和下游分析提供了重要资源，支持社会科学研究。
...
<details>
  <summary>Details</summary>
Motivation: 解决低资源语言（如客家话）翻译中的挑战，特别是在专业和文化特有词汇上的覆盖不足和语法连贯性问题。

Method: 测试了多种模型配置，包括纯词典方法、RAG结合Gemini 2.0的两阶段方法（Model 3）以及结合检索和高级语言建模的Model 4。

Result: 最佳模型（Model 4）BLEU得分达31%，显著优于纯词典方法（12%）。两阶段方法（Model 3）得分为26%，展示了迭代修正的价值。

Conclusion: 研究强调了定制资源、领域知识和与本地社区合作的重要性，为提升翻译准确性和流畅性提供了框架，同时支持文化保护。
...
<details>
  <summary>Details</summary>
Motivation: 解决大型推理模型在简单问题上过度推理导致的效率问题。

Method: 基于R1风格蒸馏模型，利用多阶段强化学习框架AutoThink动态决定是否进行显式推理。

Result: 在五个数学基准测试中，AutoThink在准确性与效率上优于现有方法，显著减少计算开销。
...
Method: 使用单模态模型（如ModernBERT、ConvNeXt-V2）、多模态融合技术和生成式模型（如GPT-4o、LLaVA），并评估生成式模型在单模态输入下的表现。

Result: 多模态方法显著优于单模态方法，但生成式模型在精度上落后于监督方法和指令调优模型。生成式模型能有效处理社交媒体常见问题（如leet speak、文本拉长）。

Conclusion: 多模态方法在事件检测中表现更优，但生成式模型需改进以提升精度和事件分类能力。
...
<details>
  <summary>Details</summary>
Motivation: 研究MLLMs为何无法识别模拟时钟时间，并探索微调是否能解决这一问题。

Method: 使用GPT-4.1进行实验，测试模型在不同时钟上的表现，分析其抽象和泛化能力。

Result: 模型在读取时钟时间上有进步，但仍存在局限性，可能仅记忆训练数据模式。

Conclusion: MLLMs在抽象和泛化能力上仍有不足，需进一步研究其学习机制。
...
<details>
  <summary>Details</summary>
Motivation: 现有规则检索方法因查询事实与规则抽象表示间的语义差距导致准确性低，影响推理性能。

Method: 提出SIAR方法，利用LLM生成潜在推理规则并用于查询增强；引入R³方法重新评估规则相关性。

Result: 实验证明SIAR和R³在多场景下显著提升检索效果和推理性能。

Conclusion: SIAR和R³有效解决了规则检索中的语义差距问题，提升了推理任务的性能。
...
<details>
  <summary>Details</summary>
Motivation: 随着CUAs能力的提升，其安全风险日益复杂，需系统化研究以指导未来研究和实践。

Method: 通过文献综述，围绕四个目标展开：定义CUAs、分类威胁、提出防御策略、总结评估方法。

Result: 提出了CUAs安全威胁的分类、防御策略的全面分类法，以及评估指标。

Conclusion: 为未来研究提供了结构化基础，并为实践者提供了设计安全CUAs的指导。
...
<details>
  <summary>Details</summary>
Motivation: 单代理链式思维和多代理系统在解决复杂业务工作流任务时存在协作和效率问题。

Method: 构建集成知识图并维护提示树，以获取跨阶段相关信息。

Result: Cochain在多个数据集上表现优于基线方法，甚至小模型结合Cochain优于GPT-4。

Conclusion: Cochain有效解决了业务工作流中的协作问题，具有高效和低成本的优势。
...
Method: 通过两个强大的LRM生成和验证CoT过程，并引入RV和CD分数，建立自给自足的管道来整理数据集。

Result: 实验证明RV和CD分数对LRM训练有积极影响，并训练出一系列高性能LRM。

Conclusion: OmniThought数据集显著提升了LRM在复杂任务中的表现，推动了LRM的发展。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）部署时计算资源消耗大，KV Cache量化可平衡内存和精度，但异常令牌影响量化效果。

Method: 识别解码过程中的异常令牌并排除量化，优化KV Cache的通道和令牌量化策略。

Result: 在2位量化下显著提升精度，内存使用减少6.4倍，吞吐量提高2.3倍。

Conclusion: 排除异常令牌的量化方法有效提升KV Cache性能，为LLMs部署提供高效解决方案。
...
Result: 在Phi-3和Phi-2模型上，多语言通用知识LoRA模块显著提升了单语和跨语言任务的性能。

Conclusion: GenKnowSub方法有效提升了零样本泛化能力，适用于不同规模的语言模型。
...
<details>
  <summary>Details</summary>
Motivation: 当前方法在将多语言能力迁移到目标语言时，可能因源模型主要基于英语训练而限制目标语言的表达能力。

Method: SALT通过目标语言预训练模型的嵌入，基于源与目标词汇重叠的相似性，为每个非重叠词汇的嵌入空间生成独特的回归线。

Result: 实验表明，SALT显著优于其他迁移方法，加速了语言适应的收敛速度，并在跨语言理解任务中表现突出。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在理解LLMs如何表现出类似人格和智能的行为，并探索人工与生物认知的异同。

Method: 采用基于提示的方法，系统研究提示诱导的转变（PIT）和提示诱导的幻觉（PIH）。

Result: 揭示了人工与生物认知的结构相似性与差异，提出提示工程不仅是技术工具，也是科学方法。

Conclusion: 人类与AI的合作为认知科学的未来提供了原型，提示工程成为探索意义深层结构的科学方法。
...
Method: 论文系统分类了E2E多说话人ASR的神经方法，分析了SIMO与SISO架构的特性、算法改进、长语音处理策略及基准测试比较。

Result: 通过比较标准基准测试，论文总结了不同方法的性能与优缺点。

Conclusion: 论文指出了构建鲁棒且可扩展的多说话人ASR的开放挑战与未来研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 理解ICL的机制及其争议，澄清其是否仅为数据记忆或涉及更复杂的算法发展。

Method: 利用Pythia扩展套件和中间检查点，系统研究ICL性能及残差流子空间的机制分析。

Result: ICL超越了简单的数据记忆，但未形成独立符号算法；揭示了训练动态、模型能力和机制可解释性的影响。
...
<details>
  <summary>Details</summary>
Motivation: 探索Abugida语言的音节序列预测，以改进文本预测、拼写校正和数据增强等应用。

Method: 使用Transformer模型从不同类型的输入（如辅音序列、元音序列、部分音节和掩码音节）重建完整音节序列。

Result: 辅音序列在音节预测中表现优异，而元音序列更具挑战性；模型在部分和掩码音节重建任务中表现稳健。

Conclusion: 研究为Abugida语言的序列预测提供了新见解，并展示了实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法生成的多轮对话数据难以兼顾多样性和质量，限制了LLMs在对话中的上下文连贯性。

Method: 提出Review-Instruct框架，通过“Ask-Respond-Review”迭代过程，结合多个Reviewer反馈优化指令。

Result: 在多个基准测试中显著提升性能，MMLU-Pro和MT-Bench分别提升2.9%和2%。

Conclusion: 多代理框架能有效生成高质量对话数据，Review阶段和多Reviewer设计是关键。
...
Method: 结合俄语GitHub仓库的人工注释和合成生成注释，并通过自动化验证确保符合多种编程语言标准。

Result: 在StRuCom上微调的Qwen2.5-Coder模型在chrf++和BERTScore上显著优于基线模型。
...
Method: 提出LLMs本体能力分类法，并设计OntoURL基准，包含15个任务、58,981个问题，覆盖40个本体和8个领域。

Result: 实验显示LLMs在理解本体知识方面表现良好，但在推理和学习任务中存在明显不足。

Conclusion: OntoURL是推动LLMs与形式化知识表示结合的重要基准，揭示了LLMs处理符号知识的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 为情感语音识别研究提供易于访问、可复现的数据集和标准化基准。

Method: 通过数据集选择、整理和标准化流程，构建多语言情感语音数据集。

Result: 数据集公开可用，并提供了多个模型的性能结果。

Conclusion: CAMEO为多语言情感语音识别研究提供了有价值的资源和基准。
...
<details>
  <summary>Details</summary>
Motivation: 奖励模型训练成本高，而高质量合成数据增多，探索能否用简单指标替代奖励模型。

Method: 基于BLEU的BLEUBERI方法，结合GRPO优化策略，直接使用BLEU作为奖励函数。

Result: BLEUBERI在多个基准测试中表现与奖励模型相当，且生成内容更事实准确。

Conclusion: BLEUBERI证明字符串匹配指标是奖励模型的有效廉价替代方案。
...
<details>
  <summary>Details</summary>
Motivation: 专利权利要求起草复杂且依赖专家，小型企业难以负担。现有自动化生成方法评估不一致，需更准确的评估标准。

Method: 引入Patent-CE基准和PatClaimEval多维评估方法，结合专家标注的五大标准。

Result: PatClaimEval与人工评估相关性最高，优于其他测试指标。
...
<details>
  <summary>Details</summary>
Motivation: 探讨推理链长度是否对事实准确性有普遍提升，特别是在非数学任务中。

Method: 从先进推理模型中提取推理轨迹，结合知识图谱信息，微调多种模型，并进行大规模实验。

Result: 小型推理模型在单次运行中显著提升事实准确性，增加计算和标记预算可进一步提升2-8%。

Conclusion: 测试时扩展计算资源能有效提升开放域问答任务中的推理准确性。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在长上下文信息利用中存在数据质量、训练效率和优化目标不足的问题。

Method: SoLoPO框架包含短上下文偏好优化（PO）和短到长奖励对齐（SoLo-RA），提升模型对长上下文的处理能力。

Result: 实验表明SoLoPO显著提升了模型在长上下文任务中的性能和效率。

Conclusion: SoLoPO框架有效解决了长上下文对齐问题，提升了模型的泛化能力和效率。
...
<details>
  <summary>Details</summary>
Motivation: 解决图像文档中多语言信息的提取与处理问题，缩小语言差距，提升跨语言环境下的信息获取能力。

Method: 使用Tesseract进行OCR文本提取，结合Gemini API实现跨语言翻译与摘要生成，辅以TensorFlow情感分析、Transformers主题分类和Regex日期提取。

Result: 系统通过Gradio界面提供实用工具，成功实现了多语言信息的提取与处理，提升了信息获取效率。

Conclusion: 该系统展示了如何利用现有库、模型和API解决实际问题，为跨语言信息处理提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 探讨无位置编码的Transformer在平均硬注意力机制下的表达能力，揭示其与半代数集的对应关系。

Method: 通过理论分析，证明NoPE-AHATs能表达非负整数解的多变量多项式方程（Diophantine方程）对应的语言。

Result: NoPE-AHATs的表达能力精确对应于半代数集，且其分析问题是不可判定的。

Conclusion: NoPE-transformers在平均硬注意力机制下具有强大的表达能力，但存在不可判定的分析问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法未利用历史信息，限制了生成简洁响应的能力。

Method: 提出HAPO，基于历史状态设计长度奖励函数，联合优化正确性和效率。

Result: 实验显示HAPO减少响应长度33-59%，准确率仅下降2-5%。

Conclusion: HAPO有效提升LLM的简洁推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 在分布式系统中处理长上下文时，计算开销、内存使用和网络带宽较高，亟需优化。

Method: 采用语义缓存技术存储和重用中间上下文摘要，减少冗余计算。

Result: 在NaturalQuestions、TriviaQA和合成ArXiv数据集上，冗余计算减少50-60%，同时保持答案准确性。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型的知识储备有限，现有检索方法常引入无关或噪声信息，影响推理准确性。

Method: 采用强化学习框架，引入知识精炼步骤和定制化检索奖励，通过“搜索-精炼-思考”迭代优化证据。

Result: 在单跳和多跳QA基准测试中表现优异，尤其在复杂多跳推理场景中。

Conclusion: AutoRefine通过高质量搜索和证据合成，显著提升了检索增强推理的效果。
...
<details>
  <summary>Details</summary>
Motivation: 早期风险检测（ERD）需要快速准确地识别用户面临的社会和健康问题，但传统分类指标可能不足以满足需求，因此需要新的方法。

Method: 采用时间微调策略，显式地将时间纳入学习过程，分析用户完整的发帖历史，并结合时间指标评估模型性能。

Result: 在西班牙语的抑郁和饮食障碍任务中，该方法取得了与MentalRiskES 2023最佳模型竞争的结果，优化了上下文和时间进展的决策。

Conclusion: 通过时间微调，可以结合精度和速度作为单一目标，有效解决ERD问题，充分发挥Transformer模型的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 探索Transformer在多语言形态学任务中的泛化能力，尤其是其在音韵和次音韵层面的隐式现象捕捉能力。

Method: 提出了一种语言无关的探测方法，用于分析基于音素训练的Transformer的音韵特征编码，并在七种形态多样的语言中进行了实验。

Result: 局部音韵特征（如辅音浊化）在音素嵌入中表现良好，而长距离依赖（如元音和谐）在编码器中表现更好。

Conclusion: 研究结果为形态学模型的训练提供了实证策略，强调了次音韵特征获取的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM在科学写作支持上能力有限，无法满足研究沟通的高要求，特别是跨章节的概念连贯性和迭代修订需求。

Method: 构建了包含7,040篇论文和140,000条修订指令的数据集，开发了参数规模从1.5B到14B的开源LLM XtraGPT。

Result: XtraGPT在实验中显著优于同规模基线模型，接近专有系统质量，自动和人工评估均证实其有效性。

Conclusion: XtraGPT框架为科学写作提供了高质量的修订支持，填补了现有LLM的不足。
...
<details>
  <summary>Details</summary>
Motivation: 当前领域缺乏合适的数据集和自动评估标准，阻碍了关键问题生成任务的发展。

Method: 构建大规模人工标注数据集，并研究基于大型语言模型（LLMs）的自动评估方法。

Result: 发现基于LLMs的参考评估方法与人类判断最相关，并通过零样本评估11个LLMs建立了强基线。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法结合语音编码器和LLM时性能不佳或灵活性不足，需要一种更优的解决方案。

Method: 利用ASR后验矩阵生成伪音频嵌入，与LLM的文本嵌入拼接，实现语音与文本的高效结合。

Result: 在8个MLS测试集上平均WERR提升49%，且模型具有模块化和零样本适应性。
...
<details>
  <summary>Details</summary>
Motivation: 公众对癌症预防、筛查和治疗的认知不足可能导致延误诊断和不充分治疗，研究旨在评估LLMs在生成准确、安全和可及的癌症信息方面的能力。

Method: 采用混合方法评估框架，结合定量指标、定性专家评分及统计方法（Welch's ANOVA、Games-Howell和Hedges' g），比较了五种通用和三种医疗LLMs的表现。

Result: 通用LLMs在语言质量和情感表达上表现更好，医疗LLMs在信息可及性上更优，但后者存在更高的潜在危害、毒性和偏见。

Conclusion: 研究揭示了领域专业知识与健康信息安全性之间的权衡，强调了改进模型设计以减少危害和偏见、提升安全性和情感表达的必要性。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLMs越来越多地作为领域导向代理部署，其遵循领域特定指南的能力尚未得到充分评估，亟需一个全面基准。

Method: 提出GuideBench，从规则多样性、规则更新鲁棒性和人类偏好对齐三方面评估LLMs。

Result: 实验表明，现有LLMs在遵循领域导向指南方面仍有显著改进空间。

Conclusion: GuideBench为评估和提升LLMs的领域指南遵循能力提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 研究早期伊斯兰时期《古兰经》的口头传统及其书面记录的演变，探索tajwid规则的系统性。

Method: 使用开罗《古兰经》的数字编码版，开发Python模块以添加或删除tajwid的正字法层。

Result: 开罗《古兰经》的文本可作为对齐和比较其他手稿的基准，为研究阿拉伯文字提供强大工具。

Conclusion: 通过文本映射，可以深入研究《古兰经》中附加符号系统的本质。
...
<details>
  <summary>Details</summary>
Motivation: 医疗领域的大型语言模型安全性问题日益突出，现有评估缺乏临床特异性和对抗性攻击的覆盖。

Method: CARES包含18,000多个提示，涵盖八项医疗安全原则、四种危害级别和四种提示风格，并提出了三阶段响应评估协议和细粒度安全评分。

Result: 分析显示，许多先进模型仍易受对抗性攻击，同时对安全但非典型查询过度拒绝。

Conclusion: CARES为医疗LLM在对抗性和模糊条件下的安全性测试和改进提供了严谨框架。
...
<details>
  <summary>Details</summary>
Motivation: 为了促进越南两个民族之间的文化交流，但巴拿语资源匮乏阻碍了翻译模型的训练。

Method: 采用序列到序列的预训练语言模型，利用越南语模型迁移学习，结合数据增强和启发式方法优化翻译。

Result: 方法显著提升了巴拿语-越南语翻译效果，促进了语言保护和民族理解。

Conclusion: 迁移学习和数据增强有效解决了资源不平衡问题，为类似语言翻译提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 探究显式CoT推理对指令遵循任务的影响，并寻找缓解性能下降的方法。

Method: 评估15个模型在IFEval和ComplexBench上的表现，通过案例研究和注意力分析识别问题，提出四种策略（上下文学习、自我反思、自我选择性推理、分类器选择性推理）。
...
Method: 引入GODBench基准整合视频和文本模态，并提出RoT多步推理框架。

Result: 实验表明现有方法在创意评论生成上仍有挑战，而RoT显著提升了创造力。

Conclusion: RoT为MLLM的创意生成提供了有效方法，GODBench为相关研究提供了新基准。
...
<details>
  <summary>Details</summary>
Motivation: 研究数据压缩与LLMs能力的关系，特别是在代码智能领域，以解决现有研究对多语言、多任务代码评估的不足。

Method: 通过评估开源Code LLMs，引入Format Annealing方法，并使用新的代码验证集测量压缩效率（BPC）。

Result: 实证结果显示代码智能与BPC之间存在对数关系，修正了先前线性关系的假设。

Conclusion: 研究深化了对压缩在代码智能发展中作用的理解，并提供了一个稳健的评估框架。
...
TL;DR: 论文通过分类生物医学QA基准为推理和知识两类，发现仅32.8%问题需复杂推理，评估模型表现并开发BioMed-R1提升推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前医学推理基准混合了推理与事实记忆，需分离以更准确评估模型能力。

Method: 使用PubMedBERT分类器分离11个生物医学QA基准，评估模型表现，并通过微调和强化学习训练BioMed-R1。

Result: 发现模型在推理和知识表现间存在差距，BioMed-R1在同类模型中表现最佳。

Conclusion: 未来可通过临床案例和对抗性训练进一步提升模型推理能力。
...
TL;DR: 提出了两种无参考指标用于评估分类法的质量，分别评估鲁棒性和逻辑充分性，并与黄金标准分类法F1分数相关性良好。


<details>
  <summary>Details</summary>
Motivation: 现有指标未覆盖某些错误类型，需开发新指标以更全面评估分类法质量。

Method: 第一种指标通过计算语义与分类相似性相关性评估鲁棒性；第二种利用自然语言推理评估逻辑充分性。

Result: 在五个分类法上测试，两种指标与黄金标准分类法的F1分数相关性良好。

Conclusion: 新指标能有效补充现有分类法质量评估方法，覆盖更广的错误类型。
...
TL;DR: HelpSteer3-Preference是一个高质量、多样化的人类标注偏好数据集，用于训练RLHF语言模型，显著提升了奖励模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有偏好数据集的质量和多样性需要不断提升以满足RLHF训练的需求。

Method: 发布了包含40,000多样本、涵盖多领域任务的HelpSteer3-Preference数据集，并基于其训练奖励模型。

Result: 训练的奖励模型在RM-Bench和JudgeBench上分别达到82.4%和73.7%的性能，显著优于之前结果。

Conclusion: HelpSteer3-Preference数据集能有效提升RLHF训练效果，并支持生成式奖励模型和策略模型的对齐。
...
TL;DR: LLMs通过强化学习优化汇编代码性能，Qwen2.5-Coder-7B-PPO模型表现优于行业标准编译器gcc -O3。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在汇编代码优化中的潜力，因其对执行细节的精细控制能力。

Method: 使用PPO强化学习框架训练LLMs，奖励函数结合功能正确性和执行性能。

Result: 模型在8,072个真实程序上测试，通过率96.0%，平均加速1.47倍。
...
TL;DR: SoftCoT++通过扰动潜在思维和对比学习，扩展了SoftCoT方法，以在连续潜在空间中实现多样化的推理路径探索，显著提升了推理性能。
...
Method: 通过多个专用初始令牌扰动潜在思维，并应用对比学习以增强软思维表示的多样性。

Result: 在五个推理基准和两种LLM架构上，SoftCoT++显著优于SoftCoT，并与自一致性缩放技术兼容。

Conclusion: SoftCoT++为连续空间推理提供了多样化探索的有效方法，性能优越且兼容现有技术。
...
TL;DR: 研究评估了基于Transformer的模型（GPT2、LLaMA-7B和LLaMA2-7B）在解释阅读时眼动行为（注视时长）方面的表现，发现其优于早期模型，但仍无法完全匹配人类预测能力。


<details>
  <summary>Details</summary>
Motivation: 探索先进语言模型在解释人类语言理解认知过程中的表现，尤其是注视时长与预测能力的关系。

Method: 使用Transformer模型（GPT2、LLaMA-7B和LLaMA2-7B）分析Rioplantense西班牙语读者的注视时长数据。

Result: Transformer模型优于早期模型，但仍无法完全解释人类预测能力的方差。

Conclusion: 尽管技术进步，现有语言模型的预测方式仍与人类阅读行为存在差异。
...
TL;DR: 论文提出了一种名为SSOCL的双层自监督持续学习框架，用于处理连续无标记的生理数据流，提升情绪识别的性能。


<details>
  <summary>Details</summary>
Motivation: 生理数据（如EEG）存在跨主体差异和噪声标签问题，现有方法难以应对连续无标记数据流的挑战。

Method: 采用双层架构，结合动态记忆缓冲区和伪标签分配，通过快速适应模块和聚类映射模块处理数据流。

Result: 在两个主流EEG任务上验证，SSOCL框架在连续数据流中表现出色，跨主体泛化能力强，优于现有方法。
...
TL;DR: 该论文探讨了基础模型（FMs）在乳腺癌乳腺X光分类中的公平性和偏见问题，发现数据集聚合和领域适应策略虽能提升性能，但无法完全消除偏见，而公平感知技术能更稳定地实现公平性能。


<details>
  <summary>Details</summary>
Motivation: 尽管计算机辅助诊断工具在乳腺癌筛查中有所发展，但其临床推广仍受数据变异性和偏见的限制。基础模型虽表现出强大的泛化能力，但仍受虚假相关性的影响。

Method: 利用来自不同来源的大规模数据集（包括代表性不足的地区数据和内部数据集），研究了FMs的性能和偏见，并测试了数据集聚合、领域适应和公平感知技术。

Result: 模态特定的预训练提升了性能，但跨域泛化能力不足；数据集聚合改善了整体性能但未完全消除偏见；公平感知技术实现了更稳定的公平性能。

Conclusion: 强调了在基于FMs的模型中纳入严格的公平性评估和缓解策略的必要性，以促进包容性和泛化性AI的发展。
...
TL;DR: 研究探讨多模态大语言模型是否通过共同表征整合不同模态，通过机器教学理论评估图像与坐标表示的教学复杂度，发现图像表示更高效，但概念简单性可能跨模态一致。


<details>
  <summary>Details</summary>
Motivation: 验证多模态大语言模型是否真正通过共同表征整合不同模态，如图像与文本描述是否映射到潜在空间的相似区域。

Method: 使用机器教学理论，在Quick, Draw!数据集中评估图像（位图）与坐标（TikZ格式）表示的教学复杂度。

Result: 图像表示通常需要更少教学片段且准确率更高，但概念的教学复杂度排名在两种模态中相似，表明概念简单性可能跨模态一致。

Conclusion: 多模态模型的潜在空间中，概念的简单性可能是固有属性，不受具体表征形式影响。
...
TL;DR: Aquarius是一套面向营销场景的工业级视频生成模型，支持大规模集群和百亿参数模型，通过高效工程架构和算法创新实现高保真、多比例、长视频合成。
...
Result: 实现了36%的MFU和2.35倍的推理加速，支持多比例、多分辨率、多时长视频生成。

Conclusion: Aquarius展示了工业级视频生成的高效性，未来将扩展更多下游应用和评估指标。
...
TL;DR: 本文提出了一种基于4层TSMamba架构的AE分类器系统，用于检测恶意无人机，显著提高了分类准确率并降低了计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 恶意无人机对下一代网络构成威胁，如未经授权的监视和数据盗窃，需要有效的检测方法。

Method: 采用4层TSMamba架构的AE生成残差值，再通过ResNet分类器处理，实现低复杂度高准确率。

Result: 实验显示，二元和多类分类的召回率分别达到99.8%和96.7%，计算复杂度降低。
...
TL;DR: 该研究通过扩展单图像超分辨率（SISR）的SRGAN结构，提出了一种增强视频超分辨率的方法，引入3D非局部块以捕捉时空关系，并开发了实验训练管道。


<details>
  <summary>Details</summary>
Motivation: SRGAN在单图像增强中表现优异，但未考虑视频处理中的时间连续性，因此需要改进以处理时空数据。

Method: 提出了一种改进框架，结合3D非局部块，并开发了基于分块学习和数据退化技术的训练管道，以模拟真实视频条件。
...
TL;DR: 提出了一种名为ARFC-WAHNet的网络，用于红外小目标检测，通过自适应感受野卷积和小波注意力分层网络解决传统方法的局限性。
...
Conclusion: ARFC-WAHNet在检测精度和鲁棒性上显著提升，适用于复杂场景。
...
TL;DR: SRMamba是一种基于范围视图的LiDAR点云超分辨率方法，通过Hough Voting和Hole Compensation策略解决稀疏场景中的点云空间结构恢复问题，并在多光束点云上表现出色。


<details>
  <summary>Details</summary>
Motivation: 由于LiDAR点云的稀疏性和不规则结构，尤其是新视角下的点云上采样问题具有挑战性，需要一种有效的方法恢复3D空间结构。

Method: 采用基于Hough Voting的投影技术和Hole Compensation策略消除水平线性空洞，利用Visual State Space模型和多方向扫描机制建立长距离依赖关系，并通过非对称U-Net网络适应不同光束数的LiDAR输入。

Result: 在SemanticKITTI和nuScenes数据集上的实验表明，SRMamba在定性和定量评估中均优于其他算法。

Conclusion: SRMamba为解决稀疏场景下LiDAR点云超分辨率问题提供了一种高效且通用的解决方案。
...
TL;DR: MIRAGE是一个多模态基准测试，用于评估模型在计数、空间关系推理及两者结合任务中的表现，揭示了现有模型的局限性。
...
Method: 通过设计多样化和复杂的场景，MIRAGE测试模型在细粒度识别和推理任务中的能力。

Result: MIRAGE揭示了当前先进模型在基础能力上的不足，强调了改进表示和推理框架的必要性。

Conclusion: MIRAGE为未来时空推理研究提供了方向，推动了模型在核心认知能力上的进步。
...
TL;DR: MMLongBench是首个针对长上下文视觉语言模型（LCVLMs）的多样化基准测试，覆盖13,331个样本和五种任务类别，评估了46个模型的性能，发现现有模型在长上下文任务中仍有较大改进空间。


<details>
  <summary>Details</summary>
Motivation: 随着长上下文视觉语言模型的发展，缺乏全面评估其能力的基准测试，因此需要MMLongBench来填补这一空白。

Method: 通过跨模态标记化方案，将13,331个样本分为五种任务类别，并在五种标准化输入长度（8K-128K标记）下评估46个LCVLMs的性能。

Result: 研究发现：单任务性能不能代表整体能力；现有模型在长上下文任务中表现不佳；推理能力强的模型表现更好。

Conclusion: MMLongBench为诊断和提升下一代LCVLMs提供了基础，揭示了未来改进的方向。
...
TL;DR: 论文提出了一种无需训练的跨图像对比解码方法（CICD），用于缓解大型视觉语言模型（LVLM）中的语言先验问题，减少幻觉内容生成。


<details>
  <summary>Details</summary>
Motivation: 语言先验是LVLM产生幻觉内容的主要原因之一，导致模型生成视觉不一致但语言合理的内容。这些先验源自预训练大型语言模型（LLM）的固有特性，与视觉输入无关。

Method: CICD通过识别关键和有害先验，利用对比解码消除有害先验，同时保持文本流畅性和一致性。图像间有限的信息重叠避免了视觉信息损失。

Result: 在四个基准测试和六种LVLM上验证了CICD的有效性，尤其在图像描述任务中表现突出。

Conclusion: CICD是一种简单有效的方法，显著缓解了语言先验问题，同时保持了模型的文本生成能力。
...
TL;DR: 论文提出两种改进持续学习（CL）在注意力多实例学习（MIL）模型中的方法：注意力知识蒸馏（AKD）和伪袋记忆池（PMP），显著提高了模型在WSI数据集上的准确性和内存效率。


<details>
  <summary>Details</summary>
Motivation: 传统MIL模型缺乏对动态数据集的适应性，持续学习在MIL中效果有限，研究发现遗忘主要发生在注意力层。

Method: 提出AKD和PMP：AKD通过保留注意力层知识减少遗忘，PMP通过选择性存储信息丰富的伪袋降低内存占用。

Result: 实验表明，该方法在多样WSI数据集上显著优于现有CL方法，提高了准确性和内存效率。

Conclusion: 为大规模弱标注临床数据集中的持续学习奠定了基础，推动了更具适应性和鲁棒性的诊断模型发展。
...
TL;DR: 研究探讨了CLIP嵌入是否包含AI生成图像的信息，提出了一种轻量级分类方法，在CIFAKE基准测试中达到95%准确率。
...
Result: 在CIFAKE基准测试中达到95%准确率；少量数据微调后性能为85%。某些图像类型（如广角照片和油画）分类困难。

Conclusion: 揭示了AI生成图像分类中的新挑战，值得进一步研究特定图像类型的分类问题。
...
TL;DR: 提出了一种新的3D视线估计方法GA3CE，通过3D上下文编码学习主体与场景的空间关系，显著提升了无约束场景下的3D视线方向估计。


<details>
  <summary>Details</summary>
Motivation: 解决在无约束场景（如远距离或主体背对）中，因缺乏眼部近景而难以估计3D视线方向的问题。

Method: 使用3D姿态和物体位置表示主体与场景，提出GA3CE方法，结合D$^3$位置编码，在3D空间中学习空间关系。

Result: 在单帧设置下，相比基线方法，平均角度误差减少了13%-37%。

Conclusion: GA3CE通过3D上下文编码和D$^3$位置编码，显著提升了3D视线估计的准确性。
...
TL;DR: 论文提出稀疏时空图卷积网络（ST-GCNs）用于骨架动作识别，发现现有ST-GCNs存在过参数化问题，并提出一种稀疏生成器，能在减少参数的同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 现有ST-GCNs在骨架动作识别中性能差异不大，表明其可能过参数化，需探索稀疏化以提升效率。

Method: 提出稀疏ST-GCNs生成器，从随机初始化的密集网络中训练稀疏架构，并集成多级稀疏结构。

Result: 稀疏ST-GCNs在减少95%参数时性能下降<1%，多级稀疏ST-GCNs仅需66%参数且性能提升>1%。

Conclusion: 稀疏化能显著减少ST-GCNs参数并保持或提升性能，为高效骨架动作识别提供新思路。
...
Method: 提出voxel-to-Gaussian初始化策略，利用LiDAR数据提供几何先验，并通过LiDAR引导的3D可变形注意力机制优化3D高斯。

Result: 在道路和非道路数据集上实验表明，GaussianFormer3D在预测精度上与现有多模态融合方法相当，同时降低了内存消耗并提高了效率。
...
TL;DR: 论文评估了通用鸟类检测模型BirdDetector在零样本和微调设置下对Salvin信天翁繁殖种群监测的效果，发现微调显著提升检测精度。
...
Method: 使用无人机图像评估BirdDetector模型，结合零样本和微调设置，采用增强推理和图像增强技术。

Result: 零样本提供基线，微调显著提高检测精度。

Conclusion: 预训练深度学习模型在物种特异性监测中具有潜力。
...
TL;DR: 提出了一种基于LoRA的两阶段方法，用于在Stable Diffusion XL中个性化生成图像，避免灾难性遗忘和过拟合。
...
Conclusion: 该方法成功分离主题编码与整体构图，保留了模型的生成能力并实现高保真个性化。
...
TL;DR: 本文提出了一种生成森林环境语义分割点云的新方法，通过模拟器和改进的SfM算法，解决了公开标注数据集稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 由于高成本、传感器需求和采集耗时，公开的森林点云数据集稀缺，且缺乏基于SfM算法的语义分割数据集。

Method: 使用自定义森林模拟器生成带语义标签的RGB图像，并通过改进的SfM软件保留语义信息进行3D重建。

Result: 生成的点云包含几何和语义细节，可用于训练和评估深度学习模型。

Conclusion: 该方法为森林点云语义分割提供了有价值的资源，填补了现有数据集的空白。
...
<details>
  <summary>Details</summary>
Motivation: 微创手术（MIS）存在视觉和技术挑战，现有机器学习方法依赖小规模标注数据，而通用VLMs的潜力尚未在手术领域充分验证。

Method: 通过在多类手术数据集（如腹腔镜和内镜黏膜下剥离术）上测试多种VLMs，评估其性能。

Result: 基准测试显示VLMs在手术场景中难以准确关联语言与视觉区域。

Conclusion: 通用VLMs在手术领域表现有限，需进一步改进以适应复杂手术任务。
...
TL;DR: 论文提出uLLSAM方法，利用多模态大语言模型（MLLMs）为SAM注入视觉语言知识（VLK），提升其在显微图像跨域数据上的分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有生物医学图像分割模型在未见域数据上表现不佳，缺乏视觉语言知识是主要原因。

Method: 提出VLSA模块注入VLK，并设计SBR优化边界感知，提升SAM的泛化能力。

Result: 在9个域内和10个跨域显微数据集上，Dice和SA指标分别提升7.71%/12.10%和6.79%/10.08%。

Conclusion: uLLSAM通过结合MLLMs和SAM，显著提升了显微图像分割的泛化性能。
...
TL;DR: 提出了一种完全弱监督的类增量学习语义分割方法，仅需图像级标签即可训练。


<details>
  <summary>Details</summary>
Motivation: 传统类增量语义分割方法需要昂贵的像素级标注，而现有部分弱监督方法仍有局限，因此提出完全弱监督方法以降低成本。

Method: 通过结合定位器和基础模型的伪标签生成鲁棒伪标签，并引入范例引导的数据增强以减少灾难性遗忘。
...
TL;DR: SynRailObs是一个高保真合成数据集，用于铁路环境中障碍物检测，弥补现有数据集的不足，支持多种天气和地理条件。


<details>
  <summary>Details</summary>
Motivation: 现有公开数据集无法满足铁路安全研究中高质量、多样化障碍物检测的需求。

Method: 使用扩散模型生成难以获取的障碍物，创建SynRailObs数据集，并在真实铁路环境中测试其效果。

Result: 实验表明，基于SynRailObs训练的模型在不同距离和环境条件下表现一致，并具备零样本能力。

Conclusion: SynRailOps在铁路安全应用中具有显著潜力，可推动障碍物检测技术的发展。
...
TL;DR: EA-3DGS是一种针对户外场景的高质量实时渲染方法，通过引入网格结构和高效高斯剪枝策略，解决了3D高斯泼溅（3DGS）在户外场景中的内存和调整问题。


<details>
  <summary>Details</summary>
Motivation: 当前NeRF方法在建筑规模场景重建中表现优异，但训练和推理速度慢；3DGS在物体和小规模场景中表现良好，但在户外场景中缺乏有效调整机制且内存占用高。

Method: 1. 使用自适应四面体网格初始化高斯组件；2. 提出高效高斯剪枝策略；3. 结构感知的密集化策略；4. 参数量化减少存储需求。

Result: 在13个场景（包括公共数据集和自采集场景）上验证了方法的优越性。

Conclusion: EA-3DGS在户外场景中实现了高质量实时渲染，解决了内存和调整问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的文本编码器因训练数据限制，难以捕捉运动的时间和运动学结构，影响了运动生成的质量。

Method: MoCLIP在CLIP基础上增加了运动编码头，采用对比学习和tethering损失训练，以增强运动感知表示。

Result: 实验表明，MoCLIP提高了Top-1、Top-2和Top-3准确率，同时保持竞争力的FID，改进了文本到运动的对齐效果。

Conclusion: MoCLIP是一个多功能且有效的框架，显著提升了运动生成的质量和兼容性。
...
TL;DR: 该研究评估了通用和医学专用基础模型嵌入在放射学多分类任务中的表现，发现MedImageInsight嵌入与支持向量机适配器组合效果最佳，同时具备高效性和公平性。
...
Result: MedImageInsight嵌入+SVM适配器表现最佳（mAUC 93.8%），且计算高效、公平性良好。

Conclusion: 基础模型嵌入（尤其是MedImageInsight）为放射学图像分析提供了高效、准确且公平的解决方案。
...
TL;DR: 提出了一种基于集中特征调节的新型热红外目标检测框架CRT-YOLO，通过高效多尺度注意力模块和集中特征金字塔网络，显著提升了检测性能。
...
Method: 结合高效多尺度注意力模块和集中特征金字塔网络，实现全局范围的热红外信息交互。

Result: 在两个基准数据集上的实验表明，CRT-YOLO显著优于传统方法。

Conclusion: CRT-YOLO框架在热红外目标检测领域具有重要潜力，模块有效性通过消融研究得到验证。
...
TL;DR: NeuSEditor是一种用于从多视角图像中编辑神经隐式曲面的新方法，通过身份保留架构和几何感知蒸馏损失，显著提升了编辑效果。


<details>
  <summary>Details</summary>
Motivation: 隐式曲面表示虽紧凑连续，但编辑时难以保持身份和几何一致性。现有方法在此方面表现不足。

Method: NeuSEditor采用身份保留架构分离前景与背景，并引入几何感知蒸馏损失，无需持续更新数据集或源提示。

Result: NeuSEditor在定量和定性上优于PDS和InstructNeRF2NeRF等现有方法。

Conclusion: NeuSEditor提供了一种高效、高质量的隐式曲面编辑解决方案。
...
TL;DR: RefPose是一种利用参考图像和几何对应关系进行6D姿态估计的创新方法，适用于未见过的物体。
...
Method: 通过参考图像和几何对应关系预测初始姿态，并通过渲染-比较方法迭代优化姿态，引入相关性体积引导注意力机制。

Result: 在BOP基准数据集上达到最先进性能，同时保持竞争力运行时。
...
TL;DR: 本文提出了一种基于LTI系统的步态对称性评估方法，通过建模节段间协调性并引入差异度量，与传统EMG或加速度方法不同。


<details>
  <summary>Details</summary>
Motivation: 传统步态对称性评估依赖EMG信号或加速度差异，存在局限性，需要更高效的方法。

Method: 使用LTI系统建模节段间协调性，提出差异度量评估对称性，并在五名受试者中测试。

Result: 方法在对称和非对称步态中均有效，验证了其可行性。

Conclusion: 提出的LTI系统方法为步态对称性评估提供了新视角，优于传统技术。
...
TL;DR: 提出了一种基于空间增强多模态大语言模型的方法，帮助视觉障碍人士更好地理解和导航环境，结合硬件设备提升实用性。


<details>
  <summary>Details</summary>
Motivation: 视觉障碍人士在导航和物体定位方面面临挑战，现有MLLM模型缺乏空间推理能力，且缺少轻量易用的系统。

Method: 通过微调MLLM以增强空间推理能力，结合硬件眼镜附件，利用VLM实时提供空间感知反馈。

Result: 在VizWiz数据集上评估，显示准确性和用户体验显著提升。

Conclusion: 该方法填补了先进机器学习模型与实用辅助设备之间的空白，为视觉障碍人士提供了更有效的独立导航工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法多局限于单一数据集，而实际应用需适应多样化条件，因此需统一评估标准。

Method: 构建PoseBench3D框架，重新评估18种方法在4个常用数据集上的表现，使用MPJPE和PA-MPJPE指标。

Result: 生成100多项跨数据集评估结果，分析预处理技术和数据集参数对性能的影响。

Conclusion: PoseBench3D为未来研究提供统一评估平台，提升模型泛化能力分析。
...
TL;DR: 研究提出了一种基于4D-CTA的动态心脏模型框架，结合数字孪生技术，为冠状动脉介入提供精确的个性化工具。
...
Method: 利用4D-CTA数据构建动态模型，制造透明血管物理模型，并开发虚拟血管造影系统。

Result: 虚拟与真实血管造影形态一致性达80.9%，导丝运动轨迹误差低于1.1毫米，透明模型在CABG训练中表现优异。

Conclusion: 患者特异性数字-物理孪生方法有效再现冠状动脉的解剖结构和动态特性，为教育和临床规划提供动态环境。
...
TL;DR: 论文提出了一种名为VISTA的新方法，通过信息论分析揭示了现有MLLMs中交叉熵损失的隐式对齐目标的局限性，并提出显式对齐目标以提升跨模态信息融合效果。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型（MLLMs）在模态对齐上存在偏向文本信息的问题，限制了视觉等其他模态的有效融合。

Method: 通过信息论分析交叉熵损失的隐式对齐目标，提出VISTA方法，引入显式对齐目标以最大化跨模态互信息。

Result: VISTA在多个基准数据集（如VQAv2、MMStar、MME）上显著优于基线模型，且无需额外训练模块或数据。

Conclusion: VISTA为MLLMs的模态对齐研究提供了新方向，显著提升了视觉理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有跨模态检索研究缺乏专门针对中国文化遗产的数据集，限制了该领域模型的开发与评估。

Method: 提出CulTi数据集（5,726对图像-文本），并基于中文CLIP提出LACLIP方法，通过加权相似度计算增强局部对齐。

Result: 实验表明，LACLIP在CulTi数据集上显著优于现有模型，尤其在细粒度语义关联方面表现突出。

Conclusion: CulTi数据集和LACLIP方法为中国文化遗产的跨模态检索提供了有效工具，填补了领域空白。
...
TL;DR: 论文提出首个光学-SAR融合目标检测数据集M4-SAR，并开发了端到端融合检测框架E2E-OSDet，显著提升复杂环境下的检测性能。


<details>
  <summary>Details</summary>
Motivation: 光学和SAR图像各有优缺点，融合二者可提升检测精度，但缺乏标准化数据集阻碍了研究进展。

Method: 构建M4-SAR数据集，开发统一评测工具包，并提出E2E-OSDet框架以解决跨域差异。

Result: 实验表明融合光学和SAR数据可将mAP提升5.7%，尤其在复杂环境中效果显著。

Conclusion: M4-SAR和E2E-OSDet为光学-SAR融合目标检测提供了标准化基准和高效解决方案。
...
TL;DR: M2AD是一个新的大规模视觉异常检测（VAD）基准测试，旨在评估VAD方法在视角和光照交互条件下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有VAD系统对真实世界成像变化（尤其是视角和光照的复杂交互）敏感，但当前基准测试未充分关注这一问题。

Method: 通过系统捕获10类999个样本的12种视角和10种光照设置（共120种配置），构建包含119,880张高分辨率图像的M2AD数据集。

Result: 实验表明，当前最先进的VAD方法在M2AD上表现不佳，凸显了视角-光照交互的挑战性。

Conclusion: M2AD为开发和验证能够应对真实世界复杂性的VAD方法提供了重要工具，数据集和测试套件将公开。
...
TL;DR: 本文提出了一种名为“自条件”的机制，通过利用去噪网络的丰富语义来指导其解码层，从而提升扩散模型的生成和识别能力。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像合成中表现出色，但其生成预训练是否能提升模型自身的训练效果，以及特征质量是否能超越自监督学习模型，是本文探讨的核心问题。

Method: 引入自条件机制，利用去噪网络的内部语义指导解码层，形成更紧密的瓶颈以提升生成质量。

Result: 实验表明，该方法在生成FID和识别准确率上均有显著提升，计算开销仅增加1%，且适用于多种扩散架构。
...
TL;DR: ForensicHub是首个统一的多领域假图像检测与定位基准和代码库，旨在打破领域壁垒，促进跨领域比较和研究发展。


<details>
  <summary>Details</summary>
Motivation: FIDL领域高度分散，缺乏统一基准，导致领域间数据、模型和评估协议互不兼容，阻碍了跨领域比较和整体发展。

Method: 提出模块化、配置驱动的架构，分解法医流程为可互换组件；实现10个基线模型、6个主干网络、2个新基准，并集成现有基准。

Result: 通过ForensicHub深入分析，提供了8个关键见解，涵盖模型架构、数据集特性和评估标准。

Conclusion: ForensicHub显著推动了FIDL领域的统一，为未来突破奠定了基础。
...
TL;DR: MoMADiff结合掩码建模与扩散过程，通过帧级连续表示生成3D人体运动，支持用户提供关键帧，提升运动质量和控制精度。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理分布外运动时表现不佳，VQVAE方法难以忠实表示新运动，扩散方法缺乏对单帧的精细控制。

Method: 结合掩码建模与扩散过程，使用帧级连续表示生成运动，支持用户指定关键帧。

Result: 在运动质量、指令忠实度和关键帧遵循方面优于现有方法。

Conclusion: MoMADiff在生成3D人体运动时表现出色，具有强泛化能力和精确控制。
...
TL;DR: WildDoc是一个专为自然环境中文档理解评估设计的新基准，填补了现有基准（如DocVQA和ChartQA）在真实场景多样性上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要针对扫描或数字文档，未能充分反映真实世界中光照变化和物理变形等复杂挑战。

Method: WildDoc通过手动捕获多样化真实环境中的文档图像，并利用现有基准的文档源进行对比，每份文档在四种不同条件下拍摄以评估模型鲁棒性。

Result: 评估显示，当前最先进的多模态大语言模型在WildDoc上表现显著下降，鲁棒性不足。

Conclusion: WildDoc突显了真实世界文档理解的独特挑战，为未来研究提供了重要基准。
...
TL;DR: 论文提出了一种双师生学习框架（DTSL），通过结合时间滞后模型和跨架构模型的输出一致性，改进了半监督医学图像分割的性能。


<details>
  <summary>Details</summary>
Motivation: 减少医学图像分割中的手动标注成本，同时提高分割性能。

Method: 采用双师生学习框架（DTSL），结合时间滞后模型和跨架构模型的输出一致性，生成伪标签。

Result: 在多个数据集上表现优于现有方法，消融实验验证了模块的有效性。

Conclusion: DTSL框架通过灵活调节学习节奏和利用跨架构模型，显著提升了半监督医学图像分割的性能。
...
TL;DR: 本文提出了一种多模态多视角分类模型，用于基于音频和视觉特征对菠萝进行四种质量等级的分类，以减少浪费并提高收入。


<details>
  <summary>Details</summary>
Motivation: 通过非破坏性方法确定菠萝的保质期质量，以减少浪费并增加收入。

Method: 构建多模态多视角分类模型，利用PQC500数据集（包含500个菠萝的音频和视觉数据），改进对比性视听掩码自编码器，并采用音频主导的采样方法进行高效训练。

Result: 提出的跨模态模型在音频主导采样下达到84%的准确率，分别比仅音频和仅视觉的单模态模型高6%和18%。

Conclusion: 跨模态模型在菠萝质量分类中表现优于单模态模型，验证了多模态方法的有效性。
...
TL;DR: CleanPatrick是首个针对图像数据清理的大规模基准测试，基于Fitzpatrick17k皮肤病数据集，通过众包标注和专家审核生成高质量数据，并评估多种清理方法的性能。


<details>
  <summary>Details</summary>
Motivation: 当前图像数据清理基准依赖合成噪声或有限人工研究，缺乏真实性和可比性。CleanPatrick旨在填补这一空白，推动数据为中心的AI发展。

Method: 收集496,377条众包标注，识别离题样本、近似重复和标签错误，结合项目反应理论模型和专家审核生成高质量数据，并作为排名任务评估多种清理方法。

Result: 自监督表示在近似重复检测中表现优异，经典方法在有限预算下能有效检测离题样本，但细粒度医学分类的标签错误检测仍具挑战性。

Conclusion: CleanPatrick为图像清理策略提供了系统比较平台，推动了更可靠的数据中心AI发展。
...
<details>
  <summary>Details</summary>
Motivation: 街景数据在计算机视觉应用中广泛使用，但现有数据集假设为系统性覆盖，而实际存在覆盖不均的问题，需揭示其偏差来源。

Method: 定量分析28个城市街景数据的覆盖偏差，提出评估方法，并通过阿姆斯特丹案例的半结构化访谈研究收集过程的影响。

Result: 发现城市特性（如布局）导致街景数据偏差，即使密集采样也无法避免。案例研究揭示了数据收集过程对代表性的具体影响。

Conclusion: 城市特性和数据收集过程是街景数据偏差的主要来源，需针对性评估和改进以减少偏差。
...
TL;DR: CUBIC是一种新方法，通过自动发现可解释的概念来识别深度视觉模型中的偏见，无需依赖预定义的偏见候选或模型失败示例。


<details>
  <summary>Details</summary>
Motivation: 深度视觉模型常依赖数据集中的虚假相关性学习偏见，而现有方法需要大量人工标注。CUBIC旨在解决这一问题。

Method: CUBIC利用图像-文本潜在空间和线性分类器探针，通过分析概念对超类标签潜在表示的影响来识别偏见。

Result: 实验表明，CUBIC能有效发现未知偏见，且无需模型失败样本或先验偏见知识。

Conclusion: CUBIC为无监督偏见识别提供了一种高效且可扩展的方法。
...
TL;DR: HSRMamba改进Visual Mamba，通过条带扫描和小波分解提升超分辨率性能，减少计算负担和模型尺寸。


<details>
  <summary>Details</summary>
Motivation: 解决Visual Mamba因1D扫描导致的图像生成伪影问题，并优化高频空间与低频光谱特征的模态冲突。

Method: 引入条带扫描方案减少全局单向扫描的伪影，结合小波分解优化特征模态冲突。

Result: 在计算效率和模型尺寸上表现优异，超分辨率性能超越现有方法。

Conclusion: HSRMamba在性能和效率上均达到最优，成为单高光谱图像超分辨率的新标杆。
...
TL;DR: 本文提出Group Preference Optimization (GPO)，通过扩展DPO为组偏好优化并引入奖励标准化，解决了T2I扩散模型中DPO对偏好对敏感和数据收集困难的问题。


<details>
  <summary>Details</summary>
Motivation: DPO在T2I扩散模型中对偏好对敏感且数据收集困难，限制了其性能提升。

Method: 提出GPO方法，将DPO从两两比较扩展为组比较，并引入奖励标准化进行重加权。

Result: GPO显著提升了Stable Diffusion 3.5 Medium的计数和文本渲染能力20个百分点，且无需额外推理开销。
...
TL;DR: 论文提出了一种名为PL-DC的新框架，用于解决半监督实例分割中伪标签质量不稳定的问题，通过解耦类别和掩码质量评估、动态修正类别标签以及像素级掩码不确定性加权，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 半监督实例分割中，伪标签的噪声问题导致性能不稳定，现有方法在类别和掩码质量评估上存在妥协。

Method: 提出PL-DC框架，包括解耦双阈值过滤机制、动态类别修正模块和像素级掩码不确定性加权机制。

Result: 在COCO和Cityscapes数据集上取得显著性能提升，特别是在少量标注数据下表现优异。

Conclusion: PL-DC框架有效解决了伪标签质量问题，为半监督实例分割提供了新思路。
...
TL;DR: Hybrid-Emba3D提出了一种双向Mamba模型，通过几何特征耦合和跨路径特征混合，解决了点云分类任务中局部几何特征提取与模型复杂度的矛盾，实现了95.99%的分类准确率。
...
Method: Hybrid-Emba3D结合几何特征耦合机制（局部几何池化）和跨路径特征混合（协作特征增强器），动态聚合几何信息，提升局部特征判别力。

Result: 在ModelNet40上达到95.99%的分类准确率，仅增加0.03M参数。

Conclusion: Hybrid-Emba3D通过双向建模和特征混合，显著提升了点云分类性能，为SSM在点云任务中的应用提供了新思路。
...
TL;DR: 提出了首个大规模多语言音视频深度伪造检测基准数据集，包含8种语言的250小时真实与伪造视频，60%为生成数据。测试显示现有检测器在开放集场景下性能下降。


<details>
  <summary>Details</summary>
Motivation: 填补多语言开放集深度伪造检测的空白，评估现有检测器在复杂场景下的表现。

Method: 构建包含8种语言、7种生成模型的数据集，划分训练、验证和测试集以模拟开放集场景，测试多种预训练和微调检测器。

Result: 现有先进检测器在开放集场景中性能显著下降。

Conclusion: 多语言开放集深度伪造检测具有挑战性，需进一步研究提升检测器泛化能力。
...
TL;DR: 提出了一种基于可解释特征分析的GAN生成图像训练数据集识别框架，结合频谱变换、颜色分布和局部特征，分类准确率达98-99%。


<details>
  <summary>Details</summary>
Motivation: 解决GAN生成媒体的真实性和数据集溯源问题，应对版权、隐私和法律合规挑战。

Method: 整合频谱变换（傅里叶/DCT）、颜色分布指标和局部特征描述符（SIFT），提取统计特征，使用监督分类器进行分类。

Result: 在二元分类和多类数据集溯源中达到高准确率，频率域特征和颜色直方图揭示了GAN训练的特定模式。
...
<details>
  <summary>Details</summary>
Motivation: 遥感领域中的VLM预训练常因图像的多重冗余描述导致时间和资源浪费，需解决此问题以提高效率。

Method: 采用两种技术计算自适应重要性权重：基于BLEU分数的非参数唯一性和基于注意力机制的学习方法。

Result: 实验表明，WFA策略在文本到图像检索任务中实现了高效且有效的VLM预训练。

Conclusion: 提供了根据下游任务需求和资源限制选择适当技术的指南，并公开了代码。
...
TL;DR: PhiNet v2是一种基于Transformer的架构，处理时序视觉输入，无需强数据增强，性能接近最先进视觉基础模型。


<details>
  <summary>Details</summary>
Motivation: 当前自监督学习未充分利用生物视觉处理系统的见解，PhiNet v2旨在实现更接近人类认知的视觉处理。

Method: 采用变分推断从连续图像序列中学习鲁棒视觉表示，避免强数据增强。

Result: PhiNet v2在性能上接近最先进模型，且能处理时序输入。

Conclusion: PhiNet v2是迈向更生物合理的计算机视觉系统的重要一步。
...
TL;DR: 提出了一种基于文本-图像协同的概念擦除框架（Co-Erasing），通过结合视觉监督和文本提示，显著提升了擦除效果，同时减少了对其他良性概念的干扰。


<details>
  <summary>Details</summary>
Motivation: 当前的概念擦除方法依赖手工设计的文本提示，难以平衡擦除效果和对其他概念的影响。文本与图像模态之间的知识鸿沟是主要限制。

Method: Co-Erasing通过文本提示和对应不良图像的联合描述，结合负向引导降低目标概念的生成概率，并设计了文本引导的图像概念细化策略。

Result: 实验表明，Co-Erasing在擦除效果和可用性之间取得了更好的平衡，显著优于现有方法。

Conclusion: Co-Erasing通过文本-图像协同解决了模态间的知识鸿沟问题，为概念擦除提供了更高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型（LLMs）和多模态大语言模型（MLLMs）在推理任务上的表现是否与人类相当尚不明确，因此需要建立一个细粒度的评估基准。

Method: 收集了9,794个多模态问题，涵盖双语（中英文）和纯文本问题，包括视觉推理、定义判断、类比推理和逻辑判断四种类型，并记录人类成功率和易错选项。

Result: 实验发现当前MLLMs在多模态推理任务中的表现与人类存在显著差异。

Conclusion: 该基准为下一代模型的开发提供了重要参考。
...
TL;DR: 本文提出了一种解决手部接触估计中类别和空间不平衡问题的框架HACO，通过平衡接触采样和顶点级类别平衡损失，有效提升了密集手部接触估计的性能。


<details>
  <summary>Details</summary>
Motivation: 理解手部与世界的接触对人类交互至关重要，但目前密集手部接触估计的学习方法仍未被充分探索，尤其是数据中的类别和空间不平衡问题。

Method: 提出HACO框架，采用平衡接触采样解决类别不平衡问题，使用顶点级类别平衡（VCB）损失解决空间不平衡问题。

Result: 通过大规模手部接触数据，成功预测密集手部接触，避免了类别和空间不平衡问题的影响。

Conclusion: HACO框架为密集手部接触估计提供了一种有效的解决方案，代码将公开。
...
<details>
  <summary>Details</summary>
Motivation: 当前胸部疾病自动诊断方法多基于CNN，忽略了全局特征。自注意力机制在计算机视觉中表现优异，因此结合CNN和Transformer的优势，提出新模型。

Method: 基于DenseNet和Swin Transformer，采用集成深度学习方法结合两者优势，损失函数结合加权二元交叉熵和不对称损失。
...
TL;DR: 论文提出了CompAlign基准和CompQuest评估框架，用于评估和改进文本到图像（T2I）模型在组合场景生成中的表现，尤其是3D空间关系的准确性。实验表明现有模型在复杂空间关系任务上表现不佳，但通过提出的对齐框架可以显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有T2I模型在生成包含多对象、属性和空间关系的组合场景时表现不佳，亟需一个专门评估和改进此类任务的基准和方法。

Method: 提出CompAlign基准（包含900个复杂多主题生成提示）和CompQuest评估框架（通过分解提示为原子问题并利用MLLM提供细粒度反馈）。进一步提出基于反馈的对齐框架以改进扩散模型。

Result: 实验显示模型在复杂3D空间关系任务上表现较差，开源与商业模型存在性能差距。对齐框架显著提升了组合生成准确性。

Conclusion: CompAlign和CompQuest为组合图像生成提供了有效评估和改进工具，对齐框架显著提升了模型性能，尤其在复杂任务上。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在实例级和集群级一致性学习上存在局限性，且过度依赖一致性导致不可靠的插补和对齐。

Method: 提出FreeCSL框架，学习共识原型以构建共享空间，并设计启发式图聚类增强集群语义。

Result: 实验表明，FreeCSL在不完整多视图聚类任务中表现优于现有方法。

Conclusion: FreeCSL通过共识语义学习和集群结构恢复，实现了更稳健的聚类效果。
...
TL;DR: FALCON是一种自适应平衡硬负样本和假负样本的学习策略，用于提升视觉语言预训练（VLP）的效果。


<details>
  <summary>Details</summary>
Motivation: 假负样本在VLP中导致冲突的监督信号，影响嵌入空间和硬负采样的有效性。

Method: FALCON通过动态选择负样本的硬度，结合跨模态对齐改进的代理，优化小批量构建。

Result: 实验表明，FALCON显著提升了ALBEF和BLIP-2框架的性能，并在多种下游任务中表现优异。

Conclusion: FALCON有效缓解了假负样本的影响，提升了VLP的鲁棒性和性能。
...
TL;DR: 论文提出了一种基于卷积的高效扩散模型DiCo，通过引入紧凑通道注意力机制提升性能，在ImageNet基准测试中表现优于现有扩散模型。


<details>
  <summary>Details</summary>
Motivation: 现有Diffusion Transformer（DiT）计算开销大，且全局自注意力存在冗余，卷积可能成为更高效的替代方案。

Method: 提出Diffusion ConvNet（DiCo），用标准卷积模块构建扩散模型，并引入紧凑通道注意力机制以减少通道冗余。

Result: DiCo在ImageNet上表现优异，DiCo-XL在256x256和512x512分辨率下FID分别为2.05和2.53，速度提升2.7x和3.1x。

Conclusion: DiCo证明了卷积在扩散模型中的潜力，实现了高效且高性能的视觉生成。
...
TL;DR: 本文首次将测地距离引入多模态学习，通过图结构和最短路径算法计算样本间的测地距离，解决了传统距离度量在多模态学习中的局限性。


<details>
  <summary>Details</summary>
Motivation: 在多模态学习中，非线性流形广泛存在，传统距离度量难以区分语义不同的高相似样本。

Method: 构建图结构表示样本邻接关系，通过最短路径算法计算测地距离，并采用分层图结构和增量更新策略提高计算效率。

Result: 实验验证了该方法能有效捕捉样本间复杂关系，提升多模态学习模型性能。

Conclusion: 测地距离作为一种新型距离度量，在多模态学习中具有显著优势和应用潜力。
...
TL;DR: 提出了一种基于自适应图的事件相机噪声去除框架，显著提升事件相机在物体识别中的性能。


<details>
  <summary>Details</summary>
Motivation: 事件相机生成的数据存在大量冗余和噪声，如何在去除噪声的同时保留关键时空信息是主要挑战。

Method: 结合自适应事件分割、多因素边权重机制和自适应图去噪策略。

Result: 在四个数据集上识别准确率分别达到83.77%、76.79%、99.30%和96.89%，优于现有方法。
...
TL;DR: 论文提出了一种针对负偏好的训练方法，以改进扩散模型生成结果与人类偏好的一致性，无需新策略或数据集。


<details>
  <summary>Details</summary>
Motivation: 现有偏好对齐方法忽视了无条件/负条件输出的处理，限制了分类器自由引导（CFG）的效果。

Method: 训练一个专门针对负偏好的模型，仅需对现有技术进行微小修改。

Result: 方法兼容多种模型（如SD1.5、SDXL等），显著提升了生成结果与人类偏好的一致性。

Conclusion: 提出的方法简单有效，能无缝集成现有技术，提升模型输出质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像增强方法过于关注像素级信息，忽略了语义特征。

Method: 使用预训练的深度神经网络提取特征，结合GPU加速的NSGA-II算法优化多目标（图像熵、感知相似性、亮度），并通过局部搜索微调。
...
Method: 提出DRAGON数据集，涵盖多种模型和主题，并通过语言模型优化提示生成更真实的图像。

Result: 数据集支持多种研究场景，并附带测试集作为新方法的基准。

Conclusion: DRAGON为合成内容检测和溯源提供了全面且更新的资源。
...
Method: 结合在线几何先验生成几何感知特征，通过平面扫描投影候选深度假设，并聚合相似性构建成本体积。

Result: 在航空和卫星图像中表现出优于传统方法的泛化能力。

Conclusion: MV-DeepSimNets可集成到标准多分辨率图像匹配流程中，提升多视图重建效果。
...
TL;DR: FairHyp是一个公平性导向的框架，通过专门模块解决高光谱图像（HSI）的非均匀性问题，提升分类、去噪、超分辨率和修复等任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有模型假设维度同质性导致性能不佳，FairHyp旨在解决HSI中光谱依赖、空间连续性和特征效率的非均匀性问题。

Method: FairHyp包含Runge-Kutta启发的空间变适配器、多感受野卷积模块和光谱上下文状态空间模型，分别处理空间、特征和光谱非均匀性。

Result: 实验表明FairHyp在多种任务和成像条件下优于现有方法。

Conclusion: FairHyp将公平性重新定义为HSI建模的结构需求，为高维视觉任务提供了平衡适应性、效率和保真度的新范式。
...
TL;DR: MTevent数据集为高速机器人视觉提供了首个结合高速运动、长距离感知和真实物体交互的资源，填补了RGB相机在高动态环境中的局限性。


<details>
  <summary>Details</summary>
Motivation: 高速移动机器人面临RGB相机在动态环境中的性能限制，事件相机因其低延迟和异步特性成为潜在解决方案。

Method: 通过立体事件相机和RGB相机捕捉75个场景，每个场景平均16秒，包含16个独特物体，涵盖极端视角、光照变化和遮挡等挑战。

Result: 在6D姿态估计任务中，RGB方法的平均召回率仅为0.22，凸显了其在动态环境中的不足。

Conclusion: MTevent数据集为事件相机在机器人视觉中的研究提供了重要资源，推动了高速动态环境下的感知技术进步。
...
TL;DR: 提出了一种新的批次构建策略B3，通过预训练教师模型和社区检测算法优化对比学习中的负样本质量，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 对比学习的效果受批次大小和质量影响较大，现有方法需要大批次才能获得高质量负样本，限制了应用。

Method: 使用预训练教师模型对数据集中的样本进行排序，构建稀疏相似图，通过社区检测算法识别强负样本集群，并基于此构建批次。

Result: 在MMEB多模态嵌入基准测试中，B3方法在7B和2B模型规模上分别比之前最佳方法提升了1.3和2.9分，且仅需64的小批次即可超越现有方法。

Conclusion: B3通过优化批次构建策略，显著提升了对比学习的效率和性能，尤其是在小批次场景下表现突出。
...
TL;DR: 提出CROC框架，通过合成对比测试案例自动评估文本到图像生成任务的评价指标鲁棒性，并开发CROCScore新指标。
...
Method: 提出CROC框架，生成百万级对比提示-图像对数据集（CROC$^{syn}$），并训练CROCScore指标。

Result: 现有指标在否定提示和身体部位识别等任务中表现不佳，CROCScore达到开源方法最优。

Conclusion: CROC框架为评价指标鲁棒性提供了高效自动化解决方案，并展示了新指标的应用潜力。
...
TL;DR: 论文提出了一种新的基准任务TGLG，用于评估视觉语言模型在实时交互环境中的表现，并提出了VLM-TSI模型，显著优于基线，但整体性能仍有提升空间。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在离线任务中表现优异，但在实时交互环境中需要同时满足语义准确性和时间精确性，因此提出了TGLG任务和VLM-TSI模型。

Method: 提出了TGLG任务，要求模型根据流式视频生成语义和时间对齐的语句；设计了TRACE指标评估语义相似性和时间对齐；开发了VLM-TSI模型，通过时间同步交错视觉和语言令牌实现实时生成。

Result: VLM-TSI显著优于基线模型，但整体性能仍有改进空间，表明TGLG任务的挑战性。

Conclusion: TGLG任务和VLM-TSI模型为实时视觉语言模型研究提供了新方向，未来需进一步探索提升性能的方法。
...
TL;DR: 该论文提出了一种名为MARRS的新框架，用于生成协调且细粒度的人类反应动作，解决了现有方法在信息损失和代码利用率低方面的问题。


<details>
  <summary>Details</summary>
Motivation: 当前的自回归建模方法在运动生成任务中表现优异，但在人类动作-反应合成任务中存在量化信息损失和代码利用率低的问题，同时需要处理细粒度的手部动作。

Method: 论文提出了MARRS框架，包括UD-VAE（独立编码身体和手部单元）、ACF（通过随机掩码提取信息）、AUM（促进单元间交互）以及扩散模型（使用MLP预测噪声）。

Result: 定量和定性结果表明，该方法在生成协调且细粒度的反应动作方面表现优异。

Conclusion: MARRS框架在人类动作-反应合成任务中取得了显著进展，代码将在论文接受后发布。
...
TL;DR: 论文提出动态基础模型转移（DBMS）方法，通过调整基础模型和压缩参数，显著提升高压缩率下的性能表现。


<details>
  <summary>Details</summary>
Motivation: 现有方法在极高压缩率下性能下降严重，且预训练基础模型并非最优选择。

Method: 动态调整基础模型和压缩参数，优化压缩性能。

Result: DBMS在极高压缩率下仍能保持微调模型性能，优于现有方法。

Conclusion: DBMS方法高效且通用，适用于多种模型类型。
...
### [134] [EmotionHallucer: Evaluating Emotion Hallucinations in Multimodal Large Language Models](https://arxiv.org/abs/2505.11405)
*Bohao Xing,Xin Liu,Guoying Zhao,Chengyu Liu,Xiaolan Fu,Heikki Kälviäinen*

Main category: cs.CV

TL;DR: 论文提出了首个用于检测和分析多模态大语言模型（MLLMs）中情绪幻觉的基准EmotionHallucer，并揭示了当前模型在此问题上的表现差异。


<details>
  <summary>Details</summary>
Motivation: 情绪理解是重要但具有挑战性的任务，而MLLMs常因幻觉生成无关或荒谬内容。目前缺乏专门评估情绪幻觉的研究。

Method: 基于情绪心理学知识和多模态感知，通过对抗性二元问答框架评估38个模型。

Result: 研究发现大多数模型存在情绪幻觉问题，闭源模型表现优于开源模型，且推理能力带来额外优势。

Conclusion: 论文提出了PEP-MEK框架，平均提升9.90%的检测性能，为未来研究提供了资源。
...
### [135] [Dynam3D: Dynamic Layered 3D Tokens Empower VLM for Vision-and-Language Navigation](https://arxiv.org/abs/2505.11383)
*Zihan Wang,Seungjun Lee,Gim Hee Lee*

Main category: cs.CV

TL;DR: Dynam3D模型通过动态分层的3D表示解决了视频-语言大模型在3D导航中的几何理解、大规模探索和环境适应性不足的问题，并在多个VLN基准测试中取得了最佳性能。


<details>
  <summary>Details</summary>
Motivation: 视频-语言大模型在3D导航中存在对3D几何和空间语义理解不足、大规模探索能力有限以及对动态环境适应性差的问题。

Method: 提出Dynam3D模型，将2D CLIP特征投影到3D空间，构建多层次的3D表示，并采用动态分层更新策略。

Result: 在R2R-CE、REVERIE-CE和NavRAG-CE等VLN基准测试中取得了最佳性能，并通过实验验证了其在实际部署中的有效性。

Conclusion: Dynam3D通过动态分层的3D表示和语言对齐的预训练，显著提升了3D导航任务的性能，并展示了实际应用的潜力。
...
### [136] [MutualNeRF: Improve the Performance of NeRF under Limited Samples with Mutual Information Theory](https://arxiv.org/abs/2505.11386)
*Zifan Wang,Jingwei Li,Yitang Li,Yunze Liu*

Main category: cs.CV

TL;DR: MutualNeRF利用互信息理论提升NeRF在有限样本下的性能，通过统一衡量图像相关性，优化稀疏视角采样和少样本视图合成。
...
Method: 引入互信息作为统一度量，优化视角选择和视图合成，采用贪心算法和正则化项。
...
### [137] [Patho-R1: A Multimodal Reinforcement Learning-Based Pathology Expert Reasoner](https://arxiv.org/abs/2505.11404)
...
<details>
  <summary>Details</summary>
Motivation: 当前病理学视觉语言模型在诊断准确性和推理合理性上存在不足，主要由于数据集缺乏深度和结构化诊断范例。

Method: 利用病理学教科书和专家构建高质量数据集，采用三阶段训练：预训练、监督微调和强化学习。

Result: Patho-R1和PathoCLIP在多项病理学任务中表现优异，包括零样本分类和视觉问答。

Conclusion: Patho-R1通过高质量数据集和多阶段训练显著提升了病理学推理能力。
...
Method: 采用YOLOv8为基础的分割模型，对面部图像进行皱纹检测与分割。

Result: 展示了模型在皱纹检测与分割任务中的性能表现。

Conclusion: YOLOv8模型在面部皱纹检测与分割中表现出潜力。
...
TL;DR: 论文提出Face Consistency Benchmark（FCB），用于评估AI生成视频中角色一致性的框架，填补现有解决方案的不足。


<details>
  <summary>Details</summary>
Motivation: AI视频生成技术虽进步显著，但角色一致性仍是挑战，现有模型难以保持外观和属性的连贯性。

Method: 引入FCB框架，提供标准化指标评估和比较AI生成视频中角色的一致性。
...
TL;DR: 本文提出了一种基于零样本RGB-D模型的6自由度姿态估计方法，用于机器人辅助微创手术中的手术工具，通过改进SAM-6D和深度估计方法，显著提升了在未见工具上的姿态估计性能。


<details>
  <summary>Details</summary>
Motivation: 传统标记方法和监督学习方法在机器人辅助微创手术中存在遮挡、反射和泛化性不足的问题，零样本方法尚未被探索。

Method: 结合零样本RGB-D模型（FoundationPose和SAM-6D），改进深度估计（RAFT-Stereo）和分割模块（Mask R-CNN替代SAM），提升姿态估计的鲁棒性和准确性。

Result: 改进后的SAM-6D在零样本姿态估计中优于FoundationPose，为RMIS中的RGB-D零样本方法设定了新基准。

Conclusion: 该方法提升了未见手术工具的姿态估计泛化性，并首次将RGB-D零样本方法应用于RMIS。
...
TL;DR: HumaniBench是一个专注于人类中心AI原则的基准测试，包含32K真实世界图像问题对，评估7项HCAI原则，揭示了当前LMM在公平性、伦理等方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前大型多模态模型（LMMs）在视觉语言任务上表现优异，但在公平性、伦理等人类中心标准上仍有不足，需引入更全面的评估工具。

Method: 通过GPT4辅助的标注流程构建HumaniBench，涵盖7项HCAI原则和7种任务，评估15种先进LMMs。

Result: 专有模型表现较好，但鲁棒性和视觉定位仍是弱点，开源模型在准确性与人类对齐原则间存在平衡问题。

Conclusion: HumaniBench为诊断LMMs对齐差距提供了严格测试平台，推动模型行为更准确且社会责任。
...
<details>
  <summary>Details</summary>
Motivation: 现有多层生成方法无法处理层间交互（如全局布局、物理接触和视觉效果），同时保持高质量。

Method: 提出PSDiffusion，通过全局层交互机制，单次前馈过程生成多层图像（RGB背景和多个RGBA前景）。

Result: 模型能同时生成高质量的多层图像，确保层间空间和视觉交互的全局一致性。

Conclusion: PSDiffusion为多层图像生成提供了高效且一致的解决方案。
...
TL;DR: 提出了一种无监督度量方法，用于仅通过间接测量和扩散模型的评分函数估计分布偏移，无需干净测试图像。


<details>
  <summary>Details</summary>
Motivation: 解决成像逆问题中因训练和测试图像分布偏移导致的性能下降问题，且无需干净测试图像。

Method: 利用扩散模型的评分函数和间接测量，提出一种无监督度量方法，估计KL散度。

Result: 该度量方法仅通过损坏测量即可近似干净图像的KL散度，并通过对齐分布外评分改善重建质量。

Conclusion: 该方法在多种逆问题中有效减少KL散度并提升重建质量。
...
TL;DR: QVGen是一种专为视频扩散模型设计的量化感知训练框架，能够在极低比特量化（如4位或更低）下实现高性能和高效推理。


<details>
  <summary>Details</summary>
Motivation: 视频扩散模型的计算和内存需求高，传统量化方法直接应用于视频扩散模型效果不佳，因此需要一种新的量化框架。

Method: QVGen通过引入辅助模块（Φ）减少量化误差，并采用秩衰减策略逐步消除Φ，结合SVD和基于秩的正则化γ来保留性能。

Result: QVGen在4位量化下首次达到与全精度相当的质量，并在3位量化下显著优于现有方法。

Conclusion: QVGen为视频扩散模型的量化提供了一种高效且高性能的解决方案。
...
TL;DR: 论文提出了一种评估工程通用人工智能（eAGI）的框架，基于Bloom分类法，适用于工程设计领域。


<details>
  <summary>Details</summary>
Motivation: 评估eAGI代理的性能是开发eAGI的关键挑战，需要一种专门化的评估框架。

Method: 提出了一种可扩展的评估框架，基于Bloom分类法，适用于工程设计，支持文本和结构化设计工件的评估。

Result: 开发了丰富的评估问题分类法，支持自动化定制评估基准。

Conclusion: 该框架为eAGI代理的评估提供了先进工具，推动了AI代理在工程设计中的发展。
...
TL;DR: 论文研究了如何通过特征引导方法提高基于大语言模型（LLM）的自主代理在博弈论环境中的可靠性，降低了背叛概率，并探讨了其在现实应用中的潜力。


<details>
  <summary>Details</summary>
Motivation: 由于LLM的不可预测性，其驱动的自主代理在需要负责任行为的领域中存在安全隐患，因此需要探索提高其可靠性的方法。

Method: 在迭代囚徒困境的变体中，通过稀疏自编码器提取可解释特征，并独立于游戏和提示引导残差流，以修改代理行为。

Result: 使用良好谈判特征引导后，平均背叛概率降低了28个百分点，并确定了多个开源LLM代理的可行引导范围。

Conclusion: 博弈论评估与表示引导对齐方法可推广到现实应用，如终端设备和嵌入式平台。
...
TL;DR: 论文探讨了具身AI与GOFAI的关系，指出当前AI驱动的机器人仅弱具身化，并讨论了跨具身学习的可能性与挑战。


<details>
  <summary>Details</summary>
Motivation: 将具身AI的研究置于GOFAI和行为基础的替代方案的背景下，探讨其局限性和发展方向。

Method: 通过文献回顾和批判性讨论，分析具身AI的弱具身化问题及跨具身学习的可行性。

Result: 识别了跨具身学习的基本障碍，并提出了未来发展的方向。

Conclusion: 当前AI驱动的机器人具身化程度有限，需进一步研究以克服跨具身学习的挑战。
...
TL;DR: 该论文提出了一个评估框架，用于分解现实任务为子任务，跟踪LLM性能和用户策略，并开发了一套新指标，揭示了LLM内容整合的利弊。


<details>
  <summary>Details</summary>
Motivation: 现有AI基准测试未能捕捉人机协作的多轮复杂性，需更全面的评估方法。

Method: 开发了包含语义相似性、结构连贯性等指标的框架，并在金融估值任务中验证。

Result: LLM内容整合提升输出质量，但受响应不连贯、子任务多样性和信息与用户知识差距的制约。

Conclusion: 论文为人机协作提供了更全面的评估方法和实用见解，强调需平衡新颖性与任务性能。
...
TL;DR: 提出了一种基于迭代程序化规划（IPP）的框架，通过使用大型语言模型（LLM）生成可解释的代码策略来解决网格任务。该方法通过代码生成而非传统搜索或强化学习来合成策略，并结合多种提示策略和迭代优化机制。实验表明，IPP在多个模型和任务上显著优于直接代码生成，并实现了新的最先进结果。


<details>
  <summary>Details</summary>
Motivation: 传统方法如搜索或强化学习在网格任务中效率较低，且生成的策略难以解释。通过LLM生成代码策略，既能提高性能，又能保持策略的可解释性。

Method: 使用LLM生成可执行程序作为策略，结合直接代码生成、伪代码条件优化和课程提示等策略，并通过迭代反馈机制优化代码。

Result: 在六个LLM和两个网格基准测试（GRASP和MiniGrid）上，IPP比直接代码生成性能提升10%至10倍，并在GRASP上达到新的最先进水平。

Conclusion: IPP框架通过代码生成和迭代优化显著提升了网格任务的解决能力，同时保持了策略的可解释性和成本效益。
...
TL;DR: 探讨AI系统是否可能拥有感受（如痛苦或快乐），并提出数学问题设置和方法以评估其主观体验。
...
Method: 基于强化学习和心智哲学理论，提出数学问题设置和初步方法。

Result: 提出促进强化的方法，优化问题设置。

Conclusion: 需进一步研究AI主观体验的评估方法。
...
<details>
  <summary>Details</summary>
Motivation: 临床试验资源密集且风险高，历史试验的参考价值大，但现有方法难以高效识别相似试验。

Method: 总结试验协议并基于查询协议搜索相似试验，优化了召回率和精确度。

Result: 方法在召回率和精确度上分别提升78%和53%，且在部分试验相似性搜索和零样本患者-试验匹配中表现最佳。

Conclusion: 该方法显著提升了试验设计的效率和安全性，具有广泛的应用潜力。
...
TL;DR: 论文提出了一种基于人机交互（HAII）的强化学习（RL）农业管理方法，通过量化农民对AI施肥策略的信任，优化技术性能与人文信任的匹配。


<details>
  <summary>Details</summary>
Motivation: AI在农业中的应用受限于算法建议与农民实际经验、本地知识和传统实践之间的差距，因此研究强调透明性、可用性和信任。

Method: 采用信任框架（能力、善意、诚信）开发数学模型，量化农民对AI的信任，并将调查结果融入多目标RL框架。

Result: 研究发现农民与AI建议的关键偏差，并将其整合到信任模型中，确保AI建议技术稳健、经济可行、情境感知和社会可接受。

Conclusion: 通过将技术性能与人文信任对齐，研究支持AI在农业中的更广泛应用。
...
TL;DR: 论文提出了一种基于LLM的程序合成方法（PoE-World），用于从少量观察中学习复杂、随机的世界模型，并展示了在Atari游戏中的高效表现和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统基于深度学习的世界模型需要大量训练数据且难以从稀疏观察中灵活更新知识，而程序化世界模型通过LLM支持从少量数据中强泛化。

Method: 提出PoE-World方法，将世界模型表示为LLM合成的程序化专家的指数加权乘积，适用于复杂非网格世界领域。

Result: 该方法能从少量观察中学习复杂随机世界模型，并在Atari的Pong和Montezuma's Revenge中表现出高效和泛化能力。

Conclusion: PoE-World为复杂环境中的世界建模提供了一种高效且泛化能力强的解决方案。
...
TL;DR: 提出了一种新型语义通信框架，能够联合捕获任务特定信息和上下文信息，以提升下游任务性能。
...
Method: 引入了一种灵活的语义通信框架，结合任务特定和上下文信息。

Result: 在图像数据集和计算机视觉任务中表现出色，包括下游任务性能提升、泛化能力强、带宽效率高和重建延迟低。

Conclusion: 该框架为下一代语义通信提供了有效的解决方案。
...
TL;DR: 论文提出了一种基于长篇叙述形式的脑筋急转弯基准，用于深入探究大型语言模型（LLMs）的推理策略，关注正确性、解决方案质量和创造力。


<details>
  <summary>Details</summary>
Motivation: 准确性作为评估AI系统的标准指标，但无法揭示模型的推理过程。脑筋急转弯因其多解性（如创意解法或暴力解法）适合探究模型的推理能力。

Method: 研究LLMs在多个推理层次的表现，包括语义解析、数学形式生成、自我纠正、分步解决方案草图生成及利用提示。

Result: LLMs在某些情况下能提出创意解法，显示其具备解决新颖问题的潜力，但仍存在依赖暴力解法的情况。

Conclusion: LLMs在创意推理方面有潜力，但需进一步改进以提升其推理能力。
...
TL;DR: 提出了一种基于模式连接的非线性机器遗忘框架MCU，通过参数掩码和自适应调整策略提升性能与效率。


<details>
  <summary>Details</summary>
Motivation: 解决现有线性参数更新方法中的权重纠缠问题，提升机器遗忘的效能与适应性。

Method: 利用模式连接寻找非线性遗忘路径，引入参数掩码和自适应调整策略。

Result: 在图像分类任务中表现出优越性能，提供了一系列遗忘模型。

Conclusion: MCU是一个即插即用框架，可无缝集成现有方法，显著提升遗忘效果。
...
TL;DR: InfantAgent-Next是一个多模态通用代理，能够通过文本、图像、音频和视频与计算机交互，采用模块化架构整合工具和视觉代理，优于现有方法。
...
Method: 采用高度模块化架构，整合工具和纯视觉代理，分步协作解决任务。

Result: 在OSWorld上达到7.27%准确率，优于Claude-Computer-Use，并在GAIA和SWE-Bench等基准测试中表现优异。
...
TL;DR: MPS-Prover是一种新型的逐步自动定理证明系统，通过数据剪枝和多视角树搜索机制提升效率，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有逐步证明器存在搜索偏差和效率低下的问题，需要改进。
...
TL;DR: 论文研究了在大型语言模型（LLM）中扩展测试时间计算时，不同推理提示策略的表现，发现随着计算量增加，复杂策略逐渐落后于简单的链式思考（Chain-of-Thought）。作者提出了一种基于概率理论的方法来预测最佳策略，并提出了两种改进扩展性能的方法。


<details>
  <summary>Details</summary>
Motivation: 探索在测试时间计算扩展下，不同推理提示策略的表现差异，尤其是多数投票（majority voting）这一标准设置。

Method: 系统实验（6种LLM × 8种提示策略 × 6个基准测试），结合理论分析和概率预测方法。

Result: 复杂提示策略在初始表现优越，但随着计算量增加，简单链式思考表现更优。提出的预测方法能快速选择最佳策略。

Conclusion: 研究呼吁重新审视复杂提示策略的作用，释放简单策略的潜力，并为提升测试时间扩展性能提供新思路。
...
TL;DR: 论文提出了一种新概念（facets），用于在决策和枚举之间进行推理，填补了现有方法的空白。


<details>
  <summary>Details</summary>
Motivation: 现有的抽象论证框架（AFs）在决策、计数/枚举和细粒度推理之间存在昂贵的推理需求，需要一种更高效的方法。

Method: 引入facets概念，研究其复杂性，并提供实现和实验验证可行性。

Result: 涉及facets的任务比计数扩展更容易，实验证明了其可行性。

Conclusion: facets为论证推理提供了更高效的工具，填补了现有方法的空白。
...
TL;DR: 提出了一种基于深度强化学习（DRL）的实时注塑工艺优化框架，结合产品质量和利润目标，动态适应环境和运营变化。


<details>
  <summary>Details</summary>
Motivation: 注塑工艺优化在动态环境和经济条件下平衡产品质量和利润具有挑战性。

Method: 开发利润函数，构建代理模型预测质量和周期时间，使用SAC和PPO算法离线训练DRL代理。

Result: DRL框架动态适应变化，保持质量同时最大化利润，比传统方法快135倍。

Conclusion: 该框架具有可扩展性和适应性，适合现代制造环境中的智能决策。
...
<details>
  <summary>Details</summary>
Motivation: 现有检索器依赖公共知识且对复杂查询表现不佳，生成器存在保真度问题。

Method: 提出RAGSynth框架，包括数据建模和合成数据生成，以及SynthBench基准测试。

Result: 合成数据显著提升检索器鲁棒性和生成器保真度，且能跨领域泛化。

Conclusion: RAGSynth有效提升RAG系统性能，并已开源。
...
<details>
  <summary>Details</summary>
Motivation: 提高AI系统的可解释性，确保用户信任其决策过程。

Method: 使用形式化模型生成溯因解释，并通过膨胀解释覆盖更多输入空间。

Result: 提出了找到最通用溯因解释的方法，确保解释的广泛适用性。

Conclusion: 最通用的解释能提高用户对AI决策的信任，适用于多种输入场景。
...
TL;DR: 论文提出了一种基于推理的VLM防护模型GuardReasoner-VL，通过在线强化学习提升模型的安全决策能力。


<details>
  <summary>Details</summary>
Motivation: 提升视觉语言模型（VLM）的安全性，通过激励模型在决策前进行深思熟虑的推理。

Method: 构建包含123K样本和631K推理步骤的数据集GuardReasoner-VLTrain，通过监督微调（SFT）和在线强化学习（RL）增强推理能力，并采用安全感知数据拼接和动态剪裁参数优化训练。

Result: 模型在实验中表现优异，平均F1分数超过第二名19.27%。

Conclusion: GuardReasoner-VL显著提升了VLM的安全性，并公开了数据、代码和模型。
...
TL;DR: 论文提出Thought-Aligner，一种动态思想校正模块，用于提升LLM自主代理在多步任务中的安全性，将行为安全从50%提升至90%。


<details>
  <summary>Details</summary>
Motivation: LLM自主代理的思想轨迹可能引发安全风险，需动态校正以确保安全。

Method: 提出Thought-Aligner模块，通过轻量级模型实时校正高风险思想，未改变代理框架。

Result: 实验表明，Thought-Aligner将行为安全提升至90%，延迟低于100ms。
...
TL;DR: 提出了一种基于光照感知的多模态融合网络（IMF），通过结合外感知和本体感知，动态调整模态权重，优化道路地形感知。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶车辆的传感器易受光照和天气变化影响，难以实时感知道路条件。

Method: 设计光照感知子网络估计光照特征，并构建多模态融合网络动态调整模态权重，结合预训练和光照损失优化训练。

Result: IMF在实验中表现优于现有方法，多模态融合在变化光照条件下具有全面优势。

Conclusion: IMF通过光照感知和多模态融合显著提升了道路地形的感知准确性。
...
TL;DR: 本文提出了一种三步骤方法分析客户旅程，包括定义序列距离、预测购买概率及推荐反事实序列以提高购买率。


<details>
  <summary>Details</summary>
Motivation: 全渠道平台的兴起引发了对客户旅程的关注，但其定量研究因数据序列性和分析复杂性而不足。

Method: 定义序列距离并可视化代表性序列；基于距离预测购买概率；若无购买，推荐反事实序列以提高购买率。

Result: 实验表明典型序列可被提取，且能识别对购买重要的部分。
...
TL;DR: 提出Group Think方法，通过单一大语言模型模拟多推理代理并发协作，降低延迟并提升推理质量。


<details>
  <summary>Details</summary>
Motivation: 现有多代理推理方法通常基于轮询交互，延迟较高。Group Think旨在通过并发推理和动态调整，优化推理效率和资源利用率。

Method: Group Think利用单一大语言模型模拟多推理代理，实现令牌级动态协作，减少冗余推理。

Result: 实验表明，Group Think显著降低延迟并提升推理质量，尤其适合边缘计算场景。
...
TL;DR: 论文提出了一种双模态多尺度滑动窗口（DMSW）模型，通过结合学业表现和行为数据动态捕捉学生行为模式，显著提高了辍学风险预测的准确性。


<details>
  <summary>Details</summary>
Motivation: 在离线教育环境中，数据质量差、规模有限和高度异质性阻碍了高级机器学习模型的应用，同时教育理论缺乏可量化指标，难以用于数据驱动建模。

Method: 通过数据分析和教育文献综述，识别学生行为的突变作为辍学风险的早期信号，提出DMSW模型动态捕捉行为模式。

Result: DMSW模型比传统方法预测准确率提高15%，帮助教育者更早识别高风险学生并提供支持。

Conclusion: 研究填补了理论与实践的差距，为辍学预测提供了创新工具，有助于提升学生保留率和教育成果。
...
TL;DR: 本文提出了一种结合大型语言模型（LLM）和蒙特卡洛树搜索（MCTS）的新框架，用于高效、可解释的Alpha因子挖掘。


<details>
  <summary>Details</summary>
Motivation: 传统Alpha因子挖掘方法依赖人工经验或自动化方法，但存在搜索效率低或可解释性差的问题。

Method: 通过LLM的指令遵循和推理能力，结合MCTS驱动的探索，生成和优化符号化Alpha公式，并引入频繁子树避免机制。

Result: 实验表明，该方法在预测准确性、交易性能和可解释性上优于现有方法。

Conclusion: 该框架为公式化Alpha因子挖掘提供了更高效的解决方案。
...
TL;DR: 论文比较了开源仿真模型与真实工业数据集，评估了强化学习方法（策略梯度和进化策略）在半导体前端工厂仿真中的优化效果。进化策略方法表现更优，尤其在复杂场景下。


<details>
  <summary>Details</summary>
Motivation: 现有基准数据集（如Minifab或SMT2020）缺乏真实场景的复杂细节和约束，需评估优化方法在不同复杂度下的扩展性。

Method: 使用强化学习方法（策略梯度和进化策略）进行优化，重点关注瓶颈工具的选择和组合，以及多样化训练数据集的应用。

Result: 进化策略方法在真实工业数据集上提升了4%的延迟和1%的吞吐量；在Minifab和SMT2020上延迟提升两位数百分比，吞吐量提升个位数百分比。

Conclusion: 进化策略方法在复杂场景下扩展性更好，但计算成本较高；多样化训练数据集对泛化能力至关重要。
...
TL;DR: 提出了一种新的强化学习设计，用于优化大规模块堆叠仓库中自主移动机器人的充电策略，比较了不同奖励和动作空间配置对性能的影响。


<details>
  <summary>Details</summary>
Motivation: 研究不同奖励和动作空间配置对强化学习代理性能的影响，以优化充电策略。

Method: 使用启发式充电策略作为基线，比较灵活和指导性强化学习设计的性能，扩展了SLAPStack模拟框架。

Result: 灵活的强化学习方法在服务时间上表现更优，但收敛时间较长且稳定性较差；指导性设计稳定性更高但泛化能力有限。

Conclusion: 贡献包括扩展SLAPStack框架、提出新的强化学习设计以及引入自适应基线启发式方法，为充电策略优化提供了新思路。
...
TL;DR: 论文提出了一种利用大型语言模型（LLM）判断状态-对象组合可行性的方法（FLM），以提升开放世界组合零样本学习（OW-CZSL）的性能。


<details>
  <summary>Details</summary>
Motivation: 人类能轻松判断对象属性的可行性（如“火可以是热的，但不能是湿的”），但在OW-CZSL中，零样本预测器对未见组合表现不佳。作者希望通过外部知识改进这一问题。

Method: FLM通过查询LLM判断状态-对象组合的可行性，并提取正向答案的logit值。利用LLM的上下文学习能力，减少对罕见或不可行组合的误导。

Result: 实验表明，Vicuna和ChatGPT表现最佳，FLM在三个基准测试中均显著提升了OW-CZSL性能。

Conclusion: FLM是一种简单有效的方法，通过LLM的语义理解能力显著改善了组合零样本学习的性能。
...
TL;DR: 论文探讨了全局XAI方法在检测大型语言模型（LLM）偏见中的有效性，提出了一种文本到序数映射策略，并引入RuleSHAP算法以提升检测能力。
...
Method: 通过文本到序数映射将非数值输入/输出转换为数值特征，注入不同复杂度的偏见，并测试RuleFit和SHAP的检测能力，最终提出RuleSHAP算法。

Result: RuleFit对复杂偏见检测效果差，SHAP能近似但无法生成可操作规则。RuleSHAP显著提升检测能力（MRR@1平均提升94%）。
...
TL;DR: 论文提出了一种名为Federated Foundation Models (FFMs)的新范式，结合了多模态多任务基础模型(FMs)和联邦学习(FL)的优势，以解决具身AI系统中的泛化、个性化、隐私和安全问题。


<details>
  <summary>Details</summary>
Motivation: 随着具身AI系统在多模态、个性化和交互性方面的需求增加，现有方法（如FMs和FL）单独使用时无法满足复杂需求，因此需要一种统一的方法。

Method: 提出FFMs范式，结合FMs的泛化能力和FL的隐私保护特性，并提出了EMBODY框架，涵盖部署中的关键维度。

Result: 论文提出了FFMs的概念和EMBODY框架，为具身AI系统中的挑战提供了具体的研究方向和评估方法。

Conclusion: FFMs为具身AI提供了一种新的解决方案，结合了泛化和隐私保护的优势，未来研究可以基于EMBODY框架展开。
...
TL;DR: GLOVA是一个模拟电路尺寸优化框架，通过风险敏感强化学习和集成批评器提升对PVT变化的鲁棒性，显著提高样本效率并减少时间成本。
...
Method: 采用风险敏感强化学习考虑可靠性边界，引入集成批评器实现高效学习，并提出μ-σ评估和仿真重排序方法降低验证成本。

Result: GLOVA在样本效率上提升80.5倍，时间减少76.0倍，优于现有方法。

Conclusion: GLOVA有效解决了PVT变化带来的挑战，为模拟电路设计提供了高效可靠的自动化解决方案。
...
TL;DR: 论文探讨了纯强化学习（RL）在提升大语言模型（LLMs）推理能力中的作用，挑战了过程奖励模型（PRM）的必要性，并提出自监督框架Self-PRM。


<details>
  <summary>Details</summary>
Motivation: 研究旨在验证纯RL训练是否能独立提升推理能力，并探索RL与PRM的关系。

Method: 通过实验分析纯RL训练对数学问题解决的影响，并提出Self-PRM框架进行自我评估和重排。

Result: 纯RL训练能协同提升问题解决和PRM能力，但Self-PRM在难题上精度低。

Conclusion: PRM可能非必要，纯RL可同时提升推理和PRM能力，未来需进一步优化RL扩展。
...
TL;DR: LD-Scene利用大语言模型和潜在扩散模型，通过自然语言生成用户可控的对抗场景，提升自动驾驶系统的测试效果。


<details>
  <summary>Details</summary>
Motivation: 安全关键场景在真实驾驶数据中稀少且难以收集，现有方法可控性差且依赖专家知识。

Method: 结合LLM和LDM，通过自然语言生成对抗场景，包括轨迹分布捕获和基于LLM的指导模块。
...
Method: 采用双阶段训练：预估计查询难度以分配推理成本，再通过预算引导的GPRO优化推理长度。

Result: 在MATH基准测试中，推理长度压缩达74.47%，准确性几乎不受影响。
...
<details>
  <summary>Details</summary>
Motivation: 解决Meta-World历史版本中未记录的变更导致算法对比不公平的问题。

Method: 利用Meta-World的过去版本，分析多任务和元强化学习基准设计。

Result: 发布了新的开源版本，支持历史结果复现，技术更友好，任务集更灵活。

Conclusion: 新版本Meta-World为多任务和元强化学习研究提供了更可靠的基准。
...
TL;DR: 论文探讨了如何从文本中提取日期数据，比较了手动和自动生成正则表达式的方法，发现自动生成的正则表达式在减少误报方面表现更好。


<details>
  <summary>Details</summary>
Motivation: 日期数据对医疗决策至关重要，但提取方法尚不明确。AI转录文档时，现有方法（如复杂模型或正则表达式）存在局限性。

Method: 测试公开可用的正则表达式，手动创建易分解的正则表达式，并使用正则表达式合成技术自动生成正则表达式。

Result: 自动生成的正则表达式误报率更低，但漏报率略有增加。

Conclusion: 正则表达式合成可有效识别复杂日期和日期范围，是一种新的确定性逻辑学习方法。
...
<details>
  <summary>Details</summary>
Motivation: 在人工智能中，学习有效策略以优化性能是关键任务，但未观测混杂因素可能导致数据偏差，影响学习效果。本文旨在解决这一问题。

Method: 基于深度Q网络（DQN），提出了一种新算法，能够找到与观测数据兼容的最坏情况下环境的安全策略。

Result: 在12个存在混杂偏差的Atari游戏中，新算法在所有输入行为与目标策略不匹配且存在未观测混杂因素的游戏中均优于标准DQN。

Conclusion: 新算法在存在混杂偏差的高维复杂领域中表现出色，为离策略学习提供了更稳健的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究人类与AI在协同创作中的控制权平衡问题，以优化共创过程。

Method: 通过系统文献综述（172篇论文）提出MOSAAIC框架，并分析六个案例验证其适用性。

Result: MOSAAIC框架能有效表征和优化协同创作中的控制权分配。
...
### [185] [Two Minds Better Than One: Collaborative Reward Modeling for LLM Alignment](https://arxiv.org/abs/2505.10597)
...
TL;DR: 论文提出协作奖励建模（CRM）框架，通过结合同行评审和课程学习，提升奖励模型在噪声偏好数据下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 人类反馈中的噪声偏好会导致奖励模型过拟合虚假模式，影响策略优化效果。

Method: 提出CRM框架，训练两个并行奖励模型互相评估数据选择，并结合课程学习从易到难训练。

Result: 实验表明，CRM在40%噪声下RewardBench准确率提升9.94分，且兼容隐式奖励对齐方法。

Conclusion: CRM是一种实用且通用的鲁棒对齐策略。
...
### [186] [UDDETTS: Unifying Discrete and Dimensional Emotions for Controllable Emotional Text-to-Speech](https://arxiv.org/abs/2505.10599)
*Jiaxuan Liu,Zhenhua Ling*

Main category: cs.LG

TL;DR: UDDETTS是一种结合离散和维度情感控制的神经编解码语言模型，用于可控情感TTS，解决了传统方法在情感复杂性和连续性上的不足。


<details>
  <summary>Details</summary>
Motivation: 传统TTS方法依赖离散情感标签，无法捕捉情感的复杂性和连续性，且数据不平衡导致模型过拟合。

Method: 提出UDDETTS模型，引入ADV空间描述维度情感，支持离散标签或ADV值控制，采用半监督训练策略。

Result: 实验表明UDDETTS能实现ADV空间的线性情感控制，并具备优异的情感语音合成能力。

Conclusion: UDDETTS为情感TTS提供了更灵活和精确的控制方法。
...
### [187] [Enhancing IoT Cyber Attack Detection in the Presence of Highly Imbalanced Data](https://arxiv.org/abs/2505.10600)
*Md. Ehsanul Haque,Md. Saymon Hosen Polash,Md Al-Imran Sanjida Simla,Md Alomgir Hossain,Sarwar Jahan*

Main category: cs.LG

TL;DR: 论文提出了一种混合采样技术，用于解决物联网（IoT）网络中高度不平衡数据集的入侵检测问题，并通过多种机器学习模型验证了其有效性。
...
Result: 随机森林表现最佳，Kappa分数为0.9903，测试准确率为0.9961，AUC为0.9994；软投票模型也表现出色，准确率为0.9952，AUC为0.9997。

Conclusion: 混合采样技术与稳健的模型和特征选择相结合，显著提高了物联网安全在高度不平衡数据环境中的防御能力。
...
### [188] [Continuity and Isolation Lead to Doubts or Dilemmas in Large Language Models](https://arxiv.org/abs/2505.10606)
*Hector Pasten,Felipe Urrutia,Hector Jimenez,Cristian B. Calderon,Cristóbal Rojas,Alexander Kozachinskiy*

Main category: cs.LG

TL;DR: 论文揭示了Transformer中存在的隔离性和连续性现象，这些现象限制了其学习简单序列模式的能力。
...
Result: 隔离性表明可学习序列必须相互隔离，连续性则形成吸引子盆地，导致序列向学习序列坍塌。

Conclusion: 研究揭示了Transformer的理论局限性，并在实际规模上验证了这些现象。
...
### [189] [MONAQ: Multi-Objective Neural Architecture Querying for Time-Series Analysis on Resource-Constrained Devices](https://arxiv.org/abs/2505.10607)
...
<details>
  <summary>Details</summary>
Motivation: 智能手机和IoT设备的普及需要高效的时间序列分析，但现有硬件感知NAS方法未针对边缘部署的通用时间序列分析。

Method: MONAQ将NAS重新定义为多目标神经架构查询任务，结合多模态查询生成和LLM代理的多目标搜索，通过代码生成实现部署就绪模型。

Result: 在15个数据集上的实验表明，MONAQ发现的模型优于手工模型和NAS基线，且更高效。

Conclusion: MONAQ通过多模态输入和LLM的推理能力，为边缘设备上的时间序列分析提供了高效的解决方案。
...
### [190] [How many measurements are enough? Bayesian recovery in inverse problems with general distributions](https://arxiv.org/abs/2505.10630)
*Ben Adcock,Nick Huang*

Main category: cs.LG

TL;DR: 研究了贝叶斯恢复的样本复杂度，适用于一般先验、前向算子和噪声分布。结果表明样本复杂度取决于先验的内在复杂性和分布集中性，特别适用于生成先验。


<details>
  <summary>Details</summary>
Motivation: 探索贝叶斯逆问题中样本复杂度的通用理论框架，特别是针对深度神经网络（DNN）生成先验的恢复问题。

Method: 提出非渐近边界，分析样本复杂度与先验覆盖数和分布集中性的关系，并应用于DNN生成先验和随机采样问题。

Result: 样本复杂度与潜在维度对数线性相关，且正交矩阵的相干性对恢复至关重要。

Conclusion: 统一并扩展了现有工作，为任意分布的贝叶斯逆问题提供了严格的样本复杂度保证。
...
### [191] [FRET: Feature Redundancy Elimination for Test Time Adaptation](https://arxiv.org/abs/2505.10641)
*Linjing You,Jiabao Lu,Xiayuan Huang,Xiangli Nie*

Main category: cs.LG

TL;DR: 论文提出FRET方法，通过消除特征冗余提升测试时适应（TTA）性能，包括简单版S-FRET和基于图网络的G-FRET，后者结合GCN和对比学习，显著提升模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 测试数据分布偏移时，现有TTA方法忽略特征冗余问题，影响模型适应性。

Method: 提出S-FRET直接优化特征冗余分数，G-FRET结合GCN和对比学习减少冗余并增强特征区分性。

Result: 实验表明S-FRET有效，G-FRET在多种任务和数据集上达到最优性能。

Conclusion: G-FRET能提取非冗余且高区分性特征，显著提升TTA鲁棒性。
...
### [192] [Accelerating Visual-Policy Learning through Parallel Differentiable Simulation](https://arxiv.org/abs/2505.10646)
*Haoxiang You,Yilang Liu,Ian Abraham*

Main category: cs.LG

TL;DR: 提出了一种高效视觉策略学习算法，利用可微分模拟和一阶解析策略梯度，减少计算开销并提升优化稳定性。


<details>
  <summary>Details</summary>
Motivation: 解决现有视觉策略学习方法在计算和内存开销上的问题，同时提升优化稳定性。

Method: 通过将渲染过程与计算图解耦，无需专用可微分渲染软件，降低计算和内存开销。

Result: 在标准视觉控制基准测试中显著减少训练时间，性能优于基线方法，复杂任务如人形机器人跑步策略学习效果显著。

Conclusion: 该方法高效且稳定，适用于复杂任务，显著提升性能。
...
TL;DR: IceMamba是一种结合注意力机制和状态空间模型的深度学习架构，用于季节性海冰预测，性能优于25种现有模型。


<details>
  <summary>Details</summary>
Motivation: 北极海冰快速减少对生态系统和全球气候构成威胁，亟需精确的季节性海冰预测方法。

Method: 提出IceMamba，结合注意力机制与状态空间模型，并与25种现有模型（动力学、统计和深度学习）进行对比。

Result: IceMamba在平均RMSE和ACC上表现最佳，IIEE排名第二。

Conclusion: IceMamba提升了季节性海冰预测能力，为气候适应策略提供重要支持。
...
TL;DR: 提出了一种基于共形预测的新方法（CPCF）来评估持续学习中的灾难性遗忘（CF），并通过实验验证其可靠性和实用性。
...
Method: 利用自适应共形预测（CP）设计CPCF指标，通过监测模型对已学习任务的置信度来评估CF。

Result: 在四个基准数据集上的实验表明，CPCF与先前任务的准确性高度相关，验证了其可靠性。

Conclusion: CPCF是一种有效且可解释的工具，适用于动态学习环境中CF的评估。
...
### [195] [A probabilistic framework for dynamic quantization](https://arxiv.org/abs/2505.10689)
*Gabriele Santini,Francesco Paissan,Elisabetta Farella*

Main category: cs.LG

TL;DR: 提出了一种动态量化神经网络的概率框架，通过轻量级代理自适应调整量化参数，实现高效计算。


<details>
  <summary>Details</summary>
Motivation: 解决传统量化方法在动态输入下性能损失和计算开销的问题。

Method: 使用概率模型对预激活进行建模，通过轻量级代理实现输入自适应的量化参数调整。
...
### [196] [Asymptotically-Optimal Gaussian Bandits with Side Observations](https://arxiv.org/abs/2505.10698)
*Alexia Atsidakou,Orestis Papadigenopoulos,Constantine Caramanis,Sujay Sanghavi,Sanjay Shakkottai*

Main category: cs.LG

TL;DR: 论文研究了具有一般侧信息的高斯多臂老虎机问题，提出了一种基于LP的渐近最优算法。


<details>
  <summary>Details</summary>
Motivation: 探索高斯噪声下多臂老虎机问题中侧信息矩阵对反馈的影响，涵盖标准老虎机、全反馈和图结构反馈等特例。

Method: 构建了一个基于线性规划（LP）的渐近实例依赖下界，并设计了一种渐近最优算法。

Result: 提出了一种针对该通用设置的第一个已知渐近最优算法。

Conclusion: 通过LP优化成本（遗憾）来可靠估计每个臂的次优性差距，为高斯侧信息老虎机问题提供了理论支持。
...
### [197] [Clustering Rooftop PV Systems via Probabilistic Embeddings](https://arxiv.org/abs/2505.10699)
*Kutay Bölat,Tarek Alskaif,Peter Palensky,Simon Tindemans*

Main category: cs.LG

TL;DR: 提出了一种基于概率实体嵌入的聚类框架，用于管理高维、缺失值影响的分布式光伏系统数据。


<details>
  <summary>Details</summary>
Motivation: 随着屋顶光伏系统数量增加，如何高效管理和分析高维、缺失值影响的时间序列数据成为挑战。

Method: 通过概率分布编码光伏系统的发电模式和不确定性，利用统计距离和凝聚聚类进行分组。

Result: 在多年度住宅光伏数据集上，该方法生成的聚类结果在代表性和鲁棒性上优于物理基线，并支持可靠的缺失值填补。

Conclusion: 该框架为光伏系统数据管理提供了有效解决方案，并通过超参数研究优化了性能与鲁棒性的平衡。
...
### [198] [ZEUS: Zero-shot Embeddings for Unsupervised Separation of Tabular Data](https://arxiv.org/abs/2505.10704)
*Patryk Marszałek,Tomasz Kuśmierczyk,Witold Wydmański,Jacek Tabor,Marek Śmieja*

Main category: cs.LG

TL;DR: ZEUS是一种零-shot学习方法，用于无监督聚类表格数据，无需额外训练或调参，性能优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 表格数据的聚类因数据集依赖性高且缺乏监督信号而具有挑战性，传统方法需要大量调参且性能不稳定。

Method: ZEUS通过分解数据集为有意义组件进行聚类，基于合成数据的预训练实现零-shot学习。

Result: 实验表明，ZEUS性能与传统方法和深度学习方法相当或更优，且速度更快、更易用。

Conclusion: ZEUS是首个零-shot无监督表格数据聚类方法，具有高效性和普适性。
...
### [199] [GNN-Suite: a Graph Neural Network Benchmarking Framework for Biomedical Informatics](https://arxiv.org/abs/2505.10711)
*Sebestyén Kamp,Giovanni Stracquadanio,T. Ian Simpson*

Main category: cs.LG

TL;DR: GNN-Suite是一个模块化框架，用于构建和评估图神经网络（GNN）在计算生物学中的性能，通过标准化实验和可重复性，展示了其在识别癌症驱动基因中的实用性。
...
Method: 使用Nextflow工作流程标准化实验，构建分子网络并标注节点特征，比较多种GNN架构和逻辑回归模型。

Result: GCN2在STRING网络中表现最佳（BACC 0.807），所有GNN均优于逻辑回归基线。

Conclusion: GNN-Suite有助于识别最佳模型和数据整合方法，未来将扩展数据集并优化架构以提高预测准确性。
...
### [200] [Learning Repetition-Invariant Representations for Polymer Informatics](https://arxiv.org/abs/2505.10726)
*Yihan Zhu,Gang Liu,Eric Inae,Tengfei Luo,Meng Jiang*

Main category: cs.LG

TL;DR: 论文提出了一种名为GRIN的新方法，用于学习聚合物重复单元不变的表示，解决了现有图神经网络方法无法处理聚合物结构变化的问题。
...
Result: GRIN在均聚物和共聚物基准测试中优于现有方法，能够学习稳定且重复不变的表示，并能泛化到未见过的聚合物链大小。
...
### [201] [Random Client Selection on Contrastive Federated Learning for Tabular Data](https://arxiv.org/abs/2505.10759)
*Achmad Ginanjar,Xue Li,Priyanka Singh,Wen Hua*

Main category: cs.LG

TL;DR: 本文分析了垂直联邦学习（VFL）中的信息泄漏问题，提出随机客户端选择作为对比联邦学习（CFL）中梯度攻击的防御策略，并通过实验验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 垂直联邦学习（VFL）在隐私保护方面存在信息泄漏风险，尤其是梯度攻击对对比联邦学习（CFL）构成威胁，需要有效的防御方法。

Method: 通过实验分析梯度攻击在CFL环境中的影响，并评估随机客户端选择作为防御策略的效果。

Result: 实验表明，随机客户端选择能有效防御CFL网络中的梯度攻击。

Conclusion: 随机客户端选择是提升对比联邦学习系统安全性的有效策略，为构建更安全的协作学习框架提供了重要参考。
...
### [202] [Deep Symbolic Optimization: Reinforcement Learning for Symbolic Mathematics](https://arxiv.org/abs/2505.10762)
...
TL;DR: DSO是一种新颖的计算框架，用于符号优化，特别适用于科学发现中的复杂符号结构搜索，如方程发现。


<details>
  <summary>Details</summary>
Motivation: 旨在自动推导符号形式的数学模型，提升科学发现的效率和准确性。

Method: 结合生成神经网络、强化学习、梯度优化、进化搜索和局部搜索技术，并融入领域先验和约束。

Result: 在基准测试中表现出色，实现了高精度和可解释性。

Conclusion: DSO框架在自动化符号优化方面具有变革性潜力。
...
### [203] [Context-Aware Probabilistic Modeling with LLM for Multimodal Time Series Forecasting](https://arxiv.org/abs/2505.10774)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法难以有效整合外生文本并适应LLM的概率特性，限制了上下文感知和分布建模能力。

Method: CAPTime使用预训练时间序列编码器和可学习交互对齐文本上下文，结合混合分布专家与冻结LLM实现概率预测。

Result: 实验表明CAPTime在多模态任务中表现优越，尤其在数据稀缺场景下通过混合概率解码展现鲁棒性。

Conclusion: CAPTime通过上下文感知和概率解码，显著提升了时间序列预测的准确性和泛化能力。
...
### [204] [Cell Library Characterization for Composite Current Source Models Based on Gaussian Process Regression and Active Learning](https://arxiv.org/abs/2505.10799)
*Tao Bai,Junzhuo Zhou,Zeyuan Deng,Peng Cao*

Main category: cs.LG

TL;DR: 论文提出了一种基于高斯过程回归（GPR）和主动学习（AL）的新方法，用于高效准确地表征复合电流源（CCS）模型，显著提升了精度并降低了计算和存储成本。


<details>
  <summary>Details</summary>
Motivation: 传统非线性延迟模型（NLDM）在先进工艺节点下难以准确建模复杂的动态效应和交互作用，而CCS模型的高精度要求、大数据量和模拟成本带来了挑战。

Method: 采用高斯过程回归（GPR）结合主动学习（AL）的方法，建立高效的CCS表征框架。

Result: 在TSMC 22nm工艺下，对57个单元的电流波形进行测试，平均绝对误差为2.05 ps，相对误差为2.27%，且运行时和存储需求分别降至商业工具的27%和1/19.5。

Conclusion: 该方法显著优于传统商业工具和学习方法，为CCS模型的高效表征提供了有效解决方案。
...
### [205] [Attention-Based Reward Shaping for Sparse and Delayed Rewards](https://arxiv.org/abs/2505.10802)
*Ian Holmes,Min Chi*

Main category: cs.LG

TL;DR: ARES是一种基于注意力机制的奖励塑造算法，能够为稀疏或延迟奖励的环境生成密集奖励函数，完全离线训练且适用于任何RL算法。


<details>
  <summary>Details</summary>
Motivation: 解决现实世界中强化学习因稀疏和延迟奖励而难以应用的问题。

Method: 利用transformer的注意力机制生成密集奖励函数，仅需一组片段及其最终回报作为输入，完全离线训练。

Result: 在多种环境和RL算法中验证，ARES能显著提升延迟奖励场景下的学习效果。

Conclusion: ARES是首个完全离线、适应极端奖励延迟和低质量数据的方法，且不限于目标导向任务。
...
### [206] [Distilled Circuits: A Mechanistic Study of Internal Restructuring in Knowledge Distillation](https://arxiv.org/abs/2505.10822)
*Reilly Haskins,Benjamin Adams*

Main category: cs.LG

TL;DR: 知识蒸馏通过训练学生模型匹配教师模型的输出，但内部计算机制尚不明确。研究发现学生模型会重组、压缩或丢弃教师组件，依赖更少组件。提出了基于影响加权的对齐度量，发现蒸馏保留了功能行为但改变了内部计算。


<details>
  <summary>Details</summary>
Motivation: 理解知识蒸馏过程中内部计算机制的变化及其对学生模型的影响。

Method: 应用机制解释性技术分析教师与学生模型的内部电路、表示和激活模式，引入影响加权的对齐度量。

Result: 学生模型重组、压缩或丢弃教师组件，依赖更少组件；蒸馏保留了功能行为但改变了内部计算。

Conclusion: 知识蒸馏虽保留功能行为，但内部计算变化显著，影响学生模型的鲁棒性和泛化能力。
...
### [207] [MergeBench: A Benchmark for Merging Domain-Specialized LLMs](https://arxiv.org/abs/2505.10833)
*Yifei He,Siqi Zeng,Yuzheng Hu,Rui Yang,Tong Zhang,Han Zhao*

Main category: cs.LG

TL;DR: MergeBench是一个评估模型合并的综合套件，针对大规模、领域专用LLMs，提供标准化评估和实用指南。


<details>
  <summary>Details</summary>
Motivation: 解决现有模型合并方法在模型规模和任务多样性上的局限性，推动模型合并的实际应用。

Method: 基于Llama和Gemma家族模型（2B至9B规模），覆盖五个关键领域，评估八种代表性合并方法。

Result: 模型合并对更强的基础模型效果更好，调优和稀疏化等技术有助于知识保留，但仍存在计算成本和性能差距等挑战。

Conclusion: MergeBench为未来研究提供了基础，促进模型合并的理解和应用，代码已开源。
...
### [208] [LARGO: Latent Adversarial Reflection through Gradient Optimization for Jailbreaking LLMs](https://arxiv.org/abs/2505.10838)
*Ran Li,Hao Wang,Chengzhi Mao*

Main category: cs.LG

TL;DR: LARGO是一种基于梯度优化的新型潜在自反射攻击方法，用于生成流畅的越狱提示，显著提升了攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有攻击方法在离散语言空间中难以应用梯度优化，因此需要一种更高效的方法来发现大型语言模型的漏洞。
...
### [209] [Ready2Unlearn: A Learning-Time Approach for Preparing Models with Future Unlearning Readiness](https://arxiv.org/abs/2505.10845)
*Hanyu Duan,Yi Yang,Ahmed Abbasi,Kar Yan Tam*

Main category: cs.LG

TL;DR: Ready2Unlearn是一种在训练阶段优化模型以支持未来高效遗忘请求的方法，区别于现有主要在部署阶段被动处理遗忘请求的方法。


<details>
  <summary>Details</summary>
Motivation: 现有遗忘算法通常在模型部署阶段被动实现，效率较低。Ready2Unlearn旨在通过训练阶段的主动优化，提升未来遗忘请求的处理效率和模型性能。

Method: 基于元学习原则，Ready2Unlearn在训练阶段主动优化模型，使其具备遗忘准备性，兼容任何基于梯度上升的遗忘算法。

Result: 实验表明，Ready2Unlearn能减少遗忘时间、保持模型能力，并增强对遗忘数据的抗恢复性。

Conclusion: Ready2Unlearn为机器学习模型提供了一种前瞻性的遗忘准备策略，有望推动更可靠和高效的遗忘方法研究。
...
### [210] [AutoRAN: Weak-to-Strong Jailbreaking of Large Reasoning Models](https://arxiv.org/abs/2505.10846)
*Jiacheng Liang,Tanqiu Jiang,Yuhui Wang,Rongyi Zhu,Fenglong Ma,Ting Wang*

Main category: cs.LG

TL;DR: AutoRAN是一个自动化、弱到强的越狱攻击框架，针对大型推理模型（LRMs），通过弱对齐模型模拟目标模型的高层推理结构，生成并迭代优化提示，攻击成功率接近100%。


<details>
  <summary>Details</summary>
Motivation: 揭示弱推理模型如何有效利用更强推理模型的关键漏洞，强调需要针对推理模型改进安全措施。

Method: 利用弱对齐模型模拟目标模型的推理结构，生成叙事提示，并通过目标模型的中间推理步骤迭代优化候选提示。

Result: 在多个基准数据集上，AutoRAN对GPT-o3/o4-mini和Gemini-2.5-Flash等LRMs的攻击成功率接近100%。

Conclusion: 弱推理模型能有效攻击更强推理模型，突显了推理模型安全措施的不足，需进一步改进。
...
### [211] [Foundation model for mass spectrometry proteomics](https://arxiv.org/abs/2505.10848)
...
TL;DR: 提出了一种基于预训练的统一基础模型，用于质谱数据的多任务分析。


<details>
  <summary>Details</summary>
Motivation: 质谱数据的复杂性需要高效的计算方法，机器学习在质谱分析中显示出潜力，但目前方法分散且任务专用。

Method: 通过去新测序预训练质谱编码器，并在四个下游任务中验证其性能。

Result: 预训练模型在多个任务中表现优异，多任务微调进一步提升了性能。
...
### [212] [ImputeINR: Time Series Imputation via Implicit Neural Representations for Disease Diagnosis with Missing Data](https://arxiv.org/abs/2505.10856)
*Mengxuan Li,Ke Liu,Jialong Guo,Jiajun Bu,Hongwei Wang,Haishuai Wang*

Main category: cs.LG

TL;DR: 论文提出了一种名为ImputeINR的新方法，利用隐式神经表示（INR）学习时间序列的连续函数，以解决医疗数据中大量缺失值的问题，并在高缺失率下表现优异。


<details>
  <summary>Details</summary>
Motivation: 医疗数据中常存在大量缺失值，现有插补方法对稀疏数据建模效果不佳，尤其是高缺失率情况下。

Method: 采用隐式神经表示（INR）学习时间序列的连续函数，实现细粒度插补。

Result: 在八个数据集和五种缺失率下，ImputeINR表现优异，尤其在高缺失率时，且能提升下游疾病诊断任务的性能。

Conclusion: ImputeINR是一种高效的时间序列插补方法，特别适用于高缺失率的医疗数据。
...
### [213] [On DeepSeekMoE: Statistical Benefits of Shared Experts and Normalized Sigmoid Gating](https://arxiv.org/abs/2505.10860)
*Huy Nguyen,Thong T. Doan,Quang Pham,Nghi D. Q. Bui,Nhat Ho,Alessandro Rinaldo*

Main category: cs.LG

TL;DR: 本文对DeepSeekMoE中的共享专家策略和归一化Sigmoid门控机制进行了理论分析，验证了其在样本效率和模型设计中的优势。


<details>
  <summary>Details</summary>
Motivation: 尽管DeepSeekMoE在DeepSeek系列模型中表现突出，但其共享专家策略和归一化Sigmoid门控机制的理论价值尚未充分研究。

Method: 通过统计视角对这两种机制进行理论分析，并进行收敛性分析，同时在合成数据和真实数据集上进行实验验证。

Result: 理论分析表明这两种机制在样本效率上有显著提升，实验进一步验证了其有效性。

Conclusion: 研究为专家和门控结构的设计提供了理论支持，并揭示了路由器行为的多种特性。
...
### [214] [Improving the Data-efficiency of Reinforcement Learning by Warm-starting with LLM](https://arxiv.org/abs/2505.10861)
*Thang Duong,Minglai Yang,Chicheng Zhang*

Main category: cs.LG

TL;DR: 论文探讨了利用大型语言模型（LLM）生成高质量数据以预热强化学习（RL）算法的方法，提出LORO算法，结合LLM和RL，在多个环境中表现优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 研究如何利用LLM生成覆盖最优策略状态-动作对的离线数据集，以提高RL算法的样本效率和收敛性。

Method: 提出LORO算法，先用LLM生成初始策略数据集，再用RL算法探索环境并优化策略。

Result: 在CartPole和Pendulum等环境中，LORO的累积奖励比纯RL基线高出4倍。
...
### [215] [Hashing for Structure-based Anomaly Detection](https://arxiv.org/abs/2505.10873)
...
<details>
  <summary>Details</summary>
Motivation: 解决低维流形中不符合结构模式的样本识别问题。

Method: 在偏好空间中嵌入数据，利用局部敏感哈希避免高维距离显式计算。

Result: 实现了高性能的异常检测，同时降低了计算成本。

Conclusion: 该方法在偏好空间中表现优异，代码已公开。
...
### [216] [MultiLink: Multi-class Structure Recovery via Agglomerative Clustering and Model Selection](https://arxiv.org/abs/2505.10874)
*Luca Magri,Filippo Leveni,Giacomo Boracchi*

Main category: cs.LG

TL;DR: 论文提出MultiLink算法，用于从噪声和异常值污染的数据中恢复多类几何结构，结合偏好分析和聚类，具有速度快、对阈值不敏感等优势。


<details>
  <summary>Details</summary>
Motivation: 解决在噪声和异常值污染的数据中恢复多类几何结构的问题，如平面、圆柱体、单应矩阵等。

Method: 提出MultiLink算法，结合动态模型拟合和模型选择，通过新的链接方案决定是否合并聚类。

Result: 实验表明MultiLink在速度和鲁棒性上优于现有方法，适用于多类和单类问题。

Conclusion: MultiLink是一种高效、鲁棒的算法，适用于复杂几何结构恢复问题，代码已公开。
...
### [217] [Preference Isolation Forest for Structure-based Anomaly Detection](https://arxiv.org/abs/2505.10876)
*Filippo Leveni,Luca Magri,Cesare Alippi,Giacomo Boracchi*

Main category: cs.LG

TL;DR: 提出了一种名为PIF的异常检测框架，结合自适应隔离方法和偏好嵌入，通过高维偏好空间识别异常点。


<details>
  <summary>Details</summary>
Motivation: 解决异常检测问题，特别是针对不符合低维流形结构模式的样本。

Method: 提出三种隔离方法：Voronoi-iForest（通用方案）、RuzHash-iForest（避免显式计算距离）、Sliding-PIF（利用局部性先验）。

Result: 通过嵌入高维偏好空间和隔离点识别异常。

Conclusion: PIF框架结合了自适应隔离和偏好嵌入的优势，有效识别异常。
...
### [218] [Graph and Simplicial Complex Prediction Gaussian Process via the Hodgelet Representations](https://arxiv.org/abs/2505.10877)
*Mathieu Alain,So Takao,Xiaowen Dong,Bastian Rieck,Emmanuel Noutahi*

Main category: cs.LG

TL;DR: 论文提出了一种基于高斯过程（GPs）的框架，用于处理图结构数据标签预测问题，特别针对数据稀缺时图神经网络（GNNs）过拟合的问题。通过扩展到单纯复形（SCs）并利用Hodge分解，该方法能处理高阶属性并提升预测性能。


<details>
  <summary>Details</summary>
Motivation: 解决图神经网络在数据稀缺时过拟合的问题，同时扩展高斯过程框架以处理更高阶的图结构数据属性。

Method: 将高斯过程框架扩展到单纯复形（SCs），并结合Hodge分解以利用同调信息（如孔洞数量）。

Result: 该方法在各种应用中提升了预测性能，为高斯过程在图和单纯复形级别的预测中更广泛应用铺平了道路。

Conclusion: 通过结合单纯复形和Hodge分解的高斯过程框架，有效提升了图结构数据标签预测的性能，尤其在数据稀缺时表现优异。
...
### [219] [Approximation and Generalization Abilities of Score-based Neural Network Generative Models for Sub-Gaussian Distributions](https://arxiv.org/abs/2505.10880)
...
### [220] [Prior-Guided Diffusion Planning for Offline Reinforcement Learning](https://arxiv.org/abs/2505.10881)
*Donghyeon Ki,JunHyeok Oh,Seong-Woong Shim,Byung-Jun Lee*

Main category: cs.LG

TL;DR: 论文提出Prior Guidance (PG)框架，通过可学习分布替代高斯先验，优化扩散模型的行为克隆，解决现有引导采样策略的不足。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型引导采样策略在多模态动作生成、分布漂移或推理成本方面存在不足，需改进。

Method: 提出Prior Guidance (PG)，用可学习分布替代高斯先验，通过行为正则化目标优化，直接生成高价值轨迹。

Result: PG在多样化的长时域离线强化学习基准测试中优于现有扩散策略和规划器。

Conclusion: PG框架有效解决了现有方法的不足，提升了扩散模型在离线强化学习中的性能。
...
### [221] [Global Convergence of Adaptive Sensing for Principal Eigenvector Estimation](https://arxiv.org/abs/2505.10882)
*Alex Saad-Falcon,Brighton Ancelin,Justin Romberg*

Main category: cs.LG

TL;DR: 论文提出了一种基于压缩采样的自适应感知Oja算法变体，用于高效高维PCA，证明了其在噪声环境下的全局收敛性，并简化了证明方法。


<details>
  <summary>Details</summary>
Motivation: 传统PCA方法在高维空间中计算成本高，而Oja等子空间跟踪算法需要全维观测。本文旨在解决这些问题。

Method: 通过每次迭代仅采集两个压缩测量（一个沿当前估计方向，一个随机正交方向），分析其收敛性。

Result: 算法在噪声下全局收敛，分为预热和局部收敛两阶段，收敛速度与现有下界一致。

Conclusion: 该方法为难以获取全维样本的应用提供了高效解决方案，且证明方法更简洁。
...
### [222] [Multi-Objective Preference Optimization: Improving Human Alignment of Generative Models](https://arxiv.org/abs/2505.10892)
*Akhil Agnihotri,Rahul Jain,Deepak Ramachandran,Zheng Wen*

Main category: cs.LG

TL;DR: 论文提出了一种多目标偏好优化算法（MOPO），用于解决LLMs在多个潜在冲突目标下的对齐问题，无需将目标聚合为单一目标。


<details>
  <summary>Details</summary>
Motivation: 现有偏好优化技术（如DPO、IPO）只能处理单一目标，而实际应用中用户有多目标需求（如帮助性和无害性），缺乏自然聚合方法。

Method: MOPO算法通过约束KL正则化优化框架，主目标最大化，次目标通过可调安全阈值下界约束，直接利用成对偏好数据，无需点奖励假设或启发式提示工程。

Result: 在合成基准测试中，MOPO逼近Pareto前沿；在1.3B参数语言模型上，MOPO获得更高奖励且Pareto优于基线，消融实验验证了稳定性和超参数鲁棒性。

Conclusion: MOPO有效解决了多目标偏好对齐问题，适用于大规模训练，并在实际应用中优于现有方法。
...
### [223] [CTP: A hybrid CNN-Transformer-PINN model for ocean front forecasting](https://arxiv.org/abs/2505.10894)
*Yishuo Wang,Feng Zhou,Muping Zhou,Qicheng Meng,Zhijun Hu,Yi Wang*

Main category: cs.LG

TL;DR: CTP是一种结合CNN、Transformer和PINN的新型深度学习框架，用于海洋锋面预测，显著优于现有方法。
...
Method: CTP整合了局部空间编码、长程时间注意力和物理约束，解决了现有问题。

Result: 在南海和黑潮区域的实验中，CTP在单步和多步预测中均达到SOTA性能。

Conclusion: CTP通过多架构融合显著提升了海洋锋面预测的准确性和稳定性。
...
### [224] [Automated Identification of Logical Errors in Programs: Advancing Scalable Analysis of Student Misconceptions](https://arxiv.org/abs/2505.10913)
...
TL;DR: 论文提出了一种基于可解释的抽象语法树（AST）嵌入模型（SANN）的框架，用于自动检测学生编程中的逻辑错误，以提升编程教育效果。


<details>
  <summary>Details</summary>
Motivation: 在计算机科学教育中，识别学生编程困难的根源对提供有效学习支持至关重要。通过分析学生代码中的逻辑错误，可以深入了解学习障碍并提供针对性帮助。

Method: 采用基于子树注意力神经网络（SANN）的AST嵌入模型，自动识别程序结构中的逻辑错误。

Result: 实验表明，该框架能准确捕捉学生的逻辑错误，并深入理解其学习过程。
...
### [225] [A Dataset for Spatiotemporal-Sensitive POI Question Answering](https://arxiv.org/abs/2505.10928)
*Xiao Han,Dayan Pan,Xiangyu Zhao,Xuyuan Hu,Zhaolin Deng,Xiangjie Kong,Guojiang Shen*

Main category: cs.LG

TL;DR: POI-QA是一个新的时空敏感问答数据集，用于评估模型在时空推理任务中的表现，填补了现有QA数据集的不足。
...
Method: 通过挖掘和整合开源车辆轨迹数据与高精度POI数据，手动验证时空事实，并生成双语QA对。

Result: 评估显示，即使是性能最佳的模型（Qwen2.5-7B）在简单任务上的HR@10仅为0.41，远低于人类的0.56。

Conclusion: POI-QA是一个有效的基准数据集，揭示了LLM在时空推理中的局限性，并推动了相关算法的发展。
...
### [226] [Physics-informed Temporal Alignment for Auto-regressive PDE Foundation Models](https://arxiv.org/abs/2505.10930)
*Congcong Zhu,Xiaoyan Xu,Jiayue Han,Jingrun Chen*

Main category: cs.LG

TL;DR: 论文提出了一种名为PITA的自监督学习框架，用于解决自回归PDE基础模型在时间依赖数据中的误差累积问题。


<details>
  <summary>Details</summary>
Motivation: 自回归PDE基础模型在处理时间依赖数据时存在误差累积问题，特别是在分布外数据上表现不佳。

Method: PITA通过物理约束的自监督学习框架，对齐不同时间步的物理动态，无需依赖已知物理先验。

Result: 实验表明，PITA显著提升了现有基础模型在时间依赖PDE数据上的准确性和鲁棒性。

Conclusion: PITA为解决自回归PDE模型的误差累积问题提供了一种有效方法，具有强大的泛化能力。
...
### [227] [Privacy-Aware Lifelong Learning](https://arxiv.org/abs/2505.10941)
*Ozan Özdenizci,Elmar Rueckert,Robert Legenstein*

Main category: cs.LG

TL;DR: 论文提出了一种隐私感知的终身学习（PALL）方法，结合终身学习和隐私保护机制，解决模型在增量学习中选择性遗忘敏感信息的挑战。


<details>
  <summary>Details</summary>
Motivation: 终身学习算法需增量获取新知识而不遗忘旧知识，而机器遗忘则需选择性删除敏感信息以满足隐私法规。两者目标矛盾，现有方法未充分解决。

Method: PALL通过优化任务特定的稀疏子网络和参数共享，结合记忆回放机制，实现精确遗忘且不降低性能。

Result: 实验证明PALL在图像分类任务中具有可扩展性，是首个集成终身学习和隐私遗忘的先进解决方案。

Conclusion: PALL为负责任AI提供了一种高效、隐私保护的终身学习方法，解决了矛盾目标的关键挑战。
...
### [228] [Certifying Stability of Reinforcement Learning Policies using Generalized Lyapunov Functions](https://arxiv.org/abs/2505.10947)
*Kehan Long,Jorge Cortés,Nikolay Atanasov*

Main category: cs.LG

TL;DR: 论文提出了一种通过增强RL值函数与神经网络残差项来学习广义Lyapunov函数的方法，以证明学习控制策略的稳定性，并扩展了经典Lyapunov方法。


<details>
  <summary>Details</summary>
Motivation: 传统Lyapunov方法难以构造学习控制策略的稳定性证明，而RL策略的值函数是自然的Lyapunov候选函数，但需要改进。

Method: 研究LQR问题，提出通过增强值函数与残差项构造Lyapunov函数，并放宽Lyapunov下降条件为多步平均下降。在非线性场景中，用神经网络残差项增强RL值函数学习广义Lyapunov函数。

Result: 成功证明了Gymnasium和DeepMind Control基准上RL策略的稳定性，并通过多步Lyapunov损失联合训练神经控制器和稳定性证书，扩大了吸引区域的认证内近似。

Conclusion: 该方法通过简化稳定性证书的构造，为学习策略的广泛系统提供了稳定性证明，连接了经典控制理论与现代学习方法。
...
### [229] [FP64 is All You Need: Rethinking Failure Modes in Physics-Informed Neural Networks](https://arxiv.org/abs/2505.10949)
*Chenhui Xu,Dancheng Liu,Amir Nassereldine,Jinjun Xiong*

Main category: cs.LG

TL;DR: PINNs的失败模式通常归因于局部最优解，但研究发现实际原因是算术精度不足（FP32）。改用FP64后，优化器能成功求解PDE，揭示了精度导致的训练停滞而非局部最优。


<details>
  <summary>Details</summary>
Motivation: 挑战传统观点，揭示PINNs失败模式的真正原因是算术精度不足，而非局部最优解。

Method: 通过比较FP32和FP64精度下的LBFGS优化器表现，分析PINNs的训练动态。

Result: FP64精度下，PINNs能成功求解PDE，避免了FP32导致的训练停滞。

Conclusion: 算术精度是神经网络可靠求解PDE的关键因素，FP64能有效避免失败模式。
...
### [230] [Shackled Dancing: A Bit-Locked Diffusion Algorithm for Lossless and Controllable Image Steganography](https://arxiv.org/abs/2505.10950)
*Tianshuo Zhang,Gao Jia,Wenzhe Zhai,Rui Yann,Xianglei Xing*

Main category: cs.LG

TL;DR: SD²是一种基于扩散模型的生成隐写方法，通过结合比特位锁定和扩散采样注入，实现可控信息嵌入，平衡了安全性、容量和图像质量。
...
Method: SD²结合比特位锁定与扩散采样注入，在生成轨迹中实现可控信息嵌入，利用扩散模型的表达能力合成多样化载体图像。

Result: SD²在100%准确率下实现完整信息恢复，显著优于现有方法，在安全性、嵌入容量和稳定性上表现优异。
...
### [231] [SubGCache: Accelerating Graph-based RAG with Subgraph-level KV Cache](https://arxiv.org/abs/2505.10951)
*Qiuyu Zhu,Liang Zhang,Qianxiong Xu,Cheng Long,Jie Zhang*

Main category: cs.LG

TL;DR: SubGCache通过聚类查询并重用预计算的关键值缓存，显著减少了图检索增强生成的推理延迟。


<details>
  <summary>Details</summary>
Motivation: 针对不同查询可能检索到相似子图作为提示的问题，提出SubGCache以减少计算开销。

Method: 基于子图嵌入聚类查询，为每个聚类构建代表性子图并预计算其KV缓存，查询时直接重用。

Result: 实验表明，SubGCache在保持生成质量的同时，推理延迟显著降低，TTFT最高减少6.68倍。

Conclusion: SubGCache是一种高效的方法，适用于图检索增强生成，显著提升性能。
...
### [232] [Constrained Preferential Bayesian Optimization and Its Application in Banner Ad Design](https://arxiv.org/abs/2505.10954)
*Koki Iwai,Yusuke Kumagae,Yuki Koyama,Masahiro Hamasaki,Masataka Goto*

Main category: cs.LG

TL;DR: 论文提出了一种新的约束偏好贝叶斯优化（CPBO）方法，首次将不等式约束引入偏好贝叶斯优化（PBO），适用于实际优化任务。
...
Method: 提出了一种新的获取函数，扩展PBO以支持不等式约束，专注于探索可行区域。

Result: 技术评估表明CPBO能有效找到最优解。实际应用中，通过设计师参与的广告设计系统验证了其潜力。

Conclusion: CPBO填补了PBO在约束优化中的空白，展示了在现实约束下指导创意设计的价值。
...
### [233] [Relational Graph Transformer](https://arxiv.org/abs/2505.10960)
...
TL;DR: RelGT是一种专为关系表设计的图变换器架构，通过多元素标记化策略和局部-全局注意力机制，显著提升了关系深度学习任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有图神经网络（GNN）在捕捉关系数据的复杂结构和长程依赖方面存在局限性，而图变换器在关系实体图上的应用面临独特挑战。

Method: RelGT采用多元素标记化策略（特征、类型、跳数距离、时间和局部结构），结合局部子图注意力和全局可学习中心注意力。

Result: 在RelBench基准测试的21个任务中，RelGT性能优于GNN基线，最高提升18%。

Conclusion: RelGT证明了图变换器在关系深度学习中的潜力，为多表关系数据建模提供了高效解决方案。
...
### [234] [Group-in-Group Policy Optimization for LLM Agent Training](https://arxiv.org/abs/2505.10978)
*Lang Feng,Zhenghai Xue,Tingcong Liu,Bo An*

Main category: cs.LG

TL;DR: 提出了一种名为GiGPO的新型强化学习算法，用于解决长时程LLM代理训练中的信用分配问题，通过两层结构实现细粒度信用分配，并在实验中表现出显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前基于群体的强化学习在长时程LLM代理训练中面临信用分配困难的问题，GiGPO旨在解决这一挑战。

Method: GiGPO采用两层结构：在情节级别计算宏观相对优势，在步骤级别通过锚状态分组机制估计微观相对优势。

Result: 在ALFWorld和WebShop基准测试中，GiGPO性能提升超过12%和9%，同时保持相同的GPU内存开销和额外时间成本。

Conclusion: GiGPO通过细粒度信用分配显著提升了长时程LLM代理训练的性能，且无需额外资源开销。
...
### [235] [GenoArmory: A Unified Evaluation Framework for Adversarial Attacks on Genomic Foundation Models](https://arxiv.org/abs/2505.10983)
...
<details>
  <summary>Details</summary>
Motivation: 现有GFM基准缺乏对抗攻击评估，GenoArmory填补这一空白。

Method: 评估五种GFMs的对抗鲁棒性，使用四种攻击算法和三种防御策略。

Result: 分类模型比生成模型更具鲁棒性；对抗攻击常针对生物重要区域。

Conclusion: GenoArmory为GFM安全性提供重要工具，揭示模型脆弱性与任务类型和生物特征的关系。
...
### [236] [ReaCritic: Large Reasoning Transformer-based DRL Critic-model Scaling For Heterogeneous Networks](https://arxiv.org/abs/2505.10992)
*Feiran You,Hongyang Du*

Main category: cs.LG

TL;DR: 论文提出ReaCritic，一种基于大型推理Transformer的批评模型，通过水平和垂直推理提升DRL在异构网络中的性能。
...
Method: ReaCritic结合水平推理（并行状态-动作输入）和垂直推理（深层Transformer堆栈），兼容多种DRL算法。
...
### [237] [Logo-LLM: Local and Global Modeling with Large Language Models for Time Series Forecasting](https://arxiv.org/abs/2505.11017)
*Wenjie Ou,Zhishuo Zhao,Dongyue Guo,Yi Lin*

Main category: cs.LG

TL;DR: Logo-LLM是一个基于LLM的时间序列预测框架，通过提取多尺度特征并整合局部与全局信息，显著提升了预测性能。


<details>
  <summary>Details</summary>
Motivation: Transformer和LLM方法在时间序列预测中忽视了局部动态特征，Logo-LLM旨在解决这一问题。

Method: 利用预训练LLM的不同层提取局部和全局特征，并引入Local-Mixer和Global-Mixer模块进行特征整合。

Result: Logo-LLM在多种基准测试中表现优异，尤其在少样本和零样本场景下泛化能力强。

Conclusion: Logo-LLM通过多尺度特征建模，显著提升了时间序列预测的准确性和泛化能力。
...
### [238] [Informed, but Not Always Improved: Challenging the Benefit of Background Knowledge in GNNs](https://arxiv.org/abs/2505.11023)
*Kutalmış Coşkun,Ivo Kavisanczki,Amin Mirzaei,Tom Siegl,Bjarne C. Hiller,Stefan Lüdtke,Martin Becker*

Main category: cs.LG

TL;DR: 研究发现，在癌症亚型分类任务中，使用背景知识（BK）的图神经网络（GNN）表现与无背景知识的模型相当，且BK图的扰动对其影响有限。通过评估框架发现，GNN架构与BK特性的对齐是关键。


<details>
  <summary>Details</summary>
Motivation: 探究背景知识（BK）在图机器学习中的作用及其对模型性能的实际贡献，尤其是在低数据领域如生物医学研究中。

Method: 引入评估框架，包括合成场景和BK图扰动测试，分析GNN在合成和真实生物医学数据中的表现。

Result: 发现BK对GNN性能提升有限，且模型对BK图的扰动不敏感；但GNN架构与BK特性的对齐可能带来显著改进。

Conclusion: GNN与BK特性的精确对齐是提升性能的关键，未来研究需关注如何优化这种对齐。
...
### [239] [Leveraging Real-Time Data Analysis and Multiple Kernel Learning for Manufacturing of Innovative Steels](https://arxiv.org/abs/2505.11024)
...
<details>
  <summary>Details</summary>
Motivation: 热喷涂组件在钢铁制造中的标准化翻新过程可能难以满足动态需求，因此需要改进工艺。

Method: 设计了数据聚合器和质量预测器，结合实时监控和数据驱动方法，采用多核学习策略实现预测质量管理。

Result: 小规模测试验证了该组合能准确预测涂层质量，并在发现显著偏差时主动通知操作员。

Conclusion: 该方法通过实时数据分析和预测质量管理，有效提升了热喷涂涂层工艺的适应性和质量。
...
### [240] [Exploiting the Asymmetric Uncertainty Structure of Pre-trained VLMs on the Unit Hypersphere](https://arxiv.org/abs/2505.11029)
*Li Ju,Max Andersson,Stina Fredriksson,Edward Glöckner,Andreas Hellander,Ekta Vats,Prashant Singh*

Main category: cs.LG

TL;DR: AsymVLM提出了一种在单位超球面上构建概率嵌入的方法，解决了现有确定性视觉语言模型无法捕捉模态不对称不确定性的问题。


<details>
  <summary>Details</summary>
Motivation: 确定性视觉语言模型（VLMs）无法处理自然语言和视觉数据中的模糊性和不确定性，现有概率后适应方法未考虑模态不对称性及单位超球面约束。

Method: 提出AsymVLM，在预训练VLMs的基础上，于单位超球面上构建概率嵌入，量化不确定性。

Result: 在基准测试中验证了概率嵌入的有效性，并通过消融研究证明了文本和视觉数据不确定性结构的不对称性。

Conclusion: AsymVLM通过考虑模态不对称性和单位超球面约束，显著提升了不确定性量化性能。
...
### [241] [Deep Latent Variable Model based Vertical Federated Learning with Flexible Alignment and Labeling Scenarios](https://arxiv.org/abs/2505.11035)
*Kihun Hong,Sejun Park,Ganguk Hwang*

Main category: cs.LG

TL;DR: 论文提出了一种垂直联邦学习（VFL）的统一框架，解决了数据对齐、未标记数据和多机构协作的问题，实验表现优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有VFL方法对数据对齐、参与方数量和标记数据有严格限制，限制了其实际应用。

Method: 将VFL中的对齐问题重新解释为缺失数据问题，提出一个支持任意对齐和标记场景的统一框架。
...
### [242] [Efficient Attention via Pre-Scoring: Prioritizing Informative Keys in Transformers](https://arxiv.org/abs/2505.11040)
*Zhexiang Li,Haoyu Wang,Yutong Bao,David Woodruff*

Main category: cs.LG

TL;DR: 论文提出了一种预评分机制，改进HyperAttention的关键词捕获能力，通过K-means、K-median聚类和杠杆评分方法，显著降低困惑度并提升效率。


<details>
  <summary>Details</summary>
Motivation: HyperAttention的均匀残差采样限制了关键键的捕获，导致困惑度上升，需改进以提升性能。

Method: 引入三种预评分方法（K-means、K-median聚类和杠杆评分），完全替代HyperAttention的原始采样机制。

Result: 在ChatGLM2上困惑度从12降至8.3，ViT上精度与LevAttention相当，速度比FlashAttention快20倍。

Conclusion: 预评分机制有效提升Transformer效率，平衡速度与建模精度。
...
### [243] [Exploration by Random Distribution Distillation](https://arxiv.org/abs/2505.11044)
*Zhirui Fang,Kai Yang,Jian Tao,Jiafei Lyu,Lusong Li,Li Shen,Xiu Li*

Main category: cs.LG

TL;DR: 提出了一种名为RDD的新方法，通过将目标网络的输出建模为正态分布样本，结合伪计数和差异项，统一了计数型和预测误差型探索方法。


<details>
  <summary>Details</summary>
Motivation: 在线强化学习中探索是一个关键挑战，现有方法（如计数型和预测误差型）各有局限，需要一种更有效的统一方法。

Method: RDD方法通过采样目标网络的输出为正态分布样本，将预测网络与目标网络的差异作为内在奖励，结合伪计数和差异项。

Result: RDD在实验中优于现有方法，理论分析和实验结果均验证了其有效性。

Conclusion: RDD成功统一了两种探索方法，在高维空间中表现出色，并实现了内在奖励的衰减模式。
...
### [244] [Halting Recurrent GNNs and the Graded $μ$-Calculus](https://arxiv.org/abs/2505.11050)
*Jeroen Bollen,Jan Van den Bussche,Stijn Vansummeren,Jonni Virtema*

Main category: cs.LG

TL;DR: 提出了一种用于循环图神经网络（GNN）的停止机制，证明其能够表达所有可定义在分级模态μ-演算中的节点分类器，并开发了新的近似语义和模型检查算法。


<details>
  <summary>Details</summary>
Motivation: 当前循环GNN要么假设图大小已知，要么缺乏终止保证，需要一种新的停止机制来解决这些问题。

Method: 提出了一种停止机制，开发了新的分级μ-演算近似语义和计数算法，并将其实现到循环GNN中。

Result: 证明了停止模型能够表达分级模态μ-演算中的所有节点分类器，并开发了不依赖图大小的模型检查算法。

Conclusion: 新方法解决了循环GNN的终止问题，并扩展了其表达能力，为相关研究提供了新工具。
...
### [245] [NeuralSurv: Deep Survival Analysis with Bayesian Uncertainty Quantification](https://arxiv.org/abs/2505.11054)
*Mélodie Monod,Alessandro Micheli,Samir Bhatt*

Main category: cs.LG

TL;DR: NeuralSurv是首个结合贝叶斯不确定性量化的深度生存模型，通过两阶段数据增强方案灵活捕捉时间变化的协变量-风险关系，并在实验中表现出优于现有模型的校准性能。


<details>
  <summary>Details</summary>
Motivation: 传统深度生存模型缺乏不确定性量化能力，而贝叶斯方法在数据稀缺时能提供更稳健的模型校准和不确定性估计。

Method: 提出非参数、架构无关的框架，采用两阶段数据增强方案，并设计高效的均值场变分算法进行后验推断。

Result: 在合成基准和真实数据集中，NeuralSurv的校准性能优于现有模型，同时保持或超越其判别性能。

Conclusion: 贝叶斯方法显著提升了模型校准能力，并为生存函数提供了稳健且校准良好的不确定性估计。
...
### [246] [Assessing the Performance of Analog Training for Transfer Learning](https://arxiv.org/abs/2505.11067)
...
<details>
  <summary>Details</summary>
Motivation: 当前模拟内存计算的训练算法因设备非线性和不对称性而表现不佳，需要更鲁棒的算法。

Method: 采用c-TTv2算法，利用chopped技术解决设备非线性和不对称性问题，并在Swin-ViT模型上进行实验。

Result: c-TTv2算法在CIFAR100数据集上表现出色，对设备规格变化具有鲁棒性。

Conclusion: c-TTv2算法为模拟内存计算提供了一种有效的训练解决方案。
...
### [247] [Addition is almost all you need: Compressing neural networks with double binary factorization](https://arxiv.org/abs/2505.11076)
*Vladimír Boža,Vladimír Macko*

Main category: cs.LG

TL;DR: 论文提出了一种名为Double Binary Factorization (DBF)的新方法，通过将权重矩阵分解为两个二进制矩阵和缩放向量，在保持计算效率的同时显著提升了压缩率和准确性。


<details>
  <summary>Details</summary>
Motivation: 为了解决大型语言模型（LLMs）的计算和存储需求，二进制量化方法虽然高效，但会导致显著的精度下降。

Method: DBF将密集权重矩阵分解为两个二进制矩阵和缩放向量的乘积，支持通过调整中间维度精细控制压缩率。

Result: 在1-bit/weight范围内，DBF优于现有二值化方法；在2-bit/weight范围内，与QuIP#和QTIP等最佳量化方法竞争。

Conclusion: DBF不仅高效且灵活，还支持基于通道剪枝标准的非均匀分层压缩比估计。
...
### [248] [ShiQ: Bringing back Bellman to LLMs](https://arxiv.org/abs/2505.11081)
...
TL;DR: 论文提出了一种基于Q学习的损失函数ShiQ，用于优化预训练大语言模型（LLM），解决了传统Q学习在LLM中直接应用的无效性问题。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习（RL）在LLM中多采用直接策略优化，而Q学习方法因其样本高效性和离线学习能力在非LLM任务中表现优异，但在LLM中直接应用无效。论文旨在解决这一问题。

Method: 通过从Bellman方程推导理论基础的损失函数，将Q学习方法适配到LLM，并开发了ShiQ算法，支持离策略、令牌级学习。

Result: 在合成数据和真实基准（如UltraFeedback和BFCL-V3）上验证了ShiQ在单轮和多轮LLM任务中的有效性。

Conclusion: ShiQ为LLM的Q学习提供了一种高效且实用的解决方案，扩展了RL在LLM中的应用范围。
...
### [249] [Fault Diagnosis across Heterogeneous Domains via Self-Adaptive Temporal-Spatial Attention and Sample Generation](https://arxiv.org/abs/2505.11083)
*Guangqiang Li,M. Amine Atoui,Xiangshun Li*

Main category: cs.LG

TL;DR: 提出了一种名为TSA-SAN的自适应时空注意力网络，用于解决多模式过程中健康状态类别部分重叠的故障诊断问题。


<details>
  <summary>Details</summary>
Motivation: 现实工业场景中，多模式过程的健康状态类别通常仅部分重叠，数据不完整和分布差异大对现有故障诊断方法构成挑战。

Method: 通过健康类别数据构建模式间映射生成多模式样本，插值增强故障数据多样性，结合自适应实例归一化和时空注意力机制训练模型。
...
### [250] [A Fast Kernel-based Conditional Independence test with Application to Causal Discovery](https://arxiv.org/abs/2505.11085)
*Oliver Schacht,Biwei Huang*

Main category: cs.LG

TL;DR: FastKCI是一种基于核的条件独立性测试方法，通过并行化和分区处理解决了KCI测试计算复杂度高的问题，适用于大规模数据。


<details>
  <summary>Details</summary>
Motivation: KCI测试在因果发现中虽强大，但计算复杂度高，限制了其在大规模数据中的应用。FastKCI旨在解决这一瓶颈。

Method: 采用混合专家方法，基于高斯混合模型对条件变量分区，并行执行局部KCI测试，并通过重要性加权采样汇总结果。

Result: 实验表明，FastKCI在保持统计功效的同时显著提升了计算速度。
...
### [251] [Bidirectional Distillation: A Mixed-Play Framework for Multi-Agent Generalizable Behaviors](https://arxiv.org/abs/2505.11100)
*Lang Feng,Jiahao Lin,Dong Xing,Li Zhang,De Ma,Gang Pan*

Main category: cs.LG

TL;DR: BiDist提出了一种双向蒸馏框架，解决MARL中未见合作者的泛化问题，通过前向和反向蒸馏扩展策略空间。


<details>
  <summary>Details</summary>
Motivation: 现有自学习方法局限于内部空间泛化，无法应对未见合作者。

Method: BiDist采用双向蒸馏：前向蒸馏模拟历史策略空间，反向蒸馏探索新分布。

Result: BiDist在合作、竞争和社会困境任务中表现出卓越泛化能力，显著扩展策略空间。
...
### [252] [Inferring the Most Similar Variable-length Subsequences between Multidimensional Time Series](https://arxiv.org/abs/2505.11106)
*Thanadej Rattanakornphan,Piyanon Charoenpoonpanich,Chainarong Amornbunchornvej*

Main category: cs.LG

TL;DR: 提出一种高效算法，用于在多维时间序列中寻找长度不同的最相似子序列，并在模拟和真实数据集中验证其正确性和效率。
...
Method: 基于理论保证的算法，能够精确找到长度不同的多维子序列的最相似匹配。

Result: 在模拟数据中，算法运行时间仅为基线方法的四分之一；在真实数据中，速度提升高达20倍，并能提取有意义的模式。
...
### [253] [Maximizing Asynchronicity in Event-based Neural Networks](https://arxiv.org/abs/2505.11165)
*Haiqing Hao,Nikola Zubić,Weihua He,Zhipeng Sui,Davide Scaramuzza,Wenhui Wang*

Main category: cs.LG

TL;DR: EVA（EVent Asynchronous）是一种新型异步到同步（A2S）框架，通过借鉴语言建模技术，提升了事件相机的表示表达能力与泛化能力，在识别与检测任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 事件相机的高时间分辨率与低冗余特性使其在视觉应用中具有潜力，但异步稀疏数据难以与标准ML兼容。现有A2S方法牺牲了表示能力，亟需改进。

Method: EVA框架结合线性注意力和自监督学习技术，借鉴语言建模方法，逐事件生成高表达性表示。

Result: EVA在DVS128-Gesture和N-Cars识别任务中超越现有方法，并在Gen1检测任务中达到47.7 mAP，首次验证A2S框架在检测任务中的可行性。

Conclusion: EVA通过创新设计显著提升了事件相机的ML兼容性，为实时事件视觉应用开辟了新方向。
...
### [254] [FairSHAP: Preprocessing for Fairness Through Attribution-Based Data Augmentation](https://arxiv.org/abs/2505.11111)
*Lin Zhu,Yijun Bian,Lei You*

Main category: cs.LG

TL;DR: FairSHAP是一个新颖的预处理框架，利用Shapley值属性提升个体和群体公平性，通过透明机制识别和修改不公平特征。


<details>
  <summary>Details</summary>
Motivation: 在高风险领域中，机器学习模型的公平性至关重要，现有预处理方法缺乏透明性，无法明确不公平特征的来源。

Method: FairSHAP通过Shapley值识别关键实例，并通过实例级匹配修改数据，减少歧视风险，同时保持数据完整性和模型准确性。

Result: FairSHAP显著提升了人口平等和机会平等，并在某些情况下提高了预测性能。

Conclusion: FairSHAP是一种模型无关且透明的方法，可无缝集成到现有机器学习流程中，并提供关于偏见的可操作见解。
...
### [255] [Dual-Balancing for Physics-Informed Neural Networks](https://arxiv.org/abs/2505.11117)
*Chenhong Zhou,Jie Chen,Zaifeng Yang,Ching Eng Png*

Main category: cs.LG

TL;DR: 提出了一种新型Dual-Balanced PINN（DB-PINN），通过动态调整损失权重解决PINNs中的多目标优化问题，显著提高了收敛速度和预测精度。


<details>
  <summary>Details</summary>
Motivation: 传统PINNs因多目标优化问题导致精度低、收敛慢，需要一种新方法解决梯度分布和条件拟合的不平衡问题。

Method: DB-PINN通过动态调整损失权重，结合inter-balancing和intra-balancing，分别解决梯度分布和条件拟合的不平衡问题，并引入稳健的权重更新策略。

Result: 实验表明，DB-PINN在收敛速度和预测精度上显著优于其他基于梯度的权重调整方法。

Conclusion: DB-PINN通过动态平衡损失权重，有效解决了PINNs中的不平衡问题，提升了性能。
...
### [256] [GraphOracle: A Foundation Model for Knowledge Graph Reasoning](https://arxiv.org/abs/2505.11125)
*Enjun Du,Siyi Liu,Yongqi Zhang*

Main category: cs.LG

TL;DR: GraphOracle是一种基于关系依赖图（RDG）的知识图谱基础模型，通过查询依赖的注意力机制和预训练实现跨领域推理，性能提升高达35%。


<details>
  <summary>Details</summary>
Motivation: 知识图谱的动态性和跨领域推理需求使得开发基础模型具有挑战性。

Method: 将知识图谱转换为关系依赖图（RDG），开发查询依赖的注意力机制，预训练后快速微调。

Result: 在31个基准测试中表现最优，性能提升高达35%。

Conclusion: GraphOracle在跨领域推理和性能提升方面表现出色。
...
### [257] [FedDuA: Doubly Adaptive Federated Learning](https://arxiv.org/abs/2505.11126)
*Shokichi Takakura,Seng Pei Liew,Satoshi Hasegawa*

Main category: cs.LG

TL;DR: FedDuA是一种新的联邦学习框架，通过自适应选择全局学习率解决FedAvg的收敛慢问题，无需额外通信或计算成本。


<details>
  <summary>Details</summary>
Motivation: FedAvg在异构数据和参数空间各向异性下收敛慢，需要优化。

Method: 通过镜像下降视角形式化中心服务器优化，提出基于客户端间和坐标间异质性的自适应学习率框架FedDuA。

Result: 理论证明其步长规则极小极大最优，实验显示优于基线且对超参数鲁棒。

Conclusion: FedDuA高效且鲁棒，适用于联邦学习中的异构环境。
...
### [258] [What's Inside Your Diffusion Model? A Score-Based Riemannian Metric to Explore the Data Manifold](https://arxiv.org/abs/2505.11128)
...
Method: 引入基于分数的黎曼度量，利用Stein分数函数定义度量张量，保持切向距离的同时拉伸垂直距离，从而生成沿流形轮廓的自然测地线。
...
### [259] [Fairness-aware Anomaly Detection via Fair Projection](https://arxiv.org/abs/2505.11132)
*Feng Xiao,Xiaoying Tang,Jicong Fan*

Main category: cs.LG

TL;DR: 论文提出了一种公平性感知的无监督异常检测方法FairAD，通过将不同人口统计组的数据映射到共同目标分布，确保公平性，并提出了无需阈值的公平性度量。


<details>
  <summary>Details</summary>
Motivation: 在金融、医疗等高社会影响领域，无监督异常检测可能因数据偏差导致不公平，需确保不同群体的公平性。

Method: FairAD通过学习投影将不同组数据映射到共同目标分布，利用密度估计识别异常，并提出阈值无关的公平性度量。

Result: 实验表明，FairAD在平衡和倾斜数据下均能实现检测准确性与公平性的更好平衡。
...
### [260] [Towards Robust Spiking Neural Networks:Mitigating Heterogeneous Training Vulnerability via Dominant Eigencomponent Projection](https://arxiv.org/abs/2505.11134)
...
<details>
  <summary>Details</summary>
Motivation: SNNs' low-energy operation is compromised by vulnerability to data shifts when trained with direct encoding and BPTT.

Method: Proposed Dominant Eigencomponent Projection (DEP) to orthogonally project gradients, reducing Hessian spectral radius.

Result: DEP prevents catastrophic collapse and enhances SNN robustness against heterogeneous data poisoning.

Conclusion: DEP supports safer, more reliable SNN deployment by addressing training vulnerabilities.
...
### [261] [Covariance Density Neural Networks](https://arxiv.org/abs/2505.11139)
*Om Roy,Yashar Moshfeghi,Keith Smith*

Main category: cs.LG

TL;DR: 论文提出了一种基于密度矩阵的图神经网络方法，改进了现有的协方差神经网络（VNN），通过将样本协方差矩阵视为准哈密顿量，提升了网络的判别性和性能。


<details>
  <summary>Details</summary>
Motivation: 现有图神经网络在建模网络数据时缺乏对底层图结构选择的共识，协方差神经网络（VNN）虽使用样本协方差矩阵作为图移位算子（GSO），但仍有改进空间。

Method: 构建密度矩阵，将样本协方差矩阵视为随机变量空间的准哈密顿量，作为GSO，实现数据多尺度提取。

Result: 该方法能显式控制稳定性与判别性的权衡，增强抗噪性，在脑机接口EEG运动想象分类中表现优于EEGnet且速度更快。

Conclusion: 基于协方差密度的神经网络为脑机接口的迁移学习提供了有效解决方案。
...
### [262] [Bi-directional Recurrence Improves Transformer in Partially Observable Markov Decision Processes](https://arxiv.org/abs/2505.11153)
*Ashok Arora,Neetesh Kumar*

Main category: cs.LG

TL;DR: 提出了一种新型双向循环模型架构，用于提升部分可观测马尔可夫决策过程（POMDPs）中的样本效率和减少参数数量。


<details>
  <summary>Details</summary>
Motivation: 在部分可观测环境中，传统方法如循环网络和基于Transformer的模型存在参数过多或效率不足的问题。

Method: 采用单层双向循环单元替代多层前馈网络，以更好地捕捉序列依赖和上下文信息。

Result: 在23个POMDP环境中，新模型平均性能优于现有方法87.39%至482.04%。

Conclusion: 新型双向循环架构显著提升了POMDP任务的样本效率和性能。
...
### [263] [Visual Planning: Let's Think Only with Images](https://arxiv.org/abs/2505.11409)
...
<details>
  <summary>Details</summary>
Motivation: 现有大语言模型（LLMs）和多模态扩展（MLLMs）主要依赖文本进行推理，但在涉及空间和几何信息的任务中，语言可能并非最自然或有效的模态。

Method: 提出视觉规划范式，通过图像序列执行推理，并引入基于GRPO的强化学习框架VPRL，用于后训练大型视觉模型。
...
### [264] [Attention on the Sphere](https://arxiv.org/abs/2505.11157)
*Boris Bonev,Max Rietmann,Andrea Paris,Alberto Carpentieri,Thorsten Kurth*

Main category: cs.LG

TL;DR: 提出了一种球面域的广义注意力机制，使Transformer能直接处理球面数据，通过数值积分权重和邻域注意力提升性能与效率。


<details>
  <summary>Details</summary>
Motivation: 在气象学、宇宙学和机器人学等领域，保持球面对称性和拓扑结构对物理准确性至关重要，需一种能处理球面数据的Transformer。

Method: 集成数值积分权重实现几何保真注意力，提出球面邻域注意力以减少计算复杂度并保留对称性，提供优化CUDA实现。

Result: 在浅水方程模拟、球面图像分割和深度估计任务中，球面Transformer性能优于平面方法。

Conclusion: 球面Transformer通过几何先验在球面域学习中表现优越，验证了其实际应用价值。
...
### [265] [Gaussian Weight Sampling for Scalable, Efficient and Stable Pseudo-Quantization Training](https://arxiv.org/abs/2505.11170)
*Myeonghwan Ahn,Sungjoo Yoo*

Main category: cs.LG

TL;DR: 论文提出了一种伪量化训练（PQT）方法，通过引入浮点友好的噪声分布R，解决了全量化训练（FQT）的稳定性和效率问题。该方法支持低精度浮点参数（如FP6）和高精度噪声（如9-bit），计算开销低（1.40%），内存占用少（2字节/参数），并在GPT2和Llama2模型上表现稳定甚至优于BF16基线。


<details>
  <summary>Details</summary>
Motivation: 全量化训练（FQT）虽然能加速训练，但面临稳定性挑战和指数级搜索空间的问题。伪量化训练（PQT）作为替代方案尚未被充分研究，因此论文探索其实际应用并提出改进方法。

Method: 提出一种浮点友好的噪声分布R，具有随机精度退火等理想特性，通过高效的伪量化（加法后浮点转换）支持低精度浮点参数。

Result: 方法具有可扩展性（支持FP6参数和9-bit噪声）、高效性（A100 GPU上计算开销仅1.40%，内存占用2字节/参数）和稳定性（在GPT2和Llama2模型上表现优于BF16基线）。

Conclusion: PQT结合高斯权重采样为低精度浮点参数提供了有效的理论基础，显著提升了训练效率和稳定性。
...
### [266] [VitaGraph: Building a Knowledge Graph for Biologically Relevant Learning Tasks](https://arxiv.org/abs/2505.11185)
...
TL;DR: 本文提出了一种通过整合和优化公开数据集构建的多用途生物知识图谱，用于提升计算生物学和精准医学研究的可靠性。


<details>
  <summary>Details</summary>
Motivation: 人类生物学的复杂性对科学研究提出了挑战，而高质量的数据是机器学习模型可靠预测的基础。

Method: 通过清理不一致和冗余数据、整合主要公共数据源，并丰富图谱节点特征（如分子指纹和基因本体），构建了一个多用途生物知识图谱。

Result: 生成的图谱为计算生物学和精准医学提供了先进的研究平台，并在药物重定位、蛋白质相互作用预测和副作用预测等任务中表现出色。

Conclusion: 该生物知识图谱为图机器学习和网络医学模型提供了可靠的基准数据集，推动了相关领域的研究进展。
...
### [267] [Modeling Cell Dynamics and Interactions with Unbalanced Mean Field Schrödinger Bridge](https://arxiv.org/abs/2505.11197)
*Zhenyi Zhang,Zihan Wang,Yuhao Sun,Tiejun Li,Peijie Zhou*

Main category: cs.LG

TL;DR: 论文提出了一种名为UMFSB的框架和CytoBridge算法，用于从稀疏时间分辨的快照数据中建模细胞间相互作用的不平衡随机动态。


<details>
  <summary>Details</summary>
Motivation: 现有方法在建模细胞间相互作用方面存在局限性，而细胞间通讯对细胞状态转换动态至关重要。

Method: 提出UMFSB框架和CytoBridge算法，通过神经网络显式建模细胞转换、增殖和相互作用。

Result: 在合成基因调控数据和真实scRNA-seq数据集上验证，CytoBridge能更准确地识别生长、转换和相互作用模式。

Conclusion: CytoBridge优于现有方法，能消除虚假转换并更精确重建发育景观。
...
### [268] [RanDeS: Randomized Delta Superposition for Multi-Model Compression](https://arxiv.org/abs/2505.11204)
*Hangyu Zhou,Aaron Gokaslan,Volodymyr Kuleshov,Bharath Hariharan*

Main category: cs.LG

TL;DR: 论文提出了一种通过随机正交变换减少模型合并中任务干扰的方法，显著提升了性能，且无需额外内存开销。
...
Method: 将模型合并重新定义为压缩-检索方案，使用随机正交变换对参数向量进行去相关处理。

Result: 该方法显著减少了干扰，在视觉和语言任务中均提升了性能。

Conclusion: 随机正交变换方法支持高效灵活的多模型服务，无需额外内存且计算开销低。
...
### [269] [Minimizing False-Positive Attributions in Explanations of Non-Linear Models](https://arxiv.org/abs/2505.11210)
*Anders Gjølbye,Stefan Haufe,Lars Kai Hansen*

Main category: cs.LG

TL;DR: PatternLocal是一种新的XAI技术，通过将判别模型权重转换为生成表示，有效减少抑制变量的影响，提升解释的可靠性。


<details>
  <summary>Details</summary>
Motivation: 抑制变量可能影响模型预测但无关目标结果，导致XAI方法产生假阳性特征归因，降低解释的实用性。现有方法主要适用于线性模型，对非线性模型和实例解释的扩展有限。

Method: PatternLocal基于局部线性替代模型（如LIME、KernelSHAP或梯度方法），将判别模型权重转换为生成表示，抑制抑制变量的影响。

Result: 在XAI-TRIS基准测试中，PatternLocal表现优于其他XAI方法，显著减少非线性任务中的假阳性归因。

Conclusion: PatternLocal通过抑制抑制变量，提升了XAI方法的可靠性和实用性，适用于非线性模型的解释。
...
### [270] [Bayesian Hierarchical Invariant Prediction](https://arxiv.org/abs/2505.11211)
*Francisco Madaleno,Pernille Julie Viuff Sand,Francisco C. Pereira,Sergio Hernan Garrido Mejia*

Main category: cs.LG

TL;DR: BHIP通过层次贝叶斯框架改进ICP，提升计算可扩展性并支持先验信息，测试了两种稀疏先验，验证了其在合成和真实数据中的潜力。


<details>
  <summary>Details</summary>
Motivation: 解决ICP在异构数据下因果机制不变性测试的局限性，并提升计算效率。

Method: 采用层次贝叶斯框架，测试马掌钉和钉板两种稀疏先验，明确测试因果机制的不变性。

Result: 在合成和真实数据中表现优于ICP，能更可靠地识别因果特征。

Conclusion: BHIP作为一种替代ICP的推断方法，具有潜力。
...
### [271] [Sample Efficient Reinforcement Learning via Large Vision Language Model Distillation](https://arxiv.org/abs/2505.11221)
*Donghoon Lee,Tung M. Luu,Younghwan Lee,Chang D. Yoo*

Main category: cs.LG

TL;DR: LVLM2P框架通过从大型视觉语言模型（LVLM）中提取知识，提升强化学习（RL）代理的样本效率，减少早期探索需求。
...
Method: 利用LVLM作为教师模型，提供基于RL代理轨迹的指导动作，直接从视觉观察生成动作建议。

Result: 实验显示LVLM2P显著提升了基线RL算法的样本效率。
...
### [272] [Learning traffic flows: Graph Neural Networks for Metamodelling Traffic Assignment](https://arxiv.org/abs/2505.11230)
...
TL;DR: 提出了一种基于消息传递神经网络的交通分配问题解决方案，用于加速大规模网络的均衡流量预测。


<details>
  <summary>Details</summary>
Motivation: 传统方法计算成本高且难以实时分析，需要一种更高效的替代方案。

Method: 使用消息传递神经网络作为元模型，模拟传统交通模拟器的算法结构。

Result: 模型在域外数据上表现稳健，能显著降低计算成本并支持实时决策。

Conclusion: 该方法为大规模交通规划和实时场景评估提供了高效解决方案。
...
### [273] [Memory-Efficient Orthogonal Fine-Tuning with Principal Subspace Adaptation](https://arxiv.org/abs/2505.11235)
*Fei Wu,Jia Hu,Geyong Min,Shiqiang Wang*

Main category: cs.LG

TL;DR: 提出了MOFT方法，通过主成分子空间适应提升正交微调的内存效率，同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 大模型参数增长导致全参数微调成本高昂，PEFT成为关键方法，但现有正交微调方法内存效率低。

Method: 在低秩子空间内进行正交变换，利用SVD获取主成分子空间，并引入可学习缩放向量。

Result: 在37个任务和4个模型中，MOFT性能优于基线，同时显著减少内存占用。
...
### [274] [Massive-STEPS: Massive Semantic Trajectories for Understanding POI Check-ins -- Dataset and Benchmarks](https://arxiv.org/abs/2505.11239)
*Wilson Wongso,Hao Xue,Flora D. Salim*

Main category: cs.LG

TL;DR: 论文提出大规模公开数据集Massive-STEPS，用于解决POI推荐领域的数据陈旧和多样性不足问题，并评估多种推荐模型。


<details>
  <summary>Details</summary>
Motivation: 解决POI推荐领域依赖老旧数据（2012-2013）和缺乏多样化城市级数据的问题。

Method: 基于Semantic Trails数据集构建Massive-STEPS，包含12个城市2017-2018年的24个月数据，并评估监督和零样本推荐模型。

Result: Massive-STEPS提供了更丰富、更新的数据，支持多种模型评估。

Conclusion: 发布Massive-STEPS促进人类移动性和POI推荐的可重复与公平研究。
...
### [275] [A Set-Sequence Model for Time Series](https://arxiv.org/abs/2505.11243)
*Elliot L. Epstein,Apaar Sadhwani,Kay Giesecke*

Main category: cs.LG

TL;DR: 提出了一种Set-Sequence模型，用于金融预测问题，无需手工特征，通过联合学习跨截面和时间序列数据，显著优于基准方法。
...
Conclusion: Set-Sequence模型高效灵活，适用于变长单位预测，代码将开源。
...
### [276] [Rethinking Irregular Time Series Forecasting: A Simple yet Effective Baseline](https://arxiv.org/abs/2505.11250)
*Xvyuan Liu,Xiangfei Qiu,Xingjian Wu,Zhengyu Li,Chenjuan Guo,Jilin Hu,Bin Yang*

Main category: cs.LG

TL;DR: APN框架通过TAPA模块和简单查询模块，高效处理不规则多元时间序列，提升预测准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 不规则多元时间序列（IMTS）预测在多个关键领域至关重要，但现有方法复杂且资源密集，且数据不规则和缺失问题突出。

Method: 提出APN框架，包含TAPA模块（动态调整补丁边界和时间感知加权平均）和简单查询模块，最后用浅层MLP预测。
...
### [277] [Delta Attention: Fast and Accurate Sparse Attention Inference by Delta Correction](https://arxiv.org/abs/2505.11254)
...
<details>
  <summary>Details</summary>
Motivation: 传统稀疏注意力方法虽然降低了计算复杂度，但会导致性能下降，原因是稀疏计算引起的分布偏移。

Method: 提出了一种简单有效的校正方法，将稀疏注意力输出的分布调整得更接近二次注意力。

Result: 在131K RULER基准上，性能平均提升36%，恢复88%的二次注意力准确性，同时保持98.5%的稀疏性。

Conclusion: 该方法在显著提升性能的同时，保持了高效性，适用于任何稀疏注意力方法。
...
### [278] [Fourier Low-rank and Sparse Tensor for Efficient Tensor Completion](https://arxiv.org/abs/2505.11261)
*Jingyang Li,Jiuqian Shang,Yang Chen*

Main category: cs.LG

TL;DR: FLoST模型通过傅里叶变换分解时间维度，结合低秩矩阵和稀疏性，高效建模时空数据中的低频稳定性和高频变化，优于传统张量补全方法。


<details>
  <summary>Details</summary>
Motivation: 传统低秩张量模型对称处理所有模式，无法捕捉时空数据中时间维度的独特模式（低频稳定性和高频变化）。

Method: 提出FLoST模型，利用傅里叶变换分解时间维度，低频部分用低秩矩阵建模，高频部分用稀疏性表示。

Result: FLoST在参数数量和计算效率上优于传统方法，尤其在时间维度较大时表现更优。

Conclusion: FLoST为时空数据补全提供了更高效和可解释的解决方案。
...
### [279] [Driving Mechanisms and Forecasting of China's Pet Population-An ARIMA-RF-HW Hybrid Approach](https://arxiv.org/abs/2505.11269)
*Shengjia Chang,Xianshuo Yue*

Main category: cs.LG

TL;DR: 提出了一种动态加权的ARIMA-RF-HW混合模型，结合ARIMA、随机森林和Holt-Winters平滑，用于提高中国宠物数量预测准确性。


<details>
  <summary>Details</summary>
Motivation: 研究旨在通过整合多种方法提升宠物数量预测的准确性，为政策制定和企业服务提供支持。

Method: 使用2005-2023年数据，结合经济、社会和政策指标，通过Z-score标准化和缺失值填补预处理数据，构建混合模型。

Result: 关键驱动因素包括城市收入、消费和政策数量，预测显示猫数量稳定增长，狗数量波动。

Conclusion: 研究为政策制定者优化宠物健康管理提供支持，并指导企业开发差异化服务，促进行业可持续发展。
...
### [280] [Multiclass threshold-based classification](https://arxiv.org/abs/2505.11276)
*Francesco Marchetti,Edoardo Legnaro,Sabrina Guastavino*

Main category: cs.LG

TL;DR: 提出了一种基于阈值的多类分类框架，通过几何视角替代softmax的概率解释，实现分类性能的后验优化。


<details>
  <summary>Details</summary>
Motivation: 解决传统argmax规则在多类分类中的局限性，通过多维阈值优化分类性能。

Method: 将softmax输出视为多维单纯形上的几何对象，引入多维阈值进行后验优化，并设计面向分数的损失函数。

Result: 实验表明多维阈值调整能显著提升性能，且提出的损失函数与标准方法竞争力相当。

Conclusion: 该框架为多类分类提供了灵活的后验优化方法，扩展了二分类中的优势。
...
### [281] [SubROC: AUC-Based Discovery of Exceptional Subgroup Performance for Binary Classifiers](https://arxiv.org/abs/2505.11283)
*Tom Siegl,Kutalmış Coşkun,Bjarne Hiller,Amin Mirzaei,Florian Lemmerich,Martin Becker*

Main category: cs.LG

TL;DR: SubROC是一个开源框架，用于高效识别和描述机器学习模型在不同子群体中的性能差异。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在现实应用中性能不均，需要识别性能差异的子群体以指导部署和数据收集。

Method: 基于Exceptional Model Mining，结合ROC和PR AUC评估，支持快速子群体搜索和显著性测试。

Result: SubROC能可靠且高效地找到模型的优势和劣势，并通过案例研究和多数据集分析验证其实用性。

Conclusion: SubROC为识别和描述模型性能差异提供了高效、易用的解决方案。
...
### [282] [Bidirectional Information Flow (BIF) -- A Sample Efficient Hierarchical Gaussian Process for Bayesian Optimization](https://arxiv.org/abs/2505.11294)
...
Method: BIF框架在H-GP中建立父模型与子模型的双向信息交换，支持在线学习。

Result: BIF在合成和实际神经刺激优化任务中，父模型和子模型的R²分数分别提高了85%和5倍。

Conclusion: BIF显著提升了H-GP的性能，适用于具有层次结构的问题。
...
### [283] [Graph Representational Learning: When Does More Expressivity Hurt Generalization?](https://arxiv.org/abs/2505.11298)
*Sohir Maskey,Raffaele Paolino,Fabian Jogl,Gitta Kutyniok,Johannes F. Lutzeyer*

Main category: cs.LG

TL;DR: 本文探讨了图神经网络（GNNs）的表达能力与预测性能之间的关系，提出了一种新的预度量方法，揭示了表达能力强的GNNs可能泛化能力较差，除非训练集足够大或训练与测试图之间的距离较小。


<details>
  <summary>Details</summary>
Motivation: 研究GNNs的表达能力与泛化性能之间的关系，以解决现有方法中表达能力与性能不明确的问题。

Method: 引入一组预度量来捕捉图之间的结构相似性，并通过理论推导和实验验证，分析GNNs的泛化性能。

Result: 发现表达能力强的GNNs可能泛化能力较差，除非训练集足够大或训练与测试图之间的距离较小。

Conclusion: 研究为GNNs的表达能力与泛化性能之间的关系提供了理论支持，并提出了实际应用中的优化方向。
...
### [284] [Heterogeneity-Aware Client Sampling: A Unified Solution for Consistent Federated Learning](https://arxiv.org/abs/2505.11304)
*Shudi Weng,Chao Ren,Ming Xiao,Mikael Skoglund*

Main category: cs.LG

TL;DR: 论文揭示了异构通信和计算在联邦学习中的不一致性机制，并提出FedACS方法消除不一致性，实验证明其优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 研究异构通信和计算能力对联邦学习优化动态的影响，填补相关理论空白。

Method: 提出FedACS方法，通过异构感知的客户端采样消除不一致性。

Result: FedACS在动态异构环境中以$O(1/\sqrt{R})$收敛，实验性能提升4.3%-36%，通信和计算成本显著降低。

Conclusion: FedACS是首个统一解决异构联邦学习不一致性的通用方法，具有理论和实践优势。
...
### [285] [Effective Probabilistic Time Series Forecasting with Fourier Adaptive Noise-Separated Diffusion](https://arxiv.org/abs/2505.11306)
*Xinyan Wang,Rui Dai,Kaikui Liu,Xiangxiang Chu*

Main category: cs.LG

TL;DR: FALDA是一种基于傅里叶分解和扩散模型的时间序列预测框架，通过轻量级去噪器和条件扩散模型提升性能，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有概率预测方法在长期时间序列预测中存在不确定性管理不足的问题，FALDA旨在通过分解和扩散模型优化不确定性建模。

Method: FALDA结合傅里叶分解和条件扩散模型，利用DMRR框架统一扩散回归方法，并通过轻量级去噪器DEMA提升性能。

Result: 在六个真实数据集上，FALDA在长期预测中优于现有方法，计算效率高且准确性未降低，性能提升达9%。

Conclusion: FALDA通过优化不确定性建模和计算效率，成为时间序列预测领域的先进方法。
...
### [286] [Diffusion Learning with Partial Agent Participation and Local Updates](https://arxiv.org/abs/2505.11307)
*Elsa Rizk,Kun Yuan,Ali H. Sayed*

Main category: cs.LG

TL;DR: 本文提出了一种改进的扩散学习方法，通过局部更新和部分代理参与减少通信开销，并提高边缘设备的稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统扩散学习方法因每次迭代都需要通信而产生高开销，且边缘设备的波动性导致通信不可靠。

Method: 结合局部更新和部分代理参与，减少通信频率并适应设备可用性。
...
### [287] [Reinforcement Learning Closures for Underresolved Partial Differential Equations using Synthetic Data](https://arxiv.org/abs/2505.11308)
*Lothar Heimbach,Sebastian Kaltenbach,Petr Karnakov,Francis J. Alexander,Petros Koumoutsakos*

Main category: cs.LG

TL;DR: 提出了一种基于合成数据和强化学习的PDE闭包模型框架，用于解决粗粒度PDE的精度损失问题。


<details>
  <summary>Details</summary>
Motivation: 解决PDE在粗粒度近似中因忽略细节而导致的精度损失问题。

Method: 利用制造解方法生成合成数据，结合强化学习训练闭包模型。

Result: 在一维和二维Burgers方程及二维平流方程中验证了方法的有效性，并能推广到均匀PDE。

Conclusion: 该方法为数据稀缺系统开发高效闭包模型提供了潜力。
...
### [288] [Where You Place the Norm Matters: From Prejudiced to Neutral Initializations](https://arxiv.org/abs/2505.11312)
*Emanuele Francazi,Francesco Pinto,Aurelien Lucchi,Marco Baity-Jesi*

Main category: cs.LG

TL;DR: 论文研究了归一化层（如批归一化和层归一化）在神经网络初始化阶段对预测统计特性的影响，揭示了其位置选择如何塑造初始预测行为和学习动态。


<details>
  <summary>Details</summary>
Motivation: 归一化层在现代神经网络中广泛应用，但其对模型行为（尤其是初始化阶段）的理论理解尚不充分，本文旨在填补这一空白。

Method: 通过分析归一化层的存在和位置选择，研究其对网络初始化阶段预测分布的影响，从无偏（中性）到高度集中（偏见）的类别预测。

Result: 研究发现归一化层的位置会系统性影响初始预测行为，进而塑造学习动态。

Conclusion: 本文为归一化层如何影响早期训练行为提供了理论依据，并为更可控和可解释的网络设计提供了指导。
...
### [289] [Anomaly Detection for Non-stationary Time Series using Recurrent Wavelet Probabilistic Neural Network](https://arxiv.org/abs/2505.11321)
*Pu Yang,J. A. Barria*

Main category: cs.LG

TL;DR: 提出了一种无监督的循环小波概率神经网络（RWPNN），用于非平稳环境中的异常检测，通过非参数密度估计网络建模时序特征。


<details>
  <summary>Details</summary>
Motivation: 针对非平稳环境中时间序列异常检测（TSAD）任务的需求，提出一种能够适应数据变化且不依赖强分布假设的模型。

Method: 结合堆叠循环编码器-解码器（SREnc-Dec）和多感受野小波概率网络（MRWPN），前者捕捉潜在空间的时序特征，后者构建概率模型。

Result: 在45个真实世界时间序列数据集上验证了RWPNN的性能，显示其在TSAD任务中的鲁棒性和准确性，并能提供异常事件的早期预警。

Conclusion: RWPNN扩展了小波概率网络，适用于高维数据，为非平稳环境中的异常检测提供了有效解决方案。
...
### [290] [The Final Layer Holds the Key: A Unified and Efficient GNN Calibration Framework](https://arxiv.org/abs/2505.11335)
...
<details>
  <summary>Details</summary>
Motivation: GNN的预测置信度通常不足，现有校准方法无法捕捉模型与置信度的内在关系，导致理论保证有限且计算开销增加。

Method: 建立统一理论框架，揭示模型置信度由最后一层的类中心级和节点级校准共同决定，减少最后一层参数的权重衰减并辅以节点级校准。

Result: 实验验证了该方法的优越性。

Conclusion: 该方法通过理论框架和实验验证，有效解决了GNN置信度不足的问题。
...
### [291] [Sobolev Training of End-to-End Optimization Proxies](https://arxiv.org/abs/2505.11342)
*Andrew W. Rosemberg,Joaquim Dias Garcia,Russell Bent,Pascal Van Hentenryck*

Main category: cs.LG

TL;DR: 论文研究了通过Sobolev训练范式将求解器敏感性集成到优化代理中，显著提升了代理模型的性能。


<details>
  <summary>Details</summary>
Motivation: 传统迭代求解器耗时较长，优化代理通过机器学习模型快速近似求解参数化优化问题，但需进一步提升其准确性和可靠性。

Method: 在完全监督和自监督两种设置下，通过Sobolev训练范式将求解器的方向导数信息融入代理模型训练。

Result: 在完全监督设置下，均方误差降低56%，约束违规减少400%；自监督设置下，平均最优性差距减半。

Conclusion: Sobolev训练为大规模优化任务提供了快速可靠的代理模型，适用于安全关键场景。
...
### [292] [What Can We Learn From MIMO Graph Convolutions?](https://arxiv.org/abs/2505.11346)
*Andreas Roth,Thomas Liebig*

Main category: cs.LG

TL;DR: 论文提出了一种多输入多输出（MIMO）图卷积的直接近似方法，并引入了局部MIMO图卷积（LMGC），证明了其在多计算图下的线性独立性。


<details>
  <summary>Details</summary>
Motivation: 现有图神经网络（GNNs）通常在单输入单输出（SISO）情况下近似图卷积，而实际应用多为MIMO情况，因此需要直接近似MIMO图卷积。

Method: 通过卷积定理推导MIMO图卷积，并直接近似；提出局部MIMO图卷积（LMGC），证明其在多计算图下的性质。

Result: LMGC在多种边权重选择下对多集是单射的，且多计算图下的表示线性独立。实验表明LMGC能结合多种方法的优势。

Conclusion: LMGC为MIMO图卷积提供了一种有效的局部近似方法，具有理论和实验上的优势。
...
### [293] [Training NTK to Generalize with KARE](https://arxiv.org/abs/2505.11347)
*Johannes Schwab,Bryan Kelly,Semyon Malamud,Teng Andrea Xu*

Main category: cs.LG

TL;DR: 论文提出通过显式优化神经切线核（NTK）来替代传统深度神经网络（DNN）训练，使用KARE方法最小化泛化误差，实验表明其性能优于原始DNN和DNN诱导的NTK。


<details>
  <summary>Details</summary>
Motivation: 传统DNN训练通过梯度下降隐式优化NTK，但显式优化NTK可能更有效，挑战DNN的常规优势。

Method: 使用KARE方法显式训练NTK，最小化其泛化误差，而非传统经验风险最小化。

Result: 实验显示，显式训练的NTK性能优于原始DNN和DNN诱导的NTK。
...
### [294] [Context parroting: A simple but tough-to-beat baseline for foundation models in scientific machine learning](https://arxiv.org/abs/2505.11349)
*Yuanzhao Zhang,William Gilpin*

Main category: cs.LG

TL;DR: 时间序列基础模型在科学机器学习中表现出预测物理系统的能力，但研究发现它们并未真正理解物理规律，而是通过简单的“上下文复述”策略进行预测。


<details>
  <summary>Details</summary>
Motivation: 探讨时间序列基础模型是否真正学习到物理系统的内在规律，还是仅依赖简单的预测策略。

Method: 比较基础模型与直接上下文复述模型的预测表现，分析其策略差异。

Result: 直接上下文复述模型在预测多样动态系统时表现优于基础模型，且计算成本极低。

Conclusion: 上下文复述是一种简单但难以超越的基线方法，有助于识别更高级的上下文学习策略。
...
### [295] [Fractal Graph Contrastive Learning](https://arxiv.org/abs/2505.11356)
...
Method: FractalGCL采用基于重归一化的数据增强和分形维度感知的对比损失，并通过理论推导减少计算成本。

Result: 实验显示FractalGCL在标准基准和交通网络上均优于传统方法，性能提升约7%。

Conclusion: FractalGCL通过分形理论显著提升了图表示质量，同时优化了计算效率。
...
### [296] [LGBQPC: Local Granular-Ball Quality Peaks Clustering](https://arxiv.org/abs/2505.11359)
*Zihang Jia,Zhen Zhang,Witold Pedrycz*

Main category: cs.LG

TL;DR: 论文提出了一种改进的LGBQPC算法，通过优化GB生成和聚类过程，解决了GBDPC在处理复杂数据时的局限性。
...
### [297] [Efficient End-to-End Learning for Decision-Making: A Meta-Optimization Approach](https://arxiv.org/abs/2505.11360)
...
<details>
  <summary>Details</summary>
Motivation: 端到端学习在训练预测性ML模型时优于传统方法，但计算复杂度高，尤其是大规模问题。

Method: 引入神经网络架构，通过交替投影确保可行性约束，近似解决优化问题。

Result: 证明了指数收敛、近似保证和泛化边界，计算效率高，适用于多种优化问题。

Conclusion: 该方法在多个实际应用中表现出色，包括电力生成、最短路径和库存管理问题。
...
### [298] [Understanding Nonlinear Implicit Bias via Region Counts in Input Space](https://arxiv.org/abs/2505.11370)
*Jingwei Li,Jing Xu,Zifan Wang,Huishuai Zhang,Jingzhao Zhang*

Main category: cs.LG

TL;DR: 该论文提出了一种通过输入空间中相同预测标签的连通区域数量来表征隐式偏置的方法，并发现小区域数量与几何简单决策边界和良好泛化性能相关。


<details>
  <summary>Details</summary>
Motivation: 研究非线性背景下隐式偏置的定义和机制，以解释神经网络的强大泛化能力。

Method: 通过统计输入空间中相同预测标签的连通区域数量来表征隐式偏置，并与参数依赖的度量（如范数或归一化边距）进行比较。

Result: 小区域数量与几何简单决策边界和良好泛化性能相关，且较大的学习率和较小的批量大小可以诱导小区域数量。

Conclusion: 区域数量是一种有效的隐式偏置表征方法，能够适应非线性、过参数化模型，并与泛化性能密切相关。
...
### [299] [On the Interconnections of Calibration, Quantification, and Classifier Accuracy Prediction under Dataset Shift](https://arxiv.org/abs/2505.11380)
*Alejandro Moreo*

Main category: cs.LG

TL;DR: 论文研究了在数据集偏移条件下，分类器的校准、量化和准确性预测三个基本问题的等价性及其相互转化方法。


<details>
  <summary>Details</summary>
Motivation: 在数据集偏移情况下，传统的分类器校准、正例比例估计和准确性预测方法面临挑战，因此需要探索这些问题的内在联系。

Method: 通过证明三个问题的等价性，并提出基于其他领域方法的直接适配新方法。

Result: 新方法在性能上常与专门方法相当，有时甚至更优。

Conclusion: 论文旨在促进这些研究领域的交叉融合，推动统一方法的开发。
...
### [300] [IISE PG&E Energy Analytics Challenge 2025: Hourly-Binned Regression Models Beat Transformers in Load Forecasting](https://arxiv.org/abs/2505.11390)
*Millend Roy,Vladimir Pyltsov,Yinbo Hu*

Main category: cs.LG

TL;DR: 论文评估了从经典回归到深度学习模型的电力负荷预测效果，发现XGBoost在数据有限时优于复杂模型。


<details>
  <summary>Details</summary>
Motivation: 电力负荷预测对电网稳定和可再生能源整合至关重要，但现有深度学习模型在长期预测中的效果尚不明确。

Method: 使用PCA降维，将预测任务建模为回归问题，结合温度和GHI数据，堆叠24个模型生成年度预测。

Result: XGBoost在数据有限时表现最佳，而深度学习模型（如TimeGPT）未能显著优于传统方法。

Conclusion: 模型选择应基于数据特性而非复杂度，传统方法在长期电力预测中仍有优势。
...
### [301] [Finding Counterfactual Evidences for Node Classification](https://arxiv.org/abs/2505.11396)
*Dazhuo Qiu,Jinwen Chen,Arijit Khan,Yan Zhao,Francesco Bonchi*

Main category: cs.LG

TL;DR: 论文提出了一种基于反事实学习的搜索方法，用于识别GNN节点分类任务中的反事实证据，以提升公平性和准确性。


<details>
  <summary>Details</summary>
Motivation: 解决GNN在公平性和可解释性上的问题，尤其是在无法进行随机对照试验的现实场景中，依赖观测数据检测反事实。

Method: 开发了高效的搜索算法和索引方案，结合节点特征和结构信息，识别反事实证据，且不依赖特定GNN。

Result: 通过下游应用验证了反事实证据在提升GNN公平性和准确性上的潜力。

Conclusion: 反事实证据为GNN的改进提供了新方向，尤其在公平性和可解释性方面。
...
### [302] [Is Grokking a Computational Glass Relaxation?](https://arxiv.org/abs/2505.11411)
*Xiaotian Zhang,Yue Shang,Entao Yang,Ge Zhang*

Main category: cs.LG

TL;DR: 论文通过将神经网络中的“grokking”现象类比为物理系统中的玻璃松弛过程，提出了一种新的解释框架，并开发了一种优化器WanD，消除了grokking现象。


<details>
  <summary>Details</summary>
Motivation: 研究神经网络泛化能力的机制，特别是“grokking”现象（神经网络在训练性能接近完美后突然泛化），以揭示其背后的物理机制。

Method: 将神经网络参数视为物理系统的自由度，训练损失为系统能量，通过采样Boltzmann熵景观分析“grokking”过程。

Result: 实验表明“grokking”过程中不存在熵壁垒，挑战了将其视为一级相变的传统理论，并发现高熵状态对泛化有显著优势。

Conclusion: 通过开发优化器WanD，证明了“grokking”并非仅由权重范数演化引起，为优化器设计提供了新思路。
...
### [303] [Uncertainty quantification with approximate variational learning for wearable photoplethysmography prediction tasks](https://arxiv.org/abs/2505.11412)
*Ciaran Bench,Vivek Desai,Mohammad Moulaeifard,Nils Strodthoff,Philip Aston,Andrew Thompson*

Main category: cs.LG

TL;DR: 论文探讨了使用蒙特卡洛Dropout和改进的变分在线牛顿法来量化PPG信号分析中的不确定性，以提升模型的可信度和性能。


<details>
  <summary>Details</summary>
Motivation: PPG信号可用于非侵入性心脏健康评估，但深度学习模型缺乏可解释性且易过拟合，存在误诊风险。

Method: 采用蒙特卡洛Dropout和改进的变分在线牛顿法，评估AF分类和BP回归模型的不确定性。

Result: 超参数选择显著影响模型性能和不确定性质量，不同不确定性表达方式需不同参数优化。

Conclusion: 需平衡预测性能和校准质量，超参数选择应根据不确定性表达方式调整。
...
### [304] [MoE-CAP: Benchmarking Cost, Accuracy and Performance of Sparse Mixture-of-Experts Systems](https://arxiv.org/abs/2505.11415)
...
TL;DR: MoE-CAP是一个专为稀疏混合专家系统设计的基准测试，揭示了当前硬件难以平衡成本、准确性和性能的动态关系。


<details>
  <summary>Details</summary>
Motivation: 稀疏混合专家架构在扩展大型语言模型时依赖异构计算和内存资源，现有基准测试难以准确捕捉这些系统的权衡。

Method: 引入MoE-CAP基准测试和CAP雷达图，并提出稀疏感知性能指标S-MBU和S-MFU。

Result: 分析表明，当前硬件下优化两个维度会牺牲第三个维度，即MoE-CAP权衡。

Conclusion: MoE-CAP和稀疏感知指标为异构硬件平台上的MoE系统提供了更准确的性能评估工具。
...
### [305] [Mergenetic: a Simple Evolutionary Model Merging Library](https://arxiv.org/abs/2505.11427)
*Adrian Robert Minut,Tommaso Mencattini,Andrea Santilli,Donato Crisostomi,Emanuele Rodolà*

Main category: cs.LG

TL;DR: Mergenetic是一个开源库，支持通过进化算法合并语言模型，无需额外训练，成本低且硬件需求低。


<details>
  <summary>Details</summary>
Motivation: 现有框架缺乏支持语言模型中灵活实验进化合并策略的功能，因此开发了Mergenetic。

Method: Mergenetic结合了合并方法和进化算法，并采用轻量级适应度估计器降低评估成本。

Result: Mergenetic在不同任务和语言中表现优异，且仅需普通硬件。
...
### [306] [MegaScale-MoE: Large-Scale Communication-Efficient Training of Mixture-of-Experts Models in Production](https://arxiv.org/abs/2505.11432)
...
TL;DR: MegaScale-MoE是一个针对大规模混合专家（MoE）模型高效训练的生产系统，通过优化通信和计算重叠，显著提升训练效率。
...
Method: MegaScale-MoE采用通信高效的并行策略，优化注意力与FFN层的通信，并通过通信压缩和调整通信模式提升效率。

Result: 在1,440个NVIDIA Hopper GPU上训练352B MoE模型时，吞吐量达1.41M tokens/s，效率比Megatron-LM提升1.88倍。

Conclusion: 该系统为MoE训练提供了高效解决方案，并希望其设计思路能推动未来MoE系统的研究。
...
### [307] [A Generative Framework for Causal Estimation via Importance-Weighted Diffusion Distillation](https://arxiv.org/abs/2505.11444)
*Xinran Song,Tianyu Chen,Mingyuan Zhou*

Main category: cs.LG

TL;DR: 论文提出了一种名为IWDD的新方法，结合扩散模型预训练和重要性加权蒸馏，用于解决观测数据中的因果推断问题，显著提升了预测性能。


<details>
  <summary>Details</summary>
Motivation: 观测数据中的协变量不平衡和非随机治疗分配导致因果推断困难，现有方法如IPW在深度学习框架中应用有限。

Method: IWDD框架结合扩散模型预训练和重要性加权蒸馏，引入随机化调整以简化计算并降低梯度估计方差。

Result: IWDD在样本外预测中表现最优，显著提升了因果估计效果，支持个体化治疗策略的开发。
...
### [308] [Signal attenuation enables scalable decentralized multi-agent reinforcement learning over networks](https://arxiv.org/abs/2505.11461)
*Wesley A Suttle,Vipul K Sharma,Brian M Sadler*

Main category: cs.LG

TL;DR: 论文提出了一种基于信号衰减的去中心化多智能体强化学习方法，适用于无线通信和雷达网络中的功率分配问题。


<details>
  <summary>Details</summary>
Motivation: 经典多智能体强化学习方法需要全局状态可观测性，限制了算法的去中心化和扩展性。信号衰减特性为去中心化提供了可能。

Method: 提出了两种约束多智能体马尔可夫决策过程模型，推导了局部邻域近似方法，并开发了去中心化鞍点策略梯度算法。

Result: 通过雷达网络中的目标检测功率分配问题验证了方法的有效性，并建立了误差界限。

Conclusion: 该方法为无线通信和雷达网络中的其他问题提供了扩展模型。
...
### [309] [msf-CNN: Patch-based Multi-Stage Fusion with Convolutional Neural Networks for TinyML](https://arxiv.org/abs/2505.11483)
*Zhaolan Huang,Emmanuel Baccelli*

Main category: cs.LG

TL;DR: msf-CNN是一种新型技术，通过有向无环图优化CNN融合设置，显著减少MCU推理时的RAM使用。


<details>
  <summary>Details</summary>
Motivation: 解决MCU内存限制和实时性需求，优化CNN模型在微型设备上的运行效率。
...
### [310] [Potential failures of physics-informed machine learning in traffic flow modeling: theoretical and experimental analysis](https://arxiv.org/abs/2505.11491)
*Yuan-Zheng Lei,Yaobang Gong,Dianwei Chen,Yao Cheng,Xianfeng Terry Yang*

Main category: cs.LG

TL;DR: 研究分析了物理信息机器学习（PIML）在交通流建模中的表现，发现其失败原因并非物理残差本身，而是梯度的几何条件。实验表明，数据稀疏性和时间平均会误导模型，而CFL条件是数据集适用性的关键指标。


<details>
  <summary>Details</summary>
Motivation: 探讨PIML在交通流建模中的失败原因，挑战了物理残差阻碍优化的常见假设。

Method: 通过扰动训练模型并分析Hessian矩阵的主特征向量，评估损失值，研究梯度的几何条件。

Result: 物理残差在高物理驱动设置下会降低性能，稀疏采样和时间平均数据会误导模型。CFL条件是数据集适用性的关键指标。

Conclusion: PIML的成功需要梯度的几何条件满足，且高阶模型（如ARZ）的误差下限通常更大。
...
### [311] [Time Travel is Cheating: Going Live with DeepFund for Real-Time Fund Investment Benchmarking](https://arxiv.org/abs/2505.11065)
*Changlun Li,Yao Shi,Chen Wang,Qiqi Duan,Runke Ruan,Weijie Huang,Haonan Long,Lijun Huang,Yuyu Luo,Nan Tang*

Main category: cs.CE

TL;DR: DeepFund是一个实时基金基准工具，用于在实时市场条件下严格评估大型语言模型（LLMs）的表现，避免了历史回测中的信息泄漏问题。


<details>
  <summary>Details</summary>
Motivation: 现有评估LLM驱动交易策略的基准依赖历史回测，可能导致信息泄漏和过于乐观的性能估计，因此需要一种更严格的实时评估方法。

Method: DeepFund采用多智能体架构，直接连接实时股票市场数据，确保评估的公平性和无信息泄漏。

Result: 在DeepFund实时评估环境中，即使是先进的LLM模型（如DeepSeek-V3和Claude-3.7-Sonnet）也出现净交易亏损，揭示了LLM在主动基金管理中的局限性。

Conclusion: DeepFund为LLM在金融领域的实际应用提供了更严格的评估标准，并揭示了当前LLM在复杂基金管理任务中的挑战。
...
### [312] [Textured mesh Quality Assessment using Geometry and Color Field Similarity](https://arxiv.org/abs/2505.10824)
...
<details>
  <summary>Details</summary>
Motivation: 现有TMQA方法难以提供准确和鲁棒的评估，而场表示在3D几何和颜色信息中表现出色。

Method: 利用有符号距离场和新提出的最近表面点颜色场提取几何相似性、几何梯度相似性、空间颜色分布相似性和空间颜色梯度相似性。

Result: 在三个基准数据集上表现优于现有方法，且计算复杂度低。

Conclusion: FMQM是一种实用高效的3D图形和可视化解决方案，代码已开源。
...
### [313] [Explain What You Mean: Intent Augmented Knowledge Graph Recommender Built With LLM](https://arxiv.org/abs/2505.10900)
...
<details>
  <summary>Details</summary>
Motivation: 推荐系统中的交互稀疏性是主要障碍，尤其是在用户和产品基数不均衡或新实体引入（冷启动问题）时。现有方法要么资源消耗大，要么受限于数据质量和可用性。

Method: IKGR利用检索增强生成和编码方法构建知识图谱，通过学习用户-项目潜在亲和力并通过意图连通性进一步丰富图谱。

Result: 在真实数据集上的实验表明，IKGR能够填补知识空白，并在公开和内部数据集上显著优于现有基线。

Conclusion: IKGR通过意图知识图谱有效解决了稀疏性问题，并提供了可解释的推荐结果。
...
### [314] [Who You Are Matters: Bridging Topics and Social Roles via LLM-Enhanced Logical Recommendation](https://arxiv.org/abs/2505.10940)
...
TL;DR: 论文提出了一种结合大型语言模型（LLM）和推荐系统的方法（TagCF），通过用户角色识别和行为逻辑建模任务，提升推荐性能。


<details>
  <summary>Details</summary>
Motivation: 主流推荐系统方法忽视了用户特征和社会角色的建模，而这些是影响用户兴趣和偏好转变的逻辑混杂因素。

Method: 提出TagCF框架，利用LLM的世界知识和逻辑推理能力生成虚拟逻辑图，同时通过用户角色对齐行为逻辑与用户反馈。

Result: 实验表明，提取的用户-项目逻辑图是一种通用知识，可提升多种推荐任务的性能。

Conclusion: 该方法通过显式建模用户角色和行为逻辑，显著提升了推荐系统的表现，并验证了其通用性。
...
### [315] [From Intent Discovery to Recognition with Topic Modeling and Synthetic Data](https://arxiv.org/abs/2505.11176)
...
<details>
  <summary>Details</summary>
Motivation: 在短文本和冷启动问题下，传统方法难以识别用户意图，因此需要新的解决方案。

Method: 采用分层主题建模和意图发现，并生成合成查询数据以增强数据集。

Result: 主题扩展显著提升了相关性和一致性，合成查询质量高且不影响多样性。

Conclusion: LLM框架在主题建模和意图识别中具有可扩展性和实用性，能有效增强数据集覆盖范围。
...
### [316] [User-centric Music Recommendations](https://arxiv.org/abs/2505.11198)
*Jaime Ramirez Castillo,M. Julia Flores,Ann E. Nicholson*

Main category: cs.IR

TL;DR: 本文提出了一种以用户为中心的四阶段推荐框架，旨在提升解释性和用户参与度。通过分析用户15年的音乐播放记录，预测用户偏好并推荐音乐。


<details>
  <summary>Details</summary>
Motivation: 提升音乐推荐的解释性和用户参与度，通过分析用户长期音乐习惯实现个性化推荐。

Method: 收集用户15年的Last.fm播放记录，构建时间上下文数据集，结合标签和音频特征预测用户偏好，并推荐相似曲目。

Result: 框架成功预测用户偏好（如舞曲性），并实现个性化推荐，可扩展至更多用户和特征。

Conclusion: 该框架能有效学习用户音乐习惯，具有扩展潜力，适用于其他用户和特征。
...
### [317] [On the Role of Weight Decay in Collaborative Filtering: A Popularity Perspective](https://arxiv.org/abs/2505.11318)
...
<details>
  <summary>Details</summary>
Motivation: 权重衰减在CF模型训练中至关重要，但其作用机制尚不明确，研究旨在揭示其功能并提出改进方案。

Method: 通过理论和实证分析，发现权重衰减编码流行度信息，并提出PRISM方法预编码流行度信息。

Result: PRISM提升性能4.77%，减少训练时间38.48%，并可调节初始化强度以缓解流行度偏差。

Conclusion: PRISM是一种简单有效的方法，可替代权重衰减，提升CF模型训练效率和性能。
...
### [318] [The Future is Sparse: Embedding Compression for Scalable Retrieval in Recommender Systems](https://arxiv.org/abs/2505.11388)
*Petr Kasalický,Martin Spišák,Vojtěch Vančura,Daniel Bohuněk,Rodrigo Alves,Pavel Kordík*

Main category: cs.IR

TL;DR: 论文提出了一种轻量级、可学习的嵌入压缩技术，用于解决高基数实体表示的内存问题，同时保持检索性能。


<details>
  <summary>Details</summary>
Motivation: 工业级推荐系统面临高基数实体（如用户或物品）的密集嵌入存储和访问难题，内存限制成为瓶颈。

Method: 通过将密集嵌入投影到高维稀疏激活空间，设计了一种轻量级压缩技术。

Result: 该方法显著减少了内存需求，同时保持了检索性能，适用于资源受限的大规模部署。

Conclusion: 利用稀疏性是提升大规模推荐系统效率的有效途径。
...
### [319] [Phi: Leveraging Pattern-based Hierarchical Sparsity for High-Efficiency Spiking Neural Networks](https://arxiv.org/abs/2505.10909)
*Chiyue Wei,Bowen Duan,Cong Guo,Jingyang Zhang,Qingyue Song,Hai "Helen" Li,Yiran Chen*

Main category: cs.AR

TL;DR: 论文提出了一种名为Phi的基于模式的分层稀疏框架，通过两级稀疏层次优化SNN计算，显著提升了速度和能效。


<details>
  <summary>Details</summary>
Motivation: 现有SNN加速器忽略了二进制激活中的独特分布模式，未能充分利用稀疏性。

Method: 提出两级稀疏层次（向量级和元素级），结合k-means模式选择和硬件架构设计。

Result: Phi实现了3.45倍加速和4.93倍能效提升。

Conclusion: Phi框架有效优化了SNN计算，展示了模式稀疏性的潜力。
...
### [320] [Vaiage: A Multi-Agent Solution to Personalized Travel Planning](https://arxiv.org/abs/2505.10922)
...
<details>
  <summary>Details</summary>
Motivation: 传统旅行规划平台无法动态适应用户偏好和实时信息，Vaiage旨在解决这一问题。

Method: 采用图结构多智能体框架，结合LLM进行目标推荐和行程规划，支持自然语言交互和实时约束调整。

Result: 实验显示Vaiage平均得分8.5/10，优于其他变体，智能体协调显著提升行程质量。

Conclusion: 结合LLM推理与符号智能体协调，Vaiage在开放规划任务中表现优异。
...
### [321] [Explaining Strategic Decisions in Multi-Agent Reinforcement Learning for Aerial Combat Tactics](https://arxiv.org/abs/2505.11311)
*Ardian Selmonaj,Alessandro Antonucci,Adrian Schneider,Michael Rüegsegger,Matthias Sommer*

Main category: cs.MA

TL;DR: 论文探讨了多智能体强化学习（MARL）在军事战略规划中的应用，重点分析了可解释性方法在模拟空战场景中的重要性。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏可解释性，MARL在敏感军事场景中的实际部署受到限制，而可解释性对信任、安全和与人类策略的一致性至关重要。

Method: 研究回顾并评估了MARL的可解释性方法，并在模拟空战场景中应用了多种解释技术。

Result: 通过将AI生成的战术与人类可理解的推理联系起来，强调了透明性对可靠部署和人机交互的重要性。

Conclusion: 研究强调了可解释性在MARL军事应用中的关键作用，支持战略规划和军事人员培训。
...
### [322] [Anti-aliasing of neural distortion effects via model fine tuning](https://arxiv.org/abs/2505.11375)
*Alistair Carson,Alec Wright,Stefan Bilbao*

Main category: eess.AS

TL;DR: 论文提出了一种通过师生微调方法减少神经网络模型中频率混叠的技术，显著降低了LSTM和TCN模型的混叠现象。


<details>
  <summary>Details</summary>
Motivation: 神经网络在吉他失真效果建模中应用广泛，但高频率和高增益输入会导致频率混叠问题，影响模型性能。

Method: 采用师生微调方法，教师模型为预训练且参数固定的模型，学生模型为可学习的副本，通过无混叠数据集进行微调。

Result: 该方法显著减少了LSTM和TCN模型的混叠现象，效果优于两倍过采样。

Conclusion: 师生微调方法有效抑制混叠，但会略微影响谐波失真，LSTM模型在抗混叠和保持模拟设备相似性方面表现最佳。
...
### [323] [MID-L: Matrix-Interpolated Dropout Layer with Layer-wise Neuron Selection](https://arxiv.org/abs/2505.11416)
*Pouya Shaeri,Ariane Middel*

Main category: cs.NE

TL;DR: MID-L是一种动态选择神经元的模块，通过输入相关的门控向量减少计算量，同时保持准确性。


<details>
  <summary>Details</summary>
Motivation: 现代神经网络通常激活所有神经元，导致计算冗余和效率低下。

Method: MID-L采用可微分的Top-k掩码策略，动态选择最有信息的神经元。

Result: 在多个基准测试中，MID-L平均减少55%的活跃神经元，节省1.7倍FLOPs，且保持或超过基线准确率。

Conclusion: MID-L是一种通用、即插即用的动态计算层，适用于计算受限系统。
...
### [324] [On Next-Token Prediction in LLMs: How End Goals Determine the Consistency of Decoding Algorithms](https://arxiv.org/abs/2505.11183)
*Jacob Trauger,Ambuj Tewari*

Main category: stat.ML

TL;DR: 本文研究了大型语言模型中基于交叉熵损失的概率下一词预测方法，分析了不同解码算法（贪婪、前瞻、随机采样和温度缩放随机采样）与目标损失函数的一致性。研究发现，随机采样在模拟真实概率分布时具有一致性，而其他目标（如最小化序列的0-1损失）则无多项式时间最优算法。解码算法的选择需根据目标（信息检索或创意生成）而定。


<details>
  <summary>Details</summary>
Motivation: 研究不同解码算法在大型语言模型中的一致性，填补了该领域理论研究的空白，并探讨了算法选择对目标达成的重要性。

Method: 分析了贪婪、前瞻、随机采样和温度缩放随机采样等解码算法，研究了它们与不同目标损失函数的一致性。

Result: 随机采样在模拟真实概率分布时一致，而其他目标无多项式时间最优算法。解码算法的选择需根据具体目标（如信息检索或创意生成）而定。

Conclusion: 解码算法的选择对模型性能至关重要，当前许多算法在理论基础上存在不足，需根据目标需求优化选择。
...
### [325] [A Fourier Space Perspective on Diffusion Models](https://arxiv.org/abs/2505.11278)
*Fabian Falck,Teodora Pandeva,Kiarash Zahirnia,Rachel Lawrence,Richard Turner,Edward Meeds,Javier Zazo,Sushrut Karmalkar*

Main category: stat.ML

TL;DR: 扩散模型在图像、音频等数据模态中表现出色，但其前向过程在傅里叶空间中高频分量更快被噪声污染，导致逆向生成时高频质量下降。本文提出一种新的前向过程，使所有频率以相同速率被噪声污染，显著提升了高频主导数据集的生成质量。


<details>
  <summary>Details</summary>
Motivation: 研究扩散模型前向过程在傅里叶空间中的归纳偏差，发现高频分量更快被噪声污染导致逆向生成时高频质量下降，从而探索改进方法。

Method: 理论分析和实验验证DDPM前向过程在傅里叶空间中的高频分量更快被噪声污染的问题，并提出一种新的前向过程，使所有频率以相同速率被噪声污染。

Result: 新方法在高频主导的数据集上显著提升了生成质量，同时在标准图像基准上与DDPM表现相当。

Conclusion: 通过调整前向过程在傅里叶空间中的噪声污染速率，可以显著改善高频分量的生成质量，同时不影响低频表现。
...
### [326] [An Exponential Averaging Process with Strong Convergence Properties](https://arxiv.org/abs/2505.10605)
*Frederik Köhne,Anton Schiela*

Main category: stat.ML

TL;DR: 论文提出了一种改进的指数移动平均方法（$p$-EMA），通过调整权重分配方式，解决了传统EMA在随机收敛性上的不足，并分析了其在随机梯度下降（SGD）中的应用。


<details>
  <summary>Details</summary>
Motivation: 传统指数移动平均（EMA）在随机动态系统中无法实现强随机收敛性，因为其对最新观测的权重分配方式限制了噪声的减少。

Method: 提出$p$-EMA方法，将最新观测的权重以次谐波速率递减至零，从而改进收敛性。

Result: 在温和的自相关假设下，$p$-EMA具有随机收敛性保证。

Conclusion: $p$-EMA为随机动态系统中的平滑问题提供了更优解，并在SGD的自适应步长控制中展示了应用潜力。
...
### [327] [Minimax learning rates for estimating binary classifiers under margin conditions](https://arxiv.org/abs/2505.10628)
*Jonathan García,Philipp Petersen*

Main category: stat.ML

TL;DR: 论文研究了基于水平函数的二元估计器分类问题，在几何边界条件下推导了学习速率的上界和下界，并应用于多种函数类。


<details>
  <summary>Details</summary>
Motivation: 研究在几何边界条件下分类问题的学习速率，填补理论空白，尤其是无噪声场景下的下界推导。

Method: 使用水平函数描述决策边界，结合几何边界条件和有界Kolmogorov熵的函数类，推导学习速率的上界和下界。

Result: 在Barron正则函数和Hölder连续函数中，学习速率接近最优的O(n^-1)；在强边界条件下，凸决策边界的学习速率接近O(n^-1/2)。

Conclusion: 论文在几何边界条件下提供了分类问题学习速率的理论框架，并验证了多种函数类的最优速率。
...
### [328] [Supervised Models Can Generalize Also When Trained on Random Label](https://arxiv.org/abs/2505.11006)
*Oskar Allerbo,Thomas B. Schön*

Main category: stat.ML

TL;DR: 论文探讨了监督模型是否可以在不使用输出信息y的情况下训练，并证明了其可行性。通过将模型构建为平滑器，并提出一种基于预测分布的新模型选择标准，实验表明这种方法在真实和合成数据上表现接近传统方法。


<details>
  <summary>Details</summary>
Motivation: 研究监督模型是否可以不依赖输出信息y进行训练，探索无监督学习的扩展可能性。

Method: 将模型构建为平滑器（形式为f̂=Sy），并通过训练随机标签独立构造平滑矩阵S。提出基于预测分布的新模型选择标准。

Result: 在线性核岭回归、平滑样条和神经网络等模型上，无y训练版本表现接近传统方法，显著优于随机猜测。

Conclusion: 证明了监督模型可以在不使用输出信息y的情况下有效训练，并提出了一种新的模型选择标准。
...
### [329] [Inexact Column Generation for Bayesian Network Structure Learning via Difference-of-Submodular Optimization](https://arxiv.org/abs/2505.11089)
*Yiran Yang,Rui Chen*

Main category: stat.ML

TL;DR: 本文提出了一种基于整数规划的贝叶斯网络结构学习方法，通过动态生成行和列来解决变量和约束数量指数级增长的问题，并利用子模优化和凸差算法高效求解定价问题。


<details>
  <summary>Details</summary>
Motivation: 现有贝叶斯网络结构学习的整数规划方法因变量和约束数量庞大而效率低下，亟需改进。

Method: 采用行和列生成技术，将定价问题转化为子模优化问题，并应用凸差算法求解。
...
### [330] [Nash: Neural Adaptive Shrinkage for Structured High-Dimensional Regression](https://arxiv.org/abs/2505.11143)
*William R. P. Denault*
...
Method: 提出Neural Adaptive Shrinkage（Nash），利用神经网络自适应调整每个协变量的惩罚项，并通过变分推断算法高效训练。

Result: 实验表明，Nash在真实数据上比现有方法更准确和适应性强。

Conclusion: Nash为稀疏回归提供了一种高效且适应性强的解决方案。
...
### [331] [Adaptive Linear Embedding for Nonstationary High-Dimensional Optimization](https://arxiv.org/abs/2505.11281)
*Yuejiang Wen,Paul D. Franzon*

Main category: stat.ML

TL;DR: SA-REMBO扩展了REMBO，通过多随机高斯嵌入和自适应选择机制，解决了高维贝叶斯优化中的局部结构和非平稳性问题。


<details>
  <summary>Details</summary>
Motivation: 高维贝叶斯优化受限于维数灾难和全局低维假设的刚性，REMBO虽通过线性投影缓解，但假设单一全局嵌入和静态目标。

Method: 提出SA-REMBO，支持多随机高斯嵌入，通过索引变量和乘积核在GP代理中联合建模，实现局部嵌入自适应选择。

Result: 理论分析表明乘积核具有表达性和稳定性，实验证明SA-REMBO在传统方法失败的复杂高维任务中表现优越。

Conclusion: SA-REMBO为复杂高维设计空间中的可扩展贝叶斯优化提供了灵活且强大的扩展。
...
### [332] [Convergence Rates of Constrained Expected Improvement](https://arxiv.org/abs/2505.11323)
*Haowei Wang,Jingyi Wang,Zhongxiang Dai,Nai-Yuan Chiang,Szu Hui Ng,Cosmin G. Petra*

Main category: stat.ML

TL;DR: 本文研究了约束贝叶斯优化（CBO）方法中约束期望改进（CEI）算法的收敛速率，证明了其在RKHS和GP假设下的收敛速率，并通过数值实验验证了理论分析。


<details>
  <summary>Details</summary>
Motivation: 约束贝叶斯优化方法在带约束的黑盒优化中取得了显著成功，但CEI算法的理论收敛速率尚未明确。本文旨在填补这一空白。
...
Conclusion: CEI在理论和实验中均表现出良好的收敛性能，为约束贝叶斯优化提供了理论支持。
...
### [333] [STRIDE: Sparse Techniques for Regression in Deep Gaussian Processes](https://arxiv.org/abs/2505.11355)
*Simon Urbainczyk,Aretha L. Teckentrup,Jonas Latz*

Main category: stat.ML

TL;DR: 提出一种结合变分学习和MCMC的粒子EM方法，用于高效训练大规模数据的深度高斯过程。


<details>
  <summary>Details</summary>
Motivation: 高斯过程（GPs）在处理大规模数据或多尺度特征时存在局限性，稀疏GPR和深度GPs分别解决了部分问题，但变分近似方法存在不确定性表示不准确的问题。

Method: 结合变分学习和MCMC，开发粒子EM方法，同时在大规模数据中寻找诱导点（变分）并准确训练GPs（采样）。
...
### [334] [Learning hidden cascades via classification](https://arxiv.org/abs/2505.11228)
*Derrick Gilchrist Edward Manoharan,Anubha Goel,Alexandros Iosifidis,Henri Hansen,Juho Kanniainen*

Main category: cs.SI

TL;DR: 提出了一种基于部分可观测性的机器学习框架，用于推断社交网络中的传播动态，尤其在个体状态不可直接观测时表现优异。


<details>
  <summary>Details</summary>
Motivation: 现实中个体状态（如感染或知情）通常不可观测，但中间指标（如症状）可提供传播动态的关键信息。

Method: 采用分布分类方法，利用分类器推断潜在的传播动态。

Result: 在合成网络和真实内幕交易网络中表现良好，尤其适用于高循环连通性的复杂网络。

Conclusion: 该方法在无法直接观测个体状态时，为分析真实传播现象提供了有效工具。
...
### [335] [A Physics-Informed Convolutional Long Short Term Memory Statistical Model for Fluid Thermodynamics Simulations](https://arxiv.org/abs/2505.10919)
*Luca Menicali,Andrew Grace,David H. Richter,Stefano Castruccio*

Main category: physics.flu-dyn

TL;DR: 提出了一种基于物理信息的时空替代模型，用于模拟Rayleigh-Bénard对流，结合卷积神经网络和创新的循环架构，显著降低计算成本。
...
Conclusion: 该模型为长期模拟提供了可扩展的DNS替代方案。
...
### [336] [$\mathcal{A}LLM4ADD$: Unlocking the Capabilities of Audio Large Language Models for Audio Deepfake Detection](https://arxiv.org/abs/2505.11079)
*Hao Gu,Jiangyan Yi,Chenglong Wang,Jianhua Tao,Zheng Lian,Jiayi He,Yong Ren,Yujie Chen,Zhengqi Wen*

Main category: cs.SD

TL;DR: 论文探讨了利用音频大语言模型（ALLM）进行音频深度伪造检测（ADD）的可行性，提出了ALLM4ADD框架，并通过实验验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 随着高保真音频生成模型的兴起及其潜在滥用风险，音频深度伪造检测变得日益重要。研究旨在探索ALLM在ADD任务中的应用潜力。

Method: 将ADD任务重新定义为音频问答问题，通过提示模型判断音频真伪，并进行监督微调以提升性能。

Result: 实验表明，ALLM4ADD在数据稀缺场景下能显著提升伪造音频检测性能。

Conclusion: 本研究为利用ALLM开发更有效的ADD系统提供了新思路，有望推动相关领域研究。
...
### [337] [Audio Turing Test: Benchmarking the Human-likeness of Large Language Model-based Text-to-Speech Systems in Chinese](https://arxiv.org/abs/2505.11200)
...
TL;DR: 论文提出了一种名为Audio Turing Test (ATT)的多维中文语料库数据集ATT-Corpus，用于改进TTS系统的评估方法，取代传统主观的MOS评分，并开发了Auto-ATT用于自动评估。


<details>
  <summary>Details</summary>
Motivation: 传统TTS系统评估方法（如MOS）存在主观性、环境不一致性和解释性不足的问题，且现有数据集缺乏多维度设计。

Method: 引入ATT-Corpus数据集和基于图灵测试的评估协议，要求评估者判断声音是否像人类；同时微调Qwen2-Audio-Instruct模型作为Auto-ATT用于自动评估。

Result: ATT能有效区分模型在特定能力维度的表现，Auto-ATT与人类评估结果高度一致。

Conclusion: ATT和Auto-ATT为TTS系统提供了更快速、可靠的评估工具，填补了中文TTS评估的空白。
...
### [338] [BanglaFake: Constructing and Evaluating a Specialized Bengali Deepfake Audio Dataset](https://arxiv.org/abs/2505.10885)
*Istiaq Ahmed Fahad,Kamruzzaman Asif,Sifat Sikder*

Main category: cs.SD

TL;DR: 论文介绍了BangalFake数据集，用于解决孟加拉语深度伪造音频检测的挑战，包含12,260真实和13,260伪造音频，并通过定性和定量分析验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 由于孟加拉语等低资源语言的深度伪造音频检测面临数据集有限和声学特征微妙的挑战，需要构建高质量数据集以推动研究。

Method: 使用SOTA TTS模型生成高质量合成语音，并通过MOS评分和t-SNE可视化分析数据集。

Result: MOS评分显示自然度为3.40，可懂度为4.01；t-SNE可视化揭示了真实与伪造音频的区分难点。

Conclusion: BangalFake数据集为孟加拉语深度伪造检测提供了关键资源，填补了低资源语言研究的空白。
...
### [339] [Seeing Sound, Hearing Sight: Uncovering Modality Bias and Conflict of AI models in Sound Localization](https://arxiv.org/abs/2505.11217)
*Yanhao Jia,Ji Xie,S Jivaganesh,Hao Li,Xu Wu,Mengmi Zhang*

Main category: cs.SD

TL;DR: 研究探讨了AI在声音定位中处理跨模态冲突的能力，发现人类优于AI，并提出了一种改进模型。


<details>
  <summary>Details</summary>
Motivation: 理解AI在跨模态冲突中的表现，并探索如何改进其性能。

Method: 评估多模态模型，进行心理物理学实验，并通过3D模拟数据微调模型。

Result: 人类表现优于AI，改进后的模型性能提升并接近人类水平。

Conclusion: 感官输入质量和系统架构影响多模态表示的准确性。
...
### [340] [Multi-Stage Speaker Diarization for Noisy Classrooms](https://arxiv.org/abs/2505.10879)
*Ali Sartaz Khan,Tolulope Ogunremi,Ahmed Attia,Dorottya Demszky*

Main category: cs.SD

TL;DR: 研究探讨了多阶段说话人日志模型在嘈杂教室环境中的有效性，结合去噪和混合VAD方法显著降低了日志错误率。


<details>
  <summary>Details</summary>
Motivation: 教室环境中的说话人日志面临录音质量差、背景噪声高、语音重叠等挑战，尤其是儿童语音的准确捕捉。

Method: 使用Nvidia的NeMo日志流程，评估去噪对日志准确性的影响，比较多种VAD模型，并探索结合ASR的混合VAD方法。
...
### [341] [Improving Inference-Time Optimisation for Vocal Effects Style Transfer with a Gaussian Prior](https://arxiv.org/abs/2505.11315)
*Chin-Yun Yu,Marco A. Martínez-Ramírez,Junghyun Koo,Wei-Hsiang Liao,Yuki Mitsufuji,György Fazekas*

Main category: cs.SD

TL;DR: ST-ITO方法通过引入高斯先验优化音频效果参数，显著提升了风格迁移的准确性和真实性。


<details>
  <summary>Details</summary>
Motivation: 解决ST-ITO方法在音频效果迁移中因无差别处理参数配置和依赖嵌入空间导致的偏差和不真实问题。

Method: 引入基于DiffVox数据集的高斯先验，将优化问题转化为最大后验估计。

Result: 在MedleyDB数据集上，参数均方误差降低33%，主观评测也显示方法优越性。

Conclusion: 结合先验知识可提升音频效果迁移的效果，为更高效的音频处理系统铺路。
...
### [342] [Machine Learning Approaches to Vocal Register Classification in Contemporary Male Pop Music](https://arxiv.org/abs/2505.11378)
...
<details>
  <summary>Details</summary>
Motivation: 流行音乐中歌手常使用多种音色和质感，导致声区识别困难，尤其是通过换声区时。

Method: 使用支持向量机（SVM）和卷积神经网络（CNN）分析梅尔频谱图像的纹理特征。

Result: 两种模型均能稳定分类声区，展示了在多声区和音乐类型中更鲁棒分类的潜力。

Conclusion: 提出的方法为声区分类提供了有效工具，AVRA软件有望进一步推动声乐分析的应用。
...
### [343] [TAIJI: MCP-based Multi-Modal Data Analytics on Data Lakes](https://arxiv.org/abs/2505.11270)
*Chao Zhang,Shaolei Zhang,Quehuan Liu,Sibei Chen,Tong Li,Ju Fan*

Main category: cs.DB

TL;DR: 论文提出了一种基于模型上下文协议（MCP）的新型多模态数据分析系统，通过语义操作符层次结构和AI代理驱动的NL2Operator翻译器提升查询准确性，并通过MCP框架优化效率和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 数据湖中多模态数据的多样性对数据分析提出了挑战，现有大型语言模型（LLMs）在准确性、效率和时效性方面表现不足。

Method: 1. 设计语义操作符层次结构；2. 开发AI代理驱动的NL2Operator翻译器；3. 提出基于MCP的执行框架；4. 引入数据更新机制。

Result: 系统提升了多模态数据分析的准确性和效率，支持模块化部署和高可扩展性。

Conclusion: 通过MCP框架和更新机制，系统有效解决了多模态数据分析中的挑战，平衡了数据时效性和推理效率。
...
### [344] [Linear Convergence of the Frank-Wolfe Algorithm over Product Polytopes](https://arxiv.org/abs/2505.11259)
...
<details>
  <summary>Details</summary>
Motivation: 探讨乘积多面体上的Frank-Wolfe算法的收敛性，为高维多面体交集问题提供高效解决方案。

Method: 分析乘积多面体的两种条件数，基于单个多面体组件的条件数，推导线性收敛率。

Result: 在μ-Polyak-Łojasiewicz凸目标下，算法表现出线性收敛性，并通过实验验证了其实际效率。

Conclusion: 提出的方法在高维多面体交集问题中具有实用性和高效性。
...
### [345] [Revisiting Stochastic Approximation and Stochastic Gradient Descent](https://arxiv.org/abs/2505.11343)
*Rajeeva Laxman Karandikar,Bhamidi Visweswara Rao,Mathukumalli Vidyasagar*

Main category: math.OC

TL;DR: 本文重新审视了随机逼近（SA）和随机梯度下降（SGD），提出了新的收敛充分条件，放宽了对噪声或测量误差的限制（无需有限二阶矩或均值），并为零阶SGD提供了收敛条件。


<details>
  <summary>Details</summary>
Motivation: 研究SA和SGD的收敛性，放宽传统假设，扩展其适用性。

Method: 通过改进证明方法，推导出新的收敛充分条件，适用于噪声无有限二阶矩或均值的情况，并为零阶SGD提供理论支持。

Result: 提出了迄今为止最弱的收敛条件，显著扩展了SA和SGD的应用范围。

Conclusion: 本文的理论成果为SA和SGD在更广泛场景下的应用提供了理论基础。
...
### [346] [Understanding Gen Alpha Digital Language: Evaluation of LLM Safety Systems for Content Moderation](https://arxiv.org/abs/2505.10588)
*Manisha Mehta,Fausto Giunchiglia*

Main category: cs.CY

TL;DR: 研究评估了AI系统对Alpha世代（2010-2024年出生）数字语言的理解能力，发现现有工具难以识别其独特的沟通方式中的潜在危害。
...
Method: 评估了四种主流AI模型（GPT-4、Claude、Gemini和Llama 3）在检测Alpha世代语言中隐藏的骚扰和操纵行为的能力，使用了100条来自游戏平台、社交媒体和视频内容的表达样本。

Result: 研究发现AI模型在理解Alpha世代语言时存在严重缺陷，直接影响在线安全。

Conclusion: 研究呼吁重新设计安全系统以适应年轻群体的沟通方式，并强调了结合Alpha世代视角的重要性。
...
### [347] [Anchoring AI Capabilities in Market Valuations: The Capability Realization Rate Model and Valuation Misalignment Risk](https://arxiv.org/abs/2505.10590)
*Xinmin Fang,Lingfeng Tao,Zhengxiong Li*

Main category: cs.CY

TL;DR: 论文提出了一种能力实现率（CRR）模型，用于量化AI潜力与实际表现之间的差距，并分析了AI相关公司的估值溢价与错配风险。


<details>
  <summary>Details</summary>
Motivation: 研究AI能力对股票估值的锚定效应，揭示市场估值与AI实际表现之间的差异。
...
Conclusion: 建议提高透明度、减少投机泡沫，使AI创新与可持续市场价值保持一致。
...
### [348] [Inclusivity of AI Speech in Healthcare: A Decade Look Back](https://arxiv.org/abs/2505.10596)
*Retno Larasati*

Main category: cs.CY

TL;DR: AI语音识别技术在医疗中的应用潜力巨大，但研究发现数据集和研究偏向高资源语言和标准口音，导致包容性不足，可能加剧医疗不平等。


<details>
  <summary>Details</summary>
Motivation: 揭示AI语音识别技术在医疗中的包容性问题，强调其对边缘群体的潜在负面影响。

Method: 分析现有数据集和研究的偏向性，提出包容性数据集设计和偏见缓解研究的必要性。

Result: 发现AI语音识别技术在高资源语言和标准口音上的偏向性，可能导致对边缘群体的误判。

Conclusion: 呼吁设计包容性数据集、开展偏见缓解研究，并制定政策框架，以确保AI语音技术在医疗中的公平应用。
...
### [349] [Toward a Public and Secure Generative AI: A Comparative Analysis of Open and Closed LLMs](https://arxiv.org/abs/2505.10603)
*Jorge Machado*

Main category: cs.CY

TL;DR: 论文比较了开源和专有生成式AI系统的优缺点，并提出了一个开放、公共和安全的框架。


<details>
  <summary>Details</summary>
Motivation: 缺乏对开源和专有生成式AI系统的全面比较研究，需要评估其风险和挑战。
...
### [350] [Towards Automated Situation Awareness: A RAG-Based Framework for Peacebuilding Reports](https://arxiv.org/abs/2505.10586)
*Poli A. Nemkova,Suleyman O. Polat,Rafid I. Jahan,Sagnik Ray Choudhury,Sun-joo Lee,Shouryadipta Sarkar,Mark V. Albert*

Main category: cs.CY

TL;DR: 论文提出了一种动态检索增强生成（RAG）系统，通过整合实时数据自动生成态势感知报告，并采用三级评估框架确保报告质量。


<details>
  <summary>Details</summary>
Motivation: 解决人工分析大量异构数据导致的延迟问题，提升人道主义响应、冲突监测和早期行动的决策效率。

Method: 构建动态RAG系统，整合多源实时数据，并设计三级评估框架（自动NLP指标、专家反馈、LLM评估）确保报告质量。

Result: 系统在多个实际场景中测试，能生成连贯、有洞察力且可操作的报告，显著减轻人工负担并加速决策。

Conclusion: 自动化报告生成方法有效，代码和评估工具已开源，促进进一步研究和应用。
...
### [351] [Towards an LLM-powered Social Digital Twinning Platform](https://arxiv.org/abs/2505.10681)
*Önder Gürcan,Vanja Falck,Markus G. Rousseau,Larissa L. Lima*

Main category: cs.CY

TL;DR: Social Digital Twinner是一个创新的社交模拟工具，用于探索复杂自适应社交系统中假设场景的潜在影响。


<details>
  <summary>Details</summary>
Motivation: 通过数据驱动和基于证据的方法解决社会问题。

Method: 工具由三部分组成：数据基础设施、基于LLM的代理模拟引擎和用户界面，支持自然语言交互。

Result: 展示了在挪威Kragero解决青少年辍学问题的能力。

Conclusion: 该工具促进了实时参与和协作设计干预措施的能力。
...
### [352] [ChestyBot: Detecting and Disrupting Chinese Communist Party Influence Stratagems](https://arxiv.org/abs/2505.10746)
*Matthew Stoffolano,Ayush Rout,Justin M. Pelletier*

Main category: cs.CY

TL;DR: 本文介绍了ChestyBot，一种基于语用学的语言模型，用于实时检测未标记的外国恶意影响推文，准确率高达98.34%。


<details>
  <summary>Details</summary>
Motivation: 俄罗斯和中国的外国信息操作利用美国宽松的信息环境，威胁民主制度和威斯特伐利亚体系。现有检测和缓解策略往往无法实时识别这些活动。

Method: 提出ChestyBot，一种基于语用学的语言模型，用于检测未标记的外国恶意影响推文。

Result: ChestyBot的检测准确率高达98.34%。

Conclusion: 该模型支持一种新框架，可在外国影响操作形成的早期阶段进行干扰。
...
### [353] [Phare: A Safety Probe for Large Language Models](https://arxiv.org/abs/2505.11365)
...
<details>
  <summary>Details</summary>
Motivation: 现有评估通常优先考虑性能而非识别失败模式，因此需要一种更全面的方法以确保LLMs的安全部署。

Method: Phare框架通过多语言诊断，评估17种最先进的LLMs，揭示系统性漏洞模式。

Result: 研究发现LLMs在多个安全维度上存在系统性漏洞，包括奉承、提示敏感性和刻板印象再现。

Conclusion: Phare通过突出具体失败模式而非简单排名，为构建更健壮、对齐且可信的语言系统提供了可操作的见解。
...
### [354] [Analyzing Patterns and Influence of Advertising in Print Newspapers](https://arxiv.org/abs/2505.10791)
*N Harsha Vardhan,Ponnurangam Kumaraguru,Kiran Garimella*

Main category: cs.CY

TL;DR: 本文通过图像处理和OCR技术，从印度五种流行报纸中提取广告数据，分析广告模式及其对媒体报道的影响。
...
Method: 开发数据驱动管道，结合图像处理和OCR技术，分析12,000多期报纸中的广告数据。

Result: 发现广告与媒体报道存在显著相关性，公司广告增加会带来更积极和广泛的报道。

Conclusion: 广告投放显著影响媒体报道，这一关系在不同时间和广告商中保持一致。
...
### [355] [The heteronomy of algorithms: Traditional knowledge and computational knowledge](https://arxiv.org/abs/2505.11030)
*David M. Berry*

Main category: cs.CY

TL;DR: 论文主张通过数字Bildung（教育）培养公民对计算系统的批判能力，以应对算法对理性的替代，并呼吁跨学科研究以理解计算系统的影响。


<details>
  <summary>Details</summary>
Motivation: 随着计算系统和媒体对社会的影响加深，公民需要具备批判计算的能力，以维护公共理性的自主性。

Method: 提出结合哲学、政治学、历史学、人类学、社会学、媒体研究、计算机科学和人文学科的概念与方法，进行跨学科研究。

Result: 强调计算系统的普及带来的认知挑战，并指出当前计算哲学偏向实用性而非对智识领域的关怀。

Conclusion: 呼吁开展批判性跨学科研究，以深入理解计算系统的构建、实现及其规范影响。
...
### [356] [Measurement to Meaning: A Validity-Centered Framework for AI Evaluation](https://arxiv.org/abs/2505.10573)
...
TL;DR: 论文提出了一种结构化框架，用于评估AI系统的能力，强调现有评估方法的局限性，并通过心理测量学的有效性分析改进评估设计。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统的评估方法过于依赖狭窄的基准测试，可能导致对模型能力的误导性结论，因此需要更严谨的评估框架。

Method: 提出了一种结构化框架，结合心理测量学的有效性分析，帮助确定评估证据与声称能力之间的关联性。

Result: 框架通过案例研究（如视觉和语言模型评估）验证，表明其能更准确地连接评估证据与声称能力。

Conclusion: 该框架为AI系统评估提供了更严谨的方法，有助于提升评估的实用性和决策效果。
...
### [357] [GRNN:Recurrent Neural Network based on Ghost Features for Video Super-Resolution](https://arxiv.org/abs/2505.10577)
*Yutong Guo*

Main category: eess.IV

TL;DR: 提出了一种基于“Ghost特征”的视频超分辨率方法，减少特征冗余，并结合RNN解决梯度消失问题，提升了PSNR和SSIM。


<details>
  <summary>Details</summary>
Motivation: 现有视频超分辨率（VSR）系统计算成本高，且特征冗余问题未被充分讨论。

Method: 使用Ghost特征减少冗余，结合RNN解决梯度消失，输入当前帧、下一帧、前一帧输出及隐藏状态。

Result: 在多个基准模型和数据集上，PSNR和SSIM有所提升，视频纹理细节保留更好。

Conclusion: Ghost特征与RNN结合有效提升了VSR性能，减少了计算成本。
...
### [358] [Predicting Risk of Pulmonary Fibrosis Formation in PASC Patients](https://arxiv.org/abs/2505.10691)
...
TL;DR: 本文提出了一种结合深度学习和放射组学的多中心胸部CT分析框架，用于预测COVID-19后遗症（PASC）相关的肺纤维化，准确率达82.2%。


<details>
  <summary>Details</summary>
Motivation: PASC（长期COVID）症状多样且持续时间不确定，肺纤维化可能影响长期呼吸功能，亟需有效的检测方法。

Method: 采用卷积神经网络（CNN）和可解释的特征提取技术，结合Grad-CAM可视化和放射组学特征分析。

Result: 分类任务中达到82.2%准确率和85.5% AUC，首次在文献中展示了深度学习在PASC相关肺纤维化早期检测中的潜力。

Conclusion: 深度学习驱动的计算方法为PASC相关肺纤维化的早期检测和风险评估提供了新思路。
...
### [359] [ExploreGS: a vision-based low overhead framework for 3D scene reconstruction](https://arxiv.org/abs/2505.10578)
...
<details>
  <summary>Details</summary>
Motivation: 传统激光雷达点云采集成本高，而ExploreGS旨在通过视觉模型降低成本，同时保持高质量重建。

Method: 框架结合场景探索与模型重建，利用Bag-of-Words模型实现实时处理，支持机载3D高斯泼溅训练。

Result: 在仿真和真实环境中的实验表明，ExploreGS在资源受限设备上高效且适用，重建质量与先进方法相当。

Conclusion: ExploreGS为无人机提供了一种低成本、高质量的3D场景重建解决方案。
...
### [360] [MOSAIC: A Multi-View 2.5D Organ Slice Selector with Cross-Attentional Reasoning for Anatomically-Aware CT Localization in Medical Organ Segmentation](https://arxiv.org/abs/2505.10672)
*Hania Ghouse,Muzammil Behzad*

Main category: eess.IV

TL;DR: 提出了一种基于视觉语言模型的解剖感知切片选择方法，用于高效多器官分割，并引入新指标SLC评估切片定位精度。


<details>
  <summary>Details</summary>
Motivation: 现有3D分割方法计算和内存消耗大，2D方法缺乏跨视图上下文信息，需解决这些问题以提高分割效率和准确性。

Method: 使用融合三视图（2.5D）的视觉语言模型进行器官存在检测，选择性保留高相关切片，减少输入量。

Result: 模型在所有器官上显著优于基线方法，实现了高效且空间一致的器官切片过滤。
...
### [361] [ROIsGAN: A Region Guided Generative Adversarial Framework for Murine Hippocampal Subregion Segmentation](https://arxiv.org/abs/2505.10687)
...
<details>
  <summary>Details</summary>
Motivation: 海马体亚区的精确分割对理解疾病机制和治疗干预至关重要，但目前缺乏针对组织图像的自动分割方法。

Method: 提出了ROIsGAN，一种基于U-Net的生成对抗网络，结合区域引导判别器损失（Dice和二元交叉熵损失）来优化边界和结构细节。

Result: ROIsGAN在DG、CA1和CA3亚区的分割任务中表现优于传统模型，Dice得分提升1-10%，IoU提升高达11%。

Conclusion: 该研究为海马体亚区的自动分割提供了基础数据集和方法，支持神经科学研究的高精度分析。
...
### [362] [Adaptive Spatial Transcriptomics Interpolation via Cross-modal Cross-slice Modeling](https://arxiv.org/abs/2505.10729)
*NingFeng Que,Xiaofei Wang,Jingjing Chen,Yixuan Jiang,Chao Li*

Main category: eess.IV

TL;DR: C2-STi提出了一种空间转录组学（ST）切片插值方法，通过模块化设计解决跨切片连续性、基因关联和细胞结构等挑战。


<details>
  <summary>Details</summary>
Motivation: 由于中间组织切片的缺失和高成本，多切片ST分析受限，C2-STi旨在填补这一技术空白。

Method: 设计了距离感知局部结构调制模块、金字塔基因共表达关联模块和跨模态对齐模块。

Result: 在公开数据集上优于现有方法，适用于单切片和多切片ST插值。

Conclusion: C2-STi为ST分析提供了高效且灵活的切片插值解决方案。
...
### [363] [Pretrained hybrid transformer for generalizable cardiac substructures segmentation from contrast and non-contrast CTs in lung and breast cancers](https://arxiv.org/abs/2505.10855)
*Aneesh Rangnekar,Nikhil Mankuzhy,Jonas Willmann,Chloe Choi,Abraham Wu,Maria Thor,Andreas Rimner,Harini Veeraraghavan*

Main category: eess.IV

TL;DR: 论文提出了一种混合变压器卷积网络（HTN），用于在不同成像对比和患者扫描位置下对心脏亚结构进行分割，结果显示其性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: AI自动分割在临床应用中可能因数据特征与训练集不同而性能下降，因此需要一种更鲁棒的方法。

Method: 通过改进预训练的变压器为HTN，并在不同对比度和扫描位置的CT数据上进行训练和验证。

Result: HTN在几何和剂量指标上表现优异，且对CT对比度和扫描位置变化具有鲁棒性。

Conclusion: HTN在临床应用中表现出色，且所需标注数据量远少于传统方法。
...
### [364] [Generative Models in Computational Pathology: A Comprehensive Survey on Methods, Applications, and Challenges](https://arxiv.org/abs/2505.10993)
*Yuan Zhang,Xinfeng Zhang,Xiaoming Qi Xinyu Wu,Feng Chen,Guanyu Yang,Huazhu Fu*

Main category: eess.IV

TL;DR: 综述回顾了生成模型在计算病理学中的进展，涵盖图像生成、文本生成、多模态生成等应用，分析了150多项研究，并讨论了挑战与未来方向。


<details>
  <summary>Details</summary>
Motivation: 生成模型在计算病理学中具有潜力，如高效学习、数据增强和多模态表示，但需系统总结其进展与挑战。

Method: 通过分析150多项代表性研究，梳理生成架构的演变（从GAN到扩散模型），并评估数据集与协议。

Result: 总结了生成模型的应用与局限，如高保真图像生成难度、临床解释性及伦理问题。

Conclusion: 未来需开发统一、多模态且临床可部署的生成系统，为研究者提供参考。
...
### [365] [Diffusion Model in Hyperspectral Image Processing and Analysis: A Review](https://arxiv.org/abs/2505.11158)
*Xing Hu,Xiangcheng Liu,Qianqian Duan,Danfeng Hong,Dawei Zhang*

Main category: eess.IV

TL;DR: 综述了扩散模型在高光谱图像处理中的应用，展示了其在降噪、分类和异常检测等任务中的优势。


<details>
  <summary>Details</summary>
Motivation: 高光谱图像的高维性和噪声干扰对传统模型提出了挑战，扩散模型因其处理高维数据的独特优势成为研究热点。

Method: 通过模拟数据在时间上的扩散过程，扩散模型能有效处理高维数据并生成高质量样本。

Result: 扩散模型显著提高了高光谱图像分析的准确性和效率。

Conclusion: 扩散模型为高光谱图像处理提供了新方向，但仍需解决一些挑战。
...
### [366] [From Fibers to Cells: Fourier-Based Registration Enables Virtual Cresyl Violet Staining From 3D Polarized Light Imaging](https://arxiv.org/abs/2505.11394)
...
TL;DR: 论文提出了一种基于深度学习的图像转换方法，用于从3D-PLI数据生成虚拟的细胞染色图像，以解决染色过程中的失真问题。


<details>
  <summary>Details</summary>
Motivation: 需要互补成像技术来全面评估大脑微结构，但传统染色方法会引入失真且样本有限。

Method: 利用深度学习进行图像转换，结合傅里叶配准方法处理训练数据的对齐问题。

Result: 方法成功从3D-PLI预测出Cresyl violet染色图像，并能匹配单个细胞实例。

Conclusion: 该方法为研究细胞与纤维的详细关系提供了一种高效且准确的解决方案。
...
### [367] [A Cautionary Tale on Integrating Studies with Disparate Outcome Measures for Causal Inference](https://arxiv.org/abs/2505.11014)
*Harsh Parikh,Trang Quynh Nguyen,Elizabeth A. Stuart,Kara E. Rudolph,Caleb H. Miles*

Main category: stat.ME

TL;DR: 论文探讨了在数据集结果测量不一致时，数据整合的效率和偏差问题，通过理论和实证分析提出了不同假设下的权衡。


<details>
  <summary>Details</summary>
Motivation: 解决数据集间结果测量不一致时数据整合的效率和偏差问题，为研究者提供指导。

Method: 引入三种不同强度的假设链接结果测量，通过理论和实证分析评估效率和偏差。

Result: 强假设下整合可提高渐近效率，但假设错误会导致偏差；弱假设可能在有限样本中提升效率，但随样本量增加效果减弱。

Conclusion: 在整合结果测量不一致的数据集时，需谨慎选择假设以避免偏差，同时权衡效率增益。
...
### [368] [Quantum thermodynamics and semi-definite optimization](https://arxiv.org/abs/2505.04514)
*Nana Liu,Michele Minervini,Dhrumil Patel,Mark M. Wilde*

Main category: quant-ph

TL;DR: 论文探讨了量子热力学与优化理论中半定规划问题的数学等价性，提出了一种基于自由能最小化的梯度上升方法，适用于经典与量子-经典混合算法。


<details>
  <summary>Details</summary>
Motivation: 研究量子热力学中系统的最小能量问题与优化理论中的半定规划问题之间的数学联系，并探索一种基于自由能最小化的高效解决方案。

Method: 采用Jaynes的量子热力学思想，将最小自由能问题转化为化学势最大化问题，并利用（随机）梯度上升方法求解。

Result: 证明了该方法在低温下能有效逼近最小能量，并可应用于经典与量子-经典混合算法，具有快速收敛性。

Conclusion: 该方法不仅为半定规划问题提供了物理动机，还解释了已有算法的优越性能。
...
### [369] [Generalization Bounds for Quantum Learning via Rényi Divergences](https://arxiv.org/abs/2505.11025)
*Naqueeb Ahmad Warsi,Ayanava Dasgupta,Masahito Hayashi*

Main category: quant-ph

TL;DR: 论文通过量子Rényi散度框架，提出了一种新的量子学习算法泛化误差上界，并展示了改进的sandwich量子Rényi散度优于Petz散度。


<details>
  <summary>Details</summary>
Motivation: 提升对量子学习理论的理解，建立更优的泛化误差上界。

Method: 利用变分方法评估量子Rényi散度，包括Petz和新的改进sandwich量子Rényi散度。

Result: 改进的sandwich量子Rényi散度在分析和数值上表现更优。

Conclusion: 新方法为量子学习算法的泛化误差提供了更紧的上界。
...
### [370] [GarmentPile: Point-Level Visual Affordance Guided Retrieval and Adaptation for Cluttered Garments Manipulation](https://arxiv.org/abs/2503.09243)
*Ruihai Wu,Ziyu Zhu,Yuran Wang,Yue Chen,Jiarui Wang,Hao Dong*

Main category: cs.RO

TL;DR: 论文提出了一种学习点级可用性表示的方法，用于处理杂乱衣物场景中的复杂交互和几何关系，并通过适应性模块重组高度纠缠的衣物。


<details>
  <summary>Details</summary>
Motivation: 杂乱衣物操作因衣物的复杂变形性和相互纠缠关系而具有挑战性，需同时保持衣物的清洁度和操作稳定性。

Method: 学习点级可用性表示，建模复杂空间和多模态操作候选，同时考虑衣物几何、结构和物体间关系；引入适应性模块重组高度纠缠的衣物。

Result: 框架在模拟和现实环境中对多种衣物类型和堆积配置表现出有效性。

Conclusion: 提出的方法能有效处理杂乱衣物场景，提升操作稳定性和清洁度。
...
### [371] [Predicting Human Behavior in Autonomous Systems: A Collaborative Machine Teaching Approach for Reducing Transfer of Control Events](https://arxiv.org/abs/2505.10695)
*Julian Wolter,Amr Gomaa*

Main category: cs.RO

TL;DR: 提出一种数据驱动方法，利用人类交互数据训练AI模型，减少不必要的控制转移（ToC），提升系统鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统控制转移（ToC）方法在非关键情况下常被不必要触发，影响系统效率。

Method: 通过模拟工业吸尘器的交互工具收集数据，开发基于LSTM的模型预测用户行为。

Result: 非专家数据也能有效训练模型，减少不必要的ToC事件。

Conclusion: AI可从人类问题解决行为中学习，结合传感器数据提升工业自动化和人机协作。
...
### [372] [REI-Bench: Can Embodied Agents Understand Vague Human Instructions in Task Planning?](https://arxiv.org/abs/2505.10872)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中，非专家用户（如老人和儿童）的指令常包含模糊指代表达，这导致LLM任务规划性能显著下降。

Method: 提出了首个包含模糊REs的机器人任务规划基准（REI-Bench），并提出任务导向的上下文认知方法以生成清晰指令。

Result: 模糊REs导致任务规划成功率下降高达77.9%，而所提方法在性能上优于现有提示和思维链技术。

Conclusion: 该研究提升了机器人任务规划的实用性，尤其为非专家用户提供了更可靠的服务。
...
### [373] [Geofenced Unmanned Aerial Robotic Defender for Deer Detection and Deterrence (GUARD)](https://arxiv.org/abs/2505.10770)
...
TL;DR: 本文提出了一种基于无人机的野生动物驱赶系统，结合实时计算机视觉和路径规划算法，解决了传统方法在适应性和效率上的不足。


<details>
  <summary>Details</summary>
Motivation: 鹿类等野生动物对农作物的破坏威胁农业生产，传统驱赶方法在可扩展性和适应性上存在局限。

Method: 系统整合了YOLO实时检测模块、高效路径规划算法和自主充电站，通过仿真和实地测试验证。

Result: 系统表现出高检测精度、高效覆盖和长续航能力，验证了无人机驱赶的可行性。

Conclusion: 无人机驱赶系统为精准农业提供了可扩展的解决方案，未来可进一步推广。
...
### [374] [TartanGround: A Large-Scale Dataset for Ground Robot Perception and Navigation](https://arxiv.org/abs/2505.10696)
...
Method: 通过集成自动化管道收集数据，包括RGB立体相机、深度、光流、LiDAR点云等多种传感器数据，模拟不同地面机器人运动模式。
...
### [375] [GROQLoco: Generalist and RObot-agnostic Quadruped Locomotion Control using Offline Datasets](https://arxiv.org/abs/2505.10973)
...
<details>
  <summary>Details</summary>
Motivation: 解决多足机器人在复杂地形和不同形态下的实时适应问题，避免在线优化的需求。

Method: 利用专家演示数据（包括周期性步态和非周期性步态），直接处理本体感知数据，训练通用模型。

Result: 在多种机器人和地形上实现零样本迁移，包括商用机器人Unitree Go1和70kg的Stoch 5。

Conclusion: GROQLoco展示了通用运动策略在多足机器人和多样化地形中的潜力。
...
### [376] [GrowSplat: Constructing Temporal Digital Twins of Plants with Gaussian Splats](https://arxiv.org/abs/2505.10923)
...
TL;DR: 提出了一种结合3D高斯泼溅和样本对齐管道的框架，用于构建植物的时间数字孪生体。


<details>
  <summary>Details</summary>
Motivation: 植物生长的精确时间重建对表型和育种至关重要，但由于复杂的几何形状、遮挡和非刚性变形而具有挑战性。

Method: 通过多视角相机数据重建高斯泼溅，采用两阶段配准（粗对齐和精细对齐）生成离散时间步的4D模型。

Result: 在荷兰植物表型中心的数据上验证了方法，成功重建了红杉和藜麦的时间模型。

Conclusion: 该方法能够生成详细的植物生长时间模型，为表型研究提供了新工具。
...
### [377] [DexGarmentLab: Dexterous Garment Manipulation Environment with Generalizable Policy](https://arxiv.org/abs/2505.11032)
*Yuran Wang,Ruihai Wu,Yue Chen,Jiarui Wang,Jiaqi Liang,Ziyu Zhu,Haoran Geng,Jitendra Malik,Pieter Abbeel,Hao Dong*

Main category: cs.RO

TL;DR: 论文提出DexGarmentLab环境，专注于灵巧（尤其是双手）服装操作，结合高质量3D资产和改进的模拟技术。通过服装结构对应自动生成数据集，减少人工干预，并提出HALO算法提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 服装操作因类别、几何和变形的多样性而复杂，现有研究难以模拟人类灵巧性。缺乏真实的模拟环境是主要障碍。

Method: 提出DexGarmentLab环境，包含15种任务场景的高质量3D资产和改进的模拟技术。利用服装结构对应自动生成数据集，并提出HALO算法，通过分层策略提升泛化能力。

Result: HALO算法在实验中表现优于现有方法，能泛化到未见过的服装形状和变形场景。

Conclusion: DexGarmentLab和HALO算法为灵巧服装操作提供了有效解决方案，显著减少人工干预并提升泛化能力。
...
### [378] [Planar Velocity Estimation for Fast-Moving Mobile Robots Using Event-Based Optical Flow](https://arxiv.org/abs/2505.11116)
*Liam Boyle,Jonas Kühne,Nicolas Baumann,Niklas Bastuck,Michele Magno*

Main category: cs.RO

TL;DR: 论文提出了一种基于事件相机和平面运动学的新型速度估计方法，解决了传统轮式里程计和IMU融合方法在复杂环境下的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统速度估计方法依赖轮地接触假设或复杂车辆动力学模型，在多变环境（如湿滑路面）下表现不佳。

Method: 结合事件相机的光学流和平面运动学，无需依赖轮地接触假设，利用事件相机的高动态范围和低延迟特性。

Result: 实验表明，该方法在侧向误差上比现有Event-VIO方法提升了38.3%，并在高速场景下验证了有效性。

Conclusion: 该方法在复杂环境下表现优异，具有实际部署潜力。
...
### [379] [Open-Source Multi-Viewpoint Surgical Telerobotics](https://arxiv.org/abs/2505.11142)
...
TL;DR: 论文探讨了通过引入多视角可视化与控制范式，提升微创手术机器人的协作与感知能力，并开源系统以促进研究。


<details>
  <summary>Details</summary>
Motivation: 重新思考手术远程操作的视觉化与控制范式，提升机器感知能力，实现共享自治。

Method: 构建同步多视角、多传感器的手术机器人系统，集成高性能视觉组件并升级控制逻辑。

Result: 系统支持多视角协作与实时3D感知，为算法助手直接控制机器人工具提供可能。

Conclusion: 开源系统将推动研究社区改进技术，加速前沿研究的临床转化。
...
### [380] [Exploiting Radiance Fields for Grasp Generation on Novel Synthetic Views](https://arxiv.org/abs/2505.11467)
*Abhishek Kashyap,Henrik Andreasson,Todor Stoyanov*

Main category: cs.RO

TL;DR: 论文探讨了基于视觉的机器人抓取中，通过合成新视角图像提升抓取精度的方法。


<details>
  <summary>Details</summary>
Motivation: 多视角图像能提高抓取精度，但实际移动摄像头耗时且受限，因此探索通过新视角合成技术补充信息。

Method: 利用高斯泼溅等场景表示技术合成虚拟图像，补充稀疏采样的真实视角。

Result: 在Graspnet-1billion数据集上，合成视角增加了力闭合抓取并提升了覆盖率。

Conclusion: 未来可扩展至单图像构建辐射场，结合扩散模型或通用辐射场技术进一步优化抓取提取。
...
### [381] [PARSEC: Preference Adaptation for Robotic Object Rearrangement from Scene Context](https://arxiv.org/abs/2505.11108)
*Kartik Ramachandruni,Sonia Chernova*

Main category: cs.RO

TL;DR: PARSEC是一个用于学习用户组织偏好的物体重排基准，基于11万条众包数据，提出ContextSortLM模型，其在多场景下表现优于其他模型。


<details>
  <summary>Details</summary>
Motivation: 解决家庭机器人个性化物体重排的挑战，包括无明确指令、多物体环境中的合理放置及泛化能力。

Method: 提出PARSEC基准和ContextSortLM模型，利用多场景上下文学习用户偏好。

Result: ContextSortLM在多场景中表现最佳，优于单上下文模型，但在环境语义建模上仍有挑战。

Conclusion: 多上下文源模型表现更优，未来需改进环境语义建模。
...
### [382] [Conditioning Matters: Training Diffusion Policies is Faster Than You Think](https://arxiv.org/abs/2505.11123)
...
<details>
  <summary>Details</summary>
Motivation: 扩散策略在视觉-语言-动作模型中表现优异，但训练效率低，主要因条件难以区分时训练目标退化为边缘动作分布（损失崩溃）。

Method: 提出Cocos方法，将条件流匹配的源分布改为条件依赖，通过锚定条件输入的语义增强条件整合，防止损失崩溃。

Result: 实验证明Cocos收敛更快、成功率更高，能以更少梯度步和参数匹配大规模预训练模型的性能。

Conclusion: Cocos轻量、易实现且兼容多种策略架构，为扩散策略训练提供了通用改进方案。
...
### [383] [X2C: A Dataset Featuring Nuanced Facial Expressions for Realistic Humanoid Imitation](https://arxiv.org/abs/2505.11146)
*Peizhen Li,Longbing Cao,Xiao-Ming Wu,Runze Yang,Xiaohan Yu*

Main category: cs.RO

TL;DR: X2C数据集和X2CNet框架解决了人形机器人模仿真实面部表情的挑战，提供了高质量的数据集和模仿框架。


<details>
  <summary>Details</summary>
Motivation: 缺乏多样化且标注清晰的人形机器人面部表情数据集阻碍了真实面部表情模仿的进展。

Method: 提出了X2C数据集和X2CNet框架，通过10万对图像和控制值学习人形表情与控制值的对应关系。

Result: X2CNet实现了对不同表演者面部表情的模仿，并在真实人形机器人上展示了其能力。

Conclusion: X2C数据集和X2CNet框架为人形机器人真实面部表情模仿提供了重要基础和潜力。
...
### [384] [Real-Time Verification of Embodied Reasoning for Generative Skill Acquisition](https://arxiv.org/abs/2505.11175)
...
Method: VERGSA结合数学推理验证与技能学习，动态生成任务提示并自动化奖励标注。
...
### [385] [Learning Multimodal AI Algorithms for Amplifying Limited User Input into High-dimensional Control Space](https://arxiv.org/abs/2505.11366)
*Ali Rabiee,Sima Ghafoori,MH Farhadi,Robert Beyer,Xiangyu Bai,David J Lin,Sarah Ostadabbas,Reza Abiri*

Main category: cs.RO

TL;DR: 本文提出了一种新型多模态AI方法ARAS，用于非侵入式高维度运动控制，通过结合深度强化学习和环境感知，显著提升了瘫痪患者的辅助设备控制能力。


<details>
  <summary>Details</summary>
Motivation: 当前侵入式辅助技术存在公众接受度低、寿命有限等问题，而非侵入式方法又依赖易受干扰的信号且训练时间长。本研究旨在解决这些问题。

Method: 采用多模态共享自主框架，结合深度强化学习，将低维度用户输入与环境感知动态融合，实现复杂任务的智能控制。

Result: ARAS在50,000次仿真中表现优异，23名人类受试者测试中实现了92.88%的任务成功率，性能接近侵入式技术。

Conclusion: ARAS为非侵入式高维度运动控制提供了可行方案，性能优越且易于商业化。
...
### [386] [LLM-Explorer: Towards Efficient and Affordable LLM-based Exploration for Mobile Apps](https://arxiv.org/abs/2505.10593)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的探索方法过度依赖LLM生成动作，导致高昂的令牌费用和计算资源消耗，且效果不佳。

Method: LLM-Explorer主要利用LLM维护知识，而非直接生成动作，知识以非LLM方式指导动作生成。

Result: 在20个典型应用上与5个基线比较，LLM-Explorer实现了最快的速度和最高覆盖率，成本比现有LLM方法低148倍。

Conclusion: LLM-Explorer证明了减少LLM依赖并专注于知识维护是一种更高效且经济的移动应用探索方法。
...
### [387] [CRPE: Expanding The Reasoning Capability of Large Language Model for Code Generation](https://arxiv.org/abs/2505.10594)
*Ningxin Gui,Qianghuai Jia,Feijun Jiang,Yuling Jiao,dechun wang,Jerry Zhijian Yang*

Main category: cs.SE

TL;DR: CRPE是一种三阶段框架，用于提升大型语言模型（LLM）的代码推理能力，显著提高了代码生成任务的性能。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在代码生成任务中分析和逻辑处理的挑战。

Method: 采用三阶段框架（数据合成和模型训练），开发了增强版COT-Coder。

Result: COT-Coder-7B-StepDPO和COT-Coder-32B-StepDPO在LiveCodeBench上表现优异，分别达到21.88和35.08的pass@1准确率，超越同类模型。

Conclusion: CRPE是一种全面的开源方法，从数据获取到推理增强机制，显著提升了LLM的代码推理能力。
...
### [388] [The Hitchhikers Guide to Production-ready Trustworthy Foundation Model powered Software (FMware)](https://arxiv.org/abs/2505.10640)
...
TL;DR: 本文探讨了如何将基础模型（FMs）整合为软件核心组件（FMware），并提供了从研究到生产的全面挑战与解决方案。


<details>
  <summary>Details</summary>
Motivation: 基础模型（如大语言模型）正在重塑软件行业，但如何将其有效整合为生产级系统（FMware）仍面临诸多挑战。

Method: 通过分析模型选择、数据对齐、提示工程和自主代理编排等关键问题，结合工业经验和研究，提出技术路线图。

Result: 提供了从演示到生产系统的实用策略，包括测试、优化、部署和与传统软件集成的方法。

Conclusion: 本文为构建可信赖的FMware提供了行动指南和技术路线图，助力行业应对技术变革。
...
### [389] [Let the Trial Begin: A Mock-Court Approach to Vulnerability Detection using LLM-Based Agents](https://arxiv.org/abs/2505.10961)
...
TL;DR: VulTrial是一个多代理框架，通过角色分工提升漏洞检测性能，显著优于单代理基线，并在小数据调优下进一步优化。


<details>
  <summary>Details</summary>
Motivation: 源代码漏洞检测具有挑战性，尤其是当良性代码与漏洞代码相似时。

Method: 采用四角色代理框架（安全研究员、代码作者、主持人和评审团），利用GPT-3.5和GPT-4o进行实验。

Result: VulTrial性能提升显著（GPT-4o下分别提升102.39%和84.17%），小数据调优进一步优化（提升139.89%和118.30%）。

Conclusion: 多代理框架有效提升漏洞检测性能，且成本效益模型（如GPT-3.5）在单代理设置下表现更优。
...
### [390] [DecompileBench: A Comprehensive Benchmark for Evaluating Decompilers in Real-World Scenarios](https://arxiv.org/abs/2505.11340)
...
Method: DecompileBench包含真实世界函数提取（23,400个函数）、运行时感知验证和基于LLM的自动化人本评估。

Result: 基于LLM的方法在代码可理解性上优于商业工具，尽管功能正确性低52.2%。

Conclusion: LLM方法有望改变人本逆向工程，DecompileBench开源以推动研究和帮助安全专家选择工具。
...
### [391] [System Identification and Control Using Lyapunov-Based Deep Neural Networks without Persistent Excitation: A Concurrent Learning Approach](https://arxiv.org/abs/2505.10678)
*Rebecca G. Hart,Omkar Sudhir Patil,Zachary I. Bell,Warren E. Dixon*

Main category: eess.SY

TL;DR: 论文提出了一种基于DNN的控制器，首次实现了同时进行轨迹跟踪和在线系统识别，无需持续激励。通过新的并行学习适应律，实现了DNN参数估计的收敛，并在仿真中表现出显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有DNN控制方法多关注跟踪误差收敛，而忽略了系统动态识别的挑战。本文旨在同时解决轨迹跟踪和系统识别问题。

Method: 构建两种新的并行学习适应律，用于调整DNN各层权重，结合Lyapunov稳定性分析确保跟踪误差、权重估计误差和观测误差的收敛。

Result: 仿真结果显示，在相同初始和操作条件下，函数逼近性能提升了40.5%至73.6%，同时在轨迹外数据点上提升了58.88%至74.75%。

Conclusion: 该方法在无需持续激励的情况下，有效实现了轨迹跟踪和系统识别的双重目标，显著提升了DNN控制器的性能。
...
### [392] [MatTools: Benchmarking Large Language Models for Materials Science Tools](https://arxiv.org/abs/2505.10852)
*Siyu Liu,Jiamin Xu,Beilin Ye,Bo Hu,David J. Srolovitz,Tongqi Wen*

Main category: cond-mat.mtrl-sci

TL;DR: MatTools是一个评估大语言模型（LLM）在材料科学中生成和执行代码能力的基准工具，包含QA和实际任务两部分。


<details>
  <summary>Details</summary>
Motivation: 评估LLM在材料科学问题中的能力，促进更有效的AI系统开发。

Method: 构建MatTools基准，包括69,225个QA对和49个实际任务，评估LLM的代码生成与执行能力。
...
### [393] [Space Group Equivariant Crystal Diffusion](https://arxiv.org/abs/2505.10994)
*Rees Chang,Angela Pak,Alex Guerra,Ni Zhan,Nick Richardson,Elif Ertekin,Ryan P. Adams*

Main category: cond-mat.mtrl-sci

TL;DR: SGEquiDiff是一种生成模型，用于加速晶体材料的逆向设计，通过处理空间群约束和对称性，实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 空间群对称性对材料性质有重要影响，但目前的方法难以自然处理这些约束。

Method: SGEquiDiff结合了SE(3)不变的离散采样器、基于Transformer的自回归采样和空间群等变扩散。

Result: 模型在标准数据集上表现优异，并通过量子力学计算验证。
...
### [394] [TokenWeave: Efficient Compute-Communication Overlap for Distributed LLM Inference](https://arxiv.org/abs/2505.11329)
*Raja Gond,Nipun Kwatra,Ramachandran Ramjee*

Main category: cs.DC

TL;DR: TokenWeave通过令牌分割和优化层归一化计算顺序，显著减少分布式LLM推理中的通信和计算开销，提升性能。


<details>
  <summary>Details</summary>
Motivation: 分布式大型语言模型（LLM）推理在GPU间通信时可能产生高达20%的开销，现有技术虽尝试分解任务但仍有额外开销。

Method: 提出Token-Splitting技术分割令牌批次，重叠计算与通信；优化层归一化顺序；实现融合AllReduce-RMSNorm内核。

Result: 在多种模型和负载下，TokenWeave实现最高29%的延迟降低和26%的吞吐提升。

Conclusion: TokenWeave通过创新优化显著提升性能，甚至优于无通信模型。
...
### [395] [TCC-Bench: Benchmarking the Traditional Chinese Culture Understanding Capabilities of MLLMs](https://arxiv.org/abs/2505.11275)
*Pengju Xu,Yan Wang,Shuyuan Zhang,Xuan Zhou,Xin Li,Yue Yuan,Fengzhao Li,Shunyuan Zhou,Xingyu Wang,Yi Zhang,Haiying Zhao*

Main category: cs.MM

TL;DR: 论文提出了TCC-Bench，一个双语VQA基准，用于评估多模态大语言模型对传统中国文化的理解能力，发现现有模型在文化相关任务上表现不足。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在非西方文化背景下的表现有限，限制了其广泛适用性。

Method: 通过半自动化流程（GPT-4o生成问题+人工筛选）构建TCC-Bench，包含丰富多样的文化数据。

Result: 实验表明，现有模型在文化相关的视觉内容推理上仍面临挑战。

Conclusion: 需进一步研究开发更具文化包容性和情境感知的多模态系统。
...
### [396] [Concept Drift Guided LayerNorm Tuning for Efficient Multimodal Metaphor Identification](https://arxiv.org/abs/2505.11237)
...
<details>
  <summary>Details</summary>
Motivation: 多模态隐喻（如网络迷因）因其非常规表达和隐含意义而难以识别，现有方法在字面和比喻解释间存在差距，且生成方法计算成本高。

Method: CDGLT框架结合概念漂移机制（利用SLERP生成新概念嵌入）和提示构造策略，优化特征提取与融合。

Result: 在MET-Meme基准测试中达到最优性能，同时显著降低训练成本。消融实验验证了概念漂移和LN调优的有效性。

Conclusion: CDGLT为高效准确的多模态隐喻理解提供了重要进展，代码已开源。
...
### [397] [Creating General User Models from Computer Use](https://arxiv.org/abs/2505.10831)
*Omar Shaikh,Shardul Sapkota,Shan Rizvi,Eric Horvitz,Joon Sung Park,Diyi Yang,Michael S. Bernstein*

Main category: cs.HC

TL;DR: 该论文提出了一种通用用户模型（GUM）架构，通过观察用户与计算机的交互行为，学习用户的知识和偏好，并支持灵活推理，以实现更智能的人机交互。


<details>
  <summary>Details</summary>
Motivation: 当前用户模型局限于特定应用，无法实现灵活推理。GUM旨在通过多模态观察构建通用用户模型，以更全面地理解用户需求。

Method: GUM通过输入非结构化用户观察（如设备截图），生成置信度加权的命题，捕捉用户知识和偏好。支持多模态推理、上下文检索和动态更新。

Result: GUM能够准确推断用户需求，并支持主动助手（GUMBOs）发现和执行用户未明确请求的操作。

Conclusion: GUM通过多模态模型理解非结构化上下文，实现了长期的人机交互愿景，并为新型交互系统奠定了基础。
...
### [398] [Large Language Model Use Impact Locus of Control](https://arxiv.org/abs/2505.11406)
...
<details>
  <summary>Details</summary>
Motivation: 探讨AI协作写作如何影响人们的心理控制点。

Method: 通过462名参与者的实证研究，分析就业状态对AI依赖和控制点的影响。
...
### [399] [EdgeWisePersona: A Dataset for On-Device User Profiling from Natural Language Interactions](https://arxiv.org/abs/2505.11417)
*Patryk Bartkowiak,Michal Podstawski*

Main category: cs.HC

TL;DR: 论文提出了一种新数据集和评估基准，用于评估和改进可部署在边缘设备上的小语言模型，重点关注智能家居环境中多会话自然语言交互的用户画像。


<details>
  <summary>Details</summary>
Motivation: 解决小语言模型在用户画像重建任务中的性能不足问题，同时满足边缘设备上的隐私保护和低延迟需求。

Method: 利用结构化用户画像生成模拟交互会话，并评估多种紧凑语言模型在画像重建任务中的表现。

Result: 小模型在用户画像重建任务中表现有限，显著落后于大模型，但边缘设备处理具有隐私和延迟优势。

Conclusion: 该数据集为开发隐私保护的智能AI系统提供了重要测试平台。
...
### [400] [Humans expect rationality and cooperation from LLM opponents in strategic games](https://arxiv.org/abs/2505.11011)
*Darija Barak,Miguel Costa-Gomes*

Main category: econ.GN

TL;DR: 研究探讨人类在与LLMs对手的多玩家p-beauty竞赛中的行为差异，发现人类对LLMs选择更低数字，尤其是高策略能力者。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs融入社会经济互动，需理解人类在战略环境中对LLMs的反应。

Method: 采用受控货币激励实验室实验，比较人类对LLMs和人类对手的行为。

Result: 人类对LLMs选择更低数字，尤其是零纳什均衡选择，高策略能力者更明显。

Conclusion: 研究揭示了人类-LLM多玩家互动中的行为异质性，对混合系统机制设计有重要启示。
...
### [401] [Comparative Analysis of Black-Box Optimization Methods for Weather Intervention Design](https://arxiv.org/abs/2505.10843)
*Yuta Higuchi,Rikuto Nagai,Atsushi Okazaki,Masaki Ogura,Naoki Wakamiya*

Main category: physics.ao-ph

TL;DR: 论文提出了一种基于黑盒优化的天气干预方法，解决了天气控制中梯度信息获取难和计算资源需求高的问题，并通过实验验证了贝叶斯优化在高维搜索空间中的有效性。


<details>
  <summary>Details</summary>
Motivation: 气候变化加剧了天气相关灾害的威胁，天气控制研究变得重要，但优化过程因天气现象的复杂性和计算资源需求而极具挑战。

Method: 采用黑盒优化方法设计天气干预，无需梯度信息，并在两种控制场景（一次性初始值干预和基于模型预测控制的序列干预）中评估。

Result: 实验表明，贝叶斯优化在总降雨量减少方面优于其他方法，尤其在高维搜索空间中表现突出。
...
### [402] [Conceptual framework for the application of deep neural networks to surface composition reconstruction from Mercury's exospheric data](https://arxiv.org/abs/2505.11053)
...
TL;DR: 该研究探索了利用深度神经网络（DNN）从水星中性外逸层测量数据推断其表面成分的可行性，展示了多层感知器（MLP）的潜力。


<details>
  <summary>Details</summary>
Motivation: 通过外逸层测量补充表面成像，研究表面释放过程、行星环境相互作用及演化。

Method: 使用模拟数据训练多层感知器（MLP），预测表面成分。

Result: MLP能准确预测表面成分，验证了方法的可行性。

Conclusion: 该方法有望应用于BepiColombo任务数据，提升外逸层模型分析能力。
...
### [403] [Agent Name Service (ANS): A Universal Directory for Secure AI Agent Discovery and Interoperability](https://arxiv.org/abs/2505.10609)
*Ken Huang,Vineeth Sai Narajala,Idan Habler,Akram Sheriff*

Main category: cs.CR

TL;DR: 本文提出了一种基于DNS的新型架构Agent Name Service (ANS)，用于解决AI代理的安全发现问题，支持多协议通信和可验证身份。


<details>
  <summary>Details</summary>
Motivation: 随着AI代理的普及，缺乏一个公开的代理发现框架，需要一种安全、可扩展的解决方案。

Method: ANS采用PKI证书验证代理身份，提供模块化协议适配层，支持多种通信标准，并定义了安全解析算法。

Result: 实现了基于JSON Schema的结构化通信，并进行了全面的威胁分析，为多代理系统提供了安全发现和交互的基础服务。
...
### [404] [MPMA: Preference Manipulation Attack Against Model Context Protocol](https://arxiv.org/abs/2505.11154)
*Zihan Wang,Hongwei Li,Rui Zhang,Yu Liu,Wenbo Jiang,Wenshu Fan,Qingchuan Zhao,Guowen Xu*

Main category: cs.CR

TL;DR: 论文提出了一种针对MCP协议的新型安全威胁MPMA，通过定制MCP服务器操纵LLM偏好，并设计了两种攻击方法DPMA和GAPMA，后者通过遗传算法提升隐蔽性。


<details>
  <summary>Details</summary>
Motivation: 随着MCP协议的广泛应用，第三方定制版本暴露了潜在安全漏洞，可能导致经济利益的操纵行为。

Method: 设计了两种攻击方法：直接的DPMA和基于遗传算法的GAPMA，后者通过优化描述提升隐蔽性。

Result: 实验证明GAPMA在高效性和隐蔽性之间取得了平衡。

Conclusion: 研究揭示了MCP在开放生态系统中的关键漏洞，呼吁加强防御机制以确保公平性。
...
### [405] [Automating Security Audit Using Large Language Model based Agent: An Exploration Experiment](https://arxiv.org/abs/2505.10732)
*Jia Hui Chin,Pu Zhang,Yu Xin Cheong,Jonathan Pan*

Main category: cs.CR

TL;DR: 本文探讨了利用大型语言模型（LLM）作为自主代理执行部分安全审计（如Windows系统的密码策略合规性审计）的框架，实验显示其比传统手动审计更高效。
...
Method: 通过GPT-4和Langchain进行实验，开发了一个框架，用于自动化执行密码策略合规性审计。

Result: 实验表明，该代理能准确标记密码策略违规，效率高于传统手动审计。

Conclusion: 尽管在复杂动态环境中可能存在局限性，但该框架为实时威胁监控和合规检查提供了扩展可能性。
...
### [406] [Neural-Inspired Advances in Integral Cryptanalysis](https://arxiv.org/abs/2505.10790)
*Liu Zhang,Yiran Yao,Danping Shi,Dongchen Chai,Jian Guo,Zilong Wang*

Main category: cs.CR

TL;DR: 该论文利用神经网络探索密码分析中的积分特性，优化搜索框架，显著提升了积分区分器的性能，并在SKINNY密码上实现了更多轮数的密钥恢复攻击。


<details>
  <summary>Details</summary>
Motivation: 受Gohr等人研究的启发，旨在通过神经网络发现未充分利用的密码分析特征，并将其应用于积分特性的研究中。

Method: 采用神经网络学习积分特性，开发了一种中间相遇搜索框架，平衡准确性和计算效率。

Result: 减少了SKINNY64/64的11轮积分区分器的活跃明文比特数，并发现了12轮密钥依赖积分区分器。此外，还实现了SKINNYn/n的15轮密钥恢复攻击。

Conclusion: 神经网络在密码分析中具有显著潜力，能够发现传统方法无法找到的优化区分器，并提升攻击效率。
...
### [407] [Optimal Allocation of Privacy Budget on Hierarchical Data Release](https://arxiv.org/abs/2505.10871)
*Joonhyuk Ko,Juba Ziani,Ferdinando Fioretto*

Main category: cs.CR

TL;DR: 该论文提出了一种针对分层数据发布的最优隐私预算分配方法，通过约束优化问题最大化数据效用，同时平衡数据粒度和隐私损失。


<details>
  <summary>Details</summary>
Motivation: 分层数据结构中隐私保护与数据效用之间的权衡是一个重要挑战，现有方法常因预算分配不当导致数据无用或隐私保护不足。

Method: 将问题建模为约束优化问题，旨在在总隐私预算下最大化数据效用，并通过理论分析和真实数据集实验验证。

Result: 实验表明，最优隐私预算分配显著提升了发布数据的效用，并改善了下游任务性能。

Conclusion: 该方法为分层数据发布提供了有效的隐私预算分配策略，平衡了数据效用与隐私保护。
...
### [408] [On the Security Risks of ML-based Malware Detection Systems: A Survey](https://arxiv.org/abs/2505.10903)
*Ping He,Yuhao Mao,Changjiang Li,Lorenzo Cavallaro,Ting Wang,Shouling Ji*

Main category: cs.CR

TL;DR: 该论文通过CIA原则定义安全风险范围，将基于机器学习的恶意软件检测系统分解为不同操作阶段，提出阶段分类法，总结技术进展并讨论攻击与防御的不足，通过案例研究提供新见解，并建议未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 现有研究多集中于对抗性恶意软件样本，缺乏对实际安全风险的全面分析，论文旨在填补这一空白。

Method: 利用CIA原则定义安全风险范围，分解系统为操作阶段并建立分类法，总结技术进展，进行案例研究。

Result: 提出了阶段分类法，总结了技术进展与不足，通过案例研究提供了新的实证见解。

Conclusion: 基于分析，建议从阶段内和阶段间两个角度探索未来研究方向。
...
### [409] [Nosy Layers, Noisy Fixes: Tackling DRAs in Federated Learning Systems using Explainable AI](https://arxiv.org/abs/2505.10942)
*Meghali Nandi,Arash Shaghaghi,Nazatul Haque Sultan,Gustavo Batista,Raymond K. Zhao,Sanjay Jha*

Main category: cs.CR

TL;DR: DRArmor是一种新型防御机制，通过结合可解释AI和针对性检测策略，有效对抗联邦学习中的数据重建攻击（DRA），保护用户数据隐私。


<details>
  <summary>Details</summary>
Motivation: 联邦学习（FL）虽然能保护数据隐私，但易受数据重建攻击（DRA）威胁，如LoKI和Robbing the Fed。现有防御方法通常针对整个模型，效果有限。

Method: DRArmor通过分析模型层的贡献和梯度不一致性，识别恶意层，并针对性应用噪声注入、像素化和剪枝等防御技术。

Result: 在200客户端的FL环境中，DRArmor对LoKI攻击表现出色，真阳性率和真阴性率分别为0.910和0.890，平均准确率87%，数据泄漏率降低62.5%。

Conclusion: DRArmor在保护隐私的同时保持了模型性能，优于现有防御机制。
...
### [410] [LLMs unlock new paths to monetizing exploits](https://arxiv.org/abs/2505.11449)
...
TL;DR: LLMs将改变网络攻击的经济学，使攻击者能够针对个体用户定制攻击，而无需依赖大规模漏洞。


<details>
  <summary>Details</summary>
Motivation: 探讨LLMs如何使网络攻击更高效、个性化，以及其对防御策略的新需求。

Method: 通过实验展示LLMs如何自动发现敏感信息并定制攻击，如勒索软件。

Result: LLMs能高效发现敏感信息并实现个性化攻击，部分攻击已具备实际可行性。

Conclusion: LLMs的普及将推动新型防御策略的需求，以应对更智能化的网络威胁。
