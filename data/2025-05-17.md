...
Motivation: 当前大型语言模型需要巨额资源和数据，本文旨在解决上下文嵌入这一子任务，以更高效的方式实现语言建模。

Method: 利用GNN中的图卷积操作编码上下文，并与LSTM结合预测下一个词，实验基于自定义维基百科语料库。

Result: 在有限资源下，该方法能较好地预测下一个词。
...
Method: DRA利用Submodular Mutual Information (SMI)调整奖励，减少冗余补全的权重，增强多样性补全的奖励。

Result: 在五个数学推理基准测试中，DRA-GRPO和DGA-DR.GRPO表现优于基线，平均准确率达58.2%，仅需7,000个微调样本和约55美元的训练成本。

Conclusion: DRA通过显式引入语义多样性，显著提升了GRPO的性能，为低资源语言模型后训练提供了高效解决方案。
...
Method: 通过预注册的大规模激励实验，让参与者完成在线测试，AI或人类说服者试图引导其选择正确答案或错误答案。
...
Result: 在14个未见数据集上验证，优化后的系统提示能快速适应新任务且性能提升。

Conclusion: 双层系统提示优化方法有效提升了LLMs的泛化能力和适应性。
...
TL;DR: VeriFact是一个新框架，旨在通过识别和补全缺失事实来提升长文本生成的事实性评估，同时引入FactRBench基准测试，评估精确率和召回率。
...
Result: VeriFact显著提升事实完整性并保留关键关系信息；FactRBench显示大模型在精确率和召回率上表现更好。
...
<details>
  <summary>Details</summary>
Motivation: 传统文本分析耗时且易受偏见影响，LLMs为高效、高质量的文本分析提供了新工具。

Method: 通过迭代协作过程，结合预定义和数据驱动的分类法，编写提示词并评估分类法，最终应用于数据集。

Result: 展示了如何生成生活领域分类法，并通过测试实现高编码一致性。
...
<details>
  <summary>Details</summary>
Motivation: 固定分词方案导致多语言或专业应用效率低下，现有方法计算成本高且难以保留语义。

Method: 引入Tokenadapt（模型无关的分词器移植方法）和Supertokens预分词学习，结合局部和全局估计初始化新词嵌入。

Result: TokenAdapt显著降低困惑度，优于现有方法如ReTok和TransTokenizer，压缩效率提升明显。

Conclusion: TokenAdapt框架有效解决分词器锁定问题，减少重新训练需求，提升模型性能。
...
Method: 使用GMV的NLP工具uQuery和基于RoBERTa的bsc-bio-ehr-en3模型，对200份乳腺癌和400份肺癌报告进行NER任务，通过Transformers架构微调模型。
...
<details>
  <summary>Details</summary>
Motivation: 探索LLM中‘真理方向’在不同对话格式中的泛化能力，以提升LLM谎言检测的可靠性。

Method: 研究‘真理方向’在短对话和长对话中的表现，并提出在对话末尾添加固定关键词短语的解决方案。

Result: 短对话中‘真理方向’泛化良好，但长对话中表现不佳；添加关键词短语后泛化能力显著提升。

Conclusion: LLM谎言检测在新场景中的泛化仍具挑战性，但通过特定方法可以改进。
...
<details>
  <summary>Details</summary>
Motivation: 填补细读评估在大型语言模型（LLMs）中的空白，探索LLMs在文学解读中的能力。

Method: 设计三个逐步困难的任务集，测试LLMs在提取风格特征、检索上下文信息及多跳推理中的表现。

Result: LLMs在细读任务中表现中等（准确率49.7%-69.7%），但仍落后于人类评估者。

Conclusion: KRISTEVA为LLMs的文学解读能力提供了首个基准，显示其潜力但仍有提升空间。
...
<details>
  <summary>Details</summary>
Motivation: LLMs在自然语言任务中表现出色，但其在冲突预测中的应用尚未充分探索，这对早期预警、人道主义规划和政策制定至关重要。

Method: 通过两部分评估框架，分别在参数化（仅依赖预训练知识）和非参数化（结合外部冲突数据）设置下测试LLMs的预测能力。

Result: 研究发现LLMs在冲突预测中具有潜力，但结合外部知识能显著提升性能。

Conclusion: LLMs在冲突预测中表现有限，但通过结合外部数据可增强其能力，为实际应用提供支持。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在解决西班牙语在不同地区的使用差异，强调区域化AI模型对提升用户体验和实现包容性目标的关键作用。

Method: 通过深入的社会文化和语言情境分析，比较拉丁美洲和西班牙的西班牙语变体差异。

Result: 研究发现这些差异导致社会语言不协调，区域化AI模型能有效弥合分歧并提升用户信任。

Conclusion: 实施五种西班牙语子变体不仅能增强用户对AI的依赖，还能提升国际化战略的文化敏感性。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型（LLMs）的兴起，AI生成文本的滥用问题日益严重，水印技术成为一种潜在解决方案。然而，现有水印方案在鲁棒性、文本质量和安全性之间存在权衡。

Method: 提出了一种混合水印框架，结合logits-based和sampling-based方案，通过串行、并行和混合策略，利用token熵和语义熵自适应嵌入水印。

Result: 实验验证表明，该方法在多种数据集和模型上优于现有基线，达到SOTA性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有提示优化方法依赖大型LLM生成提示，但可能导致轻量级模型响应质量下降，因此需要一种更通用、可解释的优化方法。

Method: 提出模型无关的提示质量指标，并基于轻量级LLM生成的偏好数据集训练MePO优化器。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在通过高效的超参数优化方法提升多任务自然语言模型的性能，特别是在联合实体和关系提取任务中。

Method: 采用分阶段的贝叶斯优化，结合多保真度、超参数空间剪枝和渐进减半技术，并使用Optuna TPE采样器和Hyperband剪枝器。此外，通过元学习器调整分类概率阈值。

Result: 方法在2021年Eberts和Ulges提出的联合实体和关系提取模型变体上得到验证。

Conclusion: 分阶段的超参数优化结合元学习器调整阈值，能有效提升模型性能。
...
<details>
  <summary>Details</summary>
Motivation: LLMs在生成响应时容易因过拟合而产生错误信息（幻觉），主要原因是缺乏及时、准确和个性化的输入数据。

Method: 采用知识图谱（KGs）存储结构化且持续更新的个人数据（如日历数据），并通过检索增强生成（RAG）辅助LLMs生成个性化响应。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机源于对LLMs中潜在偏见的关注，尤其是隐式偏见如何影响模型行为，以及缺乏标准化的评估方法。

Method: 通过结合已有的LLM逻辑和数学问题数据集，引入社会人口统计角色，开发了DIF基准方法。

Result: 结果显示DIF能统计验证LLM中的隐式偏见，且回答准确性与隐式偏见呈负相关。

Conclusion: 结论强调隐式偏见不仅是伦理问题，也是技术问题，DIF为评估LLM偏见提供了标准化工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在长上下文输入中的检索和推理能力不足，难以平衡检索精度和召回率，影响问答效果。
...
TL;DR: 论文探讨了大型语言模型（LLMs）因训练数据问题导致的越狱漏洞，揭示了一种通用攻击方法，并指出行业在AI安全方面的不足。


<details>
  <summary>Details</summary>
Motivation: 研究动机是揭示LLMs因训练数据中的问题内容而容易受到越狱攻击的威胁，以及行业对此的应对不足。

Method: 通过识别和测试一种通用的越狱攻击方法，评估其对多个先进LLMs的有效性。

Result: 研究发现该攻击方法能成功绕过安全控制，使模型生成有害输出，且许多模型在攻击公开后仍易受攻击。

Conclusion: 结论指出，随着LLMs的普及和开源化，缺乏有效干预可能导致危险知识的广泛传播，呼吁行业加强AI安全措施。
...
<details>
  <summary>Details</summary>
Motivation: 探讨PLMs在非洲语言中的表现，揭示其内部机制。

Method: 训练分层探针分析六种非洲语言，设计控制任务验证探针性能。

Result: 优化的PLMs在目标语言中编码更多语言信息，句法和语义信息分布在不同层。

Conclusion: 研究验证了PLMs内部知识，为非洲语言PLMs优化提供依据。
...
<details>
  <summary>Details</summary>
Motivation: 研究LLM在跨语言RAG中的表现，填补现有基准测试的空白。

Method: 构建基于新闻文章的XRAG数据集，涵盖单语和多语检索场景，并提供相关性标注。

Result: 实验发现LLM在单语检索中语言正确性不足，在多语检索中跨语言推理能力较弱。

Conclusion: XRAG为研究LLM推理能力提供了有价值的基准，揭示了跨语言RAG的新挑战。
...
<details>
  <summary>Details</summary>
Motivation: 验证知识注入假设在医学问答中的适用性，并探索微调数据的作用。

Method: 使用S-MedQA数据集，分析不同专业领域微调对模型性能的影响。

Result: 专业领域微调不一定带来最佳表现，领域迁移对性能提升更关键。

Conclusion: 建议重新思考医学领域中微调数据的作用，强调领域迁移的重要性。
...
<details>
  <summary>Details</summary>
Motivation: LLM输出存在不可靠性，尤其是幻觉回答，用户需手动评估，导致信任问题。

Method: 结合知识图谱、检索增强代理、CoT逻辑生成、n跳子图搜索和蕴含句子生成，实现精确证据检索。

Result: 方法提升了现有模型在自由上下文中识别准确证据的能力，为LLM结论提供可靠资源检查。

Conclusion: GE-Chat框架有效增强LLM输出的可信度，帮助用户判断结论的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 当前监督微调和强化学习方法在训练推理模型时局限于特定领域，而持续预训练（CPT）虽无需任务特定信号，但如何有效合成推理数据及其跨领域影响尚不明确。

Method: 提出Reasoning CPT方法，利用合成数据重建文本背后的隐藏思维过程，并将其应用于Gemma2-9B模型，对比标准CPT在MMLU基准上的表现。

Result: Reasoning CPT在所有评估领域均表现更优，推理能力可跨领域迁移，且在难题上表现尤为突出，最高提升8分。模型还能根据问题难度调整推理深度。

Conclusion: Reasoning CPT是一种有效的跨领域推理能力提升方法，尤其在复杂问题上表现显著。
...
<details>
  <summary>Details</summary>
Motivation: 理解大型语言模型中长链思维（CoT）的推理策略多样性，克服基于人类直觉的分类方法的局限性。

Method: 自动提取模型生成的CoT中的推理标准，嵌入语义空间并聚类，生成对比性评分标准以解释推理行为。

Result: 框架比现有方法更具解释性和全面性，并能预测和引导模型使用更有效的推理策略。

Conclusion: 训练数据格式对推理行为影响显著，强调了格式感知模型设计的重要性。
...
Method: 用小型共享码本替换大型输出嵌入矩阵，预测紧凑码本的logits后映射到完整词汇空间。
...
Method: 提出RAIDEN-R1框架，结合VRAR奖励机制，使用单术语和多术语挖掘策略生成量化奖励，并构建高质量角色感知数据集。
...
<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型在多语言环境下处理敏感、领域特定任务的可靠性和适用性。

Method: 比较模型标注与人工标注的黄金标准，分析不同提示条件下的表现及错误模式。

Result: 揭示了模型在跨语言适应性和主观判断任务中的优缺点。

Conclusion: 研究为大型语言模型在现实场景中的应用提供了重要参考，尤其是在多语言和敏感任务中。
...
Method: Quicker采用全自动化流程，从问题分解到临床建议生成，并支持定制化决策和交互式用户界面。

Result: 实验表明Quicker在问题分解、文献筛选和推荐生成方面表现优异，协助人类评审员显著缩短时间至20-40分钟。
...
<details>
  <summary>Details</summary>
Motivation: AI的发展受限于评估质量，而LLM-as-a-Judge模型是核心解决方案。通过强化思维链推理能力，可以提升模型的判断能力。

Method: 采用强化学习方法J1，将可验证和不可验证的提示转化为带有可验证奖励的判断任务，激励思考并减少判断偏差。

Result: J1在8B和70B规模上表现优于其他模型，包括从DeepSeek-R1蒸馏的模型，并在某些基准上超越o1-mini和R1。

Conclusion: J1通过改进思维链推理和奖励策略，显著提升了LLM-as-a-Judge模型的判断能力，为AI评估提供了有效解决方案。
...
Method: 提出LDIR方法，通过最远点采样生成低维（少于500维）密集嵌入，数值表示与不同锚文本的语义相关性。
...
<details>
  <summary>Details</summary>
Motivation: 人类思维本质上是多模态的，而现有研究多局限于单模态输入，限制了语言重建的生态效度。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索如何利用单一模型处理多领域的方面情感分析任务，以减少操作复杂性并保持性能。

Method: 方法包括使用内部数据集，训练一个多领域联合模型，并评估其在四元组意见提取任务中的表现。

Result: 结果表明，多领域联合模型的性能与专用单领域模型相当，同时降低了操作复杂性。

Conclusion: 结论强调了多领域模型的实用性，并分享了处理非提取性预测和评估失败模式的经验。
...
Result: RPG在CodeRepetEval、HumanEval和MBPP基准测试中表现优于基线方法，有效减少重复。

Conclusion: RPG方法能有效缓解代码生成中的结构重复问题，提升生成代码质量。
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM生成的PLS在健康信息理解中的有效性不明确的问题，填补现有评估方法的不足。

Method: 通过大规模众包评估（150名参与者），结合主观评分（简洁性、信息性、连贯性、忠实性）和客观理解测试（多选题）。

Result: LLM生成的PLS在主观评分中表现接近人工写的，但人工写的PLS在理解效果上更优；自动评估指标与人类判断不一致。
...
<details>
  <summary>Details</summary>
Motivation: 现实中的RAG应用常面临长文本输入场景，冗余信息和噪声导致推理成本高且性能下降。

Method: LongRefiner采用双级查询分析、分层文档结构和基于多任务学习的自适应优化，利用单一基础模型实现。

Result: 在七个QA数据集上的实验表明，LongRefiner性能优异，计算成本和延迟仅为最佳基线的1/10。

Conclusion: LongRefiner高效、可扩展且实用，为长文本RAG应用提供了有价值的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统链式思维方法（CoT）是线性因果推理，限制了模型的灵活性。DCoLT旨在通过扩散模型的中间步骤实现更自由的推理方式。

Method: DCoLT将扩散过程的每一步视为潜在“思考”动作，通过基于结果的强化学习优化整个推理轨迹。在SEDD和LLaDA两种扩散语言模型上实现。

Result: 实验显示，DCoLT增强的模型在数学和代码生成任务中表现优于其他方法，LLaDA在多个数据集上的推理准确率显著提升。

Conclusion: DCoLT为扩散语言模型提供了一种高效的推理框架，支持非线性推理，显著提升了任务表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有RAG方法直接使用检索到的文档，但文档有效性差异大，影响模型训练效果。受人类认知学习启发，引入课程学习以提升泛化能力。

Method: 构建多难度级别的训练数据，分阶段训练检索器和生成器，采用课程学习优化性能。

Result: 在四个开放域QA数据集上表现优于现有方法，性能提升2%至4%。

Conclusion: CL-RAG框架有效提升了RAG系统的性能和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前代码安全基准仅关注单一任务，缺乏多维度评估，如安全代码生成、漏洞修复和识别。
...
<details>
  <summary>Details</summary>
Motivation: 在跨语言迁移的标记分类任务中，标签投影是一个关键但具有挑战性的步骤。尽管词对齐器（WAs）和标记方法被广泛使用，但其低层次设计决策的影响尚未系统研究。

Method: 系统研究了词对齐器在标签投影中的低层次设计决策（如标签投影算法、过滤策略和预标记化），并提出了一种集成翻译-训练和翻译-测试预测的新策略。

Result: 优化后的词对齐器性能与标记方法相当，而新提出的集成策略显著优于标记方法，并降低了对低层次设计决策的敏感性。

Conclusion: 通过优化词对齐器的设计决策和引入集成策略，可以提高跨语言迁移在标记分类任务中的鲁棒性和性能。
...
<details>
  <summary>Details</summary>
Motivation: 受语言模型中测试损失与模型和数据规模呈幂律关系的启发，探索偏好建模中是否存在类似规律，并验证其扩展潜力。

Method: 收集公共论坛的偏好数据，使用1.5B至72B参数的模型进行训练，分析不同评估指标（对抗性、客观性和主观性）的缩放行为。

Result: 对抗性和客观性指标随规模和模型增大而提升，主观性指标无显著缩放趋势。WorldPM在多个基准测试中性能提升显著，最高达8%。

Conclusion: WorldPM展示了偏好建模的扩展潜力，为偏好微调提供了有效基础，并在实际应用中表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大型推理模型（LRMs）虽然具备长链推理的潜力，但其推理行为（如自我修正、回溯和验证）的时机和一致性不可预测，限制了其可靠性和可扩展性。

Method: 采用三阶段流程：个体对齐、参数空间合并和领域特定强化学习，通过自动生成的可自验证任务显式对齐元能力。
...
<details>
  <summary>Details</summary>
Motivation: 公众对癌症预防、筛查和治疗的认知不足可能导致诊断延迟和治疗不足，研究旨在评估LLMs在改善这一问题上的潜力。

Method: 采用混合方法评估框架，结合定量指标、定性专家评分和统计方法（Welch's ANOVA、Games-Howell、Hedges' g），比较五款通用和三款医学LLMs的表现。

Result: 通用LLMs在语言质量和情感表达上表现更好，医学LLMs在沟通可访问性上更优，但后者潜在危害、毒性和偏见更高。

Conclusion: 研究揭示了领域专业知识与安全性之间的权衡，强调需针对性改进模型设计以减少危害和偏见，提升安全性和情感表达。
...
<details>
  <summary>Details</summary>
Motivation: 传统超声分析左心房血流动力学的局限性，以及4D Flow MRI在低速度和低分辨率下的挑战，促使开发专用计算框架。

Method: 开发开源计算框架，支持多中心数据的高精度自动分割（Dice > 0.9，Hausdorff 95 < 3 mm），并首次全面评估能量、涡度和压力参数。

Result: 框架在不同质量数据中表现稳健，验证了血流动力学参数作为预后生物标志物的潜力。

Conclusion: 该框架为左心房血流动力学研究提供了标准化工具，并展示了其在疾病预后评估中的应用前景。
...
Method: 采用状态空间模型（SSMs）和简单架构，通过序列拼接实现信息流动，避免复杂的交叉注意力机制。

Result: 在短序列基准上表现优异，长序列生成显著优于Transformer方法，并提出了新的长序列评估基准。

Conclusion: SSM架构为长序列双人运动生成提供了有效解决方案，推动了该领域的研究。
...
Result: PSI表现接近人类学习水平，优于基于非结构化特征向量的原型模型和弱结构化表示的PSI变体。
...
TL;DR: LSG-SLAM是一种基于3D高斯泼溅的大规模视觉SLAM方法，适用于户外场景，通过多模态策略和特征对齐约束提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法多依赖RGBD传感器且仅适用于室内环境，大规模户外场景的鲁棒性重建尚未充分探索。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法在提示模板设计、复杂标记交互或额外微调方面的局限性，提升跨域零/少样本异常检测的灵活性。

Method: 基于CLIP模型，添加三个简单适配器（视觉、文本和提示-查询适配器），交替学习自适应表示，并结合上下文和残差特征进行对比学习。

Result: 在12个工业和医学领域的异常检测基准测试中达到最先进性能，显著优于现有方法。

Conclusion: AdaptCLIP提供了一种无需目标域微调的灵活解决方案，适用于开放场景中的跨域异常检测。
...
TL;DR: 本文提出了一种新的源自由域适应（SFDA）框架，通过预适应生成高质量伪标签和数据依赖频率提示，结合风格相关层微调策略，显著提升了跨模态医学图像分割的性能。
...
Method: 引入预适应生成预适应模型，提出数据依赖频率提示改进图像风格转换，并采用风格相关层微调策略训练目标模型。

Result: 在跨模态腹部和心脏SFDA分割任务中，性能优于现有最先进方法。

Conclusion: 所提框架有效解决了域间隙和伪标签质量问题，为SFDA提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 理解并预测VRU在路口的过街意图对提升交互安全至关重要，误判可能导致危险冲突。

Method: 提出VRU-CIPI框架，结合GRU捕捉时间动态和Transformer自注意力编码上下文与空间依赖。

Result: 在UCF-VRU数据集上达到96.45%准确率，实时性33帧/秒。

Conclusion: 结合I2V通信可提前激活过街信号和预警，提升路口安全性。
...
TL;DR: 论文提出了一种新的遥感变化检测任务——非配准变化检测，以应对自然灾害、人为事故和军事打击等紧急情况。作者系统性地提出了八种现实场景，并开发了针对不同场景的图像转换方案，将现有配准数据集转换为非配准版本。实验表明，非配准变化检测会对现有先进方法造成严重影响。


<details>
  <summary>Details</summary>
Motivation: 针对自然灾害、人为事故和军事打击等紧急情况，现有遥感变化检测方法在非配准场景下表现不佳，亟需解决这一问题。

Method: 系统性地提出八种现实场景，并开发针对不同场景的图像转换方案，将配准数据集转换为非配准版本。

Result: 实验证明，非配准变化检测会对现有先进方法造成严重影响。

Conclusion: 非配准变化检测是一个重要且具有挑战性的任务，需要进一步研究以提升其性能。
...
TL;DR: 提出了一种名为CSPENet的网络，用于红外小目标检测，通过结合轮廓感知和显著性先验，显著提升了检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在低亮度目标定位和密集杂波环境下的轮廓信息感知方面存在不足，限制了检测性能。

Method: 设计了SCPEM模块捕获目标轮廓特征，提出DBPEA架构融合双分支先验，并开发AGFEM模块优化特征表示。

Result: 在多个公开数据集上，CSPENet表现优于现有方法。

Conclusion: CSPENet通过创新的模块设计和先验嵌入，有效解决了红外小目标检测中的关键问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在捕捉纵向依赖性和结构一致性方面存在不足，需改进以支持精准医学中的疾病进展建模。

Method: 结合Mamba长程建模与图引导解剖控制，引入傅里叶增强谱图表示以捕捉空间一致性和多尺度细节。
...
TL;DR: 提出了一种基于纹理关键驱动因素（TKDF）的面部表情识别框架，结合纹理感知特征提取器（TAFE）和双重上下文信息过滤（DCIF），在RAF-DB和KDEF数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 野外环境下的面部表情识别（FER）因表情特征的细微和局部性以及面部外观的复杂变化而具有挑战性。

Method: 通过识别纹理关键驱动因素（TKDF），设计纹理感知特征提取器（TAFE）和双重上下文信息过滤（DCIF）来提取和优化纹理特征。

Result: 在RAF-DB和KDEF数据集上取得了最先进的性能。

Conclusion: 验证了将TKDF纳入FER流程的有效性和鲁棒性。
...
Conclusion: APCoTTA有效解决了ALS点云分割中的CTTA挑战，新基准和代码已开源。
...
TL;DR: HQUIC是一种针对水下图像压缩优化的新方法，通过自适应预测和动态加权多尺度频率组件，显著提升了压缩效率。
...
Method: HQUIC采用ALTC模块预测图像的衰减系数和全局光信息，并利用辅助分支提取常见对象，动态加权多尺度频率组件。
...
<details>
  <summary>Details</summary>
Motivation: 指向能力是语言与视觉场景结合的基础，现有基准仅关注对象定位任务，缺乏多样性。

Method: PointArena包含Point-Bench数据集、Point-Battle交互竞技场和Point-Act机器人系统，用于多场景评估。

Result: Molmo-72B表现最优，专有模型接近其性能，指向任务训练显著提升模型能力。
...
TL;DR: DITM方法通过语言描述性学习图像与文本的层次化相似性，解决现有稀疏监督忽略多对多关系的问题。


<details>
  <summary>Details</summary>
Motivation: 现有图像-文本匹配方法采用稀疏二元监督，忽略了多对多关系和描述层次性。

Method: 提出DITM，利用TF-IDF计算句子描述性分数，动态调整正负对连接，并构建通用到具体的匹配。

Result: 在MS-COCO等数据集上表现优于现有方法，增强了模型的层次推理能力。

Conclusion: DITM通过描述性匹配提升了复杂图像-文本关系的建模能力。
...
TL;DR: 提出了一种基于3D草图的3D服装生成框架，通过结合条件扩散模型和自适应课程学习，使普通用户也能在AR/VR环境中轻松设计高质量虚拟服装。
...
Method: 结合条件扩散模型、共享潜在空间的草图编码器和自适应课程学习策略，从自由手绘输入生成个性化服装。
...
Result: 实验结果显示改进模型的检测精度达65%，在多尺度和小目标检测上表现优异。
...
Method: 构建强化学习环境（状态、动作、奖励），在LDM反向去噪过程中通过PPO优化决策目标。
...
TL;DR: DeepSeqCoco是一种基于深度学习的模型，用于从椰子树图像中自动准确识别疾病，其准确率高达99.5%，优于现有模型，且显著降低了训练和预测时间。
...
Result: 模型准确率高达99.5%，混合SGD-Adam配置的验证损失最低（2.81%），训练和预测时间分别减少18%和85%。
...
<details>
  <summary>Details</summary>
Motivation: 现有一次性学习方法在多领域未标记数据中存在领域偏好问题，且对次优标注图像性能下降，因此需要一种更鲁棒和领域自适应的解决方案。

Method: UOD框架包含两阶段：第一阶段通过自监督学习生成伪标志物标签，第二阶段使用领域自适应Transformer消除领域偏好并建立全局上下文。

Result: 在三个不同解剖领域的X射线数据集上，UOD实现了最先进的性能。
...
TL;DR: 本文深入探讨了文本到图像合成中LLMs与DiTs融合的设计空间，填补了现有研究在详细比较和训练方法上的空白。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注整体系统性能，缺乏对替代方法的详细比较和训练细节的公开，导致该方法潜力不明。

Method: 通过实证研究，进行与基准方法的对照比较，分析关键设计选择，并提供可复现的大规模训练方案。

Result: 提供了有意义的数据点和实用指南，为多模态生成的未来研究奠定基础。

Conclusion: 本文填补了研究空白，为多模态生成领域提供了清晰的方向和可操作的训练方法。
...
TL;DR: 该论文综述了动态场景表示与重建领域的最新进展，重点分析了200多篇相关论文，涵盖从隐式神经表示到显式高斯基元的方法。


<details>
  <summary>Details</summary>
Motivation: 动态场景表示与重建在近年来取得显著进展，但缺乏系统性总结，本文旨在填补这一空白，为研究者和实践者提供全面参考。

Method: 通过分类和评估200多篇论文，从运动表示范式、重建技术、辅助信息整合和正则化方法等角度进行分析。

Result: 论文系统总结了动态场景重建的方法和挑战，提出了统一的表示框架，并指出了未来研究方向。

Conclusion: 本文为动态场景重建领域提供了系统性综述，为研究者提供了参考，并指明了未来发展的潜在方向。
...
TL;DR: 本文评估了大型多模态模型（LMMs）在低资源普什图语OCR任务中的表现，开发了合成数据集PsOCR，并比较了多个开源和闭源模型的性能。


<details>
  <summary>Details</summary>
Motivation: 普什图语的NLP面临挑战，如草书字体和数据集稀缺，因此需要开发适合的OCR数据集和评估模型。

Method: 开发了包含100万张图像的合成数据集PsOCR，覆盖多种字体和布局，并评估了多个LMMs的性能。

Result: Gemini在闭源模型中表现最佳，而Qwen-7B在开源模型中表现突出。

Conclusion: 本研究为普什图语OCR提供了评估基础，并对类似脚本（如阿拉伯语、波斯语和乌尔都语）的研究具有参考价值。
...
TL;DR: ToonifyGB是一个两阶段框架，用于生成多样化的3D风格化头部头像。第一阶段生成风格化视频，第二阶段合成高斯混合形状，实现高效渲染。


<details>
  <summary>Details</summary>
Motivation: 扩展Toonify框架，利用高斯混合形状生成多样化的3D风格化头部头像。

Method: 1. 改进的StyleGAN生成风格化视频；2. 从视频中学习风格化中性头部模型和表情混合形状。

Result: 在Arcane和Pixar风格上验证了ToonifyGB的有效性。

Conclusion: ToonifyGB能够高效生成高质量的风格化3D头像。
...
Method: 引入模态无关表示空间，生成表示令牌并插入高层编码器，联合优化类别和表示特征，并引入正则化项。MMRL++进一步减少参数并增强模态内交互。

Result: 在15个数据集上超越现有方法，平衡任务适应和泛化。

Conclusion: MMRL和MMRL++有效提升少样本学习性能，泛化能力显著。
...
TL;DR: MoB提出了一种多目标平衡覆盖方法，通过贪心半径分配解决视觉令牌修剪中的目标权衡问题，显著提升了性能和效率。
...
Method: 基于Hausdorff距离和ε-覆盖理论，提出MoB框架，将修剪问题转化为双目标覆盖问题，并通过贪心半径分配优化预算。

Result: MoB在LLaVA-1.5-7B上仅使用11.1%的原始令牌即可保留96.4%性能，并加速LLaVA-Next-7B 1.3-1.5倍。

Conclusion: MoB不仅高效且可扩展，还能无缝集成到先进的多模态大语言模型和多样化视觉语言任务中。
...
TL;DR: 提出了一种基于条件U-Net架构的图像配准方法，用于估计未知条件下的图像，并在放疗中应用于4D-CT扫描的肿瘤运动估计。
...
Method: 采用条件U-Net架构，无需固定图像，充分利用条件信息，实现图像配准。
...
TL;DR: 论文提出了一种多源协作风格增强和域不变学习方法（MCSAD），用于联邦域泛化，通过扩展风格空间和跨域特征对齐提升模型泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有风格增强方法在数据分散场景下风格空间有限，无法充分泛化到未见目标域。

Method: 提出多源协作风格增强模块和域不变学习，通过跨域特征对齐和类关系集成蒸馏学习域不变模型。

Result: 在多个域泛化数据集上显著优于现有联邦域泛化方法。

Conclusion: MCSAD通过协作风格增强和域不变学习，有效提升了模型在未见目标域的泛化性能。
...
TL;DR: 论文提出了一种新架构，通过少量数据集特定参数解决跨数据集显著性预测的泛化问题，显著提升性能。
...
Result: 模型在MIT/Tuebingen基准测试中达到新SOTA，泛化性能提升75%，仅需50样本即可显著改善。
...
### [77] [Unlocking Location Intelligence: A Survey from Deep Learning to The LLM Era](https://arxiv.org/abs/2505.09651)
*Xixuan Hao,Yutian Jiang,Xingchen Zou,Jiabo Liu,Yifang Yin,Yuxuan Liang*

Main category: cs.CV

TL;DR: 论文综述了地理空间表示学习在深度学习和大语言模型（LLM）时代的发展，从数据、方法和应用三个视角构建了分类体系，并探讨了当前进展、局限性和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 地理空间数据在现代空间决策中至关重要，深度学习和大语言模型的出现为地理空间表示学习带来了革命性变革，需要系统梳理和总结。

Method: 通过构建分类体系，从数据、方法和应用三个视角全面回顾地理空间表示学习的技术发展。

Result: 论文总结了深度学习和大语言模型在地理空间表示学习中的成功应用，并提出了未来研究方向。

Conclusion: 地理空间表示学习在LLM时代具有广阔前景，本文为领域创新提供了路线图。
...
TL;DR: VolE是一种基于移动设备3D重建的食物体积估计框架，无需参考物或深度信息，通过视频分割生成食物掩模，性能优于现有方法。
...
Method: 利用移动设备捕捉图像和相机位置，通过AR技术生成3D模型，结合视频分割生成食物掩模。

Result: 实验显示VolE在多个数据集上表现优异，平均绝对百分比误差为2.22%。

Conclusion: VolE在食物体积估计中表现出色，为医疗营养管理和健康监测提供了有效工具。
...
TL;DR: 论文探讨了医学图像分割模型在真实临床场景中性能下降的问题，提出MixUp和Auxiliary Fourier Augmentation两种数据增强方法，显著提升了模型在分布外数据上的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割模型在真实临床场景中性能下降，主要原因是训练数据与测试数据分布不匹配。传统数据增强方法在多样化真实场景中缺乏鲁棒性。

Method: 系统评估了MixUp和Auxiliary Fourier Augmentation两种数据增强方法，这些方法无需针对特定分布偏移源，即可缓解多种变化的影响。

Result: 实验证明，这些方法显著提升了心脏电影MRI和前列腺MRI分割任务中的泛化能力和鲁棒性，同时增强了特征表示的可分离性和紧凑性。

Conclusion: 将MixUp和Auxiliary Fourier Augmentation集成到nnU-Net训练流程中，提供了一种易于实现且有效的解决方案，提升了医学分割模型在真实应用中的可靠性。
...
TL;DR: 探索人类-AI对齐在医学影像中的公平性，发现适度结合人类见解可减少公平性差距，但需平衡以避免性能损失。


<details>
  <summary>Details</summary>
Motivation: 解决深度神经网络在医学影像中因偏见导致的公平性差距问题。

Method: 系统研究人类-AI对齐对公平性和泛化能力的影响。

Result: 结合人类见解减少公平性差距并提升泛化能力，但过度对齐可能带来性能权衡。
...
TL;DR: MTVCrafter是一个直接建模3D运动序列的人类图像动画框架，通过4D运动标记和运动感知视频DiT，实现了更灵活和分离的控制，并在实验中取得了最先进的结果。
...
Result: MTVCrafter在FID-VID上达到6.98，优于第二名65%，并能泛化到多样化的开放世界角色。

Conclusion: MTVCrafter在人类图像动画领域迈出了重要一步，为姿势引导的视频生成开辟了新方向。
...
TL;DR: ADHMR通过偏好优化对齐扩散模型，提升单图像3D人体网格恢复的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决现有概率方法在单图像3D人体网格恢复中与2D观测不匹配及对野外图像鲁棒性差的问题。

Method: 训练HMR-Scorer评估预测质量，构建偏好数据集，通过直接偏好优化微调基础模型。

Result: ADHMR在实验中优于现有方法，且HMR-Scorer能提升其他模型的性能。

Conclusion: ADHMR通过偏好优化显著提升了3D人体网格恢复的准确性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 智能驾驶座舱需满足不同用户的舒适、交互和安全需求，因此需要一种能够适应多样化输入和个性化反应的智能代理。

Method: SAGE DeeR通过多视角多模态输入推理用户状态，并结合语言空间的隐式思维链增强能力，同时构建了大规模基准测试。

Result: SAGE DeeR实现了超级对齐（个性化反应）、通用性（多模态理解）和自我激发能力，并通过基准测试验证了其感知决策和对齐准确性。
...
TL;DR: 提出了一种新颖的离线地图构建方法，通过整合非正式路线（trails）并使用基于Transformer的深度学习模型，实现高效且泛化能力强的地图更新。


<details>
  <summary>Details</summary>
Motivation: 高精地图对自动驾驶规划至关重要，但传统在线地图构建面临时间一致性、传感器遮挡等问题，需要更高效且泛化的解决方案。

Method: 利用自车和其他交通参与者的trail数据，通过Transformer模型构建全局地图，支持持续更新且传感器无关。

Result: 在基准数据集上验证，性能优于现有在线地图构建方法，泛化能力更强。

Conclusion: 该方法为自动驾驶系统提供了更鲁棒和高效的地图构建方案。
...
TL;DR: 论文提出HandReader，包含三种架构（RGB、KP、RGB+KP），用于手语拼写识别，在多个数据集上取得先进结果，并发布俄语手语数据集Znaki。


<details>
  <summary>Details</summary>
Motivation: 手语拼写识别在视频处理中仍有提升空间，需改进准确性。
...
Conclusion: HandReader模型高效且灵活，Znaki数据集填补俄语手语资源空白。
...
TL;DR: 论文介绍了首个多区域、多卫星的海洋雾数据集MFogHub，解决了现有数据集单一性的问题，提升了模型的泛化能力和预测技术开发。


<details>
  <summary>Details</summary>
Motivation: 现有海洋雾数据集局限于单一区域或卫星，限制了模型评估和雾特性研究，需要更全面的数据集。

Method: 构建了包含15个沿海雾区和6颗卫星的68,000个高分辨率样本的MFogHub数据集，并测试了16个基线模型。

Result: MFogHub揭示了区域和卫星差异导致的泛化波动，为雾预测技术提供了宝贵资源。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的CZSL方法忽视了其架构和训练范式在细粒度局部特征捕捉上的局限性。

Method: 设计了两个自适应聚合器，分别从低级和高级视觉特征中提取局部和全局信息，并通过分阶段交互机制逐步整合到文本表示中。

Result: 在三个广泛使用的数据集上的实验验证了模型的有效性和优越性。

Conclusion: MSCI模型通过动态调整全局和局部视觉信息的注意力权重，显著提升了细粒度局部视觉信息的感知能力。
...
TL;DR: 论文提出StoryReasoning数据集，通过视觉相似性和面部识别解决视觉叙事中的角色和对象一致性问题，并展示了基线模型Qwen Storyteller的性能提升。


<details>
  <summary>Details</summary>
Motivation: 视觉叙事系统在跨帧保持角色一致性和正确关联动作与主体方面存在挑战，导致引用幻觉问题。

Method: 提出StoryReasoning数据集，包含4,178个故事，采用跨帧对象重识别、链式思维推理和视觉实体链接方法。

Result: 基线模型Qwen Storyteller将幻觉问题平均减少12.3%（从4.06降至3.56）。

Conclusion: 通过视觉实体链接和多帧关系建模，StoryReasoning有效提升了叙事一致性和减少幻觉问题。
...
TL;DR: MIPHEI是一种基于U-Net和ViT的模型，用于从H&E染色图像预测mIF信号，实现了高精度的细胞类型分类，为大规模H&E数据集的分析提供了新方法。


<details>
  <summary>Details</summary>
Motivation: 解决mIF技术因成本和物流限制未广泛临床应用的问题，通过H&E图像预测mIF信号，提升细胞类型识别的效率和可及性。

Method: 采用U-Net架构结合ViT基础模型作为编码器，利用ORION数据集训练，并在两个独立数据集上验证。

Result: 模型在多个标记物上表现优异，如Pan-CK（F1=0.88）、CD3e（F1=0.57），显著优于基线模型和随机分类器。

Conclusion: MIPHEI成功捕捉H&E图像与分子标记的关系，为细胞类型分析和患者预后研究提供了新工具。
...
TL;DR: 论文提出了一种名为PartCrop的统一成员推理方法，用于攻击视觉自监督模型，并在不同训练协议和结构上验证了其有效性。同时，提出了防御方法并进行了实验验证。


<details>
  <summary>Details</summary>
Motivation: 自监督学习在利用大量未标记数据方面具有潜力，但也面临隐私问题。研究旨在解决在未知训练方法和细节的黑盒设置下，对自监督模型进行成员推理的挑战。
...
Conclusion: PartCrop是一种有效的成员推理方法，同时提出了防御策略。研究还扩展了PartCrop的规模适应性，并提出了改进版本PartCrop-v2。
...
Result: 在视频分类、姿态跟踪和语义分割任务中，性能优于现有SNN方法，且能效显著提升。

Conclusion: SpikeVideoFormer在性能和效率上均表现优异，为视频任务提供了高效的SNN解决方案。
...
TL;DR: 提出一种无需成对数据的训练方法，用于学习型ISP，通过对抗训练和多判别器实现高质量图像转换。
...
Method: 采用多损失函数和对抗训练，利用预训练网络的特征图保持内容结构，同时学习目标RGB数据集的颜色和纹理特征。
...
TL;DR: 论文探讨了视觉语言模型（VLM）对图像中虚拟对象的理解能力，发现现有模型表现不足。


<details>
  <summary>Details</summary>
Motivation: 研究VLM是否真正理解图像的视觉空间属性，尤其是对虚拟对象的处理能力。

Method: 通过描述虚拟对象（如图中未出现的风筝）来测试VLM的场景理解能力，并进行系统评估。

Result: 现有VLM对虚拟对象的处理能力不足。

Conclusion: VLM在理解虚拟对象方面仍需改进。
...
TL;DR: ControlGS是一种3D高斯溅射优化方法，通过单次训练实现用户可调的渲染质量与高斯数量权衡，适用于多样场景。


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏用户直观调整高斯数量与渲染质量权衡的能力，无法满足不同硬件和通信限制下的实际需求。

Method: 通过固定设置和用户指定的超参数，ControlGS自动寻找高斯数量与渲染质量的最佳权衡点。

Result: ControlGS在多种场景中表现优于基线方法，能以更少的高斯实现更高的渲染质量，并支持广泛的调整范围。

Conclusion: ControlGS提供了一种灵活且高效的方法，适用于多样化的实际应用场景。
...
TL;DR: 论文研究了孤立手语识别（ISLR）中的跨语言模型训练和相似手语标注问题，提出了新的俄罗斯手语数据集Logos，并展示了其在跨语言任务中的有效性。
...
Method: 提出Logos数据集，探索跨语言迁移学习方法，并研究多分类头联合训练。

Result: Logos数据集作为预训练模型可提升其他语言任务性能，标注相似手语组提高了模型质量。

Conclusion: Logos数据集和方法在WLASL和AUTSL数据集上取得了优于或竞争性的结果。
...
<details>
  <summary>Details</summary>
Motivation: 当前统一多模态模型的评估缺乏统一框架，存在冗余、多样性不足等问题，亟需简化且全面的评估方法。

Method: 提出UniEval框架，包含UniBench基准（81个细粒度标签）和UniScore指标，支持统一和视觉生成模型。

Result: UniBench更具挑战性，UniScore与人类评估高度一致，优于现有指标，并揭示了SoTA模型的新见解。

Conclusion: UniEval为统一多模态模型提供了高效、全面的评估方案，填补了研究空白。
...
<details>
  <summary>Details</summary>
Motivation: 当前医学领域生成AI的评估存在方法不一致、架构比较过时和评估标准不统一的问题，缺乏对临床实用性的关注。

Method: 通过标准化数据分区和包含20多个定量指标的评估协议，对11种领先的文本到图像生成架构进行系统分析。

Result: 发现现有评估协议在生成保真度方面存在不足，导致不一致和无意义的比较。

Conclusion: CheXGenBench为医学AI社区提供了标准化基准，支持客观和可重复的比较，并发布了高质量数据集SynthCheX-75K。
...
TL;DR: 提出一种双分支分类策略，增强人脸识别系统对形态攻击的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 人脸识别技术广泛应用，但面临形态攻击等安全威胁，需提升系统鲁棒性。

Method: 通过双分支分类策略，将形态图像纳入训练，解决标签模糊问题。

Result: 在公开基准测试中验证了方法的有效性，提升了对抗形态攻击的能力。
...
TL;DR: 本文提出了一种改进的多图像问答任务中的检索框架，利用子模子集选择技术（如GraphCut）预选语义相关图像，提升检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有大型多模态模型在单图像任务中表现良好，但在多图像场景（如多图像问答）中面临扩展性和检索性能问题。

Method: 采用查询感知的子模函数（如GraphCut）预选相关图像，并结合锚点查询和数据增强优化检索流程。

Result: 在大量图像中，该方法显著提升了检索管道的有效性。

Conclusion: 子模子集选择技术可有效增强多图像任务的检索性能。
...
TL;DR: 论文提出了一种新指标“注意力准确率”和基准，用于量化多模态大语言模型（MLLMs）在视觉理解中的隐式视觉误解（IVM），并通过解耦视觉和文本模态验证其有效性。
...
Method: 通过解耦视觉和文本模态的因果注意力模块，分析注意力分布，并提出“注意力准确率”这一指标和新的基准来量化IVM。

Result: 研究发现，随着网络层加深，注意力分布逐渐集中在与正确答案相关的图像上，验证了注意力准确率的有效性。

Conclusion: 注意力准确率能够直接评估模型的视觉理解能力，且不受位置偏差影响，具有通用性和可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 随着扩散模型的发展，合成数据的训练效果逐渐提升，但模型仍会生成不现实的图像。研究旨在验证合成图像中可行性是否对CLIP分类器的训练至关重要。

Method: 提出VariReal流程，通过最小化编辑源图像以生成可行或不可行属性，并测试其对LoRA微调CLIP性能的影响。

Result: 实验表明，可行性对性能影响极小（<0.3%），且混合训练数据集对性能无显著影响。

Conclusion: 合成图像的可行性对CLIP分类器训练影响有限，无需严格区分可行与不可行图像。
...
TL;DR: 论文提出利用代码作为监督信号，解决数学图像与文本对齐问题，构建了图像到代码模型FigCodifier和数据集ImgCode-8.6M，并进一步生成多模态数学指令数据集MM-MathInstruct-3M，训练出MathCoder-VL模型，在数学推理任务中表现优异。
...
Method: 利用代码作为监督信号对齐图像和文本，构建图像到代码模型FigCodifier和数据集ImgCode-8.6M，生成多模态数学指令数据集MM-MathInstruct-3M，并训练MathCoder-VL模型。
...
TL;DR: ETT是一种端到端的视觉标记器调优方法，通过联合优化视觉标记化和目标任务（如图像生成和视觉问答），解决了现有视觉标记化与下游任务脱节的问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉标记化方法将标记器优化与下游训练分离，导致视觉标记无法适应多样化任务需求，成为性能瓶颈。

Method: ETT利用视觉嵌入和标记器代码本，通过重建和标题目标联合优化视觉标记器，无需修改现有架构。

Result: 实验表明，ETT在多项任务上性能提升2-6%，同时保持原始重建能力。
...
TL;DR: Prior Depth Anything框架结合不完整但精确的深度测量与完整但相对的深度预测，生成准确、密集且详细的深度图。通过粗到细的流程整合两种互补深度源，并在多个任务中展示出色的零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决深度测量中不完整但精确的信息与深度预测中完整但相对的结构之间的互补性问题，以生成更高质量的深度图。

Method: 设计粗到细的流程：1) 像素级度量对齐和距离感知加权预填充先验；2) 基于归一化先验和预测的条件单目深度估计模型，隐式融合两种深度源。

Result: 在7个真实数据集上，零样本泛化能力优于或匹配任务特定方法，尤其在混合先验和测试时改进方面表现突出。

Conclusion: 该框架灵活高效，能随着MDE模型的进步而演进，为深度估计任务提供了新的解决方案。
...
TL;DR: 3D-Fixup是一个基于3D先验的2D图像编辑框架，支持复杂编辑如物体平移和3D旋转，通过结合扩散模型和视频数据实现高质量编辑。
...
Method: 利用视频数据生成训练对，结合扩散模型和Image-to-3D模型的3D指导，设计数据生成流程确保高质量3D指导。

Result: 3D-Fixup实现了复杂且身份一致的3D感知编辑，结果质量高。
...
TL;DR: 论文提出了一种新的搜索算法，在短时间和中等搜索时间下，其在双人零和完美信息游戏中表现优于现有算法。
...
Result: 短搜索时间下，新算法在所有游戏中表现最佳；中等搜索时间下，在17/22游戏中表现最佳。
...
TL;DR: 本文改进了现有技术，提出高效算法检测特征的必要性和相关性，并引入全局有用性概念，验证其在决策树和复杂模型中的实用性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在识别特征重要性和必要性方面存在不足，尤其是在复杂模型中效率低下。本文旨在改进这些方法并引入新概念。

Method: 基于命题逻辑和充分理由概念，提出高效算法检测特征的必要性和相关性，并引入全局有用性概念。

Result: 实验证明，新算法在复杂模型中高效，全局有用性与相关性和必要性相关。
...
TL;DR: 论文提出了一种基于无模型目标条件强化学习的方法，用于动态环境中的实时目标识别（GR）。
...
Result: 该方法能够有效应对动态环境中目标的变化，提升GR系统的实时性。

Conclusion: 提出的方法为动态GR问题提供了新的解决方案，并推动了该领域的进一步研究。
...
TL;DR: XpertXAI是一种基于专家驱动的可解释AI模型，用于多肺病检测，优于现有方法，并提供与专家判断一致的临床解释。


<details>
  <summary>Details</summary>
Motivation: 解决深度学习模型在肺病检测中决策不透明的问题，推动临床可解释AI的应用。

Method: 扩展ClinicXAI，采用InceptionV3分类器和公共数据集，对比XpertXAI与现有解释方法及无监督CBM。

Result: XpertXAI在预测准确性和临床解释一致性上优于基线方法。

Conclusion: XpertXAI展示了可扩展的人类中心模型设计在医学诊断中的潜力。
...
TL;DR: 论文研究了离线强化学习算法在微电网电压调节中的应用，通过离线训练在缺乏在线交互的情况下仍能获得有效模型。


<details>
  <summary>Details</summary>
Motivation: 由于技术或安全原因无法进行在线环境交互时，如何利用离线数据训练模型以调节微电网电压。

Method: 采用不同离线强化学习算法，在IEEE 33-bus系统上进行实验。

Result: 实验表明，即使在低质量数据下，该方法仍具有可行性和有效性。

Conclusion: 离线强化学习可用于微电网电压调节，尤其在缺乏在线交互时表现良好。
...
TL;DR: 该研究提出了一个系统性的问题质量评估框架，定义了好问题的两个关键维度：适当性和有效性，并通过半自适应标准实现结构化和灵活性。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏对问题质量的全面评估研究，因此该研究旨在填补这一空白。

Method: 提出了基于适当性和有效性的评分系统，并利用动态上下文变量实现半自适应标准。
...
TL;DR: 本文回顾了AI从基础规则系统到复杂自主代理的发展历程，探讨了关键技术里程碑及其社会影响，认为当前AI可能已达到‘最终一代’，并呼吁智慧应对新时代。


<details>
  <summary>Details</summary>
Motivation: 探讨AI技术的快速发展及其对社会的影响，强调当前AI代理的能力可能标志着技术发展的顶峰。

Method: 通过分析关键技术里程碑（如提示、训练方法、硬件和架构创新）和实际案例（如ChatGPT和Grok），研究AI的进步。

Result: 指出AI能力飞速提升，智能可能每六个月翻倍，当前AI代理可能代表‘最终一代’。
...
TL;DR: Pre-Act是一种新方法，通过多步执行计划和详细推理提升代理性能，优于ReAct，并在小型模型上通过微调实现显著改进。


<details>
  <summary>Details</summary>
Motivation: 提升大型语言模型在代理系统中的性能，尤其是针对复杂推理任务和小型模型的局限性。

Method: 提出Pre-Act方法，生成多步执行计划并逐步细化，结合微调小型模型（如Llama 3.1）。

Result: Pre-Act在Almita数据集上比ReAct表现更好，微调后的70B模型甚至优于GPT-4。

Conclusion: Pre-Act方法有效提升代理系统性能，尤其适用于复杂任务和小型模型优化。
...
TL;DR: 该论文提出了一种多模态个性化抑郁检测挑战（MPDD），旨在解决现有抑郁检测方法在年龄多样性和个体差异上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有抑郁检测方法主要关注年轻人，忽略了其他年龄群体和个体差异，导致检测效果有限。

Method: 通过融合音频、视频模态和个体差异信息，建立基线模型，分别针对老年人和年轻人进行抑郁检测。

Result: MPDD挑战提供了两个年龄特定的数据集（MPDD-Elderly和MPDD-Young），并提出了一个多模态融合的基线模型。

Conclusion: 该挑战旨在推动更个性化、准确的抑郁检测方法的发展，促进心理健康研究和包容性检测系统的进步。
...
TL;DR: 论文提出了一种基于图RAG的管道，利用教育知识图谱和个人知识图谱，帮助MOOC学习者理解知识概念。


<details>
  <summary>Details</summary>
Motivation: MOOC中学习者与教师互动不足，LLMs存在幻觉问题，传统RAG系统无法主动引导学习者。

Method: 结合EduKG和PKG，实现个性化问题生成和基于知识图谱的问答。

Result: 在CourseMapper平台上评估，结果显示Graph RAG能有效支持个性化学习。

Conclusion: Graph RAG为MOOC学习者提供了理解新知识概念的个性化解决方案。
...
TL;DR: 该研究利用生成式AI和大型语言模型，将台湾中国研究领域的非结构化学术文本转化为结构化知识图谱，支持网络化知识导航。


<details>
  <summary>Details</summary>
Motivation: 回应系统回顾和重组台湾中国研究学术成果的需求，探索生成式AI在区域研究和数字人文中的应用。

Method: 应用生成式AI技术，从1367篇同行评审文章中提取实体关系三元组，并通过D3.js可视化构建知识图谱。

Result: 构建了领域特定知识图谱和向量数据库，揭示了未探索的学术轨迹、主题集群和研究空白。

Conclusion: 展示了生成式AI如何增强区域研究，并为区域知识系统提供了一种可扩展的数据驱动替代方案。
...
TL;DR: 本文探讨了在医疗AI中如何通过不同类型的解释增强透明度和信任，并通过医生调查和访谈评估了最有效的解释方式。
...
Result: 研究识别了能有效支持诊断决策的解释类型。

Conclusion: 研究结果为医疗AI中设计更有效的解释提供了实用见解。
...
TL;DR: MASS是一种基于LLM的多智能体模拟框架，用于投资组合构建，通过动态增加智能体数量和反向优化实现稳定超额收益。
...
Method: 动态扩展智能体规模，通过反向优化优化分布，实现端到端优化。

Result: 在多个实验中优于6种基线方法，展示了稳定超额收益。

Conclusion: MASS框架可扩展至类似任务，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估方法假设人类常识是单一的，但实证显示人类常识存在巨大差异。本文旨在提出更准确的评估框架。

Method: 通过测量模型判断与人类群体的对应关系，评估LLMs的常识能力，并比较不同模型的性能。

Result: LLMs在常识能力上低于人类中位数，与人类相关性有限；小型开源模型表现优于大型专有模型。

Conclusion: 框架强调AI需适应不同文化背景的人类群体，为常识评估提供了新视角。
...
TL;DR: 本文研究了医疗人员排班问题，比较了SMT和MILP两种方法在解决实际问题中的表现，发现SMT在复杂排班问题中表现更优。
...
Method: 提出通用约束模型，分别用SMT和MILP求解，并比较Z3和Gurobi求解器在学术和实际排班问题中的表现。

Result: 实验显示，MILP在高度约束或无解问题中表现更好，而SMT在复杂实际排班问题中更优，但对约束表达更敏感。

Conclusion: SMT方法在医疗排班领域具有潜力，未来研究值得深入。
...
TL;DR: 本文提出了‘可塑性’作为代理受观察影响的能力的度量，揭示了其与‘赋能’的镜像关系，并探讨了二者在代理设计中的权衡。


<details>
  <summary>Details</summary>
Motivation: 研究代理如何受其观察影响，并量化这种影响，以补充‘赋能’概念，完善对代理行为的理解。

Method: 通过定义新的信息论量‘广义定向信息’来量化可塑性，并分析其与赋能的关系。

Result: 发现可塑性与赋能互为镜像，且二者在代理设计中存在权衡关系。

Conclusion: 可塑性和赋能及其关系对理解代理行为至关重要，代理设计需兼顾二者。
...
<details>
  <summary>Details</summary>
Motivation: 现有解释评估方法依赖理想真实解释或模型敏感性，存在局限性，需要更独立的质量评估标准。

Method: 提出AXE框架，基于三个原则设计，无需真实解释或模型敏感性，提供独立评估。

Result: AXE验证了其有效性，可用于检测解释的公平性伪装。

Conclusion: AXE为解释评估提供了独立且实用的新方法，解决了现有方法的局限性。
...
TL;DR: 本文区分了AI Agents与Agentic AI，提出概念分类、应用映射和挑战分析，比较两者设计理念与能力。
...
Method: 通过搜索策略和基础定义，分析架构演变、操作机制、交互风格和自主水平。

Result: 对比了AI Agents（任务特定自动化）与Agentic AI（多代理协作、动态任务分解）的应用领域和挑战。

Conclusion: 提出了针对幻觉、脆弱性等问题的解决方案，为未来系统开发提供指导。
...
TL;DR: 研究评估了大型语言模型在动态环境中的自适应能力，发现战略提示可缩小模型性能差距，但高级推理方法效果不稳定，且当前模型在规划、推理和空间协调方面仍有局限。
...
Result: 大模型表现优于小模型，但战略提示可缩小差距；高级提示技术对小模型更有效；推理方法效果不稳定，且模型在关键领域仍有局限。
...
TL;DR: LAS是一种无损的ANN-SNN转换方法，用于实现完全脉冲驱动的LLMs，解决了现有方法在处理极端激活异常和非线性操作时的不足。
...
Method: LAS引入两种新型神经元，用于转换ANN中的激活异常和非线性操作，并定制了适用于脉冲LLMs的Transformer组件。

Result: 在六种语言模型和两种视觉语言模型上，LAS实现了无损转换，并在OPT-66B上提升了2%的准确率。

Conclusion: LAS通过创新的神经元设计和Transformer组件定制，实现了无损的ANN-SNN转换，验证了其有效性。
...
<details>
  <summary>Details</summary>
Motivation: 模拟内存计算（AIMC）在速度和能效上优于传统架构，但存在噪声和量化约束，导致LLM在AIMC硬件上性能下降。

Method: 开发了一种通用方法，通过训练适应噪声和低精度约束，使LLM在模拟硬件上保持性能。

Result: 方法使Phi-3-mini-4k-instruct和Llama-3.2-1B-Instruct等模型在4位权重、8位激活基准下性能接近。

Conclusion: 工作填补了高性能LLM与高效模拟硬件之间的鸿沟，为能效优化的基础模型提供了路径。
...
TL;DR: 本文研究了图遗忘过程中可能引入的偏见问题，并提出了一种公平图遗忘方法FGU，通过分区训练和双层去偏过程，在保护隐私的同时确保公平性和准确性。


<details>
  <summary>Details</summary>
Motivation: 图遗忘在保护用户隐私方面至关重要，但现有方法在删除用户信息时可能导致预测分布变化，甚至放大偏见。本文旨在探讨图遗忘是否引入偏见，并提出解决方案。

Method: 提出FGU方法，通过分区训练子图模型、重新训练修改后的子图，并结合双层去偏（子图级和全局级公平）来确保公平性。

Result: 实验表明，FGU在保持隐私和准确性的同时，显著提升了公平性，且对多样化的遗忘请求具有鲁棒性。

Conclusion: FGU是一种有效的公平图遗忘方法，解决了现有方法可能引入偏见的问题，同时兼顾隐私保护和模型性能。
...
TL;DR: 该研究探讨了联邦学习（FL）在AIoT场景中的能耗问题，提出两种基于聚类的设备选择方法，以降低能耗并提高收敛速度。


<details>
  <summary>Details</summary>
Motivation: 现有文献中，联邦学习在AIoT场景中的能耗问题常被忽视，而设备选择对模型训练的收敛速度和能耗有重要影响。

Method: 研究提出两种聚类方法，将具有相似标签分布的AIoT设备分组，形成近似异构的集群，以减少实际分布式学习中的异构性问题。

Result: 实验表明，所提出的聚类策略在保持低能耗的同时，实现了较高的收敛速度。

Conclusion: 该研究为优化联邦学习在AIoT中的能耗和性能提供了有效方法。
...
TL;DR: 研究了深度形态神经网络（DMNNs），发现层间激活对其至关重要，提出了几种新架构，并在参数约束下成功训练，证明其比线性网络更易剪枝。还提出了一种混合架构，加速了大批量梯度下降的收敛。


<details>
  <summary>Details</summary>
Motivation: 探索DMNNs的非线性特性及其层间激活的重要性，并尝试在参数约束下训练这些网络。

Method: 提出了几种新的DMNN架构，每种架构对参数施加不同约束（如多数参数需为形态操作），并设计了一种混合线性与形态层的网络。

Result: 成功训练了DMNNs，证明其比线性网络更易剪枝，混合架构显著加速了大批量梯度下降的收敛。

Conclusion: DMNNs在参数约束下可成功训练，混合架构具有潜力，但泛化能力仍有局限。
...
TL;DR: 论文探讨了如何通过组合实现分布外泛化（OOD），并指出仅测试OOD性能不足以证明算法学习了组合结构，还需验证特征是否真正组合。作者通过实验展示了三种常见神经网络在OOD任务中的不足，并开发了两种新架构。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解决分布外泛化问题，强调组合特征的重要性，以超越传统数据插值方法，实现更智能的系统。

Method: 通过实验比较MLP、CNN和Transformer在OOD任务中的表现，并设计两种具有特定偏置的新网络架构。

Result: 实验表明，即使新架构在OOD任务中表现优异，仍可能未能正确学习组合特征。

Conclusion: 仅依赖OOD测试不足以验证组合学习，需进一步确认特征的组合性。
...
TL;DR: 该论文提出了一种联邦学习方法，通过自适应噪声清理、协作条件GAN生成合成数据以及鲁棒的联邦模型训练，系统性解决数据质量问题（如噪声标签、类别缺失和不平衡分布）。实验表明，该方法在MNIST和Fashion-MNIST数据集上显著提升了模型性能，同时兼顾计算可行性和数据隐私。


<details>
  <summary>Details</summary>
Motivation: 联邦学习（FL）在保护数据隐私的同时实现协作模型训练，但数据质量问题（如噪声标签、类别缺失和不平衡分布）限制了其效果。本研究旨在系统性解决这些问题。

Method: 提出了一种联邦学习方法，包括自适应噪声清理、协作条件GAN生成合成数据以及鲁棒的联邦模型训练。

Result: 在MNIST和Fashion-MNIST数据集上的实验表明，该方法显著提升了模型性能（如macro-F1 Score），并在噪声和类别不平衡条件下表现优异。

Conclusion: 该方法有效缓解了数据质量问题，提供了一种鲁棒、可扩展且隐私合规的解决方案，适用于多样化的实际联邦学习场景。
...
<details>
  <summary>Details</summary>
Motivation: 解决黑盒组合优化问题中样本效率低和求解质量不足的挑战，尤其是在NP难问题中。

Method: 将黑盒目标视为能量函数，训练神经网络建模Boltzmann分布，通过温度调节分布形态，从均匀分布到集中于全局最优解。

Result: 在有限和无限查询预算下，验证了方法在组合任务中的竞争力，优于现有黑盒优化器。

Conclusion: 该方法通过温度依赖的分布学习能量景观结构，显著提升了全局优化能力。
...
TL;DR: 提出了一种新的多智能体强化学习框架，利用潜在社区结构和混合成员关系，支持灵活协作、迁移学习和主动学习。
...
Method: 通过社区共享策略和价值函数，结合个性化成员权重，设计基于演员-评论家的算法。

Result: 理论证明了在线性函数逼近下的收敛性，支持迁移和主动学习。

Conclusion: 首个结合社区结构、可迁移性和主动学习的多智能体强化学习框架。
...
TL;DR: 研究生成模型在噪声和对抗性数据筛选下的自循环训练行为，分析其影响并提出攻击算法。


<details>
  <summary>Details</summary>
Motivation: 生成模型的自循环训练可能导致模型崩溃或不稳定，且数据筛选可能受噪声或对抗性操纵，影响模型性能。

Method: 理论分析噪声数据筛选对生成模型的影响，设计对抗性攻击算法，并在合成和真实数据集上进行实验。

Result: 实验证明提出的攻击算法能有效干扰对手模型的性能。

Conclusion: 噪声和对抗性数据筛选显著影响生成模型的训练，需设计鲁棒性策略应对。
...
TL;DR: 论文提出了一种针对大型语言模型（LLM）训练中检查点数据的优化压缩方案LMC，通过字节分组和霍夫曼编码显著提升压缩效率，减少数据传输时间。


<details>
  <summary>Details</summary>
Motivation: 检查点数据量大且传输耗时，需优化以减少资源占用并支持更高频率的检查点。
...
Conclusion: LMC高效压缩检查点数据，支持更高频率检查点，优化训练过程。
...
TL;DR: 研究探讨机器学习算法在预测中风风险中的有效性，使用人口统计、临床和生活方式数据，评估多种模型性能，发现准确性高但敏感性不足，并提出改进策略。


<details>
  <summary>Details</summary>
Motivation: 中风是全球第二大死因和第三大致残原因，亟需可靠预测方法。

Method: 使用Logistic回归、随机森林和XGBoost等模型，解决类别不平衡和缺失数据问题。

Result: 模型准确性高，但敏感性不足，影响临床实际应用。

Conclusion: 研究为开发更可靠和可解释的中风风险早期评估模型提供了方向。
...
TL;DR: 本文提出了一种基于指数梯度下降和Bregman投影的优化技术，用于高效破解大型语言模型（LLMs），并证明了其收敛性和高效性。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs的广泛应用，理解其漏洞并提升安全性至关重要。现有方法在离散或连续空间优化中存在效率或效果问题，因此需要一种更优的解决方案。

Method: 采用指数梯度下降和Bregman投影方法，优化单热编码以确保其始终位于概率单纯形内，并开发了高效算法。

Result: 在五个开源LLMs和四个数据集上的实验表明，该方法比现有技术具有更高的成功率和效率。

Conclusion: 提出的技术为LLMs的安全性研究提供了新思路，并在实际应用中表现出色。
...
TL;DR: 该论文研究了从平滑信号中学习Kronecker结构乘积图的问题，提出了一种交替优化方案，并展示了其在合成和真实图数据上的优越性能。


<details>
  <summary>Details</summary>
Motivation: 随着多路数据的普及，乘积图在建模依赖结构方面受到关注，但现有方法能学习的图类型有限。Kronecker乘积能更复杂地建模非可分依赖，但学习问题更具挑战性。

Method: 提出一种交替优化方案，分别优化每个因子图，并提供理论保证其渐近收敛性。算法还扩展到学习强乘积的因子图。

Result: 在合成和真实图数据上的实验表明，所提方法在性能上优于现有方法。

Conclusion: Kronecker乘积图能更灵活地建模复杂依赖结构，所提算法在理论和实验上均表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 优化销售流程对B2B业务至关重要，传统方法效果有限，需引入更智能的解决方案。

Method: 采用三层架构：1）因果机器学习预测层；2）约束优化与上下文多臂老虎机优化层；3）生成式AI与反馈循环服务层。

Result: 在LinkedIn中部署后，显著优于传统系统，并提供了广泛适用的见解。

Conclusion: CPOG方法为销售优化提供了高效解决方案，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 整合影像和基因组数据可以揭示疾病复杂特征的新见解，尤其是阿尔茨海默病的多阶段分类。

Method: 采用异质二分图表示学习框架，包含基因和图像两种节点类型，利用小数据集进行分类。

Result: 方法能有效将阿尔茨海默病分为三个阶段（AD、MCI、CN），并识别关键基因，性能指标（准确率、召回率、精确率、F1分数）表现良好。
...
TL;DR: 论文提出了一种基于zentropy理论的新型神经网络ZENN，用于处理异构数据，并在分类任务和能量景观重建中表现出优越性能。
...
Method: 引入内在熵概念，设计ZENN神经网络，同时学习能量和内在熵成分，并重新设计网络架构以适应异构数据。

Result: ZENN在分类任务和能量景观重建中表现优异，特别是在预测高阶导数方面，成功应用于Fe3Pt材料行为分析。

Conclusion: ZENN为复杂异构数据提供了一种通用且鲁棒的深度学习框架，基于zentropy理论的数据驱动方法具有广泛科学应用潜力。
...
TL;DR: 本文提出了一种名为Chisme的新型协议套件，旨在解决边缘网络中异构数据分布、间歇性连接和无基础设施环境下的分布式学习挑战。Chisme包括同步和异步两种变体，通过数据相似性启发式方法优化模型训练。


<details>
  <summary>Details</summary>
Motivation: 随着智能服务需求的增长和边缘设备能力的提升，分布式学习在边缘网络中变得至关重要。然而，现有方法（如联邦学习和去中心化联邦学习）在资源受限和无基础设施环境中面临连接和同步的挑战，而现有的Gossip学习算法通常假设数据分布同质化，无法适应所有场景。

Method: Chisme协议套件包括同步去中心化联邦学习（Chisme-DFL）和异步Gossip学习（Chisme-GL）两种变体，利用数据相似性启发式方法优化模型聚合和合并机制，以适应异构数据分布。

Result: 实验表明，Chisme方法在从低连接可靠性网络到全连接无损网络的各种场景中，均优于标准方法，特别是在分布式和异构数据训练中表现更优。

Conclusion: Chisme通过结合同步和异步学习机制，有效解决了边缘网络中的分布式学习挑战，为异构数据环境下的协作学习提供了新的解决方案。
...
TL;DR: Transformer模型通过权重内学习（IWL）和上下文学习（ICL）两种模式学习，研究探讨了环境可预测性如何影响这两种模式的平衡。


<details>
  <summary>Details</summary>
Motivation: 受进化生物学启发，研究旨在理解IWL和ICL在Transformer中的交互作用及其受环境稳定性和线索可靠性的影响。

Method: 通过回归和分类任务实验，分析环境稳定性和线索可靠性对IWL和ICL平衡的影响。

Result: 高环境稳定性显著偏好IWL，而高线索可靠性增强ICL效果；学习动态显示任务依赖性模式转换。

Conclusion: 研究支持相对成本假说，揭示了可预测性在Transformer学习模式中的关键作用，为理解ICL和优化训练方法提供了新视角。
...
TL;DR: 论文提出了一种基于Transformer的统一归因方法，适用于会员级和聚合级数据，并能整合外部宏观因素，展示了在LinkedIn的大规模应用效果。


<details>
  <summary>Details</summary>
Motivation: 数据驱动的归因是现代营销智能的核心，但现有方法难以处理多源数据和宏观因素。

Method: 采用Transformer架构，统一处理会员级、聚合级数据及外部宏观因素。

Result: 在LinkedIn的大规模实施中表现出显著效果。

Conclusion: 该方法对营销和广告技术领域具有广泛适用性。
...
TL;DR: BINGO是一种新型的神经网络剪枝方法，通过单次训练生成权重重要性评分，显著降低计算和环境成本。


<details>
  <summary>Details</summary>
Motivation: 大型模型的高昂训练成本阻碍了非富裕个体参与AI发展，且增加了消费者负担。

Method: BINGO在训练过程中评估每个权重对网络准确性的贡献，生成重要性评分，实现一次性剪枝。

Result: BINGO提供了一种计算高效且保持准确性的剪枝技术。

Conclusion: BINGO有望推动AI发展不再依赖模型规模的扩大。
...
### [147] [Change Detection in Multivariate data streams: Online Analysis with Kernel-QuantTree](https://arxiv.org/abs/2410.13778)
*Michelangelo Olmo Nogara Notarianni,Filippo Leveni,Diego Stucchi,Luca Frittoli,Giacomo Boracchi*

Main category: cs.LG

TL;DR: KQT-EWMA是一种结合Kernel-QuantTree直方图和EWMA统计量的非参数变化检测算法，用于在线监测多元数据流，具有灵活性和实用性。


<details>
  <summary>Details</summary>
Motivation: 现有非参数变化检测方法难以预先控制误报率（ARL0），而KQT-EWMA解决了这一问题。

Method: 通过结合KQT直方图和EWMA统计量，实现对多元数据流的在线监测，且不依赖于数据流的分布。
...
TL;DR: 研究比较了大型语言模型（LLM）与人类在多臂老虎机任务中的探索-利用权衡策略，发现推理能力使LLM更接近人类行为，但在复杂环境中适应性不足。


<details>
  <summary>Details</summary>
Motivation: 探讨LLM在动态决策任务中是否表现出与人类相似的探索-利用权衡行为，并评估其性能。

Method: 使用多臂老虎机任务和可解释选择模型，比较LLM、人类及算法的策略，分析推理能力对LLM决策的影响。

Result: 推理使LLM更接近人类行为，但在复杂环境中适应性较差，特别是在定向探索方面。

Conclusion: LLM在模拟人类行为和自动化决策方面有潜力，但在复杂环境中的适应性仍需改进。
...
### [149] [Adversarial Attacks in Multimodal Systems: A Practitioner's Survey](https://arxiv.org/abs/2505.03084)
*Shashank Kapoor,Sanjay Surendranath Girija,Lakshit Arora,Dipen Pradhan,Ankit Shetgaonkar,Aman Raj*

Main category: cs.LG

TL;DR: 论文综述了多模态模型中的对抗攻击，填补了实践者视角的空白。


<details>
  <summary>Details</summary>
Motivation: 多模态模型虽强大，但继承了各模态的对抗攻击漏洞，缺乏实践者视角的威胁分析。

Method: 通过调查文本、图像、视频和音频四种模态的对抗攻击，总结威胁演变。

Result: 首次全面总结了多模态世界的对抗威胁。

Conclusion: 为实践者提供了威胁全景和预防措施，填补了研究空白。
...
<details>
  <summary>Details</summary>
Motivation: 随着健康食品需求增长，农产品价格预测变得重要，哈斯鳄梨价格波动复杂，传统模型难以处理非线性动态数据。

Method: 采用TCN提取时序特征，MLP处理非线性交互，注意力机制动态加权特征，使用5万条销售记录进行训练。

Result: 模型表现优异，RMSE为1.23，MSE为1.51，优于传统方法。

Conclusion: 该研究为农业市场时间序列预测提供了可扩展的有效方法，对供应链管理和价格策略优化有重要价值。
...
TL;DR: 论文研究了在一般流形约束数据上直接采样欧几里得扩散模型的方法，揭示了嵌入空间中得分函数的多尺度奇异性，并提出两种新方法（Niso-DM和Tango-DM）以改善采样精度。


<details>
  <summary>Details</summary>
Motivation: 探索如何在流形约束数据上直接应用欧几里得扩散模型，避免依赖特殊流形结构，并解决得分函数奇异性问题。

Method: 通过分析得分函数在切向和法向的奇异性结构，提出Niso-DM（引入法向非各向同性噪声）和Tango-DM（仅训练切向得分函数）。

Result: 数值实验表明，新方法在复杂几何流形上的分布采样中表现优异。

Conclusion: 提出的方法有效解决了得分函数奇异性问题，显著提升了流形约束数据的采样精度。
...
### [152] [Online Isolation Forest](https://arxiv.org/abs/2505.09593)
*Filippo Leveni,Guilherme Weigert Cassales,Bernhard Pfahringer,Albert Bifet,Giacomo Boracchi*

Main category: cs.LG

TL;DR: Online-iForest是一种专为流数据设计的在线异常检测方法，无需周期性重训练，性能媲美离线方法，效率更高。
...
Method: 提出Online-iForest，动态跟踪数据生成过程的变化。
...
TL;DR: 论文提出了一种交互式持续学习框架RiCL，通过实时人类反馈动态学习新技能，同时保留旧知识，解决了传统持续学习的两大限制。
...
Method: RiCL结合了时间一致性感知净化器、交互感知直接偏好优化策略和抗噪声对比学习模块，以处理动态反馈和噪声标签。

Result: 在FewRel和TACRED数据集上的实验表明，RiCL显著优于现有在线持续学习和噪声标签学习方法的组合。

Conclusion: RiCL为交互式持续学习提供了一种有效框架，能够处理动态反馈和噪声标签，具有实际应用潜力。
...
TL;DR: 该研究利用大型语言模型（LLM）分析高速公路事故数据，通过微调Llama3 8B模型进行零样本分类，有效识别事故主因，如酒驾、超速等，并验证了模型的实用性。


<details>
  <summary>Details</summary>
Motivation: 传统统计方法和机器学习模型难以捕捉事故中复杂的因素交互作用，因此研究探索LLM在事故成因分析中的应用。

Method: 通过整理226项高速公路事故研究数据，微调Llama3 8B模型，并采用零样本分类识别事故成因。

Result: 模型成功识别酒驾、超速等主因，结合事件数据（如道路维护）提供更深入分析，问卷结果显示88.89%的研究者认可其有效性。

Conclusion: LLM能全面分析事故成因，为规划者和政策制定者提供有价值的见解和潜在对策，提升交通安全措施效率。
...
<details>
  <summary>Details</summary>
Motivation: 传统持续学习方法在处理大量任务时表现不佳，导致灾难性遗忘问题加剧。本文旨在评估现有方法在长期持续学习中的表现，并提出解决方案。

Method: 提出Long-CL框架，包括任务核心记忆管理策略和长期记忆巩固机制，并构建了多模态和文本基准测试集。

Result: Long-CL在两个基准测试集上的性能分别比现有最优方法提高了7.4%和6.5% AP。
...
TL;DR: TransPL是一种新颖的无监督域适应方法，通过建模时间序列的联合分布和通道转移矩阵，显著提升了伪标签生成的质量。


<details>
  <summary>Details</summary>
Motivation: 传统伪标签策略无法捕捉时间序列数据的时间模式和通道转移，导致伪标签质量不佳。

Method: 利用向量量化（VQ）生成代码转移矩阵，结合贝叶斯规则进行目标域适应，生成通道加权的伪标签。

Result: 在四个时间序列UDA基准测试中，TransPL显著优于现有方法（准确率提升6.1%，F1提升4.9%）。

Conclusion: TransPL不仅性能优越，还提供了可解释的域适应过程，适用于多种UDA场景。
...
<details>
  <summary>Details</summary>
Motivation: 传统FRL方法通过加密共享本地状态或策略信息，但存在隐私风险。FedRAG旨在通过共享非敏感的状态投影函数提升性能。

Method: 提出FedRAG框架，学习各客户端的实用状态投影函数，并在中央服务器聚合参数，避免共享敏感信息。

Result: 在DeepMind Control Suite上的实验验证了FedRAG的有效性和信息增益。

Conclusion: FedRAG提供了一种隐私保护且性能优越的FRL解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 心脏病是全球主要的健康问题，早期预测对临床决策至关重要。本研究旨在利用机器学习技术提高心脏病的预测准确性。

Method: 研究采用数据预处理、模型训练和评估，使用逻辑回归、KNN和随机森林三种分类器，并通过GridSearchCV和RandomizedSearchCV进行超参数调优。

Result: 随机森林分类器表现最优，准确率为91%，F1分数为0.89，其他评估指标（如精确率、召回率和混淆矩阵）也显示模型性能均衡。

Conclusion: 该模型在辅助临床决策方面具有潜力，但数据集规模和泛化性存在局限，未来需更大规模、更多样化的数据验证。研究展示了机器学习在医疗领域的应用前景。
...
TL;DR: 提出了一种面向6G的多模态通用模型AI2MMUM，通过结合LLM和多模态对齐技术，灵活高效地执行多种物理层任务。
...
Method: 采用LLM作为主干，结合领域知识微调，使用任务指令和轻量级任务特定头部，通过冻结的无线电模态编码器和适配层桥接模态。

Result: 在WAIR-D和DeepMIMO数据集上，AI2MMUM在五项代表性任务中达到SOTA性能。

Conclusion: AI2MMUM展示了在多模态任务中的强大适应性和性能，为6G无线系统提供了有前景的解决方案。
...
TL;DR: 论文研究了分布鲁棒平均奖励强化学习问题，提出了两种算法，实现了接近最优的样本复杂度，并验证了收敛速率。
...
Method: 提出了两种算法：一种将问题转化为分布鲁棒折扣MDP，另一种引入锚定状态以稳定不确定性集合内的转移核。

Result: 在名义MDP均匀遍历的假设下，两种算法实现了样本复杂度为$\widetilde{O}\left(|\mathbf{S}||\mathbf{A}| t_{\mathrm{mix}}^2\varepsilon^{-2}\right)$的收敛保证。

Conclusion: 这是首次为分布鲁棒平均奖励强化学习提供有限样本收敛保证，并通过数值实验验证了算法的收敛速率。
...
TL;DR: 论文介绍了ImagineBench，首个结合真实和LLM生成虚拟数据的离线强化学习基准测试，旨在推动算法改进。


<details>
  <summary>Details</summary>
Motivation: 解决强化学习依赖大量真实交互数据的限制，利用LLM生成虚拟数据，但缺乏标准评估基准。

Method: 提出ImagineBench，包含真实和虚拟数据集、多样化任务领域及自然语言指令。

Result: 现有算法在未见任务上表现不佳（35.44%成功率），需改进以更好利用虚拟数据。

Conclusion: ImagineBench为未来研究提供了方向，包括虚拟数据利用、快速适应和多模态任务扩展。
...
TL;DR: 量子-经典混合机器学习（QHML）模型在小数据集上表现出色，但数据编码对其稳定性影响显著。本文提出一种基于改进梯度tanh的归一化方法，显著提升了QHML在抗癌药物响应预测中的性能。


<details>
  <summary>Details</summary>
Motivation: 解决QHML模型在数据编码接口处的敏感性问题，以提升其在抗癌药物响应预测中的稳定性。

Method: 提出一种基于改进梯度tanh的归一化方法，优化神经网络输出分布。

Result: QHML模型在优化归一化后性能优于经典深度学习模型。

Conclusion: 本研究为利用量子计算机进行生物医学数据分析提供了新思路。
...
TL;DR: 论文提出了一种基于逻辑门分解的电路发现框架，通过引入AND、OR和ADDER门，结合噪声与去噪干预，确保电路的完整性和忠实性。


<details>
  <summary>Details</summary>
Motivation: 现有电路发现方法因未保证完整性，导致电路不稳定且关键机制缺失，主要问题源于OR门的部分检测。

Method: 系统引入三种逻辑门，提出结合噪声与去噪干预的框架，可集成到现有方法中，识别并区分电路中的逻辑门。

Result: 实验验证了框架在恢复电路忠实性、完整性和稀疏性方面的能力，并揭示了三种逻辑门的比例、贡献及在语言模型中的行为特性。

Conclusion: 该框架为电路发现提供了完整性和忠实性的解决方案，同时揭示了逻辑门在语言模型中的基本性质。
...
TL;DR: 论文提出了一种基于原型对比学习（PCL）的非样本持续图学习（NECGL）方法IPAL，通过拓扑集成高斯原型（TIGP）和实例-原型亲和力蒸馏（IPAD）解决特征漂移和任务记忆问题，并在四个基准数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 图神经网络（GNN）在持续学习中存在灾难性遗忘问题，现有方法如重放技术存在内存爆炸和隐私问题，而非样本方法如原型重放（PR）又面临特征漂移挑战。

Method: 提出IPAL方法，结合PCL、TIGP和IPAD，利用图结构信息生成高斯原型，并通过亲和力蒸馏和决策边界感知（DBP）机制增强模型性能。

Result: 在四个节点分类基准数据集上，IPAL优于现有方法，实现了更好的可塑性与稳定性平衡。

Conclusion: IPAL通过结合PCL和结构信息，有效缓解了特征漂移和灾难性遗忘，为持续图学习提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 传统机器学习模型在追求预测准确性时牺牲了透明性和可解释性，导致难以满足监管要求和获得利益相关者信任。

Method: 使用XGBoost、LightGBM和CatBoost的堆叠集成模型，并结合SHAP、LIME、PDP和PFI等XAI技术增强模型解释性。

Result: 在IEEE-CIS欺诈检测数据集上，模型准确率达99%，AUC-ROC为0.99，优于其他方法。

Conclusion: 研究表明，高预测准确性和透明解释性可以结合，为金融欺诈检测提供更可信的解决方案。
...
TL;DR: 论文提出了一种自适应的多任务蒸馏方法，通过动态调整教师模型的知识传递量，并结合知识轨迹避免梯度更新错误，显著提升了深度估计和场景分割的统一建模效果。


<details>
  <summary>Details</summary>
Motivation: 深度估计和场景分割在智能交通系统中至关重要，联合建模可以减少存储和训练成本。现有方法静态传递知识，无法动态适应学生学习能力，且多教师可能导致梯度更新错误和知识遗忘。

Method: 提出自适应的蒸馏方法，动态调整教师知识传递量；设计知识轨迹记录关键信息，并基于轨迹设计蒸馏损失，以低成本引导学生学习。

Result: 在Cityscapes和NYU-v2等数据集上评估，方法明显优于现有技术。

Conclusion: 自适应蒸馏和知识轨迹的结合有效提升了联合建模的性能，代码已公开。
...
TL;DR: 论文提出了一种名为ChronoSteer的多模态时间序列预测模型，通过结合LLM和TSFM的优势，利用文本修订指令指导预测，解决了多模态数据稀缺问题，并在预测精度上显著提升。
...
Result: ChronoSteer在合成数据训练下，预测精度比单模态基线提升25.7%，比现有最佳多模态方法提升22.5%。

Conclusion: ChronoSteer成功结合LLM和TSFM，通过文本指令指导预测，显著提升多模态时间序列预测性能。
...
TL;DR: 论文提出了一种名为MiCo的分层语言代理框架，利用大语言模型（LLM）设计启发式方法，解决云服务中的在线动态多维装箱问题（ODMBP），在复杂和大规模场景中表现优异。
...
Result: 在超过10,000台虚拟机的场景中，MiCo的竞争比达到96.9%，且在非平稳请求流和多样化配置下保持高性能。

Conclusion: MiCo在复杂和大规模云环境中表现高效，验证了其有效性。
...
TL;DR: 提出一种新策略，联合训练图Transformer网络于稀疏多任务分子属性实验目标和XGBoost模型生成的合成目标，显著提升性能。
...
Method: 联合训练图Transformer网络，利用XGBoost模型生成的合成目标作为独立辅助任务。

Result: 在19个分子属性预测任务中均显著提升性能，16个任务优于XGBoost单任务学习。

Conclusion: 合成任务增强是提升神经网络多任务性能的有效方法，无需特征注入或预训练。
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习在隐私敏感应用中受欢迎，但域异构性阻碍全局模型收敛。

Method: 提出FedAPC框架，利用增强数据的均值特征生成原型，增强特征多样性和模型鲁棒性。

Result: 在Office-10和Digits数据集上表现优于现有方法。

Conclusion: FedAPC有效解决了域异构性问题，提升了模型性能。
...
TL;DR: 论文研究了多智能体多臂老虎机问题中的最佳臂识别问题，提出了两种算法（Cl-BAI和BAI-Cl），并验证了其高效性和准确性。


<details>
  <summary>Details</summary>
Motivation: 解决多智能体多臂老虎机问题中，智能体与老虎机映射关系未知时，如何高效识别每个智能体的最佳臂，同时最小化样本复杂度和通信开销。

Method: 提出两种算法：Cl-BAI（先聚类后识别最佳臂）和BAI-Cl（先识别最佳臂后聚类），均基于逐次消除框架。

Result: 两种算法均具有δ-PC保证，样本复杂度有界，且BAI-Cl的变体在M较小时达到极小极大最优。实验验证了算法在样本和通信效率上的优越性。

Conclusion: Cl-BAI和BAI-Cl在多智能体多臂老虎机问题中表现优异，尤其在M远小于N时，具有显著优势。
...
TL;DR: 论文提出QuXAI框架，基于Q-MEDLEY解释器，用于提升混合量子-经典机器学习（HQML）模型的可解释性和可靠性。


<details>
  <summary>Details</summary>
Motivation: HQML模型因复杂性常表现为黑箱行为，缺乏透明度和可靠性，现有XAI方法在量子系统中尚不成熟，亟需针对HQML的全局和局部解释性方案。

Method: 通过量子特征映射构建HQML模型，结合Q-MEDLEY解释器分析特征重要性，保留量子变换阶段并可视化结果。

Result: Q-MEDLEY能有效识别HQML模型中的关键经典特征并分离噪声，在经典验证环境中表现优异，消融实验验证了其复合结构的优势。

Conclusion: QuXAI框架显著提升了HQML模型的解释性和可靠性，为量子增强AI技术的安全负责任应用提供了支持。
...
TL;DR: 论文提出Alinear，一种超轻量级时间序列预测模型，仅需k级参数即可达到与大模型竞争的性能，挑战了模型规模必须扩大的普遍观点。
...
Method: 引入动态调整组件权重的自适应分解机制和渐进频率衰减策略，避免注意力机制的计算开销。

Result: 在七个基准数据集上，Alinear以不到1%的参数优于大模型，并在不同预测范围内保持高精度。

Conclusion: 研究表明模型规模并非越大越好，自适应设计更高效，为时间序列建模提供了新思路。
...
Method: 人工生成带缺陷的SEM图像并自动标注，使用YOLOv8等模型检测不同尺寸的缺陷。

Result: YOLOv8表现最佳（mAP 96%），能检测比间距更小的缺陷，并在真实数据中验证了有效性。
...
TL;DR: ComplexFormer提出了一种新的复杂多头注意力机制（CMHA），通过将语义和位置信息统一在复平面上建模，显著提升了Transformer模型的性能。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer在多头注意力中难以有效整合位置信息，限制了其表示能力。

Method: CMHA通过每头的欧拉变换和自适应差分旋转机制，独立建模语义和位置差异。

Result: 实验表明，ComplexFormer在语言建模、文本生成等任务中表现优异，生成困惑度更低。

Conclusion: ComplexFormer提供了一种更灵活、高效的注意力机制，适用于多种任务。
...
<details>
  <summary>Details</summary>
Motivation: 通过自动识别牛的下颌运动来监测进食行为，有助于优化饲料配方、早期发现代谢问题和动物不适症状。

Method: 使用卷积、循环和密集层的深度神经网络，融合声学和惯性信号，自动提取特征并进行信息融合。

Result: 提出的模型F1分数为0.802，比现有方法提高了14%，特征级融合优于数据和决策级融合。

Conclusion: 多传感器信号融合的深度神经网络显著提升了进食行为监测的精度，为畜牧业管理提供了有效工具。
...
TL;DR: 提出了一种跨领域知识转移框架，提升大语言模型（LLMs）在时间序列预测中的性能，实验证明其优于无辅助信息的基线方法。
...
Method: 通过系统性地为大语言模型注入结构化时间信息，提升其预测准确性，并在真实数据集上与无辅助信息的基线方法进行对比。

Result: 知识增强的预测方法在预测准确性和泛化能力上显著优于基线方法。

Conclusion: 知识转移策略有望弥合大语言模型与领域特定预测任务之间的差距。
...
TL;DR: 论文研究了大型语言模型（LLMs）性能随模型规模增长的神经缩放定律，发现表征叠加是这一现象的关键机制。
...
Method: 通过构建玩具模型，研究模型规模与损失的关系，分析不同叠加强度下的表现。

Result: 弱叠加时损失与特征频率相关；强叠加时损失与模型维度成反比，且与开源LLMs的实验结果一致。

Conclusion: 表征叠加是神经缩放定律的核心机制，未来可优化训练策略和模型架构以提升效率。
...
TL;DR: SpecOffload是一种高效推理引擎，通过将推测解码嵌入卸载过程，显著提升资源受限设备上LLM推理的性能。


<details>
  <summary>Details</summary>
Motivation: 资源受限设备上LLM推理的计算和内存利用率存在挑战，现有系统因卸载模型权重到CPU内存导致GPU核心利用率低和内存影响小。

Method: 提出SpecOffload，利用GPU潜在资源存储和执行推测解码的草稿模型，优化目标与草稿模型的交错执行，并设计规划器管理张量放置和参数选择。

Result: 相比最佳基线，SpecOffload提升GPU核心利用率4.49倍，推理吞吐量提升2.54倍。

Conclusion: SpecOffload通过推测解码与卸载的结合，以近乎零额外成本加速推理，显著提升性能。
...
TL;DR: 论文提出了一种新的并行计算扩展方法（ParScale），通过增加模型的并行计算而非参数或输出令牌，显著提升了推理效率。
...
Result: 实验表明，ParScale在相同性能提升下，内存和延迟增加显著低于参数扩展，且可通过少量令牌的后训练实现模型扩展。
...
TL;DR: 论文提出了一种基于分层深度强化学习（HDRL）的电动公交车充电调度方法，通过分层决策优化充电目标和充电功率，以最小化成本。


<details>
  <summary>Details</summary>
Motivation: 电动公交车充电调度问题复杂，涉及长时程多阶段规划与稀疏奖励，传统方法难以高效解决。

Method: 将原问题分解为高层SMDP和低层MDP，提出HDDQN-HER算法分层优化充电目标和充电功率。

Result: 实验证明分层策略叠加效果与原问题最优策略相当，且算法能有效解决非平稳性和采样效率问题。

Conclusion: HDRL方法在电动公交车充电调度中表现优异，为复杂决策问题提供了新思路。
...
TL;DR: 提出了一种基于路由器的架构，用于生成高质量合成数据以微调大型语言模型（LLM），解决了传统方法在多样性和复杂性上的不足，显著提升了功能分类和API参数选择的准确性。


<details>
  <summary>Details</summary>
Motivation: 在缺乏真实用户交互数据的情况下，传统合成数据生成方法无法复现真实数据分布，导致LLM微调效果不佳。

Method: 采用基于路由器的架构，结合领域资源（如内容元数据和知识图谱）及文本到文本、视觉到文本模型，生成高质量合成数据。

Result: 在真实用户查询评估中，功能分类和API参数选择的准确性显著提升，模型性能优于传统方法。

Conclusion: 该架构为功能调用任务设定了新基准，解决了合成数据生成的关键限制。
...
<details>
  <summary>Details</summary>
Motivation: 尽管联邦学习旨在保护数据隐私，但现有研究表明恶意服务器可能通过操纵模型更新重建客户端的私有数据。现有攻击方法在数据分布假设或批量大小方面存在限制。

Method: 利用全连接层的新几何视角，设计恶意模型参数，无需客户端数据先验知识即可实现完美数据重建。

Result: 在图像和表格数据集上的实验表明，该方法优于现有方法，能重建比现有技术大两个数量级的数据批次。
...
<details>
  <summary>Details</summary>
Motivation: 现有小型语言模型无法处理视觉输入且预测与视觉上下文不符，限制了推测解码在视觉语言模型中的应用。

Method: MASSV分两阶段：1) 通过轻量级投影器连接目标模型的视觉编码器；2) 使用目标模型生成的自蒸馏数据进行视觉指令调优。

Result: 实验显示MASSV在Qwen2.5-VL和Gemma3模型上，接受长度提升30%，推理速度提升1.46倍。
...
TL;DR: 提出一种深度学习模型，用于欧洲8小时高分辨率降水概率预报，结合雷达、卫星和数值天气预报数据，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决雷达深度学习模型预报时间短的问题，整合多源数据提升预报精度和不确定性量化。

Method: 整合雷达、卫星和数值天气预报数据，设计紧凑架构捕获长程交互，实现高效训练和快速推理。

Result: 模型在实验中优于现有数值天气预报系统、外推方法和深度学习临近预报模型。
...
TL;DR: 论文将Hebbian学习规则与噪声梯度下降联系起来，证明其能识别最高活性的突触前神经元，并发现与噪声镜像下降的内在联系。
...
Method: 将Hebbian脉冲时序依赖可塑性规则与自然损失函数的噪声梯度下降相关联。

Result: 学习规则最终能识别最高活性的突触前神经元。

Conclusion: 揭示了Hebbian学习与噪声梯度下降及噪声镜像下降的内在联系。
...
TL;DR: 提出了一种基于分层深度强化学习（HDRL）的方法DAC-MAPPO-E，用于优化电动公交车（EB）充电调度，解决了多时间尺度决策和大规模车队的可扩展性问题。


<details>
  <summary>Details</summary>
Motivation: 电动公交车的广泛采用对可持续发展至关重要，但充电调度优化面临旅行时间、能耗和电价波动等不确定性，以及多时间尺度决策和大规模车队的复杂性挑战。

Method: 将原始马尔可夫决策过程（MDP）重构为两个增强MDP，并引入DAC-MAPPO-E算法。高层通过注意力机制优化全局状态信息，低层结合MAPPO算法实现分散协调决策。

Result: 实验表明，DAC-MAPPO-E在优化EB车队充电调度方面表现出卓越性能和可扩展性。

Conclusion: DAC-MAPPO-E方法有效解决了EB充电调度的复杂性和可扩展性问题，为实际应用提供了高效解决方案。
...
TL;DR: 提出了一种名为FeRA的联邦学习防御机制，通过跨客户端注意力机制检测后门攻击，适用于异构边缘设备。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中异构设备的非独立同分布数据增加了后门攻击检测的难度，需要一种有效的防御方法。

Method: FeRA利用内部特征表示的跨客户端注意力计算异常分数，识别偏离群体共识的恶意客户端。

Result: 实验表明FeRA在多种联邦学习场景下有效降低后门攻击成功率，同时保持主任务高准确率。
...
<details>
  <summary>Details</summary>
Motivation: 图对比学习（GCL）常受假阴性问题影响，现有方法依赖先验知识，效果不佳。

Method: 提出NML-GCL，利用NMN构建负度量空间，区分假阴性和真阴性，并通过联合训练方案优化。

Result: 理论分析和广泛实验验证了方法的优越性。

Conclusion: NML-GCL有效解决了假阴性问题，提升了性能。
...
TL;DR: 论文提出了一种改进的异步去中心化随机梯度下降（ADSGD）模型，解决了计算和通信延迟问题，并在实际实验中表现优于现有方法。
...
Result: ADSGD在多种场景下的实际收敛时间优于现有方法，且具有内存和通信效率高的优势。

Conclusion: ADSGD因其简单性、高效性和对延迟的鲁棒性，非常适合现实世界的去中心化学习任务。
...
TL;DR: ALERT方法通过表示学习、统计测试和效用评估检测无线网络中特征分布变化，并触发模型重新训练，优于现有十种漂移检测方法。


<details>
  <summary>Details</summary>
Motivation: AI在无线网络中的广泛应用可能因特征分布变化导致性能下降，需要一种有效的方法检测并应对这种变化。

Method: ALERT包括表示学习（MLP）、统计测试（Kolmogorov-Smirnov和Population Stability Index）和效用评估新函数。

Result: ALERT在无线指纹识别和链路异常检测两个用例中优于十种标准漂移检测方法。

Conclusion: ALERT能有效检测特征分布变化并触发模型重新训练，适用于无线网络中的AI模型维护。
...
TL;DR: 论文探讨了如何在动态环境中实现深度强化学习（RL）的高效在线适应，提出了优先探索和选择性知识保留的关键能力。
...
Conclusion: 高效在线适应需要优先探索和选择性知识保留，为动态环境中的RL提供了新思路。
...
TL;DR: 论文研究了随机分类器集合中集体行为的涌现现象，通过Gibbs测度加权，发现存在一个有限温度参数使分类最优，且该参数具有普适性。


<details>
  <summary>Details</summary>
Motivation: 探索随机组件在宏观上表现出的确定性行为，特别是在随机分类器集合中的集体行为涌现。

Method: 采用Gibbs测度加权分类器集合，以分类损失作为能量函数，分析最优温度参数的存在及其特性。

Result: 在特定条件下，最优温度参数不依赖于教师分类器或分类器数量，具有普适性；MNIST实验验证了现象的有效性。

Conclusion: 研究揭示了随机分类器集合的自组织行为及其普适性，为理解复杂系统中的涌现现象提供了新视角。
...
TL;DR: 本文介绍了离散变分自编码器（Discrete VAEs），重点讨论了其理论基础和实现方法。


<details>
  <summary>Details</summary>
Motivation: 传统的VAEs使用高斯分布的隐空间，而离散隐空间可能更适合某些数据类型（如文本）。本文旨在提供离散VAEs的实用教程。

Method: 通过严格的数学推导，从基本原理出发，构建离散VAEs的隐空间（分类分布），并提供训练方法和实现示例。

Result: 提出了离散VAEs的具体训练方案，并提供了开源实现。

Conclusion: 离散VAEs为某些数据类型提供了更自然的建模方式，本文为学习和实现提供了实用指导。
...
<details>
  <summary>Details</summary>
Motivation: 评估SMTOs与固定权重在多任务学习中的表现，澄清争议。

Method: 通过广泛的实证评估，比较SMTOs和固定权重的性能。

Result: SMTOs表现良好，但固定权重在某些情况下也能达到类似效果。

Conclusion: SMTOs仍有优势，但固定权重在某些场景下是可行的替代方案。
...
TL;DR: 论文提出了一种实时提取临床信息并递归生成笔记的方法FactsR，以提高准确性和简洁性，同时支持实时决策。


<details>
  <summary>Details</summary>
Motivation: 现有AI记录工具依赖一次性提示生成笔记，易产生冗长内容和错误，威胁患者安全。

Method: 实时提取临床关键信息（Facts），并递归生成最终笔记。

Result: FactsR方法生成更准确、简洁的笔记，支持实时决策。

Conclusion: FactsR通过实时信息提取和递归生成，提高了笔记质量，拓展了实时决策支持的应用。
...
Result: 实验表明PIF优于现有技术，PI-Forest在测量任意距离和隔离偏好空间中的点方面表现更好。

Conclusion: PIF是一种有效的异常检测方法，PI-Forest在高维空间中表现出色。
...
TL;DR: SCGP是一种基于群论的图神经网络增强方法，通过Schreier-coset嵌入提升节点特征，解决了传统GNN中的信息压缩问题，同时保持计算效率和低内存占用。


<details>
  <summary>Details</summary>
Motivation: 传统GNN在远距离节点信息传递中存在信息压缩问题（over-squashing），现有方法如Cayley图虽有效但内存占用高。

Method: 提出SCGP方法，利用Schreier-coset嵌入增强节点特征，不改变图拓扑结构，实现高效的长距离信息传递。

Result: SCGP在节点和图分类任务中表现优于或接近现有方法，尤其适用于分层和模块化图结构，且内存占用低。

Conclusion: SCGP是一种高效、可扩展的GNN增强方法，适用于实时和资源受限的应用场景。
...
TL;DR: SEAL是一个基于NAS的框架，用于数据增量学习，通过动态调整模型结构和选择性扩展来平衡学习新任务和保留旧知识的能力。


<details>
  <summary>Details</summary>
Motivation: 在资源受限的环境中，现有的增量学习方法通常需要不断扩展模型，这在实际应用中不切实际。SEAL旨在解决这一问题。

Method: SEAL通过容量估计指标动态调整模型结构，仅在必要时扩展模型，并通过交叉蒸馏训练保持稳定性。NAS组件同时搜索架构和最优扩展策略。

Result: 实验表明，SEAL在多个基准测试中有效减少遗忘并提高准确性，同时保持较小的模型大小。

Conclusion: SEAL展示了结合NAS和选择性扩展在增量学习中的潜力，为高效自适应学习提供了新思路。
...
TL;DR: AneuG是一种基于变分自编码器（VAE）的两阶段颅内动脉瘤（IA）网格生成器，能够生成具有特定形态学特征的IA形状，解决了现有方法无法捕捉真实IA特征和忽略与母血管关系的问题。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏大型IA图像数据集，现有形状生成方法难以捕捉真实IA特征且无法控制生成特定形态学测量值，因此需要一种更有效的生成模型。

Method: AneuG采用两阶段方法：第一阶段通过图谐波变形（GHD）编码和重建动脉瘤囊形状；第二阶段基于GHD生成母血管，并可进一步生成具有特定临床相关形态学测量的IA形状。

Result: AneuG能够更准确地编码形状并生成具有生理真实性的IA网格，同时支持特定临床形态学测量的条件生成。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索人类学习的高效性是否源于多机制协同，而非单一梯度下降。
...
<details>
  <summary>Details</summary>
Motivation: 探讨神经网络中梯度下降训练的分布自由学习的基本限制，揭示数据分布假设的重要性。

Method: 通过分析小批量随机梯度下降（bSGD）和随机特征组合，提出平均概率维度复杂性（adc）理论框架。

Result: 证明目标函数可通过多项式大小的随机特征组合近似，且adc与统计查询维度存在多项式关系。

Conclusion: 无分布学习在神经网络中存在根本限制，数据分布假设在实践中至关重要。
...
TL;DR: L2T框架通过信息论强化微调，优化LLMs的推理效率，减少计算资源浪费。
...
Method: 提出L2T框架，利用密集过程奖励和PAC-Bayes边界快速估计信息增益。

Result: 理论分析和实验表明，L2T显著提升推理效果和效率。

Conclusion: L2T是一种高效且通用的推理优化方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统数值天气预报在模拟云和降水时因需要亚网格参数化而具有挑战性，早期机器学习方法生成的预报模糊，因此探索新方法以改进预报质量。
...
Conclusion: 基于分数的扩散模型在临近预报中表现出色，CorrDiff为最佳选择，且能生成校准良好的集合预报。
...
TL;DR: 论文提出了一种基于数据驱动的Koopman特征空间方法，用于改进燃气涡轮发动机的非线性动态建模和控制，优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 燃气涡轮发动机的非线性动态建模复杂且依赖难以获取的性能特性，传统实验方法存在局限性。

Method: 采用稀疏识别非线性动态技术估计转子动力学，通过元启发式算法和梯度优化构建Koopman特征空间模型，并设计全局非线性反馈控制器。

Result: Koopman控制器在参考跟踪和干扰抑制方面优于传统比例积分控制器和增益调度方法。

Conclusion: 基于Koopman特征空间的方法在全局非线性控制中表现优异，适用于复杂动态系统。
...
<details>
  <summary>Details</summary>
Motivation: 扩散策略在演示数据覆盖不足时可能生成次优轨迹甚至灾难性失败，现有方法难以有效结合PPO与扩散模型。

Method: 提出NCDPO框架，将扩散策略转化为噪声条件确定性策略，实现可处理的似然估计和梯度反向传播。

Result: NCDPO在样本效率和最终性能上优于现有方法，适用于机器人控制和多智能体游戏场景。

Conclusion: NCDPO解决了扩散策略在强化学习中的优化难题，具有高效性和鲁棒性。
...
TL;DR: 论文提出了一种新的值函数分解方法QFIX，用于多智能体强化学习，通过简单的‘修正’层扩展了现有模型的表示能力，并在实验中验证了其性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有值函数分解方法（如VDN、QMIX）在满足个体-全局最大值（IGM）属性时表示能力有限，而QPLEX虽无此限制但过于复杂。

Method: 提出QFIX，一种简单且能表示完整IGM值类的方法，通过‘修正’层扩展表示能力，并在两种多智能体框架中实现三种变体。

Result: 在SMACv2和Overcooked环境中，QFIX提升了现有方法的性能，表现优于QPLEX，且模型更简单、更小。

Conclusion: QFIX是一种高效且简单的值函数分解方法，能够显著提升多智能体强化学习的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有XAI方法因架构和模态的局限性、支持方法有限以及缺乏评估优化阶段，难以在实际应用中推广。

Method: 提出PnPXAI框架，自动检测模型架构，推荐适用的解释方法，并优化超参数。

Result: 通过用户调查验证了框架的有效性，并在医学和金融等领域展示了其多功能性。

Conclusion: PnPXAI解决了现有XAI的局限性，为实际应用提供了灵活且优化的解释方法。
...
TL;DR: PharmaDiff是一种基于药效团条件的扩散模型，用于3D分子生成，能够精确生成符合预设药效团假设的分子结构。
...
Result: PharmaDiff在匹配3D药效团约束方面表现优异，且在结构基药物设计中获得更高的对接分数。

Conclusion: PharmaDiff通过结合药效团建模与3D生成技术，为合理药物设计提供了强大且灵活的框架。
...
TL;DR: 提出了一种基于可穿戴设备和实时环境数据的AI模型，用于预测个体对空气污染的健康反应。


<details>
  <summary>Details</summary>
Motivation: 空气污染和极端天气事件加剧健康风险，需个性化监测和预测方法。

Method: 整合可穿戴设备的生理数据和实时环境暴露，使用对抗自编码器神经网络训练模型。

Result: 模型能准确重构健康信号并捕捉非线性污染反应，迁移学习提升泛化能力。

Conclusion: 该方法为个性化健康监测提供了可行且适应性强的解决方案。
...
TL;DR: 论文提出神经热力学定律（NTL），为大型语言模型（LLM）的训练动态提供新视角，理论证明热力学量在特定条件下自然出现，并指导学习率设计。


<details>
  <summary>Details</summary>
Motivation: 探索超越神经缩放定律的LLM训练动态规律。

Method: 引入NTL框架，理论分析热力学量在特定损失景观假设下的涌现。

Result: 热力学量和经典热力学原理在LLM训练中自然出现，为学习率设计提供指导。

Conclusion: NTL为LLM训练提供新科学视角和实用指南。
...
<details>
  <summary>Details</summary>
Motivation: 慢性心力衰竭管理面临挑战，需要持续监测和个性化治疗策略，机器学习模型可辅助早期识别高风险患者。

Method: 采用改进的堆叠集成学习方法，结合两个专用模型（临床和超声心动图特征）和一个元模型。

Result: 模型敏感性达95%，准确性84%，优于基线模型，适用于高风险患者识别。

Conclusion: 机器学习模型可作为决策支持工具，助力早期干预和个性化管理，具有潜在应用价值。
...
### [214] [Sybil-based Virtual Data Poisoning Attacks in Federated Learning](https://arxiv.org/abs/2505.09983)
...
Method: 利用Sybil节点放大攻击效果，基于梯度匹配生成虚拟数据，设计三种目标模型获取方案。

Result: 在非独立均匀分布数据下，方法优于其他攻击算法。

Conclusion: 该方法高效且适用于多种场景。
...
### [215] [PIG: Privacy Jailbreak Attack on LLMs via Gradient-based Iterative In-Context Optimization](https://arxiv.org/abs/2505.09921)
*Yidan Wang,Yanan Cao,Yubing Ren,Fang Fang,Zheng Lin,Binxing Fang*

Main category: cs.CR

TL;DR: 论文探讨了越狱攻击在提取敏感信息中的有效性，并提出PIG框架以解决现有方法的局限性，实验表明PIG优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）存在隐私风险，现有评估方法易被对齐模型阻挡，而越狱攻击在隐私场景中的作用尚未充分研究。

Method: 提出PIG框架，通过识别PII实体、构建隐私上下文，并采用三种梯度策略迭代更新以提取目标PII。

Result: PIG在四种白盒和两种黑盒LLMs上优于基线方法，达到SoTA效果。
...
### [216] [Analysing Safety Risks in LLMs Fine-Tuned with Pseudo-Malicious Cyber Security Data](https://arxiv.org/abs/2505.09974)
*Adel ElZemity,Budi Arief,Shujun Li*

Main category: cs.CR

TL;DR: 论文系统评估了微调大语言模型（LLMs）在网络安全应用中的安全风险，并提出了一种安全对齐方法，以平衡安全性和技术实用性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在网络安全应用中具有潜力，但也带来数据泄露和恶意软件生成等风险，需系统评估和改进。

Method: 使用OWASP Top 10框架评估7个开源LLMs，提出并测试了一种通过重写指令-响应对的安全对齐方法。

Result: 微调降低了所有LLMs的安全韧性（如Llama 3.1 8B的安全评分从0.95降至0.15），但安全对齐方法能有效提升安全性。

Conclusion: 研究为LLMs的安全风险提供了系统评估，并提出了一种实用方法，有助于开发更安全、可信的生成式AI。
...
### [217] [AttentionGuard: Transformer-based Misbehavior Detection for Secure Vehicular Platoons](https://arxiv.org/abs/2505.10273)
*Hexu Li,Konstantinos Kalogiannis,Ahmed Mohamed Hussain,Panos Papadimitratos*

Main category: cs.CR

TL;DR: AttentionGuard是一个基于Transformer的框架，用于检测车辆编队系统中的异常行为，通过自注意力机制识别攻击，实验显示其F1分数高达0.95，适用于实时安全应用。


<details>
  <summary>Details</summary>
Motivation: 车辆编队系统虽能提高燃油效率和道路利用率，但易受内部认证用户的复杂攻击，可能导致灾难性碰撞，因此需要有效的异常行为检测方法。

Method: 提出AttentionGuard，利用多头Transformer编码器处理运动数据，区分正常行为和攻击模式，覆盖多种编队场景和攻击类型。

Result: 实验结果表明，AttentionGuard在攻击检测中F1分数达0.95，延迟低至100ms，适用于实时应用，且在不同攻击和操作参数下表现稳健。

Conclusion: Transformer编码器是保护协作智能交通系统免受内部威胁的有效方法，AttentionGuard展示了其优越的检测能力和实时性能。
...
### [218] [Private Transformer Inference in MLaaS: A Survey](https://arxiv.org/abs/2505.10315)
*Yang Li,Xinyu Zhou,Yitong Wang,Liangxin Qian,Jun Zhao*

Main category: cs.CR

TL;DR: 论文探讨了Transformer模型在MLaaS中的隐私问题，并提出私有Transformer推理（PTI）作为解决方案，通过密码学技术保护数据和模型隐私。


<details>
  <summary>Details</summary>
Motivation: 由于Transformer模型在MLaaS中的集中处理导致隐私问题，需要一种既能保护用户数据又能保护模型隐私的解决方案。

Method: 利用安全多方计算和同态加密等密码学技术实现PTI，并提出了分类和评估框架。

Result: 总结了PTI的最新进展和挑战，平衡了资源效率与隐私保护。

Conclusion: PTI为高隐私需求场景提供了可行的解决方案，但仍需进一步优化性能和隐私的平衡。
...
### [219] [AutoPentest: Enhancing Vulnerability Management With Autonomous LLM Agents](https://arxiv.org/abs/2505.10321)
*Julius Henke*

Main category: cs.CR

TL;DR: 论文探讨了使用大型语言模型（LLMs）进行渗透测试的潜力，提出了AutoPentest工具，并验证其效果。
...
Method: 基于GPT-4o和LangChain框架开发AutoPentest工具，进行黑盒渗透测试。

Result: AutoPentest在Hack The Box测试中略优于ChatGPT，完成15-25%的子任务，成本为96.20美元。

Conclusion: 未来更强的LLMs可能使该工具成为漏洞管理的可行方案。
...
### [220] [Automated Alert Classification and Triage (AACT): An Intelligent System for the Prioritisation of Cybersecurity Alerts](https://arxiv.org/abs/2505.09843)
*Melissa Turcotte,François Labrèche,Serge-Olivier Paquette*

Main category: cs.CR

TL;DR: AACT系统通过学习SOC分析师对安全警报的分类行为，自动化处理警报，减少分析师负担，提高效率。


<details>
  <summary>Details</summary>
Motivation: 企业网络攻击面扩大，安全警报数量激增，导致SOC分析师疲劳，需要自动化解决方案。

Method: AACT系统通过学习分析师对警报的分类行为，实时预测分类决策，自动处理良性警报并优先处理关键警报。

Result: 系统在真实SOC环境中表现优异，减少61%的警报展示，假阴性率仅为1.36%。

Conclusion: AACT系统有效缓解SOC分析师疲劳，提升安全运营效率。
...
### [221] [Tales of the 2025 Los Angeles Fire: Hotwash for Public Health Concerns in Reddit via LLM-Enhanced Topic Modeling](https://arxiv.org/abs/2505.09665)
*Sulong Zhou,Qunying Huang,Shaoheng Zhou,Yun Hang,Xinyue Ye,Aodong Mei,Kathryn Phung,Yuning Ye,Uma Govindswamy,Zehan Li*

Main category: cs.SI

TL;DR: 研究分析了2025年洛杉矶野火期间Reddit上的讨论，通过主题建模和分层框架识别了公众关注的两大类别：情境感知（SA）和危机叙事（CN），并揭示了公众健康和安全、损失与破坏等主题的动态变化。


<details>
  <summary>Details</summary>
Motivation: 理解受灾人群在野火危机中的感知与反应，以提供更及时和共情的灾害响应。

Method: 收集385篇帖子和114,879条评论，采用主题建模方法（结合LLMs和HITL），并开发分层框架分类主题。

Result: SA类别的讨论与火灾进展紧密相关，公众健康和心理健康主题占CN类别的60%和40%。
...
### [222] [Advancing Community Detection with Graph Convolutional Neural Networks: Bridging Topological and Attributive Cohesion](https://arxiv.org/abs/2505.10197)
*Anjali de Silva,Gang Chen,Hui Ma,Seyed Mohammad Nekooei,Xingquan Zuo*

Main category: cs.SI

TL;DR: TAS-Com是一种基于拓扑和属性相似性的社区检测方法，通过结合Leiden算法优化模块化和人类标签的平衡。
...
Method: 提出TAS-Com方法，利用Leiden算法设计新损失函数，优化社区检测的模块化和标签一致性。

Result: 在多个基准网络上，TAS-Com显著优于现有先进算法。

Conclusion: TAS-Com在模块化和人类标签遵从性之间取得了良好平衡，具有实际应用潜力。
...
### [223] [Neural models for prediction of spatially patterned phase transitions: methods and challenges](https://arxiv.org/abs/2505.09718)
*Daniel Dylewsky,Sonia Kéfi,Madhur Anand,Chris T. Bauch*

Main category: physics.comp-ph

TL;DR: 本文研究了神经网络在早期预警信号（EWS）检测中的应用，探讨了其在空间模式相变中的表现，并分析了模型泛化能力。


<details>
  <summary>Details</summary>
Motivation: 干地植被生态系统易受外部强迫影响而发生临界转变，传统分岔理论难以捕捉其复杂动态。神经网络在EWS检测中表现出潜力，但其在高维相变中的泛化能力尚不明确。

Method: 通过合成数据训练神经网络模型，并在多个测试系统中评估其性能，重点关注统计指标的表现和区分突变与连续转变的能力。

Result: 结果显示模型性能在训练和测试数据源互换时变化显著，为模型泛化提供了新见解。

Conclusion: 神经网络在EWS检测中具有潜力，但需进一步研究其在高维相变中的泛化能力。
...
### [224] [Neurophysiologically Realistic Environment for Comparing Adaptive Deep Brain Stimulation Algorithms in Parkinson Disease](https://arxiv.org/abs/2505.09624)
*Ekaterina Kuzmina,Dmitrii Kriukov,Mikhail Lebedev,Dmitry V. Dylov*

Main category: q-bio.NC

TL;DR: 本文介绍了一种神经生理学上真实的基准测试方法，用于比较帕金森病（PD）的合成模型和控制算法，并首次考虑了15种被忽视的生理属性。


<details>
  <summary>Details</summary>
Motivation: 传统的自适应深部脑刺激（aDBS）模型缺乏对多种生理属性的考虑，限制了数据收集和离线优化。

Method: 提出了一种涵盖基底神经节动态、病理振荡及15种生理属性的神经生理学基准，并通过β波段活动和反馈建模。

Result: 该方法为训练和评估深度强化学习算法提供了结构化环境，优化了aDBS控制策略。
...
### [225] [Temporal Interception and Present Reconstruction: A Cognitive-Signal Model for Human and AI Decision Making](https://arxiv.org/abs/2505.09646)
*Carmel Mary Esther A*

Main category: q-bio.NC

TL;DR: 论文提出了一种新理论模型，探讨如何通过减少感知延迟实现人类与人工智能的实时意识。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解决人类和AI在实时感知中的延迟问题，探索从被动感知转向主动与未来交互的可能性。

Method: 结合宇宙信号延迟、神经反应时间和静态认知状态，提出物理与认知模型，并通过神经观测和神经接收扩展进行实验验证。

Result: 提出了一种将当下视为早期宇宙信号与人类延迟交汇的干扰区的模型，并设计了数学框架指导AI系统的进化。

Conclusion: 该研究为AI系统的高效、伦理和意识化决策提供了理论基础和实验方向。
...
### [226] [A Computational Approach to Epilepsy Treatment: An AI-optimized Global Natural Product Prescription System](https://arxiv.org/abs/2505.09643)
*Zhixuan Wang*

Main category: q-bio.NC

TL;DR: 该研究开发了一种基于AI的计算方法，优化草药治疗癫痫的方案，结合机器学习、贝叶斯优化和RCTs的元分析，显著提高了治疗效果。


<details>
  <summary>Details</summary>
Motivation: 由于传统抗癫痫药物效果有限且副作用多，许多患者转向替代医学，因此需要一种更有效的治疗方法。

Method: 研究结合机器学习算法、贝叶斯优化和RCTs的元分析，分析了1872种天然化合物和48项RCTs数据，通过LASSO回归和SHAP值分析筛选高效草药。

Result: 研究发现17种高效草药（如天麻、睡茄），在随机双盲验证试验中，AI优化的草药方案比传统方法减少了28.5%的癫痫发作频率。

Conclusion: AI优化的草药治疗方案在癫痫治疗中表现出显著优势，为替代医学提供了科学依据。
...
### [227] [ImplicitStainer: Data-Efficient Medical Image Translation for Virtual Antibody-based Tissue Staining Using Local Implicit Functions](https://arxiv.org/abs/2505.09831)
*Tushar Kataria,Beatrice Knudsen,Shireen Y. Elhabian*

Main category: eess.IV

TL;DR: ImplicitStainer是一种基于局部隐式函数的新方法，用于从H&E染色图像中虚拟生成IHC染色图像，提高了数据效率和质量。


<details>
  <summary>Details</summary>
Motivation: H&E染色无法提供所有诊断信息，而IHC染色耗时且不易获取。虚拟染色技术通过深度学习提供替代方案，但现有方法数据需求高。

Method: 利用局部隐式函数进行像素级预测，提升图像翻译性能，减少数据需求。

Result: 在两种数据集上验证，性能优于15种现有GAN和扩散模型，数据效率更高。

Conclusion: ImplicitStainer在虚拟染色中表现出色，尤其适用于数据有限的情况，代码将公开。
...
### [228] [Ordered-subsets Multi-diffusion Model for Sparse-view CT Reconstruction](https://arxiv.org/abs/2505.09985)
*Pengfei Yu,Bin Huang,Minghui Zhang,Weiwen Wu,Shaoyu Wang,Qiegen Liu*

Main category: eess.IV

TL;DR: 提出了一种基于有序子集的多扩散模型（OSMM），用于稀疏视角CT重建，通过分块学习和全局约束提升细节重建效果。
...
Method: 将投影数据分为子集，用多子集扩散模型（MSDM）独立学习，并结合全局扩散模型（OWDM）约束信息一致性。
...
### [229] [Visual Fidelity Index for Generative Semantic Communications with Critical Information Embedding](https://arxiv.org/abs/2505.10405)
*Jianhao Huang,Qunsong Zeng,Kaibin Huang*

Main category: eess.IV

TL;DR: 提出了一种结合文本提示和关键特征的混合生成语义通信系统，并设计了GVIF指标评估视觉质量，优化系统性能。


<details>
  <summary>Details</summary>
Motivation: 解决纯提示驱动生成丢失细节及缺乏系统评估指标的问题。

Method: 通过语义过滤提取关键特征，结合扩散生成模型重构图像，设计GVIF指标量化视觉保真度。

Result: GVIF指标敏感于视觉保真度，优化系统在PSNR和FID上优于基准方案。

Conclusion: 混合Gen-SemCom系统及GVIF指标有效提升了生成图像的视觉质量和通信效率。
...
### [230] [HWA-UNETR: Hierarchical Window Aggregate UNETR for 3D Multimodal Gastric Lesion Segmentation](https://arxiv.org/abs/2505.10464)
*Jiaming Liang,Lihuan Dai,Xiaoqi Sheng,Xiangguang Chen,Chun Yao,Guihua Tao,Qibin Leng,Honming Cai,Xi Zhong*

Main category: eess.IV

TL;DR: 论文提出了GCM 2025数据集和HWA-UNETR框架，解决了胃癌多模态图像分割中的数据稀缺和对齐问题，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 胃癌多模态图像分割面临数据稀缺和模态对齐问题，导致训练困难和准确性下降。

Method: 公开GCM 2025数据集，并提出HWA-UNETR框架，利用HWA块和三元融合机制实现多模态特征对齐和长距离依赖建模。

Result: 在GCM 2025和BraTS 2021数据集上，HWA-UNETR的Dice分数比现有方法高1.68%，且鲁棒性强。

Conclusion: GCM 2025数据集和HWA-UNETR框架为胃癌多模态图像分割提供了有效解决方案，性能显著提升。
...
### [231] [Multi-contrast laser endoscopy for in vivo gastrointestinal imaging](https://arxiv.org/abs/2505.10492)
*Taylor L. Bobrow,Mayank Golhar,Suchapa Arayakarnkul,Anthony A. Song,Saowanee Ngamruengphong,Nicholas J. Durr*

Main category: eess.IV

TL;DR: 论文提出了一种多对比激光内窥镜（MLE）技术，用于增强胃肠道疾病的检测能力，相比传统白光内窥镜和窄带成像，MLE在对比度和颜色差异上有显著提升。


<details>
  <summary>Details</summary>
Motivation: 传统白光内窥镜在检测胃肠道疾病时，由于组织颜色、纹理和形状的对比度较低，许多临床相关病例难以被发现。

Method: MLE平台通过可快速调谐的光谱、相干和方向性照明，实现多模态成像，包括多光谱漫反射增强组织色素对比、激光散斑对比成像量化血流、以及光度立体成像表征黏膜地形。

Result: 在31个息肉样本中，MLE的对比度提高了约三倍，颜色差异提高了五倍，显著优于白光和窄带成像。

Conclusion: MLE作为一种新型成像工具，能够无缝集成到临床环境中，为胃肠道成像提供多模态对比，具有广阔的应用前景。
...
### [232] [ManipBench: Benchmarking Vision-Language Models for Low-Level Robot Manipulation](https://arxiv.org/abs/2505.09698)
*Enyu Zhao,Vedant Raval,Hejia Zhang,Jiageng Mao,Zeyu Shangguan,Stefanos Nikolaidis,Yue Wang,Daniel Seita*

Main category: cs.RO

TL;DR: 提出了一个名为ManipBench的新基准，用于评估视觉语言模型（VLMs）在机器人低层推理能力上的表现。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏一个明确且通用的基准来评估VLMs在机器人低层推理中的能力，尤其是在物体交互和可变形物体操作方面的理解。

Method: 设计了ManipBench基准，测试了33个代表性VLMs在10个模型家族中的表现，包括不同模型规模的变体。

Result: VLMs在不同任务中的表现差异显著，且其性能与实际机器人操作任务的趋势有强相关性，但与人类理解水平仍有显著差距。
...
### [233] [FlowDreamer: A RGB-D World Model with Flow-based Motion Representations for Robot Manipulation](https://arxiv.org/abs/2505.10075)
*Jun Guo,Xiaojian Ma,Yikai Wang,Min Yang,Huaping Liu,Qing Li*

Main category: cs.RO

TL;DR: FlowDreamer提出了一种基于3D场景流的视觉世界模型，通过显式运动表示提升机器人操作的视觉预测能力。
...
### [234] [Multi-Robot Task Allocation for Homogeneous Tasks with Collision Avoidance via Spatial Clustering](https://arxiv.org/abs/2505.10073)
...
Conclusion: 空间分区统一了任务分配和碰撞避免问题，适用于实际工业应用。
...
### [235] [EmbodiedMAE: A Unified 3D Multi-Modal Representation for Robot Manipulation](https://arxiv.org/abs/2505.10105)
*Zibin Dong,Fei Ni,Yifu Yuan,Yinchuan Li,Jianye Hao*

Main category: cs.RO

TL;DR: EmbodiedMAE是一种统一的多模态3D表示方法，用于机器人操作，通过改进数据集和模型架构，显著提升了性能。
...
Method: 通过增强DROID数据集为DROID-3D，并开发多模态掩码自编码器EmbodiedMAE，跨RGB、深度和点云模态学习表示。

Result: 在70个仿真任务和20个真实机器人操作任务中，EmbodiedMAE优于现有视觉基础模型，表现出高效的训练和性能。
...
### [236] [Learning Rock Pushability on Rough Planetary Terrain](https://arxiv.org/abs/2505.09833)
*Tuba Girgin,Emre Girgin,Cagri Kilic*

Main category: cs.RO

TL;DR: 论文提出了一种新的移动导航方法，通过机器人操纵器重新定位障碍物而非避障，以提高多代理长期路径效率。


<details>
  <summary>Details</summary>
Motivation: 在非结构化环境中，传统避障方法可能导致长期效率低下，尤其是在多代理重复使用同一路径时。

Method: 结合外感和本体反馈评估障碍物的可推动性，利用机器人操纵器重新定位障碍物。

Result: 初步结果表明，该方法能减少多代理在长期使用路径上的时间消耗。

Conclusion: 该方法为月球或火星等需要自主基础设施开发的环境提供了更高效的导航解决方案。
...
### [237] [Evaluating Robustness of Deep Reinforcement Learning for Autonomous Surface Vehicle Control in Field Tests](https://arxiv.org/abs/2505.10033)
*Luis F. W. Batista,Stéphanie Aravecchia,Seth Hutchinson,Cédric Pradalier*

Main category: cs.RO

TL;DR: 评估DRL在ASV中处理外部干扰的鲁棒性，通过域随机化训练并在仿真和实际测试中验证其性能。


<details>
  <summary>Details</summary>
Motivation: 尽管DRL在ASV中有显著进展，但其在真实环境中的鲁棒性，尤其是对外部干扰的应对能力，尚未充分研究。

Method: 使用域随机化训练DRL代理，并在仿真和实际测试中评估其对不对称拖曳和偏心载荷等干扰的处理能力。

Result: DRL代理在显著干扰下表现可靠，性能优于MPC基线。

Conclusion: 研究提供了DRL在ASV中部署的有效训练策略和实际挑战的见解，并开源了实现。
...
TL;DR: IN-RIL提出了一种交替使用模仿学习（IL）和强化学习（RL）的方法，通过梯度分离机制避免冲突，显著提升了RL微调的稳定性和样本效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法中IL预训练后RL微调存在不稳定性和样本效率低的问题，IN-RIL旨在解决这些问题。

Method: IN-RIL在RL微调过程中定期注入IL更新，并通过梯度分离机制避免优化目标的冲突。
...
### [239] [Knowledge capture, adaptation and composition (KCAC): A framework for cross-task curriculum learning in robotic manipulation](https://arxiv.org/abs/2505.10522)
*Xinrui Wang,Yan Jin*

Main category: cs.RO

TL;DR: 论文提出了一种知识捕获、适应与组合（KCAC）框架，通过跨任务课程学习将知识迁移系统化地融入强化学习，显著提升了训练效率和任务成功率。


<details>
  <summary>Details</summary>
Motivation: 强化学习在机器人操作中潜力巨大，但样本效率低和缺乏可解释性限制了其实际应用。通过知识迁移提升代理的理解和适应能力是关键。
...
### [240] [Real-Time Out-of-Distribution Failure Prevention via Multi-Modal Reasoning](https://arxiv.org/abs/2505.10547)
...
Method: FORTRESS利用多模态推理识别目标和预测故障模式，实时生成安全回退计划。

Result: 在合成基准和真实机器人数据上，FORTRESS在安全分类准确性和规划成功率上表现优异。

Conclusion: FORTRESS显著提升了系统安全性，适用于复杂环境中的机器人导航。
...
### [241] [AutoCam: Hierarchical Path Planning for an Autonomous Auxiliary Camera in Surgical Robotics](https://arxiv.org/abs/2505.10398)
*Alexandre Banks,Randy Moore,Sayem Nazmuz Zaman,Alaa Eldin Abdelaal,Septimiu E. Salcudean*

Main category: cs.RO

TL;DR: AutoCam是一种自动辅助摄像头放置方法，用于提升机器人辅助微创手术中的可视化效果，结合启发式几何放置和非线性优化，确保稳定的摄像头跟踪。


<details>
  <summary>Details</summary>
Motivation: 现有辅助摄像头路径规划方法仅跟踪二维手术特征，未同时考虑摄像头方向、工作空间限制和机器人关节限制。

Method: 采用基于优先级的工作空间约束控制算法，结合启发式几何放置和非线性优化。

Result: 系统保持99.84%的显著特征可见性，姿态误差为4.36±2.11度和1.95±5.66毫米，计算效率高（循环时间6.8±12.8毫秒）。

Conclusion: 辅助摄像头可通过达芬奇患者侧操纵器自主控制跟踪显著特征，为RAMIS中多摄像头可视化方法奠定基础。
...
### [242] [Contextual Phenotyping of Pediatric Sepsis Cohort Using Large Language Models](https://arxiv.org/abs/2505.09805)
...
TL;DR: 该研究评估了基于大型语言模型（LLM）的聚类方法在儿科脓毒症数据集上的表现，发现其优于传统方法，能更好地捕捉上下文信息并识别具有不同特征的亚组。


<details>
  <summary>Details</summary>
Motivation: 传统聚类方法在处理高维、异构的医疗数据时表现不佳，缺乏上下文理解能力，因此研究探索了LLM在聚类任务中的潜力。

Method: 使用LLAMA 3.1 8B、DeepSeek-R1-Distill-Llama-8B和Stella-En-400M-V5模型生成嵌入，并应用K-means聚类。与传统方法（如K-Medoids）进行比较。

Result: Stella-En-400M-V5获得最高轮廓分数（0.86），LLM方法在识别具有不同特征的亚组上表现更优。
...
### [243] [Generative diffusion model surrogates for mechanistic agent-based biological models](https://arxiv.org/abs/2505.09630)
*Tien Comlekoglu,J. Quetzalcóatl Toledo-Marín,Douglas W. DeSimone,Shayn M. Peirce,Geoffrey Fox,James A. Glazier*

Main category: q-bio.QM

TL;DR: 利用去噪扩散概率模型（DDPM）训练生成式AI替代模型，加速计算密集型细胞-波特模型（CPM）的评估，实现22倍计算时间减少。


<details>
  <summary>Details</summary>
Motivation: CPM在复杂生物系统研究中计算成本高，且其随机性导致参数配置多样，开发替代模型具有挑战性。

Method: 使用DDPM训练生成式AI替代模型，结合图像分类器学习参数空间特征，辅助模型选择和验证。

Result: 替代模型能提前20,000时间步生成配置，计算时间减少约22倍。

Conclusion: 该方法为随机生物系统的数字孪生实现提供了可行路径。
...
### [244] [AI Greenferencing: Routing AI Inferencing to Green Modular Data Centers with Heron](https://arxiv.org/abs/2505.09989)
...
TL;DR: 论文提出将AI计算负载部署到风电场附近的模块化计算集群中，利用廉价绿色能源，并通过软件路由器Heron优化负载分配，提升计算效率。


<details>
  <summary>Details</summary>
Motivation: AI计算需求激增，而风电场有大量待接入电网的风能资源，结合两者可经济高效地利用绿色能源。

Method: 提出部署策略和Heron软件路由器，通过动态路由AI推理负载应对风能波动。

Result: Heron在1周测试中，相比现有技术，将AI计算的总吞吐量提升了80%。

Conclusion: 结合风能与AI计算负载的部署策略及Heron路由器，可显著提升绿色能源利用效率和计算性能。
...
### [245] [KAITIAN: A Unified Communication Framework for Enabling Efficient Collaboration Across Heterogeneous Accelerators in Embodied AI Systems](https://arxiv.org/abs/2505.10183)
...
<details>
  <summary>Details</summary>
Motivation: 异构加速器（如GPU、NPU、FPGA）在AI系统中广泛应用，但专有通信库导致互操作性差和性能瓶颈。

Method: KAITIAN提供统一抽象层，结合优化库和通用协议，并采用负载自适应调度机制。

Result: 实验显示KAITIAN加速训练时间42%，通信开销仅2.8-4.3%，保持模型精度。
...
### [246] [$XX^{t}$ Can Be Faster](https://arxiv.org/abs/2505.09814)
*Dmitry Rybin,Yushun Zhang,Zhi-Quan Luo*

Main category: cs.DS

TL;DR: 新算法RXTX通过结合机器学习搜索与组合优化，减少了矩阵乘其转置的计算量，比现有技术少用5%的乘法和加法，且对小矩阵也有加速效果。


<details>
  <summary>Details</summary>
Motivation: 优化矩阵乘其转置的计算效率，减少计算资源消耗。

Method: 结合机器学习搜索方法与组合优化技术，开发新算法RXTX。

Result: RXTX比现有技术少用5%的乘法和加法，对小矩阵也能实现加速。

Conclusion: RXTX是一种高效算法，适用于矩阵乘其转置的计算优化。
...
### [247] [On Unbiased Low-Rank Approximation with Minimum Distortion](https://arxiv.org/abs/2505.09647)
...
<details>
  <summary>Details</summary>
Motivation: 解决在低秩约束下如何最优逼近目标矩阵的问题，同时保持无偏性和最小化误差。

Method: 算法基于矩阵P的奇异分量，类似于向量无偏稀疏化的解决方案。

Result: 算法与现有下界匹配，证明了其最优性。

Conclusion: 该算法在低秩约束下实现了对目标矩阵的最优无偏逼近。
...
### [248] [Ontology-Based Structuring and Analysis of North Macedonian Public Procurement Contracts](https://arxiv.org/abs/2505.09798)
*Bojan Ristov,Stefan Eftimov,Milena Trajanoska,Dimitar Trajanov*

Main category: cs.DB

TL;DR: 提出了一种将政府采购数据转换为语义知识图谱的方法，提升数据透明度和分析能力。


<details>
  <summary>Details</summary>
Motivation: 传统政府采购数据格式僵化，限制了分析潜力和透明度。

Method: 利用本体建模和自动化数据转换技术，结合RDF和SPARQL查询，并引入机器学习预测模型。

Result: 系统增强了数据的可访问性和可解释性，支持复杂语义查询和高级分析。

Conclusion: 该方法提升了政府采购透明度，支持决策制定，并为北马其顿的采购活动提供了深入分析。
...
### [249] [Inferring entropy production in many-body systems using nonequilibrium MaxEnt](https://arxiv.org/abs/2505.10444)
*Miguel Aguilera,Sosuke Ito,Artemy Kolchinsky*

Main category: cond-mat.stat-mech

TL;DR: 提出了一种在高维随机系统中推断熵产生（EP）的方法，适用于多体系统和非马尔可夫系统。


<details>
  <summary>Details</summary>
Motivation: 传统EP估计方法在高维系统中因计算和统计限制难以应用，需要一种更高效的方法。

Method: 利用非平衡最大熵原理和凸对偶性，通过轨迹观测样本推断EP及其下界。

Result: 方法无需重构高维概率分布或速率矩阵，适用于复杂系统，并在1000自旋无序非平衡模型和神经尖峰数据集中验证。

Conclusion: 该方法提供了一种直观的EP分解和热力学不确定性关系解释，适用于高维复杂系统。
...
### [250] [VRSplat: Fast and Robust Gaussian Splatting for Virtual Reality](https://arxiv.org/abs/2505.10144)
...
<details>
  <summary>Details</summary>
Motivation: 3DGS在VR中面临时间伪影、投影失真和帧率不足等问题，这些问题在VR环境中尤为突出。

Method: 结合Mini-Splatting、StopThePop和Optimal Projection技术，改进3DGS栅格化器，提出高效的中心凹栅格化器，并优化高斯参数。

Result: 用户研究表明VRSplat优于其他配置，实现了72+ FPS且消除了伪影。
...
### [251] [Style Customization of Text-to-Vector Generation with Image Diffusion Priors](https://arxiv.org/abs/2505.10558)
...
<details>
  <summary>Details</summary>
Motivation: 现有T2V方法在风格定制上存在不足，无法同时保证结构规律性和内容与风格的解耦。

Method: 两阶段流程：1) 训练路径级表示的T2V扩散模型；2) 通过蒸馏定制T2I模型实现风格定制。

Result: 实验验证了方法的高效性，能生成高质量、风格一致的SVG。

Conclusion: 提出的方法在风格定制和结构规律性上优于现有技术。
...
### [252] [ILIF: Temporal Inhibitory Leaky Integrate-and-Fire Neuron for Overactivation in Spiking Neural Networks](https://arxiv.org/abs/2505.10371)
*Kai Sun,Peibo Duan,Levin Kuhlmann,Beilun Wang,Bin Zhang*

Main category: cs.NE

TL;DR: 论文提出了一种新型的ILIF神经元模型，通过引入抑制机制解决了SNN训练中的gamma困境，提高了能量效率和准确性。


<details>
  <summary>Details</summary>
Motivation: SNN训练中，过大的gamma会导致神经元过度激活和能耗增加，而过小的gamma会导致梯度消失和时间依赖性减弱。

Method: 提出了一种基于生物抑制机制的ILIF神经元模型，通过抑制单元调节膜电位和电流，平衡激活与梯度传播。

Result: 理论分析和实验表明，ILIF能有效减少神经元放电率、稳定训练并提升准确性。

Conclusion: ILIF模型成功解决了gamma困境，为SNN的高效训练提供了新思路。
...
### [253] [Large Wireless Localization Model (LWLM): A Foundation Model for Positioning in 6G Networks](https://arxiv.org/abs/2505.10134)
...
Method: 设计了LWLM模型，结合空间-频率掩码信道建模（SF-MCM）、域变换不变性（DTI）和位置不变对比学习（PICL）三个自监督学习目标，并针对下游任务设计轻量解码器。

Result: LWLM在所有定位任务中均优于基于模型和监督学习的基线方法，性能提升26.0%--87.5%，且在标注数据有限和未见基站配置下表现出强泛化能力。

Conclusion: LWLM作为无线定位的基础模型具有显著潜力，能够有效解决现有方法的局限性。
...
### [254] [A Fine-Grained Complexity View on Propositional Abduction -- Algorithms and Lower Bounds](https://arxiv.org/abs/2505.10201)
...
### [255] [Trustless Autonomy: Understanding Motivations, Benefits and Governance Dilemma in Self-Sovereign Decentralized AI Agents](https://arxiv.org/abs/2505.09757)
*Botao Amber Hu,Yuhan Liu,Helena Rong*

Main category: cs.HC

TL;DR: 论文探讨了结合LLM和去中心化技术的自治理AI代理（DeAgents），分析其动机、优势及治理困境。
...
Result: 研究结果将指导未来DeAgents系统和协议设计，并讨论AI治理问题。

Conclusion: DeAgents在去中心化和可靠性之间存在矛盾，需进一步研究以优化治理和设计。
...
### [256] [Visual Feedback of Pattern Separability Improves Myoelectric Decoding Performance of Upper Limb Prostheses](https://arxiv.org/abs/2505.09819)
...
TL;DR: 3D视觉反馈工具Reviewer通过实时投影EMG信号到解码器分类空间，显著提升肌电假肢模式识别控制的性能。


<details>
  <summary>Details</summary>
Motivation: 现有肌电假肢模式识别控制中，用户难以生成足够独特的EMG信号，且训练过程依赖试错调整。

Method: 通过10次实验，比较使用Reviewer与传统虚拟手臂视觉反馈的训练效果，评估Fitts定律任务表现。

Result: 使用Reviewer的参与者完成率更高，路径效率提升，且减少了过冲。

Conclusion: 3D视觉反馈通过结构化训练和实时反馈，显著改善新手操作者的模式识别控制能力。
...
### [257] [SOS: A Shuffle Order Strategy for Data Augmentation in Industrial Human Activity Recognition](https://arxiv.org/abs/2505.10312)
*Anh Tuan Ha,Hoang Khang Phan,Thai Minh Tien Ngo,Anh Phan Truong,Nhat Tan Le*

Main category: cs.HC

TL;DR: 该论文提出了一种通过深度学习生成高质量HAR数据的方法，并通过随机序列策略提升分类性能。
...
Conclusion: 该方法不仅扩展了有效训练数据集，还为复杂场景下的HAR系统提供了优化方向。
...
### [258] [AI LEGO: Scaffolding Cross-Functional Collaboration in Industrial Responsible AI Practices during Early Design Stages](https://arxiv.org/abs/2505.10300)
*Muzhe Wu,Yanzhi Zhao,Shuyi Han,Michael Xieyang Liu,Hong Shen*

Main category: cs.HC

TL;DR: 论文探讨了跨职能团队在AI开发早期阶段知识传递的挑战，并提出了一种名为AI LEGO的解决方案，通过可视化工具提升非技术角色对潜在危害的识别能力。


<details>
  <summary>Details</summary>
Motivation: 解决跨职能团队中技术设计理由难以传递给非技术角色进行伦理评估和危害识别的问题。

Method: 通过文献综述和与8名从业者的共同设计研究，开发了AI LEGO原型，支持技术与非技术角色协作识别潜在危害。

Result: 在18名从业者的研究中，AI LEGO显著提高了危害识别的数量和可能性，其模块化结构和角色模拟使危害识别更易操作。

Conclusion: AI LEGO通过改进知识传递和协作流程，促进了早期设计中更清晰和协作的负责任AI实践。
...
<details>
  <summary>Details</summary>
Motivation: 纯组分物性预测对过程集成和多尺度建模至关重要，现有方法需改进精度和解释性。

Method: 采用基于连接矩阵的分子表示方法生成特征，结合随机森林和调整R2进行特征筛选，使用ANN和GPR模型预测物性。

Result: 测试集均方根误差降低83.8%，特征数从13316降至100，且不同物性受不同结构特征影响。

Conclusion: 框架可行，为混合物重建和过程集成建模奠定基础。
...
### [260] [Who Said What WSW 2.0? Enhanced Automated Analysis of Preschool Classroom Speech](https://arxiv.org/abs/2505.09972)
...
TL;DR: WSW2.0是一个自动分析学前课堂语音交互的框架，结合wav2vec2和Whisper技术，提高了准确性和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 通过深度学习技术改进学前课堂语音分析的准确性和可扩展性，支持教育研究和早期语言发展。

Method: 使用wav2vec2进行说话人分类，Whisper进行语音转录，并与专家标注对比验证。

Result: WSW2.0在说话人分类和语音转录上表现良好，F1分数0.845，转录错误率较低（教师0.119，儿童0.238）。

Conclusion: WSW2.0展示了深度学习在教育研究中的潜力，为干预策略和语言发展提供了可靠工具。
...
### [261] [Forests for Differences: Robust Causal Inference Beyond Parametric DiD](https://arxiv.org/abs/2505.09706)
*Hugo Gobato Souto,Francisco Louzada Neto*

Main category: stat.ME

TL;DR: DiD-BCF是一种新型非参数模型，用于解决DiD估计中的关键挑战，如交错采用和异质性处理效应。


<details>
  <summary>Details</summary>
Motivation: 解决传统DiD方法在处理交错采用和异质性效应时的局限性。

Method: 提出基于平行趋势假设（PTA）的重新参数化框架，统一估计ATE、GATE和CATE。

Result: 在模拟中表现优于传统方法，尤其适用于非线性、选择偏差和效应异质性场景。应用于美国最低工资政策，揭示传统方法忽略的异质性效应。
...
### [262] [Estimating the number of household TV profiles based in customer behaviour using Gaussian mixture model averaging](https://arxiv.org/abs/2505.10279)
*Gabriel R. Palma,Sally McClean,Brahim Allan,Zeeshan Tariq,Rafael A. Moral*

Main category: stat.ME

TL;DR: 论文提出了一种新框架，结合高斯混合模型和贝叶斯随机游走模型，用于估计家庭电视观看行为的多个用户画像，以改进群体观看的个性化推荐。


<details>
  <summary>Details</summary>
Motivation: 电视用户面临众多频道和点播服务选择，个性化推荐能节省用户时间。但关键在于理解家庭中多人观看时的行为，目前缺乏明确的观看者信息和多用户画像组合方法。

Method: 使用高斯混合模型平均估计家庭电视画像数量，贝叶斯随机游走模型引入不确定性。基于50万条真实用户数据验证。

Result: 框架能有效估计家庭电视画像数量及其特征，包括随时间变化和不确定性量化。

Conclusion: 提出的方法为群体观看的个性化推荐提供了有效工具。
...
### [263] [LanTu: Dynamics-Enhanced Deep Learning for Eddy-Resolving Ocean Forecasting](https://arxiv.org/abs/2505.10191)
...
<details>
  <summary>Details</summary>
Motivation: 中尺度涡旋对全球海洋能量级联有重要影响，但传统数值模型在涡旋分辨预报中面临科学挑战和高计算成本，AI预报系统在复杂多尺度特征下仍有不足。

Method: 开发LanTu系统，融入跨尺度交互和多尺度物理约束，结合涡旋动力学知识优化模型。

Result: LanTu在温度、盐度、海平面异常和流场预报上优于现有数值和AI系统，预报时效超过10天。

Conclusion: 动力学增强深度学习（LanTu）为中尺度涡旋分辨海洋预报提供了高效范式。
...
### [264] [Risk-Aware Safe Reinforcement Learning for Control of Stochastic Linear Systems](https://arxiv.org/abs/2505.09734)
*Babak Esmaeili,Nariman Niknejad,Hamidreza Modares*

Main category: eess.SY

TL;DR: 本文提出了一种风险感知的安全强化学习（RL）控制设计，用于随机离散时间线性系统。通过同时学习RL控制器和安全控制器，并优化决策变量，实现了高效且安全的数据驱动控制。


<details>
  <summary>Details</summary>
Motivation: 传统RL控制器依赖安全认证器进行干预，可能导致短视行为或收敛到不理想平衡点。本文旨在通过结合RL与安全控制器，避免这些问题并提高安全性。

Method: 1) 学习分段仿射控制器以扩大不变集；2) 通过数据表示闭环系统，优化决策变量以最小化安全违规概率；3) 引入数据驱动插值技术整合RL与安全控制器。

Result: 该方法减少了数据需求，降低了安全违规的方差，并通过仿真验证了理论结果。

Conclusion: 结合RL与安全控制器的方法在保证安全性的同时提高了控制效率，适用于噪声环境。
...
### [265] [A Hybrid Strategy for Aggregated Probabilistic Forecasting and Energy Trading in HEFTCom2024](https://arxiv.org/abs/2505.10367)
*Chuanqing Pu,Feilong Fan,Nengling Tai,Songyuan Liu,Jinming Yu*

Main category: eess.SY

TL;DR: 团队GEB在IEEE HEFTCom2024竞赛中表现出色，提出了一种结合多种方法的解决方案，包括基于堆叠的风电预测、太阳能后处理模型、概率聚合方法和随机交易策略，以优化能源预测和交易收益。


<details>
  <summary>Details</summary>
Motivation: 未来能源系统中，准确预测能源产量和应对不确定性是核心挑战。团队GEB旨在提供高效解决方案，以提升预测精度和交易收益。

Method: 1. 基于堆叠的风电预测方法；2. 太阳能后处理模型；3. 概率聚合方法；4. 随机交易策略。

Result: 团队在竞赛中表现优异，风电预测和交易收益显著提升。

Conclusion: 该方法在能源预测和交易中具有实际应用价值，且代码开源便于进一步研究。
...
### [266] [Uncovering Magnetic Phases with Synthetic Data and Physics-Informed Training](https://arxiv.org/abs/2505.10393)
*Agustin Medina,Marcelo Arlego,Carlos A. Lamas*

Main category: cond-mat.str-el

TL;DR: 使用人工神经网络结合物理信息策略，高效学习磁性相变，验证了合成数据训练的有效性。


<details>
  <summary>Details</summary>
Motivation: 研究如何通过计算简单且物理信息化的方法，高效学习复杂磁性系统的相变行为。

Method: 采用监督分类（密集神经网络）和无监督检测（卷积自编码器）两种方法，结合物理信息指导（对称性破缺特征和训练配置）。

Result: 合成数据训练方案能有效揭示物理相边界，验证了机器学习预测的准确性。

Conclusion: 该方法为复杂系统提供了一种低成本、高效的相变检测替代方案，适用于更广泛的凝聚态和统计物理问题。
...
### [267] [Inconsistency Handling in DatalogMTL](https://arxiv.org/abs/2505.10394)
...
<details>
  <summary>Details</summary>
Motivation: 研究DatalogMTL中时间区间事实与规则矛盾时的不一致性处理方式。

Method: 定义了冲突（最小不一致解释）和修复（恢复一致性的方法），并研究了其性质和语义。
...
### [268] [SpecWav-Attack: Leveraging Spectrogram Resizing and Wav2Vec 2.0 for Attacking Anonymized Speech](https://arxiv.org/abs/2505.09616)
...
### [269] [Introducing voice timbre attribute detection](https://arxiv.org/abs/2505.09661)
...
<details>
  <summary>Details</summary>
Motivation: 研究语音信号中音色的感知属性，提出vTAD任务以量化描述音色特征。

Method: 使用说话人嵌入框架处理语音对，比较其在音色描述符中的强度，并在VCTK-RVA数据集上测试ECAPA-TDNN和FACodec编码器。

Result: ECAPA-TDNN在训练集包含测试说话人时表现更好，FACodec在未见说话人时泛化能力更强。

Conclusion: 不同编码器适用于不同场景，vTAD任务为音色研究提供了新方法，数据集和代码已开源。
...
### [270] [LAV: Audio-Driven Dynamic Visual Generation with Neural Compression and StyleGAN2](https://arxiv.org/abs/2505.10101)
*Jongmin Jung,Dasaem Jeong*

Main category: cs.SD

TL;DR: LAV系统结合EnCodec音频压缩与StyleGAN2生成能力，通过预录音频驱动动态视觉输出，利用隐式特征映射实现语义丰富的音视转换。


<details>
  <summary>Details</summary>
Motivation: 探索预训练音频压缩模型在艺术与计算应用中的潜力，实现更细腻且语义连贯的音视转换。

Method: 使用EnCodec嵌入作为隐式表示，通过随机初始化的线性映射直接转换为StyleGAN2的隐式空间。

Result: 系统展示了语义丰富的音视转换能力，保留了音频的语义信息。

Conclusion: LAV框架为音视转换提供了新思路，展现了预训练模型的广泛应用前景。
...
### [271] [Detecting Musical Deepfakes](https://arxiv.org/abs/2505.09633)
*Nick Sunday*

Main category: cs.SD

TL;DR: 该研究探讨了如何检测AI生成的音乐，通过使用FakeMusicCaps数据集和卷积神经网络，同时讨论了TTM平台的伦理和社会影响。


<details>
  <summary>Details</summary>
Motivation: 随着TTM平台的普及，AI生成音乐对音乐行业和音乐人带来了新挑战，需要有效的检测方法。

Method: 研究使用FakeMusicCaps数据集，通过调整音频的节奏和音高生成梅尔频谱图，并用卷积神经网络进行分类。

Result: 研究展示了检测AI生成音乐的技术成果，并验证了方法的有效性。
...
### [272] [Learning Nonlinear Dynamics in Physical Modelling Synthesis using Neural Ordinary Differential Equations](https://arxiv.org/abs/2505.10511)
*Victor Zheleznov,Stefan Bilbao,Alec Wright,Simon King*

Main category: cs.SD

TL;DR: 该论文提出了一种结合模态分解与神经常微分方程的方法，用于建模分布式音乐系统中的非线性动态行为。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于利用机器学习和模态分解技术，解决高振幅弦振动中的几何非线性效应问题，如音高滑动和亮度依赖。

Method: 方法包括模态分解生成耦合非线性常微分方程组，并引入神经常微分方程来自动建模非线性动态行为。

Result: 通过合成非线性横向弦数据，模型成功训练并再现了系统的非线性动态行为。

Conclusion: 结论表明，该方法有效结合了物理模型与机器学习，为分布式音乐系统的建模提供了新思路。
...
### [273] [Deconstructing Subset Construction -- Reducing While Determinizing](https://arxiv.org/abs/2505.10319)
...
### [274] [AI and Generative AI Transforming Disaster Management: A Survey of Damage Assessment and Response Techniques](https://arxiv.org/abs/2505.08202)
...
Method: 通过多模态数据（文本、图像、视频、音频）分析，模拟灾害场景，并结合数据隐私、安全和伦理问题进行讨论。

Result: AI和GenAI在灾害评估中展现出高效性和潜力，但也存在数据隐私和伦理风险。
...
### [275] [Healthy Distrust in AI systems](https://arxiv.org/abs/2505.09747)
...
<details>
  <summary>Details</summary>
Motivation: 当前AI研究强调信任以促进采用，但忽视了社会背景下AI可能被滥用的情况，需要合理的不信任来平衡。

Method: 通过跨学科（计算机科学、社会学、心理学等）分析信任与不信任的概念，提出“健康的不信任”填补现有研究空白。

Result: “健康的不信任”被定义为对AI使用实践的合理谨慎态度，是尊重人类自主权的关键。

Conclusion: 论文认为，健康的不信任是构建真正可信AI的必要组成部分，需在AI设计中加以考虑。
...
### [276] [Which Demographic Features Are Relevant for Individual Fairness Evaluation of U.S. Recidivism Risk Assessment Tools?](https://arxiv.org/abs/2505.09868)
*Tin Trung Nguyen,Jiannan Xu,Phuong-Anh Nguyen-Le,Jonathan Lazar,Donald Braman,Hal Daumé III,Zubin Jelveh*

Main category: cs.CY

TL;DR: 研究探讨了在累犯风险评估（RRA）工具中，哪些人口特征应纳入个体公平性评估，结论是年龄和性别应考虑，而种族应忽略。


<details>
  <summary>Details</summary>
Motivation: 尽管个体公平性有宪法基础，但尚未在法规中具体化，研究旨在填补这一空白。

Method: 通过人类受试者实验，评估哪些人口特征与个体公平性相关。

Result: 分析显示，个体相似性函数应考虑年龄和性别，但忽略种族。

Conclusion: 研究为个体公平性评估提供了具体的人口特征指导，支持年龄和性别的纳入，但排除种族。
...
### [277] [Differentiable Quantum Architecture Search in Quantum-Enhanced Neural Network Parameter Generation](https://arxiv.org/abs/2505.09653)
*Samuel Yen-Chi Chen,Chen-Yu Liu,Kuan-Cheng Chen,Wei-Jia Huang,Yen-Jui Chang,Wei-Hao Huang*

Main category: quant-ph

TL;DR: 本文提出了一种基于可微分优化的自动化方法，用于设计量子神经网络（QNN）架构，无需依赖量子硬件即可生成经典神经网络参数，并在多个任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 量子机器学习（QML）中的变分量子电路（VQC）在推理时依赖量子硬件，存在硬件缺陷和访问限制问题。Quantum-Train（QT）框架虽能生成经典参数，但设计有效量子电路架构仍需专业知识。

Method: 采用可微分优化方法，联合优化传统电路参数和架构参数，通过自动微分实现端到端优化。

Result: 在分类、时间序列预测和强化学习任务中，该方法表现优于或等同于手动设计的QNN架构。

Conclusion: 该方法为设计可生成经典神经网络参数的QNN提供了一种可扩展且自动化的途径。
...
### [278] [Quantum Computing and AI: Perspectives on Advanced Automation in Science and Engineering](https://arxiv.org/abs/2505.10012)
*Tadashi Kadowaki*

Main category: quant-ph

TL;DR: 论文提出Quantum CAE框架，结合量子计算与AI，推动科学自动化，并通过案例研究展示其在组合优化问题中的应用。
...
Result: 通过组合优化问题的案例研究验证了Quantum CAE的实用性。

Conclusion: 量子计算与AI的整合将推动自动化发现与创新，但需关注人机协作的动态关系。
...
### [279] [Role of scrambling and noise in temporal information processing with quantum systems](https://arxiv.org/abs/2505.10080)
*Weijie Xiong,Zoë Holmes,Armando Angrisani,Yudai Suzuki,Thiparat Chotibut,Supanut Thanasilp*

Main category: quant-ph

TL;DR: 论文研究了量子系统在时间信息处理中的性能，重点关注了量子储备池处理框架的可扩展性和记忆保留能力。在无噪声和有噪声条件下，分析了高阶酉设计的性能，发现测量结果会随储备池规模指数集中，但记忆会随规模和迭代指数衰减。


<details>
  <summary>Details</summary>
Motivation: 量子系统在时间信息处理中表现出色，但其性能的理论理解尚不完善。本文旨在填补这一空白，研究量子储备池处理框架的性能。

Method: 采用高阶酉设计模拟量子储备池，分析其在无噪声和有噪声条件下的可扩展性和记忆保留能力。

Result: 无噪声条件下，测量结果随储备池规模指数集中，但记忆随规模和迭代指数衰减。有噪声条件下，记忆同样指数衰减。

Conclusion: 量子储备池处理框架在小规模或有限迭代下可行，但扩展问题规模会降低性能，需权衡资源投入。
...
### [280] [A Survey on Large Language Models in Multimodal Recommender Systems](https://arxiv.org/abs/2505.09777)
...
<details>
  <summary>Details</summary>
Motivation: LLMs为MRS提供了语义推理、上下文学习和动态输入处理等新能力，但其可扩展性和模型可访问性也带来挑战。本文旨在梳理LLMs在MRS中的角色，推动该领域研究。

Method: 通过综述近期研究，提出分类法，总结提示策略、微调方法和数据适应技术，并分析评估指标和数据集。

Result: LLMs在MRS中展现出更强的灵活性和泛化能力，但也面临挑战。本文提出了新的分类法和未来研究方向。

Conclusion: LLMs为MRS带来新机遇，未来研究需解决可扩展性和可访问性问题，并探索更多应用场景。
...
### [281] [Boosting Text-to-Chart Retrieval through Training with Synthesized Semantic Insights](https://arxiv.org/abs/2505.10043)
*Yifan Wu,Lutao Yan,Yizhang Zhu,Yinan Mei,Jiannan Wang,Nan Tang,Yuyu Luo*

Main category: cs.IR

TL;DR: 论文提出了一种自动生成图表语义见解的训练数据开发流程，并基于此训练了CLIP模型ChartFinder，显著提升了文本到图表检索的性能。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图表检索系统因缺乏全面的语义信息而表现不佳，无法满足精确和模糊查询的需求。

Method: 开发了自动生成图表多层次语义见解的流程，训练了CLIP模型ChartFinder，结合视觉和语义信息。

Result: ChartFinder在精确查询中NDCG@10达到66.9%，比现有方法高11.58%；模糊查询中平均提升5%。

Conclusion: ChartFinder通过丰富的语义见解显著提升了文本到图表检索性能，为BI应用提供了更高效的解决方案。
...
### [282] [Do LLMs Memorize Recommendation Datasets? A Preliminary Study on MovieLens-1M](https://arxiv.org/abs/2505.10212)
...
Method: 通过定义数据集记忆（包括项目属性、用户画像和用户-项目交互的检索程度），并分析记忆对推荐性能的影响，比较不同模型家族和大小的记忆差异。
...
### [283] [Evaluating Large Language Models for the Generation of Unit Tests with Equivalence Partitions and Boundary Values](https://arxiv.org/abs/2505.09830)
*Martín Rodríguez,Gustavo Rossi,Alejandro Fernandez*

Main category: cs.SE

TL;DR: 研究评估了大型语言模型（LLMs）在自动生成测试用例中的潜力，并与人工测试对比，发现其效果依赖于精心设计的提示和精确的需求。


<details>
  <summary>Details</summary>
Motivation: 探讨LLMs在单元测试生成中的应用潜力，以解决程序员常忽视的测试设计复杂性。

Method: 开发优化提示，整合代码与需求，覆盖关键测试场景（如等价类和边界值），并通过定量指标和人工定性分析比较LLMs与人工测试。

Result: LLMs表现依赖于提示设计、实现质量和需求精确性，虽灵活有潜力但仍需人工监督。

Conclusion: 强调人工定性分析在自动化单元测试评估中的重要性，LLMs需与人工结合使用。
...
### [284] [Are Sparse Autoencoders Useful for Java Function Bug Detection?](https://arxiv.org/abs/2505.10375)
*Rui Melo,Claudia Mamede,Andre Catarino,Rui Abreu,Henrique Lopes Cardoso*

Main category: cs.SE

TL;DR: 论文探讨了稀疏自编码器（SAEs）作为轻量级、可解释的替代方案，用于检测Java函数中的漏洞，无需微调预训练大语言模型（LLMs）。
...
Method: 研究评估了SAEs在GPT-2 Small和Gemma 2B模型表示上的应用，测试其在不微调LLMs的情况下检测漏洞的能力。
...
### [285] [Are Large Language Models Robust in Understanding Code Against Semantics-Preserving Mutations?](https://arxiv.org/abs/2505.10443)
*Pedro Orvalho,Marta Kwiatkowska*

Main category: cs.SE

TL;DR: 评估大型语言模型（LLM）在Python程序推理中的能力，发现部分模型虽然预测正确但逻辑有误，且对代码变异的鲁棒性有限。


<details>
  <summary>Details</summary>
Motivation: LLM在编程任务中的可靠使用需要理解其推理能力和鲁棒性，但目前研究多关注预测准确性而非推理逻辑。

Method: 通过五种语义保留的代码变异（如变量重命名、循环转换等）测试六种LLM的推理能力，并结合人类专家分析。

Result: 部分LLM（如Llama3.2）在61%的情况下预测正确但逻辑有误，且对代码变异的响应不稳定。

Conclusion: LLM在代码理解中的推理能力和鲁棒性仍需改进。
...
### [286] [Virtual Dosimetrists: A Radiotherapy Training "Flight Simulator"](https://arxiv.org/abs/2505.09796)
...
### [287] [LibIQ: Toward Real-Time Spectrum Classification in O-RAN dApps](https://arxiv.org/abs/2505.10537)
*Filippo Olimpieri,Noemi Giustini,Andrea Lacava,Salvatore D'Oro,Tommaso Melodia,Francesca Cuomo*

Main category: cs.NI

TL;DR: 论文提出LibIQ库，通过dApps实现实时RF频谱分类，解决了RIC在实时监控和数据隐私方面的限制，平均分类准确率达97.8%。


<details>
  <summary>Details</summary>
Motivation: 解决O-RAN架构中RIC的延迟和数据隐私问题，以支持实时频谱监控和信号分类。

Method: 利用LibIQ库处理I/Q样本，结合CNN分类器，通过自定义dApp在5G部署中采集数据集并验证模型。

Result: 在多种场景下，模型平均分类准确率为97.8%。
...
### [288] [On Measuring Intrinsic Causal Attributions in Deep Neural Networks](https://arxiv.org/abs/2505.09660)
*Saptarshi Saha,Dhruv Vansraj Rathore,Soumadeep Saha,Utpal Garain,David Doermann*

Main category: stat.ML

TL;DR: 本文提出了一种量化神经网络中特征因果影响的方法，通过引入内在因果贡献（ICC）框架，并与Sobol指数关联，实验表明ICC比现有方法更直观可靠。


<details>
  <summary>Details</summary>
Motivation: 量化神经网络中特征的因果影响是当前研究热点，现有方法主要关注直接、间接和总因果效应，但缺乏对内在因果贡献的研究。

Method: 将神经网络视为结构因果模型（SCMs），提出可识别的生成后验框架来量化ICC，并将其与Sobol指数关联。

Result: 在合成和真实数据集上的实验表明，ICC比现有全局解释技术更直观和可靠。

Conclusion: ICC框架为神经网络特征因果影响的量化提供了更有效的工具，具有实际应用潜力。
...
### [289] [Learning Multi-Attribute Differential Graphs with Non-Convex Penalties](https://arxiv.org/abs/2505.09748)
*Jitendra K Tugnait*

Main category: stat.ML

TL;DR: 论文提出了一种基于非凸惩罚的D-trace损失函数方法，用于估计两个具有相似结构的多属性高斯图模型（GGMs）的差异。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要基于群Lasso惩罚的损失函数，本文旨在通过非凸惩罚（如log-sum和SCAD）提高估计的准确性和稀疏性。

Method: 采用非凸惩罚的D-trace损失函数，并提出了两种近端梯度下降法优化目标函数。

Result: 理论分析证明了在高维设置下的一致性、凸性和估计性能，并通过合成和真实数据验证了方法的有效性。

Conclusion: 该方法在多属性差分图估计中表现优于现有方法，尤其在稀疏性和准确性方面。
...
### [290] [LatticeVision: Image to Image Networks for Modeling Non-Stationary Spatial Data](https://arxiv.org/abs/2505.09803)
...
<details>
  <summary>Details</summary>
Motivation: 在科学和工业应用中，获取大量空间分布数据（如气候模型）成本高昂，统计模拟成为替代方案。然而，传统MLE方法计算复杂，尤其是对大规模非平稳数据。

Method: 将SAR模型的输入（空间场）和输出（参数）视为图像，利用I2I网络进行参数估计。

Result: I2I网络显著提高了非平稳SAR模型的参数估计速度和准确性。

Conclusion: I2I网络为复杂非平稳SAR模型的参数估计提供了高效且准确的解决方案。
...
TL;DR: 提出了一种基于梯度的快速稀疏投资组合优化方法，通过布尔松弛将组合问题转化为连续优化任务，解决了传统混合整数规划方法计算成本高的问题。
...
Method: 通过布尔松弛将组合问题转化为连续优化任务，利用可调参数将目标函数从凸函数逐步转变为凹函数，从而稳定地逼近稀疏二进制解。

Result: 该方法在大多数情况下与商业求解器的资产选择结果一致，少数情况下差异很小且投资组合方差误差可忽略。

Conclusion: 提出的梯度方法在计算效率和准确性上优于传统方法，适用于大规模稀疏投资组合优化。
...
### [292] [Path Gradients after Flow Matching](https://arxiv.org/abs/2505.10139)
*Lorenz Vaitl,Leon Klein*

Main category: stat.ML

TL;DR: 论文研究了结合路径梯度与Flow Matching优化连续归一化流（CNFs）的方法，显著提高了分子系统的采样效率。


<details>
  <summary>Details</summary>
Motivation: 探索如何通过路径梯度进一步优化Flow Matching训练的CNFs，以提升采样效率。

Method: 使用路径梯度对Flow Matching初始训练的CNFs进行微调，结合目标能量信息。

Result: 实验显示该方法将采样效率提高至三倍，同时保持模型和计算预算不变。

Conclusion: 路径梯度微调不仅高效，还能保留流的已学习结构。
...
### [293] [One-Stage Top-$k$ Learning-to-Defer: Score-Based Surrogates with Theoretical Guarantees](https://arxiv.org/abs/2505.10160)
*Yannis Montreuil,Axel Carlier,Lai Xing Ng,Wei Tsang Ooi*

Main category: stat.ML

TL;DR: 论文提出了一种单阶段Top-$k$学习延迟框架，通过共享的基于分数的模型选择最经济的实体或专家，统一了预测和延迟任务。


<details>
  <summary>Details</summary>
Motivation: 现有单阶段L2D方法仅能延迟到单个专家，限制了灵活性。本文旨在通过联合优化预测和延迟，支持多实体选择。

Method: 定义了一种成本敏感损失，并推导出与$k$无关的凸代理，支持跨Top-$k$泛化。还提出了动态选择实体数量的自适应变体Top-$k(x)$。

Result: 实验表明，Top-$k$方法优于Top-1延迟，Top-$k(x)$能根据输入复杂度动态平衡准确性和成本。

Conclusion: 该方法在统一预测和延迟的同时，实现了更高的灵活性和性能。
...
### [294] [Efficient MCMC Sampling with Expensive-to-Compute and Irregular Likelihoods](https://arxiv.org/abs/2505.10448)
*Conor Rosato,Harvinder Lehal,Simon Maskell,Lee Devlin,Malcolm Strens*

Main category: stat.ML

TL;DR: 论文提出了一种改进的MCMC采样方法，利用子集评估和数据驱动代理降低计算成本，并在疾病建模任务中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 由于似然函数不规则且计算成本高，传统的贝叶斯推断方法（如MCMC）效率低下。

Method: 采用子集采样器和数据驱动代理替代泰勒展开，并引入计算成本感知的自适应控制器。

Result: 改进的HINTS方法在固定计算预算下实现了最佳采样误差。
...
### [295] [FlowVAT: Normalizing Flow Variational Inference with Affine-Invariant Tempering](https://arxiv.org/abs/2505.10466)
*Juehang Qin,Shixiao Liang,Christopher Tunnell*

Main category: stat.ML

TL;DR: FlowVAT是一种基于条件调温的变分推断方法，用于解决多模态高维后验分布中的模式塌缩问题，无需复杂的超参数调优和退火计划。


<details>
  <summary>Details</summary>
Motivation: 传统退火方法需要温度计划和超参数调优，无法实现真正的黑盒变分推断。多模态高维后验分布会导致模式塌缩和模式搜索行为。

Method: FlowVAT通过同时调温基分布和目标分布，保持调温下的仿射不变性，并利用过参数化神经网络的泛化能力训练单一流。

Result: 在2、10和20维多模态分布实验中，FlowVAT优于传统和自适应退火方法，发现更多模式并获得更好的ELBO值。

Conclusion: FlowVAT无需退火计划和超参数调优，为复杂后验分布提供了全自动黑盒变分推断的解决方案。
...
### [296] [Batched Nonparametric Bandits via k-Nearest Neighbor UCB](https://arxiv.org/abs/2505.10498)
*Sakshi Arya*

Main category: stat.ML

TL;DR: 论文提出了一种非参数算法BaNk-UCB，用于批量非参数上下文赌博机问题，结合k-NN回归和UCB原则，在有限反馈场景下表现优异。


<details>
  <summary>Details</summary>
Motivation: 受医学和营销等领域中在线反馈有限的约束启发，研究批量非参数上下文赌博机问题。

Method: 提出BaNk-UCB算法，结合自适应k-NN回归和UCB原则，利用局部几何估计奖励并平衡探索与利用。

Result: 在标准Lipschitz平滑和边界假设下，提供接近最优的遗憾保证，并在合成和真实数据集上优于基线方法。

Conclusion: BaNk-UCB是一种简单高效的非参数方法，适用于有限反馈场景，表现优于传统方法。
...
### [297] [Multi-Agent Path Finding For Large Agents Is Intractable](https://arxiv.org/abs/2505.10387)
*Artem Agafonov,Konstantin Yakovlev*

Main category: cs.MA

TL;DR: 论文研究了多智能体路径规划（MAPF）问题中考虑智能体尺寸时的复杂性，首次证明该问题是NP难的。


<details>
  <summary>Details</summary>
Motivation: 实际应用中（如机器人领域），智能体尺寸不可忽略，但此前未明确考虑尺寸时的MAPF问题复杂度。

Method: 通过将经典的3SAT问题（NP完全问题）归约到MAPF问题，构造特定图实例并证明其等价性。

Result: 首次证明考虑智能体尺寸的MAPF问题是NP难的，表明不存在多项式时间算法（假设P≠NP）。

Conclusion: 研究填补了理论空白，为实际应用中的路径规划提供了复杂性依据。
...
### [298] [On the Well-Posedness of Green's Function Reconstruction via the Kirchhoff-Helmholtz Equation for One-Speed Neutron Diffusion](https://arxiv.org/abs/2505.09766)
*Roberto Ponciroli*

Main category: math.NA

TL;DR: 提出了一种利用堆外探测器实时测量数据重建核反应堆中子通量空间分布的方法，基于Kirchhoff-Helmholtz方程和数值求解格林函数。


<details>
  <summary>Details</summary>
Motivation: 解决复杂异质域（如核反应堆）中格林函数解析解难以获取的问题，验证数据驱动方法的适定性。

Method: 从单速中子扩散模型推导K-H方程，通过数值方法近似格林函数，并设计中子通量重建算法。

Result: 证明了从采样数据推断的格林函数的存在性和唯一性，验证了方法的可靠性。

Conclusion: 该方法为核反应堆中子通量分布的重建提供了可靠的数据驱动解决方案。
