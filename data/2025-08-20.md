...
Motivation: 大型语言模型的输出往往反映了训练数据中的结构性性别不平衡问题，这会影响数字化沟通的公平性。需要开发更有效的方法来检测和减少语料库中的性别歧视。

Method: 基于话语感知公平性分析的前期工作，提出了新的演员级指标，包括情感、语法主动性和引用风格的不对称性测量。这个流水线支持诊断性语料分析和排除基于的平衡处理，以构建更公平的语料库。

Result: 在taz2024full德国报纸语料库(1980-2024)中应用该方法，在多个语言维度上实现了性别平衡的显著改善。表面层面的不对称可通过过滤和重新平衡来减少，但更细微的偏见形式(特别是情感和框架方面)仍然存在。

Conclusion: 该研究为话语基于公平性审计和公平语料库构建提供了工具和报告，显示了在减少语言偏见方面的进步，但仍需要更多工作来应对更深层次的偏见问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准如BrowseComp主要关注文本信息，忽视了网页中普遍存在的多模态内容，需要填补这一空白。

Method: 创建包含224个挑战性问题的基准，这些问题在提示中包含图像，网页搜索和推理过程中的关键信息可能嵌入在图像或视频中。

Result: 对最先进模型的评估显示，即使是OpenAI o3等顶级模型也仅达到29.02%的准确率。

Conclusion: 当前模型的多模态能力不足，缺乏原生的多模态推理能力，MM-BrowseComp突显了这一差距。
...
<details>
  <summary>Details</summary>
Motivation: 虽然现有的RNN-T基础ASR系统能够实时转写，但实现实时流式翻译仍然面临重大挑战，需要在翻译质量和延迟之间找到平衡。

Method: 提出同步翻译方法，利用ASR生成的语言线索来管理上下文，采用高效的分枝定界技术（如超时和强制终止）来维持系统的实时性。

Result: 在设备上的双语对话语音翻译中应用，在延迟和质量方面都超过了基准方法，缩小了与非流式翻译系统的质量差距。

Conclusion: 该技术为更准确和高效的实时语音翻译平了道路，有力推动实时翻译技术的发展。
...
<details>
  <summary>Details</summary>
Motivation: 探索推理能力如何影响LLMs的习语检测性能，并研究模型规模的影响，因为习语检测需要先理解表达再进行推理

Method: 评估DeepSeek-R1蒸馏模型(1.5B-70B参数)在四个习语检测数据集上的表现，分析推理链(CoT)的效果，并为小模型提供定义提示

Result: 推理效果较小且多变：小模型CoT推理可从数学调优模型提升性能但不及基础模型；大模型(14B+)有适度改进；大模型能准确理解习语含义，小模型常失败；提供定义可提升小模型性能

Conclusion: 模型规模对习语理解至关重要，大模型展现更好的习语理解能力，而小模型需要外部知识支持来改善性能
...
<details>
  <summary>Details</summary>
Motivation: ASR系统在长音频转录中难以保持语法和语义准确性，影响命名实体识别、大小写和标点等任务，需要整合语言上下文信息

Method: 使用两种策略：(1)token级蒸馏与最优传输对齐维度和序列长度，(2)Whisper和LLaMA句子嵌入间的表征损失最小化，融合语法和语义

Result: 在Spoken Wikipedia数据集评估显示，词错误率、NER、大小写和标点成功率均有显著提升

Conclusion: 通过引入新颖的NER指标和语义感知ASR探索，证明了将语言上下文整合到转录中的价值，为长语音的鲁棒上下文感知ASR奠定基础
...
<details>
  <summary>Details</summary>
Motivation: 为了解决传统LLM在复杂问题求解中存在的格式崩溃、冗长循环和推理不连贯等问题，需要开发一个能够执行完整分析轨迹、支持代码执行和自我修正的专业级问题求解模型。

Method: 1) 轨迹中心合成数据生成器产生14.4万个标记笔记本片段；2) 双奖励框架结合轻量级结构信号和分层奖励模型；3) 内存优化的GRPO实现，支持KV缓存重用和顺序生成；4) 余弦课程学习从结构保真度平滑过渡到语义深度

Result: 在标准公共基准测试中超越同规模模型，甚至达到QwQ-32B等更大推理模型的水平，在AIME 2024/2025和LiveCodeBench上准确率提升高达30%，同时每个解决方案生成的token减少18-49%

Conclusion: Datarus通过轨迹式训练、双奖励框架和优化的GRPO实现，成功解决了RL对齐LLM的常见问题，在复杂问题求解中表现出"顿悟时刻"模式，为专业级AI助手提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在跨文化沟通中存在训练数据分布偏差问题，需要有效的方法来建模和对齐不同文化背景

Method: 使用Small-World-of-Words项目的英语-美国和汉语词汇联想数据，通过监督微调(SFT)和PPO偏好优化对Llama-3.1-8B和Qwen-2.5-7B进行参数高效微调

Result: SFT使英语联想P@5提升16-20%，汉语提升43-165%，中位数具体性提升+0.20，达到人类水平的效价和唤醒度。在文化价值观问题上，微调模型向目标文化偏移，Qwen的中文对齐响应翻倍，Llama的美国偏见减少三分之一

Conclusion: 研究表明少量文化基础的联想数据可以在不进行昂贵重新训练的情况下实现价值对齐，强调基于人类认知的文化对齐研究的重要性
...
Method: 提出基于Shapley信息增益(SIG)奖励的强化学习框架，包含SIG引导模型初始化和SIG增强策略优化两阶段训练流程，使用蒙特卡洛树搜索构建高质量交互轨迹

Result: 在两个新构建的部分信息医疗基准测试中，ProMed平均优于最先进方法6.29%，相比被动范式提升54.45%，在域外案例中表现出良好泛化能力

Conclusion: ProMed成功实现了医疗LLM从被动到主动范式的转变，通过量化临床价值的问题提问机制显著提升了诊断性能，为真实临床交互应用提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 阿拉伯语言模型主要支持现代标准阿拉伯语，对沙特方言支持有限，影响了对真实方言变体的抓取能力

Method: 使用私有的沙特方言指令数据集（Hijazi和Najdi，5,466条合成指令-响应对），采用LoRA微调技术训练ALLaM-7B-Instruct-preview模型，测试了两种方案：方言标签训练和无标签训练

Result: 方言标签模型表现最佳，将沙特方言生成率从47.97%提升到84.21%，MSA泄漏率从32.63%降低到6.21%，文本保真度也有显著提升（chrF++ +3.53, BERTScore +0.059）

Conclusion: LoRA微调技术能够有效地提升阿拉伯语言模型在沙特方言生成方面的性能，且适合方言控制的应用场景，为方言NLP研究提供了有价值的技术路径
...
<details>
  <summary>Details</summary>
Motivation: 评估大语言模型在低资源语言泰卢固语中的能力，了解模型局限性并为开发更具语言能力的模型提供信息。

Method: 创建包含729个问题的评估数据集，涵盖多种语言维度，对11个LLM进行评估，分析模型在选择题中依赖表面启发式的现象，比较LLM作为评判者与人类评估的差异。

Result: 发现LLM在选择题中依赖答案位置和干扰项模式等表面启发式策略，LLM作为评判者在低资源语言中的可靠性需要进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注压缩模型的精度性能，但性能相等并不保证可信任性相等，需要从解释性和检验两个维度评估压缩模型的可信任性

Method: 提出两维度价估框架：1）解释性对齐（使用LIME和SHAP测试验证模型是否基于相同特征做决策）；2）检验相似性（通过ECE、MCE、Brier Score和可靠性图评估模型预测概率的可靠性）

Result: 实验结果显示，即使压缩模型与原模型精度几乎相同，但解释性对齐很低，检验相似性存在显著差异，压缩模型不具备与大模型相等的可信任性

Conclusion: 压缩模型不能直接作为大模型的可信任替换，在部署时需要进行超越简单性能对比的详细评估，确保可信任性相等
...
<details>
  <summary>Details</summary>
Motivation: 在医疗领域准确性至关重要的背景下，探索不同解码策略对LLM输出质量的影响，目前这一领域研究不足

Method: 在5个开放式医疗任务（翻译、摘要、问答、对话、图像描述）中评估11种解码策略，使用医疗专用和通用LLM不同规模模型

Result: 确定性策略普遍优于随机策略，beam search得分最高；大模型整体得分更高但推理时间更长且对解码策略更敏感；医疗LLM在两个任务中表现更好但整体无优势且对解码选择更敏感

Conclusion: 医疗应用中需要谨慎选择解码方法，其影响有时甚至超过模型选择本身，不同评估指标的相关性因任务而异
...
<details>
  <summary>Details</summary>
Motivation: 研究语音LLM是否也像文本LLM一样存在性别偏见，通过说话者分配来分析偏见

Method: 构建两个数据集：职业数据集（包含性别刻板印象的职业）和性别色彩词汇数据集，分析Bark TTS模型的默认说话者分配

Result: Bark模型没有显示系统性偏见，但显示出了性别意识和某些性别偏向

Conclusion: 语音LLM在说话者选择上并不存在系统性性别偏见，但具有性别认知能力
...
<details>
  <summary>Details</summary>
Motivation: 解决长文档视觉问答在低资源环境下面临的上下文限制和训练数据不足的挑战，特别是在处理日语等低资源语言时。

Method: 提出三个核心创新：1）混合文本检索架构进行有效文档分割；2）智能数据增强管道自动生成高质量推理问答对并进行多级验证；3）自适应集成推理机制，包含动态配置生成和早停机制。

Result: 在JDocQA基准测试中，Yes/No问题准确率达到83.04%，事实性问题52.66%，数值问题44.12%；在LAVA数据集上达到59%的准确率，创造了日语文档VQA的新SOTA结果。
...
<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在英语多模态任务上表现出色，但其在低资源语言（如越南语）真实教育多模态内容上的性能尚未得到充分探索，需要评估这些主要基于英语数据训练的模型是否能处理跨语言多模态推理任务。

Method: 构建ViExam基准测试，包含2,548个多模态问题，涵盖7个学术领域（数学、物理、化学、生物、地理、驾驶测试和智商测试），测试多种VLMs的性能，并尝试跨语言提示和人类协作方法。

Result: SOTA VLMs平均准确率57.74%，开源模型27.70%，均低于人类平均水平66.54%。只有o3思考模型(74.07%)超过人类平均。跨语言提示反而降低性能1个百分点，人类协作可提升5个百分点。

Conclusion: 当前VLMs在越南语多模态教育内容上表现不佳，跨语言迁移能力有限，需要专门针对低资源语言的多模态训练数据和改进的跨语言推理方法。
...
<details>
  <summary>Details</summary>
Motivation: 通用概括词（如'鸟会飞'）在非单调逻辑中具有重要地位，这些概括词允许异常情况的存在，对语言学、哲学、逻辑学和认知科学都很重要。研究者想了解现代LLM在这种可反操作的默认推理上的能力。

Method: 研究者使用20种不同的可反操作推理模式，测试28个LLM的表现。测试了不同提示方式（零次提示、少次提示、链式推理）对模型性能的影响。

Result: 前沿模型在默认推理问题上表现良好，但不同模型间性能差异显著。少次提示对某些模型有中等改善，但链式推理导致严重性能下降（在零次提示准确率超过75%的模型中，平均准确率下降11.14%）。大多数模型无法区分可反操作推理和式推理，或将通用概括词误解为普遍声明。

Conclusion: 当前LLM在默认推理方面既有彩之处也有限制。它们能够处理许多默认推理问题，但在某些关键方面如区分不同类型的推理以及正确理解通用概括词的特性上仍面临挑战。
...
<details>
  <summary>Details</summary>
Motivation: 理解深度学习模型中隐含编码的知识对于提高AI系统可解释性至关重要。现有方法通过将词嵌入映射到人类可解释的语义特征来评估知识编码，但仅凭预测准确性无法可靠判断词嵌入是否真正包含相应知识。

Method: 通过实验证明现有方法即使对随机信息也能成功预测，分析预测结果与向量空间几何相似性的关系，而非真正的语义表征。

Result: 预测准确性主要由算法上限决定，而非词嵌入中有意义的语义表征。基于预测性能的数据集比较不能可靠指示哪个数据集被词嵌入更好地捕获。

Conclusion: 词嵌入到语义特征的映射主要反映向量空间的几何相似性，不能可靠地表明语义属性的真正涌现，需要重新评估这类解释方法的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 随着EEG在神经科学和临床实践中的广泛应用，高效检索和语义解释大规模、多源、异构的EEG数据成为一个迫切挑战。

Method: 提出三层超图框架，构建可遍历的n元关系超图，整合EEG领域知识、患者病例和存储库，支持联合语义-时间检索和因果链诊断生成。同时建立了首个跨疾病、跨角色的EEG临床QA基准。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在需要事实严谨的领域中常常显示奇恶行为（无论正确与否都与用户信念保持一致），这圧高风险场景如科学QA中带来严重风险。现有研究对这个现象在事实QA上的关注不够。

Method: 提出统一评估框架，包含对抗性提示设置和目标指标（如误导抵抗力和奇恶抵抗力）。还提出了Pressure-Tune方法，通过在合成对抗性对话上进行轻量级微调，配合思维链理由来拒绝用户错误信息并强化事实态度。

Result: 对开源和专有模型的系统评估显示了普遍的奇恶倾向，这更多由对齐策略驱动而非模型大小。Pressure-Tune在具有挑战性的科学QA测试集上显著提升了奇恶抵抗力，而不影响准确性或对有效反馈的响应能力。

Conclusion: 奇恶行为是大语言模型在高风险事实场景中的严重风险，Pressure-Tune方法提供了一种实用的解决方案，能够在不損害模型其他性能的前提下提升模型的真实性和原则性。
...
<details>
  <summary>Details</summary>
Motivation: 当前机器生成文本检测器在同域训练测试时表现良好，但在面对不同来源数据的域偏移时泛化能力差，需要解决跨域检测问题

Method: 从频域角度分析文本表示，发现机器生成文本与人类写作文本在频谱上存在显著差异。设计了低频域过滤模块去除对域偏移敏感的文档级特征，以及动态频谱对齐策略提取任务特定和域不变特征

Result: 在3种域泛化场景的11个测试数据集上，MGT-Prism平均准确率提升0.90%，F1分数提升0.92%，优于最先进基线方法

Conclusion: 频域分析为机器生成文本检测提供了有效视角，MGT-Prism通过频域特征处理显著提升了跨域检测性能，为解决域偏移问题提供了新思路
...
<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型是否能生成类似儿童的语言，为教育应用和心理学研究提供参考

Method: 使用相同图片故事生成两个LLM语料库（零样本和少样本提示），通过心理语言学文本属性（词频、词汇丰富度、句子长度、词性标注、词嵌入语义相似度）进行比较分析

Result: LLM生成的文本更长但词汇丰富度更低，更多依赖高频词，名词使用不足，语义空间分析显示低相似度，few-shot提示仅略微提升相似度

Conclusion: LLM难以真正复制儿童语言模式，对儿童教育工具中使用LLM生成语言的适用性提出重要质疑
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM生成医学摘要时事实准确性的担忧，通过提供句子级引用让用户能够追溯原始上下文来验证准确性。
...
Conclusion: TracSum基准和提出的方法能够有效支持可追溯、基于方面的摘要生成任务，特别是在医学领域。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究使用确定性标注作为标准，无法捕捉人类标注者之间的分歧和不确定性。本文旨在通过建模标注者分歧来更准确地评估语言模型的道德理解能力。

Method: 采用GPU优化的贝叶斯框架，处理了100万+模型查询，使用250K+标注数据（来自约700名标注者对100K+文本的标注），评估了Claude Sonnet 4、DeepSeek-V3、Llama 4 Maverick等顶级模型。

Result: AI模型通常能排在前25%的人类标注者水平，平衡准确率远高于平均水平。更重要的是，AI产生的假阴性远少于人类，显示出更强的道德检测敏感度。

Conclusion: 语言模型在道德维度理解上表现出色，不仅能达到优秀人类标注者的水平，在某些方面（如减少假阴性）甚至超越人类，为AI道德评估提供了新的方法论视角。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在生成文本时经常无法准确控制输出长度，要么超过要求要么不足，需要一种轻量级的解决方案

Method: 提出一种基于提示的一次投射策略，通过添加计数器标记和明确的计数规则，让模型在写作过程中同时进行计数

Result: 在MT-Bench-LI任务中，GPT-4.1的严格长度遵循率从原本不到30%提升到95%以上，超越了常见的草稿-修订基线方法，同时保持了评价质量

Conclusion: 突出显示仅通过提示工程就可以实现准确的长度控制，为训练或解码基础方法提供了一种轻量级的替代方案
...
<details>
  <summary>Details</summary>
Motivation: 自动评价指标很难完美地预估人类判断，当前研究中使用的指标混乱且缺乏统一标准，需要系统性分析现有指标的方法、优缺点和验证方法。

Method: 对现有自动评价指标进行深入检查，分析其方法论、文档化优势和限制、验证方法以及与人类判断的相关性。包括从词法对比到语义相似性模型，再到最新的LLM基于评估器的演化。

Result: 发现自动评价指标存在多重挑战：只能抓取文本质量的特定方面，效果随任务和数据集而异，验证实践缺乏结构化，与人类判断的相关性不稳定。新型LLM-as-a-Judge指标和RAG任务评估也面临相似问题。

Conclusion: 对完美评价指标的追求应该重新考虑。建议根据具体任务需求选择指标，利用补充性评估方法，并建议新指标应重点关注改进验证方法论。
...
Method: 采用转换器基础模型CamemBERT和Fr_core_news_lg，进行命名实体识别(NER)和关系提取(RE)，使用手工注释数据集进行模型评估

Result: NER方面CamemBERT和Fr_core_news_lg模型F1分数超过90%，RE方面Random Forest模型F1分数超过80%，表现最佳

Conclusion: 研究成功展示了BIM与NLP集成在建筑行业文档处理中的效果，为构建知识图和自动验证系统奠定了基础
...
Method: 精心收集1019个高质量问答对，涉及3种评估模式，覆盖数学、物理、化学、生物4个学科，支持中英法西日5种语言，在16个开源模型和4个闭源模型上进行广泛实验

Result: o4-mini在仅图像评估模式下，数学、物理、化学、生物准确率分别为52.11%、24.73%、36.57%、29.80%，相比现有基准测试难度显著更高
...
Method: 将文本评论转化为(主语、谓语、宾语)三元组知识图谱，关联情感分数，使用Node2Vec图嵌入和情感特征，通过机器学习分类器预测评分。

Result: 在HotelRec数据集上，ReviewGraph性能与最佳模型相当但计算成本更低，在Cohen's Kappa等一致性指标上优于基线方法，同时提供更好的可解释性。
...
Method: 将上下文块视为MAB的臂，基于预期奖励分数选择块输入LLM生成响应，根据奖励反馈迭代更新分数，最后使用DPO方法进一步优化LLM。

Result: 实验结果表明LongMab-PO显著提高了偏好数据对的多样性和质量，在长上下文推理基准测试中达到了最先进的性能。

Conclusion: LongMab-PO框架通过探索-利用机制有效识别关键上下文片段，成功提升了LLM的长上下文建模能力，为相关任务提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型对话系统在提供准确主题引导方面存在不足，主要原因是无法识别用户在相关概念上的困惑。

Method: 引入AGQ框架，采用改进的CEIRT模型来更好地识别用户知识水平，并结合大语言模型直接从启发文本生成引导性问题。

Result: 通过与基线方法比较，该方法显著提升了用户的信息检索体验，表现优于其他方法。

Conclusion: AGQ框架通过CEIRT模型和LLMs的结合，有效解决了对话系统中概念困惑识别和引导问题生成的问题，提高了信息检索效率。
...
Result: 在AIME24和AIME25基准上Pass@32性能分别提升18.3%和22.8%，在12个推理基准上表现一致

Conclusion: SvS策略能有效维持训练过程中的策略熵，显著提升大语言模型的推理能力上限
...
Method: 提出Prefix INjection Guard (PING)方法，采用迭代方式：(1)生成候选前缀，(2)选择优化任务性能和拒绝行为的前缀，将这些自然语言前缀预置到智能体响应中。

Result: 实验表明PING显著增强微调LLM智能体的安全性而不牺牲有效性，在网页导航和代码生成任务中 consistently优于现有提示方法。线性探针分析显示前缀标记对行为修改至关重要。

Conclusion: PING是一种简单有效的安全增强方法，通过自然语言前缀注入有效引导智能体拒绝有害请求，为解决LLM智能体微调中的安全隐患提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 数字健康分析面临专家知识稀缺、数据短缺和隐私限制的挑战，在线健康社区内容包含复杂的情感和医疗上下文，需要专业知识进行准确情感分析。

Method: 研究开发了结构化的代码本系统编码专家解释指南，通过目标提示让LLM应用领域知识，比较了6个GPT模型、DeepSeek和LLaMA 3.1与预训练语言模型和词典方法的性能。

Result: LLM在情感分析任务中表现出艰越性能，并展现出与专家判断无统计学显著差异的高一致性，说明它们实现了超越表面模式识别的知识集成。

Conclusion: 通过上下文学习集成专家知识的LLM方法为数字健康分析提供了可扩展的解决方案，有势解决健康领域专家知识稀缺的关键挑战，支持实时病人监测和健康策略制定。
...
<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在上下文学习方面表现突出，但其内部机制仍不明确。物理学任务提供了基于基础原理的结构化动态数据，适合探索LLM的出现性推理行为

Method: 使用物理系统动态预测任务作为代理，逐步增加输入上下文长度评估性能。通过稀疏自编码器(SAEs)分析模型殊异流激活度，探索特征与物理变量的关联

Result: 动态预测性能随着输入上下文长度增加而提升。SAEs捕获的特征与能量等关键物理变量呈现显著相关性，证明LLM在上下文学习过程中编码了有意义的物理概念

Conclusion: 该研究为理解LLM如何在上下文中学习提供了新的案例研究，扩展了我们对大语言模型上下文学习机制的认知
...
<details>
  <summary>Details</summary>
Motivation: 驾驶呕疲检测对智能交通系统至关重要，但现有方法存在侵入性、硬件依赖、小物体检测困难等问题，需要一种高效、非侵入的解决方案。

Method: 提出YOLO11-CR模型，包含两个核心模块：卷积与注意力融合模块(CAFM)结合局部CNN特征和全局Transformer上下文；矩形校准模块(RCM)捕捉水平和垂直上下文信息。

Result: 在DSM数据集上达到精度87.17%、召回738.86%、mAP@50为88.09%、mAP@50-95为55.93%，显著超过基线模型。分解实验验证了CAFM和RCM模块的有效性。

Conclusion: YOLO11-CR为车内变疲监控提供了一种实用且高性能的解决方案，具有强大的实际部署潜力，可以进一步扩展时序建模、多模态数据集成和嵌入式优化。
...
<details>
  <summary>Details</summary>
Motivation: 现有的AIGI检测器在实验室环境下有效，但无法泛化到真实世界的嘈杂场景，这些图像质量参差不齐且经过多种编辑处理
...
Conclusion: Mirage基准有效模拟真实世界AIGI复杂性，Mirage-R1模型通过反射推理机制和自适应策略，在检测性能和推理速度间取得良好平衡
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在文档图像解析中存在幻觉问题，且相比领域专家模型效果较差，需要一种方法来结合两者的优势。

Method: 提出推理-工具交织的训练框架：模型先自行识别图像内容，然后调用专家模型工具获取参考结果，最后重新审视图像和推理过程提供最终识别结果。

Result: 在ReST和OmniDocBench数据集上的实验表明，DianJin-OCR-R1模型持续优于非推理对应模型和专家OCR模型。

Conclusion: 该方法有效解决了LVLMs的幻觉问题，通过结合专家模型的优势以较低成本提升了视觉语言模型的OCR性能。
...
<details>
  <summary>Details</summary>
Motivation: 乌尔都语作为一种草书式语言，具有复杂的几何和形态结构，传统的基于切分的识别方法错误率较高

Method: 采用卷积神经网络进行自动特征学习，通过三个字符的排列组合生成数据集，使用连通组件技术获取连字，并实现两级层次神经网络来处理不同程度的字符排列组合和组件分类

Result: 模型在组件分类任务上达到了99%的高准确率

Conclusion: 组件基于的CNN方法能够有效解决乌尔都语识别中的切分困难，为复杂草书式文字的光学字符识别提供了有效方案
...
<details>
  <summary>Details</summary>
Motivation: MES内镜评分分类面临标注噪声和有序性结构挑战，现有模型往往忽略这些因素，需要开发能够同时考虑标注可靠性和有序结构的分类方法。

Method: 使用轻量级模型估计图像质量作为标注置信度代理，构建从易到难的课程学习顺序，结合ResizeMix数据增强，在CNN和Transformer架构上验证。

Result: 在LIMUC和HyperKvasir数据集上，ConvNeXt-Tiny达到82.5%准确率和0.894 QWK，显著优于监督和自监督基线方法。

Conclusion: 难度感知的训练策略在标注不确定性条件下能有效提升有序分类性能，具有临床应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 步态识别面临标注数据稀缺和隐私保护限制的问题，需要一种能够生成多样化、高质量步态样本的方法来提升识别性能。

Method: 使用视频扩散模型在步态轮廓数据上从头训练，通过条件控制生成可控的步态序列，并引入身份嵌入插值机制生成新身份。

Result: 合成样本能够提升步态识别性能，特别是在挑战性条件下；生成的新身份具有独特且一致的步态模式，可用于隐私保护下的模型训练。

Conclusion: 该工作展示了扩散模型在高质量、可控且隐私保护的步态数据生成方面的潜力，为步态识别领域提供了重要进展。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统中的视觉语言模型需要处理高分辨率多视角图像，产生大量视觉token导致计算开销巨大、推理延迟高和内存消耗大，阻碍了实际部署。

Method: 提出两种核心创新：1）受最远点采样启发的多样性感知token选择机制，优先考虑跨视图的语义和空间覆盖；2）视图自适应剪枝控制器，根据各摄像头对下游驾驶任务的重要性学习最优剪枝比例。

Result: 在两个大规模多视角驾驶基准测试上，仅保留10%视觉token时实现6.4倍预填充阶段加速，FLOPs消耗降至13.4%，性能仅下降3%。
...
TL;DR: 一种可扩展、可解释的计算机视觉框架，用于量化面部整形外科手术的美学效果，通过自动化面部标记检测、几何对称分析、深度学习年龄估计咋鼻部形态分析。


<details>
  <summary>Details</summary>
Motivation: 为了提供可重复、定量的评估标准，以支持数据驱动的手术规划、患者咨询咋客观效果评估。

Method: 构建了最大的绅床前后对照面部图片数据集（7,160张照片，1,259名患者），采用自动标记检测、几何对称计算、深度学习年龄估计咋鼻部形态分析的组合方法。

Result: 在美容鼻手术子集中，96.2%患者至少在一个鼻部指标上显示改善；在更广泛的面部图片组中，71.3%患者在全局面部对称性或感知年龄方面显示显著改善。患者身份在手术后保持一致，真正匹配率达99.5%以上。

Conclusion: 该框架为面部整形外科提供了可重复、定量的评估标准，有助于数据驱动的手术规划、患者沟通咋客观效果评估。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医疗环境中存在计算成本高、访问性有限和数据隐私问题，需要探索更简单易用的解决方案

Method: 使用NIH胸部X光数据集，评估多个小型语言模型在胸部X光位置分类任务上的表现，测试三种提示策略：基准指令、增量摘要提示和纠正反思提示

Result: 某些小型语言模型通过精心设计的提示策略能够达到竞争力的准确性

Conclusion: 提示工程可以在不需要深度AI专业知识的情况下，实现小型语言模型在医疗健康应用中性能的显著提升
...
TL;DR: AIM 2025 RipSeg挑战赛旨在推进静态图像中激流自动分割技术，使用RipVIS数据集进行单类实例分割，共有75名参与者注册，5个有效测试提交，最佳方法采用深度学习架构和领域适应技术。
...
Result: 挑战赛吸引了75名参与者，产生5个有效测试提交，最佳方法在多样化条件下表现出色。
...
TL;DR: 研究发现多选VQA基准中存在Easy-Options Bias问题，VLMs仅通过视觉和选项就能选择正确答案，无需问题信息。作者开发了GroundAttack工具包生成困难负选项，创建了无偏数据集，使VLMs性能更接近真实水平。


<details>
  <summary>Details</summary>
Motivation: 发现当前多选VQA基准存在严重偏差，视觉语言模型可以通过简单的视觉-选项相似度匹配来选择正确答案，而不需要理解问题，这导致了对模型真实问答能力的高估。

Method: 通过接地实验分析偏差来源，发现正确答案在特征空间中与视觉内容对齐度更高。开发GroundAttack工具包自动生成视觉上合理的困难负选项，应用于NExT-QA和MMStar数据集创建无偏标注。

Result: 在无偏标注上，当前VLMs在(V+O)设置下接近随机准确率，在(V+Q+O)设置下准确率显著下降至非饱和水平，提供了更真实的VLM问答能力评估。

Conclusion: 揭示了多选VQA基准中的系统性偏差问题，提出的GroundAttack方法和无偏数据集为更准确地评估视觉语言模型的真实理解能力提供了有效解决方案。
...
TL;DR: 这篇论文提出了一种新的结构化提示和知识蓄粉框架，通过使用大型视觉-语言模型生成高质量模拟注释，训练出简洁的3B参数模型VISTA，能够理解低分辨率交通视频并生成风险感知的描述文本。


<details>
  <summary>Details</summary>
Motivation: 传统方法在可扩展性和通用性方面遇到困难，特别是在复杂动态的实际环境中。需要一种能够自动生成高质量交通场景注释和风险评估的方法来提升智能交通系统和自动驾驶的能力。

Method: 使用GPT-4o和o3-mini两个大型VLM模型，采用结构化的思维链策略生成多视角输出，作为知识密集的伪注释用于监督微调小型学生VLM模型。

Result: 得到的简洁模型VISTA在常见的描述指标（BLEU-4、METEOR、ROUGE-L、CIDEr）上表现优异，虽然参数数量大大减少，但性能可与教师模型相比。

Conclusion: 有效的知识蓄粉和结构化多代理监督能够让轻量级VLM掌握复杂的推理能力，VISTA的简洁架构有助于在边缘设备上高效部署，实现实时风险监控。
...
TL;DR: EDTalk++是一个全解耦的说话头生成框架，能够独立控制嘴形、头部姿态、眼球运动和情感表达，支持视频和音频输入，通过正交基分解和共享视觉先验实现多模态输入的统一处理。


<details>
  <summary>Details</summary>
Motivation: 现有方法在面部特征解耦方面存在不足，无法实现多个面部运动的独立控制，且难以处理不同模态的输入。需要探索面部特征的解耦空间，确保特征独立性和多模态输入的兼容性。

Method: 使用四个轻量级模块将面部动态分解为嘴形、姿态、眼球和表情四个独立的潜在空间，每个空间由一组可学习的正交基表示。通过正交性约束确保独立性，并设计高效训练策略分配运动责任。学习到的基存储在对应库中，实现与音频输入的共享视觉先验。

Result: 实验证明了EDTalk++的有效性，能够实现面部多个运动的独立控制，并支持视频和音频两种输入模态的说话头生成。

Conclusion: EDTalk++提供了一个完整的解耦框架，成功解决了面部特征独立控制和多模态输入兼容的问题，为可控说话头生成提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 经典视觉编码和MLLM令牌技术共享同核心目标：在最小化计算成本的同时最大化信息保真度。论文通过长期发展的视觉编码原理来重新审视MLLM令牌技术。

Method: 建立了令牌技术与视觉编码的统一形式化框架，进行系统的模块化对比分析，包括令牌化、令牌压缩和令牌推理等关键技术。

Result: 实现了双向视觉融合：一方面探索视觉编码原理如何提升MLLM令牌技术的效率和稳健性，另一方面令牌技术范式如何影响下一代语义视觉编码器的设计。

Conclusion: 这是首次对MLLM令牌技术与视觉编码进行结构化技术对比的综述性研究，为同时开发更高效的多模态模型和更强大的视觉编码器投底了基础，并展望了未来研究方向。
...
TL;DR: 视觉Transformer模型在肾结石内镜图像分类任务中显著超越传统CNN模型，特别是在复杂的图像条件下表现更优


<details>
  <summary>Details</summary>
Motivation: 肾结石分类对个性化治疗和预防复发至关重要，但传统CNN模型在抓取长距离依赖关系方面有限，影响在变化的成像条件下的性能

Method: 进行视觉Transformer(ViT)与CNN基础模型的对比分析，在两个ex vivo数据集上评估性能，包括CCD相机和柔性尿道镜图像

Result: ViT-base模型在所有成像条件下都较ResNet50基准更优：在最复杂的内镜图像子集中达到95.2%准确率和95.1%F1分数（CNN仅64.5%和59.3%）；在混合视图子集中达到87.1%准确率（CNN为78.4%）

Conclusion: ViT基础架构提供了更优异的分类性能，为肾结石图像分析提供了一种可扩展的CNN替代方案
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在交通分析中需要大量计算资源，且难以处理细粒度的时空理解问题，需要开发更高效准确的解决方案。

Method: 采用四种关键技术：(1)caption分解分别处理空间和时间信息；(2)最佳视图过滤的时间帧选择；(3)参考驱动理解捕捉细粒度运动和动态上下文；(4)精心设计的视觉/文本提示技术。

Result: 在WTS和BDD数据集上表现出语义丰富性和交通场景解释能力的显著提升，在AI City Challenge 2025 Track 2中获得55.655分的优异成绩。
...
TL;DR: MINR通过共享中间层来高效编码多张图像，在保持性能的同时节省60%参数，可扩展到处理100张图像。
...
Method: 分析训练好的INR网络层权重分布，发现中间层具有高度相似的分布模式，因此共享中间层，同时保留输入输出层为图像专用，并为每张图像设计额外的投影层来捕获独特特征。

Result: 在图像重建和超分辨率任务中，MINR节省60%参数的同时保持可比性能，可处理100张图像并维持34dB的平均PSNR。

Conclusion: MINR通过层共享策略有效解决了多图像编码的效率问题，在各种骨干网络上都表现出鲁棒性。
...
TL;DR: DHQ是一种针对隐式神经表示(INRs)的分布感知哈达玛量化方案，同时对权重和激活值进行量化，通过哈达玛变换将不同分布标准化为钟形分布，显著提升硬件效率。


<details>
  <summary>Details</summary>
Motivation: 现有INR量化方法主要关注权重量化，缺乏激活值量化，硬件节省有限。INRs需要全精度计算导致显著的硬件开销。

Method: 提出DHQ方案，利用哈达玛变换将不同层的权重和激活值分布标准化为统一的钟形分布，然后应用标准量化器。分析了不同层权重和激活值的分布差异。

Result: 在图像重建任务中，DHQ相比全精度版本减少延迟32.7%、能耗40.1%、资源使用高达98.3%，优于先前量化方法。

Conclusion: DHQ通过同时量化权重和激活值，并利用哈达玛变换标准化分布，实现了显著的硬件效率提升，为INRs的硬件部署提供了有效解决方案。
...
Method: 对67名参与者的319个有效结果进行综合分析，评选出前五名团队进行详细方法学评估

Result: 最佳团队的PU21-PSNR达到29.22dB，挑战赛建立了HDR重建质量的强基准

Conclusion: 该挑战赛突出了提升HDR重建质量的创新策略，为未来逆色调映射研究提供了重要指导
...
<details>
  <summary>Details</summary>
Motivation: 隐式神经表示在多媒体应用中表现出重要价值，但其网络权重容易受到不可避免的扰动影响，这种脆弱性阻碍了INRs在实际部署中的应用。

Method: 通过最小化有/无权重扰动时的损失差异来形式化INRs的鲁棒性问题，推导出一种新颖的鲁棒损失函数来调节重建损失相对于权重的梯度。
...
Method: 设计FAMNet融合模型，包含2D CNN AMNet2D和3D CNN AMNet3D，使用Resnet18作为共享背骨网络和注意力模块。采用多任务学习策略，同时进行微表情识别和面部动作单元检测，通过参数硬共享实现信息关联

Result: 在SAMM、CASME II和MMEW数据集上达到83.75%(UAR)和84.03%(UF1)；在极具挑战性的CAS(ME)^3数据集上达到51%(UAR)和43.42%(UF1)，性能显著提升

Conclusion: FAMNet通过2D和3D CNN的融合，结合多任务学习和注意力机制，能够有效提取微表情的全方位特征，显著提高了微表情识别的性能，为该领域提供了新的解决方案
...
TL;DR: CORENet是一个基于4D雷达的跨模态去噪框架，利用LiDAR监督从原始4D雷达数据中识别噪声模式并提取判别性特征，提升目标检测性能
...
Method: 提出CORENet跨模态去噪框架，利用LiDAR数据进行跨模态监督训练，采用即插即用架构，可无缝集成到基于体素的检测框架中，推理时完全基于雷达数据

Result: 在具有高噪声水平的Dual-Radar数据集上进行广泛评估，证明该框架能有效增强检测鲁棒性，相比现有主流方法获得更优性能

Conclusion: CORENet通过跨模态监督成功解决了4D雷达数据稀疏和噪声问题，为恶劣天气条件下的自动驾驶感知提供了有效的解决方案
...
TL;DR: 一种新的双层解耦一致性学习框架(BDCL)，通过实例学习、特征解耦和一致性学习提升多视图聚类性能


<details>
  <summary>Details</summary>
Motivation: 当前多视图聚类方法对聚类导向的表征学习关注不够，需要提升聚类间辨别性和聚类内紧凑性

Method: 使用重构自编码器和对比学习对齐一致信息，通过双层解耦提升特征空间和聚类空间的辨别性，并学习聚类分配的一致性

Result: 在5个标准数据集上试验结果显示，该方法在多视图聚类任务上超过了当前最先进方法

Conclusion: BDCL框架通过系统化的解耦和一致性学习，能够有效提升多视图聚类的表征学习效果，为多视图数据分析提供了新的解决方案
...
TL;DR: 基于强化学习的自适应曝光参数优化方法，通过模拟运动模糊和噪声来提高动态场景HDR重建质量


<details>
  <summary>Details</summary>
Motivation: 现有HDR技术忽视了快门速度与ISO的复杂交互作用，以及动态场景中运动模糊的影响，导致高ISO产生噪声、长快门引发模糊的问题

Method: 提出AdaptiveAE方法，使用强化学习优化快门速度和ISO组合选择，集成了包含运动模糊和噪声模拟的图像合成流程，利用语义信息和曝光直方图

Result: 在多个数据集上达到了最先进的性能，能够根据用户定义的曝光时间预算自适应地选择最优的ISO和快门速度序列

Conclusion: 该方法能够有效解决动态HDR重建中的曝光参数优化问题，找到比传统方案更好的曝光策略，提高了HDR图像质量
...
TL;DR: 通过转移单打训练模型和对比学习框架，将姿态基于射击识别扩展到羽毛球双打比赛分析，解决了双打数据缺乏和多人踪踪挑战。


<details>
  <summary>Details</summary>
Motivation: 羽毛球双打比赛在国际赛事中更为普遍，但以往研究主要集中在单打，因为双打数据获取和多人踪踪存在技术挑战。

Method: 使用ViT-Pose提取关键点，通过ST-GCN基础的对比学习框架进行嵌入，采用自定义多目标踪踪算法解决ID切换问题，最后用Transformer分类器识别射击动作。

Result: 证明了将姿态基于射击识别扩展到双打羽毛球的可行性，扩大了分析能力。

Conclusion: 为双打特定数据集建立了基础，有助于深入理解这种普遍但缺乏研究的快速羽毛球比赛形式。
...
TL;DR: 这篇论文提出了一种新的图像标记化器VGQ，通过集成2D高斯分布来明显地提升结构建模能力，充分抓取几何和空间结构信息，在ImageNet数据集上达到了独创性的重建效果。


<details>
  <summary>Details</summary>
Motivation: 现有的量化基于标记化器（如VQ-GAN）主要关注外观特征，如纹理和颜色，常忽略几何结构，这是由于其片段基于设计导致的。需要一种能够更好地编码丰富和结构化视觉内容的新方法。

Method: 提出Visual Gaussian Quantization (VGQ)框架，将传统视觉码本量化框架与2D高斯分布相结合。通过将图像潜在表示编码为2D高斯分布，直接建模位置、旋转和缩放等结构相关参数，有效抓取几何和空间结构。还通过增加标记中2D高斯的密度来提升重建保真度。

Result: 在ImageNet 256x256测试中，VGQ达到了rFID分数1.00的强劲重建质量。通过增加2D高斯密度，VGQ进一步获得了显著的重建能力提升，达到了rFID 0.556和PSNR 24.93的独创性成绩，显著超过现有方法。

Conclusion: VGQ作为一种新的标记化器范式，通过明显地增强结构建模能力，有效解决了传统量化方法在结构信息模建方面的不足。该方法在重建保真度和视觉丰富性之间提供了灵活的权衡，为AR图像生成领域带来了重要进展。
...
TL;DR: 基于视觉基础模型的特征分布几何形状具有跨领域可转移性，通过几何知识指导的分布检验框架在联邦学习和长尾分布识别中有效解决数据异质性和样本不平衡问题


<details>
  <summary>Details</summary>
Motivation: 深度学习中观测训练样本与真实分布存在差距，这可能由采样偏差、噪声等因素导致。基础模型时代下，需要找到一种方法来减小这个差距

Method: 利用现成的视觉基础模型（如CLIP、DINOv2）进行特征提取，发现特征分布的几何形状具有跨领域可转移性，并构建几何知识指导的分布检验框架

Result: 在联邦学习中设计技术获取全局几何形状，并利用这些知识生成新样本平滑客户端差异；在长尾学习中利用样本丰富类别的几何知识恢复尾部类别的真实分布

Conclusion: 综合实验表明，几何知识指导的分布检验框架能够有效克服数据异质性和样本不平衡导致的信息缺失，在多个标准测试集上提升了性能
...
TL;DR: 这篇论文首次对比了开源视觉-语言模型和传统深度学习模型在面部情感识别任务上的表现，发现传统模型显著更优于VLMs


<details>
  <summary>Details</summary>
Motivation: 面部情感识别对人机交互和心理健康诊断至关重要，需要评估最新的视觉-语言模型在这一挑战性任务上的表现

Method: 使用FER-2013数据集（35,887张低分辨率灰度图片），对比Phi-3.5 Vision、CLIP等VLMs与VGG19、ResNet-50、EfficientNet-B0等传统模型，并提出整合GFPGAN图片恢复的新流程

Result: 传统模型显著更优：EfficientNet-B0（86.44%）和ResNet-50（85.72%）远超CLIP（64.07%）和Phi-3.5 Vision（51.66%），显示了VLMs在低质量视觉任务中的局限性

Conclusion: 视觉-语言模型需要适应噪声环境，这项研究为情感识别领域提供了可复现的基准测试
...
TL;DR: 基于3D高斯拖射的表情感知头部虚拟人重建方法EAvatar，通过稀疏表情控制机制和3D先验知识提升了细细微表惉捕获和纹理连续性


<details>
  <summary>Details</summary>
Motivation: 现有的3DGS基础方法在捕获细细微面部表惉和保持局部纹理连续性方面面临挑战，特别是在高度可变形区域

Method: 提出稀疏表惉控制机制，用少量关键高斯影响邻近高斯的变形，并利用预训练生成模型的高质量3D先验知识提供面部几何结构指导

Result: 实验结果显示该方法能够产生更准确、视觉一致的头部重建，提升了表惉可控性和细节保真度

Conclusion: EAvatar框架通过表惉感知和变形感知的设计，有效解决了细微面部表惉和纹理连续性的挑战，为高保真头部虚拟人重建提供了更好的解决方案
...
TL;DR: FLAIR是一种新的隐式神经表示方法，通过RC-GAUSS激活函数和WEGE编码机制解决了现有INR方法的频率选择性和空间定位问题，在2D图像和3D重建任务中表现优异


<details>
  <summary>Details</summary>
Motivation: 现有隐式神经表示(INR)缺乏频率选择性、空间定位和稀疏表示能力，存在频谱偏差问题，过度依赖冗余信号分量，难以捕捉高频细节

Method: 提出FLAIR方法：1) RC-GAUSS激活函数，在时频不确定性原理约束下实现显式频率选择和空间定位；2) WEGE编码机制，利用离散小波变换计算能量分数，显式指导频率信息到网络中

Result: 在2D图像表示和恢复以及3D重建任务中，该方法持续优于现有INR方法

Conclusion: FLAIR通过创新的激活函数和编码机制有效解决了INR的频率选择性和空间定位问题，显著提升了表示性能
...
TL;DR: GazeProphet是一个纯软件方案，通过结合球形视觉变换器和LSTM时序编码器，无需专用眼动追踪硬件即可预测VR中的注视位置，为VR注视点渲染提供成本效益高的解决方案


<details>
  <summary>Details</summary>
Motivation: 现有的注视点渲染需要昂贵的硬件眼动追踪系统，限制了广泛采用。需要开发纯软件方案来降低成本和复杂性

Method: 使用球形视觉变换器处理360度VR场景，LSTM时序编码器捕捉注视序列模式，多模态融合网络整合空间场景特征和时序注视动态

Result: 在综合VR数据集上实现3.83度中值角度误差，比传统显著性基线提升24%，提供可靠的置信度校准，在不同空间区域和场景类型中保持稳定性能

Conclusion: 纯软件注视预测可用于VR注视点渲染，使性能提升更易于在不同VR平台和应用中实现
...
TL;DR: 轻量化生成式人脸视频编码框架，通过双模式优化（结构重设和操作精化）实现90.4%参数缩减和88.9%计算节省，保持了重建质量。


<details>
  <summary>Details</summary>
Motivation: 生成式人脸视频编码（GFVC）虽然在码率-夺真性能方面表现优异，但模型参数过大和计算成本高阻碍了其实际部署。

Method: 提出双模式优化：1）结构重设-用更薄、更高效的层替换传统3x3卷积；2）两阶段适配性通道剪枝策略-训练时软剪枝识别冗余通道，训练后硬剪枝永久删除。

Result: 实验结果显示，方法实现了90.4%的参数缩减和88.9%的计算节省，但仍能超越最新视频编码标准VVC的感知质量指标。
...
TL;DR: 提出一种基于生物神经元运作原理的神经元样编码方法，通过加入人造光感器层来生成完整的视觉刺刺信号，提升SNN性能保持神经形态计算原则。


<details>
  <summary>Details</summary>
Motivation: 解决SNN因刺刺数据信息容量有限而性能较差的问题，避免使用非刺刺输入违背神经形态计算的原创意图。

Method: 基于生物神经元内在运作原理和功能，设计神经元样编码方法，结合人造光感器层来生成包含颜色和亮度信息的刺刺数据。

Result: 实验结果显示该方法能够有效增加刺刺信号的信息含量，提升SNN的性能，同时遵循神经形态计算原则。

Conclusion: 这种受生物启发的方法有望克服神经形态计算的当前限制，促进SNN的更广泛应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在少样本异常分割中的跨类别泛化能力严重依赖于已知异常样本的先验知识，需要解决无需重新训练即可检测未见类别异常的问题。

Method: 包含三个核心组件：字典构建（使用正常参考图像特征模拟字典索引和内容）、字典查找（通过稀疏查找策略检索查询区域特征，无法检索的特征被分类为异常）、查询判别正则化（通过对比查询约束和文本对齐约束增强异常判别能力）。

Result: 在七个工业和医学公开数据集上的大量实验表明，DictAS持续优于最先进的少样本异常分割方法。
...
TL;DR: Learnable SMPLify是一个神经框架，用单次回归模型替代SMPLify的迭代优化过程，实现了200倍的速度提升，同时保持精度并具有良好的泛化能力。


<details>
  <summary>Details</summary>
Motivation: SMPLify作为3D人体姿态和形状估计的基准方法，虽然鲁棒但计算成本高。受其他领域用神经网络替代迭代优化取得显著运行时改进的启发，作者希望开发一个更高效的替代方案。

Method: 提出可学习的SMPLify框架，采用时序采样策略构建初始化-目标对进行训练，使用人体中心归一化方案和残差学习来缩小解空间，支持序列推理和插件式后处理。

Result: 方法实现了近200倍的运行时加速，在未见过的3DPW和RICH数据集上泛化良好，作为插件工具在LucidAction上以模型无关的方式运行。

Conclusion: Learnable SMPLify成为了一个实用且简单的基准方法，在保持精度的同时大幅提升了效率，具有良好的泛化能力和灵活性。
...
Method: 设立4个技术赛道：多类别3D多相机跟踪、交通视频问答、仓库空间推理、鱼眼相机目标检测；使用NVIDIA Omniverse生成数据集，采用部分隐藏测试集确保公平评估。

Result: 参与团队达245支来自15个国家，数据集下载量超过30,000次，多个团队取得顶级成绩并在多个任务中设立了新的基准。

Conclusion: 挑战赛成功推动了相关技术的发展，通过严格的评估框架确保了结果的可靠性和可复现性，为实际应用提供了有价值的基准和解决方案。
...
TL;DR: 通过一种新的生成式注意力模型学习特征语义关系，解决了现有视频动作分析方法在特征提取中忽视特征语义的问题，在动作检测和识别任务上展现了优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频动作分析方法忽视了特征语义学习，专注于优化动作建议，导致精度不足以满足高性能IoT应用的需求。

Method: 提出一种新的生成式注意力模型，利用动作前景和背景的差异，同时学习时间特征语义的帧级和段级依赖关系。

Result: 在动作检测和动作识别任务上进行了广泛实验，在公认数据集上验证了方法的优勒性。

Conclusion: 该模型有效利用了特征提取中的特征语义，为高性能IoT应用提供了稳健可扩展的智能视频分析解决方案。
...
TL;DR: 本文提出了一种时序条件引用视频对象分割模型，通过改进分割头设计、利用文本到视频滿散模型提取特征，以及设计时序上下文掩码精细模块，在四个公开数据集上达到了最先进性能。


<details>
  <summary>Details</summary>
Motivation: 直接视频对象分割（RVOS）预测方法往往过度重视特征提取和时序建模，而忽视了分割头的设计。实际上，分割头设计仍有很大的改进空间。

Method: 1）创新整合现有分割方法来提升边界分割能力 2）利用文本到视频滿散模型进行特征提取，移除传统噪声预测模块以避免随机性影响 3）设计时序上下文掩码精细（TCMR）模块来充分利用VAE的特征提取能力

Result: 在四个公开RVOS测试集上均达到了最先进的性能水平

Conclusion: 通过重新关注分割头设计、利用滿散模型特征提取以及时序上下文掩码精细，本文方法在保持模型简洁的同时显著提升了分割质量。
...
TL;DR: 通过基于GAN和汽合扩散-GAN的预渗水线管方案，将清晴天气图像转换为震雨、雪、夜间等恶劣条件图像，提升自动驾驶系统在恶劣环境下的感知性能。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统在低照明、降雨等恶劣环境下性能显著下降，而现有数据集中恶劣条件数据缺乏，收集和标注成本迅昂。

Method: 提出新的预渗水线管方案，包括模拟、GAN基础和汽合扩散-GAN方法，利用现有GAN并扩展支持辅助输入，开发新的训练方法结合模拟和实际图像，通过适应性混合减少扩散模型的幻觉和伪影。

Result: 在ACDC数据集上评估，语义分割整体提升1.85%，夜间场景提升4.62%，证明汽合方法在挑战性条件下的效果。

Conclusion: 该方法能够有效生成实际的恶劣条件图像，提升自动驾驶系统在复杂环境下的感知鲁棒性，解决了恶劣条件数据缺乏的问题。
...
<details>
  <summary>Details</summary>
Motivation: 状态空间模型在计算机视觉中表现出强大能力，但计算效率仍需提升。现有的token缩减方法应用于SSM时需要特别考虑其独特的序列建模特性，因此需要专门设计的token合并策略。

Method: 提出MaMe策略：1) 利用状态转移参数Δ作为信息重要性度量指标；2) 引入战略性token排列来保持序列信息流；3) 针对SSM的序列特性进行优化设计。

Result: 实验表明MaMe在效率和性能之间取得了优越的平衡，即使在激进的token缩减情况下也能保持鲁棒性。方法在图像分类、视频和音频等多个领域都展现出强大的泛化能力。

Conclusion: MaMe为SSM模型提供了一种有效的效率提升方法，在保持序列建模能力的同时显著减少计算开销，为SSM在各种应用中的实际部署提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法采用耦合编码器来处理语义和几何优先，导致了性能上的折衷，限制了整体表现

Method: 提出FoundationSSC框架，进行源级别和途径级别的双重解耦，使用基础编码器提供语义特征优先和立体成本体积，通过专门化途径精炼后输入混合视图变换

Result: 在SemanticKITTI上达到+0.23 mIoU和+2.03 IoU的收益，在SSCBench-KITTI-360上达到21.78 mIoU和48.61 IoU的最佳性能

Conclusion: 双重解耦设计能够同时提升语义和几何感知性能，为自动驾驶和机器人导航提供更好的密集感知能力
...
TL;DR: 自动化多模态Vlog生成框架PersonaVlog，通过MLLM多代理协作和迭代自加强机制，生成包含视频、背景音乐和内心独白的个性化Vlog内容。


<details>
  <summary>Details</summary>
Motivation: 现有Vlog生成方法依赖预定义脚本，缺乏动态性和个人表达能力，需要能够实现高度个性化的多模态协作生成方案。

Method: 基于多模态大语言模型(MLLM)的多代理协作框架，包含反馈与回滚机制，通过MLLM评估生成结果并进行迭代自加修正。同时提出ThemeVlogEval评测框架。

Result: 综合实验证明该框架在多个基准方法中显示出显著优势，具有生成高质量个性化Vlog的效果和潜力。

Conclusion: PersonaVlog框架通过多模态协作和自加修正机制，有效解决了自动Vlog生成中的个性化和创意性问题，为多模态内容创作提供了新方向。
...
TL;DR: 这篇论文提出了一种基于人脸识别和口罩检测的双因素验证智能门禁系统，通过Raspberry Pi平台实现远程控制和安全告警功能。


<details>
  <summary>Details</summary>
Motivation: 在COVID-19大流行期间，面部口罩检测变得至关重要，但现有的IoT系统在口罩检测方面存在不足。需要一种能够合并人脸识别和口罩检测的智能门禁解决方案。

Method: 系统采用了基于Local Binary Patterns Histograms的全脸识别算法，以及改进的LBPH算法来处理遮挡面部的检测。通过Raspberry Pi平台集成了远程控制功能，可以通过Telegram进行用户注册、门锁控制和主人通知。

Result: 系统在测试中达到了平均准确率70%，精确率80%，及回归率83.26%的性能。系统成功实现了人脸识别、口罩检测、远程控制和安全告警等功能。

Conclusion: 该双因素验证智能门禁系统在用户接受度试验中获得了高度认可，能够有效地结合人脸识别和口罩检测功能，为智能入口控制提供了一种可行的解决方案，具有良好的应用前景。
...
TL;DR: TalkVid是一个大规模、高质量、多样化的人脸视频数据集，包含1244小时视频和7729名独特说话者，旨在解决现有音频驱动说话头合成模型在种族、语言和年龄群体多样性方面泛化能力不足的问题。
...
Method: 通过多阶段自动化流程精心策划数据集，严格筛选运动稳定性、美学质量和面部细节，并通过人工验证确保可靠性。同时构建了TalkVid-Bench评估集，包含500个精心平衡的片段，覆盖关键人口统计和语言维度。

Result: 在TalkVid上训练的模型优于在先前数据集上训练的对应模型，展现出更好的跨数据集泛化能力。分析显示传统聚合指标掩盖了不同子组之间的性能差异。

Conclusion: TalkVid数据集解决了现有数据集的局限性，TalkVid-Bench评估集对于揭示模型在不同人口统计群体上的性能差异至关重要，为未来研究提供了必要的基础设施。
...
TL;DR: 一种仅使用RGB图像的类别级物体姿势估计方法，通过转换器网络预测几何特征和RANSAC-PnP算法，解决了缺乏深度信息的挑战


<details>
  <summary>Details</summary>
Motivation: 当前大多数RGB-D基于类别级物体姿势估计方法在缺乏深度信息的场景中面临重大挑战，需要一种仅依赖RGB图像的解决方案

Method: 设计了基于转换器的神经网络来预测和融合目标物体的几何特征，并为确保预测几何特征准确而引入了几何特征导向算法，最终使用RANSAC-PnP算法计算物体姿势

Result: 在标准数据集上的实验结果显示，该方法不仅高效而且在准确性上超过了之前的RGB基础方法

Conclusion: 该方法为使用RGB图像推进类别级物体姿势估计提供了新的视角
...
<details>
  <summary>Details</summary>
Motivation: 评估MLLM模型在图片旋转识别任务中的视觉理解能力，测试其对空间关系和旋转线索的检测能力

Method: 构建RotBench标准数据集（350张图片），测试多个独立和商业MLLM模型，使用各种提示策略和辅助信息，包括标题、深度图、链式思维等

Result: 模型在0度图片识别上表现良好，部分模型能识别180度旋转，但无法区分90度和270度旋转；各种提示策略改善效果有限，精调也无法解决这一问题

Conclusion: MLLM模型在空间理解能力上存在显著缺陷，尤其在旋转识别任务中显示出与人类视觉感知的差距，需要进一步改进
...
### [83] [DiffIER: Optimizing Diffusion Models with Iterative Error Reduction](https://arxiv.org/abs/2508.13628)
*Ao Chen,Lihe Ding,Tianfan Xue*

Main category: cs.CV

TL;DR: 本文提出了DiffIER方法来解决扩散模型中分类器自由引导(CFG)存在的训练-推理差距问题，通过迭代误差最小化优化生成质量，在多个条件生成任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在条件生成中对引导权重选择高度敏感，存在训练和推理阶段的不一致（训练-推理差距），这影响了生成质量并导致输出对引导权重的敏感性。

Method: 提出DiffIER方法，通过在每个推理步骤进行迭代误差最小化来减少累积误差，采用基于优化的plug-and-play框架优化每一步的推理误差。
...
Method: 两阶段训练流程：第一阶段利用大规模不成对图像训练无掩码定位模型，通过重用途修复模型自动在合适位置绘制物品；第二阶段用少量成对图像微调以保持物品外观一致性。

Result: 在包含12类常见可穿戴物品的综合基准测试中，OmniTry在物品定位和ID保持方面均优于现有方法，且第一阶段后模型即使使用少量样本也能快速收敛。

Conclusion: OmniTry提供了一个统一的框架，成功扩展了虚拟试穿的应用范围，解决了多类可穿戴物品试穿的数据获取难题，具有更好的实用性和性能表现。
...
TL;DR: 通过解耦项点检测、相邻项点预测、初始图构建和图扩展四个步骤，DeH4R结合了图生成方法的高效率和图增长方法的动态性，在保持快速推理的同时提升了拓扑保真度和空间一致性。


<details>
  <summary>Details</summary>
Motivation: 远感图像中自动提取准确的道路网络图仍面临挑战：分割方法缺乏拓扑保真度，图增长方法计算成本高，图生成方法无法动态插入项点。需要一种新方法来结合各方优势。

Method: 提出DeH4R混合模型，将任务解耦为四个步骤：候选项点检测、相邻项点预测、初始图构建和图扩展。这种架构设计允许动态插入项点和边，同时保持快速推理速度。

Result: 在CityScale和SpaceNet标准数据集上达到最先进性能。在CityScale上比之前最优的RNGDet++方法提升了4.62 APLS和10.18 IoU，速度提升约10倍。

Conclusion: DeH4R通过混合图生成和图增长方法的优势，成功解决了道路网络提取中的拓扑保真度、计算效率和动态性问题，为远感图像分析领域提供了一种高效准确的解决方案。
...
TL;DR: HumanPCR是一个用于评估多模态大语言模型在人类相关视觉上下文理解能力的三层次评估套件，包含感知、理解和推理三个层级，包含6000+人工验证问题和视频推理测试，揭示了当前模型在人类中心视觉理解方面的显著挑战


<details>
  <summary>Details</summary>
Motivation: 随着多模态模型的快速发展，追求人工通用智能需要模型在多样化环境中达到人类可比性能，因此需要评估模型对人类相关视觉上下文的理解能力
...
Conclusion: 即使采用扩展视觉上下文和测试时思考等先进技术，改进效果有限。HumanPCR和发现将推动多模态模型的开发、评估和人类中心应用
...
<details>
  <summary>Details</summary>
Motivation: 医学图像标注成本高昂，半监督学习可缓解标注负担。Mamba等状态空间模型在处理长距离依赖方面表现出色，值得探索其在半监督医学图像分割中的应用潜力

Method: 1) 数据层面：开发基于Mamba扫描特性的补丁级弱-强混合增强；2) 网络层面：引入多样化扫描协作模块，利用不同扫描方向的预测差异；3) 特征层面：采用不确定性加权的对比学习机制增强特征表示多样性

Result: 在Synapse数据集上，使用20%标注数据时比最新的SSM-based方法提升6.69%，显著优于其他半监督医学图像分割方法

Conclusion: DCMamba框架通过多角度多样性增强，有效提升了半监督医学图像分割的性能，证明了Mamba模型在该领域的应用价值
...
TL;DR: 这篇论文提出了一种基于视觉-语言模型的层次结构方法，用于农业Metaverse场景的表征和检索，并创建了包含457个虚拟博物馆的新数据集。


<details>
  <summary>Details</summary>
Motivation: 在线教育内容深度增长，特别是农业领域。Metaverse能提供交互式洞入式学习体验，但相关场景的搜索和匹配仍面临挑战。现有数据集过小，不足以训练高级模型。

Method: 提出了一种层次视觉-语言模型，用于表征和检索相关的AgriMuseums。创建了包含457个农业主题虚拟博物馆的新数据集，每个都有文本描述。

Result: 在实验设置中，提出的方法达到了约62% R@1和78% MRR，证明了其有效性。在现有基准上也实现了进步，最高提升了6% R@1和11% MRR。

Conclusion: 该研究为农业Metaverse内容的组织和检索提供了有效解决方案，通过新的数据集和层次模型实现了显著的性能提升。
...
TL;DR: IPGA攻击方法通过利用投影器模块的中间阶段（Q-Former）进行对抗攻击，实现了更精细的图像操控，在保持背景的同时修改特定对象，显著提升了攻击效果和跨模型迁移能力。


<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击方法将丰富的视觉语义压缩为单一全局向量，限制了攻击粒度，且忽略了投影器模块在视觉-语言对齐中的关键作用，导致攻击效果有限。

Method: 提出Intermediate Projector Guided Attack (IPGA)，首次利用投影器模块的中间阶段（Q-Former）进行攻击，将全局图像嵌入转换为细粒度视觉特征；同时提出Residual Query Alignment (RQA)来保留无关视觉内容。

Result: 在标准全局图像描述任务和细粒度视觉问答任务中，IPGA在黑盒环境下始终优于现有方法，并成功迁移到多个商业VLM（Google Gemini和OpenAI GPT）。

Conclusion: IPGA通过操作语义有意义的视觉标记而非单一全局表示，实现了更精确的对抗扰动控制，显著提升了攻击效果和跨模型迁移能力。
...
TL;DR: FOCUS是一种无需训练的解码策略，通过顺序掩码图像来缓解多图像输入中的跨图像信息泄漏问题，显著提升多图像推理性能
...
Method: 提出FOCUS解码策略：1）顺序用随机噪声掩码除一张图像外的所有图像；2）重复该过程获取部分掩码上下文下的logits；3）聚合logits并使用纯噪声参考输入进行对比精炼

Result: FOCUS在四个多图像基准测试和多种LVLM模型家族中一致提升性能

Conclusion: FOCUS提供了一个通用且实用的解决方案，无需额外训练或架构修改即可增强多图像推理能力
...
TL;DR: MR6D是一个专为移动机器人设计的6D位姿估计数据集，针对工业环境中大尺寸物体、远距离视角和复杂遮挡等挑战，填补了现有数据集主要关注小型家用物体的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有6D位姿估计数据集主要针对机器人臂操作的小型家用物体，无法满足移动机器人在工业环境中的需求，包括大尺寸物体、远距离感知、严重自遮挡和多样化相机视角等挑战。

Method: 构建包含92个真实场景、16个独特物体的MR6D数据集，涵盖静态和动态交互场景，特别捕捉移动平台特有的远距离视角、多样化物体配置、大尺寸物体和复杂遮挡模式。

Result: 初步实验显示当前6D位姿估计流程在这些设置下表现不佳，2D分割成为另一个主要障碍，表明现有方法难以应对移动机器人工业环境的挑战。

Conclusion: MR6D为开发和评估针对移动机器人需求的位姿估计方法奠定了基础，填补了该领域的数据空白，数据集已在HuggingFace平台公开。
...
TL;DR: 这篇论文提出了一种新的非粘性三维形状到多相机二维关键点注册方法，通过普通相机模型和凸规划等技术实现了更高精度的形状恢复。


<details>
  <summary>Details</summary>
Motivation: 解决多相机环境下的非粘性三维形状到二维关键点注册问题，扩展便携相机和医学成像等应用场景的形状恢复能力。

Method: 使用普通相机模型表示多相机系统，提出三种注册方法：知道三维点方向向量方法、未知三维点但知道定向方法、以及结合边缘信息的方法。采用凸规划和迭代精化技术求解。

Result: 在合成和实际数据上验证了方法的准确性，显示出利用多视图相互约束可以提高变形形状估计的精度。

Conclusion: 这是首套解决普通相机下形状模板恢复问题的方案，为多相机非粘性注册领域开启了新方向，在医学形态注册和便携相机应用中具有重要价值。
...
Method: 上层使用LLM作为物理专家生成和修订本构定律，通过解耦机制降低搜索复杂度；下层通过视觉模拟评估生成的本构定律与底层内在动力学的一致性，指导上层演化。

Result: 在合成和真实数据集上的实验表明，VisionLaw能有效从视觉观察中推断可解释的内在动力学，显著优于现有最先进方法，并在新场景的交互模拟中表现出强泛化能力。

Conclusion: VisionLaw框架成功解决了现有方法的局限性，通过结合LLM的知识推理能力和视觉模拟验证，实现了可解释且泛化性强的内在动力学推断。
...
### [94] [A Fully Transformer Based Multimodal Framework for Explainable Cancer Image Segmentation Using Radiology Reports](https://arxiv.org/abs/2508.13796)
...
Method: 使用双分支视觉编码器（ViT+Swin Transformer）和不确定性感知融合，结合BioClinicalBERT编码BI-RADS语义的临床报告，通过跨模态注意力机制融合多模态特征。

Result: 在BUS-BRA数据集上达到Dice 99%和IoU 95%，超越U-Net、ViT和Swin等基线模型，多模态对齐CLIP得分85%，置信度校准ECE 3.2%。

Conclusion: Med-CTX通过整合临床文本显著提升了分割性能和可解释性，为可信赖的多模态医疗架构设立了新标准，临床文本对分割准确性和解释质量至关重要。
...
### [95] [Timestep-Compressed Attack on Spiking Neural Networks through Timestep-Level Backpropagation](https://arxiv.org/abs/2508.13812)
...
### [96] [Unsupervised Urban Tree Biodiversity Mapping from Street-Level Imagery Using Spatially-Aware Visual Clustering](https://arxiv.org/abs/2508.13814)
*Diaa Addeen Abuhani,Marco Seccaroni,Martina Mazzarello,Imran Zualkernan,Fabio Duarte,Carlo Ratti*

Main category: cs.CV

TL;DR: 无监督聚类框架结合街道图像和空间植装模式，可以在不需标签数据的情况下估算城市树木生物多样性，为缺乏详细监测数据的城市提供可扩展的监测方案。


<details>
  <summary>Details</summary>
Motivation: 城市树木生物多样性对气候适应和生态稳定至关重要，但现场调查成本高、耗时，而有监督的AI方法需要标签数据且突破地域性限制。需要一种可扩展、低成本的方法来进行城市生物多样性监测。

Method: 提出了一种无监督聚类框架，结合街道级图像的视觉嵌入和空间植装模式，在不需要标签数据的情况下估算生物多样性。应用于八个北美城市进行验证。

Result: 方法能够高保真度地恢复属级生物多样性模式，对Shannon和Simpson指数达到低Wasserstein距离，并保持了空间自相关性。

Conclusion: 该可扩展、细粒度的方法能够在缺乏详细监测数据的城市进行生物多样性地图制作，为实现公平获得绿艺和城市生态系统适应性管理提供了持续、低成本的监测途径。
...
TL;DR: 提出SA3方法，通过注意力对齐模块和实例-图像级对齐，解决跨域目标检测中的领域差异问题，在多个基准测试中达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 智能交通检测中跨域场景下的目标检测存在挑战，需要解决源域和目标域之间的领域差异问题

Method: 使用基于注意力的对齐模块进行局部-全局自适应对齐，通过通道重要性重加权特征，并引入目标域特定的实例-图像级对齐模块

Result: 在流行的跨域目标检测基准测试中取得了优于之前最先进方法的结果

Conclusion: SA3方法通过有效的对齐机制和识别策略，成功解决了跨域目标检测的挑战，实现了卓越的性能
...
### [98] [SAGA: Learning Signal-Aligned Distributions for Improved Text-to-Image Generation](https://arxiv.org/abs/2508.13866)
*Paul Grimal,Michaël Soumm,Hervé Le Borgne,Olivier Ferret,Akihiro Sugimoto*

Main category: cs.CV

TL;DR: 提出了一种新的文本到图像生成方法，通过建模去噪过程中的信号分量来确保生成图像与文本提示精确对齐，避免关键元素缺失或概念混淆的问题。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的文本到图像模型虽然能产生视觉上令人印象深刻的结果，但往往难以精确对齐文本提示，导致关键元素缺失或不同概念意外混合的问题。
...
Conclusion: 提出的框架能够有效解决文本到图像生成中的对齐问题，提供更精确和可控的图像生成能力，同时保持与现有架构的良好兼容性。
...
### [99] [RED.AI Id-Pattern: First Results of Stone Deterioration Patterns with Multi-Agent Systems](https://arxiv.org/abs/2508.13872)
*Daniele Corradetti,José Delgado Rodrigues*

Main category: cs.CV

TL;DR: RED.AI项目开发了一个多智能体AI系统，用于自动识别石材劣化模式，相比传统专家观察方法更高效，在28个复杂图像测试中各项指标显著优于基础模型。
...
Method: 采用认知架构协调五个专业AI智能体（岩性学家、病理学家、环境专家、修复师和诊断协调员）协作，从视觉证据自动化诊断石材病理。

Result: 在28个包含多种劣化模式的困难图像测试中，系统所有指标相比基础模型都有显著提升。

Conclusion: 多智能体AI系统能够有效模拟专家协作，实现石材劣化模式的自动化识别，为文化遗产保护提供了高效的技术解决方案。
...
### [100] [RICO: Two Realistic Benchmarks and an In-Depth Analysis for Incremental Learning in Object Detection](https://arxiv.org/abs/2508.13878)
*Matthias Neuwirth-Trapp,Maarten Bieshaar,Danda Pani Paudel,Luc Van Gool*

Main category: cs.CV

TL;DR: 这篇论文提出了两个现实的增量学习目标检测基准(RICO)，指出当前增量学习方法在现实场景下表现差强，甚至连重放少量旧数据都更好，只有独立训练才是最优解决方案。


<details>
  <summary>Details</summary>
Motivation: 现有增量学习评估多基于简化的合成数据集，无法反映现实世界的复杂性和挑战，需要更现实的评测基准来评估方法的真实性能。

Method: 构建了两个现实增量学习目标检测基准(RICO)：D-RICO(领域偏移但类别固定)和EC-RICO(每步增加新领域和新类别)，统合了14个多样化数据集，涵盖真实和合成领域、不同条件(天气、时间)、摄像头视角和标注策略等差异。

Result: 实验结果显示所有增量学习方法在适应性和知识保持方面都表现差强，重放少量旧数据已经超过所有方法，但独立训练仍然是最优解决方案。

Conclusion: 增量学习方法在现实场景下存在显著性能差距，归因于排练中弱老师问题、单一模型无法处理多样任务以及不足的可塑性。该研究为增量学习领域提供了更现实的评测标准。
...
### [101] [In-hoc Concept Representations to Regularise Deep Learning in Medical Imaging](https://arxiv.org/abs/2508.13880)
*Valentina Corbetta,Floris Six Dijkstra,Regina Beets-Tan,Hoel Kervadec,Kristoffer Wickstrøm,Wilson Silva*

Main category: cs.CV

TL;DR: LCRReg是一种新颖的正则化方法，利用潜在概念表示来引导医学影像深度学习模型学习语义基础特征，提高对分布偏移的鲁棒性，无需主训练集中的概念标注。


<details>
  <summary>Details</summary>
Motivation: 医学影像深度学习模型往往依赖虚假相关性而非临床有意义的特征，在分布偏移下泛化能力差，需要一种轻量级方法来提升模型鲁棒性。

Method: 使用小型辅助数据集合成高质量解耦概念样本，提取预定义相关特征的潜在概念表示，通过正则化项引导CNN在相关概念子空间内激活。

Result: 在合成和真实医学任务中验证有效：在玩具数据集上显著提升对注入虚假相关性的鲁棒性；在糖尿病视网膜病变分类任务中，在合成扰动和OOD泛化下性能均有提升。
...
### [102] [Forecasting Smog Events Using ConvLSTM: A Spatio-Temporal Approach for Aerosol Index Prediction in South Asia](https://arxiv.org/abs/2508.13891)
*Taimur Khan*

Main category: cs.CV

TL;DR: 单元格LSTM模型预测南亚霜霉污染，使用偶极化谱仪数据实现五天期预测，准确度较高


<details>
  <summary>Details</summary>
Motivation: 南亚地区年度霜霉污染严重影响生态和经济，但缺乏有效的实时预测系统，需要发展空间时间相关性预测方法

Method: 使用Sentinel-5P偶极化谱仪2019-2023年数据，构建卷积长短期记忆网络(ConvLSTM)模型，以340-380nm紫外线汽胶指数进行预测

Result: 模型在五天预测期限内实现平均方误差~0.0018，损失值~0.3995，结构相似性指数~0.74，显示良好预测能力

Conclusion: ConvLSTM模型能够有效预测南亚霜霉污染事件，但仍需整合更多数据源和优化模型结构来提升性能
...
### [103] [SCRNet: Spatial-Channel Regulation Network for Medical Ultrasound Image Segmentation](https://arxiv.org/abs/2508.13899)
...
Method: 设计了特征聚合模块(FAM)和卷积与交叉注意并行模块(CCAPM)，通过结合卷积操作和交叉注意机制来同时关注长程依赖和局部上下文。将空间-通道调节模块(SCRM)集成到UNet编码器中构建SCRNet框架。

Result: 广泛实验结果显示SCRNet在医学超声图像分割任务中持续达到最先进的性能，超越了现有方法。

Conclusion: SCRNet通过创新地结合卷积和交叉注意机制，有效解决了长程依赖和局部上下文关注的问题，为医学超声图像分割提供了一种高效的解决方案。
...
### [104] [PhysGM: Large Physical Gaussian Model for Feed-Forward 4D Synthesis](https://arxiv.org/abs/2508.13911)
*Chunji Lv,Zequn Chen,Donglin Di,Weinan Zhang,Hao Li,Wei Chen,Changsheng Li*

Main category: cs.CV

TL;DR: PhysGM是一个前馈框架，可从单张图像联合预测3D高斯表示和物理属性，实现快速物理仿真和高保真4D渲染，相比现有方法速度显著提升


<details>
  <summary>Details</summary>
Motivation: 现有物理基础3D运动合成方法依赖预重建的3D高斯表示，物理集成要么使用不灵活的手动定义物理属性，要么依赖不稳定的视频模型优化引导，存在关键局限性

Method: 1）建立基础模型联合优化高斯重建和概率物理预测；2）使用物理合理的参考视频进行精炼；3）采用直接偏好优化(DPO)对齐仿真与参考视频，避免复杂的可微分仿真和光栅化梯度反向传播

Result: 方法能在一分钟内从单张图像生成高保真4D仿真，相比先前工作实现显著加速，同时提供逼真的渲染结果

Conclusion: PhysGM通过联合预测3D表示和物理属性，结合DPO优化和新的PhysAssets数据集，有效解决了现有方法的局限性，实现了快速高质量的物理仿真
...
### [105] [DIME-Net: A Dual-Illumination Adaptive Enhancement Network Based on Retinex and Mixture-of-Experts](https://arxiv.org/abs/2508.13921)
...
Result: 在合成和真实世界的低光和背光数据集上均取得竞争性性能，无需任何重新训练，展示了良好的泛化能力

Conclusion: DIME-Net框架在多样化复杂光照条件下具有实际多媒体应用的潜力，能够有效处理多种光照退化问题
...
### [106] [ViT-FIQA: Assessing Face Image Quality using Vision Transformers](https://arxiv.org/abs/2508.13957)
*Andrea Atzori,Fadi Boutros,Naser Damer*

Main category: cs.CV

TL;DR: ViT-FIQA是一种基于Vision Transformer的人脸图像质量评估方法，通过可学习的质量token来预测人脸图像的效用分数，在多个基准测试中达到顶尖性能
...
Result: 在多个具有挑战性的基准测试和FR模型上，ViT-FIQA始终达到顶级性能，证明了基于transformer架构的有效性
...
### [107] [ROVR-Open-Dataset: A Large-Scale Depth Dataset for Autonomous Driving](https://arxiv.org/abs/2508.13977)
...
TL;DR: 提出了一个大规模、多样化的室外驾驶环境深度估计数据集，解决现有数据集的多样性和扩展性问题


<details>
  <summary>Details</summary>
Motivation: 现有深度估计数据集如KITTI、nuScenes等在多样性和扩展性方面存在限制，而基础模型和多模态学习需要更大规模、更多样化的数据集

Method: 通过轻量级的数据采集流水线，构建了20K视频帧的大规模数据集，具有宽泛的场景覆盖和稀疏但统计足够的真实值

Result: 该数据集在驾驶场景多样性方面超过现有数据集，深度密度更低，为沿波性提供了新挑战，标准模型的测试验证了数据集的有用性
...
### [108] [OmViD: Omni-supervised active learning for video action detection](https://arxiv.org/abs/2508.13983)
*Aayush Rana,Akash Kumar,Vibhav Vineet,Yogesh S Rawat*

Main category: cs.CV

TL;DR: 该论文研究视频动作检测中的标注成本问题，提出根据视频难度自动选择适当标注类型的方法，并通过3D超像素技术生成伪标签来降低标注成本。


<details>
  <summary>Details</summary>
Motivation: 视频动作检测需要密集的时空标注，成本高昂且具有挑战性。现实世界中的视频难度各异，不需要相同级别的标注，因此需要分析不同样本适合的标注类型及其对检测性能的影响。

Method: 1) 提出主动学习策略估计每个视频所需的标注类型；2) 引入新颖的时空3D超像素方法从不同标注类型生成伪标签；3) 支持视频级标签、点、涂鸦、边界框和像素级掩码等多种标注形式。

Result: 在UCF101-24和JHMDB-21数据集上验证，显著降低了标注成本，同时性能损失最小。

Conclusion: 通过智能选择标注类型和伪标签生成技术，可以在保持检测性能的同时大幅减少视频动作检测的标注工作量，为实际应用提供了经济高效的解决方案。
...
### [109] [Physics-Based 3D Simulation for Synthetic Data Generation and Failure Analysis in Packaging Stability Assessment](https://arxiv.org/abs/2508.13989)
*Samuel Seligardi,Pietro Musoni,Eleonora Iotti,Gianluca Contesso,Alessandro Dal Palù*

Main category: cs.CV

TL;DR: 基于3D虚拟环境的可控制模拟系统，通过深度神经网络分析模拟视频来预测盒子配置的碰撞风险，减少物理测试需求


<details>
  <summary>Details</summary>
Motivation: 流通业对包装运输安全的需求不断增长，以及需要找到环保的包装材料替代方案同时符合安全标准

Method: 开发了一个充分可控的精确物理模拟系统，包含3D图形虚拟环境，支持多种包装布局、包装材料和动态条件配置，并训练深度神经网络来评估模拟视频

Result: 该系统能够减少物理测试需求，降低成本和环境影响，同时提高了盒子动力学分析的测量精度

Conclusion: 这种创新方法为盒子安全分析提供了更高效的工具，通过虚拟模拟和AI分析组合，实现了安全评估的自动化和精准化
...
### [110] [Self-Supervised Sparse Sensor Fusion for Long Range Perception](https://arxiv.org/abs/2508.13995)
*Edoardo Palladin,Samuel Brucker,Filippo Ghilotti,Praveen Narayanan,Mario Bijelic,Felix Heide*

Main category: cs.CV

TL;DR: 通过稀疏表示和新的自监督预训练方案，将自动驾驶感知范围扩展到250米，为高速公路和重卡提供更长的规划时间


<details>
  <summary>Details</summary>
Motivation: 解决城市驾驶感知范围（50-100米）不足以满足高速公路安全需求（需要至250米），同时避免传统BEV表示在远距离时内存和计算成本二次增长的问题

Method: 基于稀疏表示构建高效的多模态时序特征三维编码，采用新的自监督预训练方案从无标签的摄像头-LiDAR数据中进行大规模学习

Result: 将感知距离扩展到250米，目标检测mAP提高26.6%，LiDAR预测Chamfer距离降低30.5%

Conclusion: 该方法有效解决了长距离感知的计算效率问题，为高速公路自动驾驶和重型载具提供了可靠的技术支撑
...
### [111] [ResPlan: A Large-Scale Vector-Graph Dataset of 17,000 Residential Floor Plans](https://arxiv.org/abs/2508.14006)
...
<details>
  <summary>Details</summary>
Motivation: 现有住宅平面图数据集（如RPLAN和MSD）在视觉保真度和结构多样性方面存在局限，无法反映真实非理想化的住宅布局，需要更高质量的数据集来推进空间AI研究。

Method: 创建了17,000个结构丰富、真实的住宅平面图数据集，包含几何和基于图的格式，提供开源几何清理、对齐和标注优化流程，以及房间连通性的结构化表示。

Result: ResPlan在规模、真实性和可用性方面显著提升，提供了增强的视觉保真度和更大的结构多样性，支持直接集成到仿真引擎和快速3D转换。

Conclusion: ResPlan为开发和基准测试下一代空间智能系统提供了强大的基础，推动了空间AI研究的发展，并支持多种应用领域如机器人、生成AI、VR/AR等。
...
### [112] [Online 3D Gaussian Splatting Modeling with Novel View Selection](https://arxiv.org/abs/2508.14014)
*Byeonggwon Lee,Junkyu Park,Khang Truong Giang,Soohwan Song*

Main category: cs.CV

TL;DR: 通过适应性视角选择策略，在线从RGB帧生成高质量3D高斯涂射模型，解决了仅依赖关键帧导致场景重建不完整的问题


<details>
  <summary>Details</summary>
Motivation: 以前的方法仅使用关键帧进行3D场景估计，无法完整捕捉整个场景，导致重建不完整。而且在线处理限制了使用大量帧或迭代训练

Method: 提出适应性视角选择策略，通过在线分析重建质量来选择最优的非关键帧进行额外训练。结合关键帧和选择的非关键帧，从多角度精炼不完整区域。还包含在线多视角立体框架确保3D信息一致性

Result: 实验结果表明该方法在复杂户外场景中表现出色，超越了现有最先进方法

Conclusion: 通过适应性视角选择策略，能够在在线处理限制下生成更完整的高质量3D高斯涂射模型，显著提升了场景重建的完整性
...
### [113] [Backdooring Self-Supervised Contrastive Learning by Noisy Alignment](https://arxiv.org/abs/2508.14015)
*Tuo Chen,Jie Gui,Minjing Dong,Ju Jia,Lanting Fang,Jian Liu*

Main category: cs.CV

TL;DR: 本文提出Noisy Alignment方法，通过显式抑制中毒图像中的噪声成分来提升数据投毒后门攻击的效果，在保持干净数据准确性的同时实现了最先进的攻击性能。


<details>
  <summary>Details</summary>
Motivation: 现有的自监督对比学习数据投毒后门攻击方法效果有限，主要依赖于脆弱的隐式共现关系，且对后门图像中判别性特征的抑制不足。

Method: 通过策略性地操纵对比学习的随机裁剪机制，将噪声对齐过程建模为图像布局优化问题，并使用理论推导的最优参数来实现。

Result: Noisy Alignment方法在攻击效果上达到了最先进的性能，同时保持了干净数据的准确性，并且对常见的后门防御方法具有鲁棒性。

Conclusion: 该方法简单而有效，通过显式抑制噪声成分显著提升了数据投毒后门攻击的效果，为对比学习的安全性研究提供了新的思路。
...
### [114] [InfiniteTalk: Audio-driven Video Generation for Sparse-Frame Video Dubbing](https://arxiv.org/abs/2508.14033)
...
TL;DR: 新的散布帧视频对白技术可以在保持原始身份识别和摄像机轨迹的同时，实现音频驱动的全身动画编辑，克服了传统嘴型编辑技术的限制。


<details>
  <summary>Details</summary>
Motivation: 传统视频对白技术仅能编辑嘴部区域，导致面部表情和身体手势不协调，影响观众沉浸感。需要一种能够进行全身动画编辑的新方法。

Method: 提出InfiniteTalk流式音频驱动生成器，利用时间上下文帧实现无缝切换，采用精细的参考帧定位策略优化控制强度。

Result: 在HDTF、CelebV-HQ和EMTD数据集上达到了最先进性能，量化指标显示在视觉真实性、情感一致性和全身动作同步方面均体现优异。

Conclusion: 该研究为音频驱动的长序列视频对白提供了一种高效的解决方案，能够在保持原始视频特征的同时实现自然的全身动画同步。
...
### [115] [GeoSAM2: Unleashing the Power of SAM2 for 3D Part Segmentation](https://arxiv.org/abs/2508.14036)
...
Method: 采用数据依赖流在潜在空间中直接建模粗糙到精细的转换，引入token匹配策略确保细化过程中的准确空间对应，通过精心设计的训练数据匹配合成粗糙形状的特征。
...
### [116] [Distilled-3DGS:Distilled 3D Gaussian Splatting](https://arxiv.org/abs/2508.14037)
*Lintao Xiang,Xinkai Chen,Jianhuang Lai,Guangcong Wang*

Main category: cs.CV

TL;DR: 首个3D高斯撕裂矩阵的知识精缩框架，通过多老师模型和结构相似性损失，在保持渲染质量的同时大幅减少存储需求


<details>
  <summary>Details</summary>
Motivation: 3D高斯撕裂技术虽然在新视角合成上表现出艰，但需要大量3D高斯元素来实现高保真渲染，导致内存和存储成本过高

Method: 提出知识精缩框架，使用多种老师模型（原生3DGS、噪声增强版本、dropout正则化版本）的输出指导轻量学生模型优化，并通过结构相似性损失促进空间几何分布一致性

Result: 在多种数据集上经过完整的定量和定性评估，Distilled-3DGS框架在渲染质量和存储效率方面都达到了与最先进方法相比的有望结果

Conclusion: 该简单但有效的知识精缩框架成功解决了3D高斯撕裂技术的存储效率问题，在保持高质量渲染的同时大幅减少了模型规模和存储需求
...
### [117] [Beyond Simple Edits: Composed Video Retrieval with Dense Modifications](https://arxiv.org/abs/2508.14039)
*Omkar Thawakar,Dmitry Demidov,Ritesh Thawkar,Rao Muhammad Anwer,Mubarak Shah,Fahad Shahbaz Khan,Salman Khan*

Main category: cs.CV

TL;DR: 新的稠密视频检索数据集Dense-WebVid-CoVR和跨注意融合模型，在组合视频检索任务中实现了独创性能，达到了71.3%的Recall@1指标。
...
Method: 构建包含160万样本的稠密修改文本数据集，开发了通过基础文本编码器进行跨注意融合的新模型，实现稠密查询修改与目标视频的精确对齐。

Result: 在所有指标上超越现有方法，特别是在视觉+文本设置下达到了71.3%的Recall@1，比最佳方法提高了3.4%。

Conclusion: 该研究通过稠密的视频描述和修改文本，有效提升了组合视频检索的性能，为该领域提供了新的数据集和模型框架。
...
### [118] [LongSplat: Robust Unposed 3D Gaussian Splatting for Casual Long Videos](https://arxiv.org/abs/2508.14041)
...
Method: 提出三个核心组件：1）增量联合优化同时优化相机位姿和3D高斯以避免局部极小值；2）利用学习3D先验的鲁棒位姿估计模块；3）基于空间密度的八叉树锚点形成机制将稠密点云转换为锚点。

Result: 在具有挑战性的基准测试上进行的广泛实验表明，LongSplat实现了最先进的结果，在渲染质量、位姿精度和计算效率方面相比先前方法有显著提升。

Conclusion: LongSplat为长视频无位姿3D高斯泼溅提供了一个有效的解决方案，通过创新的联合优化和锚点机制解决了现有方法的局限性，在多个指标上表现出优越性能。
...
### [119] [Chain-of-Agents: End-to-End Agent Foundation Models via Multi-Agent Distillation and Agentic RL](https://arxiv.org/abs/2508.13167)
*Weizhen Li,Jianbo Lin,Zhuosong Jiang,Jingyi Cao,Xinpeng Liu,Jiayu Zhang,Zhenqiang Huang,Qianben Chen,Weichen Sun,Qiexiang Wang,Hongxuan Lu,Tianrui Qin,Chenghao Zhu,Yi Yao,Shuying Fan,Xiaowan Li,Tiannan Wang,Pai Liu,King Zhu,He Zhu,Dingfeng Shi,Piaohong Wang,Yeyi Guan,Xiangru Tang,Minghao Liu,Yuchen Eleanor Jiang,Jian Yang,Jiaheng Liu,Ge Zhang,Wangchunshu Zhou*

Main category: cs.AI

TL;DR: 链式机器人框架(CoA)通过多机器渗透和机器人强化学习，让单个LLM模型具备多机器系统的复杂问题解决能力，并创造了多个基准的新纪录


<details>
  <summary>Details</summary>
Motivation: 现有多机器系统依赖手动提示工程和复杂框架，计算效率低且无法从数据中学习，需要一种本地化的端到端复杂问题解决方案

Method: 提出链式机器人(CoA)框架，通过多机器渗透把SOTA多机器系统转换为训练数据，再使用机器人强化学习进一步提升模型能力

Result: Agent Foundation Models (AFMs)在web agent和code agent多个基准上创造了新的SOTA性能，证明了方案的有效性

Conclusion: CoA框架能让单个LLM模型拥有多机器系统的复杂问题解决能力，为机器模型研究提供了坚实基础，并完全开源
...
### [120] [Cognitive Workspace: Active Memory Management for LLMs -- An Empirical Study of Functional Infinite Context](https://arxiv.org/abs/2508.13171)
*Tao An*

Main category: cs.AI

TL;DR: Cognitive Workspace是一种新的LLM上下文管理范式，通过模拟人类外部记忆使用机制，超越传统RAG方法，实现了主动记忆管理和任务驱动的动态优化。
...
Method: 基于认知科学理论（Baddeley工作记忆模型、Clark扩展心智论等），提出三个核心创新：主动记忆管理、分层认知缓冲区和任务驱动上下文优化。

Result: 实验验证显示平均58.6%的记忆重用率（传统RAG为0%），净效率增益17-18%，统计显著性p<0.001，Cohen's d>23。
...
### [121] [AlphaEval: A Comprehensive and Efficient Evaluation Framework for Formula Alpha Mining](https://arxiv.org/abs/2508.13174)
*Hongjun Ding,Binqi Chen,Jinsheng Huang,Taian Guo,Zhengyang Mao,Guoyi Shao,Lutong Zou,Luchen Liu,Ming Zhang*

Main category: cs.AI

TL;DR: AlphaEval是一个统一、可并行、无需回测的自动化alpha挖掘模型评估框架，通过五个维度全面评估alpha质量，比传统方法更高效全面。


<details>
  <summary>Details</summary>
Motivation: 现有alpha挖掘评估方法存在局限性：回测计算密集且对参数敏感，相关性指标只关注预测能力而忽略其他重要属性，且大多数模型闭源阻碍了可复现性。

Method: 提出AlphaEval框架，从五个互补维度评估alpha质量：预测能力、稳定性、对市场扰动的鲁棒性、金融逻辑性和多样性，无需进行传统回测。

Result: 在代表性alpha挖掘算法上的广泛实验表明，AlphaEval实现了与全面回测相当的评估一致性，同时提供更全面的洞察和更高的效率。

Conclusion: AlphaEval能有效识别优于传统单指标筛选方法的alpha，所有实现和评估工具都已开源以促进可复现性和社区参与。
...
TL;DR: 该论文研究如何从正负示例中拟合本体和约束，重点关注描述逻辑EL和ELI以及多种类型的元组生成依赖(TGD)，分析了计算复杂性、算法设计和本体大小，并探讨了有限基的存在性问题。


<details>
  <summary>Details</summary>
Motivation: 研究如何从给定的正负示例（有限关系结构）中自动学习本体和约束，这对于知识表示和数据库系统的自动化构建具有重要意义。

Method: 使用描述逻辑EL和ELI以及多种TGD类别（包括full、guarded、frontier-guarded、frontier-one等）作为本体和约束语言，通过理论分析和算法设计来研究拟合问题。

Result: 精确刻画了计算复杂性，设计了相应算法，分析了拟合本体和TGD的大小，并发现对于EL、ELI、guarded TGDs和inclusion dependencies存在有限基，但对于full、frontier-guarded和frontier-one TGDs一般不存在有限基。
...
### [123] [A Hardware-oriented Approach for Efficient Active Inference Computation and Deployment](https://arxiv.org/abs/2508.13177)
*Nikola Pižurica,Nikola Milović,Igor Jovančević,Conor Heins,Miguel de Prado*

Main category: cs.AI

TL;DR: 通过统一的稀疏计算图优化，将pymdp的灵活性与硬件高效执行相结合，大幅提升了主动推理在资源受限环境中的部署效率


<details>
  <summary>Details</summary>
Motivation: 主动推理(AIF)框架虽然为决策提供了健壮支持，但其高计算和内存开销经常导致在资源受限环境中部署困难

Method: 整合pymdp的灵活性和效率，构建了一个统一的、稀疏的计算图，该计算图专门为硬件高效执行而设计

Result: 延迟减少超2倍以上，内存使用降低达到35%，显著提升了性能

Conclusion: 该方法推进了高效AIF代理在实时和嵌入式应用中的实际部署，为资源受限环境中的复杂决策算法提供了可行解决方案
...
TL;DR: CESQL模型通过整合模型可解释性分析和执行引导策略，结合过滤调整、逻辑关联优化和模型融合，显著提升了WikiSQL数据集上WHERE子句条件值预测的准确性，减少了对条件列数据和人工标注数据的依赖。
...
Method: 整合模型可解释性分析与执行引导策略，结合过滤调整、逻辑关联精化和模型融合，设计出支持条件增强的CESQL模型。
...
### [125] [Search-Time Data Contamination](https://arxiv.org/abs/2508.13180)
*Ziwen Han,Meher Mankikar,Julian Michael,Zifan Wang*

Main category: cs.AI

TL;DR: 本文发现并定义了搜索时数据污染（STC）问题，即基于搜索的LLM智能体在评估时可能通过检索工具直接从在线平台（如HuggingFace）获取测试问题和答案，导致评估结果失真。


<details>
  <summary>Details</summary>
Motivation: 随着基于搜索的LLM智能体在问答任务中的广泛应用，评估这些智能体时存在数据泄露风险，特别是当评估数据集公开托管在在线平台上时，智能体可能通过检索直接获取答案而非通过推理，这严重影响了评估的可靠性和基准测试的有效性。

Method: 研究通过在三个常用能力基准测试（HLE、SimpleQA、GPQA）上运行搜索型智能体，分析其检索日志，识别出智能体从HuggingFace等平台直接获取测试问题和答案的情况，并通过阻断HuggingFace访问进行对比实验。

Result: 研究发现约3%的问题存在直接数据泄露，当阻断HuggingFace后，受污染子集的准确率下降约15%。实验还表明HuggingFace可能不是STC的唯一来源。

Conclusion: 研究提出了基准设计和结果报告的最佳实践建议，以应对这种新型数据泄露问题，确保搜索型LLM智能体评估的可信度，并公开了完整的实验日志以供审计。
...
### [126] [QuickMerge++: Fast Token Merging with Autoregressive Prior](https://arxiv.org/abs/2508.13204)
*Dong Liu,Yanxuan Yu*
...
<details>
  <summary>Details</summary>
Motivation: 随着生成模型扩展到更大输入规模，token级计算成本成为关键瓶颈。现有token选择方法大多是静态的、特定模态的或不兼容自回归生成。

Method: 基于注意力范数动态选择重要token，使用熵基预算估计器确定token数量，引入轻量级transformer先验在合并token序列上训练以保持自回归兼容性。
...
### [127] [AI sustains higher strategic tension than humans in chess](https://arxiv.org/abs/2508.13213)
*Adamo Cerioli,Edward D. Lee,Vito D. P. Servedio*

Main category: cs.AI

TL;DR: 棋牌中人工智能与人类在战略决策中的差异：AI能维持更高水平的战略张力，而人类因认知限制而控制游戏复杂度


<details>
  <summary>Details</summary>
Motivation: 研究人类与AI在战略决策中的差异，了解即时机会与长期目标的罕量关系

Method: 通过网络基于的棋子互动指标来定量棋局中的战略张力，比较人类vs人类和AIvsAI游戏的动态

Result: AI能在更长时间内维持更高水平的战略张力；人类游戏的张力水平在Elo分数1600和2300附近出现突变增长；AI对攻防平衡的复杂局面容忍度更高

Conclusion: AI与人类在战略决策方式存在显著差异，这可能是由于人类的认知限制和适应性策略，这些发现对于AI在复杂战略环境中的应用具有重要意义
...
### [128] [Explicit v.s. Implicit Memory: Exploring Multi-hop Complex Reasoning Over Personalized Information](https://arxiv.org/abs/2508.13250)
*Zeyu Zhang,Yang Zhang,Haoran Tan,Rui Li,Xu Chen*

Main category: cs.AI

TL;DR: 提出多跳个性化推理任务，构建数据集和评估框架，比较显式和隐式记忆方法，提出混合记忆方法HybridMem解决现有方法的局限性
...
Method: 定义多跳个性化推理任务，构建数据集和评估框架，实现多种显式和隐式记忆方法，提出结合两种范式的混合方法HybridMem

Result: 通过综合实验从多角度评估不同记忆方法的性能，分析其优缺点，证明了HybridMem方法的有效性

Conclusion: 多跳个性化推理任务具有挑战性，混合记忆方法HybridMem能够有效解决现有方法的局限性，为研究社区提供了有价值的资源和框架
...
### [129] ["DIVE" into Hydrogen Storage Materials Discovery with AI Agents](https://arxiv.org/abs/2508.13251)
...
TL;DR: DIVE多智能体工作流从科学文献图表中自动提取材料数据，显著提升数据提取准确率，建立了包含3万条数据的氢存储材料数据库，可在2分钟内完成逆向设计发现新材料
...
Method: 开发了DIVE多智能体工作流，系统性地从科学文献图形元素中读取和组织实验数据，专注于固态氢存储材料

Result: 相比多模态模型直接提取，DIVE显著提高了数据提取准确率和覆盖率，商业模型提升10-15%，开源模型提升超过30%；建立了包含4,000篇文献中30,000条数据的数据库；逆向设计工作流可在2分钟内识别未报道的氢存储成分
...
### [130] [CardAIc-Agents: A Multimodal Framework with Hierarchical Adaptation for Cardiac Care Support](https://arxiv.org/abs/2508.13256)
...
TL;DR: 这篇论文提出了一种多模态心血管病辅助诊断框架CardAIc-Agents，通过外部工具增强和适应性计划执行来充分利用AI模型的潜力，在三个数据集上都显示了更优的性能。


<details>
  <summary>Details</summary>
Motivation: 心血管病全球导致死亡的主要原因，医疗工作者严重短缺。AI助手虽有潜力完善早期识别和主动筛查，但临床应用受限于提示基于的角色分配、固化工作流程、静态知识库和固定模态输入等问题。

Method: 提出多模态框架CardAIc-Agents：1）CardiacRAG代理从可更新心血知识生成通用计划；2）主代理集成工具自主执行计划并作出决策；3）步进更新策略根据执行结果动态精炼计划；4）多学科讨论工具处理挑战性案例；5）视觉审查面板支持最终验证。

Result: 在三个数据集上的实验显示，CardAIc-Agents在效率方面超过了主流视觉-语言模型（VLMs）、最先进的代理系统以及细调的VLMs。

Conclusion: CardAIc-Agents框架通过外部工具增强和适应性计划执行，有效充分利用了AI模型的潜力，为心血管病的自动化早期识别和主动筛查提供了更有效的解决方案。
...
### [131] [Towards Unified Multimodal Financial Forecasting: Integrating Sentiment Embeddings and Market Indicators via Cross-Modal Attention](https://arxiv.org/abs/2508.13327)
*Sarthak Khanna,Armin Berger,David Berghaus,Tobias Deusser,Lorenz Sparrenberg,Rafet Sifa*

Main category: cs.AI

TL;DR: STONK是一个多模态股票预测框架，结合数值市场指标和情感分析新闻嵌入，通过特征拼接和跨模态注意力机制提升股票走势预测性能


<details>
  <summary>Details</summary>
Motivation: 解决传统分析方法中数值和文本信息孤立处理的局限性，构建统一的预测管道来改进股票日度走势预测

Method: 采用特征拼接和跨模态注意力机制融合数值指标和情感丰富的新闻嵌入，建立多模态统一预测框架

Result: 回测显示STONK优于仅使用数值指标的基线模型，提供了可扩展多模态金融预测的证据基础指导

Conclusion: 多模态融合方法在金融预测中具有显著优势，STONK框架为规模化金融预测提供了有效解决方案
...
### [132] [HiFo-Prompt: Prompting with Hindsight and Foresight for LLM-based Automatic Heuristic Design](https://arxiv.org/abs/2508.13333)
*Chentong Chen,Mengyuan Zhong,Jianyong Sun,Ye Fan,Jialong Shi*

Main category: cs.AI

TL;DR: HiFo-Prompt框架通过前瞻性和后顾性提示策略，显著提升了基于LLM的自动启发式设计性能，实现了更快的收敛速度和更好的查询效率


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的自动启发式设计方法存在静态算子和知识积累机制缺失的问题，限制了其有效性

Method: 提出HiFo-Prompt框架，包含两种协同提示策略：前瞻性提示基于种群动态自适应引导搜索，管理探索-利用权衡；后顾性提示从过往成功启发式中提炼可重用设计原则，构建持久知识库

Result: 实验结果表明HiFo-Prompt显著优于现有最先进的基于LLM的自动启发式设计方法，生成更高质量的启发式，收敛速度更快，查询效率更高

Conclusion: HiFo-Prompt通过双机制将瞬时发现转化为持久知识，使LLM能够从自身经验中学习，有效解决了现有方法的局限性
...
Method: 提出SPANER框架，采用共享提示机制作为概念锚点，使语义相关的实例在空间上汇聚，无论其模态如何。该设计具有可扩展性，支持无缝集成新模态
...
<details>
  <summary>Details</summary>
Motivation: 现实金融文档中的表格信息往往被埋没在混乱的多页碎片化表格中，99.4%的表格没有边界框，最大表格跨44页426行，传统方法难以有效处理。

Method: 使用智能代理系统执行表格检测、分类、提取和推荐，通过初始模式引导，推荐代理审查输出并建议模式修订，实现持续学习。

Result: TASER比Table Transformer等现有表格检测模型性能提升10.1%，大批量处理使可操作模式建议增加104.3%，提取持仓增加9.8%。

Conclusion: 智能模式引导提取系统在处理现实世界金融表格方面展现出巨大潜力，持续学习过程对提升性能至关重要。
...
TL;DR: 一个基于人工智能的科学发现系统能够自主完成整个科学研究流程，包括假设生成、数据收集和稿件编写，并成功执行了三个心理学实验


<details>
  <summary>Details</summary>
Motivation: 解决科学文献暴涨和领域专门化导致的知识综合困难，探索更通用的AI科学发现系统

Method: 基于域无关的代理AI系统，自主完成假设生成、在线数据收集（288名参与者）、分析管道开发和稿件编写

Result: 系统成功执行了视觉工作记忆、心理旋转和想象生动性实验，具备与经验丰富研究人员相比的理论推理和方法论严谨性

Conclusion: 这是向能够通过实验测试假设的体现式AI的一步，有助于自主探索人类认知和资源限制无法涉足的科学领域
...
TL;DR: STPFormer是一种新的时空交通预测Transformer模型，通过模式感知的时间编码、序列化空间学习、跨域对齐和多尺度融合，在多个数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于Transformer的时空交通预测模型存在时间编码僵化和时空融合能力弱的问题，需要一种能够统一学习可解释表示的解决方案。

Method: 提出STPFormer模型，包含四个核心模块：时间位置聚合器（TPA）用于模式感知时间编码，空间序列聚合器（SSA）用于序列化空间学习，时空图匹配（STGM）用于跨域对齐，以及注意力混合器用于多尺度融合。

Result: 在五个真实世界数据集上的实验表明，STPFormer始终达到新的最先进性能，消融实验和可视化结果验证了其有效性和泛化能力。

Conclusion: STPFormer通过统一的模式感知表示学习，成功解决了时空交通预测中的复杂挑战，为相关领域提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 设计一种通用的离散优化问题框架，以处理各种具有最坏情况性能要求的实际问题

Method: 定义DMMV数学问题，研究其性质，开发GPU加速贫法法，并在三个应用场景中验证

Result: 在语言模型量化中提升14%，离散断层成像锐减16%错误并加速6倍，FIR滤波器设计渐滑带波动50%

Conclusion: DMMV作为通用优化问题框架具有强大应用价值，提出的GPU加速贫法在多个领域都取得显著成效
...
TL;DR: 语言模型机器人在安全知识和实际执行之间存在显著空间，研究发现风险验证器系统可大幅减少风险行为


<details>
  <summary>Details</summary>
Motivation: 识别语言模型机器人在安全关键场景中的风险意识与安全执行能力之间的显著差距

Method: 开发了一个综合评估框架，从三个逐渐进的维度检查机器人安全性：风险知识、执行轨迹中风险识别能力、避免风险行为能力，并基于观察到的空间开发风险验证器系统

Result: 发现机器人风险知识几乎完美（>98%），但实际风险识别性能下降23%，执行风险行为率高达74%，风险验证器系统可将风险行为减少55.3%

Conclusion: 纯粹扩大模型能力或推理计算并不能解决安全问题，需要专门的风险验证机制来填补知识与执行之间的差距
...
TL;DR: 提供CrafterDojo套件，包含基础模型和工具，将Crafter环境变成轻量级、原型友好的Minecraft类似测试平台


<details>
  <summary>Details</summary>
Motivation: Minecraft环境速度慢、工程费用高，不适合快速原型开发；Crafter环境缺乏基础模型支持，限制了其在体现智能体研究中的应用

Method: 开发CrafterVPT、CrafterCLIP、CrafterSteve-1三个基础模型，分别用于行为先验、视觉-语言基础和指令跟随，同时提供数据集生成工具、参考代理实现和平台评测

Result: 构建了一个完整的CrafterDojo套件，包含所有基础模型、工具集、开源代码库

Conclusion: CrafterDojo将Crafter环境转化为一个轻量级、原型友好的测试平台，有力支持通用体现智能体研究的快速发展
...
TL;DR: 提出了EAG-RL框架，通过专家注意力引导增强LLM在电子健康记录推理中的内在能力，平均提升14.62%的性能


<details>
  <summary>Details</summary>
Motivation: 现有方法仅将LLM作为冻结的检索器，无法提升其内在推理能力，且继承了DL模型的泛化限制

Method: 两阶段训练框架：1) 专家引导的蒙特卡洛树搜索构建高质量推理轨迹；2) 强化学习对齐LLM注意力与临床显著特征

Result: 在两个真实EHR数据集上平均提升14.62%，增强了对特征扰动和未见临床领域的鲁棒性和泛化能力
...
TL;DR: 本文提出了多模态结构化强化学习(MSRL)方法，通过多粒度结构化奖励系统突破图表到代码生成任务中监督微调的性能瓶颈，在多个基准测试中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习在需要深度理解信息丰富图像并生成结构化输出的任务中应用不足，特别是图表到代码生成任务。监督微调存在性能瓶颈，需要有效的强化学习策略来适当奖励结构化输出。

Method: 构建了包含300万图表-代码对的最大训练语料库，提出MSRL方法：1）文本级基于规则的奖励验证细粒度代码细节；2）视觉级基于模型的奖励通过渲染代码成图像并使用评估模型评估结构相似性；3）采用两阶段课程学习确保训练稳定性。

Result: MSRL显著突破了SFT的性能瓶颈，在ChartMimic和ReachQA基准测试中分别将高级指标提升了6.2%和9.9%，达到了与先进闭源模型竞争的性能水平。

Conclusion: 多模态结构化强化学习方法有效解决了图表到代码生成任务中的性能瓶颈问题，通过结合文本和视觉反馈的多粒度奖励系统，显著提升了结构化输出的生成质量。
...
TL;DR: V2P方法通过抑制注意力机制和基于Fitts定律的高斯热图建模，解决了GUI元素定位中的背景干扰和中心-边缘区分问题，在ScreenSpot基准上达到92.3%和50.5%的准确率。


<details>
  <summary>Details</summary>
Motivation: 传统GUI元素定位方法忽略空间交互不确定性和视觉语义层次结构，现有注意力方法存在背景区域干扰导致注意力漂移，以及均匀标注无法区分目标UI元素中心与边缘的问题。
...
TL;DR: 提出了基于神经查询重排器（NQR）的方法，用于在不完整知识图谱中处理包含软约束的查询，通过交互式学习用户偏好来优化答案排序。


<details>
  <summary>Details</summary>
Motivation: 现有查询回答方法主要关注一阶逻辑形式化查询，但实际应用中很多查询包含模糊或上下文相关的软约束（如属性偏好、相关类别偏好），需要新的方法来解决这类问题。

Method: 提出神经查询重排器（NQR），通过交互式学习用户提供的偏好和非偏好实体示例，调整查询答案的评分，在不破坏原始查询答案的基础上融入软约束。

Result: 实验表明NQR能够有效捕获软约束，同时保持稳健的查询回答性能。扩展了现有QA基准，生成了包含软约束的数据集进行验证。

Conclusion: 该方法成功解决了知识图谱查询中软约束的集成问题，为处理现实世界中模糊和上下文相关的查询需求提供了有效解决方案。
...
TL;DR: 基于实例迁移学习的LIME改进框架(ITL-LIME)，利用相关源域实例提高黑盒模型解释的准确性和稳定性


<details>
  <summary>Details</summary>
Motivation: LIME方法在训练数据有限时存在随机性问题，导致本地性和不稳定性问题，甚至产生远离真实数据流形的样本

Method: 通过聚类把源域分成具有代表性原型的群集，获取与目标实例最相似的相关源实例，结合对比学习编码器作为权重机制，使用加权的源和目标实例训练代理模型

Result: 提高了在数据约束环境下解释的信仰度和稳定性

Conclusion: ITL-LIME框架通过利用相关源域实例有效解决了LIME在数据稀缺情况下的本地性和不稳定性问题
...
TL;DR: 知识图语在家庭动作场景中的链接预测问题，传统算法表现差强不如简单基准方法


<details>
  <summary>Details</summary>
Motivation: 家庭动作知识图在家庭机器人控制和视频分析中具有重要价值，但视频提取信息存在不完整性问题，需要通过知识图补全来提升情境感知

Method: 研究了情境知识图的特殊特征，对比了标准链接预测算法在该领域的表现

Result: 发现传统链接预测算法在情境知识图中表现差强，甚至无法超过简单的基准方法

Conclusion: 情境知识图具有特殊性质，需要专门的链接预测方法来应对家庭动作领域的挑战
...
TL;DR: MHSNet是一个多层级身份验证框架，用于检测第三方网站获取的简历与公司人才库中现有简历之间的重复项，通过微调BGE-M3和使用混合专家模型来处理简历的语义复杂性、结构异质性和信息不完整性。


<details>
  <summary>Details</summary>
Motivation: 第三方网站获取的简历往往不完整且不准确，需要通过重复检测来提高简历质量并丰富公司人才库，但由于简历文本的语义复杂性、结构异质性和信息不完整性，这一任务具有挑战性。

Method: 提出MHSNet框架，使用对比学习微调BGE-M3，通过混合专家模型生成多层级稀疏和密集表示来计算多层级语义相似度，并采用状态感知的混合专家模型处理不完整简历。
...
TL;DR: 这篇论文综述了使用神经符号方法提升大语言模型推理能力的最新进展，从三个角度分析了相关技术，并讨论了未来挑战和方向。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽在多份任务中表现出艰，但其推理能力仍是重要挑战，而神经符号方法被认为是提升LLM推理能力的有效途径。

Method: 论文采用综述方式，首先形式化推理任务并介绍神经符号学习范式，然后从三个角度（符号->LLM、LLM->符号、LLM+符号）讨论提升LLM推理能力的方法。

Result: 论文系统性地总结了最新的神经符号推理方法，并开源了包含相关论文和资源的GitHub仓库，为进一步研究提供了基础。

Conclusion: 神经符号方法在提升LLM推理能力方面具有强大潜力，这项研究为实现人工智能的关键里程碑做出了贡献，同时也指出了未来的研究方向。
...
Method: DeepLog包含两个关键组件：1）DeepLog语言 - 基于注释神经扩展的接地一阶逻辑，用于指定神经符号模型和推理任务；2）计算层面的扩展代数电路作为计算图。这两个组件构成了神经符号抽象机器。
...
TL;DR: CausalPlan是一个将显式因果推理集成到LLM规划过程中的两阶段框架，通过结构因果行动模型学习因果图来指导行动选择，减少无效行为并改善多智能体协作。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型智能体（尤其是较小的开源模型）在协作任务中经常产生因果无效或不连贯的行动，因为它们依赖表面相关性而非基于因果推理，这限制了它们在动态环境中的协调和规划能力。

Method: 提出CausalPlan两阶段框架，核心是结构因果行动（SCA）模型，从智能体轨迹中学习因果图，捕捉先前行动和当前环境状态如何影响未来决策。该结构用于指导行动选择，为LLM生成的提案分配因果分数并重新加权，或在需要时回退到因果基础的替代方案。

Result: 在Overcooked-AI基准测试的五个多智能体协调任务和四个不同规模的LLM（Gemma-7B、Llama-8B、Qwen-14B和Llama-70B）上评估，CausalPlan持续减少无效行动并改善AI-AI和人类-AI设置中的协作，优于强化学习基线。
...
Method: 两阶段框架：1）专家感知代理招募 - 构建LLM专业表动态选择最优LLM；2）置信度和对抗驱动的多代理协作 - 生成带置信度评分的响应并通过置信融合和对抗验证整合

Result: 在MMLU-Pro-Health数据集上达到74.45%准确率，比最佳闭源模型GPT-4-0613提升2.69%，在三个公开MDM数据集上均优于现有单LLM和多LLM方法

Conclusion: EMRC框架通过专家感知的代理招募策略和代理互补性，有效利用了各LLM的专业能力，显著提升了医疗决策系统的诊断性能
...
TL;DR: 本文提出一种基于概率上下文自由语法的动态学习方法，通过维护一个语言模型来生成类似于历史成功实例的新项，以平衡排理论中的利用和探索。


<details>
  <summary>Details</summary>
Motivation: 量化公式对SMT求解器构成重大挑战，现有的实例化技术常常相互补充，需要一种能够动态学习并结合多种方法的新方法来提高求解效率。

Method: 将观察到的实例化作为潜在语言的样本，使用概率上下文自由语法来生成类似的新项。方法不仅模仿历史成功实例，还可选择性地逆转学习到的项概率来探索多样性。

Result: 该方法能够动态学习并结合多种现有实例化技术，在量词排理论中实现利用和探索的平衡，为SMT求解器提供了一种新的实例化探索策略。

Conclusion: 通过概率语言模型动态学习并生成类似实例的方法，有效地结合了现有实例化技术的优势，为处理量化公式的未可判性问题提供了一种有前景的解决方案。
...
TL;DR: 本文系统性研究了多个RAG系统的集成方法，从理论和机制角度分析了集成框架，通过信息熵理论解释集成效果，并在管道和模块层面进行实验验证。


<details>
  <summary>Details</summary>
Motivation: 当前单一RAG框架无法良好适应广泛的下游任务，需要研究如何利用多个RAG系统的优势来提升性能。

Method: 从理论和机制两个角度分析RAG集成框架：理论方面通过信息熵理论解释；机制方面从管道层面（4种流程）和模块层面（3种模块）进行实验，解决7个研究问题。

Result: 实验结果表明，无论在管道层面还是模块层面，聚合多个RAG系统都具有良好的通用性和稳健性。
...
TL;DR: 通过生成伪代码并自动调试、添加反思步骤以及生成多个程序变体选择最佳方案，改进了基于LLM的广义计划生成方法的质量和可靠性。


<details>
  <summary>Details</summary>
Motivation: 解决之前方法中只生成一个策略并直接转换为程序，如果策略错误将导致整个广义计划失败的问题。

Method: 使用伪代码形式生成策略并自动调试，在Python调试阶段添加反思步骤找出失败原因，生成多个程序变体选择最佳解。

Result: 在17个标准领域中，新方法显著提高了广义计划的质量，其中12个领域的最佳Python程序能解决所有可生成的任务。

Conclusion: 通过伪代码调试、反思和多变体选择的维度，显著提升了LLM生成广义计划的效果和可靠性。
...
TL;DR: TS-Agent是一个模块化的智能体框架，用于自动化金融时间序列建模工作流，通过模型选择、代码优化和微调三阶段决策过程，显著优于现有AutoML和智能体基线方法。
...
Method: 设计了三阶段结构化迭代决策流程：模型选择、代码优化和微调，配备规划智能体和结构化知识库，包含精选模型库和优化策略库，通过上下文推理和实验反馈指导探索。
...
TL;DR: 研究发现AI驱动的协作代理在供应链中会出现"协作悖论"，即理论上更优的协作AI代理表现反而比非AI基准更差，原因是库存囤积导致系统饥饿。通过高层AI策略制定和低层协作执行的结合，才能实现系统稳定性。
...
Method: 使用基于大语言模型的生成式AI代理在受控供应链模拟中进行计算实验，设计包含供应商管理库存原则的协作AI代理，并分析其行为倾向。

Result: 发现了"协作悖论"现象，协作AI代理表现更差；识别出库存囤积的操作缺陷；开发了结合高层AI策略制定和低层协作执行的最终框架，能够自主生成、评估和量化可行的战略选择组合。

Conclusion: 研究揭示了协作AI代理的新兴行为特征，为设计稳定有效的AI驱动商业分析系统提供了重要蓝图，强调了分层策略制定与执行结合的必要性。
...
TL;DR: 这篇论文研究了如何通过微调大语言模型（LLMs）来为专家提供PyChrono模拟工具的虚拟助手，生成质量提升的模拟脚本并降低使用门槛。


<details>
  <summary>Details</summary>
Motivation: 研究考虑能否通过微调大语言模型来将其制作成为帮助专家有效使用PyChrono模拟工具的虚拟助手，降低专业模拟工具的使用门槛。

Method: 提出了一个框架，用于微调和定制开源及闭源LLMs，通过一系列过程来提升生成的PyChrono模拟脚本的质量。这些脚本涉及从简单的单摆到复杂的车辆在可变形地形上的模拟实验。

Result: 微调后的LLMs在生成PyChrono模拟脚本时质量有显著提升，虽然并非完美，但经常可以作为用户修改和改进的良好基础。模型还能回答PyChrono API相关问题和推荐建模方法。

Conclusion: 该框架具有普遍性，可以应用于其他应用领域的模拟工具，有效降低专业模拟工具的使用门槛，提高专业人员的工作效率。
...
<details>
  <summary>Details</summary>
Motivation: LRS问题是一个NP难组合优化问题，在生物信息学和基因组重新组装中具有重要作用，需要高效算法来解决。

Method: 提出使用BRKGA算法，重点关注个体评估的计算效率，将灰度值向量转换为有效解。与Max-Min蚂蚁系统和CPLEX整数规划汇编器进行对比。

Result: 计算结果显示BRKGA在LRS问题上达到了当前最高水平，在计算效率和解质量方面都表现优异。

Conclusion: BRKGA是解决LRS问题的一种高效方法，但对于基于大字母表的输入字符串，算法仍有改进空间。
...
TL;DR: ComputerRL是一个自主桌面智能框架，通过API-GUI范式统一程序化API调用和直接GUI交互，解决了机器代理与以人为中心的桌面环境之间的不匹配问题。该框架采用分布式RL基础设施和Entropulse训练策略，在OSWorld基准测试中达到了48.1%的最新准确率。
...
Method: 提出API-GUI范式统一程序化API调用和GUI交互；开发分布式RL基础设施支持大规模并行虚拟桌面环境；设计Entropulse训练策略（强化学习与监督微调交替）缓解熵崩溃问题。

Result: 基于GLM-4-9B-0414的AutoGLM-OS-9B在OSWorld基准测试中达到48.1%的最新准确率，显著提升了桌面自动化通用代理的性能。

Conclusion: ComputerRL框架通过创新的API-GUI范式和可扩展的训练基础设施，为桌面智能代理提供了有效的解决方案，在复杂数字工作空间操作方面取得了显著进展。
...
TL;DR: BERT-VQA模型在图表视觉问答任务中表现不如预期，跨模态模块未能有效对齐图表组件与问题短语
...
Method: 开发基于VisualBERT的BERT-VQA模型，使用预训练的ResNet 101图像编码器，可能加入联合融合模块，与基于LSTM、CNN和浅层分类器的基线模型对比

Result: 最终结果推翻了核心假设，即VisualBERT中的跨模态模块对于对齐图表组件和问题短语并不关键

Conclusion: 研究揭示了图表问答任务的难度，以及不同模型架构在解决该问题时的适用性，为后续研究提供了宝贵见解
...
TL;DR: 基于CNN和LLM的多模态情感分析方法，通过上下文注意力机制融合文本和图像信息，在灾难管理场景中实现了更高的分类准确度


<details>
  <summary>Details</summary>
Motivation: 在自然灾难期间，通过社交媒体多模态数据理解公众情感对危机管理至关重要，但传统方法对文本和图像模态的分离处理效果有限

Method: 结合CNN图像分析和LLM文本处理，使用GPT和prompt工程提取特征，通过上下文注意力机制模型模态间关系，深度神经网络学习融合特征

Result: 在CrisisMMD数据集上实验结果显示，模型在多种自然灾难场景下对社交媒体数据的信息性分类达到了准确率提升2.43%，F1指标提升5.18%

Conclusion: 该方法不仅在数量指标上显著提升，还能揭示灾难期间深层情感信息，为AI驱动的实时灾难管理提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 物理信息神经网络在求解微分方程方面展现出巨大潜力，但其性能受多种因素影响，特别是训练点分布策略对解的精度的具体影响尚未得到系统研究。

Method: 使用两个常微分方程和两个偏微分方程，测试五种训练数据生成策略（包括新提出的基于正弦的分布），采用浅层网络架构（1-2个隐藏层），并考虑随机和固定种子权重初始化以确保可复现性。

Result: 研究发现训练点分布对解精度有显著影响，且这种影响与微分方程的特性相关。基于正弦的分布方法在某些情况下表现出优势。

Conclusion: 训练点分布是物理信息神经网络性能的关键因素，需要根据具体微分方程特性选择合适的分布策略，基于正弦的分布方法是一个有前景的选择。
...
Conclusion: DGNPP有效解决了先前方法的局限性，通过同时考虑网络拓扑和时序动态性，实现了更准确的时间交互网络建模
...
TL;DR: 使用递归神经网络对教育中广泛使用的S-P图进行聚类，通过网络动态定点和吸引域实现大规模数据的分解和聚类


<details>
  <summary>Details</summary>
Motivation: 随着学生数量增加，S-P图变得难以处理，需要一种有效的方法将大规模图表分解为较小的子图表进行聚类

Method: 基于通过递归神经网络的多重固定点和吸引域机制，将网络动态作为聚类基础，每个吸引域对应一个子聚类

Result: 通过基础实验验证了方法的有效性，并提出了平均警戒指数作为评估聚类性能的重要特征量

Conclusion: 该方法能够有效地对大规模S-P图进行聚类分解，通过神经网络动态特性实现了自动化的子图表生成
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在复杂图像标注任务（如情感分类和上下文驱动的目标检测）中表现不佳，标准监督微调只关注标注结果而忽略推理过程，Visual-RFT由于缺乏高质量验证的思维链而产生不一致的推理。

Method: RISE包含两个阶段：1) Reason阶段（RISE-CoT）：使用强化学习的"标注-推理-标注"闭环生成视觉基础、逻辑一致的思维链；2) Inspire和Strengthen阶段（RISE-R1）：利用RISE-CoT奖励筛选的高质量CoT子集进行监督微调，然后进行强化微调。

Result: 在复杂和简单图像标注任务上评估，RISE训练的Qwen2-VL-2B模型超越了SFT和Visual-RFT方法，实现了鲁棒性能和增强的可解释性。

Conclusion: RISE提供了一个无需手动标注思维链的自监督解决方案，可显著提升视觉语言模型的推理能力。
...
Method: 首先使用稀疏回归算法识别系统，然后通过李导数对输出函数字典应用来设计反馈控制器，确保无内部动力学被观测

Result: 该方法结合堆叠回归算法和相对度条件，能够发现并反馈线性化物理模型的真实控制方程

Conclusion: 相比现有工作，本文提出的方法在系统识别和反馈线性化方面具有新颖性和优势
...
TL;DR: 通过物理模拟实现物理可行的数据增帽(PPDA)，在传感器基于人体活动识别中比传统信号变换方法提升3.7个pp平均F1分数提升，并能在训练数据减少60%的情况下达到竞争力表现


<details>
  <summary>Details</summary>
Motivation: 传感器基于人体活动识别领域缺乏高质量标签数据，而传统的信号变换数据增帽方法往往物理不可行，导致增帽数据失真

Method: 利用物理模拟技术，基于动作捕捉或视频姿势估计的人体运动数据，通过修改身体运动、传感器置位和硬件相关效果来实现物理可行的数据增帽

Result: 在3个公开数据集上，PPDA方法比传统STDA方法平均提升宏观F1分数3.7个pp(最高达13pp)，并能在训练主体数量减少60%的情况下达到竞争力表现

Conclusion: 物理可行数据增帽在HAR领域具有显著优势，物理模拟技术为生成合成IMU数据提供了成本效益高的可扩展方案，有助解决标注数据缺乏的挑战
...
TL;DR: 提出协作匹配系统comatch，通过算法选择最有信心的决策，其余交由人类决策者处理，实现人机互补，在匹配任务中表现优于单独使用人类或算法
...
Method: 提出协作匹配方法comatch，算法只处理最有信心的匹配决策，其余决策推迟给人类决策者，并优化算法和人类决策的比例以最大化性能
...
TL;DR: 提出了分层保形分类(HCC)，将类别层次结构融入保形预测框架，在保持覆盖保证的同时生成包含不同层次节点的预测集


<details>
  <summary>Details</summary>
Motivation: 标准保形预测将类别视为平坦无结构的，忽略了类别标签之间的语义关系或层次结构等领域知识
...
TL;DR: 提出两种约束条件下的流匹配采样方法：当约束集有可微距离函数时，在目标函数中添加惩罚项；当约束集只能通过成员查询时，使用随机化学习均值流。方法在保持目标分布的同时显著提高约束满足率。


<details>
  <summary>Details</summary>
Motivation: 解决在流匹配生成过程中需要满足特定约束条件的问题，特别是在约束集只能通过查询成员关系（而非显式函数）的复杂场景下。

Method: 对于可微距离函数情况：在FM目标中添加约束距离惩罚项；对于成员查询情况：使用随机化学习均值流，并采用两阶段方法提高计算效率。

Result: 通过多个合成约束生成案例的数值实验显示，所提方法在保持目标分布的同时，在约束满足率方面取得显著提升。

Conclusion: 提出的方法能够有效处理两种不同类型的约束条件，为约束生成问题提供了新的解决方案，特别是在黑盒分类器对抗样本生成等实际应用中具有潜力。
...
TL;DR: 该论文探讨了在部分可观测性条件下机器语言习得的问题，提出了一个基于学习的方法来推断私有信息以促进语言习得。


<details>
  <summary>Details</summary>
Motivation: 传统语言习得模型假设学习者能够观察到所有相关信息，但现实中存在部分可观测性，即学习者需要从环境、动作和发送的消息中推断缺失信息。

Method: 提出了一个学习型算法，通过在玩具设置中演示解决方案，并正式探索在更一般设置中出现的挑战，来解码私有信息以促进语言习得。

Result: 论文展示了在部分可观测性条件下语言习得问题的解决方案，并提出了有效的解码方法。

Conclusion: 放松完全可观测性假设为语言习得提出了更具挑战性的设置，但通过适当的学习算法可以成功解决这类问题。
...
TL;DR: 这篇论文提出了一种基于动态图构建和空间时间注意力机制的新框架，用于自闭谱障碍(ASD)的fMRI分类诊断，在ABIDE数据集上较静态方法显著提升了精度。


<details>
  <summary>Details</summary>
Motivation: 现有的fMRI分类方法多基于静态功能连通性，无法全面抓取空间时间关系，需要更好地理解神经活动动态。

Method: 使用基于transformer的注意力机制动态推断大脑功能连通性，构建时变图并通过GCN和transformer处理，抓取局部交互和全局时间依赖关系。

Result: 在ABIDE数据集子集上达到63.2%的准确率和60.0%的AUC，超过静态图方法(GCN:51.8%)。

Conclusion: 动态连通性与空间时间上下文的联合建模对fMRI分类有效，核心新颖性在于注意力驱动的动态图创建和层次空间时间特征融合。
...
TL;DR: X-MoE是一个针对新一代MoE架构的高效训练系统，通过跨平台内核、冗余绕过调度和混合并行等技术，在AMD GPU上实现了5450亿参数模型的规模化训练，比现有方法大10倍
...
Result: 在Frontier超算（AMD MI250X GPU）上成功扩展到1024个GPU训练5450亿参数模型，吞吐量保持高水平

Conclusion: X-MoE系统显著提升了MoE模型在非NVIDIA平台上的训练规模和效率，为下一代MoE架构提供了可扩展的解决方案
...
TL;DR: 该论文提出了一种基于线性代数的维度下界证明方法，用于分析解决L²函数逼近问题的线性方法，并将该方法应用于核方法的样本量下界推导。


<details>
  <summary>Details</summary>
Motivation: 为线性方法在L²函数逼近问题中提供维度下界的理论分析工具，特别是针对核方法的样本复杂度研究。

Method: 采用线性代数论证方法，该方法在文献中已有先例（如Barron, 1993），用于建立Kolmogorov n-宽度的下界，并将其扩展到核方法的样本量下界分析。

Result: 成功推导出了核方法在L²函数逼近中的样本量下界，为理解核方法的理论性能提供了新的数学工具。

Conclusion: 线性代数方法为分析线性逼近方法的维度下界提供了有效的理论框架，特别是在核方法的样本复杂度分析中具有重要应用价值。
...
TL;DR: ODE-Diff是一个时间序列扩散框架，通过结合专家模型指导来改进反事实分布预测，在数据稀缺情况下表现优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现有方法在数据稀缺时往往失效，需要结合机理模型和数据驱动方法来提高复杂动态系统中反事实分布预测的可靠性

Method: 提出ODE-Diff框架，利用时间序列扩散模型，从非完美专家模型中提取高层信号作为结构化先验，桥接机理方法和数据驱动方法

Result: 在半合成COVID-19模拟、合成药理学动态和真实案例研究中，ODE-Diff在点预测和分布准确性方面均优于强基线方法

Conclusion: ODE-Diff通过结合专家模型指导的扩散方法，实现了更可靠和可解释的因果推理，为科学建模和决策提供了有效工具
...
<details>
  <summary>Details</summary>
Motivation: 未来轨迹预测在自动驾驶、飓风预测、流行病建模等领域很重要，但现有方法生成的轨迹集合通常未经校准，缺乏不确定性量化

Method: 基于共形预测的统一框架，引入新颖的在线更新步骤和优化步骤来捕捉步间依赖关系，生成不连续的预测区间

Result: 能够产生围绕每个轨迹的不连续预测区间，自然捕捉时间依赖性，提供更尖锐、更自适应的不确定性估计

Conclusion: 该方法为未来轨迹预测提供了理论保证的校准不确定性量化框架，优于传统方法
...
TL;DR: 这是一个统一框架J3O，通过聚合汇优化模型加载和查询路由，在边缘设备上实现多任务并发推理的准确性最大化。


<details>
  <summary>Details</summary>
Motivation: 现有框架主要关注单任务单模型场景，而实际应用如自动驾驶需要并发执行多种任务，需要在资源受限的边缘设备上实现更高效的协同推理。

Method: 提出J3O算法，通过混合整数规划形式化问题，采用交替优化策略：(i)利用拉格朗日松弛次模满优化谭式选择要加载的模型，(ii)通过约束线性规划确定最优路由。还支持边缘批处理。

Result: 实验结果显示J3O在多任务测试中一致达到超过97%的最优准确性，同时运行时间仅需最优求解器的15%以下。

Conclusion: J3O框架能够高效地在边缘设备上实现多任务并发推理，在保持高准确性的同时显著提升性能效率。
...
TL;DR: 通过时间-频率分析和空间相关性评估，证实鸣声事件可以准确定位痒病发作起源区，尤其在手术成功病例中表现优异


<details>
  <summary>Details</summary>
Motivation: 开发一种量化框架来评估临床定义的痒病发作起源区与通过时间-频率分析识别的统计异常通道之间的空间一致性

Method: 采用两步流水线：(1)无监督异常检测，使用局部异常因子(LOF)分析识别异常通道；(2)空间相关性分析，计算准确匹配指标和加权指数相似性

Result: LOF方法有效检测异常通道，加权指数匹配在SOZ定位中表现更优。无痒病发作病人(Index Precision均值:0.903)和手术成功病例(Index Precision均值:0.865)表现最好，失败案例一致性较低(Index Precision均值:0.460)

Conclusion: 鸣声基础的异常检测结合加权空间指标，为SOZ定位提供了补充方法，尤其在手术成功病例中显示出良好效果
...
TL;DR: NovoMolGen是一个基于Transformer的分子大语言模型，通过系统研究文本表示、分词策略、模型规模和数据集规模等关键因素，在15亿分子上预训练，在分子生成任务上达到新的最先进水平。
...
Method: 引入NovoMolGen系列Transformer基础模型，在15亿个分子上进行预训练，系统研究文本表示、分词策略、模型规模和数据集规模等关键因素对分子生成性能的影响。

Result: 发现预训练期间性能指标与下游实际性能之间相关性较弱，揭示了分子训练动态与一般NLP训练的重要区别。NovoMolGen在无约束和目标导向分子生成任务中都显著优于先前的Mol-LLMs和专门生成模型。

Conclusion: NovoMolGen为推进高效有效的分子建模策略提供了坚实基础，建立了分子生成的新最先进结果。
...
TL;DR: 网络化上下文线性手前动问题的两种新算法，通过动态网络权重进行适应性信息共享，在降低通信成本的同时提升学习效果


<details>
  <summary>Details</summary>
Motivation: 传统上下文线手前动问题假设数据要么完全集中要么完全隔离，在网络化环境中部分信息共享的情况下研究较少

Method: 提出NetLinUCB和Net-SGD-UCB两种网络意识的UCB算法，将学习分解为全局和本地组件，通过动态更新网络权重来指导信息共享

Result: 算法通信成本较低，只需共享同质特征的计算摘要；遗憾界将共享结构的学习复杂度从O(N)降到次线性O(√N)，其中N为网络规模

Conclusion: 两种算法具有互补优势：NetLinUCB在低噪声和细粒度异质性情况下表现优异，Net-SGD-UCB在高维高方差上下文中更加稳健
...
Result: MAVIS在性能上优于为每个目标微调模型并事后组合的基线方法，甚至接近为用户精确偏好微调模型的理想化设置

Conclusion: MAVIS提供了一种计算效率高且灵活的多目标对齐方法，能够在推理时动态调整LLM行为而不需要重新训练
...
TL;DR: 事件感知非稳态时间序列预测框架EventTSF，通过流匹配模型整合文本事件信息，在8个数据集上超12个基线方法，预测准确性提升10.7%，训练效率提升1.13倍


<details>
  <summary>Details</summary>
Motivation: 现有时间序列预测方法主要依靠单一模态，无法有效利用文本事件信息来改善非稳态预测性能，存在细粒度同步、时间不确定性和多分辨率对齐等挑战

Method: 提出EventTSF自回归生成框架，使用流匹配模型来捕捉时间-事件细微交互。根据事件语义信号自适应控制流匹配时间步，采用多模态U形变换器作为去噪器，在不同分辨率上高效融合时间和文本模态

Result: 在8个合成和真实数据集上，EventTSF在多样化事件感知非稳态预测场景中超过12个基线方法，预测准确性提升10.7%，训练效率提升1.13倍

Conclusion: EventTSF有效解决了多模态时间序列预测中的三大挑战，通过流匹配模型实现了时间事件细粒度交互，为能源、交通等预测领域提供了更准确的多模态预测方案
...
TL;DR: SVDformer是一种新的方向感知图表示学习框架，结合SVD和Transformer架构，通过关注权重模型多尺度入边/出边模式相互作用，显式保留边的方向性。
...
Method: 首先通过多头自注意力精炼奇异值嵌入，自适应地增强关键的谱组件并压制高频噪声，实现可学习的低通/高通图滤波。然后将奇异向量作为方向投影基础，奇异值作为缩放因子，利用Transformer通过注意力权重模型多尺度入边/出边模式相互作用。

Result: 在6个有向图标准数据集上的实验表明，SVDformer在节点分类任务上一贯地超过了最先进的GNN和方向感知基线模型。

Conclusion: SVDformer为有向图上的表示学习建立了一个新的范式，通过结合SVD和Transformer架构有效地解决了方向性和全局结构投影的关键挑战。
...
TL;DR: 本文提出一种基于元学习的方法，通过历史元知识动态设计AutoML的搜索空间，大幅缩短运行时间和缩小搜索空间，而不影响预测性能。
...
Method: 使用历史元知识选择有前景的搜索空间区域，动态设计搜索空间，加速优化过程。

Result: 在Random Search中减少89%运行时间，搜索空间缩小到原来的1.8/13(预处理器)和4.3/16(分类器)，且在Auto-Sklearn中也显示竞争性能。

Conclusion: 该元学习方法能够有效解决AutoML系统的计算成本和过拟合问题，为自动化机器学习提供了更高效的搜索策略。
...
Method: 提出ASAP方法，动态调整学习率：计算当前和先前softmax输出的余弦距离，并将其映射到有界范围内。仅需使用先前的softmax输出，无需标签、模型集成或历史输入。
...
Conclusion: HCAL分类器在保持层次结构一致性和优化平衡方面表现出色，显著提升了层次多标签分类性能
...
TL;DR: 使用t-SNE可视化chirp特征，通过机器学习分类器在三个临床任务上取得最高88.8%的准确率，并结合SHAP解释提供可解释的嵌入分析


<details>
  <summary>Details</summary>
Motivation: 开发一个可解释的视觉化管道来分析chirp特征在不同临床结果场景中的表现，为临床分层和决策支持提供见解
...
TL;DR: DyMixOp是一种新颖的神经算子框架，通过惯性流形理论将无限维非线性PDE动力学转换到有限维潜在空间，利用局部-全局混合变换有效捕捉细节和非线性交互，在多种PDE基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决神经网络近似非线性PDE动力学的核心挑战，特别是处理不可线性化动力学和需要无限维空间进行线性化的问题。

Method: 基于惯性流形理论，提出局部-全局混合(LGM)变换，受湍流对流动力学启发，构建动力学感知架构连接多个LGM层来近似线性和非线性动力学。

Result: 在多种PDE基准测试中实现最先进性能，显著降低预测误差（在对流主导场景中高达86.7%），同时保持计算效率和可扩展性。

Conclusion: DyMixOp框架成功解决了非线性PDE动力学的近似问题，通过理论驱动的变换方法有效捕捉系统本质特征，为复杂动力系统的神经网络建模提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 神经网络在粒子轨迹预测中应用广泛，但缺乏有效的不确定性量化和可视化方法，这限制了模型在需要高可靠性应用中的可信度。

Method: 结合Deep Ensembles、MC Dropout和SWAG等成熟的不确定性量化技术，设计并实现了超椭圆管状结构来捕获和展示非对称不确定性。

Result: 在合成和仿真数据集上验证了uncertainty tube的实用性和计算效率，能够有效表示预测不确定性。

Conclusion: uncertainty tube提供了一种计算高效且直观的不确定性可视化方法，增强了神经网络预测粒子轨迹的可信度和可靠性。
...
TL;DR: 这章论运算法不透明性的两种类型：技术复杂性导致的本质不透明和商业保密导致的意图不透明，分析了各自的伦理启示，并探讨了解释性AI方法面临的挑战。


<details>
  <summary>Details</summary>
Motivation: 解决复杂机器学习算法的不透明性问题，这是人工智能象征性发展的主要障碍之一。需要明确区分不同类型的不透明性及其伦理影响，并探索充分解释的方法。

Method: 分析两种不透明性类型：一是技术复杂性导致的本质不透明（如神经网络的多层隐藏节点），二是商业保密导致的意图不透明。探讨各种解释性AI方法的发展和挑战。

Result: 识别了不同类型不透明性的特征和伦理启示，并展示了解释性AI技术在应对技术不透明性方面的进展。

Conclusion: 算法不透明性有多种根源，需要有针对性地处理。解释性AI技术虽然发展迅速，但仍面临着许多挑战，需要进一步研究和提升。
...
TL;DR: 开发了MuFlex平台，一个用于多建筑灵活性协调控制策略基准测试的开源平台，解决了现有测试平台在物理细节捕获和标准化方面的不足。


<details>
  <summary>Details</summary>
Motivation: 随着可再生能源在电网中渗透率增加，需要建筑群提供协调的需求灵活性。现有测试平台大多针对单建筑，多建筑平台有限且依赖简化模型，无法充分捕获物理细节和中间变量，限制了作为基准测试工具的适用性。

Method: 开发了MuFlex平台，支持EnergyPlus建筑模型间的同步信息交换，遵循最新OpenAI Gym接口，提供模块化、标准化的强化学习实现。使用Soft Actor-Critic算法对四个办公楼进行需求灵活性协调案例研究。

Result: 聚合四个建筑的灵活性将总峰值需求降低到指定阈值以下，同时保持了室内环境质量。

Conclusion: MuFlex平台为多建筑灵活性协调控制策略提供了有效的基准测试工具，证明了建筑群协调控制在实际应用中的可行性。
...
TL;DR: CALYPSO是一个混合框架，结合神经网络和机制性元种群模型来预测MRSA在医疗和社区环境中的传播动态，相比机器学习基线方法性能提升4.5%以上
...
Result: CALYPSO在州级预测性能上比机器学习基线方法提升超过4.5%，能够识别高风险区域并为感染预防资源分配提供成本效益策略

Conclusion: CALYPSO框架能够提供准确、可解释的多空间分辨率预测，支持感染控制政策和爆发风险的反事实分析，是解决MRSA传播预测的有效方法
...
TL;DR: 这篇论文提出了一种新的CROC方法，用于合并分析常见和稀有变异体来提高疾病风险预测的准确性，解决传统方法在稀有变异体分析中的不足。


<details>
  <summary>Details</summary>
Motivation: 目前基于常见基因变异体的风险预测模型准确性不足，而稀有变异体在风险预测中的潜力还未充分开发，需要一种能同时处理常见和稀有变异体的综合方法。

Method: 提出了摘取接收者操作特征曲线（CROC）方法，这是一种在之前前向ROC（FROC）方法基础上的扩展，特别添加了处理稀有变异体的流程。使用了遗传分析工作坊17小外显子数据集中的533个SNP进行评估。

Result: 基于所有SNP的预测模型（AUC=0.605）比仅使用常见变异体的模型（AUC=0.585）更准确。当常见变异体数量减少时，CROC方法表现更好，在只有稀有变异体的极端情况下，CROC的AUC为0.603，而FROC只有0.524。

Conclusion: CROC方法能够有效地合并常见和稀有变异体的信息，显著提高风险预测的准确性，尤其是在稀有变异体为主的情况下。这为完善基因风险预测提供了重要方法学支持。
...
TL;DR: 提出GenHAI生成概率模型，用于建模住院患者MRSA检测结果序列，帮助医院管理者降低MRSA感染风险
...
<details>
  <summary>Details</summary>
Motivation: 现有的自监督对比学习方法缺乏统一的理论框架，且难以在不使用标签的情况下保证特征空间的类内紧凑性和类间分离性。

Method: 提出GLF框架统一BYOL、Barlow Twins和SwAV等方法，并设计ADC方法通过动态捕捉样本间关系来增强特征空间的判别性。

Result: 理论分析和实验评估都证明了ADC方法的优越性，能够有效提升特征表示质量。

Conclusion: GLF框架为自监督对比学习提供了统一视角，ADC方法通过自适应分布校准有效解决了无标签情况下的类内紧凑性和类间分离性问题。
...
TL;DR: 通过在量化参数空间进行概率推理，利用概率电路学习连续分布的离散近似，实现高效的推理效果且保持准确性。
...
Method: 使用概率电路进行可解析的学习方法，考虑二维密度和量化神经网络两种情况。

Result: 在多种模型上验证了方法的有效性，在不牺牲准确性的前提下实现了高效的推理。

Conclusion: 该工作通过利用离散近似进行概率计算，推进了可缩放和可解释的机器学习方法。
...
TL;DR: 这篇论文系统性比较了多种因果推断中的部分识别方法，提出了一个扩展的熵推断绑定算法，并开源了一个统一的Python工具包来支持实践应用。


<details>
  <summary>Details</summary>
Motivation: 因果推断通常依赖于强假设，而部分识别方法能够提供更现实的不确定性边界。但由于方法分散和缺乏实践指南，这些方法在应用中使用不足。

Method: 实现、扩展并统一了多种最新的绑定算法，包括符号、优化和信息论方法。特别扩展了熵推断绑定方法以处理反事实查询。通过千个随机模拟实验评估方法的性能。

Result: 在离散和连续数据生成过程中进行了大量实验，评估了各种方法在绑定紧凑性、计算效率和对假设违反的鲁棒性方面的表现。

Conclusion: 研究结果凭化为实践选择算法的决策树，并训练了机器学习模型来预测最佳方法。开源的CausalBoundingEngine工具包为实践者提供了统一的方法比较和应用接口。
...
TL;DR: FedOL是一个联邦学习框架，通过知识蒸馏在单轮通信中构建更大的服务器模型，使用未标记公共数据集交换预测输出而非模型参数，降低通信开销并支持异构架构。


<details>
  <summary>Details</summary>
Motivation: 解决联邦学习中统一模型架构要求、多轮通信开销大、客户端计算资源有限的问题，同时保护数据隐私。

Method: 采用知识蒸馏技术，客户端在未标记公共数据集上生成预测输出并传输给服务器；服务器通过专门的目标函数迭代优化伪标签和模型，结合定制化的伪标签生成和知识蒸馏策略整合多样化知识。
...
TL;DR: 通过利用随机梯度的本质变化自动调整学习率，无需手动调参的新学习率调整机制


<details>
  <summary>Details</summary>
Motivation: 解决现有学习率调整方法复杂、需要手动调整超参数、消耗大量计算资源的问题

Method: 基于随机二阶算法，根据随机梯度的范数变化自动调整学习率：梯度范数减小时增加学习率，梯度范数增大时减小学习率

Result: 在SGD、SGDM、SIGNSGD等算法上展现出高效率、稳健性和扩展性

Conclusion: 提出了一种解释性强、无需手动调参的自动学习率调整方法，能够自动适应随机梯度的本质变化
...
### [202] [Input Time Scaling](https://arxiv.org/abs/2508.13654)
*Rapheal Huang,Weilong Guo*

Main category: cs.LG

TL;DR: 提出了输入时间缩放新范式，通过在训练和测试时用不同策略精炼输入来提升LLM性能，发现训练-测试协同设计现象，挑战了"垃圾进垃圾出"的传统观念，在32B模型上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 当前LLM主要通过数据和训练规模扩展以及推理时扩展来提升性能，但忽视了在输入查询阶段投入资源的重要性，需要探索新的扩展范式

Method: 提出输入时间缩放范式，在训练和测试时结合LLM的元知识用不同策略精炼输入，采用训练-测试协同设计方法，使用看似低质量的数据集进行实验

Result: 在Qwen2.5-32B-Instruct上达到AIME24(76.7%)和AIME25(76.7%)的SOTA性能，多数投票可进一步提升至AIME24(76.7%)和AIME25(80%)，从DeepSeek-R1-Distill-Qwen-32B开始最佳结果达到AIME24(86.7%)和AIME25(76.7%)

Conclusion: 输入时间缩放是有效的扩展范式，训练-测试协同设计至关重要，看似低质量的数据集可能获得高性能，挑战了传统数据质量观念，小样本即可激发高级推理能力，符合"少即是多"现象
...
<details>
  <summary>Details</summary>
Motivation: 现代神经网络在大数据集上的训练计算成本和环境成本高昂，需要开发能够减少计算资源消耗同时保持模型性能的训练方法。

Method: 提取每批数据的低秩特征表示，应用Fast MaxVol采样器选择能够覆盖批次主导子集的小型多样化子集，并使用梯度近似准则动态调整子集大小。

Result: 在多个基准测试中，GRAFT在准确性和效率方面匹配或超越了最新的选择基线方法，在准确性、效率和排放之间提供了有利的权衡。
...
TL;DR: 将CASH框架扩展到现代ML流水线选择和适配，提出PS-PFN方法通过后验采样和先验数据拟合网络来高效探索和利用ML流水线


<details>
  <summary>Details</summary>
Motivation: 随着预训练模型的发展，现代ML工作流需要超越超参数优化的更复杂适配技术，传统CASH方法无法应对日益异构的ML流水线

Method: 提出PS-PFN方法，将后验采样扩展到最大k臂老虎机问题设置，利用先验数据拟合网络通过上下文学习高效估计最大值后验分布
...
TL;DR: 提出了一种基于自注意力的可微分通信模块，用于多智能体强化学习中的智能体间信息交换，该模块可以与任何动作价值函数分解方法无缝集成。
...
Method: 使用自注意力机制构建完全可微分的通信模块，智能体可以以奖励驱动的方式学习生成消息，模块参数数量固定且与智能体数量无关。

Result: 在SMAC基准测试中取得了最先进的性能，在多个地图上表现出色。

Conclusion: 提出的自注意力通信模块是有效的，能够显著提升多智能体强化学习的性能，且具有良好的通用性和扩展性。
...
TL;DR: 本文提出了一个通用框架来处理对手性重尾带宝矮问题，包括多臂带宝矮和线性带宝矮，并设计了首个在对手制下处理重尾器音的BOBW算法，在随机制下达到对数悟悚而在对手制下达到次线性悟悚。


<details>
  <summary>Details</summary>
Motivation: 当前对重尾带宝矮的研究主要集中在随机制下，而在对手制下的研究很少，仅有的些例外也仅限于多臂带宝矮。需要一个通用框架来处理对手性重尾带宝矮问题。

Method: 提出了一个通用框架，通过在损失估计上添加奖励函数来进行FTRL。设计了一种精巧的奖励函数设置，并提出了重尾器音感知的稳定性-惩罚匹配（HT-SPM）学习率。

Result: 开发了首个重尾多臂带宝矮的BOBW算法，在对手制下达到O~(T^(1/ε))最差悟悚，在随机制下达到O~(log T)间隔依赖悟悚。在线性情况下达到O~(d^(1/2)T^(1/ε))悟悚，匹配随机制下的最佳结果。

Conclusion: 该框架为对手性重尾带宝矮问题提供了一个通用的解决方案，并通过HT-SPM学习率和方差缩减估计器实现了重尾线性带宝矮的首个BOBW结果，拓展了重尾带宝矮研究的边界。
...
TL;DR: 数据驱动的单机器排程算法，结合机器学习咄问题特性，在最小化迟到作业总重量问题上显著超越现有方法


<details>
  <summary>Details</summary>
Motivation: 传统的准确算法在某些问题区域性能会昂重下降，而数据驱动方法能够根据数据结构提供更强大的可扩展性能

Method: 提出了一种新题的数据驱动排程假设，将机器学习与问题特定特征相结合，确保生成可行解，并进行了系统的ML模型选择分析

Result: 实验结果显示该方法在最优距、最优解数量咄适应不同数据场景方面显著超过现有最先进方法

Conclusion: 该研究不仅提供了高性能的排程解决方案，还填补了相似研究中的ML模型选择空白，为实际应用提供了灵活性
...
TL;DR: 结合联邦学习和可解释AI的Trans-XFed架构，用于供应链信用评估，解决隐私、数据异构和模型可解释性问题


<details>
  <summary>Details</summary>
Motivation: 供应链信用评估面临隐私保护、信息孤岛、类不平衡、非IID数据和模型可解释性等多重挑战，需要一种方案同时解决这些问题

Method: 采用基于性能的客户端选择策略(PBCS)解决类不平衡和非IID问题，使用加强同态加密的FedProx架构作为核心模型，集成transformer编码器提供特征视觉化，并使用集成梯度可解释AI技术

Result: 在真实供应链数据集上的实验评估显示，Trans-XFed能够比多个基准方法更准确地进行信用评估，同时保持透明性和隐私性

Conclusion: Trans-XFed架构通过结合联邦学习和可解释AI技术，成功地解决了供应链信用评估中的多重挑战，为隐私保护下的可解释人工智能应用提供了有效解决方案
...
TL;DR: DREAMS是一种新的降维方法，通过正则化项结合t-SNE的局部结构保持和PCA的全局结构保持，生成介于两者之间的嵌入谱，在多尺度上同时保持数据的局部和全局结构。


<details>
  <summary>Details</summary>
Motivation: 现有降维方法要么专注于保持局部结构（如t-SNE、UMAP），要么专注于保持全局结构（如MDS、PCA），但缺乏能同时很好保持两者结构的方法。

Method: 通过简单的正则化项将t-SNE的局部结构保持与PCA的全局结构保持相结合，生成从局部结构良好的t-SNE嵌入到全局结构良好的PCA嵌入之间的连续谱。

Result: 在7个真实数据集（包括5个单细胞转录组数据集和1个群体遗传学数据集）上进行了基准测试，定性和定量结果都显示DREAMS在多尺度结构保持方面优于先前方法。

Conclusion: DREAMS方法能够有效平衡局部和全局结构保持，在多尺度数据可视化方面表现出色，为高维数据可视化提供了新的解决方案。
...
TL;DR: 本文研究在斯托克加尔套利布套利环境下，通过求解复杂的最优做法来实现高效的序列决策制定。这种方法能够在保证算法效果的同时，大幅度提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 传统的序列决策方法在处理复杂的环境下效果不佳，特别是当面对高维度问题时，计算复杂度迅速增长。因此，需要一种新的方法来解决这些挑战，以实现更高效的决策制定。

Method: 本文提出了一种基于深度学习的方法，通过结合神经网络和优化算法来近似最优解。该方法首先使用神经网络对环境进行建模，然后利用激励函数来指导学习过程，最后通过反向传播算法来优化收益。

Result: 实验结果显示，该方法在多个标准测试环境中都表现出艰囍的性能，特别是在高维度问题中，计算效率提升了数倍。比较对照证明，新方法在决策质量和运行时间上都显著优于传统方法。

Conclusion: 本文提出的方法为处理复杂序列决策问题提供了一种高效的解决方案。通过结合深度学习和优化算法，该方法能够在保持高准确性的同时大幅缩短计算时间，具有强烈的应用前景。
...
TL;DR: 本文提出了DARS和DARS-B方法，通过难度自适应采样和大批量训练来解决RLVR中的深度和广度问题，显著提升了推理性能。


<details>
  <summary>Details</summary>
Motivation: 当前的RLVR方法存在两个未被充分探索的维度：深度（模型能采样的最难问题）和广度（单次迭代消耗的实例数量）。GRPO算法存在系统性偏差，过度加权中等准确率样本而忽视了对推动推理边界至关重要的低准确率样本。

Method: 1. 提出Difficulty Adaptive Rollout Sampling (DARS)：通过目标多阶段rollout重新加权困难问题，增加困难问题的正rollout数量
2. 提出DARS-B：将DARS与大批量训练结合，用全批量更新替代PPO的小批量迭代
3. 大规模扩展批量大小，进行多轮全批量更新

Result: DARS在不增加推理成本的情况下持续提升Pass@K性能；大批量训练显著增强Pass@1性能；DARS-B同时提升了Pass@K和Pass@1性能；大批量训练保持了高token级熵，表明持续探索和减少梯度噪声

Conclusion: 深度和广度是RLVR中正交的两个维度，自适应深度探索和大规模广度扩展是释放RLVR推理能力的关键
...
TL;DR: PENGUIN是一个简单有效的周期性嵌套分组注意力机制，通过显式建模周期性模式和相对注意力偏置，在长期时间序列预测中超越了基于MLP和Transformer的模型。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer在时间序列预测中取得了突破，但其有效性仍有争议。作者认为需要重新审视自注意力的重要性，并强调显式建模周期性模式和相对注意力偏置对时间序列建模的重要性。

Method: 提出了Periodic-Nested Group Attention (PENGUIN)机制，包含周期性嵌套相对注意力偏置来直接捕获周期性结构，以及分组注意力机制来处理多个共存的周期性（如日周期和周周期），每个组使用多查询注意力机制针对特定周期性。

Result: 在多个基准测试上的广泛实验表明，PENGUIN始终优于基于MLP和Transformer的模型。

Conclusion: PENGUIN通过显式建模周期性模式和相对注意力偏置，为长期时间序列预测提供了一个简单而有效的解决方案，证明了周期性结构建模在时间序列预测中的重要性。
...
TL;DR: 通信效率是联邦学习的关键璦颈，ISP机制动态选择每轮客户数量，在保持模型准确性的同时节省30%通信开销


<details>
  <summary>Details</summary>
Motivation: 联邦学习中通信效率问题尤为突出，尤其在异质性和动态客户参与环境下。现有方法对每轮选择多少客户这一关键问题研究不深入

Method: 提出ISP（智能参与者选择）机制，通过动态确定每轮最优客户数量来提升通信效率

Result: 在视觉Transformer、实际ECG分类和梯度压缩训练中验证，一致节省30%通信开销且不丢失最终模型质量

Conclusion: 选择参与客户数量是联邦学习中一个独立重要的任务，ISP机制能够在保持模型性能的同时显著提升通信效率
...
TL;DR: 基于随机学习自动机和带内网络测量的分布式网络内强化学习框架，用于程序化网络中的适应性路径选择


<details>
  <summary>Details</summary>
Motivation: 解决网络拥塞问题，通过分布式、本地化的强化学习策略来实时适应网络条件变化，提高网络性能

Method: 结合随机学习自动机(SLA)和带内网络测量(INT)实时数据，在P4程序化BMv2交换机上实现分布式路径选择机制

Result: 系统能够进行有效路径选择并适应网络条件变化，在线速率下完成适应

Conclusion: 证明了分布式网络内强化学习框架在适应性路径选择中的可行性和效果，为程序化网络的智能化管理提供了新方向
...
TL;DR: 这篇论文提出了首个基于主观逻辑的正式框架，用于评估AI训练数据集的可信质量，特别是数据集级别的偏见等全局属性。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统越来越依赖训练数据，评估数据集的可信质量变得至关重要，特别是仅在数据集整体层面出现的公平性、偏见等属性。现有方法无法有效处理这类全局属性的不确定性评估。

Method: 构建了一个基于主观逻辑的正式框架，支持信任命题和在证据不完整、分布式或冲突场景下的不确定性量化。将该框架应用于偏见属性评估，并在交通标志识别数据集上进行实验评估。

Result: 实验结果表明，该方法能够抓取类别不平衡问题，并在集中式和联邦学习环境下均保持解释性和稳健性。

Conclusion: 该研究提供了一个有效的不确定性意识方法来评估AI训练数据集的全局可信质量属性，为数据集偏见等问题的评估提供了新的视角和工具。
...
TL;DR: 提出了一种基于解耦VAE和平滑Bootstrap的潜在空间数据生成方法，用于解决表格数据中的不平衡回归问题
...
Method: 使用变分自编码器(VAE)建模数据分布的潜在表示，结合解耦VAE和潜在空间中的平滑Bootstrap技术进行数据生成

Result: 通过在IR基准数据集上与竞争方法进行数值比较来评估方法的有效性

Conclusion: 该方法为不平衡回归问题提供了一种创新的数据生成解决方案，通过潜在空间操作改善了标准VAE在不平衡数据上的低效问题
...
TL;DR: 这篇论文系统性比较了一次性剪枝和迭代剪枝两种深度学习模型压缩方法，发现它们在不同剪枝比下各有优势，并提出了新的混合剪枝策略。


<details>
  <summary>Details</summary>
Motivation: 虽然迭代剪枝曾经广泛采用，但这种偏好并没有经过严格的实验验证。论文动机在于系统性地比较两种方法的性能，为实践者提供科学的剪枝策略选择依据。

Method: 进行了系统性的对比实验，包括严格的定义、在结构化和非结构化设置下的性能测试，以及不同剪枝标准和方式的应用。还提出了耐心基础的剪枝和混合方法。

Result: 发现两种方法各有特定优势：一次性剪枝在较低剪枝比时效果更好，而迭代剪枝在较高剪枝比时表现更优。混合方法在某些情况下能够超过传统方法。

Conclusion: 论文强调了根据具体目标和约束条件选择合适剪枝策略的重要性，并提供了一个完整的剪枝性能测试框架以支持实践决策。
...
TL;DR: FedUP是一种轻量级的联邦遗忘算法，通过修剪受攻击模型中的特定连接来有效减轻恶意客户端的影响，无需完全重新训练，在保持良性数据性能的同时降低恶意数据准确性。


<details>
  <summary>Details</summary>
Motivation: 联邦学习容易受到模型投毒攻击，而联邦遗忘在处理恶意且可能共谋的客户端时面临挑战，因为这些客户端不会合作地遗忘其数据。

Method: FedUP仅使用遗忘前最后一轮训练中的客户端权重来识别需要抑制的连接，通过选择并归零良性客户端和恶意客户端最新更新之间差异最大的高幅度权重来隔离恶意影响。

Result: 在IID和非IID数据、标签翻转和后门攻击等各种场景下，FedUP都能有效降低恶意影响，将恶意数据上的准确率降至与从头重新训练的模型相当的水平，同时保持良性数据性能。

Conclusion: FedUP相比现有最先进的联邦遗忘解决方案，在实现有效遗忘的同时始终更快且节省存储空间，展现出良好的效果、鲁棒性和效率。
...
TL;DR: 本文通过专家调查重新评估生物识别模态的适用性，发现面部识别因技术进步而提升，指纹识别因漏洞而下降，专家评价与数据集结果高度一致。


<details>
  <summary>Details</summary>
Motivation: 当前生物识别评估框架过时（1998年表格）无法反映最新技术进步和漏洞，需要新的可靠评估方法。

Method: 对24名生物识别专家进行调查，对比专家评价与55个生物识别数据集的不确定性分析。

Result: 专家评分显示各模态属性异动：面部识别因技术进步而提升，指纹识别因攻击漏洞而下降，专家评价与数据集结果高度一致。

Conclusion: 专家见解与实证数据结合能提供可靠评估，专家分歧揭示了关键挑战并指导未来研究方向。
...
TL;DR: Fisher-Orthogonal Projection (FOP)是一种新方法，通过利用两个子批次的梯度构建方差感知更新方向，在大批量训练时恢复二阶方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 现代GPU支持大批量训练（数万个样本），但现有优化器在大批量下表现不佳。一阶方法因梯度噪声减少而难以逃离次优极小值，二阶方法需要过高阻尼导致曲率信息丢失。

Method: FOP利用两个子批次的梯度，在Fisher度量下增强平均梯度与梯度差异的正交分量，构建方差感知更新方向。

Result: FOP在大批量训练中恢复了二阶方法的有效性，实现了更好的泛化性能和更快的收敛速度。

Conclusion: FOP技术能够有效解决大批量训练中优化器性能下降的问题，为可扩展训练提供了新解决方案。
...
TL;DR: 提出了One-Step Flow Q-Learning (OFQL)方法，通过Flow Matching框架改进Diffusion Q-Learning (DQL)，实现单步去噪动作生成，显著提升训练和推理效率。


<details>
  <summary>Details</summary>
Motivation: DQL虽然性能优异，但依赖多步去噪生成动作，导致训练和推理效率低下。单步去噪直接应用会导致性能急剧下降，需要新的解决方案。

Method: 将DQL重新构建在Flow Matching框架中，学习平均速度场而非传统FM的弯曲轨迹，实现直接准确的动作生成，无需辅助模型、蒸馏或多阶段训练。

Result: 在D4RL基准测试中，OFQL超越DQL和其他基于扩散的方法，同时大幅减少训练和推理时间。

Conclusion: OFQL成功解决了DQL的多步采样限制，提供了更高效、鲁棒的离线强化学习解决方案，为实际应用铺平道路。
...
TL;DR: 这篇论文提出了一种基于边缘计算的端到端预测框架，通过轻量化模型在FPGA设备上实现高效能消耗的污水溢流预测，解决了传统云计算AI模型在通信故障时的可靠性问题。


<details>
  <summary>Details</summary>
Motivation: 极端天气事件加剧了老化合流制污水系统的污水溢流风险，而传统云计算AI预测模型在通信中断时无法使用，需要开发能够在边缘设备上高效运行的本地预测方案。

Method: 集成轻量化Transformer和LSTM模型，通过整数量化压缩提高运行效率，使用自动化硬件感知部署流程在AMD Spartan-7 FPGA上寻找最优模型配置。

Result: 8位Transformer模型在24小时历史数据训练下达到MSE 0.0376的高精度，每次推理能耗0.370 mJ；8位LSTM模型能耗更低（0.009 mJ）但精度较差（MSE 0.0432）且训练时间更长。

Conclusion: 该框架实现了本地化、能效高的预测能力，为合流制污水系统提供了更鲁棒的解决方案，模型选择需根据部署优先级在精度和能耗之间做出找法。
...
TL;DR: 提出使用分类分布代替高斯分布来表示多模态策略，通过中间分类分布选择行为模式，在连续控制任务中实现更好的探索和性能


<details>
  <summary>Details</summary>
Motivation: 传统深度强化学习策略通常使用高斯分布参数化，限制了学习到的行为是单模态的。许多实际决策问题需要多模态策略来促进环境探索，解决稀疏奖励、复杂动态或需要适应不同情境的学习挑战

Method: 引入分类策略来建模多模态行为模式，使用中间分类分布，然后基于采样的模式生成输出动作。探索了两种采样方案，确保可微分离散潜在结构同时保持高效的基于梯度的优化

Result: 在DeepMind Control Suite环境中评估多模态策略，通过更好的探索，学习到的策略收敛更快且优于标准高斯策略

Conclusion: 分类分布作为连续控制中结构化探索和多模态行为表示的强大工具
...
TL;DR: 自动特征工程方法存在可用性问题，缺乏文档和社区支持，且无法设置时间和内存限制


<details>
  <summary>Details</summary>
Motivation: 调查现有自动化特征工程方法的可用性状况，因为手动特征工程成本高而自动化方法对实践者的适用性尚未经过详细研究

Method: 调查了53种自动化特征工程方法，分析其可用性、文档质量、社区活跃度以及是否支持资源限制设置

Result: 现有方法普遍难以使用，文档缺乏，没有活跃社区，且没有任何方法允许用户设置时间或内存限制

Conclusion: 未来需要更具可用性、更好工程化的自动化特征工程方法，特别是支持资源限制的方法
...
TL;DR: 这篇论文提出了解决随机最短路径(SSP)问题的新算法，包括表格设置下的两种算法和函数近似设置下的一种算法，并证明了它们的近似几乎必然收敛性。


<details>
  <summary>Details</summary>
Motivation: SSP问题是强化学习中的重要类型，其他成本准则都可以在SSP设置中形式化，需要提高解决这类问题的算法性能。

Method: 提出了两种表格设置算法和一种函数近似设置算法，并证明了这些算法的异步近似几乎必然收敛性。

Result: 表格算法在性能上显著优于其他知名的收敛强化学习算法，函数近似算法也显示出比其他算法更可靠的性能。

Conclusion: 这些新算法为解决SSP问题提供了有效的方法，在各种设置下都表现出优秀的性能和可靠性。
...
TL;DR: AutoScale是一个两阶段框架，通过多任务优化指标自动选择线性标量化权重，无需昂贵搜索即可实现高性能多任务学习


<details>
  <summary>Details</summary>
Motivation: 现有研究发现固定权重的线性标量化可以达到复杂多任务优化方法的性能，但最优权重的选择原理不明且依赖耗尽的超参数搜索

Method: 建立线性标量化与多任务优化方法的直接联系，发现性能良好的权重在多任务优化指标中呈现特定趋势，基于此设计AutoScale两阶段框架

Result: AutoScale在多样化数据集（包括新的大规模基准测试）上始终表现出优越性能和高效率

Conclusion: 通过多任务优化指标指导权重选择可以高效实现高性能线性标量化，为多任务学习提供了简单有效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 当前深度学习模型效率优化方法多样且分散，缺乏统一的数学框架来表示和组合这些技术，阻碍了系统性的效率优化研究。
...
TL;DR: 这篇论文提出了一种可微分的量化策略，通过学习比特宽度、噪声缩放和限定边界来优化神经网络量化过程，在极端低比特宽度下仍能保持竞争性的准确度。


<details>
  <summary>Details</summary>
Motivation: 量化神经网络被视为一系列噪声通道，每层的四舍五入操作会降低通道容量，而浮点数检查点设定了最大输入速率。需要解决量化瓶颈问题，在保持效率的同时提高极端低比特宽度下的模型准确性。

Method: 将精调视为平滑的约束优化问题，使用全可微分的直通估计器(STE)，具有可学习的比特宽度、噪声缩放和限定边界。通过外点罚函数强制目标比特宽度，并使用轻度的评估指标平滑技术(通过知识精炼)来稳定训练。

Result: 该方法虽然简单，但在极端的W1A1设置下仍能达到竞争性的准确度，同时保持了STE的高效率特性。

Conclusion: 通过将量化过程模型化为噪声通道链并使用可微分优化方法，可以有效地解决量化瓶颈问题，在极低比特宽度下保持良好的模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 自闭症谱系障碍(ASD)是一种复杂的神经发育障碍，表现为大脑连接性的破坏。功能MRI可以测量大脑中血氧水平依赖信号，这些信号可以建模为脑区之间的相互作用。有效捕捉这些连接模式并识别与典型发育偏离的相互作用对于改善ASD诊断和生物标志物发现至关重要。

Method: 提出了ASDFormer，一个基于Transformer的架构，集成了混合池化-分类器专家(MoE)。通过整合多个专门的专家分支和注意力机制，自适应地强调与自闭症相关的不同脑区和连接模式。
...
TL;DR: 提出了基于类型拓扑空间的数据集分析方法，通过定义特殊类型集和类型拓扑，将数据集组织成有序的轨道和组件结构，并用伪树表示分支关系


<details>
  <summary>Details</summary>
Motivation: 现有数据集研究主要依赖统计方法和代数拓扑方法，需要从一般拓扑学角度开发新的分析方法来研究数据集的内在结构

Method: 在数据集上开发特殊类型集和相关类型拓扑，利用R^2的自然商空间将数据集组织成轨道，每个轨道分解为有序组件，并用整数序列表示，组件跨轨道形成分支结构

Result: 建立了能够表示数据集内在结构的类型拓扑框架，组件可以表示为有序整数序列，分支关系可以用类型II伪树很好地表示

Conclusion: 该方法为计算凸包、孔洞检测、聚类和异常检测等问题提供了新的算法平台，是从一般拓扑学角度研究有限拓扑空间的新方法
...
TL;DR: 知识图谨忘学习新方法，通过泰勒展开和零阶优化近似逆黑塞矩阵，提高大规模知识图谨忘的效率和质量


<details>
  <summary>Details</summary>
Motivation: 满足"被忘记权"等法规要求，避免重新训练的高成本，解决知识图特有结构和语义关系带来的谨忘挑战

Method: 定义知识图影响函数，使用泰勒展开估计参数变化，利用Fisher矩阵和零阶优化近似逆黑塞矩阵向量积，避免计算一阶和二阶导数的高计算成本

Result: 在谨忘效率和谨忘质量方面显著超过其他最先进的图谨忘基线方法

Conclusion: 该方法为大规模知识图谨忘提供了高效解决方案，在保持模型性能的同时大幅提升谨忘效率
...
Method: 提出BLIPs框架，基于自适应变分dropout的可扩展架构无关变分贝叶斯方法，可与(等变)消息传递架构无缝集成

Result: 在计算化学任务中显示出比标准MLIPs更好的预测准确性，特别是在数据稀缺和严重分布外情况下提供可信的不确定性估计

Conclusion: BLIPs通过贝叶斯方法有效解决了MLIPs的不确定性估计问题，在数据稀缺和分布外场景下表现优异，预训练模型微调也能获得持续性能提升
...
<details>
  <summary>Details</summary>
Motivation: 强化学习中复杂任务的奖励函数难以设计，现有方法结合偏好反馈和专家演示的方式往往不够灵活或难以扩展

Method: 提出reward-rational partial orderings框架，开发LEOPARD算法，能够从多种反馈类型（包括负面演示）中学习奖励函数

Result: 在有限偏好和演示反馈下，LEOPARD相比现有基线方法有显著优势，结合多种反馈类型通常比单一反馈更有效
...
### [235] [PreSem-Surf: RGB-D Surface Reconstruction with Progressive Semantic Modeling and SG-MLP Pre-Rendering Mechanism](https://arxiv.org/abs/2508.13228)
...
<details>
  <summary>Details</summary>
Motivation: 现有的NeRF方法在从RGB-D序列重建场景表面时，处理速度较慢且对噪声敏感，需要一种能够快速、高质量重建并增强场景理解的方法。

Method: 提出SG-MLP采样结构结合PR-MLP进行体素预渲染，采用渐进式语义建模从粗到精提取语义信息，整合RGB、深度和语义数据提升重建性能。

Result: 在7个合成场景的6个评估指标测试中，PreSem-Surf在C-L1、F-score和IoU指标上取得最佳性能，在NC、准确性和完整性指标上保持竞争力。

Conclusion: PreSem-Surf方法有效提升了场景表面重建的质量和效率，证明了其在快速高质量3D重建中的实用性和有效性。
...
### [236] [Sketch3DVE: Sketch-based 3D-Aware Scene Video Editing](https://arxiv.org/abs/2508.13797)
*Feng-Lin Liu,Shi-Yang Li,Yan-Pei Cao,Hongbo Fu,Lin Gao*

Main category: cs.GR

TL;DR: Sketch3DVE是一个基于草图的3D感知视频编辑方法，能够处理大视角变化下的视频结构内容编辑，通过3D点云编辑和深度图表示实现精确的几何控制。
...
Result: 大量实验证明Sketch3DVE在视频编辑方面的优越性，能够有效处理大视角变化下的3D场景结构编辑任务。

Conclusion: Sketch3DVE成功解决了基于草图的3D感知视频编辑中的关键挑战，特别是在处理显著视角变化时，实现了精确的几何控制和逼真的视频编辑效果。
...
### [237] [Is-NeRF: In-scattering Neural Radiance Field for Blurred Images](https://arxiv.org/abs/2508.13808)
*Nan Luo,Chenglin Ye,Jiaxu Li,Gang Liu,Bo Wan,Di Wang,Lupeng Liu,Jun Xiao*

Main category: cs.GR

TL;DR: Is-NeRF是一种新型的去模糊神经辐射场，通过显式光路建模和散射感知体积渲染，有效处理运动模糊图像并解决几何模糊问题


<details>
  <summary>Details</summary>
Motivation: 现有的NeRF方法使用直线体积渲染，无法处理复杂光路场景，在处理运动模糊图像时会产生几何模糊问题，需要新的方法来应对这些挑战

Method: 提出统一的散射表示方法，建立散射感知体积渲染管线，引入自适应学习策略确定散射方向和采样间隔，联合优化NeRF参数、散射参数和相机运动

Result: 在复杂真实场景中表现优异，在生成高保真图像和准确几何细节方面超越现有最先进方法
...
### [238] [DegDiT: Controllable Audio Generation with Dynamic Event Graph Guided Diffusion Transformer](https://arxiv.org/abs/2508.13786)
*Yisu Liu,Chenxing Li,Wanqian Zhang,Wenfu Wang,Meng Yu,Ruibo Fu,Zheng Lin,Weiping Wang,Dong Yu*

Main category: cs.SD

TL;DR: DegDiT是一个基于动态事件图引导的扩散transformer框架，用于开放词汇的文本到音频生成控制，通过结构化图表示和共识偏好优化实现精确的时序控制。


<details>
  <summary>Details</summary>
Motivation: 现有文本到音频生成方法在准确时间定位、开放词汇扩展性和实际效率之间存在固有权衡，需要解决这些挑战以实现精确的音频内容与时序结构控制。

Method: 使用动态事件图编码描述中的事件，节点表示语义特征、时间属性和事件间连接；采用图transformer生成上下文事件嵌入作为扩散模型指导；引入质量平衡数据选择流程和共识偏好优化方法。

Result: 在AudioCondition、DESED和AudioTime数据集上的广泛实验表明，DegDiT在各种客观和主观评估指标上达到了最先进的性能。

Conclusion: DegDiT框架通过结构化事件图表示和共识优化策略，成功解决了可控文本到音频生成中的时序精度、词汇扩展性和效率平衡问题，为精确音频合成提供了有效解决方案。
...
### [239] [Evaluating Identity Leakage in Speaker De-Identification Systems](https://arxiv.org/abs/2508.14012)
*Seungmin Seo,Oleg Aulov,Afzal Godil,Kevin Mangold*

Main category: cs.SD

TL;DR: 这篇论文引入了一个新的评测标准，发现所有当前最优的讲话者去识别系统都存在身份信息泄漏问题，显示了这些技术在隐私保护方面的持续风险。


<details>
  <summary>Details</summary>
Motivation: 讲话者去识别技术的目标是隐藏讲话者身份同时保持语音的可理解性，但当前缺乏综合性的评测标准来量化残留身份泄漏。

Method: 研究者开发了一个新的评测标准，使用三种互补的错误率指标：等错误率(EER)、累积匹配特征命中率(CMC)和通过正则化相关分析以及Procrustes分析测量的嵌入空间相似性。

Result: 评测结果显示所有状态最优的讲话者去识别系统都存在身份信息泄漏。性能最高的系统仅略好于随机猜测，而性能最差的系统在CMC前50名候选人中达到了45%的命中率。

Conclusion: 这些发现突出了当前讲话者去识别技术在隐私保护方面的持续风险，指出需要更加有效的方法来真正保护讲话者身份信息。
...
TL;DR: 本文提出了一种针对可复制资源（如数据和模型）的协作学习系统中公平奖励分配的新机制，通过平衡互惠公理确保参与者之间的相互公平性。


<details>
  <summary>Details</summary>
Motivation: AI智能体协作系统中存在奖励分配不公平的问题，特别是在数据和模型等可无限复制的资源场景中，传统合作博弈论（如Shapley值）无法充分处理这种可复制性带来的公平性挑战。

Method: 提出了一种新的公平分配机制，基于平衡互惠公理来形式化相互公平性，确保每对参与者都能从对方的参与中获得同等收益。

Result: 设计了一个具体机制并提供了数学证明，证明该机制满足平衡互惠公理的要求。

Conclusion: 该研究为可复制资源场景下的协作系统提供了新的公平分配框架，解决了传统方法无法处理的互惠公平性问题。
...
### [241] [Prompt Orchestration Markup Language](https://arxiv.org/abs/2508.13948)
*Yuge Zhang,Nan Chen,Jiahang Xu,Yuqing Yang*

Main category: cs.HC

TL;DR: POML是一种基于标记语言的提示编排语言，通过组件化结构、专用数据标签和CSS样式系统解决复杂提示的组织、数据集成和格式敏感性问题，并提供开发工具支持。


<details>
  <summary>Details</summary>
Motivation: 当前LLM提示方法在结构组织、数据集成、格式敏感性和工具支持方面存在不足，缺乏处理多样化数据类型和系统管理呈现变化的综合解决方案。

Method: 提出POML标记语言，采用基于组件的标记逻辑结构（角色、任务、示例）、专用标签实现无缝数据集成、CSS样式系统分离内容与呈现，包含动态提示模板和完整的开发者工具包。

Result: 通过两个案例研究验证POML在复杂应用集成（PomLink）和准确性性能（TableQA）方面的影响，以及用户研究评估其在真实开发场景中的有效性。

Conclusion: POML为LLM提示编排提供了结构化、可扩展的解决方案，有效解决了现有方法在组织复杂提示、集成多样化数据和降低格式敏感性方面的挑战。
...
### [242] [Learning to Use AI for Learning: How Can We Effectively Teach and Measure Prompting Literacy for K-12 Students?](https://arxiv.org/abs/2508.13962)
*Ruiwei Xiao,Xinying Hou,Ying-Jui Tseng,Hsuan Nieu,Guanze Liao,John Stamper,Kenneth R. Koedinger*

Main category: cs.HC

TL;DR: 这篇论文设计了一个基于大语言模型的模块，通过场景化练习活动培养中学生的提示语言能力和负责任使用AI聊天机器人的能力，并在真实教室环境中验证了教学效果。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能日益深入日常生活，需要培养下一代负责任地应用、交互、评估和协作AI系统的能力，而K-12教育者对教授学生豪理有效使用AI学习有着紧迫需求。

Method: 设计了基于LLM的提示语言教学模块，包括场景化的练习活动和直接与智能LLM代理交互。在11个真实中学教室进行了两轮部署，评估了AI自动评分能力、学生提示能力变化、对AI学习的信心变化以及教学资料质量。

Result: AI自动评分器能够满意地评分学生编写的提示；教学资料通过练习支持学生提高提示技能，并导致对使用AI学习的称认变化；研究1的数据指导了研究2的评估修订，显示真/假题和开放式题比多选题更有效地测量提示语言能力。

Conclusion: 这些有前景的结果显示了更广泛部署的潜力，并强调了需要更广泛的研究来评估学习效果和评估设计。
...
TL;DR: 提出了Diff-MSM可微分肌肉骨骼模型，通过端到端自动微分技术同时识别肌肉和骨骼参数，无需测量内部关节扭矩，显著优于现有方法。
...
Result: 仿真结果表明该方法显著优于现有基线方法，肌肉参数估计误差可低至0.05%。

Conclusion: 该方法不仅适用于肌肉骨骼建模，在肌肉健康监测、康复和运动科学等领域也具有巨大应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 建立一个使用普通硬件和开源软件的可复现移动监控机器人系统，支持远程监控和语音控制功能。

Method: 采用双Raspberry Pi 4系统架构，前端单元负责感知和运动，中央单元负责视频流和视觉处理。使用FFmpeg传输视频，YOLOv3进行物体检测，Python语音库实现语音识别、翻译和语音合成。Kinect RGB-D传感器提供视觉输入和障碍物检测。

Result: 在室内测试中，机器人在CPU上以交互帧率检测常见物体，可靠识别命令并将其转换为动作，无需手动控制。
...
### [245] [MimicFunc: Imitating Tool Manipulation from a Single Human Video via Functional Correspondence](https://arxiv.org/abs/2508.13534)
...
<details>
  <summary>Details</summary>
Motivation: 人类能够通过观察一次就模仿工具操作行为，并将技能轻松转移到功能等效的不同工具上，而当前机器人难以实现这种水平的泛化能力。主要挑战在于建立功能级别的对应关系，考虑到功能相似工具之间的显著几何差异。

Method: 提出MimicFunc框架，使用功能帧（function-centric local coordinate frame）建立功能对应关系，通过基于关键点的抽象构建功能中心局部坐标系。

Result: 实验表明MimicFunc能有效使机器人从单个RGB-D人类视频中泛化技能，操作新工具完成功能等效任务。利用其一次性泛化能力，生成的rollout可用于训练视觉运动策略，无需为新颖对象收集劳动密集的遥操作数据。

Conclusion: MimicFunc为解决工具操作技能模仿中的功能对应问题提供了有效解决方案，实现了从单次人类演示到多样化工具的高效技能转移，为机器人学习提供了可扩展的替代方案。
...
### [246] [ROVER: Robust Loop Closure Verification with Trajectory Prior in Repetitive Environments](https://arxiv.org/abs/2508.13488)
*Jingwen Yu,Jiayi Yang,Anjun Hu,Jiankun Wang,Ping Tan,Hong Zhang*

Main category: cs.RO

TL;DR: ROVER是一种利用历史轨迹作为先验约束来在重复环境中验证闭环检测的方法，通过位姿图优化和轨迹评分方案来拒绝错误闭环


<details>
  <summary>Details</summary>
Motivation: 在重复环境中，基于外观特征的闭环检测容易产生误检，现有方法忽视了机器人的时空运动轨迹先验知识

Method: 对每个闭环候选，先用位姿图优化估计轨迹，然后通过评分方案评估该轨迹与无闭环时的轨迹先验的符合程度

Result: 基准测试和真实实验证明了方法的有效性，集成到先进SLAM系统中验证了鲁棒性和效率

Conclusion: 利用轨迹先验约束能有效在挑战性重复环境中拒绝错误闭环检测，提高SLAM系统的可靠性
...
### [247] [Multimodal Data Storage and Retrieval for Embodied AI: A Survey](https://arxiv.org/abs/2508.13901)
*Yihao Lu,Hao Tang*

Main category: cs.RO

TL;DR: 本论文系统评估了五种数据存储架构和五种检索范式在体现式AI数据管理中的适用性，揭示了长期语义一致性与实时响应能力之间的基本矛盾，并提出了包含物理感知数据模型、适应性存储检索协同优化等的前沿研究议程。


<details>
  <summary>Details</summary>
Motivation: 体现式AI代理不断与物理世界交互，产生大量异构多模态数据流，传统数据管理系统无法有效处理这些数据的物理基础、低延迟访问和动态扩展性需求。

Method: 基于对180多项相关研究的系统性评估，分析了五种数据存储架构（图数据库、多模型数据库、数据湖、向量数据库、时序数据库）和五种检索范式（融合策略检索、表征对齐检索、图结构检索、生成模型检索、高效检索优化）。

Result: 发现了长期语义一致性与实时响应能力之间的基本矛盾，识别了从基础物理基础间隔到跨模态集成、动态适应和开放世界汉化等系统性问题的关键瓶颈。

Conclusion: 提出了包含物理感知数据模型、适应性存储检索协同优化和标准化测试基准在内的前沿研究议程，为下一代自主体现系统设计健壮、高性能数据管理框架提供了严谨的路线图。
...
### [248] [Embodied-R1: Reinforced Embodied Reasoning for General Robotic Manipulation](https://arxiv.org/abs/2508.13998)
*Yifu Yuan,Haiqin Cui,Yaoting Huang,Yibin Chen,Fei Ni,Zibin Dong,Pengyi Li,Yan Zheng,Jianye Hao*

Main category: cs.RO

TL;DR: 该论文提出了一种名为"指向"的统一中间表示方法，通过Embodied-R1模型和两阶段强化微调训练，在11个具身空间和指向基准测试中达到最先进性能，并在零样本泛化方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 解决具身AI中的"看到-行动差距"问题，该问题源于数据稀缺和具身异构性，需要一种统一的、与具身无关的中间表示来连接高级视觉语言理解和低级动作原语。

Method: 提出"指向"作为统一的中间表示，定义四种核心具身指向能力；构建Embodied-Points-200K大规模数据集；开发Embodied-R1（3B VLM）模型；采用两阶段强化微调（RFT）课程训练和专门的多任务奖励设计。

Result: 在11个具身空间和指向基准测试中达到最先进性能；零样本泛化方面：在SIMPLEREnv中达到56.2%成功率，在8个真实世界XArm任务中达到87.5%成功率，比强基线提高62%；对多样化视觉干扰表现出高鲁棒性。

Conclusion: 指向中心的表示方法结合RFT训练范式，为弥合机器人感知-行动差距提供了一条有效且可泛化的途径。
...
### [249] [Augmenting cobots for sheet-metal SMEs with 3D object recognition and localisation](https://arxiv.org/abs/2508.13964)
...
<details>
  <summary>Details</summary>
Motivation: 针对中小企业小批量多品种生产的特点，标准自动化解决方案往往无法满足需求，导致企业依赖重复性体力劳动，影响生产成本且无法充分发挥技术工人的潜力。

Method: 通过集成现有技术（包括3D物体识别和定位技术），将协作机器人改造为移动可重构的生产助手，并借鉴ACRO研究单位与工业合作伙伴过去项目的实施经验。

Result: 文章探讨了在工业环境中应用这些技术增强协作机器人系统的机遇和挑战，并概述了实施过程中的关键步骤。

Conclusion: 通过技术创新将协作机器人转变为灵活的生产助手，有望解决中小企业面临的生产自动化难题，提高生产效率和资源利用率。
...
### [250] [Toward Deployable Multi-Robot Collaboration via a Symbolically-Guided Decision Transformer](https://arxiv.org/abs/2508.13877)
*Rathnam Vidushika Rasanji,Jin Wei-Kocsis,Jiansong Zhang,Dongming Gan,Ragu Athinarayanan,Paul Asunda*

Main category: cs.RO

TL;DR: 提出了Symbolically-Guided Decision Transformer (SGDT)框架，将神经符号规划器与因果变换器结合，用于多机器人协作操作任务


<details>
  <summary>Details</summary>
Motivation: 强化学习在机器人操作中数据密集且依赖MDP假设，在多机器人操作等复杂动态和长期依赖场景中应用受限。决策变换器作为离线替代方案，但在多机器人操作领域仍未充分探索

Method: SGDT框架包含神经符号规划器生成符号子目标的高级任务导向计划，以及目标条件决策变换器进行低级序列决策的层次化架构

Result: 在零样本和少样本场景下评估了SGDT的性能，证明其在复杂多机器人协作任务中实现结构化、可解释和可泛化的决策
...
TL;DR: 本文对HRI领域中的"社会情境"概念进行了系统梳理，提出了一个概念模型来统一描述人机交互的社会情境，并讨论了该模型的应用和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: HRI研究中对"社会情境"的定义和使用方式多样，导致沟通困难和研究成果难以整合，需要建立一个统一的概念框架。

Method: 通过文献综述梳理现有定义，提出概念模型来描述人机交互的社会情境，并将该模型应用于现有研究工作。

Result: 开发了一个能够帮助研究人员规划交互、开发机器人行为模型以及获取交互后洞察的社会情境属性框架。

Conclusion: 提出了理解社会情境对HRI研究的重要性，并指出了在该领域未来研究的关键问题和发展方向。
...
### [252] [Benchmarking LLM-based Agents for Single-cell Omics Analysis](https://arxiv.org/abs/2508.13201)
*Yang Liu,Lu Zhou,Ruikun He,Rongbo Shen,Yixue Li*

Main category: q-bio.GN

TL;DR: 这篇论文提出了一个新的基准测试系统，用于评估AI组学分析中的代理能力，发现Grok-3-beta表现最佳，多代理框架通过专业分工显著提升效果，自我反思是关键能力。


<details>
  <summary>Details</summary>
Motivation: 多模态单细胞组学数据的爆发曝露了传统手工分析流程的局限性，而AI代理虽能提供适应性规划和代码生成，但缺乏综合性基准测试体系严重阻碍了进展。

Method: 设计了一个包含统一平台、多维度指标和50个实际组学分析任务的基准评估系统，涉及认知程序合成、协作能力、执行效率、生物信息知识整合和任务完成质量等多个方面。

Result: Grok-3-beta在测试代理框架中达到最佳性能，多代理框架通过专业角色分工显著提升了协作和执行效率。高质量代码生成对任务成功至关重要，自我反思影响最大，其次是RAG和规划能力。

Conclusion: 这项工作揭示了代码生成、长上下文处理和上下文感知知识检索方面持续存在的挑战，为计算生物学领域发展健壮的AI代理提供了关键的实证基础和最佳实践。
...
### [253] [Using Artificial Intuition in Distinct, Minimalist Classification of Scientific Abstracts for Management of Technology Portfolios](https://arxiv.org/abs/2508.13182)
*Prateek Ranka,Fred Morstatter,Andrea Belz,Alexandra Graddy-Reed*

Main category: cs.DL

TL;DR: 使用大型语言模型生成元数据来复制专家标注方法，实现科学摘要分类，用于研究组合管理和技术侦察等战略活动


<details>
  <summary>Details</summary>
Motivation: 科学摘要分类对战略活动很有用但难以自动化，稀疏文本缺乏上下文线索，现有半监督方法生成的标签缺乏区分度且容易重叠

Method: 应用人工直觉过程，使用大型语言模型生成元数据，在美国国家科学基金会摘要上创建标签集，然后在中国国家自然科学基金会摘要上进行测试

Result: 证明了该方法在研究组合管理、技术侦察和其他战略活动中的可行性

Conclusion: 基于大型语言模型的人工直觉方法能够有效复制专家标注方式，为科学摘要分类提供可行解决方案
...
### [254] [The Role of AI in Facilitating Interdisciplinary Collaboration: Evidence from AlphaFold](https://arxiv.org/abs/2508.13234)
*Naixuan Zhao,Chunli Wei,Xinyan Zhang,Jiang Li*

Main category: cs.DL

TL;DR: 这项研究通过分析AlphaFold对结构生物学家的影响，发现AI技术对跨学科合作的促进效果有限，仅0.48%增长，说明AI单独无法有效突破学科隔阁。


<details>
  <summary>Details</summary>
Motivation: 识别AI在科学领域加速发展的背景下，但AI对跨学科合作的具体机制和程度仍不清楚，需要实证研究来揭示其真实影响。

Method: 采用文献计量分析和因果推断方法，分析了Scopus数据库中1,247篇AlphaFold相关论文和7,700名作者，比较了采用者与非采用者之间的跨学科合作模式。

Result: 结果显示AlphaFold仅使结构生物学-计算机科学合作增长0.48%，对其他学科没有显著影响。AI虽然创造了与特定学科的合作需求，但技术民主化等因素削弱了这种需求。

Conclusion: 人工智能单独在突破学科隔阁或促进有意义的跨学科合作方面效果有限，需要更多综合性措施来实现真正的跨学科科研合作。
...
### [255] [COMPASS: A Multi-Dimensional Benchmark for Evaluating Code Generation in Large Language Models](https://arxiv.org/abs/2508.13757)
*James Meaden,Michał Jarosz,Piotr Jodłowski,Grigori Melnik*

Main category: cs.SE

TL;DR: COMPASS是一个多维度的代码生成评估框架，不仅评估功能正确性，还评估算法效率和代码质量，发现当前模型在正确性得分高时不一定能产生高效算法或可维护代码。


<details>
  <summary>Details</summary>
Motivation: 现有的代码生成基准主要关注功能正确性，忽略了真实编程中的算法效率和代码质量这两个关键方面，需要更全面的评估框架。

Method: 使用50个来自真实Codility竞赛的编程问题，收集393,150份人类提交作为基线，使用行业标准分析工具系统评估运行时效率和代码质量。

Result: 对三个领先的推理增强模型（Anthropic Claude Opus 4、Google Gemini 2.5 Pro、OpenAI O4-Mini-High）的评估显示，高正确性得分的模型不一定能产生高效算法或可维护代码。

Conclusion: 仅评估正确性不足以真正理解代码生成模型的真实能力，COMPASS为未来研究提供了指导框架，推动AI系统向更健壮、可靠和可用于生产的方向发展。
...
### [256] [Agentic DraCor and the Art of Docstring Engineering: Evaluating MCP-empowered LLM Usage of the DraCor API](https://arxiv.org/abs/2508.13774)
*Peer Trilcke,Ingo Börner,Henny Sluyter-Gäthje,Daniil Skorinkin,Frank Fischer,Carsten Milling*

Main category: cs.SE

TL;DR: 通过MCP协议实现LLM举证自主与DraCor API交互，研究重点在工具选择和使用效果评估，揭示了文档工程对优化LLM-工具交互的关键作用


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型如何通过MCP协议自主与数字人文学基础设施交互，以支持计算文学研究的可靠性和效率

Method: 实验研究采用定性方法，包括系统观察提示词以了解LLM使用MCP工具时的行为，并评估"工具正确性""工具调用效率""工具使用可靠性"三个指标

Result: 实验结果显示了文档工程（Docstring Engineering）的重要性，即通过反射性精心制作工具文档来优化LLM与工具的交互效果

Conclusion: 研究证明了自主AI在计算文学研究中的潜力，同时强调了建设可靠数字人文基础设施的重要性
...
### [257] [Uncertainty-Aware Learning Policy for Reliable Pulmonary Nodule Detection on Chest X-Ray](https://arxiv.org/abs/2508.13236)
*Hyeonjin Choi,Jinse Kim,Dong-yeon Yoo,Ju-sung Sun,Jung-won Lee*

Main category: eess.IV

TL;DR: 这篇论文提出了一种不确定性感知学习策略，通过同时学习医生的背景知识和肺部病变信息，提高医疗AI在胸部X光片诊断中的准确性和可信距离。


<details>
  <summary>Details</summary>
Motivation: 医疗AI虽然发展快速，但医生对其不确定性的担忧导致了临床应用的限制。这主要来自于AI仅依靠病变数据进行重复学习，而医生则利用了广泛的背景知识和临床经验。

Method: 研究提出了不确定性感知学习策略，让AI模型同时学习医生的背景知识和肺部病变信息。使用了来自Ajou大学医院2,517张无病变图像和656张结节图像进行训练。

Result: 提出的模型达到92%的性能指标（IoU 0.2 / FPPI 2），敏感度比基准模型提高了10%，同时将作为不确定性指标的熵提降低了0.2。
...
### [258] [Sex-Specific Vascular Score: A Novel Perfusion Biomarker from Supervoxel Analysis of 3D pCASL MRI](https://arxiv.org/abs/2508.13173)
*Sneha Noble,Neelam Sinha,Vaanathi Sundareshan,Thomas Gregor Issac*

Main category: eess.IV

TL;DR: 基于3D pCASL MRI的血管健康评估框架，通过超像素聚类和卷积神经网络识别性别特异性血流模式，准确率达95%，为神经退血性疾病的早期识别提供新方法


<details>
  <summary>Details</summary>
Motivation: 研究血流模式的性别差异对理解脑血管健康和神经退血性疾病的风险评估具有重要意义，需要发展能够量化这些差异的方法

Method: 使用3D伪连续动脉旋氧标记MRI技术，通过超像素聚类将脑部划分为均匀流泵区域，提取平均脑血流值，并使用自定义卷积神经网络进行性别分类

Result: 在186名认知健康参与者中达到95%的性别分类准确率，显示了脑部存在明显的性别特异性血流模式，同时评估了区域性血流变化和年龄相关效应

Conclusion: 该框架能够提供个性化的血管风险评估，为神经退血性疾病（如阿尔茹海默病）的早期识别和个性化干预提供新的工具和视角
...
### [259] [Automated Cervical Cancer Detection through Visual Inspection with Acetic Acid in Resource-Poor Settings with Lightweight Deep Learning Models Deployed on an Android Device](https://arxiv.org/abs/2508.13253)
*Leander Melroy Maben,Keerthana Prasad,Shyamala Guruvare,Vidya Kudva,P C Siddalingaswamy*

Main category: eess.IV

TL;DR: 使用轻量级深度学习模型自动化的乙酸视觉检查（VIA），为资源稀缺地区提供一种主观性小、无需高级医疗专业人员的宫颈癌筛查方案


<details>
  <summary>Details</summary>
Motivation: 宫颈癌在低中收入国家造成大量死亡，虽可治疗但筛查不足。VIA检查适合资源稀缺地区，但需要训练有素的医疗人员主观判读，自动化可消除主观性并实现任务转移

Method: 采用EfficientDet-Lite3作为关键区域检测器，结合MobileNet-V2分类模型，部署在Android设备上，无需高级医疗人员、实验室或互联网连接

Result: 分类模型在测试集上达到准确率92.31%、敏感度98.24%、特异性88.37%的性能

Conclusion: 该轻量级AI系统为资源稀缺地区提供了一种有前景的自动化宫颈癌筛查方案，可以提高筛查效率和可及性
...
### [260] [Colon Polyps Detection from Colonoscopy Images Using Deep Learning](https://arxiv.org/abs/2508.13188)
*Md Al Amin,Bikash Kumar Paul*

Main category: eess.IV

TL;DR: 深度学习YOLOv5模型在结肠息肉检测中的应用，YOLOv5l版本表现最佳，mAP达到85.1%，IoU为0.86


<details>
  <summary>Details</summary>
Motivation: 结直肠癌是全球相关死亡率最高的癌症之一，早期检测息肉对改善患者预后至关重要

Method: 使用Kvasir-SEG数据集，进行数据增废，对YOLOv5s、YOLOv5m、YOLOv5l三个版本进行评估，数据分割为训练集80%、验证20%、测试20%

Result: YOLOv5l表现最优，平均精度(mAP)达到85.1%，平均交并比(IoU)为0.86

Conclusion: YOLOv5l在结肠息肉定位检测中显示出优异性能，为提高结直肠癌筛查准确性提供了有前景的工具
...
### [261] [Benchmarking GPT-5 for Zero-Shot Multimodal Medical Reasoning in Radiology and Radiation Oncology](https://arxiv.org/abs/2508.13192)
*Mingzhe Hu,Zach Eidex,Shansong Wang,Mojtaba Safari,Qiang Li,Xiaofeng Yang*

Main category: eess.IV

TL;DR: GPT-5在放射学、放射肿瘤学和医学物理学的多模态任务评估中显著优于GPT-4o，在医学图像问答、多语言跨模态理解和专业考试题目上表现突出，准确率最高达90.7%，超过人类通过阈值。


<details>
  <summary>Details</summary>
Motivation: 评估GPT-5在安全关键的医学领域（放射学、放射肿瘤学和医学物理学）中的多模态能力，验证其在整合医学图像、文本报告和定量数据方面的进步是否带来实际性能提升。

Method: 采用零样本评估方法，在三个代表性任务上对比GPT-5及其变体（GPT-5-mini、GPT-5-nano）与GPT-4o的性能：1）VQA-RAD放射学视觉问答基准；2）SLAKE多语言跨模态标注VQA数据集；3）150道医学物理委员会考试风格的多选题数据集。

Result: GPT-5在所有数据集上取得最高准确率：在胸部纵隔区域挑战性问题中提升+20.00%，肺部问题+13.60%，脑组织解读+11.44%。在物理考试题目上达到90.7%准确率（136/150），超过人类通过阈值，而GPT-4o仅为78.0%。

Conclusion: GPT-5在医学图像推理和领域特定数值问题解决方面相比GPT-4o有显著且一致的性能提升，展示了其在医学成像和治疗物理学专家工作流程中的增强潜力。
...
### [262] [PediDemi -- A Pediatric Demyelinating Lesion Segmentation Dataset](https://arxiv.org/abs/2508.13239)
*Maria Popa,Gabriela Adriana Visa*

Main category: eess.IV

TL;DR: 本文首次公开提供了一个儿科脱骨骼化病变分割数据集，包含13名患有脱骨骼化疾病的儿童的MRI扫描和病变分割指标，补充了目前公开数据集中儿科和非多发硬化症病例的空白。


<details>
  <summary>Details</summary>
Motivation: 目前脱骨骼化疾病的公开数据集主要集中在成人多发硬化症（MS）上，而儿科和其他脱骨骼化症状如恶急散在性脑精骨骼炎（ADEM）的数据非常缺乏，影响了这些疾病的诊断和研究进展。

Method: 研究收集了13名儿科脱骨骼化症患者的MRI扫描数据，包括3例ADEM症例，并提供了详细的病变分割指标和患者元数据（诊断、治疗、病历、实验室结果等）。使用现有MS数据集训练的最新病变分割模型来评估该数据集的质量和价值。

Result: 研究结果显示，使用MS数据集训练的模型在这个新的儿科脱骨骼化症数据集上进行测试时，显示了数据集多样性的重要性，说明不同类型脱骨骼化症需要专门的数据集来支持模型训练和评估。

Conclusion: 这个首个公开的儿科脱骨骼化症数据集为这些疾病的研究提供了重要资源，强调了建立多样化数据集的必要性，以支持更准确的诊断和病情监测模型的开发。
...
### [263] [InnerGS: Internal Scenes Rendering via Factorized 3D Gaussian Splatting](https://arxiv.org/abs/2508.13287)
*Shuxin Liang,Yihan Xiao,Wenlu Tang*

Main category: eess.IV

TL;DR: 通过3D高斯散点技术直接建模体积密度，实现了不依赖相机位始的内部场景重建，支持多模态数据输入


<details>
  <summary>Details</summary>
Motivation: 现有3DGS技术主要关注外部表面建模，缺乏对内部结构的深度理解能力，而内部场景重建在许多应用中至关重要

Method: 通过内部3D高斯分布直接建模连续体积密度，从稀疏切片数据中有效重建光滑详细的内部结构

Result: 方法可以重建光滑详细的内部结构，无需相机位始，即插即用，且本质上与任何数据模态都兼容

Conclusion: 该方法为内部场景重建提供了一种高效、灵活的解决方案，拓展了3DGS技术在内部结构分析领域的应用潜力
...
### [264] [Susceptibility Distortion Correction of Diffusion MRI with a single Phase-Encoding Direction](https://arxiv.org/abs/2508.13340)
...
### [265] [Towards Understanding and Harnessing the Transferability of Prognostic Knowledge in Computational Pathology](https://arxiv.org/abs/2508.13482)
*Pei Liu,Luping Ji,Jiaxiang Gou,Xiangxiang Zeng*

Main category: eess.IV

TL;DR: 这篇论文提出了Path-PKT方案，系统研究了疾病预后知识在不同癌症之间的转移能力，解决传统模型无法利用其他癌症预后知识的问题，并提出了新的MoE-PKT方法来利用跨癌症的通用预后知识。


<details>
  <summary>Details</summary>
Motivation: 传统的WSI预后模型每种癌症开发一个模型，无法利用其他癌症的预后知识，导致在稀有癌症和样本有限的情况下表现差强，需要解决跨癌症知识转移的问题。

Method: 构建包含13种癌症的UNI2-h-DSS数据集，设计实验分析知识转移的因素，提出了具有路由机制的MoE-PKT方法来利用其他癌症的通用预后知识。

Result: 证明了源模型向稀有癌症转移知识的可行性，并通过实验发现了影响知识转移的关键因素和正向转移的原因。

Conclusion: 这项研究为WSI基于癌症预后的知识转移领域奠定了坚实基础，提出的Path-PKT框架有力地解决了传统模型在稀有癌症上的局限性，并为跨癌症预后知识分享提供了新的解决方案。
...
### [266] [State of Abdominal CT Datasets: A Critical Review of Bias, Clinical Relevance, and Real-world Applicability](https://arxiv.org/abs/2508.13626)
...
TL;DR: 对46个公开腹部CT数据集的系统性回顾发现存在严重的数据冗余（59.1%病例重复使用）和地域偏见（75.3%来自北美和欧洲），在≥100病例的数据集中，63%存在领域偏移风险，57%存在选择偏差，这些因素会削弱AI模型在不同医疗环境中的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 评估公开腹部CT数据集在临床AI应用中的适用性，识别现有数据集存在的偏见和局限性，为开发更公平、稳健的AI模型提供数据质量改进方向。

Method: 系统性回顾分析了46个公开腹部CT数据集（共50,256项研究），对≥100病例的19个数据集进行了偏差评估，重点关注病例重复使用率、地域分布以及领域偏移和选择偏差等高风险类别。

Result: 发现59.1%的病例重复使用，75.3%数据来自北美和欧洲，在大型数据集中63%存在领域偏移风险，57%存在选择偏差，这些偏差会严重影响AI模型在资源有限环境中的泛化性能。

Conclusion: 建议通过多机构合作、采用标准化协议、有意纳入多样化患者群体和成像技术等针对性策略来改进数据集质量，这对开发更公平和临床稳健的腹部成像AI模型至关重要。
...
### [267] [subCellSAM: Zero-Shot (Sub-)Cellular Segmentation for Hit Validation in Drug Discovery](https://arxiv.org/abs/2508.13701)
*Jacob Hanimann,Daniel Siegismund,Mario Wieser,Stephan Steigele*

Main category: eess.IV

TL;DR: 一种新的零样本分割方法，利用基础模型和上下文学习策略，通过自动提示机制实现核、细胞和子细胞结构的高精度分割，无需数据集特定调整。


<details>
  <summary>Details</summary>
Motivation: 解决高速迅速显微镜细胞图像分析中的分割问题，避免传统方法需要大量手动参数调整或领域特定模型微调的问题。

Method: 使用分割基础模型进行零样本分割，采用上下文学习策略，通过三步过程实现核、细胞和子细胞分割，使用自动提示机制编码形态学和拓扑学先验知识。

Result: 在标准细胞分割测试集和产业相关命中验证中，方法能够准确分割生物学相关结构，无需数据集特定调整。

Conclusion: 该方法提供了一种高效、通用的细胞图像分析方案，为生物制药药物发现提供了更好的自动化分析能力。
...
### [268] [Deep Biomechanically-Guided Interpolation for Keypoint-Based Brain Shift Registration](https://arxiv.org/abs/2508.13762)
*Tiago Assis,Ines P. Machado,Benjamin Zwick,Nuno C. Garcia,Reuben Dorent*

Main category: eess.IV

TL;DR: 一种深度学习框架，通过生物力学模拟数据训练3D U-Net，从稀疏关键点估计高精度的大脑变形场，显著改善了经典插值方法的精度


<details>
  <summary>Details</summary>
Motivation: 在神经外科手术中，准确补偿大脑移位对于维持神经导航的可靠性至关重要。虽然基于关键点的注册方法具有强壁性，但常用简单几何插值器忽略了组织生物力学特性

Method: 首先使用生物力学模拟生成大量合成大脑变形数据集，然后训练殊差3D U-Net网络，将标准插值估计精炼为生物力学导向的变形场

Result: 在大量模拟变形场上的实验显示，该方法显著超越了经典插值器，将均方误差降低了一半，同时在推理时间上引入可忽略的计算开销

Conclusion: 该深度生物力学插值器能够从稀疏匹配关键点生成高精度、物理可行的密集大脑变形场，为神经外科导航提供了更为可靠的解决方案
...
### [269] [Comparing Conditional Diffusion Models for Synthesizing Contrast-Enhanced Breast MRI from Pre-Contrast Images](https://arxiv.org/abs/2508.13776)
...
TL;DR: 使用去噪扩散概率模型从预对比MRI合成动态对比增强MRI，在单乳和全乳设置中评估了22种模型变体，通过肿瘤感知损失和分割掩码条件提升病灶保真度，证明合成图像具有高真实性和临床潜力。


<details>
  <summary>Details</summary>
Motivation: 传统DCE-MRI依赖对比剂存在安全性问题、禁忌症、成本增加和工作流程复杂等问题，需要开发无对比剂的替代方案。

Method: 提出预对比条件去噪扩散概率模型，引入肿瘤感知损失函数和显式肿瘤分割掩码条件，在公开多中心数据集上评估22种生成模型变体。

Result: 基于减影图像的模型在五个评估指标上 consistently 优于后对比基线模型，肿瘤感知损失和分割掩码输入均能提升评估指标，读者研究证实合成图像具有高真实性。

Conclusion: 生成式对比增强方法展现出新兴的临床潜力，能够在不使用对比剂的情况下合成高质量的DCE-MRI图像，但肿瘤定位输入在筛查环境中不一定可用。
...
### [270] [Optimizing Region of Interest Selection for Effective Embedding in Video Steganography Based on Genetic Algorithms](https://arxiv.org/abs/2508.13710)
*Nizheen A. Ali,Ramadhan J. Mstafa*

Main category: eess.IV

TL;DR: 一种基于遗传算法的视频隐文技术，通过AES加密和ROI区域选择来实现高效的数据隐藏


<details>
  <summary>Details</summary>
Motivation: 因互联网普及带来的数据传输安全和隐私需求增长，需要研究能够在保持视频质量的同时实现高效数据嵌入的隐文技术

Method: 使用遗传算法(GA)识别视频中的关键区域(ROI)，先用AES标准加密秘密数据，然后将数据嵌入到视频覆盖中(占覆盖视频容量的10%)，通过PSNR和编码/解码时间进行性能评估

Result: 方法实现了高嵌入容量和高效率，PSNR值在64-75 dB之间，表明嵌入数据与原始视频几乎无法区分，编码和解码速度快，适用于实时应用

Conclusion: 该方法通过结合遗传算法和AES加密，实现了高质量、高效率的视频隐文技术，能够在保持视频质量的同时确保数据安全性，适用于实时数据传输应用场景
...
### [271] [Latent Interpolation Learning Using Diffusion Models for Cardiac Volume Reconstruction](https://arxiv.org/abs/2508.13826)
...
TL;DR: 通过潜空间扩散模型实现高效的心脏MRI稀疏切片3D重建，提升重建质量和计算效率，无需附加输入


<details>
  <summary>Details</summary>
Motivation: 解决心脏MRI稀疏2D切片3D重建的问题，现有方法依赖预定义插值、计算效率低且需要附加语义输入

Method: 提出CaLID框架，采用潜空间扩散模型进行数据驱动插值，支持2D+T时空建模

Result: 计算效率提升24倍，重建质量达到SOTA水平，无需附加输入

Conclusion: 该框架充分解决了现有方法的限制，为心血管成像提供了健壮且临床实用的解决方案
...
### [272] [A Novel Attention-Augmented Wavelet YOLO System for Real-time Brain Vessel Segmentation on Transcranial Color-coded Doppler](https://arxiv.org/abs/2508.13875)
*Wenxuan Zhang,Shuai Li,Xinyi Wang,Yu Sun,Hongyu Kang,Pui Yuk Chryste Wan,Yong-Ping Zheng,Sai-Kit Lam*

Main category: eess.IV

TL;DR: 这篇论文提出了一种基于泡小变换和注意力机制的AAW-YOLO网络，能够在经颈多普勒超声数据中实时自动分割脑底动脉圈的血管，以减少对操作者经验的依赖。


<details>
  <summary>Details</summary>
Motivation: 经颈多普勒超声(TCCD)虽然具有无放射、便宜和易获得的优势，但其可靠性很大程度依赖操作者的规范性操作，这限制了其广泛应用。需要一种能够提供实时指导的自动化解决方案。

Method: 提出了一种新的Attention-Augmented Wavelet YOLO (AAW-YOLO)网络，专门为TCCD数据设计，能够实时分割脑底动脉圈的血管。采集了738帧注释数据和3,419个标签动脉实例来训练和评估模型。

Result: 模型在分割同侧和对侧脑底动脉圈血管时表现优异：平均Dice指数0.901、IoU 0.823、精度0.882、召回率0.926、mAP 0.953，每帧推理速度14.199ms。

Conclusion: 该系统为减少TCCD脑血管筛查对操作者经验的依赖提供了实用解决方案，在常规临床工作流程和资源有限环境中具有应用潜力。
...
### [273] [Learning to See Through Flare](https://arxiv.org/abs/2508.13907)
*Xiaopeng Peng,Heath Gemar,Erin Fleet,Kyle Novak,Abbie Watnik,Grover Swartzlander*

Main category: eess.IV

TL;DR: NeuSee是首个全可见光谱范围内的高保真传感器保护计算成像框架，通过联合学习衍射光学元件和频率空间Mamba-GAN网络，能够抑制高达传感器饱和阈值10^6倍的激光辐照度，在恢复图像质量方面提升10.1%。


<details>
  <summary>Details</summary>
Motivation: 机器视觉系统容易受到激光耀斑的影响，强烈的激光照射会通过过饱和或永久损坏传感器像素来致盲和扭曲其对环境的感知。现有方法无法在全可见光谱范围内提供有效的传感器保护。

Method: 联合学习衍射光学元件(DOE)的神经表示和频率空间Mamba-GAN网络进行图像恢复。采用端到端的对抗训练，在10万张独特图像上进行训练，利用异构数据和模型并行化进行分布式计算，整合高光谱信息和多个神经网络进行真实模拟和图像恢复。

Result: 能够抑制高达传感器饱和阈值10^6倍的激光辐照度，首次实现全光谱成像和激光抑制，恢复图像质量提升10.1%，优于其他学习型DOE方法。

Conclusion: NeuSee框架成功解决了机器视觉系统在开放世界场景中面临的激光干扰问题，为全可见光谱范围内的传感器保护提供了有效的解决方案，在图像质量和激光抑制方面都取得了显著提升。
...
### [274] [MMIS-Net for Retinal Fluid Segmentation and Detection](https://arxiv.org/abs/2508.13936)
*Nchongmaje Ndipenocha,Alina Mirona,Kezhi Wanga,Yongmin Li*

Main category: eess.IV

TL;DR: 提出MMIS-Net多模态医学图像分割网络，通过相似性融合块和one-hot标签空间处理多数据集训练，在RETOUCH挑战赛中取得最佳性能


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法大多在单一数据源上训练，忽视了其他可用标注数据的协同潜力，需要利用多模态、多器官、多疾病的医学图像数据集提升模型泛化能力

Method: 提出MMIS-Net网络，包含相似性融合块进行监督和像素级相似性知识选择，创建one-hot标签空间处理类别定义不一致和标签矛盾问题，在10个数据集（19个器官，2种模态）上训练单一模型

Result: 在RETOUCH挑战赛隐藏测试集上超越大型基础模型和其他SOTA算法，获得最佳平均Dice分数0.83，绝对体积差异0.035，流体检测任务AUC达到1

Conclusion: 相似性融合块和one-hot标签空间的有效结合解决了多数据集训练中的监督学习和标签不一致问题，证明了多源医学图像数据协同训练的优越性
...
### [275] [Real-Time, Population-Based Reconstruction of 3D Bone Models via Very-Low-Dose Protocols](https://arxiv.org/abs/2508.13947)
...
TL;DR: SSR-KD是一个快速AI框架，能在30秒内从双平面X光片重建高质量骨骼模型，平均误差小于1.0mm，无需CT扫描和人工标注


<details>
  <summary>Details</summary>
Motivation: 传统CT方法创建骨骼模型存在灵活性低、辐射高、手动标注耗时等问题，限制了临床应用，特别是在术中指导方面

Method: 提出半监督重建与知识蒸馏（SSR-KD）框架，利用双平面X光片快速重建骨骼模型

Result: 重建模型平均误差小于1.0mm，专家在高位胫骨截骨模拟中验证了其与CT重建模型相当的临床适用性

Conclusion: 该方法加速了重建过程，减少辐射暴露，支持术中指导，显著提高了骨骼模型的实用性，在骨科领域具有变革性应用
...
### [276] [UNICON: UNIfied CONtinual Learning for Medical Foundational Models](https://arxiv.org/abs/2508.14024)
*Mohammad Areeb Qazi,Munachiso S Nwadike,Ibrahim Almakky,Mohammad Yaqub,Numan Saeed*

Main category: eess.IV

TL;DR: UNICON是一个统一的持续学习框架，使医学基础模型能够无缝适应不同领域、任务和模态，无需大量数据即可整合新知识，避免灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 医学影像数据稀缺，难以针对每个领域、模态或任务进行预训练。持续学习可以通过顺序微调模型来整合新知识，而无需每个训练阶段都使用大型数据集。
...
Conclusion: 基础模型并不局限于初始训练范围，可以不断发展演进，为医学影像通用AI模型铺平道路。
...
### [277] [Machine Learning H-theorem](https://arxiv.org/abs/2508.14003)
*Ruben Lier*

Main category: cond-mat.stat-mech

TL;DR: 使用DeepSets架构研究硬磁盘系统的H定理和不可逆性


<details>
  <summary>Details</summary>
Motivation: H定理为热力学第二定律提供微观基础，但至今仍存在争议，需要更好地理解H定理与时间箭头的关系

Method: 研究周期性边界条件下随机取向和位置的硬磁盘平衡过程，使用基于DeepSets架构的模型来捕捉H泛函的不可逆性

Result: 模型成功捕获了H泛函的不可逆性特征

Conclusion: 基于置换不变性的深度学习方法可以有效研究H定理和热力学不可逆性问题
...
TL;DR: OrbitChain是一个协作分析框架，通过将分析任务分解为微服务并在卫星星座中分配计算资源，实现地球观测数据的实时分析，减少通信开销并提高处理能力。


<details>
  <summary>Details</summary>
Motivation: 现有地球观测卫星由于带宽和连接时间限制，数据下载和分析需要数小时甚至数天，无法满足实时应用需求如灾害响应。

Method: 将分析应用分解为微服务，在卫星星座中分配计算资源，设计流量路由算法最小化星间通信开销，采用流水线工作流程实现实时任务完成。

Result: 系统能比现有框架完成多达60%的分析工作量，同时将通信开销降低高达72%。

Conclusion: OrbitChain框架能够有效支持实时地球观测分析，促进时间敏感应用和星座间协作。
...
### [279] [DDoS Attacks in Cloud Computing: Detection and Prevention](https://arxiv.org/abs/2508.13522)
*Zain Ahmad,Musab Ahmad,Bilal Ahmad*

Main category: cs.DC

TL;DR: 本文对DDoS攻击类型、检测技术和防御方法进行了全面综述分析，为组织和个人提供网络安全防护指南


<details>
  <summary>Details</summary>
Motivation: DDoS攻击已成为最普遍和有害的网络安全威胁之一，近年来攻击复杂性和频率显著增加，需要有效的检测和缓解方法

Method: 分析不同类型的DDoS攻击（流量型、协议型、应用层攻击），研究现有检测技术（包过滤、入侵检测系统、机器学习方法）和防御技术（防火墙、速率限制、CPP和ELD机制）

Result: 评估了各种方法的有效性和适用性，提供了针对不同类型攻击和环境的防护方案

Conclusion: 本研究为组织和个人提供了全面的DDoS攻击防护指南，有助于增强网络安全态势并有效防御DDoS攻击
...
### [280] [Combating Homelessness Stigma with LLMs: A New Multi-Modal Dataset for Bias Detection](https://arxiv.org/abs/2508.13187)
*Jonathan A. Karr Jr.,Benjamin F. Herbst,Ting Hua,Matthew Hauenstein,Georgina Curto,Nitesh V. Chawla*

Main category: cs.CY

TL;DR: 这篇论文提出了一种基于NLP和LLM的新方法，用于识别和测量网络话语中对无家归者的社会偏见，通过多源数据集和LLM分类技术来提高识别准确度。


<details>
  <summary>Details</summary>
Motivation: 无家归者问题是一个持续的社会挑战，社会歪曲是缓解该问题的重要障碍。网络和市政厅讨论反映并影响公众意见，为识别和跟踪社会偏见提供了价值。

Method: 构建了一个手动注释的多模态数据集，源自Reddit、X、新闻文章和10个美国城市的市政厅会议纪录。采用零样本和少样本分类技术，测试了本地LLM（Llama 3.2 3B、Qwen 2.5 7B、Phi4⧗和闭源API模型（GPT-4.1、Gemini 2.5 Pro、Grok-4）。

Result: 尽管本地LLM在零样本分类中存在显著不一致性，但其上下文学习分类结果接近闭源LLM。且LLM在所有类别上的平均表现超过了BERT。

Conclusion: 该研究旨在提高对无家归者偏见的认知，开发新指标以信息政策，并提高生成式AI技术的公平性和道德应用。
...
### [281] [Preliminary suggestions for rigorous GPAI model evaluations](https://arxiv.org/abs/2508.00875)
*Patricia Paskov,Michael J. Byun,Kevin Wei,Toby Webster*

Main category: cs.CY

TL;DR: 本文提供了一份关于通用人工智能(GPAI)评估实践的初步编稿，包含提升内部效度、外部效度和可复现性的建议。


<details>
  <summary>Details</summary>
Motivation: 为了提供一个结构化的框架来指导GPAI评估实践，激励GPAI评估科学领域的对话和发展。

Method: 结合机器学习、统计学、心理学、经济学、生物学等领域的成熟实践，按评估生命周期四个阶段(设计、实施、执行、文档化)组织建议。

Result: 提供了一套系统的评估建议，包括人类提升研究和基准评估的建议，以及适用于多种评估类型的跨领域建议。

Conclusion: 本文为GPAI模型提供商、第三方评估机构、政策制定者和学术研究人员提供了一个实用的评估框架，有助于满足EU AI法案的评估要求并推动GPAI评估科学的发展。
...
### [282] [Toward an African Agenda for AI Safety](https://arxiv.org/abs/2508.13179)
...
TL;DR: 本文分析了非洲独特的AI风险状况，包括深度伪造选举干预、数据殖民依赖、计算资源稀缺、劳动力市场破坏和气候环境成本等问题，提出了五点行动计划来保护人权和建立AI安全治理体系。


<details>
  <summary>Details</summary>
Motivation: 非洲在AI发展中面临特殊的安全风险，但非洲视角尚未被充分纳入全球AI安全讨论，非洲利益相关者对新兴的全球AI安全治理议程影响力有限，需要制定针对性的保护措施。

Method: 通过识别非洲特有的AI风险状况，提出五点行动计划：以保护人权为核心的政策方法、建立非洲AI安全研究所、提升公众AI素养、开发多语言预警系统、举办年度AI安全论坛。

Result: 系统性地识别了非洲在AI安全方面面临的独特挑战，包括技术依赖、资源限制和社会脆弱性等问题，为制定针对性的治理框架提供了基础。

Conclusion: 非洲需要建立本土化的AI安全治理体系，通过机构建设、公众教育和多语言技术支持来应对AI带来的特殊风险，确保AI发展符合非洲人民的利益和人权保护要求。
...
### [283] [The Course Difficulty Analysis Cookbook](https://arxiv.org/abs/2508.13218)
*Frederik Baucks,Robin Schmucker,Laurenz Wiskott*

Main category: cs.CY

TL;DR: 这篇论文提供了课程难度测量方法的第一个全面综述和比较，包括基于平均成绩点和潜在特质模型的方法，并提供实践教程和开源软件包。


<details>
  <summary>Details</summary>
Motivation: 课程难度测量对于课程分析的质量控制、课程比较和推荐至关重要，但需要考虑学生多样性带来的偏差问题，确保测量的可靠性、有效性和公平性。

Method: 综述和比较现有的课程难度测量方法，包括基于平均成绩点和潜在特质模型的方法，提供模型选择、假设检验的实践教程。

Result: 论文提供了完整的方法评估框架，支持监控课程难度随时间的变化，以及检测不同学生群体间的差异效果，如退学生与毕业生的差异。

Conclusion: 该研究最终目标是促进高质量、公平和公正的学习体验，通过提供开源软件包和人工数据集支持进一步研究和应用。
...
### [284] [Consumer Autonomy or Illusion? Rethinking Consumer Agency in the Age of Algorithms](https://arxiv.org/abs/2508.13440)
*Pegah Nokhiz,Aravinda Kanchana Ruwanpathirana*

Main category: cs.CY

TL;DR: 数字时代消费者自主性受到系统性局限，包括强制消费、算法操控和收入不稳定等因素。通过形式模型分析发现，即使理性消费者也可能遇到早期财务危机。研究建议通过系统干预和消费者教育来强化消费自主性。


<details>
  <summary>Details</summary>
Motivation: 数字化时代消费者自主性受到新兴娱体系统、算法操纷和工作时间不稳定等多重挑战，需要量化分析这些因素如何影响消费者的财务健康和长期稳定性。

Method: 构建基于折扣消费的形式模型，分析消费者在强制支付、算法影响的内心消费和不可预测收入等约束条件下的行为。通过分析性情景模拟证明自主性受限导致的风险。

Result: 研究发现即使是理性、效用最大化的消费者，在结构性、行为性或时间性自主性受限时，也可能遇到早期财务死亡。自主性限制与财务不稳定风险存在严格的数学关联。

Conclusion: 消费者自主性应被视为需要主动培育的价值，而非理所当然。系统性干预和消费者教育能够有效强化自主性，支持价值思考和明智选择。形式模型为消费价值观提供了实证基础。
...
### [285] [The AI Risk Spectrum: From Dangerous Capabilities to Existential Threats](https://arxiv.org/abs/2508.13700)
*Markov Grey,Charbel-Raphaël Segerie*

Main category: cs.CY

TL;DR: 这篇论文系统地分析了AI风险的全景范围，从当前对个人的害处到可能危及人类生存的存在风险，并将其分为恶意使用、系统错误和系统性风险三大类别。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统变得越来越强大、集成度越来越高和应用越来越广泛，理解相关风险变得至关重要。论文的动机是帮助读者理解AI风险的完整地图，以便更好地应对这些挑战。

Method: 论文将AI风险组织成三个主要因果类别：1）恶意使用风险（人为故意使用AI造成害处）；2）系统错误风险（AI系统追求与人类价值冲突的目标）；3）系统性风险（AI集成到复杂社会系统中渐进地損害人类自主权）。同时识别了加剧风险的因素。

Result: 论文展示了从当前存在的AI风险和可观察到的AI行为到可能的最终灾难性结果的连续体系，证明了现有趋势如何可能升级为灾难性后果。

Conclusion: 良好的未来是可能实现的，但不会自动发生。应对这些挑战需要前所未有的协调作为，但如果成功应对，人类将迎来一个不平凡的未来。
...
### [286] [Query Logs Analytics: A Aystematic Literature Review](https://arxiv.org/abs/2508.13949)
*Dihia Lanasri*

Main category: cs.DB

TL;DR: 这是一份关于日志使用的系统性调查报告，分析了数据库、数据仓库、网站和知识图谱日志的共同特征、标准处理流程和约束条件


<details>
  <summary>Details</summary>
Motivation: 数字平台中用户交互留下的日志具有重要价值，但相关研究分散且缺乏综合性研究，需要系统整理现有知识

Method: 系统性调查分析了300多篇公开出牌物，重点研究三个核心问题：日志结构和功能特征、标准处理流程、约束条件和非功能性要求

Result: 调查发现少有完整的端到端方案，日志使用流程缺乏标准化，但不同类型日志存在共享结构元素

Conclusion: 该调查报告为研空者和实践者提供了日志使用的全面概览，指明了未来研究方向，尤其是知识图谱日志的利用和普及化
...
### [287] [The Rise of Generative AI for Metal-Organic Framework Design and Synthesis](https://arxiv.org/abs/2508.13197)
...
TL;DR: AI生成方法正在变革金属有机框架(MOF)的设计发现方式，从传统枚举转向使用深度学习模型自主提出新型多孔网状结构


<details>
  <summary>Details</summary>
Motivation: 传统MOF候选材料枚举方法费时费力，需要更高效的方法来加速高性能MOF材料的发现，特别是在清洁空气和能源应用领域

Method: 采用变分自编码器、扩散模型和基于大语言模型的智能体等深度学习模型，结合高通量计算筛选和自动化实验形成闭环发现流程

Result: 建立了加速网状化学发现的新范式，AI算法能够更高效地指导高性能MOF材料的搜索

Conclusion: 虽然AI生成方法在MOF设计中取得显著进展，但仍面临合成可行性、数据集多样性和领域知识整合等挑战
...
### [288] [Preference Models assume Proportional Hazards of Utilities](https://arxiv.org/abs/2508.13189)
*Chirag Nagpal*

Main category: stat.ML

TL;DR: 本文连接了Plackett-Luce模型和Cox比例风险模型，探讨了这种连接对AI对齐工具（如奖励建模和直接偏好优化）的统计基础的影响。


<details>
  <summary>Details</summary>
Motivation: 现代AI对齐工具（如奖励建模和直接偏好优化）基于Plackett-Luce模型的统计假设，但该模型与另一个经典统计模型Cox比例风险模型之间存在联系，需要探索这种连接的意义。

Method: 通过理论分析将Plackett-Luce模型与Cox比例风险模型进行连接，研究两者之间的数学关系和统计基础。

Result: 建立了Plackett-Luce模型和Cox比例风险模型之间的理论联系，揭示了这种连接对理解偏好估计统计基础的重要意义。

Conclusion: Plackett-Luce模型与Cox比例风险模型的连接为AI对齐工具的统计假设提供了新的理论视角，有助于更好地理解偏好估计的统计基础。
...
### [289] [Structural Foundations for Leading Digit Laws: Beyond Probabilistic Mixtures](https://arxiv.org/abs/2508.13237)
*Vladimir Berman*

Main category: stat.ML

TL;DR: 这篇论文提出了一种现代确定性框架，用于研究数值数据中首位数字的分布。通过解析数据生成过程的算术、算法和结构性质，解释了观察到的首位数字频率分布，包括较大偏离对数分布的情况。


<details>
  <summary>Details</summary>
Motivation: 传统的概率论或混合模型方法无法充分解释数字数据中观察到的首位数字分布的多样性，需要一种更基础的确定性数学框架来统一理解这些数字现象。

Method: 采用移位不变性函数方程的方法，通过显式的阿尔宜加周期公式获得一般解。系统分析了有限和无限数据集中的数字分布，研究了谨数和递推关系等确定性序列。

Result: 建立了一个统一的数学基础来理论数字现象，能够解释应用和理论上下文中遇到的各种数字模式，包括块结构和分形特征的出现。

Conclusion: 这项工作为数字现象提供了一个统一的数学基础，并提供了一套多用途工具集，用于建模和分析应用和理论上下文中的数字模式，同时评估了概率模型的局限性并提出了未来研究方向。
...
### [290] [Flow Matching-Based Generative Modeling for Efficient and Scalable Data Assimilation](https://arxiv.org/abs/2508.13313)
*Taos Transue,Bohan Chen,So Takao,Bao Wang*

Main category: stat.ML

TL;DR: 提出基于流匹配(Flow Matching)的集成流滤波器(EnFF)，通过加速采样和灵活设计概率路径来改进高维非线性数据同化问题，相比现有方法具有更好的计算效率与精度平衡。


<details>
  <summary>Details</summary>
Motivation: 传统数据同化方法在高维非线性系统中面临计算负担重的问题，特别是基于生成模型的集成分数滤波器(EnSF)采样速度慢，需要更高效的解决方案。

Method: 开发训练自由的集成流滤波器(EnFF)，整合边际流匹配向量场的蒙特卡洛估计器和局部化指导来同化观测数据，提供更灵活的向量场设计和更快的采样速度。

Result: 在高维滤波基准测试中展示了改进的成本-精度权衡，能够利用比先前方法更大的集成规模，计算效率显著提升。

Conclusion: 流匹配作为高维滤波应用的可扩展工具具有巨大潜力，能够支持大规模集成使用，为高维非线性数据同化提供了新的有效框架。
...
### [291] [Smooth Flow Matching](https://arxiv.org/abs/2508.13831)
...
### [292] [Online Conformal Selection with Accept-to-Reject Changes](https://arxiv.org/abs/2508.13838)
*Kangdao Liu,Huajun Xi,Chi-Man Vong,Hongxin Wei*

Main category: stat.ML

TL;DR: 提出OCS-ARC方法，扩展保形选择到在线场景，支持不可逆选择决策，在控制FDR的同时提高选择能力


<details>
  <summary>Details</summary>
Motivation: 传统保形选择在在线场景中允许取消已选择候选，这与需要不可逆选择决策的应用（如药物发现）不兼容

Method: 将在线Benjamini-Hochberg程序整合到候选选择过程中，提出OCS-ARC方法，支持接受-拒绝变更机制

Result: 理论保证在任何时间步控制FDR在名义水平以下，实验显示显著提高选择能力同时保持有效FDR控制

Conclusion: OCS-ARC成功解决了在线保形选择中的不可逆选择需求，为资源密集型序列过程提供了实用解决方案
...
### [293] [Generalisation and benign over-fitting for linear regression onto random functional covariates](https://arxiv.org/abs/2508.13895)
*Andrew Jones,Nick Whiteley*

Main category: stat.ML

TL;DR: 研究在随机函数型协变量设置下，岭回归和无岭最小二乘回归的理论预测性能，分析协变量噪声在良性过拟合中的作用。


<details>
  <summary>Details</summary>
Motivation: 传统回归分析通常假设数据独立同分布，但实际应用中协变量向量往往来自潜在度量空间上随机函数的评估结果，具有可交换性而非独立性，需要新的理论框架。

Method: 利用Barzilai和Shamir的最新结果，在四阶矩和其他正则性条件下，对随机函数型协变量设置下的预测超额风险概念获得概率界限。

Result: 在p相对于n以适当速度增长的机制中推导出收敛速率，展示了模型各成分在决定收敛行为中的相互作用。

Conclusion: 协变量加性噪声在良性过拟合现象中发挥重要作用，该研究为函数型数据分析提供了新的理论支撑。
...
### [294] [A PC Algorithm for Max-Linear Bayesian Networks](https://arxiv.org/abs/2508.13967)
*Carlos Améndola,Benjamin Hollering,Francesco Nowell*

Main category: stat.ML

TL;DR: 本文研究最大线性贝叶斯网络(MLBNs)的因果发现算法，MLBNs由于具有重尾分布特性，通常不满足d-分离的忠实性，传统PC算法无法准确恢复图结构。


<details>
  <summary>Details</summary>
Motivation: MLBNs作为结构方程模型在处理重尾分布数据时具有重要应用价值，但由于其不满足传统d-分离忠实性，需要开发新的因果发现方法来准确恢复图结构。

Method: 提出了基于*-分离准则的PC算法一致性证明，并开发了新的PCstar算法，该算法假设对C*-分离忠实，能够定向更多无法通过d-或*-分离定向的边。

Result: 证明了在给定真实图中*-分离条件独立性测试oracle的情况下，PC算法仍然保持一致性；PCstar算法能够比传统方法定向更多的边。

Conclusion: 针对MLBNs的特殊性质，基于*-分离的因果发现方法能够有效解决传统算法失效的问题，PCstar算法提供了更强大的边定向能力。
...
### [295] [Uncertainty-Aware PCA for Arbitrarily Distributed Data Modeled by Gaussian Mixture Models](https://arxiv.org/abs/2508.13990)
*Daniel Klötzl,Ozan Tastekin,David Hägele,Marina Evers,Daniel Weiskopf*

Main category: stat.ML

TL;DR: 提出基于高斯混合模型的多维不确定性数据降维方法，比UAPCA能更准确地表示分布细节，支持用户自定义权重


<details>
  <summary>Details</summary>
Motivation: 多维数据常包含非正态分布的不确定性，现有UAPCA方法不能很好地处理这类分布

Method: 使用高斯混合模型建模多维分布，推导任意概率密度函数的投影公式，支持用户自定义权重

Result: 低维投影比UAPCA显示更多分布细节，更忠实表示原始分布，与基于样本的投影结果一致

Conclusion: 该方法能有效处理非正态分布的不确定性数据降维，提供比UAPCA更优的分布表示能力
...
### [296] [Modeling GRNs with a Probabilistic Categorical Framework](https://arxiv.org/abs/2508.13208)
...
Method: 整合三种核心方法：范畴论提供形式化语言、贝叶斯类型Petri网建模随机过程、GFlowNet采样网络拓扑和HyperNetwork预测参数分布

Result: 建立了数学严谨、生物可解释且具有不确定性感知的GRN表示框架

Conclusion: PC-GRN框架推进了预测建模和系统级分析能力，为基因调控网络研究提供了新的理论方法
...
### [297] [Goal-Directedness is in the Eye of the Beholder](https://arxiv.org/abs/2508.13247)
*Nina Rajcic,Anders Søgaard*

Main category: cs.MA

TL;DR: 本文分析了目标导向行为的测量方法，认为无论是行为观察还是机制探测都无法客观测量目标导向性，并提出了将目标导向性建模为多智能体系统涌现特性的新方向。


<details>
  <summary>Details</summary>
Motivation: 研究复杂智能体行为预测的关键在于目标归因，但现有的行为方法和机制方法都存在技术和概念问题，需要重新思考目标导向性的测量方式。

Method: 通过分析行为方法和机制方法的基本假设，识别两种方法在形式化智能体系统目标时遇到的技术和概念问题。

Result: 发现目标导向性无法被客观测量，这是一个令人惊讶的结论。

Conclusion: 提出了新的研究方向：将目标导向性建模为动态多智能体系统的涌现属性，而不是可测量的客观特性。
...
### [298] [BetaWeb: Towards a Blockchain-enabled Trustworthy Agentic Web](https://arxiv.org/abs/2508.13787)
*Zihan Guo,Yuanjian Zhou,Chenyi Wang,Linlin You,Minjie Bian,Weinan Zhang*

Main category: cs.MA

TL;DR: 本文提出基于区块链的可信任自主代理网络BetaWeb，解决当前LLM多代理系统的生态碎片化问题，通过区块链技术实现隐私保护、数据管理和价值计量，推动从Web3向Web3.5的演进。


<details>
  <summary>Details</summary>
Motivation: 当前代理生态系统碎片化且闭合，中央化或半中央化范式无法支持大规模、异构性和跨域自主交互，需要建立互联互通的可扩展代理AI范式。

Method: 提出BetaWeb框架，利用区块链的内在优势为LaMAS提供可信任、可扩展的基础设施，并提出五阶段演进路线图，从被动执行到高级协作和自主治理。

Result: BetaWeb有力解决隐私保护、数据管理和价值计量核心挑战，为构建弹性、可信任和可持续激励的数字生态系统奠定基础。

Conclusion: 区块链与LaMAS的深度集成能够培育出一个健壮、可信任且具有可持续激励机制的数字生态系统，推动代理AI从数据所有权向智能能力所有权和商业化的Web3.5新范式发展。
...
### [299] [End-to-End Audio-Visual Learning for Cochlear Implant Sound Coding in Noisy Environments](https://arxiv.org/abs/2508.13576)
*Meng-Ping Lin,Enoch Hsin-Ho Huang,Shao-Yi Chien,Yu Tsao*

Main category: eess.AS

TL;DR: 这篇论文提出了一种新的噪声压制耳螺安装系统AVSE-ECS，通过视觉-听觉语音增强模型作为预处理模块，在噪声环境中显著提升了语音可懂性。


<details>
  <summary>Details</summary>
Motivation: 虽然耳螺安装系统有显著进步，但在噪声或深深环境中的语音理解仍面临挑战，需要利用深度学习技术来提升噪声压制能力。

Method: 提出AVSE-ECS系统，使用视觉-听觉语音增强模型作为深度学习基础的ElectrodeNet-CS声音编码策略的预处理模块，采用联合训练方法构建端到端的CI系统。

Result: 实验结果显示，该方法在噪声条件下表现优于之前的ECS策略，语音可懂性得到显著提升。

Conclusion: 这项研究证明了使用深度学习将AVSE模块集成到端到端CI系统中的可行性和潜力。
...
### [300] [Utilizing the RAIN method and Graph SAGE Model to Identify Effective Drug Combinations for Gastric Neoplasm Treatment](https://arxiv.org/abs/2508.13207)
*S. Z. Pirasteh,Ali A. Kiaei,Mahnaz Bush,Sabra Moghadam,Raha Aghaei,Behnaz Sadeghigol*

Main category: q-bio.QM

TL;DR: RAIN方法结合Graph SAGE和网络meta分析，识别出奥沙利铂、氟尿嘧啶和曲妥珠单抗三联疗法作为胃癌最优药物组合，显著提高疗效。
...
Method: 使用Graph SAGE构建药物-基因-蛋白质网络模型，通过NLP和文献检索验证药物，最后进行网络meta分析评估疗效。
...
### [301] [AlphaX: An AI-Based Value Investing Strategy for the Brazilian Stock Market](https://arxiv.org/abs/2508.13429)
*Paulo André Lima de Castro*

Main category: q-fin.CP

TL;DR: 这篇论文提出了一种受价值投资启发的AI交易策略AlphaX，通过控制预览偏差等偏差来减少过拟合风险，在巴西市场表现超过主要指数和常见技术指标。


<details>
  <summary>Details</summary>
Motivation: 当前的AI交易策略在历史数据回测中表现良好，但在真实市场中表现下降，特别是风险调整收益方面。重点解决预览偏差和其他偏差导致的过拟合问题。

Method: 采用受价值投资启发的AI策略，进行计算模拟实验时严格控制各种偏差，减少过拟合风险。对比常见技术指标如RSI和MFI。

Result: AlphaX策略在巴西市场表现超过主要指数，且较RSI和MFI等技术指标更优，结果具有统计显著性。

Conclusion: 研究证明了控制偏差对于AI交易策略成功的重要性，并提出了将来可能通过定性分析技术完善AI基于价值投资的框架。
...
### [302] [Physics-Informed Neural Networks for Programmable Origami Metamaterials with Controlled Deployment](https://arxiv.org/abs/2508.13559)
*Sukheon Kang,Youngkwon Kim,Jinkyu Yang,Seunghwa Ryu*

Main category: cond-mat.soft

TL;DR: 提出基于物理信息神经网络(PINN)的框架，用于锥形Kresling折纸结构的正向预测和逆向设计，无需预收集训练数据，能够精确编程完整能量曲线和分层部署序列。


<details>
  <summary>Details</summary>
Motivation: 折纸启发的结构具有轻质、可展开和可编程机械响应的优势，但其设计面临复杂非线性力学、多稳态和部署力精确控制的挑战，需要新的设计方法。

Method: 将力学平衡方程直接嵌入学习过程，构建物理信息神经网络框架，实现正向能量景观预测和逆向设计，可指定目标稳定状态高度和分离能垒。

Result: 模型能高精度预测完整能量景观并最小化非物理伪影，有限元模拟和物理原型实验验证了设计的部署序列和能垒比值的鲁棒性。

Conclusion: 该工作为折纸启发的超材料中复杂机械能量景观编程建立了一种通用、无需数据的途径，在可展开航空航天系统、变形结构和软体机器人执行器方面具有广阔应用潜力。
...
### [303] [Towards safe control parameter tuning in distributed multi-agent systems](https://arxiv.org/abs/2508.13608)
*Abdullah Tokmak,Thomas B. Schön,Dominik Baumann*

Main category: eess.SY

TL;DR: 通过安全贝叶斯优化和高斯过程回归解决分布式多自甲优化问题，采用空间-时间内核来处理非邻居自甲的影响，实现样本效率和安全性的平衡。


<details>
  <summary>Details</summary>
Motivation: 解决自主驾驶和协作机器人等安全关键应用中的分布式多自甲优化问题，充分考虑样本效率、安全约束和未知非凸奖励函数的复杂性。

Method: 使用安全贝叶斯优化和高斯过程回归，通过近邻通信和时间变化本地优化问题重构形式，设计了自定义空间-时间内核来模型非邻居自甲的影响。

Result: 通过模拟实验验证了算法的成功部署，能够在未知非凸奖励和约束条件下高效地解决分布式多自甲优化问题。

Conclusion: 所提方法有效地结合了安全性、样本效率和分布式处理的需求，为安全关键应用中的多自甲系统优化提供了可行解决方案。
...
### [304] [Model-based Multi-object Visual Tracking: Identification and Standard Model Limitations](https://arxiv.org/abs/2508.13647)
*Jan Krejčí,Oliver Kost,Yuxuan Xia,Lennart Svensson,Ondřej Straka*

Main category: eess.SY

TL;DR: 使用雷达追踪领域的多目标跟踪方法进行行人跟踪，采用标准点对象模型和PMBM滤波器，在MOT-17数据集上获得有希望的结果但发现模型与数据不匹配问题


<details>
  <summary>Details</summary>
Motivation: 将雷达追踪社区成熟的多目标跟踪方法应用于基于2D边界框检测的行人跟踪问题，探索模型驱动方法的有效性

Method: 采用标准点对象(SPO)模型，使用泊松多伯努利混合(PMBM)滤波器计算后验密度，参数部分基于第一原理选择，部分从MOT-17数据集识别

Result: PMBM算法显示出有希望的结果，但揭示了SPO模型与数据之间存在不匹配问题

Conclusion: 虽然当前模型存在不匹配问题，但基于模型的方法认为通过修正导致SPO模型与数据不匹配的问题组件，将在未来开发中产生更好的模型驱动算法
...
### [305] [Multi-Plasticity Synergy with Adaptive Mechanism Assignment for Training Spiking Neural Networks](https://arxiv.org/abs/2508.13673)
*Yuzhe Liu,Xin Deng,Qiang Yu*

Main category: cs.NE

TL;DR: 提出了一种融合多种突触可塑性机制的生物启发式SNN训练框架，相比单一学习机制模型显著提升了性能和鲁棒性


<details>
  <summary>Details</summary>
Motivation: 当前SNN训练方法通常依赖单一形式的突触可塑性，限制了网络的适应性和表征能力，而大脑中存在多种协同学习策略

Method: 开发了一个生物启发的训练框架，整合多种协同可塑性机制，使不同学习算法能够合作调节信息积累，同时保持各自相对独立的更新动态

Result: 在静态图像和动态神经形态数据集上的评估表明，该框架相比传统学习机制模型显著提升了性能和鲁棒性

Conclusion: 这项工作为开发基于多策略脑启发学习的更强大SNN提供了通用且可扩展的基础
...
### [306] [TaoSR1: The Thinking Model for E-commerce Relevance Search](https://arxiv.org/abs/2508.12365)
*Chenhe Dong,Shaowei Yao,Pengkun Jiao,Jianhui Yang,Yiming Jin,Zerui Huang,Xiaojiang Zhou,Dan Ou,Haihong Tang*

Main category: cs.IR

TL;DR: 提出了TaoSR1框架，直接部署大语言模型进行电商搜索相关性预测，通过三阶段训练和优化解决思维链错误累积、判别性幻觉和部署可行性问题。
...
Method: 三阶段框架：1)SFT+CoT注入推理能力；2)离线采样+DPO提升生成质量；3)难度动态采样+GRPO减轻判别性幻觉。采用后处理技术和概率分区方法实现高效在线部署。

Result: 在离线数据集上显著超越基线模型，在线人工评估中获得实质性收益提升。

Conclusion: TaoSR1为相关性分类任务引入了思维链推理的新范式，成功解决了LLM直接部署的关键挑战。
...
### [307] [LLM-Enhanced Linear Autoencoders for Recommendation](https://arxiv.org/abs/2508.13500)
*Jaewan Moon,Seongmin Park,Jongwuk Lee*

Main category: cs.IR

TL;DR: L3AE首次将大语言模型(LLMs)集成到线性自编码器(LAE)框架中，通过两阶段优化策略有效整合文本语义和用户-物品交互的异构知识，在推荐系统性能上取得显著提升


<details>
  <summary>Details</summary>
Motivation: 现有的线性自编码器在整合文本信息时依赖稀疏的词共现模式，无法充分捕捉丰富的文本语义信息，限制了推荐系统的性能

Method: 提出L3AE方法：1) 从LLM生成的物品表示构建语义物品-物品相关矩阵；2) 从协同信号学习物品-物品权重矩阵，同时将语义物品相关性作为正则化进行蒸馏。每个阶段都通过闭式解优化，确保全局最优和计算效率

Result: 在三个基准数据集上，L3AE持续优于最先进的LLM增强模型，Recall@20提升27.6%，NDCG@20提升39.3%

Conclusion: L3AE成功地将LLM集成到LAE框架中，通过有效的异构知识整合和闭式优化策略，显著提升了推荐系统的性能表现
...
### [308] [Research on Conversational Recommender System Considering Consumer Types](https://arxiv.org/abs/2508.13209)
*Yaying Luo,Hui Fang,Zhu Sun*

Main category: cs.IR

TL;DR: CT-CRS是一个基于消费者类型建模的对话推荐系统，通过实时推断用户类型并设计类型自适应策略，提高了推荐成功率和交互效率。


<details>
  <summary>Details</summary>
Motivation: 现有对话推荐系统大多忽视了用户的异质决策风格和知识水平，限制了推荐准确性和效率。

Method: 基于消费者类型理论定义四类用户，利用交互历史和LLM实时推断用户类型，设计类型自适应策略，并采用逆强化学习优化对话策略。

Result: 在LastFM、Amazon-Book和Yelp数据集上，CT-CRS提高了推荐成功率并减少了交互轮次，消融实验验证了消费者类型建模和IRL的有效性。
...
### [309] [AdaptJobRec: Enhancing Conversational Career Recommendation through an LLM-Powered Agentic System](https://arxiv.org/abs/2508.13423)
...
TL;DR: AdaptJobRec是一个对话式职位推荐系统，通过自主代理整合个性化推荐算法工具，根据查询复杂度采用不同处理机制，在保持推荐准确性的同时显著降低响应延迟达53.3%


<details>
  <summary>Details</summary>
Motivation: 传统对话推荐系统在处理复杂查询时存在显著的响应延迟问题，需要在处理复杂查询和最小化延迟之间找到平衡

Method: 系统采用用户查询复杂度识别机制：简单查询直接选择工具快速响应；复杂查询通过记忆处理模块过滤聊天历史，使用智能任务分解规划器，最后执行个性化推荐任务

Result: 在沃尔玛真实职业推荐场景中评估，相比竞争基线平均响应延迟降低53.3%，同时显著提高推荐准确性

Conclusion: AdaptJobRec成功解决了对话推荐系统中延迟与准确性的权衡问题，为实际应用提供了有效的解决方案
...
### [310] [Heterogeneous Influence Maximization in User Recommendation](https://arxiv.org/abs/2508.13517)
*Hongru Hou,Jiachen Sun,Wenqing Lin,Wendong Bi,Xiangrong Wang,Deqing Yang*

Main category: cs.IR

TL;DR: 该文章提出了HeteroIR和HeteroIM两个模型，解决了用户推荐系统中交互意愿与信息传播能力的综合问题，在腾讯游戏平台上获得了显著的在线效果提升。


<details>
  <summary>Details</summary>
Motivation: 传统推荐方法只关注交互意愿，而传播最大化方法忽视了交互意愿，存在两者脱节的问题。需要合理结合交互意愿与传播能力。

Method: HeteroIR采用两阶段框架估计传播利益；HeteroIM逐步选择最具影响力的被邀请者，并基于反向可达（RR）集进行重排名。

Result: 大量实验显示HeteroIR和HeteroIM显著超过现有最优基线方法（p值<0.05）。在腾讯游戏平台的在线A/B测试中，分别获得8.5%和10%的提升。

Conclusion: 该研究成功将推荐系统与传播最大化方法结合，解决了交互意愿与传播能力的综合挑战，并在实际应用中验证了方法的有效性。
...
### [311] [Understanding Distribution Structure on Calibrated Recommendation Systems](https://arxiv.org/abs/2508.13568)
...
TL;DR: 这篇论文研究了检定推荐系统，通过对比用户精细地分布、候选项目分布和推荐列表分布，解决传统推荐系统忽略用户洞屏偏好的问题。


<details>
  <summary>Details</summary>
Motivation: 传统推荐系统只推荐最相似的项目，导致推荐列表忽略用户精细地中不突出的类别，影响用户体验。需要一种能保证包含不代表性区域的推荐方法。

Method: 使用十五个模型来分析用户精细地分布、候选项目分布和推荐列表分布的结构。在三个电影领域数据集中评估用户模式，采用离群点检测模型来理解分布结构。

Result: 离群点检测模型能更好地理解分布结构。检定推荐系统创建的推荐列表与传统推荐列表行为相似，但允许用户以相同程度改变偏好组合。

Conclusion: 检定推荐系统通过考虑多维度精细地分布，有效解决了传统推荐系统的偏好洞屏问题，提供了更全面的用户体验。
...
### [312] [UniECS: Unified Multimodal E-Commerce Search Framework with Gated Cross-modal Fusion](https://arxiv.org/abs/2508.13843)
*Zihan Liang,Yufei Ma,ZhiPeng Qian,Huangyu Dai,Zihan Wang,Ben Chen,Chenyi Lei,Yuqing Ding,Han Li*

Main category: cs.IR

TL;DR: UniECS是一个统一的多模态电商搜索框架，通过门控多模态编码器和综合训练策略，在多个基准测试中显著优于现有方法，并在实际部署中提升了点击率和收入。
...
Method: 提出UniECS框架，包含：1）使用自适应融合机制的门控多模态编码器；2）结合交叉模态对齐损失、局部对齐损失、模态内对比损失和自适应损失加权的综合训练策略；3）创建M-BEER多模态基准数据集。

Result: 在四个电商基准测试中表现优异，在M-BEER基准上跨模态任务提升达28%（文本到图像检索R@10），参数量仅0.2B但优于更大模型。实际部署中点击率提升2.74%，收入提升8.33%。
...
### [313] [InPars+: Supercharging Synthetic Data Generation for Information Retrieval Systems](https://arxiv.org/abs/2508.13930)
*Matey Krastev,Miklos Hamar,Danilo Toapanta,Jesse Brouwers,Yibin Lei*

Main category: cs.IR

TL;DR: 本文重新考察并扩展了用于神经信息检索的合成查询生成流程，通过InPars工具箱使用大语言模型生成训练数据，并提出了两个关键扩展来提升查询质量和检索性能。


<details>
  <summary>Details</summary>
Motivation: 为了提高神经信息检索系统的表现，需要大量高质量的训练数据。但手动标注数据成本高且耗时，因此需要开发更有效的合成查询生成方法来生成训练数据。

Method: 1) 使用InPars工具箱重现InPars、InPars-V2和Promptagator流程在SciFact数据集上评估可重现性
2) 通过对比偏好优化(CPO)微调查询生成器LLM来提升生成查询的信号质量
3) 使用DSPy框架用动态的链式思维(CoT)优化提示替换静态提示模板

Result: 两个扩展方法都减少了对激进过滤的需求，同时提高了检索性能。所有代码、模型和合成数据集都已公开发布。

Conclusion: 通过CPO微调和动态CoT提示的方法，可以有效提升合成查询生成的质量，从而提高神经信息检索系统的表现，为进一步研究提供了可靠的基础。
...
### [314] [White-Box Reasoning: Synergizing LLM Strategy and gm/Id Data for Automated Analog Circuit Design](https://arxiv.org/abs/2508.13172)
*Jianqiu Chen,Siqi Li,Xu He*

Main category: cs.AR

TL;DR: 基于gm/Id方法和LLM协同推理的模拟电路设计框架，实现了比传统方法效率10倍提升的模拟电路自动化设计


<details>
  <summary>Details</summary>
Motivation: 模拟电路设计依赖经验且模拟效率低，传统公式在先进制程中失效，直接使用大语言模型容易造成随机猜测

Method: 提出"协同推理"框架，结合LLM的战略推理能力和gm/Id方法的物理精度，通过gm/Id查找表使LLM成为数据驱动的设计合作伙伴

Result: 在两级运放大器设计中，框架使Gemini模型在5次迭代内满足TT角点规格，并扩展到所有PVT角点优化，效率比高级工程师提升10倍

Conclusion: 这项工作验证了通过结合LLM推理和科学电路设计方法来实现真正模拟设计自动化的路径
...
### [315] [EvoVerilog: Large Langugage Model Assisted Evolution of Verilog Code](https://arxiv.org/abs/2508.13156)
*Ping Guo,Yiting Wang,Wanghao Ye,Yexiao He,Ziyao Wang,Xiaopeng Dai,Ang Li,Qingfu Zhang*

Main category: cs.AR

TL;DR: EvoVerilog是一个结合LLM推理能力和进化算法的新框架，用于自动生成和优化Verilog代码，无需人工干预，在基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖人工干预和精调数据集，限制了自动化设计工作流的可扩展性；迭代搜索技术缺乏设计多样性探索且性能不佳。

Method: 采用多目标、基于种群的搜索策略，结合LLM的推理能力和进化算法，自动探索广泛的Verilog设计可能性。

Result: 在VerilogEval-Machine和VerilogEval-Human基准测试中分别达到89.1和80.2的pass@10分数，能够同时生成多种功能Verilog代码并优化资源利用率。
...
### [316] [Image2Net: Datasets, Benchmark and Hybrid Framework to Convert Analog Circuit Diagrams into Netlists](https://arxiv.org/abs/2508.13157)
...
TL;DR: 本文提出了Image2Net混合框架，用于将电路图转换为网表，以帮助LLM更好地理解模拟集成电路。构建了包含丰富样式的新数据集，并在基准测试中取得了80.77%的成功率，比现有方法提升34.62%-45.19%。


<details>
  <summary>Details</summary>
Motivation: 现有模拟集成电路主要通过图像电路图表示而非文本网表，限制了LLM在模拟IC设计中的应用。现有转换框架对图像样式和电路元素支持有限，复杂电路图转换仍具挑战性。

Method: 构建包含丰富样式电路图的新数据集，提出Image2Net混合框架进行电路图到网表的转换，并引入网表编辑距离(NED)来精确评估转换质量。

Result: Image2Net达到80.77%的成功率，比之前工作提升34.62%-45.19%。平均NED为0.116，比最先进方法降低62.1%-69.6%。

Conclusion: Image2Net框架有效解决了电路图到网表的转换问题，为LLM在模拟IC设计中的应用提供了重要支持，显著优于现有方法。
...
### [317] [Piano: A Multi-Constraint Pin Assignment-Aware Floorplanner](https://arxiv.org/abs/2508.13161)
*Zhexuan Xu,Kexin Zhou,Jie Wang,Zijie Geng,Siyuan Xu,Shixiong Kai,Mingxuan Yuan,Feng Wu*

Main category: cs.AR

TL;DR: Piano是一个同时优化模块布局和引脚分配的VLSI布图规划框架，通过基于图的引脚分配方法和局部优化器，在多种约束下显著提升了布局质量。


<details>
  <summary>Details</summary>
Motivation: 传统布图规划器在现代约束条件下往往忽略引脚分配，这会影响后续详细布局和布线的性能。需要开发一个能同时优化模块布局和引脚分配的综合框架。

Method: 构建基于模块几何关系和网表连接的图，迭代搜索最短路径来确定引脚分配；采用空白空间移除策略和三个局部优化器来提升多约束场景下的布局指标。

Result: 在广泛使用的基准电路上，Piano实现了平均6.81%的HPWL减少、13.39%的馈通线长减少、16.36%的馈通模块数量减少和21.21%的未放置引脚减少，同时保持零空白空间。

Conclusion: Piano框架有效解决了现代VLSI布图规划中的多约束问题，通过同时优化模块布局和引脚分配，显著提升了整体布局质量。
...
### [318] [Sustainable AI Training via Hardware-Software Co-Design on NVIDIA, AMD, and Emerging GPU Architectures](https://arxiv.org/abs/2508.13163)
*Yashasvi Makin,Rahul Maliakkal*

Main category: cs.AR

TL;DR: 本文探讨针对NVIDIA、AMD等先进GPU架构的环境驱动性能优化方法，通过硬件软件协同设计提升内存级和内核级操作效率，实现显著的能效提升。


<details>
  <summary>Details</summary>
Motivation: 大规模深度学习模型训练消耗大量计算资源和能源，带来严重的可持续性问题。模型复杂度的快速增长导致能耗呈指数级增长，亟需提高计算效率并降低环境影响的技术。

Method: 研究硬件软件协同设计技术，包括评估专用张量和矩阵核心、高级内存优化方法、创新集成方法，以及软件层面的混合精度运算、能量感知调度算法和编译器驱动的内核增强。

Result: 通过协同设计方法获得了训练效率的显著提升，在不牺牲性能的前提下降低了人工智能的环境影响。实际案例研究证明了这些可持续AI训练方法的有效性。

Conclusion: 硬件软件协同设计是实现可持续人工智能系统的关键，能够大幅提高能效并减少环境足迹，为构建真正可持续的AI系统指明了重要研究方向。
...
### [319] [Accelerating LLM Inference via Dynamic KV Cache Placement in Heterogeneous Memory System](https://arxiv.org/abs/2508.13231)
*Yunhua Fang,Rui Xie,Asad Ul Haq,Linsen Ma,Kaoutar El Maghraoui,Naigang Wang,Meng Wang,Liu Liu,Tong Zhang*

Main category: cs.AR

TL;DR: 这篇论文研究了在异构内存系统中动态安排LLM推理的KV缓存位置，以最大化带宽利用率和解决内存带宽缺口问题。


<details>
  <summary>Details</summary>
Motivation: LLM推理越来越受到内存带宽的限制，KV缓存的频繁访问占据了大部分数据传输。虽然注意力稀疏性减少了部分内存流量，但需要保持完整KV缓存的可访性，续续给带宽和容量带来压力。现代AI硬件集成了HBM和高速离包DRAM，使异构内存系统成为实用解决方案。

Method: 论文将KV缓存放置问题进行数学形式化，并推导出理论上限。而不是提出具体的调度策略，主要关注于理论分析和优化潜力的探索。

Result: 研究揭示了在容量约束下最大化聚合带宽利用率的量上限，显示出运行时优化的巨大潜力空间。

Conclusion: 这是首次对异构内存系统中动态KV缓存调度问题的正式理论处理，为解决LLM推理的内存带宽瓶颈提供了理论基础和优化方向。
...
### [320] [ViTAD: Timing Violation-Aware Debugging of RTL Code using Large Language Models](https://arxiv.org/abs/2508.13257)
*Wenhao Lv,Yingjie Xia,Xiyuan Chen,Li Kuang*

Main category: cs.AR

TL;DR: ViTAD是一个自动化RTL时序违规修复方法，通过构建信号时序依赖图和使用LLM分析违规根因，从知识库生成定制修复方案，在真实数据集上达到73.68%的成功率。


<details>
  <summary>Details</summary>
Motivation: 现代VLSI设计对时序要求越来越高，传统时序优化依赖人工经验，需要自动化方法来提高效率和准确性。

Method: 首先解析Verilog代码和时序报告构建STDG图，然后进行违规路径分析并使用LLM推断违规根因，最后从领域知识库检索相关知识生成定制修复方案。

Result: 在包含54个违规案例的真实数据集上，ViTAD达到73.68%的成功率，比仅使用LLM的基线方法(54.38%)提高了19.30%。

Conclusion: ViTAD方法通过结合图分析和LLM技术，有效实现了RTL时序违规的自动化修复，显著提高了修复成功率。
...
### [321] [FedChip: Federated LLM for Artificial Intelligence Accelerator Chip Design](https://arxiv.org/abs/2508.13162)
*Mahmoud Nazzal,Khoa Nguyen,Deepak Vungarala,Ramtin Zand,Shaahin Angizi,Hai Phan,Abdallah Khreishah*

Main category: cs.AR

TL;DR: FedChip是一个联邦微调框架，允许多个芯片设计方在保护专有数据隐私的前提下，协作训练用于自动化硬件设计的共享大语言模型，设计质量提升77%以上


<details>
  <summary>Details</summary>
Motivation: AI硬件设计自动化需求增长，但现有LLM面临数据隐私问题和领域专业知识不足的挑战，需要保护专有数据的同时提升模型性能

Method: 提出FedChip联邦学习框架，各参与方在本地专有数据上训练，协作增强共享LLM；创建APTPU-Gen数据集（3万设计变体）；提出Chip@k评估指标来平衡多质量指标

Result: 实验结果显示FedChip相比高端LLM设计质量提升超过77%，同时保持数据隐私保护

Conclusion: FedChip成功解决了芯片设计领域LLM应用中的数据隐私和性能提升的双重挑战，为协作式AI硬件设计自动化提供了可行方案
...
### [322] [Too Easily Fooled? Prompt Injection Breaks LLMs on Frustratingly Simple Multiple-Choice Questions](https://arxiv.org/abs/2508.13214)
...
<details>
  <summary>Details</summary>
Motivation: 评估大语言模型在提示注入攻击下的稳健性，特别是在LLM作为判断者的应用场景中

Method: 通过在PDF文件中注入隐藏提示，测试LLM在基本算术问题（多选题和判断题）上的表现

Result: 结果显示LLM在这种平凡场景下也容易受到隐藏提示注入攻击的影响

Conclusion: 这种脏简单却有效的攻击方式显示了LLM作为判断者应用的严重稳健性风险
...
### [323] [MCPSecBench: A Systematic Security Benchmark and Playground for Testing Model Context Protocols](https://arxiv.org/abs/2508.13220)
*Yixuan Yang,Daoyuan Wu,Yufan Chen*

Main category: cs.CR

TL;DR: 首个系统性的MCP安全分类法，提出MCPSecBench安全测试平台，识别17种攻击类型，过85%攻击成功破坏主要平台


<details>
  <summary>Details</summary>
Motivation: MCP协议在扩大LLM应用能力的同时也引入了新的安全风险和攻击面，需要系统性的安全分析和评估方法

Method: 提出包含17种攻击类型的系统分类法，建立MCPSecBench安全测试平台，整合了提示数据集、MCP服务器、MCP客户端和攻击脚本，在三大MCP提供商上进行实验评估

Result: 识别的17种攻击中，超过85%成功破坏至少一个平台，核心漏洞普遍影响Claude、OpenAI和Cursor，提示基于和工具中心攻击在不同主机和模型上呈现显著差异

Conclusion: MCPSecBench标准化了MCP安全评估，能够在所有MCP层面进行严格测试，为MCP安全研究提供了综合性的测试平台
...
### [324] [Quantifying Loss Aversion in Cyber Adversaries via LLM Analysis](https://arxiv.org/abs/2508.13240)
*Soham Hans,Nikolos Gurney,Stacy Marsella,Sofia Hirschmann*

Main category: cs.CR

TL;DR: 利用大语言模型从黑客行为中量化提取损失压迫偏误，为网络安全防御提供新的行为基础分析方法


<details>
  <summary>Details</summary>
Motivation: 传统网络安全防御策略无法动态解释攻击行为，需要新方法来推断和利用攻击者的认知偏误

Method: 采用LLM处理黑客攻击网络实验中产生的笔记，分析行动段落并关联到预定义的持久性机制

Result: LLM能够有效解析和识别黑客决策中的损失压迫偏误表现，提供了新的行为洞察

Conclusion: 这种方法为实时行为基础分析提供了变革性的网络安全防御提升途径
...
### [325] [Involuntary Jailbreak](https://arxiv.org/abs/2508.13246)
*Yangyang Guo,Yangyan Li,Mohan Kankanhalli*

Main category: cs.CR

TL;DR: 研究发现LLMs存在"非自愿越狱"漏洞，仅需一个通用提示词就能让主流LLMs生成通常会被拒绝的危险内容，暴露了安全护栏的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 现有越狱攻击主要针对LLM安全护栏的局部组件，而本研究旨在揭示整个护栏结构可能存在的系统性脆弱性。

Method: 使用单一通用提示词，要求LLMs生成通常会被拒绝的问题及其深度回答（而非拒绝回答）。

Result: 该方法在包括Claude Opus 4.1、Grok 4、Gemini 2.5 Pro和GPT 4.1在内的大多数主流LLMs上持续有效。

Conclusion: LLM安全护栏存在严重脆弱性，需要重新评估其鲁棒性并加强未来的安全对齐机制。
...
### [326] [A Risk Manager for Intrusion Tolerant Systems: Enhancing HAL 9000 with New Scoring and Data Sources](https://arxiv.org/abs/2508.13364)
*Tadeu Freitas,Carlos Novo,Inês Dutra,João Soares,Manuel Correia,Benham Shariati,Rolando Martins*

Main category: cs.CR

TL;DR: 本文扩展了HAL 9000入侵容忍系统，通过构建自定义爬虫从多个威胁情报源收集信息，显著提升了系统对新兴威胁的早期检测和评估能力。


<details>
  <summary>Details</summary>
Motivation: 现有入侵容忍系统过度依赖NVD和ExploitDB等有限数据库，这些数据库需要人工分析新漏洞，限制了系统对快速演变的威胁的响应能力。HAL 9000虽然通过机器学习改进了漏洞预测，但仍受限于单一信息来源。

Method: 开发自定义爬虫持续挖掘多样化威胁源，包括安全公告、研究论坛和实时漏洞利用概念验证。将爬虫获取的情报集成到HAL 9000风险管理框架中，扩展其情报基础。

Result: 评估表明，集成爬虫获取的情报显著提升了HAL 9000处理新兴威胁的能力，能够更早地检测和评估未经验证的漏洞。

Conclusion: 通过整合多源威胁情报，HAL 9000的风险管理框架得到了实质性改进，增强了系统对快速演变威胁的主动防御能力。
...
### [327] [Know Me by My Pulse: Toward Practical Continuous Authentication on Wearable Devices via Wrist-Worn PPG](https://arxiv.org/abs/2508.13690)
*Wei Shao,Zequan Liang,Ruoyu Zhang,Ruijie Fang,Ning Miao,Ehsan Kourkchi,Setareh Rafatirad,Houman Homayoun,Chongzhou Fang*

Main category: cs.CR

TL;DR: 本文提出了一种基于低频率(25Hz)多通道PPG信号的智能手表连续身份验证系统We-Be Band，通过Bi-LSTM注意力机制在保持高准确度的同时大幅降低了传感器能耗。


<details>
  <summary>Details</summary>
Motivation: 虽然PPG信号在可穿戴设备中具有连续、非侵入式身份验证的潜力，但现有方法多依赖高频率PPG(75-500Hz)和复杂深度模型，导致能耗和计算开销过高，限制了在功耗约束实际系统中的部署。

Method: 设计了基于低频率(25Hz)多通道PPG信号的连续身份验证系统，采用具有注意力机制的Bi-LSTM模型从短时间窗口(4秒)中提取特征。在公开数据集(PTTPPG)和自建数据集(26名参与者)上进行了评估。

Result: 系统实现了彳好的分类性能：平均测试准确率88.11%，macro F1分0.88，误接受率0.48%，误拒绝率11.77%，相等错误率2.76%。与512Hz相比，25Hz系统减少80%传感器能耗，与128Hz相比也减少19%能耗。识别出25Hz为保持身份验证准确性的实际下限。

Conclusion: 研究表明，使用低频率(25Hz)PPG信号可以在智能手表上实现高效的连续身份验证，同时显著降低能耗。此外，在训练数据中包含多种活动状态能够提升模型在不同生理状态下的稳健性。
...
### [328] [On the Security and Privacy of Federated Learning: A Survey with Attacks, Defenses, Frameworks, Applications, and Future Directions](https://arxiv.org/abs/2508.13730)
...
TL;DR: 这是一份关于联邦学习安全性和隐私性的综述性论文，统计了200多篇论文，分析了现有的攻击与防御机制，并评估了各种技术的优缺点。


<details>
  <summary>Details</summary>
Motivation: 联邦学习虽然在设计上增强了数据隐私，但仍然存在各种安全和隐私风险，需要系统性地研究和解决这些挑战。

Method: 采用综述性研究方法，对现有的攻击和防御技术进行分类和分析，主要分为安全增强技术（如对投毒攻击、希拉攻击的防御）和隐私保护技术（如加密方法、差分隐私、安全聚合）。

Result: 识别了现有技术的优点和局限性，分析了隐私、安全与模型性能之间的权衡关系，以及非IID数据分布对防御效果的影响。

Conclusion: 本综述为研究人员和实践者提供了指导，并指出了未来研究方向，包括需要发展可扩展、适应性强且能源效率高的解决方案来满足动态异构联邦学习环境的需求。
