...
<details>
  <summary>Details</summary>
Motivation: 构建一个适用于全球各种口音的自动语音识别（ASR）系统，需研究训练数据中说话者数量、音频时长和口音多样性对未见口音鲁棒性的影响。

Method: 系统研究训练数据中说话者数量、每个说话者的音频时长及口音多样性三个变量对ASR鲁棒性的影响。

Result: 固定训练时长下，增加说话者数量比增加单个说话者的音频时长更有效；更多说话者能带来性能提升；口音多样性对性能影响较小。
...
<details>
  <summary>Details</summary>
Motivation: 句子嵌入是现代NLP和AI系统的核心，但其内部结构不透明，难以解释。

Method: 使用字典学习对词级表示进行分解，分析池化操作和潜在特征。

Result: 发现嵌入中许多语义和句法特征是线性编码的。

Conclusion: 该方法提升了句子嵌入的可解释性和可控性。
...
Method: 使用对比学习联合学习文本表示和路径引导的层次表示，训练中学习两类概率分布，推理时合并输出。

Result: 在两个公开数据集上，Macro F1分数比现有最佳模型提升0.99-2.37%。
...
TL;DR: 评估55个公开大语言模型在低资源语言马耳他语上的表现，发现模型表现普遍不佳，尤其是生成任务。小规模微调模型表现更好，预训练和指令调优中对马耳他语的接触是关键因素。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在低资源语言上表现有限，研究旨在填补这一空白并推动更包容的语言技术。

Method: 使用新基准测试55个模型在11个判别和生成任务上的表现，进行多维分析。

Result: 多数模型表现差，生成任务尤甚；微调模型表现更优，预训练接触马耳他语是关键。

Conclusion: 建议低资源语言研究采用传统建模方法，微调虽成本高但性能更优。
...
Method: 构建包含72,413个任务实例的MedAgentGYM，提供可执行编码环境、任务描述、反馈机制和真实标注。对30多个LLM进行基准测试，并通过监督微调和强化学习优化Med-Copilot-7B。

Result: 商业API模型表现优于开源模型，Med-Copilot-7B通过微调（+36.44%）和强化学习（+42.47%）显著提升性能，接近gpt-4o水平。

Conclusion: MedAgentGYM为开发医学编码助手提供了全面且可扩展的平台，推动了生物医学研究和实践中的LLM应用。
...
Method: 测试语言模型对英语LET-ALONE结构的语法形式和语义的理解。

Result: 模型能泛化语法形式，但对语义理解不足。

Conclusion: 当前架构在语言形式和意义的泛化上存在不对称性。
...
<details>
  <summary>Details</summary>
Motivation: 研究目标是开发一种无需额外训练、高效且易于实现的系统，用于SemEval-2025任务11的子任务A（多标签情绪检测）。

Method: 使用模型集成方法预测文本中的情绪标签（如喜悦、悲伤、恐惧等），无需额外训练。

Result: EmoRAG的性能与最佳系统相当，同时更高效、可扩展且易于实现。

Conclusion: EmoRAG通过模型集成实现了高效的情绪检测，为多标签情绪分类提供了一种简单且有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖预定义实体属性模式或标注数据，导致提取结果不完整。

Method: 采用丰富、细化和统一的机制，利用实体与其结构的相互强化关系。

Result: 在三个不同领域中，ZOES显著提升了LLM提取完整实体结构的能力。

Conclusion: ZOES的机制可作为一种原则性方法，提升LLM在不同场景下的实体结构发现质量。
...
<details>
  <summary>Details</summary>
Motivation: 研究水印技术对LLMs核心对齐属性的影响，填补现有研究的空白。

Method: 通过实验分析两种水印技术的影响，并提出AR方法，利用外部奖励模型在推理时恢复对齐性。

Result: 实验表明，AR方法仅需采样2-4个水印生成即可恢复或超越未加水印的对齐分数。

Conclusion: 揭示了水印强度与模型对齐之间的平衡，为实际部署水印LLMs提供了简单有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有偏好学习方法依赖人工或高级LLM标注数据，成本高且难以扩展。PUGC旨在利用UGC中的隐式偏好，解决这一问题。

Method: PUGC将UGC转化为用户查询，生成响应并利用UGC作为参考文本进行评分，从而对齐模型与隐式偏好。

Result: 实验显示，使用PUGC和DPO训练的模型性能提升9.37%，在Mistral-7B-Instruct上达到35.93%的领先水平。

Conclusion: PUGC提供了一种可扩展、领域特定的对齐方法，显著提升了模型性能，并在奖励质量、鲁棒性等方面表现优异。
...
<details>
  <summary>Details</summary>
Motivation: LLM生成的SQL查询可能存在语义错误，但缺乏对其可靠性的深入理解，因此需要一种方法来检测和纠正这些错误。

Method: SQLens结合数据库和LLM的错误信号，识别SQL子句中的潜在语义错误，并利用这些信号指导查询修正。

Result: 在两个公开基准测试中，SQLens在错误检测的F1分数上比最佳LLM自评估方法高25.78%，并将文本到SQL系统的执行准确率提升高达20%。

Conclusion: SQLens通过细粒度的语义错误检测和修正，显著提升了文本到SQL系统的可靠性和性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在模糊场景中不可靠和小型语言模型易受误导的问题。

Method: 提出SLIDE方法集成SLM和LLM，并通过DRE方法分两步优化评估：SLM生成指导LLM初步评估，SLM调整优化LLM评分。

Result: 实验表明DRE方法优于现有方法，与人类判断更一致。

Conclusion: 结合小型和大型模型能提升开放式任务（如对话评估）的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 探讨Chain-of-Thought（CoT）推理在翻译任务中的实际有效性，验证其是否真正提升性能。

Method: 通过实验比较显式分解翻译（如多步提示）与简单提示（如“重新翻译”）的效果。

Result: 显式分解翻译未显著提升性能，而简单提示“重新翻译”效果更优。

Conclusion: CoT在翻译中的有效性需进一步研究，当前结果表明简单提示可能更高效。
...
<details>
  <summary>Details</summary>
Motivation: 探讨LLMs是否能理解多义性话语助词（如“just”）的细微语义差异。

Method: 使用专家标注的数据集，评估LLMs对“just”不同语义类别的区分能力。

Result: LLMs能区分较宽泛的语义类别，但对更微妙的差异理解不足。

Conclusion: LLMs在理解话语助词的细微语义方面仍有局限。
...
<details>
  <summary>Details</summary>
Motivation: 研究LLMs在面对无合理答案问题时的表现，揭示其局限性。

Method: 提出了一种基准测试方法，并改进了现有数据集。

Result: 现有模型在无合理答案问题上表现远未达到完美。

Conclusion: 该研究为评估LLMs的局限性提供了新视角和工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有机器翻译质量评估指标在非洲低资源语言中表现不佳，缺乏公开的训练数据和评估集。

Method: 构建了包含13种非洲语言的SSA-MTE数据集，并基于此开发了SSA-COMET和SSA-COMET-QE评估指标，同时测试了基于LLM的方法。
...
<details>
  <summary>Details</summary>
Motivation: 研究多代理系统中恶意代理如何利用提示操纵实施隐蔽攻击，以自利为目的而不破坏系统功能。

Method: 分析了四种攻击类型（Scapegoater、Boaster、Self-Dealer、Free-Rider），并通过实验验证提示操纵对系统行为的系统性偏见影响。

Result: 实验表明，精心设计的提示可以绕过GPT-4o-mini等先进监控系统，成功误导评估和协作代理。

Conclusion: 强调多代理系统需加强安全协议和内容验证机制，并开发更全面的风险场景评估监控系统。
...
Method: 使用Financial PhraseBank数据集，评估不同LLMs和提示策略（模拟系统1和系统2思维）的表现，并与微调的小模型对比。

Result: 推理未提升性能，GPT-4o在无推理提示时表现最佳。语言复杂性和标注一致性影响性能，推理可能导致过度思考。

Conclusion: 在金融情感分类中，快速直觉的“系统1”思维比慢速推理的“系统2”更接近人类判断，挑战了“更多推理总是更好”的假设。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLMs作为逻辑翻译器在词汇多样性场景下表现不佳，且现有基准缺乏词汇多样性，掩盖了这一问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多样本ICL依赖随机选择演示，缺乏有效方法，作者假设ICL与微调的数据需求类似，因此提出梯度匹配方法。

Method: 通过梯度匹配选择演示，使所选示例的微调梯度与整个训练集对齐，从而逼近完整训练集的学习效果。

Result: 在多个数据集和模型上，该方法比随机选择表现更好，例如在Qwen2.5-72B和Llama3-70B上提升4%。

Conclusion: 该方法为多样本ICL提供了更可靠和有效的解决方案，推动了其更广泛的应用。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于检索增强语言模型的事实核查系统难以处理人为设计的对抗性声明，因此需要一种更有效的方法。

Method: SUCEA框架分三步：1) 声明分割与去上下文化；2) 迭代证据检索与声明编辑；3) 证据聚合与标签预测。

Result: 在两个挑战性数据集上，SUCEA显著优于四种基于声明分解的基线方法。

Conclusion: SUCEA通过模块化设计有效解决了对抗性声明的核查难题，提升了系统的整体性能。
...
<details>
  <summary>Details</summary>
Motivation: 科学声明验证需要处理多模态数据，但缺乏直接测试声明验证能力的基准。

Method: 通过自动提取科学文章中的支持声明并手动扰动生成矛盾声明，设计诊断任务以分析模型能力。

Result: 现有模型表现较差（F1分数0.3-0.5），最佳模型仅0.77，且存在偏向支持声明的偏差。

Conclusion: 模型在多模态数据定位、信息聚合和基本理解方面存在不足，需进一步改进。
...
<details>
  <summary>Details</summary>
Motivation: 解决无监督数据中伪标签质量不高的问题，利用LLM提升语音处理任务的准确性。

Method: 通过LLM修正ASR/AST生成的伪标签，结合数据过滤策略优化知识转移效率。

Result: 在普通话ASR和西班牙语-英语AST任务中，WER降低3.77%，BLEU分数显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 当前CoT提示方法（如PRMs或自一致性）缺乏透明性，无法提供可验证的判断依据，限制了其有效性。

Method: 通过在每个推理步骤中使用Lean 4形式化数学语言表达数学主张并提供形式化证明，以识别幻觉。

Result: 在多个语言模型和数学数据集上验证了$Safe$框架，性能显著提升，同时提供可解释和可验证的证据。

Conclusion: $Safe$框架首次将Lean 4用于验证LLMs生成的自然语言内容，为幻觉问题提供了可靠解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有科学NLI数据集仅覆盖计算机科学领域，忽略了非计算机科学领域，因此需要一个新的基准来填补这一空白。

Method: 构建了MISMATCHED数据集，覆盖三个非计算机科学领域，包含2700个人工标注的句子对。使用预训练SLMs和LLMs建立基线模型，并探索加入隐含科学NLI关系的句子对的效果。

Result: 最佳基线模型的宏F1为78.17%，表明未来改进空间大。加入隐含关系的句子对能提升模型性能。

Conclusion: MISMATCHED为科学NLI提供了新基准，展示了非计算机科学领域的挑战，并表明隐含关系对模型性能有积极影响。
...
Method: 提出ADAPT，轻量级前缀调优方法，结合多样性数据策略。

Result: 数学推理任务中，ADAPT以八分之一计算量达到80%准确率。

Conclusion: 生成多样性对TTS效果至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究强调社会结构对创新能力的影响，但机器学习方法可以更细致地建模创新者的个人视角和经验轨迹。

Method: 基于动态语言表征的几何空间，量化创新者的主观视角和机会，分析数百万科学家、发明家等的数据，并进行自然实验和AI模拟。

Result: 视角多样性（而非背景多样性）能预测未来的创新成就，且成功合作者通过共同语言整合多样经验。
...
<details>
  <summary>Details</summary>
Motivation: 现有静态词嵌入模型在句子语义任务上表现不足，需要一种更高效且低成本的方法。

Method: 从预训练句子Transformer提取词嵌入，通过句子级主成分分析改进，再结合知识蒸馏或对比学习。推理时简单平均词嵌入。

Result: 在单语和跨语言任务上显著优于现有静态模型，部分数据集甚至媲美基础句子Transformer模型（SimCSE）。
...
Method: 提出Tool-MVR，包含多代理元验证（MAMV）构建高质量指令集ToolBench-V，以及探索式反思学习（EXPLORE）生成反思数据集ToolBench-R。
...
Result: 自动收集的数据集支持强基线模型，在干净环境下表现与ASR相当，在嘈杂环境中显著优于ASR。
...
<details>
  <summary>Details</summary>
Motivation: KV缓存在Transformer模型中内存需求随序列长度增加而急剧增长，传统量化方法需单独处理稀疏和非连续异常值，限制了可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注静态、单轮场景，无法满足现实规划问题的动态需求。

Method: 基于TravelPlanner数据集，引入多轮约束引入和优先级竞争约束两种新评估设置。

Result: 模型在多轮任务中的表现与单轮任务不相关，约束引入顺序和优先级处理显著影响性能。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI系统（如LLMs）的进步，其安全部署问题日益突出，当前的对齐方法无法有效防止对抗性攻击。

Method: 通过分析人类偏好微调的对齐策略，结合道德心理学研究，揭示LLMs缺乏规范性审议能力。

Result: LLMs易受对抗性攻击，现有方法无法解决其规范性冲突的脆弱性。

Conclusion: 当前的对齐方法不足以应对日益强大的AI系统带来的潜在危害，需改进对齐策略。
...
<details>
  <summary>Details</summary>
Motivation: 随着推理过程中增强推理能力的需求增加，需要评估MLLMs在检测和纠正错误方面的能力，而不仅仅是最终准确性。

Method: MMRefine通过六种不同场景和错误类型评估MLLMs的细化能力，并实验分析了开放和封闭MLLMs的性能瓶颈。

Result: 实验揭示了MLLMs在细化性能上的瓶颈和改进空间，为有效的推理增强提供了方向。

Conclusion: MMRefine为MLLMs的错误细化能力提供了评估框架，并公开了代码和数据集，促进了相关研究的发展。
...
Method: 提出REWIRE方法，通过改写被过滤的低质量文档，使其可用于训练，增加合成数据在预训练集中的比例。

Result: 在1B、3B和7B规模的实验中，混合高质量原始文本与改写文本使22项任务性能提升1.0、1.3和2.5个百分点。
...
Result: STAND在多个任务中减少推理延迟60-65%，吞吐量优于现有方法14-28%，单轨迹场景下延迟降低48-58%。

Conclusion: STAND是一种即插即用的通用解决方案，无需额外训练即可应用于任何语言模型，显著提升推理效率。
...
<details>
  <summary>Details</summary>
Motivation: 探索在低资源语言对（Bhojpuri-Hindi）中，超参数优化和数据增强技术对语音翻译模型性能的提升作用。

Method: 通过系统研究学习率调度、更新步数、预热步数、标签平滑和批量大小等超参数，并应用速度扰动和SpecAugment数据增强技术，同时尝试跨语言信号（Marathi和Bhojpuri）的联合训练。

Result: 实验表明，超参数选择和简单有效的数据增强技术显著提升了低资源环境下的翻译性能，并通过BLEU分析了翻译错误。

Conclusion: 在低资源语音翻译任务中，超参数优化和数据增强技术是提升模型性能的关键因素。
...
Result: 在12个任务和数据集中的10个上优于初始模型和4个基线，平均提升7.0%。

Conclusion: SPARTA ALIGNMENT能有效提升模型性能，并更好地泛化到未见任务。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注STEM领域的知识和推理能力，忽视了HSS领域的独特需求，如跨学科思维和抽象概念与视觉表征的结合。

Method: 提出了HSSBench，包含13,000多个样本，覆盖六类任务，并通过专家与自动化代理协作的数据生成流程创建。

Result: 测试了20多个主流MLLMs，发现即使在最先进模型上也存在显著挑战。

Conclusion: HSSBench旨在推动MLLMs在跨学科推理能力上的研究，特别是知识整合与跨领域连接的能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统小型语言模型在假新闻检测中需要大量监督训练和难以适应动态新闻环境的问题，以及大型语言模型因知识过时和缺乏合适示例而表现不佳的挑战。

Method: 提出C²EFND框架，结合大型语言模型的泛化能力和小型语言模型的分类专长，采用多轮协作学习、终身知识编辑模块和基于回放的持续学习方法。
...
Result: LLM能有效评估指令遵循性，但在正确性上表现不佳；领域特定指标提供补充信息。混合方法最可靠。

Conclusion: 结合LLM评估和任务特定指标的混合方法是评估修订质量的最可靠方法。
...
Result: 向量能解构政治概念混淆，检测任务验证语义，干预实验可生成不同政治倾向的回答。

Conclusion: 多维度框架和表示工程技术能有效揭示和干预LLMs的政治观点。
...
<details>
  <summary>Details</summary>
Motivation: 口语理解需要整合语义、副语言和语音特征，而现有SpeechLLMs在细粒度感知和复杂推理方面表现不足。

Method: 引入MMSU基准，系统整合多种语言现象（如语音学、韵律、修辞等），并通过音频-问题-答案三元组进行评估。

Result: 评估发现现有模型在口语理解和推理方面有显著改进空间。

Conclusion: MMSU为口语理解提供了新的评估标准，为未来优化指明了方向。
...
<details>
  <summary>Details</summary>
Motivation: 填补现有文献中对多模态输入如何与语言主干连接的空白，为研究者提供结构化概述。

Method: 通过分析125个MLLMs，提出基于架构策略、表示学习技术和训练范式的分类框架。

Result: 总结了当前多模态集成技术的趋势，为未来模型开发提供指导。

Conclusion: 该分类框架为研究者提供了清晰的指导，有助于开发更鲁棒的多模态集成策略。
...
Method: 提出FineLogic框架，通过四种监督风格（一种自然语言和三种符号变体）训练模型，并评估其推理能力。

Result: 自然语言监督在泛化任务中表现优异，符号监督则生成更结构化的推理链；微调主要通过逐步生成改善推理行为。
...
Method: 结合CNN提取局部n-gram模式，BERT生成上下文丰富的表示，并通过注意力机制检测错误。修正模块使用对齐和GRU解码器提出合理修改。

Result: 实验结果显示90%的准确率、89.37%的F1分数和16.24%的MSE，优于现有技术10%以上。
...
Method: 使用646份印尼四年级学生的手写考试答案（涵盖数学和英语两科，共14K+答案），评估VLM和多个LLM在评分和生成个性化反馈上的表现。

Result: VLM在手写识别上表现不佳，影响LLM的评分准确性，但LLM生成的反馈仍具一定价值。

Conclusion: 尽管存在手写识别和个性化反馈的局限性，VLM和LLM在教育评估中仍有潜在应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 加密填字游戏线索是复杂的语言任务，现有测试集不断更新，需要高效且可解释的解决方案。

Method: 系统结合LLM，通过假设答案、提出文字游戏解释和验证步骤（使用Python代码表示）来解决问题。

Result: 在Cryptonite数据集上实现了最新性能，且解决方案可解释。
...
Method: RACE框架提取关键推理步骤，计算四种诊断信号：推理轨迹的样本间一致性、基于熵的答案不确定性、推理与答案的语义对齐、推理的内部一致性。

Result: 实验表明，RACE在不同数据集和LLMs上优于现有基线，提供细粒度的幻觉检测。

Conclusion: RACE为LRMs的幻觉检测提供了鲁棒且通用的解决方案。
...
TL;DR: 论文介绍了MockConf数据集和InterAlign工具，用于自动对齐和分析同声传译任务，填补了现有语料库和工具的不足。


<details>
  <summary>Details</summary>
Motivation: 现有平行语料库和算法无法有效建模同声传译中长距离交互和特定类型的差异（如简化、功能泛化），因此需要专用数据集和工具。

Method: 收集了MockConf数据集（7小时5种欧洲语言的录音），并开发了InterAlign工具，支持并行词和跨度的长输入标注。

Result: 发布了数据集和工具，并提出了评估指标和自动对齐基线。

Conclusion: MockConf和InterAlign为同声传译研究提供了实用资源，推动了自动对齐和分析的发展。
...
<details>
  <summary>Details</summary>
Motivation: 手动生成MCQs耗时耗力，LLMs为此提供了高效解决方案。

Method: 通过向LLMs注入知识而非依赖其固有知识，对比三种模型生成MCQs的效果。

Result: GPT-3.5在多项指标中表现最优，但教育者对AI的接受度仍有保留。

Conclusion: LLMs在生成MCQs方面具有潜力，但需进一步推动教育领域的AI应用。
...
Result: 实验结果显示，UD的整合显著提高了准确率（平均提升3.85%）和F1分数（平均提升6.08%），甚至在某些语言对上超越大型语言模型。
...
TL;DR: 提出了ICPC-Eval，一个用于评估大型语言模型在真实竞赛环境中编码能力的顶级竞争性编程基准。


<details>
  <summary>Details</summary>
Motivation: 现有基准（如LiveCodeBench和CodeElo）无法充分评估大型语言模型在真实竞赛环境中的编码能力，且现有评估指标（如Pass@K）未能捕捉模型的反思能力。

Method: ICPC-Eval包含来自11个近期ICPC竞赛的118个问题，提供真实的竞赛场景、高效的本地评估工具和新的评估指标Refine@K。

Result: 结果显示，顶级推理模型（如DeepSeek-R1）需依赖多轮代码反馈才能充分发挥其推理潜力，且仍落后于顶尖人类团队。
...
TL;DR: Verbose ListOps 是一个新基准，通过将 ListOps 计算转化为长故事，测试 LLMs 在嵌套叙事推理中的表现，揭示了其在状态管理上的局限性。
...
Method: 通过程序化生成长故事形式的 ListOps 计算，迫使 LLMs 进行内部计算和状态管理，并提供对叙事长度和推理难度的精细控制。

Result: 主流 LLMs（如 OpenAI o4、Gemini 2.5 Pro）在 Verbose ListOps 上表现不佳，尤其在中等长度叙事（约 10k token）时崩溃。

Conclusion: Verbose ListOps 揭示了 LLMs 在嵌套推理中的关键弱点，为改进推理能力提供了针对性方向，而不仅仅是扩展上下文窗口。
...
TL;DR: 本文挑战了现有观点，提出结合混合HMM与自监督模型的方法，在低资源语言ASR系统中表现优于微调多语言模型，苏格兰盖尔语测试显示WER降低32%。


<details>
  <summary>Details</summary>
Motivation: 现有方法认为微调多语言端到端模型是低资源语言ASR的最佳方案，但本文质疑其效果，提出更优方案。

Method: 结合混合HMM与自监督模型，通过持续自监督预训练和半监督训练充分利用数据。

Result: 在苏格兰盖尔语测试中，WER相对最佳微调模型降低32%。

Conclusion: 混合HMM与自监督模型的方法在低资源语言ASR中表现更优，挑战了现有微调方法的优越性。
...
TL;DR: 研究探讨了大型语言模型（LLM）在多语言教育中的反馈效果，发现多语言提示能显著提升学习效果，尤其在低资源语言中。


<details>
  <summary>Details</summary>
Motivation: 评估LLM在不同语言中提供数学推理任务反馈的能力，填补多语言教育工具的研究空白。

Method: 通过模拟多语言师生互动，使用不同LLM和提示策略，分析语言对齐对学习效果的影响。

Result: 多语言提示显著提升学习效果，尤其在学生母语与反馈语言一致的低资源语言中。

Conclusion: 研究为开发多语言、基于LLM的教育工具提供了实用见解，强调语言对齐的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 神经机器翻译在特定领域（如电子商务）中仍面临词义模糊和上下文不足的问题，需要改进。

Method: 创建了包含11,400句对的捷克语-波兰语数据集ConECT，测试了视觉语言模型（VLM）和文本到文本模型，探索了上下文信息（如产品类别路径和图像描述）的融入效果。

Result: 实验表明，上下文信息的融入显著提升了翻译质量，视觉上下文尤其有效。

Conclusion: 上下文信息对提升机器翻译质量至关重要，新数据集为相关研究提供了资源。
...
TL;DR: 论文评估了大型语言模型（LLMs）在大学级算法考试中的表现，发现最新模型表现优异，但图论任务仍有困难。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在算法考试中的表现，探索其在教育中的潜在应用。

Method: 测试多个模型在罗马尼亚语考试及其英语翻译中的表现，分析问题解决能力、一致性和多语言性能。

Result: 最新模型表现接近优秀学生，具备复杂推理能力，但图论任务表现不佳。
...
TL;DR: 论文提出了一种增量半监督学习流程，结合少量领域内标注数据和相关领域辅助数据，通过多模型共识或命名实体识别筛选伪标签，显著提升了ASR模型在低标注数据场景下的性能。
...
Method: 提出增量半监督学习流程，先结合少量领域内标注数据和相关领域辅助数据，再通过多模型共识或命名实体识别筛选伪标签进行迭代优化。

Result: 在多领域Wow呼叫中心和Fisher英语语料库上评估，性能优于单步微调。共识筛选表现最佳，Wow和Fisher上相对提升分别达22.3%和24.8%。

Conclusion: 共识筛选和命名实体识别是有效的伪标签筛选方法，显著提升ASR模型性能，且后者计算成本更低。
...
TL;DR: 论文提出SCOP方法，从认知角度评估LLMs在理解过程中的表现，发现LLMs与专家理解存在差距，并指出改进方向。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs在机器理解方面潜力巨大，但其理解过程是否与专家一致尚不明确，需要系统性评估。

Method: 提出SCOP，定义五项必备理解技能，构建测试数据框架，并分析开源和闭源LLMs的表现。

Result: LLMs难以达到专家级理解水平，但局部理解优于全局理解，且存在通过错误过程得出正确答案的问题。
...
Conclusion: ComfyUI-Copilot有效提升了ComfyUI的易用性和效率，适用于不同水平的用户。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要复杂的模型修改，限制了与预训练模型的兼容性，因此需要一种更简单的方法来控制生成文本的长度。

Method: 通过增加EOS令牌在交叉熵损失中的预测重要性来控制生成文本的长度，方法独立于模型架构和解码算法。

Result: 实验表明，该方法能有效控制生成文本的长度，且通常不影响摘要质量。
...
TL;DR: AR-Checker框架通过多轮并行LLM重写和验证生成数学问题变体，动态评估LLM的鲁棒性，避免数据污染。


<details>
  <summary>Details</summary>
Motivation: 评估LLM在简单推理任务中的鲁棒性，避免手工模板或有限扰动规则带来的数据污染问题。

Method: 提出AR-Checker框架，通过多轮并行LLM重写和验证生成语义相同但可能使LLM失败的问题变体。

Result: 在GSM8K、MATH-500等数学任务及MMLU、CommonsenseQA等非数学任务上表现优异。

Conclusion: AR-Checker能动态生成测试基准，有效评估LLM鲁棒性，适用范围广。
...
TL;DR: TALL通过结合LLM和双语翻译模型，提升低资源语言的表现，实验证明其优于基线方法。
...
Method: 结合LLM与双语翻译模型，通过维度对齐层和定制Transformer处理输入。

Result: 在希伯来语实验中显著优于基线方法。

Conclusion: TALL是一种参数高效的方法，能有效提升低资源语言的LLM表现。
...
TL;DR: 论文提出辩论演讲评估作为评估LLM法官的新基准，分析了LLM在理解演讲多层面能力上的表现，并与人类法官对比。


<details>
  <summary>Details</summary>
Motivation: 辩论演讲评估需要多层次的深度理解，此前在LLM系统化评估中未受足够重视。

Method: 利用600多篇标注辩论演讲数据集，分析前沿LLM与人类法官的表现差异。

Result: 大模型在某些方面接近人类判断，但整体行为差异显著；前沿LLM生成说服性演讲能力可达人类水平。

Conclusion: 辩论演讲评估是LLM能力的重要新基准，揭示了模型与人类判断的差异及潜力。
...
<details>
  <summary>Details</summary>
Motivation: 研究LLMs的自我报告行为是否具有内省性，以及如何解释这些行为。

Method: 通过分析LLMs的两个自我报告例子，评估其内省性。

Result: 第一个例子（描述创作过程）不被视为内省；第二个例子（推断温度参数）可视为最小内省，但无意识伴随。

Conclusion: LLMs的某些自我报告行为可被视为内省，但需谨慎解释，且无意识参与。
...
TL;DR: 论文提出RIVAL框架，通过对抗训练解决LLM在口语字幕翻译任务中表现不佳的问题。


<details>
  <summary>Details</summary>
Motivation: 发现结合RLHF的LLM在口语字幕翻译中表现意外差，原因是离线奖励模型与在线LLM因分布偏移逐渐偏离。

Method: 提出RIVAL对抗训练框架，将RM与LLM建模为min-max博弈，结合定性与定量偏好奖励。
...
TL;DR: 该研究提出了一种通过语言-表情符号交互增强大语言模型（LLMs）自我伤害检测能力的方法，包括CESM-100表情符号矩阵和SHINES数据集，并展示了在多任务学习框架下的显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 社交媒体上的自我伤害检测对早期干预至关重要，但当前LLMs难以理解隐晦的语言和表情符号。

Method: 通过CESM-100丰富输入，多任务学习微调LLMs，并生成可解释的预测依据。

Result: 在零样本、少样本和微调场景下，框架显著提升了LLMs的检测和解释能力。

Conclusion: 结合意图区分和上下文线索，该方法有效解决了自我伤害信号的模糊性问题，相关资源已公开。
...
TL;DR: 研究提出了一种基于图像的可解释框架，分析中国哈尔滨社区街道的商业活力与车辆可及性、环境质量和行人感知的关系，发现适度车辆存在有益，但过度停车会降低步行体验。


<details>
  <summary>Details</summary>
Motivation: 探讨社区街道商业活力的驱动因素，尤其是车辆可及性、环境质量和行人感知的复杂交互作用。

Method: 利用街景图像和多模态大语言模型（VisualGLM-6B），结合GPT-4感知建模，构建社区商业活力指数（CCVI）并分析其与空间属性的关系。

Result: 适度车辆存在提升商业可及性，但过度停车（尤其在窄街道）会降低步行体验和满意度；绿化和清洁度显著提升满意度，但对定价影响较弱；街道宽度调节车辆影响。

Conclusion: 研究展示了AI辅助感知与城市形态分析结合的价值，为社区振兴提供了理论和工具支持，强调车辆活动的条件性作用。
...
Result: CL-ISR框架显著提高了误导文本的检测效果，尤其在语言复杂或隐含立场的情况下表现突出。

Conclusion: CL-ISR框架通过结合对比学习和隐式立场推理，为社交媒体误导文本检测提供了更高效的方法。
...
TL;DR: 论文提出了一种结合wav2vec 2.0和Phi-4多模态大语言模型的系统，用于口语能力评估，解决了BERT和W2V各自的模态限制，并在比赛中取得第二名。


<details>
  <summary>Details</summary>
Motivation: BERT和W2V在口语评估中各有局限性：BERT依赖ASR转录，无法捕捉语音特征；W2V缺乏语义解释性。

Method: 通过分数融合策略整合W2V和Phi-4多模态大语言模型。

Result: 系统在Speak & Improve Challenge 2025测试集上的RMSE为0.375，排名第二。
...
TL;DR: DiCoRe框架通过发散-收敛推理（Dreamer和Grounder）提升零样本事件检测性能，结合LLM-Judge验证，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决零样本事件检测中复杂事件本体和领域特定触发词提取的挑战，提升大型语言模型的实用性。

Method: 采用Dreamer进行发散推理以增加事件覆盖率，Grounder通过有限状态机约束解码对齐任务指令，LLM-Judge验证输出。
...
TL;DR: 论文提出了一种量化框架，研究神经语言模型的归纳偏差是否与人类处理约束一致，并通过m-local熵衡量语言局部不确定性。


<details>
  <summary>Details</summary>
Motivation: 探讨神经语言模型的归纳偏差是否与人类处理约束一致，填补相关研究空白。

Method: 提出m-local熵作为信息论度量，基于损失上下文惊奇度，量化语言局部不确定性。实验在扰动自然语言语料库和PFSA定义的语言上进行。

Result: 实验表明，m-local熵较高的语言对Transformer和LSTM模型更难学习，说明神经语言模型对语言局部统计结构敏感。
...
TL;DR: 该论文首次深入分析了大型音频-语言模型（LALMs）如何内部感知和识别听觉属性，发现属性信息在失败时随层深度减少，早期层解析属性与更高准确性相关。
...
Conclusion: 研究为听觉属性处理提供了新见解，为未来改进奠定了基础。
...
TL;DR: 研究比较了人类与大型语言模型（LLM）对图像描述中语义错误的严重性评估差异，发现人类对错误严重性的判断受视觉上下文影响，而LLM在性别和颜色错误评估上表现不一致。


<details>
  <summary>Details</summary>
Motivation: 探讨LLM是否能准确复现人类对错误严重性的判断，特别是在多模态（文本+图像）和单模态（仅文本）设置下。

Method: 扩展van Miltenburg等人的实验框架，评估四种错误类型（年龄、性别、服装类型、服装颜色）在单模态和多模态设置下的表现。

Result: 人类对不同错误类型的严重性判断不同，视觉上下文显著影响颜色和类型错误的严重性；LLM在性别和颜色错误评估上与人类不一致，仅Doubao和DeepSeek-V3表现较好。

Conclusion: LLM在复现人类错误严重性判断方面存在局限性，部分模型表现较好，但整体仍需改进。
...
TL;DR: 论文提出Knowledgeable-r1方法，通过联合采样和多策略分布平衡检索与模型固有知识，显著提升RAG系统的鲁棒性和推理准确性。


<details>
  <summary>Details</summary>
Motivation: 当前RAG系统过度依赖检索上下文，可能忽视模型固有知识并依赖不准确信息。

Method: 采用联合采样和多策略分布，探索模型的知识能力，促进参数化与上下文知识的自我整合。

Result: 在参数与上下文冲突任务及一般RAG任务中表现优异，反事实场景下比基线高17.07%。

Conclusion: Knowledgeable-r1有效平衡检索与模型知识，提升RAG系统的性能和鲁棒性。
...
TL;DR: 通过机制可解释性方法分析GPT-2和Llama2模型中社会、人口和性别偏见的内部结构，发现偏见计算高度集中在少数层，移除这些组件会影响其他NLP任务。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs中社会、人口和性别偏见的内部结构，以理解其来源和表现形式。

Method: 采用机制可解释性方法，分析模型内部边缘，评估偏见的稳定性、定位性和泛化性，并进行系统性消融实验。

Result: 偏见计算集中在少数层，且在不同微调设置下变化；移除偏见组件会减少偏见输出，但影响其他NLP任务。
...
Conclusion: ECoRAG通过优化证据性压缩显著提升了LLM性能，同时提高了效率。
...
TL;DR: Qwen3 Embedding系列是基于Qwen3基础模型的文本嵌入和重排序技术，通过多阶段训练流程和模型合并策略，提供多种模型尺寸，在多语言和多样化任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 提升文本嵌入和重排序能力，满足多语言和多领域的需求。

Method: 结合大规模无监督预训练和高质量数据集的有监督微调，利用Qwen3 LLMs生成多样化训练数据。

Result: 在MTEB等多语言基准测试和检索任务中取得最优成绩。

Conclusion: Qwen3 Embedding系列公开可用，支持社区研究和开发。
...
TL;DR: 论文研究了大规模神经语言模型（LMs）在上下文学习中的反事实推理能力，特别是在线性回归任务中表现出的噪声推断能力。


<details>
  <summary>Details</summary>
Motivation: 探索语言模型在假设场景下进行反事实推理的能力，以预测变化带来的后果。

Method: 通过一个定义明确的合成实验（线性回归任务），研究模型如何从上下文观察中推断和复制噪声。

Result: 发现语言模型在受控环境中能进行反事实推理，且自注意力、模型深度和预训练数据多样性是关键因素。

Conclusion: 研究不仅适用于回归任务，还初步展示了Transformer在序列数据（如故事生成）中反事实推理的潜力。
...
TL;DR: RELIC框架用于评估大语言模型（LLMs）的指令跟随能力，通过语言识别任务（判断字符串是否符合形式语法）。研究发现，即使最先进的LLMs在复杂语法和样本上表现接近随机，且随着任务难度增加，模型会依赖浅层启发式而非复杂指令。
...
Method: 引入RELIC框架，通过语言识别任务（基于形式语法）自动生成复杂度可调的实例，避免数据污染。

Result: LLMs的准确率可通过语法和样本复杂度预测，复杂任务中表现接近随机，且依赖浅层启发式。

Conclusion: RELIC为LLMs指令跟随能力提供了可扩展的评估工具，揭示了其在复杂任务中的局限性。
...
TL;DR: 论文介绍了Common Pile v0.1，一个8TB的公开许可文本数据集，用于训练大型语言模型（LLMs），解决了现有数据集规模小或质量低的问题。通过训练两个7B参数的模型，验证了其性能与未授权文本训练的模型相当。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs训练中未授权文本使用引发的知识产权和伦理问题，并提供高质量公开许可数据集。

Method: 收集、整理并发布Common Pile v0.1数据集，包含30个来源的多样化文本。训练两个7B参数的LLMs（Comma v0.1-1T和Comma v0.1-2T）验证数据集效果。

Result: 训练出的模型性能与未授权文本训练的模型（如Llama 1和2 7B）相当。

Conclusion: Common Pile v0.1为LLMs训练提供了可行的公开许可数据集，同时开源了数据集、代码和模型检查点。
...
TL;DR: 研究了自监督辅助任务在极低资源环境下对形态屈折任务的有效性，发现自动编码在数据极少时表现最佳，而字符掩码语言建模（CMLM）在数据增加时更有效。


<details>
  <summary>Details</summary>
Motivation: 探索自监督任务在字符级任务（如形态屈折）中的应用，尤其是在资源稀缺的语言中。

Method: 使用编码器-解码器变换器，在19种语言和13种辅助目标上训练，比较不同自监督任务的效果。

Result: 自动编码在数据极少时表现最佳，CMLM在数据增加时更有效；基于已知语素边界的掩码采样能持续提升性能。

Conclusion: 自监督任务在低资源形态建模中具有潜力，尤其是结合语素信息的CMLM。
...
<details>
  <summary>Details</summary>
Motivation: 现有语言学理论对不连续结构的分析各自独立且不收敛，缺乏统一框架。

Method: 结合短语结构语法的成分性、依存语法的头依赖关系和范畴语法的函子-论元关系，提出统一数学推导方法。

Result: 证明连续与不连续结构可通过统一数学推导分析。

Conclusion: 为自然语言结构分析提供了一种统一框架，整合了多种语法形式的优势。
...
TL;DR: 论文提出了一种系统性推理方法，通过分解文本并验证每个子事实，提升LLM在幻觉检测中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将幻觉检测视为NLI任务，但复杂推理任务需要更明确的推理过程以提高准确性。

Method: 定义了三步推理过程：声明分解、子声明归因与蕴含分类、聚合分类，并引入评估中间步骤质量的指标。

Result: 系统性推理显著提升了幻觉检测的准确性，并通过中间指标验证了其有效性。

Conclusion: 系统性推理框架能更精细地处理蕴含决策，为幻觉检测提供了更优方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在处理知识冲突时直接比较知识源，导致LLMs被冗长上下文淹没，无法有效识别不一致。

Method: Micro-Act框架通过感知上下文复杂度，将知识源分解为细粒度比较步骤，实现深层推理。

Result: 在五个基准数据集上，Micro-Act显著优于现有方法，尤其在时间和语义冲突类型中表现突出。
...
<details>
  <summary>Details</summary>
Motivation: 多AI代理协作工作流常因提示设计不佳导致错误传播和性能下降，限制了系统的可靠性和扩展性。

Method: ProRefine利用大型语言模型的文本反馈，动态优化多步推理任务的提示，无需额外训练或真实标签。

Result: 在五个数学推理基准数据集上，ProRefine比零样本思维链基线高出3到37个百分点。

Conclusion: ProRefine不仅提高了准确性，还使小模型能匹配大模型的性能，为高效、可扩展的AI部署提供了可能。
...
<details>
  <summary>Details</summary>
Motivation: 现有遗忘方法在平衡遗忘和保留时存在优化不稳定和性能下降问题，需要更高效的解决方案。

Method: 采用约束优化问题，使用logit-margin flattening损失函数，结合原始对偶算法动态调整遗忘与保留的权衡。

Result: 在TOFU和MUSE基准测试中表现优于现有方法，能有效移除目标信息并保持下游任务性能。

Conclusion: 该方法为LLM遗忘提供了更稳定和高效的解决方案，适用于实际部署场景。
...
TL;DR: 论文介绍了Search Arena数据集，用于评估搜索增强语言模型的用户偏好，发现用户偏好受引用数量影响，且不同来源的偏好差异显著。
...
Method: 构建了包含24,000对多轮交互的众包数据集，涵盖多种意图和语言，并收集了12,000个人类偏好投票。

Result: 用户偏好受引用数量影响，社区驱动平台更受欢迎；搜索增强模型在非搜索环境中表现良好，但搜索环境中依赖参数知识会显著影响质量。

Conclusion: 开源数据集支持未来研究，揭示了用户偏好与引用行为的关系。
...
TL;DR: 语言模型在偏好判断中存在系统性偏差，过度依赖表面特征（如长度、结构等），导致奖励滥用和不可靠评估。研究发现这些偏差源于训练数据中的伪影，并提出一种基于反事实数据增强的后训练方法，有效减少了偏差。


<details>
  <summary>Details</summary>
Motivation: 语言模型作为人类偏好判断的代理存在系统性偏差，影响其可靠性和公平性，因此需要研究并解决这一问题。

Method: 通过控制反事实对量化偏好模型的偏差程度，并采用反事实数据增强（CDA）方法进行后训练优化。

Result: 偏好模型在60%以上的情况下表现出偏差，与人类偏好相比有40%的校准误差。CDA方法将校准误差从39.4%降至32.5%，偏差差异从20.5%降至10.0%。

Conclusion: 反事实数据增强方法能有效减少偏好模型的偏差，提高其可靠性，同时保持整体性能。
...
### [94] [Dynamic Epsilon Scheduling: A Multi-Factor Adaptive Perturbation Budget for Adversarial Training](https://arxiv.org/abs/2506.04263)
*Alan Mitkiy,James Smith,Hana Satou,Hiroshi Tanaka,Emily Johnson,F Monkey*

Main category: cs.CV

TL;DR: 论文提出了一种动态调整对抗扰动预算的方法（DES），通过结合梯度代理、预测置信度和模型不确定性，动态调整每个实例和训练迭代的扰动预算，显著提升了对抗鲁棒性和标准准确性。
...
Method: 提出DES框架，结合梯度代理、预测置信度和模型不确定性，动态调整扰动预算。
...
### [95] [RSVP: Reasoning Segmentation via Visual Prompting and Multi-modal Chain-of-Thought](https://arxiv.org/abs/2506.04277)
*Yi Lu,Jiawang Cao,Yongliang Wu,Bozheng Li,Licheng Tang,Yangguang Ji,Chong Wu,Jay Wu,Wenbo Zhu*

Main category: cs.CV

TL;DR: RSVP是一个两阶段框架，结合多模态推理与视觉分割，提升视觉定位和分割精度。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型在视觉定位和分割方面的不足，弥合认知推理与视觉感知的差距。

Method: 采用两阶段结构：推理阶段生成区域建议，分割阶段通过视觉语言分割模块细化分割掩码。

Result: 在ReasonSeg和SegInW数据集上表现优异，零样本设置下分别提升6.5 gIoU和9.2 cIoU，达到49.7 mAP。
...
### [96] [Evaluating MLLMs with Multimodal Multi-image Reasoning Benchmark](https://arxiv.org/abs/2506.04280)
...
TL;DR: MMRB是首个评估多图像结构化视觉推理的基准，包含92个子任务，覆盖空间、时间和语义推理，并采用GPT-4o生成的多解决方案和CoT风格标注。实验显示开源MLLMs在多图像推理任务中显著落后于商业模型。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM基准主要关注单图像推理或多图像理解任务，缺乏对多图像输入推理能力的评估，因此需要填补这一空白。

Method: 提出MMRB基准，包含92个子任务，采用GPT-4o生成标注，并设计句子级匹配框架进行快速评估。

Result: 开源MLLMs在多图像推理任务中表现显著落后于商业模型，且当前多模态奖励模型在多图像奖励排序任务中几乎无法胜任。
...
### [97] [HuGeDiff: 3D Human Generation via Diffusion with Gaussian Splatting](https://arxiv.org/abs/2506.04351)
*Maksym Ivashechkin,Oscar Mendez,Richard Bowden*

Main category: cs.CV

TL;DR: 提出了一种弱监督流程，通过图像扩散模型生成可控属性的真实人类图像数据集，再通过基于transformer的架构映射到3D点云，最后训练点云扩散模型，显著提升了3D人类生成的速度、文本对齐性、真实感和渲染质量。


<details>
  <summary>Details</summary>
Motivation: 3D人类生成在计算机视觉和图形学中有广泛应用，但现有方法在细节、手部和面部渲染、真实性和可控性方面存在不足，且缺乏多样性和标注数据。

Method: 1. 使用图像扩散模型生成可控属性的真实人类图像数据集；2. 提出基于transformer的架构将图像特征映射到3D点云；3. 训练点云扩散模型，条件为原始文本提示。

Result: 相比现有方法，实现了数量级的速度提升，显著改善了文本对齐性、真实感和渲染质量。

Conclusion: 提出的弱监督流程有效解决了3D人类生成的挑战，代码和数据集将公开。
...
### [98] [ReXVQA: A Large-scale Visual Question Answering Benchmark for Generalist Chest X-ray Understanding](https://arxiv.org/abs/2506.04353)
*Ankit Pal,Jung-Oh Lee,Xiaoman Zhang,Malaikannan Sankarasubbu,Seunghyeon Roh,Won Jung Kim,Meesun Lee,Pranav Rajpurkar*

Main category: cs.CV

TL;DR: ReXVQA是最大的胸部放射学视觉问答（VQA）基准，包含69.6万问题和16万胸部X光研究，评估了8种多模态大语言模型，其中MedGemma表现最佳（83.24%准确率），甚至超过人类专家（77.27%）。


<details>
  <summary>Details</summary>
Motivation: 为胸部放射学提供更全面、临床真实的VQA基准，超越传统模板化问题，评估AI在核心放射学推理技能上的表现。

Method: 构建ReXVQA数据集，包含多样化的临床任务，评估8种多模态模型，并与3名放射科住院医师进行对比研究。

Result: MedGemma模型表现最佳（83.84%准确率），超越人类专家（77.27%），揭示了AI与人类在放射学推理上的差异。

Conclusion: ReXVQA为评估通用放射学AI系统设定了新标准，支持下一代AI系统模仿专家级临床推理。
...
### [99] [WorldPrediction: A Benchmark for High-level World Modeling and Long-horizon Procedural Planning](https://arxiv.org/abs/2506.04363)
*Delong Chen,Willy Chung,Yejin Bang,Ziwei Ji,Pascale Fung*

Main category: cs.CV

TL;DR: WorldPrediction是一个基于视频的基准测试，用于评估AI模型的世界建模和程序规划能力，强调时间和语义抽象的动作。


<details>
  <summary>Details</summary>
Motivation: 当前AI模型（尤其是生成模型）如何学习世界模型并进行程序规划尚不明确，需要一个新基准来填补这一空白。

Method: 通过区分正确动作或动作序列与干扰项的任务设置，评估不同类型的世界模型和规划器，并使用视觉观察表示状态和动作。

Result: 前沿模型在WorldPrediction-WM和WorldPrediction-PP上的准确率分别为57%和38%，而人类能完美完成任务。

Conclusion: WorldPrediction为评估AI的世界建模和规划能力提供了可靠基准，揭示了当前模型与人类能力的差距。
...
### [100] [Ice Hockey Puck Localization Using Contextual Cues](https://arxiv.org/abs/2506.04365)
*Liam Salass,Jerrin Bright,Amir Nazemi,Yuhao Chen,John Zelek,David Clausi*

Main category: cs.CV

TL;DR: PLUCC利用球员行为上下文线索改进冰球检测，通过多尺度特征和门控解码器实现高性能。
...
Result: 在PuckDataset上，PLUCC的AP提升12.2%，RSLE提升25%，表现最优。
...
### [101] [Fine-Tuning Video Transformers for Word-Level Bangla Sign Language: A Comparative Analysis for Classification Tasks](https://arxiv.org/abs/2506.04367)
*Jubayer Ahmed Bhuiyan Shawon,Hasan Mahmud,Kamrul Hasan*

Main category: cs.CV

TL;DR: 该研究通过微调VideoMAE、ViViT和TimeSformer等视频变换器架构，在孟加拉手语数据集（BdSLW60和BdSLW401）上实现了高性能的手语识别，显著优于传统方法。
...
Method: 使用视频变换器架构（VideoMAE、ViViT、TimeSformer），结合数据增强技术和10折分层交叉验证，在BdSL数据集上进行微调和评估。

Result: VideoMAE模型在BdSLW60和BdSLW401数据集上分别达到95.5%和81.04%的准确率，表现最佳。

Conclusion: 视频变换器模型在孟加拉手语识别中具有强大的潜力，尤其是在数据集规模和视频质量优化的情况下。
...
### [102] [Visualizing and Controlling Cortical Responses Using Voxel-Weighted Activation Maximization](https://arxiv.org/abs/2506.04379)
...
<details>
  <summary>Details</summary>
Motivation: 尽管DNN编码模型能预测大脑对视觉刺激的反应，但无法揭示驱动这些反应的具体特征。

Method: 使用Inception V3网络提取特征，通过线性回归预测fMRI反应，并应用激活最大化生成优化图像。
...
### [103] [Is Perturbation-Based Image Protection Disruptive to Image Editing?](https://arxiv.org/abs/2506.04394)
...
Result: 发现保护方法未能完全阻止编辑，反而可能增强编辑效果，导致更好的生成结果。

Conclusion: 基于扰动的方法不足以提供对扩散模型编辑的鲁棒保护，需探索更有效的解决方案。
...
### [104] [Normalize Filters! Classical Wisdom for Deep Vision](https://arxiv.org/abs/2506.04401)
*Gustavo Perez,Stella X. Yu*

Main category: cs.CV

TL;DR: 论文提出了一种滤波器归一化方法，通过可学习的缩放和偏移，解决了深度学习卷积滤波器在图像大气传输中响应失真的问题，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 传统图像滤波器经过精心归一化以确保一致性和可解释性，而深度学习中的卷积滤波器缺乏此类约束，导致在大气传输中响应失真，影响结果准确性。

Method: 提出滤波器归一化方法，类似批归一化，结合可学习的缩放和偏移，确保滤波器的大气等变性，实现共域对称性。

Result: 在人工和自然强度变化基准测试中取得显著改进，ResNet34甚至大幅超越CLIP。

Conclusion: 滤波器归一化不仅正则化学习、促进多样性，还提高了鲁棒性和泛化能力，解决了未归一化滤波器性能下降的问题。
...
### [105] [HMAR: Efficient Hierarchical Masked Auto-Regressive Image Generation](https://arxiv.org/abs/2506.04421)
...
<details>
  <summary>Details</summary>
Motivation: VAR在并行生成高分辨率图像时存在质量下降、序列长度超线性增长和采样计划固定等问题。HMAR旨在解决这些问题。

Method: HMAR采用马尔可夫过程和多步掩码生成策略，仅依赖前一尺度的信息生成图像，并引入高效的注意力机制。

Result: HMAR在ImageNet基准测试中表现优于VAR、扩散模型和自回归基线，训练和推理速度更快，内存占用更低。

Conclusion: HMAR不仅提升了图像生成质量与效率，还提供了灵活的采样计划和零样本图像编辑能力。
...
TL;DR: 本文研究了使用第一视角设备进行高动态范围场景的真实感重建的挑战，提出了基于视觉-惯性束调整（VIBA）和高斯泼溅的物理图像形成模型，显著提升了重建质量。
...
Method: 1. 使用VIBA校准滚动快门RGB相机的时间戳和高频轨迹；2. 将基于高斯泼溅的物理图像形成模型纳入重建流程。

Result: 在开源Project Aria设备和Meta Quest3设备上的实验表明，VIBA带来+1 dB的PSNR提升，图像形成模型进一步带来+1 dB提升。

Conclusion: 提出的方法显著提升了真实感重建的视觉质量，适用于多种高斯泼溅表示变体。
...
### [107] [Towards Large-Scale Pose-Invariant Face Recognition Using Face Defrontalization](https://arxiv.org/abs/2506.04496)
*Patrik Mesec,Alan Jović*

Main category: cs.CV

TL;DR: 论文提出了一种名为“面部去正面化”的方法，通过增强训练数据集来提高面部特征提取模型的性能，并在多个数据集上验证了其有效性。
...
Method: 1) 训练一个基于FFWM的面部去正面化模型；2) 使用ArcFace损失训练ResNet-50模型，结合原始和去正面化数据。

Result: 在LFW、AgeDB和CFP数据集上优于现有方法，但在Multi-PIE极端姿态下效果不佳。

Conclusion: 面部去正面化方法有效，但需注意现有方法可能在小数据集上过拟合。
...
### [108] [FALO: Fast and Accurate LiDAR 3D Object Detection on Resource-Constrained Devices](https://arxiv.org/abs/2506.04499)
...
Conclusion: FALO在精度和速度上均表现出色，适合部署在资源受限的嵌入式设备上。
...
### [109] [AuthGuard: Generalizable Deepfake Detection via Language Guidance](https://arxiv.org/abs/2506.04501)
*Guangyu Shen,Zhihua Li,Xiang Xu,Tianchen Zhao,Zheng Zhang,Dongsheng An,Zhuowen Tu,Yifan Xing,Qin Zhang*

Main category: cs.CV

TL;DR: AuthGuard通过结合语言指导和统计线索，提升了深度伪造检测的泛化能力，并在多个数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测技术依赖于训练时学到的统计伪影，难以应对新的伪造方法。

Method: 结合分类与图像-文本对比学习，利用MLLMs生成文本指导，并集成数据不确定性学习。

Result: 在DFDC和DF40数据集上AUC分别提升6.15%和16.68%，在DDVQA数据集上性能提升24.69%。

Conclusion: AuthGuard通过语言指导和统计线索的结合，显著提升了深度伪造检测的泛化能力和准确性。
...
### [110] [Pruning Everything, Everywhere, All at Once](https://arxiv.org/abs/2506.04513)
...
Result: 在ResNet56和ResNet110上分别实现86.37%和95.82%的FLOPs减少，且模型性能几乎无损。

Conclusion: 该方法在高效剪枝的同时提升模型鲁棒性，为GreenAI和减少碳排放提供了重要进展。
...
### [111] [EECD-Net: Energy-Efficient Crack Detection with Spiking Neural Networks and Gated Attention](https://arxiv.org/abs/2506.04526)
*Shuo Zhang*
...
Method: 采用SRCNN增强图像分辨率，SCU降低功耗，GAT模块融合多尺度特征，提升检测鲁棒性。
...
### [112] [Enhancing Frequency for Single Image Super-Resolution with Learnable Separable Kernels](https://arxiv.org/abs/2506.04555)
*Heng Tian*

Main category: cs.CV

TL;DR: 本文提出了一种名为可学习可分离核（LSKs）的即插即用模块，通过直接增强图像频率分量来提升单图像超分辨率（SISR）性能。LSKs显著减少了参数数量和计算需求，并通过可视化分析验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常通过间接方式（如专用损失函数）提升SISR性能，本文旨在通过直接增强频率分量来优化性能。

Method: 提出LSKs模块，将其分解为可合并的一维核以减少参数和计算量，并从频率角度分析其适用性。

Result: 实验表明，LSKs减少了60%以上的参数和计算量，同时提升了模型性能，尤其在高放大因子下表现更优。

Conclusion: LSKs是一种高效且有效的SISR增强模块，适用于实际应用。
...
### [113] [Perceptual Decoupling for Scalable Multi-modal Reasoning via Reward-Optimized Captioning](https://arxiv.org/abs/2506.04559)
...
<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型在升级推理模块时视觉-语言对齐的高成本问题，同时确保视觉提取生成的描述既忠实于图像又能支持准确推理。

Method: 提出RACRO方法，通过推理引导的强化学习策略优化视觉提取器的描述生成，实现感知与推理的对齐。

Result: 在数学和科学多模态基准测试中，RACRO实现了最佳平均性能，并支持更高级推理模型的即插即用适配。

Conclusion: RACRO通过优化视觉提取与推理的对齐，显著提升了多模态推理的性能和可扩展性。
...
### [114] [LGM-Pose: A Lightweight Global Modeling Network for Real-time Human Pose Estimation](https://arxiv.org/abs/2506.04561)
...
### [115] [Follow-Your-Creation: Empowering 4D Creation through Video Inpainting](https://arxiv.org/abs/2506.04590)
*Yue Ma,Kunyu Feng,Xinhua Zhang,Hongyu Liu,David Junhao Zhang,Jinbo Xing,Yinhan Zhang,Ayden Yang,Zeyu Wang,Qifeng Chen*

Main category: cs.CV

TL;DR: 介绍了一种名为Follow-Your-Creation的4D视频生成与编辑框架，通过视频修复模型作为生成先验，实现从单目视频输入生成和编辑4D内容。


<details>
  <summary>Details</summary>
Motivation: 解决从单目视频生成和编辑4D内容的需求，利用视频修复模型的先验知识，填补因相机轨迹变化或用户编辑导致的缺失内容。

Method: 结合深度点云渲染生成不可见区域掩码，与用户编辑掩码结合形成复合掩码数据集，通过随机采样训练模型。采用自迭代调优策略处理大相机运动下的时间一致性，并引入时间包装模块提升生成质量。

Result: 能够生成具有多视角一致性的4D视频，支持基于提示的内容编辑，质量和灵活性显著优于现有方法。

Conclusion: 该方法有效利用基础模型的先验知识，在不降低其原始性能的情况下，实现了高质量的4D视频生成与编辑。
...
### [116] [Hierarchical-Task-Aware Multi-modal Mixture of Incremental LoRA Experts for Embodied Continual Learning](https://arxiv.org/abs/2506.04595)
...
<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法忽视高层规划和多层次知识学习，需解决这些问题。

Method: HEC框架分层学习，Task-aware MoILE通过聚类和路由选择LoRA专家，SVD保留关键参数。

Result: 实验显示方法在减少旧任务遗忘上优于其他方法。

Conclusion: 方法有效支持智能体持续学习新任务同时保留旧知识。
...
### [117] [SmartAvatar: Text- and Image-Guided Human Avatar Generation with VLM AI Agents](https://arxiv.org/abs/2506.04606)
*Alexander Huang-Menders,Xinhang Liu,Andy Xu,Yuyao Zhang,Chi-Keung Tang,Yu-Wing Tai*

Main category: cs.CV

TL;DR: SmartAvatar是一个基于视觉-语言-代理的框架，通过单张照片或文本提示生成完全绑定、动画就绪的3D人体化身。
...
Method: 结合大型视觉语言模型（VLM）和现成参数化人体生成器，通过自主验证循环迭代优化生成参数。

Result: 生成的化身在网格质量、身份保真度、属性准确性和动画就绪性上优于现有方法。

Conclusion: SmartAvatar是一个高效、可定制且适用于消费级硬件的3D化身生成工具。
...
### [118] [Perfecting Depth: Uncertainty-Aware Enhancement of Metric Depth](https://arxiv.org/abs/2506.04612)
*Jinyoung Jun,Lei Chu,Jiahao Li,Yan Lu,Chang-Su Kim*

Main category: cs.CV

TL;DR: 提出了一种名为Perfecting Depth的两阶段框架，用于传感器深度增强，结合随机扩散模型和确定性细化，提高深度图的可靠性和准确性。


<details>
  <summary>Details</summary>
Motivation: 解决传感器深度图中不可靠区域的自动检测问题，同时保留几何线索，以提升深度图的质量和应用范围。
...
### [119] [Deep Learning Reforms Image Matching: A Survey and Outlook](https://arxiv.org/abs/2506.04619)
*Shihua Zhang,Zizhuo Li,Kaining Zhang,Yifan Lu,Yuxin Deng,Linfeng Tang,Xingyu Jiang,Jiayi Ma*

Main category: cs.CV

TL;DR: 本文综述了深度学习如何逐步改进传统的图像匹配流程，包括可学习模块的引入和端到端方法的融合，并评估了其在多个任务中的表现，同时指出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 传统图像匹配流程在复杂场景中表现不佳，深度学习技术的进步显著提升了其鲁棒性和准确性，本文旨在系统梳理这些改进。

Method: 通过分类和评估深度学习驱动的策略，包括替换传统流程中的步骤为可学习模块（如检测器-描述符、离群值过滤器）以及融合多个步骤为端到端模块（如稀疏匹配器、姿态回归器）。

Result: 在相对姿态恢复、单应性估计和视觉定位任务上对代表性方法进行了基准测试，展示了深度学习的优势。

Conclusion: 深度学习显著推动了图像匹配领域的发展，但仍存在挑战，未来研究应关注进一步创新和优化。
...
### [120] [Unfolding Spatial Cognition: Evaluating Multimodal Models on Visual Simulations](https://arxiv.org/abs/2506.04633)
*Linjie Li,Mahtab Bigverdi,Jiawei Gu,Zixian Ma,Yinuo Yang,Ziang Li,Yejin Choi,Ranjay Krishna*

Main category: cs.CV

TL;DR: STARE是一个评估多模态大语言模型在空间认知任务中表现的基准，涵盖几何变换、空间推理和现实世界应用。模型在简单任务表现良好，但在复杂任务中接近随机水平，且视觉模拟对模型效果不一致。
...
Method: STARE包含4K任务，涵盖2D/3D几何变换、空间推理（如立方体展开和七巧板）及现实世界应用（如视角和时间推理）。

Result: 模型在简单2D任务表现优异，但在复杂3D任务中接近随机水平。人类表现近乎完美但耗时较长，视觉模拟可显著加速；模型对视觉模拟的利用效果不一致。

Conclusion: 模型在复杂空间认知任务中表现不足，视觉模拟利用效率低，需进一步改进以提升多步视觉推理能力。
...
### [121] [Text-Aware Real-World Image Super-Resolution via Diffusion Model with Joint Segmentation Decoders](https://arxiv.org/abs/2506.04641)
...
Method: 结合文本感知注意力和联合分割解码器，并提出了合成高质量图像的全流程方法。

Result: 实验表明，TADiSR显著提升了超分辨率图像中文本的清晰度，并在多项指标上达到最优性能。
...
### [122] [FPSAttention: Training-Aware FP8 and Sparsity Co-Design for Fast Video Diffusion](https://arxiv.org/abs/2506.04648)
...
Method: 提出FPSAttention，通过统一3D分块粒度、噪声步感知策略和硬件友好内核，实现量化与稀疏化的联合优化。

Result: 在1.3B和14B模型上测试，FPSAttention实现了7.09倍注意力操作加速和4.96倍端到端视频生成加速。

Conclusion: FPSAttention有效解决了扩散模型推理效率问题，为高质量视频生成提供了实用解决方案。
...
### [123] [Feature-Based Lie Group Transformer for Real-World Applications](https://arxiv.org/abs/2506.04668)
...
### [124] [Interpretable Few-Shot Image Classification via Prototypical Concept-Guided Mixture of LoRA Experts](https://arxiv.org/abs/2506.04673)
*Zhong Ji,Rongshuai Wei,Jingren Liu,Yanwei Pang,Jungong Han*

Main category: cs.CV

TL;DR: 论文提出了一种Few-Shot Prototypical Concept Classification (FSPCC)框架，通过参数高效适应和多层次特征融合，解决了数据稀缺场景下自解释模型（SEMs）的性能问题，显著提升了分类准确率和可解释性。


<details>
  <summary>Details</summary>
Motivation: 自解释模型（SEMs）在数据稀缺场景中表现不佳，主要由于参数不平衡和表示不对齐问题。
...
### [125] [Gen-n-Val: Agentic Image Data Generation and Validation](https://arxiv.org/abs/2506.04676)
*Jing-En Huang,I-Sheng Fang,Tzuhsuan Huang,Chih-Yu Wang,Jun-Cheng Chen*

Main category: cs.CV

TL;DR: Gen-n-Val是一个基于LLMs和VLLMs的代理数据生成框架，通过优化提示和验证机制生成高质量合成数据，显著提升目标检测和实例分割性能。
...
Method: 结合Layer Diffusion、LLMs和VLLMs，设计两个代理：LD提示代理优化生成单对象掩码，数据验证代理过滤低质量数据，并通过TextGrad优化系统提示。

Result: 将无效合成数据从50%降至7%，在COCO实例分割中提升1% mAP，开放词汇目标检测中提升7.1% mAP。

Conclusion: Gen-n-Val显著提升了合成数据的质量和模型性能，适用于多种视觉任务。
...
### [126] [MARS: Radio Map Super-resolution and Reconstruction Method under Sparse Channel Measurements](https://arxiv.org/abs/2506.04682)
...
<details>
  <summary>Details</summary>
Motivation: 无线电地图在智能城市和物联网等应用中至关重要，但稀疏测量下的精确重建仍具挑战性。传统方法缺乏环境感知，而深度学习方法依赖详细场景数据，泛化能力有限。

Method: 提出MARS方法，结合CNN和Transformer，利用多尺度特征融合和残差连接，同时关注全局和局部特征提取。

Result: 实验表明，MARS在不同场景和天线位置下均优于基线模型，MSE和SSIM表现更优，且计算成本低。

Conclusion: MARS具有强实用潜力，能有效提升无线电地图的重建精度。
...
### [127] [HoliSafe: Holistic Safety Benchmarking and Modeling with Safety Meta Token for Vision-Language Model](https://arxiv.org/abs/2506.04704)
*Youngwan Lee,Kangsan Kim,Kwanyong Park,Ilcahe Jung,Soojin Jang,Seanie Lee,Yong-Ju Lee,Sung Ju Hwang*

Main category: cs.CV

TL;DR: 论文提出了HoliSafe数据集和SafeLLaVA模型，解决了现有视觉语言模型（VLMs）安全性的不足。HoliSafe覆盖了五种安全/不安全的图像-文本组合，而SafeLLaVA通过可学习的安全元标记和专用安全头提升了模型安全性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在VLMs安全性上存在两个主要问题：1）数据集和基准测试覆盖不全，2）缺乏架构创新。论文旨在填补这些空白。

Method: 提出HoliSafe数据集和SafeLLaVA模型，后者包含安全元标记和安全头，用于编码有害视觉线索和分类危害性。

Result: SafeLLaVA在多个VLM基准测试中达到最先进的安全性能，HoliSafe揭示了现有模型的漏洞。

Conclusion: HoliSafe和SafeLLaVA为未来多模态对齐研究提供了新方向，推动了VLM安全性的发展。
...
### [128] [Line of Sight: On Linear Representations in VLLMs](https://arxiv.org/abs/2506.04706)
*Achyuta Rajaram,Sarah Schwettmann,Jacob Andreas,Arthur Conmy*

Main category: cs.CV

TL;DR: 论文研究了LlaVA-Next多模态模型中图像概念的表示方式，发现其残差流中存在线性可解码的特征，并通过训练稀疏自编码器提高了特征多样性。


<details>
  <summary>Details</summary>
Motivation: 探索多模态模型如何在其隐藏激活中表示图像概念，以增强模型的可解释性和多样性。

Method: 通过线性解码和稀疏自编码器（SAEs）分析LlaVA-Next模型的残差流特征，并进行目标编辑验证其因果性。

Result: 发现图像和文本特征在深层网络中逐渐共享，尽管初始表示较为分离。

Conclusion: 多模态模型的特征表示在深层网络中趋于融合，稀疏自编码器可提升特征多样性。
...
### [129] [Robust Few-Shot Vision-Language Model Adaptation](https://arxiv.org/abs/2506.04713)
*Hanxin Wang,Tian Liu,Shu Kong*

Main category: cs.CV

TL;DR: 该论文研究了预训练视觉语言模型（VLM）在少量标注样本下的适应性问题，旨在提升其在分布内（ID）和分布外（OOD）数据上的泛化能力。通过比较不同适应方法，提出了部分微调视觉编码器并结合检索增强和对抗扰动的两阶段方法SRAPF，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 预训练VLM在少量标注样本下适应下游任务时，面对OOD数据泛化能力不足，需要提升其ID和OOD准确性。

Method: 比较了提示调优、线性探测、对比微调和完全微调等方法，发现视觉编码器部分微调效果最佳。提出了结合检索增强和对抗扰动的两阶段方法SRAPF。

Result: SRAPF在ImageNet OOD基准测试中达到了最先进的ID和OOD准确性。

Conclusion: 部分微调视觉编码器并结合检索增强和对抗扰动的两阶段方法SRAPF，显著提升了VLM在少量样本下的适应性和泛化能力。
...
### [130] [Towards Holistic Visual Quality Assessment of AI-Generated Videos: A LLM-Based Multi-Dimensional Evaluation Model](https://arxiv.org/abs/2506.04715)
*Zelu Qi,Ping Shi,Chaoyang Zhang,Shuqi Wang,Fei Zhao,Da Pan,Zefeng Ying*

Main category: cs.CV

TL;DR: 论文提出了一种基于多维度特征和大语言模型（LLM）的AI生成视频（AIGV）自动视觉质量评估方法，并在NTIRE 2025比赛中取得第二名。


<details>
  <summary>Details</summary>
Motivation: AIGV技术发展迅速，但其视觉质量缺陷（如噪声、模糊、帧抖动等）影响用户体验，亟需有效的自动评估方法。

Method: 将视觉质量分解为技术质量、运动质量和视频语义三个维度，设计对应编码器提取特征，并引入LLM作为质量回归模块，结合多模态提示工程和LoRA微调技术。

Result: 在NTIRE 2025比赛中获得第二名，验证了方法的有效性。

Conclusion: 该方法为AIGV内容监管和生成模型改进提供了有效工具。
...
### [131] [Learning dissection trajectories from expert surgical videos via imitation learning with equivariant diffusion](https://arxiv.org/abs/2506.04716)
...
TL;DR: 本文提出了一种名为iDPOE的新方法，通过隐式扩散策略和等变表示学习，用于内窥镜黏膜下剥离术（ESD）视频中的轨迹预测，显著提升了预测准确性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 预测ESD视频中的解剖轨迹对提升手术技能训练和学习效率具有重要潜力，但目前研究不足。现有模仿学习方法在处理不确定未来动作、几何对称性和多样化手术场景时存在挑战。

Method: 提出iDPOE方法，通过联合状态动作分布建模专家行为，结合扩散模型进行策略学习，并嵌入等变性以增强几何对称性泛化能力。采用前向过程引导的动作推断策略解决状态不匹配问题。

Result: 在近2000个ESD视频片段的数据集上，iDPOE在轨迹预测任务中超越了现有最先进方法。

Conclusion: iDPOE是首次将模仿学习应用于解剖轨迹预测的手术技能开发，展示了其在提升预测性能和泛化能力方面的潜力。
...
### [132] [Using In-Context Learning for Automatic Defect Labelling of Display Manufacturing Data](https://arxiv.org/abs/2506.04717)
*Babar Hussain,Qiang Liu,Gang Chen,Bihai She,Dahai Yu*

Main category: cs.CV

TL;DR: 本文提出了一种基于AI的自动标注系统，用于显示面板缺陷检测，通过上下文学习能力提升SegGPT架构，并引入涂鸦标注机制。实验表明，该系统显著提升了性能，减少了人工标注需求。


<details>
  <summary>Details</summary>
Motivation: 工业检测中人工标注成本高且耗时，需要一种高效的自动标注解决方案。

Method: 采用并改进SegGPT架构，结合领域特定训练技术和涂鸦标注机制，采用两阶段训练方法。

Result: 在工业数据集上验证，IoU平均提升0.22，召回率提升14%，自动标注覆盖率达60%。

Conclusion: 该系统能有效替代人工标注，为工业检测提供实用解决方案。
...
### [133] [Bridging Annotation Gaps: Transferring Labels to Align Object Detection Datasets](https://arxiv.org/abs/2506.04737)
*Mikhail Kennerley,Angelica Alives-Reviro,Carola-Bibiane Schönlieb,Robby T. Tan*

Main category: cs.CV

TL;DR: LAT是一种标签对齐转移框架，通过伪标签生成和特征融合，解决多数据集目标检测中的标签语义和边界框不一致问题。


<details>
  <summary>Details</summary>
Motivation: 多数据集联合训练可提升泛化性，但标签语义和边界框标注的不一致性阻碍了这一目标。现有方法要么假设共享标签分类，要么需要手动重标注，或生成统一标签空间，但可能不适用于固定目标标签空间的情况。

Method: LAT通过训练数据集特定检测器生成伪标签，结合特权提案生成器（PPG）和语义特征融合（SFF）模块，实现标签空间的对齐和特征优化。

Result: LAT在多个基准测试中显著提升目标域检测性能，最高提升+4.8AP。

Conclusion: LAT无需依赖共享标签空间或手动标注，即可同时解决类别和边界框不一致问题，适用于异构数据集训练。
...
### [134] [SRD: Reinforcement-Learned Semantic Perturbation for Backdoor Defense in VLMs](https://arxiv.org/abs/2506.04743)
...
<details>
  <summary>Details</summary>
Motivation: VLMs在图像描述任务中表现出色，但容易被后门攻击，导致生成恶意描述。攻击隐蔽且跨模态，难以检测和防御。

Method: 提出SRD框架，利用Deep Q-Network学习对敏感图像区域施加离散扰动（如遮挡、颜色掩码），以破坏恶意路径的激活。设计语义保真度分数作为奖励信号。
...
### [135] [Physics Informed Capsule Enhanced Variational AutoEncoder for Underwater Image Enhancement](https://arxiv.org/abs/2506.04753)
...
<details>
  <summary>Details</summary>
Motivation: 解决水下图像增强中物理约束与语义结构保持的问题。

Method: 采用双流架构，物理估计器估计传输图和背景光，胶囊聚类提取特征，结合物理约束优化目标。
...
### [136] [Truth in the Few: High-Value Data Selection for Efficient Multi-Modal Reasoning](https://arxiv.org/abs/2506.04755)
*Shenshen Li,Kaiyuan Deng,Lei Wang,Hao Yang,Chong Peng,Peng Yan,Fumin Shen,Heng Tao Shen,Xing Xu*

Main category: cs.CV

TL;DR: 该论文提出了一种名为RAP的数据选择范式，通过识别能有效触发多模态推理的稀疏样本（认知样本），显著减少了训练数据量和计算成本。


<details>
  <summary>Details</summary>
Motivation: 挑战传统观点，即多模态大语言模型（MLLMs）需要大量训练数据才能提升推理能力，提出仅需少量高质量数据即可达到或超越全量数据的效果。

Method: 提出RAP范式，包含两个互补的估计器（CDE和ACE）和一个难度感知替换模块（DRM），用于筛选和优化训练样本。

Result: 在六个数据集上的实验表明，RAP仅需9.3%的训练数据即可实现更优性能，同时减少43%以上的计算成本。

Conclusion: RAP通过高效数据选择，显著提升了多模态推理的效率，为资源受限场景提供了实用解决方案。
...
### [137] [Toward Better SSIM Loss for Unsupervised Monocular Depth Estimation](https://arxiv.org/abs/2506.04758)
...
<details>
  <summary>Details</summary>
Motivation: 传统方法使用MAE和SSIM作为训练损失，但忽略了SSIM函数中不同组件及超参数对训练的影响。

Method: 提出新的SSIM形式，用加法替代乘法组合组件，构建更平滑梯度的损失函数。

Result: 在KITTI-2015数据集上，优化的SSIM损失函数显著优于基线方法。

Conclusion: 新SSIM形式在无监督深度估计中表现更优，梯度更平滑。
...
### [138] [HypeVPR: Exploring Hyperbolic Space for Perspective to Equirectangular Visual Place Recognition](https://arxiv.org/abs/2506.04764)
*Suhan Woo,Seongwon Lee,Jinwoo Jang,Euntai Kim*

Main category: cs.CV

TL;DR: HypeVPR是一种新颖的层次化嵌入框架，利用双曲空间解决P2E VPR的挑战，通过层次化特征聚合和粗到细搜索策略，显著提升检索速度和准确性。


<details>
  <summary>Details</summary>
Motivation: 现实世界中移动机器人等应用的VPR需求，需要适应不同视角的查询图像，P2E方法成为自然选择。

Method: 采用双曲空间表示层次化特征关系，提出层次化特征聚合机制和粗到细搜索策略。

Result: 在多个基准数据集上表现优于现有方法，检索速度提升高达5倍。

Conclusion: HypeVPR通过层次化特征表示和高效搜索策略，显著提升了P2E VPR的性能和效率。
...
### [139] [Object-X: Learning to Reconstruct Multi-Modal 3D Object Representations](https://arxiv.org/abs/2506.04789)
*Gaia Di Lorenzo,Federico Tombari,Marc Pollefeys,Daniel Barath*

Main category: cs.CV

TL;DR: Object-X是一个多模态3D对象表示框架，能够编码丰富的对象嵌入并解码为几何和视觉重建，支持多种下游任务，同时存储需求极低。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常针对特定任务设计嵌入，无法同时支持几何重建和跨任务复用。Object-X旨在解决这一问题，提供通用且高效的多模态3D表示。

Method: 通过将多模态数据（如图像、点云、文本）几何地映射到3D体素网格中，并学习融合体素与对象属性的非结构化嵌入，支持基于3D高斯泼溅的重建。

Result: 在真实数据集上，Object-X实现了高保真新视角合成，几何精度显著提升，同时在场景对齐和定位任务中表现优异，存储需求极低。

Conclusion: Object-X是一种可扩展且实用的多模态3D场景表示解决方案，适用于增强现实和机器人等应用。
...
### [140] [LotusFilter: Fast Diverse Nearest Neighbor Search via a Learned Cutoff Table](https://arxiv.org/abs/2506.04790)
*Yusuke Matsui*

Main category: cs.CV

TL;DR: LotusFilter是一种后处理模块，用于多样化近似最近邻搜索（ANNS）结果，通过预计算和贪心查找删除冗余向量。


<details>
  <summary>Details</summary>
Motivation: ANNS在某些场景下可能生成过于相似的结果，而实际应用中需要结果既相似又多样化。

Method: 预计算邻近向量表，并在过滤阶段贪心查找以删除冗余向量。

Result: 在类似真实RAG应用的设置中，LotusFilter运行速度快（0.02 ms/查询）。

Conclusion: LotusFilter有效解决了ANNS结果多样性问题，且高效实用。
...
### [141] [SupeRANSAC: One RANSAC to Rule Them All](https://arxiv.org/abs/2506.04803)
*Daniel Barath*

Main category: cs.CV

TL;DR: SupeRANSAC是一种新型统一的RANSAC管道，旨在提高计算机视觉任务中几何模型估计的鲁棒性和一致性。


<details>
  <summary>Details</summary>
Motivation: 尽管RANSAC及其变体是处理异常值数据的黄金标准，但在不同任务中实现一致高性能仍具挑战性。现有框架（如OpenCV和PoseLib）在某些任务中表现优异，但在其他任务中表现不佳。

Method: 提出SupeRANSAC，通过统一管道和详细分析技术，优化RANSAC在特定视觉任务（如单应性、基础/本质矩阵和绝对/刚性姿态估计）中的表现。
...
### [142] [MegaHan97K: A Large-Scale Dataset for Mega-Category Chinese Character Recognition with over 97K Categories](https://arxiv.org/abs/2506.04807)
*Yuyi Zhang,Yongxin Shi,Peirong Zhang,Yixin Zhao,Zhenhua Yang,Lianwen Jin*

Main category: cs.CV

TL;DR: 论文介绍了MegaHan97K数据集，填补了中文超大类别字符识别的空白，支持GB18030-2022标准，并提供平衡样本。


<details>
  <summary>Details</summary>
Motivation: 中文字符类别庞大且不断扩展，现有数据集无法满足超大类别识别需求，阻碍文化遗产保护和数字应用。

Method: 构建MegaHan97K数据集，包含97,455个类别，分为手写、历史和合成子集，解决长尾分布问题。

Result: 实验揭示了超大类别场景的新挑战（如存储需求、形态相似字符识别、零样本学习），同时为未来研究提供机会。

Conclusion: MegaHan97K是目前OCR和模式识别领域类别最多的数据集，推动了超大类别字符识别的研究。
...
### [143] [Spike-TBR: a Noise Resilient Neuromorphic Event Representation](https://arxiv.org/abs/2506.04817)
*Gabriele Magrini. Federico Becattini,Luca Cultrera,Lorenzo Berlincioni,Pietro Pala,Alberto Del Bimbo*

Main category: cs.CV

TL;DR: 提出了一种基于时空二进制表示（TBR）和脉冲神经网络的噪声鲁棒事件流编码方法Spike-TBR，在噪声环境下表现优异。


<details>
  <summary>Details</summary>
Motivation: 事件相机具有高时间分辨率、低延迟和宽动态范围，但如何高效且鲁棒地将其输出转换为标准计算机视觉格式仍具挑战性。

Method: 结合TBR的帧式优势和脉冲神经网络的噪声过滤能力，提出Spike-TBR，并评估了四种不同脉冲神经元的变体。

Result: 在噪声数据和干净数据上均表现优越，验证了方法的鲁棒性和有效性。
...
### [144] [Fool the Stoplight: Realistic Adversarial Patch Attacks on Traffic Light Detectors](https://arxiv.org/abs/2506.04823)
...
<details>
  <summary>Details</summary>
Motivation: 现有研究较少关注交通灯检测器的对抗攻击，本文旨在填补这一空白，展示此类攻击的可行性和实际威胁。

Method: 提出一种威胁模型，通过在交通灯下方放置对抗性补丁，并设计训练策略，实现通用场景下的攻击。

Result: 实验证明攻击成功实现了红绿灯标签翻转和图形分类攻击，并在真实场景中验证了攻击效果。

Conclusion: 研究表明交通灯检测系统易受对抗性补丁攻击，需加强防御措施。
...
### [145] [DualX-VSR: Dual Axial Spatial$\times$Temporal Transformer for Real-World Video Super-Resolution without Motion Compensation](https://arxiv.org/abs/2506.04830)
*Shuo Cao,Yihao Liu,Xiaohui Li. Yuanting Gao. Yu Zhou,Chao Dong*

Main category: cs.CV

TL;DR: DualX-VSR提出了一种新型的双轴时空注意力机制，解决了现有基于Transformer的视频超分辨率模型在像素级精度和运动补偿方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的视频超分辨率模型在像素级精度和运动补偿方面存在不足，限制了其在真实场景中的应用。

Method: DualX-VSR通过双轴时空注意力机制，沿正交方向整合时空信息，无需运动补偿。

Result: DualX-VSR在真实场景视频超分辨率任务中实现了高保真度和卓越性能。

Conclusion: DualX-VSR通过简化结构和改进注意力机制，显著提升了视频超分辨率的效果。
...
### [146] [OpenMaskDINO3D : Reasoning 3D Segmentation via Large Language Model](https://arxiv.org/abs/2506.04837)
*Kunshen Zhang*
...
<details>
  <summary>Details</summary>
Motivation: 现有感知系统在2D推理分割上已成熟，但缺乏3D推理分割的框架。本文旨在填补这一空白。

Method: 引入SEG token和对象标识符，处理点云数据和文本提示，生成高精度3D分割掩码。

Result: 在ScanNet数据集上的实验验证了OpenMaskDINO3D的有效性。

Conclusion: OpenMaskDINO3D为3D理解和分割提供了高效解决方案。
...
### [147] [Geological Field Restoration through the Lens of Image Inpainting](https://arxiv.org/abs/2506.04869)
...
Conclusion: 该方法为地质场重建提供了一种鲁棒的优化框架，具有更高的重建精度。
...
### [148] [Invisible Backdoor Triggers in Image Editing Model via Deep Watermarking](https://arxiv.org/abs/2506.04879)
*Yu-Feng Chen,Tzuhsuan Huang,Pin-Yen Chiu,Jun-Cheng Chen*

Main category: cs.CV

TL;DR: 本文提出了一种通过投毒训练数据在图像编辑过程中嵌入不可见触发器的后门攻击框架，利用现成的水印模型实现隐蔽攻击。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像生成和编辑中表现优异，但其易受后门攻击，尤其是图像编辑领域的隐蔽攻击研究较少。

Method: 利用现成的水印模型将不可见水印编码为后门触发器，通过投毒训练数据实现攻击。

Result: 在不同水印模型上实验，攻击成功率显著，水印特性分析进一步验证了方法的有效性。

Conclusion: 该方法成功实现了隐蔽的后门攻击，为图像编辑安全提供了新的研究方向。
...
### [149] [Learning to Plan via Supervised Contrastive Learning and Strategic Interpolation: A Chess Case Study](https://arxiv.org/abs/2506.04892)
*Andrew Hamara,Greg Hamerly,Pablo Rivas,Andrew C. Freeman*

Main category: cs.CV

TL;DR: 论文提出了一种基于直觉驱动的规划方法，通过对比学习训练Transformer编码器，将棋盘状态嵌入到潜在空间中，实现无需深度搜索的走子选择，并在6层束搜索下达到2593 Elo评分。


<details>
  <summary>Details</summary>
Motivation: 现代国际象棋引擎依赖深度树搜索和回归评估，而人类玩家则依靠直觉选择候选走法后进行浅层搜索验证。本文旨在模拟这种直觉驱动的规划过程。

Method: 使用监督对比学习训练Transformer编码器，将棋盘状态嵌入到潜在空间中，距离反映评估相似性，可视化轨迹显示游戏状态间的可解释过渡。走子选择完全在嵌入空间中进行，无需深度搜索。

Result: 模型在仅使用6层束搜索的情况下，达到2593 Elo评分。性能随模型规模和嵌入维度提升，表明潜在规划可能成为传统搜索的可行替代方案。

Conclusion: 尽管聚焦于国际象棋，但提出的基于嵌入的规划方法可推广到其他完美信息游戏中，前提是状态评估是可学习的。所有源代码已开源。
...
### [150] [From Objects to Anywhere: A Holistic Benchmark for Multi-level Visual Grounding in 3D Scenes](https://arxiv.org/abs/2506.04897)
*Tianxu Wang,Zhuofan Zhang,Ziyu Zhu,Yue Fan,Jing Xiong,Pengxiang Li,Xiaojian Ma,Qing Li*

Main category: cs.CV

TL;DR: Anywhere3D-Bench是一个新的3D视觉基准测试，涵盖四个层次的视觉定位任务，揭示了当前模型在空间和部分级别任务上的显著不足。


<details>
  <summary>Details</summary>
Motivation: 现有的3D视觉定位主要关注物体级别，而忽略了更复杂的空间和部分级别任务，因此需要一个新的基准测试来评估模型的综合能力。

Method: 研究者提出了Anywhere3D-Bench，包含2,632个参考表达-3D边界框对，覆盖四个层次的任务（人类活动区域、未占用空间、物体和物体部分），并测试了多种先进模型。

Result: 实验显示，空间和部分级别任务最具挑战性，最佳模型（OpenAI o4-mini）在这些任务上的准确率仅为23.57%和33.94%，远低于其他任务。

Conclusion: 当前模型在3D场景的空间和部分级别理解上存在显著不足，需要进一步研究提升其综合推理能力。
...
### [151] [Generating Synthetic Stereo Datasets using 3D Gaussian Splatting and Expert Knowledge Transfer](https://arxiv.org/abs/2506.04908)
*Filip Slezak,Magnus K. Gjerde,Joakim B. Haurum,Ivan Nikolov,Morten S. Laursen,Thomas B. Moeslund*

Main category: cs.CV

TL;DR: 本文提出了一种基于3D高斯泼溅（3DGS）的立体数据集生成方法，相比基于神经辐射场（NeRF）的方法更高效。通过结合显式3D表示和FoundationStereo模型的深度估计，展示了在零样本泛化基准上的竞争力。


<details>
  <summary>Details</summary>
Motivation: 探索一种低成本、高保真度的数据集生成方法，并快速微调深度立体模型。

Method: 利用3DGS生成的几何重建和FoundationStereo模型的深度估计，通过专家知识转移生成数据集。

Result: 3DGS生成的几何重建存在噪声和伪影，而FoundationStereo的视差估计更干净，零样本泛化性能更好。

Conclusion: 3DGS方法在低成本数据集生成和快速微调方面潜力巨大，但在复杂场景中的鲁棒性仍需改进。
...
### [152] [Light and 3D: a methodological exploration of digitisation techniques adapted to a selection of objects from the Mus{é}e d'Arch{é}ologie Nationale](https://arxiv.org/abs/2506.04925)
...
TL;DR: 论文探讨了文化遗产数字化中3D摄影方法的多样性，强调应根据对象特性和未来用途选择最适合的方法，而非追求绝对分类。


<details>
  <summary>Details</summary>
Motivation: 文化遗产数字化需求广泛，但现有3D数字化方法多样且无统一标准，需根据具体对象和用途灵活选择。

Method: 通过法国国家考古博物馆的藏品案例，分析不同3D摄影方法的适用性，强调跨领域合作选择工具。

Result: 研究表明，没有单一方法适用于所有对象，需根据对象特性和数字化用途定制工具。

Conclusion: 文化遗产数字化应避免绝对分类，而是通过合作选择最适合的工具，以适应多样化的需求。
...
### [153] [CzechLynx: A Dataset for Individual Identification and Pose Estimation of the Eurasian Lynx](https://arxiv.org/abs/2506.04931)
...
TL;DR: CzechLynx是首个大规模、开放访问的数据集，用于欧亚猞猁的个体识别、2D姿态估计和实例分割，包含3万多张相机陷阱图像和10万张合成图像。


<details>
  <summary>Details</summary>
Motivation: 为猞猁的个体识别和姿态估计提供高质量数据集，支持跨时空域的模型泛化测试。

Method: 数据集包括真实图像和合成图像，后者通过Unity和扩散模型生成。定义了三种评估协议：地理感知、时间感知开放集和闭集。

Result: 覆盖219个独特个体，15年监测数据，支持多种任务。

Conclusion: CzechLynx将推动猞猁识别及相关领域的研究和模型开发。
...
### [154] [Time-Lapse Video-Based Embryo Grading via Complementary Spatial-Temporal Pattern Mining](https://arxiv.org/abs/2506.04950)
*Yong Sun,Yipeng Wang,Junyu Shi,Zhiyuan Zhang,Yanmei Xiao,Lei Zhu,Manxi Jiang,Qiang Nie*

Main category: cs.CV

TL;DR: 论文提出了一种基于视频的胚胎分级任务，利用全长延时监测视频预测胚胎质量，并提出了互补时空模式挖掘框架（CoSTeM）来整合形态和动态特征。
...
Conclusion: 该工作为胚胎质量评估提供了新范式，数据集和代码将公开。
...
### [155] [Robustness as Architecture: Designing IQA Models to Withstand Adversarial Perturbations](https://arxiv.org/abs/2506.04951)
*Igor Meleshin,Anna Chistyakova,Anastasia Antsiferova,Dmitriy Vatolin*

Main category: cs.CV

TL;DR: 该论文提出了一种新的视角，通过设计而非学习来实现图像质量评估（IQA）模型的鲁棒性，提出了一种基于正交信息流的架构改进方法。


<details>
  <summary>Details</summary>
Motivation: IQA模型在实际应用中容易受到对抗性攻击，传统的数据驱动防御方法可能并非最佳解决方案。

Method: 通过强制正交信息流和规范保持操作来改进模型架构，并结合剪枝和微调进一步稳定系统。

Result: 提出了一种鲁棒的IQA架构，无需对抗训练即可抵御攻击。

Conclusion: 论文呼吁从数据优化转向设计优化，以实现模型的鲁棒性。
...
### [156] [APVR: Hour-Level Long Video Understanding with Adaptive Pivot Visual Information Retrieval](https://arxiv.org/abs/2506.04953)
...
Result: 在LongVideoBench和VideoMME上表现优异，达到SOTA。
...
### [157] [FEAT: Full-Dimensional Efficient Attention Transformer for Medical Video Generation](https://arxiv.org/abs/2506.04956)
...
### [158] [Bringing SAM to new heights: Leveraging elevation data for tree crown segmentation from drone imagery](https://arxiv.org/abs/2506.04970)
*Mélisande Teng,Arthur Ouaknine,Etienne Laliberté,Yoshua Bengio,David Rolnick,Hugo Larochelle*

Main category: cs.CV

TL;DR: 论文比较了利用Segment Anything Model (SAM)和数字表面模型(DSM)进行树冠实例分割的方法，提出了BalSAM模型，在特定场景下表现优于其他方法。
...
Method: 比较了SAM在三种森林类型中的自动树冠实例分割效果，并研究了DSM数据的整合。

Result: BalSAM在特定场景下表现优异，但未经调整的SAM效果不如定制Mask R-CNN。

Conclusion: 端到端调整SAM和整合DSM数据是提升树冠实例分割模型的有效途径。
...
### [159] [TextVidBench: A Benchmark for Long Video Scene Text Understanding](https://arxiv.org/abs/2506.04983)
*Yangyang Zhong,Ji Qi,Yuan Yao,Pengxin Luo,Yunfeng Yan,Donglian Qi,Zhiyuan Liu,Tat-Seng Chua*

Main category: cs.CV

TL;DR: 论文介绍了TextVidBench，首个专注于长视频文本问答（>3分钟）的基准测试，解决了现有数据集视频时长短和评估范围窄的问题。


<details>
  <summary>Details</summary>
Motivation: 现有数据集无法充分评估多模态大语言模型（MLLMs）的能力，尤其是长视频理解。

Method: 提出TextVidBench，包含跨领域长视频、三阶段评估框架和高精细标注；并提出IT-Rope机制、非均匀位置编码和轻量微调方法。

Result: TextVidBench对现有模型提出了显著挑战，提出的方法在长视频场景文本理解方面表现优异。

Conclusion: TextVidBench为长视频文本问答提供了更真实的评估标准，提出的方法为提升模型能力提供了新思路。
...
### [160] [Multi-scale Image Super Resolution with a Single Auto-Regressive Model](https://arxiv.org/abs/2506.04990)
*Enrique Sanchez,Isma Hadji,Adrian Bulat,Christos Tzelepis,Brais Martinez,Georgios Tzimiropoulos*

Main category: cs.CV

TL;DR: 论文提出了一种基于视觉自回归（VAR）模型的图像超分辨率（ISR）方法，通过多尺度图像标记化和偏好优化技术，解决了现有方法的固定分辨率限制和大模型依赖问题。


<details>
  <summary>Details</summary>
Motivation: 现有VARSR方法存在固定分辨率限制和大模型依赖问题，限制了其实际应用。

Method: 提出两种新组件：1）多尺度图像标记化方法，强制跨尺度标记重叠；2）直接偏好优化（DPO）正则化项，鼓励模型生成高分辨率标记。

Result: 模型在单次前向传播中实现去噪和超分辨率，达到SOTA效果，且模型更小（300M参数），无需外部训练数据。

Conclusion: 新方法解决了VARSR的局限性，展示了更高效和通用的ISR解决方案。
...
### [161] [PATS: Proficiency-Aware Temporal Sampling for Multi-View Sports Skill Assessment](https://arxiv.org/abs/2506.04996)
*Edoardo Bianchi,Antonio Liotta*

Main category: cs.CV

TL;DR: PATS是一种新的视频采样策略，专注于保持运动技能的连续性，以提升自动化技能评估的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有视频采样方法破坏了运动技能的时序连续性，影响评估效果。

Method: 提出PATS策略，自适应分割视频以确保每个片段包含完整的运动执行，并在多视点评估中重复此过程。

Result: 在EgoExo4D基准测试中，PATS显著优于现有方法，最高提升26.22%（攀岩）。

Conclusion: PATS是一种有效的自适应时序采样方法，适用于多样化的运动技能评估。
...
### [162] [Beyond Cropped Regions: New Benchmark and Corresponding Baseline for Chinese Scene Text Retrieval in Diverse Layouts](https://arxiv.org/abs/2506.04999)
*Gengluo Li,Huawen Shen,Yu Zhou*

Main category: cs.CV

TL;DR: 论文提出了一种针对中文场景文本检索的新模型CSTR-CLIP，并建立了多样化布局基准DL-CSVTR，显著提升了检索性能。


<details>
  <summary>Details</summary>
Motivation: 中文场景文本检索因复杂多样的布局而极具挑战，现有方法基于英文场景文本检索方案，效果不佳。

Method: 提出CSTR-CLIP模型，结合全局视觉信息与多粒度对齐训练，采用两阶段训练过程。

Result: CSTR-CLIP在现有基准上准确率提升18.82%，推理速度更快，且在DL-CSVTR上表现优异。
...
### [163] [Structure-Aware Radar-Camera Depth Estimation](https://arxiv.org/abs/2506.05008)
*Fuyi Zhang,Zhu Yu,Chunhao Li,Runmin Zhang,Xiaokai Bai,Zili Zhou,Si-Yuan Cao,Wang Wang,Hui-Liang Shen*

Main category: cs.CV

TL;DR: 论文探讨了单目深度估计的进展，重点介绍了基于深度学习的方法及其在未见领域泛化中的挑战。


<details>
  <summary>Details</summary>
Motivation: 单目深度估计旨在从单目相机拍摄的RGB图像中确定每个像素的深度，但其在未见领域的泛化能力仍是一个挑战。

Method: 论文回顾了多种方法，包括多尺度融合网络、将回归任务重新定义为分类问题、引入额外先验以及设计更有效的目标函数。

Result: 尽管有进展，未见领域的泛化仍具挑战性。Depth Anything在零样本单目深度估计中表现领先，但缺乏显式深度线索限制了其度量深度估计的准确性。

Conclusion: 论文总结了单目深度估计的现状，并指出未来需要进一步解决未见领域泛化问题。
...
### [164] [Point Cloud Segmentation of Agricultural Vehicles using 3D Gaussian Splatting](https://arxiv.org/abs/2506.05009)
...
Method: 使用3D高斯泼溅和透明度场生成农业车辆3D资产，模拟LiDAR生成点云数据。

Result: PTv3模型仅用合成数据训练，mIoU达91.35%，某些场景下表现优于真实数据训练的模型。

Conclusion: 合成数据可有效训练模型，且能泛化到未训练的语义类别。
...
### [165] [UAV4D: Dynamic Neural Rendering of Human-Centric UAV Imagery using Gaussian Splatting](https://arxiv.org/abs/2506.05011)
...
<details>
  <summary>Details</summary>
Motivation: 现有动态神经渲染方法未能解决无人机拍摄场景的特殊挑战，如单目相机、俯视角和多个小移动人体等问题。

Method: 使用3D基础模型和人体网格重建模型，通过人体-场景接触点解决场景尺度模糊问题，并利用SMPL模型和背景网格初始化高斯泼溅以实现整体渲染。

Result: 在三个无人机数据集上测试，PSNR提升1.5 dB，视觉清晰度优于现有方法。

Conclusion: UAV4D框架在无人机动态场景渲染中表现优异，解决了现有方法的不足。
...
### [166] [Physical Annotation for Automated Optical Inspection: A Concept for In-Situ, Pointer-Based Trainingdata Generation](https://arxiv.org/abs/2506.05026)
*Oliver Krumpek,Oliver Heimann,Jörg Krüger*

Main category: cs.CV

TL;DR: 提出了一种新型物理标注系统，用于生成自动化光学检测的训练数据，通过指针交互直接捕捉物理轨迹，提升标注效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 传统屏幕标注方法效率低且不够直观，无法充分利用人工检查员的专业知识。该系统旨在填补人工与自动化数据生成之间的鸿沟。

Method: 使用校准的跟踪指针记录用户输入，并通过投影界面提供视觉引导，将空间交互转化为标准化标注格式。

Result: 初步评估证实了系统可行性，能够捕获详细标注轨迹，并与CVAT集成优化ML工作流。

Conclusion: 该系统为非IT专家参与ML训练提供了可能，未来有望推动自动化光学检测的数据生成。
...
### [167] [FlowDirector: Training-Free Flow Steering for Precise Text-to-Video Editing](https://arxiv.org/abs/2506.05046)
*Guangzhao Li,Yanming Yang,Chenxi Song,Chi Zhang*

Main category: cs.CV

TL;DR: FlowDirector提出了一种无需反转的视频编辑框架，通过ODE在数据空间中直接演化视频，保持时间一致性和结构细节。


<details>
  <summary>Details</summary>
Motivation: 现有基于反转的方法常导致时间不一致和结构退化，FlowDirector旨在解决这些问题。
...
### [168] [A Survey on Vietnamese Document Analysis and Recognition: Challenges and Future Directions](https://arxiv.org/abs/2506.05061)
*Anh Le,Thanh Lam,Dung Nguyen*

Main category: cs.CV

TL;DR: 本文综述了越南文档分析与识别（DAR）的现状，探讨了传统OCR和深度学习方法在越南文本识别中的局限性，并提出了利用大语言模型（LLMs）和视觉语言模型改进的方向。


<details>
  <summary>Details</summary>
Motivation: 越南文本识别因复杂的变音符号、声调变化及缺乏大规模标注数据集而面临挑战，传统方法难以应对现实文档的多样性，深度学习则受限于数据稀缺和泛化问题。

Method: 综述了现有越南文档识别技术，分析了LLMs和视觉语言模型的潜力，并探讨了领域适应、多模态学习及计算效率等挑战。

Result: LLMs和视觉语言模型在文本识别和文档理解方面表现出显著改进潜力，但仍需解决数据、模型优化和多模态整合问题。

Conclusion: 未来研究方向包括数据集开发、模型优化和多模态方法整合，以推动越南DAR领域的进步和社区驱动的解决方案。
...
### [169] [SeedEdit 3.0: Fast and High-Quality Generative Image Editing](https://arxiv.org/abs/2506.05083)
*Peng Wang,Yichun Shi,Xiaochen Lian,Zhonghua Zhai,Xin Xia,Xuefeng Xiao,Weilin Huang,Jianchao Yang*

Main category: cs.CV

TL;DR: SeedEdit 3.0结合Seedream 3.0模型，显著提升了编辑指令遵循和图像内容保留能力，并通过数据增强和联合学习管道进一步优化。


<details>
  <summary>Details</summary>
Motivation: 改进现有T2I模型在真实图像编辑中的表现，提升指令遵循和内容保留能力。

Method: 1. 数据增强管道（元信息嵌入策略）；2. 联合学习管道（扩散损失与奖励损失）。

Result: 在真实图像编辑测试中，SeedEdit 3.0的可用性达56.1%，优于其他版本和竞品。

Conclusion: SeedEdit 3.0在多个方面实现了最佳平衡，显著提升了图像编辑的实用性。
...
### [170] [Interpretable Multimodal Framework for Human-Centered Street Assessment: Integrating Visual-Language Models for Perceptual Urban Diagnostics](https://arxiv.org/abs/2506.05087)
*HaoTian Lan*

Main category: cs.CV

TL;DR: 论文提出了一种多模态街道评估框架（MSEF），结合视觉和语言模型，以高效参数适应方法评估街道景观，同时捕捉主观感知与客观特征的矛盾。


<details>
  <summary>Details</summary>
Motivation: 现有客观街道指标无法充分捕捉主观感知，而主观感知对包容性城市设计至关重要。

Method: 使用视觉变换器（VisualGLM-6B）和大型语言模型（GPT-4）结合，通过LoRA和P-Tuning v2进行参数高效适应。

Result: 模型在客观特征上F1得分为0.84，与居民感知一致性达89.3%，并揭示了非线性感知模式。

Conclusion: MSEF为城市感知建模提供方法创新，并支持规划系统在基础设施与生活体验间的平衡。
...
### [171] [FG 2025 TrustFAA: the First Workshop on Towards Trustworthy Facial Affect Analysis: Advancing Insights of Fairness, Explainability, and Safety (TrustFAA)](https://arxiv.org/abs/2506.05095)
...
Method: 通过研讨会形式，汇集研究人员探讨FAA任务中的可信度问题，如解释性、不确定性、偏见和隐私。

Result: 研讨会支持FG2025的伦理目标，推动可信FAA的研究与讨论。

Conclusion: TrustFAA研讨会为FAA领域的可信度研究提供了平台，促进伦理与技术结合。
...
### [172] [Astraea: A GPU-Oriented Token-wise Acceleration Framework for Video Diffusion Transformers](https://arxiv.org/abs/2506.05096)
...
Result: 单GPU上推理速度提升2.4倍，8 GPU上提升13.2倍，视频质量损失小于0.5%。

Conclusion: ASTRAEA在保持高质量的同时，显著提升了vDiT的推理效率。
...
### [173] [DIMCIM: A Quantitative Evaluation Framework for Default-mode Diversity and Generalization in Text-to-Image Generative Models](https://arxiv.org/abs/2506.05108)
*Revant Teotia,Candace Ross,Karen Ullrich,Sumit Chopra,Adriana Romero-Soriano,Melissa Hall,Matthew J. Muckley*

Main category: cs.CV

TL;DR: 论文提出DIM-CIM框架，用于无参考评估文本到图像模型的多样性和泛化能力，发现模型规模扩大可能牺牲默认模式多样性。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法依赖参考图像或缺乏多样性类型特异性，限制了适应性和可解释性。

Method: 引入DIM-CIM框架，构建COCO-DIMCIM基准，评估模型的默认模式多样性和泛化能力。

Result: 模型规模扩大（1.5B到8.1B参数）可能提高泛化能力但牺牲默认模式多样性；训练数据多样性与默认模式多样性相关性强（0.85）。

Conclusion: DIM-CIM为评估T2I模型提供了灵活且可解释的框架，有助于全面理解模型性能。
...
### [174] [Practical Manipulation Model for Robust Deepfake Detection](https://arxiv.org/abs/2506.05119)
...
<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测模型在非理想条件下表现不稳定，容易被规避，需要改进。

Method: 开发PMM模型，使用Poisson混合、多样化掩码、生成器伪影和干扰物扩展伪造空间，并在训练图像中添加强退化。

Result: 在DFDC和DFDCP数据集上，AUC分别提升了3.51%和6.21%。

Conclusion: PMM显著提升了检测器的鲁棒性和性能，解决了以往模型的不足。
...
### [175] [CIVET: Systematic Evaluation of Understanding in VLMs](https://arxiv.org/abs/2506.05146)
*Massimo Rizzoli,Simone Alghisi,Olha Khomyn,Gabriel Roccabruna,Seyed Mahed Mousavi,Giuseppe Riccardi*

Main category: cs.CV

TL;DR: CIVET框架用于系统评估视觉语言模型（VLMs）对物体属性和关系的理解能力，发现当前VLMs仅能识别有限的基本属性，且表现受物体位置影响，无法达到人类水平。
...
Method: 提出CIVET框架，通过控制变量和标准化刺激物，系统评估五种VLMs的表现。

Result: VLMs仅能识别有限基本属性，表现依赖物体位置，且难以理解物体间关系，远低于人类水平。

Conclusion: 当前VLMs在场景理解上仍有局限，需进一步改进以接近人类能力。
...
### [176] [FRED: The Florence RGB-Event Drone Dataset](https://arxiv.org/abs/2506.05163)
...
<details>
  <summary>Details</summary>
Motivation: 传统RGB相机在捕捉高速移动的无人机时存在局限性，尤其是在复杂光照条件下。事件相机虽具备高时间分辨率和动态范围，但现有数据集缺乏精细时间分辨率或无人机特定运动模式。

Method: 作者提出了FRED数据集，包含超过7小时的密集标注无人机轨迹，涵盖5种无人机模型及复杂场景（如雨天和恶劣光照）。

Result: FRED提供了详细评估协议和标准指标，支持可复现的基准测试。
...
### [177] [Through-the-Wall Radar Human Activity Recognition WITHOUT Using Neural Networks](https://arxiv.org/abs/2506.05169)
*Weicheng Gao*

Main category: cs.CV

TL;DR: 论文提出了一种不依赖神经网络的穿墙雷达（TWR）人体活动识别（HAR）方法，通过模板匹配和拓扑相似性计算实现智能识别。


<details>
  <summary>Details</summary>
Motivation: 作者认为当前TWR HAR领域过度依赖神经网络训练，忽视了早期基于模板匹配的方法的物理可解释性和理论信号处理基础，因此尝试回归原始路径。

Method: 首先生成TWR的距离-时间图和多普勒-时间图，使用角点检测确定目标前景和噪声背景，通过多相主动轮廓模型分割微多普勒特征，并将其离散化为二维点云，最后利用Mapper算法计算点云与模板数据的拓扑相似性。

Result: 通过数值模拟和实测实验验证了方法的有效性。

Conclusion: 该方法在不使用神经网络的情况下实现了与神经网络模型相当的智能识别效果，代码已开源。
...
### [178] [Track Any Anomalous Object: A Granular Video Anomaly Detection Pipeline](https://arxiv.org/abs/2506.05175)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注异常帧或对象，忽略了像素级分析，限制了检测范围。TAO旨在解决这一问题。

Method: TAO框架将异常检测转化为像素级跟踪问题，结合分割和跟踪任务，避免阈值调整。
...
### [179] [Single GPU Task Adaptation of Pathology Foundation Models for Whole Slide Image Analysis](https://arxiv.org/abs/2506.05184)
...
TL;DR: TAPFM是一种针对病理基础模型（PFMs）的任务适应方法，通过视觉Transformer注意力进行多实例学习（MIL）聚合，优化特征表示和注意力权重，适用于单GPU环境。


<details>
  <summary>Details</summary>
Motivation: 适应预训练的PFMs用于特定临床任务面临挑战，主要由于WSI级标签的弱监督和计算资源限制。

Method: 提出TAPFM方法，利用ViT注意力进行MIL聚合，同时优化特征和注意力权重，保持MIL聚合器和PFM的独立计算图以实现稳定训练。
...
### [180] [MokA: Multimodal Low-Rank Adaptation for MLLMs](https://arxiv.org/abs/2506.05191)
...
<details>
  <summary>Details</summary>
Motivation: 现有高效多模态微调方法多借鉴自LLMs，忽略了多模态场景的内在差异，导致模态利用不足。

Method: 提出MokA策略，通过模态特定参数压缩单模态信息，并显式增强跨模态交互，实现单模态和跨模态适配。

Result: 在多种多模态场景和LLM骨干上的实验表明，MokA具有一致性和高效性。

Conclusion: MokA为多模态大模型的高效适配提供了针对性解决方案，为未来研究铺平了道路。
...
### [181] [Vision-Based Autonomous MM-Wave Reflector Using ArUco-Driven Angle-of-Arrival Estimation](https://arxiv.org/abs/2506.05195)
...
<details>
  <summary>Details</summary>
Motivation: 毫米波通信在非视距条件下可靠性差，尤其在城市或基础设施有限的环境中，亟需解决方案。

Method: 利用单目摄像头检测ArUco标记，实时估计到达角并调整反射器角度，实现信号优化反射。

Result: 实验显示，系统在60 GHz频段下平均增益23 dB，信号接收概率达0.89（阈值-65 dB）。
...
### [182] [Quantifying Cross-Modality Memorization in Vision-Language Models](https://arxiv.org/abs/2506.05198)
*Yuxin Wen,Yangsibo Huang,Tom Goldstein,Ravi Kumar,Badih Ghazi,Chiyuan Zhang*

Main category: cs.CV

TL;DR: 本文研究了跨模态记忆在视觉语言模型中的特性，通过合成数据集量化了知识记忆和跨模态可转移性，发现模态间存在显著差距，并提出了一种缓解方法。


<details>
  <summary>Details</summary>
Motivation: 探索神经网络在训练中的记忆行为，特别是跨模态记忆，以解决敏感信息泄露和知识获取效率问题。

Method: 使用合成人物数据集，训练单模态模型并评估跨模态性能，分析记忆差距。

Result: 发现模态间知识可转移但存在显著差距，且该差距普遍存在于多种场景中。

Conclusion: 提出基线方法缓解差距，呼吁未来研究开发更鲁棒的多模态学习技术。
...
### [183] [Grounding Beyond Detection: Enhancing Contextual Understanding in Embodied 3D Grounding](https://arxiv.org/abs/2506.05199)
*Yani Zhang,Dongming Wu,Hao Shi,Yingfei Liu,Tiancai Wang,Haoqiang Fan,Xingping Dong*

Main category: cs.CV

TL;DR: 论文探讨了3D物体定位任务中检测模型的性能是否优于专门训练的定位模型，并提出了DEGround框架，通过共享DETR查询和引入新模块显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 研究动机是验证检测模型在3D物体定位任务中的表现是否优于专门训练的定位模型，并探索更细粒度的上下文感知定位。
...
Conclusion: 研究表明检测模型在3D定位任务中表现优异，DEGround框架通过创新设计显著提升了性能，为细粒度上下文感知定位提供了新思路。
...
### [184] [OGGSplat: Open Gaussian Growing for Generalizable Reconstruction with Expanded Field-of-View](https://arxiv.org/abs/2506.05204)
...
<details>
  <summary>Details</summary>
Motivation: 稀疏视图重建语义感知3D场景的需求源于虚拟现实和具身AI等新兴应用，现有方法在计算成本或泛化能力上存在不足。

Method: OGGSplat利用开放高斯的语义属性作为先验，通过RGB-语义一致性修复模块和双向控制扩散模型，将修复区域提升到3D空间进行高斯参数优化。
...
### [185] [Follow-Your-Motion: Video Motion Transfer via Efficient Spatial-Temporal Decoupled Finetuning](https://arxiv.org/abs/2506.05207)
*Yue Ma,Yulong Liu,Qiyuan Zhu,Ayden Yang,Kunyu Feng,Xinhua Zhang,Zhifeng Li,Sirui Han,Chenyang Qi,Qifeng Chen*

Main category: cs.CV

TL;DR: 论文提出Follow-Your-Motion框架，通过空间-时间解耦LoRA和稀疏运动采样改进视频运动迁移任务，解决现有方法在运动一致性和调优效率上的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于LoRA的运动迁移方法在大型视频扩散变换器中存在运动不一致和调优效率低的问题，需要改进。

Method: 提出空间-时间解耦LoRA和稀疏运动采样技术，优化两阶段调优过程。

Result: 在MotionBench基准测试中验证了方法的优越性。
...
### [186] [Towards Vision-Language-Garment Models For Web Knowledge Garment Understanding and Generation](https://arxiv.org/abs/2506.05210)
...
### [187] [DSG-World: Learning a 3D Gaussian World Model from Dual State Videos](https://arxiv.org/abs/2506.05217)
...
Method: 利用双状态观测构建双分割感知高斯场，并强制双向光度和语义一致性，引入伪中间状态进行对称对齐，设计协作共剪枝策略优化几何完整性。

Result: DSG-World实现了高效的真实到仿真转换，支持高保真渲染和对象级场景操作，无需密集观测或多阶段流程。

Conclusion: 实验表明，DSG-World在新视角和场景状态上具有强泛化能力，适用于真实世界的3D重建和仿真。
...
### [188] [MonkeyOCR: Document Parsing with a Structure-Recognition-Relation Triplet Paradigm](https://arxiv.org/abs/2506.05218)
*Zhang Li,Yuliang Liu,Qiang Liu,Zhiyin Ma,Ziyang Zhang,Shuo Zhang,Zidun Guo,Jiarui Zhang,Xinyu Wang,Xiang Bai*

Main category: cs.CV

TL;DR: MonkeyOCR是一种基于SRR三元范式的视觉语言模型，用于文档解析，通过简化流程和高效处理，显著优于现有方法。
...
Conclusion: MonkeyOCR在精度和效率上均达到最先进水平，适用于多类型文档解析。
...
### [189] [SAM-aware Test-time Adaptation for Universal Medical Image Segmentation](https://arxiv.org/abs/2506.05221)
*Jianghao Wu,Yicheng Wu,Yutong Xie,Wenjia Bai,You Zhang,Feilong Tang,Yulong Li,Yasmeen George,Imran Razzak*

Main category: cs.CV

TL;DR: SAM-TTA是一种测试时适应框架，旨在提升Segment Anything Model（SAM）在医学图像分割中的性能，同时保持其泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决SAM在医学图像分割中适应性不足的问题，避免现有方法（如MedSAM）泛化能力下降的缺陷。

Method: 提出SAM-TTA框架，包括SBCT（自适应贝塞尔曲线变换）和DUMT（双尺度不确定性驱动的均值教师适应），分别解决输入级和语义级差异。

Result: 在五个公共数据集上的实验表明，SAM-TTA优于现有TTA方法，甚至在某些场景下超越完全微调的模型（如MedSAM）。
...
### [190] [Spatiotemporal Contrastive Learning for Cross-View Video Localization in Unstructured Off-road Terrains](https://arxiv.org/abs/2506.05250)
*Zhiyun Deng,Dongmyeong Lee,Amanda Adkins,Jesse Quattrociocchi,Christian Ellis,Joydeep Biswas*

Main category: cs.CV

TL;DR: MoViX是一种自监督的跨视角视频定位框架，解决了GPS缺失和季节性变化带来的定位挑战，通过学习和不变表示和方向感知实现高精度定位。


<details>
  <summary>Details</summary>
Motivation: 在GPS缺失的越野环境中，重复植被和无结构地形导致感知模糊，季节性变化进一步影响场景对齐，传统方法难以应对。

Method: MoViX采用姿态依赖的正样本采样和时间对齐的负样本挖掘，结合运动信息帧采样器和轻量级时间聚合器，学习不变表示。

Result: 在TartanDrive 2.0数据集上，MoViX在93%的情况下定位误差小于25米，100%小于50米，优于现有方法。

Conclusion: MoViX无需环境特定调优，在跨地理和机器人平台的真实数据中表现出强泛化能力。
...
### [191] [LeanPO: Lean Preference Optimization for Likelihood Alignment in Video-LLMs](https://arxiv.org/abs/2506.05260)
*Xiaodong Wang,Jinfa Huang,Li Yuan,Peixi Peng*

Main category: cs.CV

TL;DR: 论文提出LeanPO方法，解决Video-LLMs中偏好对齐技术导致的非目标响应概率上升问题，通过重新定义奖励和动态标签平滑策略提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有Video-LLMs采用偏好对齐技术（如DPO）时，目标和非目标响应的对数概率均下降，导致非目标响应概率上升，尤其在视频内容复杂时更严重。

Method: 提出LeanPO方法，包括基于策略模型平均似然的隐式奖励重新定义、自生成偏好数据管道和动态标签平滑策略。
...
### [192] [Can Foundation Models Generalise the Presentation Attack Detection Capabilities on ID Cards?](https://arxiv.org/abs/2506.05263)
...
<details>
  <summary>Details</summary>
Motivation: 当前PAD系统因隐私保护限制，通常仅针对少数ID卡训练，导致在新国家ID卡上表现不佳。

Method: 采用基础模型，通过零样本学习和微调两种方式，在智利ID卡私有数据集和芬兰、西班牙、斯洛伐克ID卡开放数据集上进行测试。

Result: 研究发现，真实图像（bona fide）是提升泛化能力的关键。

Conclusion: 基础模型能有效提升PAD系统对不同国家ID卡的泛化能力，真实图像数据尤为重要。
...
### [193] [From Play to Replay: Composed Video Retrieval for Temporally Fine-Grained Videos](https://arxiv.org/abs/2506.05274)
...
Method: 提出TF-CoVR基准，使用LLM生成查询-修改对，并设计TF-CoVR-Base框架，通过预训练视频编码器和对比学习对齐查询与候选视频。

Result: TF-CoVR-Base在零样本和微调设置下显著提升了检索性能，mAP@50分别从5.92提升到7.51和从19.83提升到25.82。
...
### [194] [Unifying Appearance Codes and Bilateral Grids for Driving Scene Gaussian Splatting](https://arxiv.org/abs/2506.05280)
...
Result: 在四个数据集（Waymo、NuScenes、Argoverse、PandaSet）上表现优异，几何精度显著提升，减少了光度不一致导致的浮游物。

Conclusion: 多尺度双边网格有效解决了光度不一致问题，对自动驾驶场景重建的几何精度至关重要。
...
### [195] [Rectified Point Flow: Generic Point Cloud Pose Estimation](https://arxiv.org/abs/2506.05282)
*Tao Sun,Liyuan Zhu,Shengyu Huang,Shuran Song,Iro Armeni*

Main category: cs.CV

TL;DR: Rectified Point Flow是一种统一的参数化方法，将点云配准和多部件形状组装建模为条件生成问题，通过学习连续点速度场实现目标定位，无需对称标签即可学习对称性，性能领先。


<details>
  <summary>Details</summary>
Motivation: 解决传统方法在点云配准和形状组装中依赖对称标签和独立训练的问题，提出统一框架以学习共享几何先验。

Method: 通过自监督编码器学习连续点速度场，将噪声点传输到目标位置，并恢复部件姿态，无需对称标签。

Result: 在六个基准测试中达到最新性能，统一框架支持多数据集联合训练，提升准确性。

Conclusion: Rectified Point Flow通过统一生成框架和自监督学习，显著提升了点云配准和形状组装的性能。
...
### [196] [Video World Models with Long-term Spatial Memory](https://arxiv.org/abs/2506.05284)
*Tong Wu,Shuai Yang,Ryan Po,Yinghao Xu,Ziwei Liu,Dahua Lin,Gordon Wetzstein*

Main category: cs.CV

TL;DR: 提出了一种基于几何空间记忆的框架，提升视频世界模型的长时一致性。


<details>
  <summary>Details</summary>
Motivation: 现有模型因时间窗口限制，难以保持场景一致性，导致遗忘问题。

Method: 引入几何空间记忆机制，存储和检索信息，并训练带有3D记忆的模型。

Result: 实验表明，模型在质量、一致性和上下文长度上优于基线。
...
### [197] [RaySt3R: Predicting Novel Depth Maps for Zero-Shot Object Completion](https://arxiv.org/abs/2506.05285)
...
<details>
  <summary>Details</summary>
Motivation: 现有3D形状补全方法缺乏3D一致性、计算成本高且难以捕捉物体边界，RaySt3R旨在解决这些问题。

Method: 利用单张RGB-D图像和查询射线，训练前馈Transformer预测深度图、物体掩码和置信度，并通过多视角融合完成3D重建。
...
### [198] [Stable Vision Concept Transformers for Medical Diagnosis](https://arxiv.org/abs/2506.05286)
*Lijie Hu,Songning Lai,Yuan Hua,Shu Yang,Jingfeng Zhang,Di Wang*

Main category: cs.CV

TL;DR: 论文提出Vision Concept Transformer (VCT)和Stable Vision Concept Transformer (SVCT)，解决医学领域中概念瓶颈模型(CBMs)的性能和稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 医学领域需要透明且可解释的AI方法，但现有CBMs仅依赖概念特征，忽略了医学图像的内在特征，且对输入扰动敏感。

Method: VCT结合概念特征与图像特征；SVCT基于VCT，采用视觉变换器(ViT)和Denoised Diffusion Smoothing，提升稳定性和解释性。

Result: 在四个医学数据集上，VCT和SVCT在保持准确性的同时提供可解释性，SVCT在扰动下仍能提供稳定解释。

Conclusion: VCT和SVCT满足了医学领域对透明、稳定且高性能AI模型的需求。
...
### [199] [EOC-Bench: Can MLLMs Identify, Recall, and Forecast Objects in an Egocentric World?](https://arxiv.org/abs/2506.05287)
*Yuqian Yuan,Ronghao Dang,Long Li,Wentong Li,Dian Jiao,Xin Li,Deli Zhao,Fan Wang,Wenqiao Zhang,Jun Xiao,Yueting Zhuang*

Main category: cs.CV

TL;DR: EOC-Bench是一个新的基准测试，用于评估多模态大语言模型（MLLMs）在动态自我中心场景中的物体认知能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注静态场景，忽略了用户交互引起的动态变化，因此需要新的评估工具。

Method: 开发了EOC-Bench，包含3,277个标注QA对，分为过去、现在和未来三类，涵盖11个评估维度和3种视觉物体引用类型。

Result: 通过混合格式的人工标注框架和多尺度时间准确性指标，对多种MLLMs进行了全面评估。
...
### [200] [AliTok: Towards Sequence Modeling Alignment between Tokenizer and Autoregressive Model](https://arxiv.org/abs/2506.05289)
*Pingyu Wu,Kai Zhu,Yu Liu,Longxiang Tang,Jian Yang,Yansong Peng,Wei Zhai,Yang Cao,Zheng-Jun Zha*

Main category: cs.CV

TL;DR: 提出了一种新型的对齐分词器AliTok，通过因果解码器建立单向依赖关系，提升自回归模型的生成效果，并在ImageNet-256上取得了优异的性能。
...
Result: 在ImageNet-256上，177M参数的模型gFID为1.50，IS为305.9；662M参数时gFID为1.35，超越现有扩散方法且采样速度快10倍。

Conclusion: AliTok通过单向依赖和一致性训练，显著提升了自回归图像生成的性能。
...
### [201] [SeedVR2: One-Step Video Restoration via Diffusion Adversarial Post-Training](https://arxiv.org/abs/2506.05301)
...
<details>
  <summary>Details</summary>
Motivation: 尽管扩散模型在视频修复中表现出色，但其高计算成本限制了实际应用。SeedVR2旨在通过单步修复解决这一问题。

Method: 提出动态调整窗口大小的注意力机制和一系列损失函数，包括特征匹配损失，以优化对抗训练。
...
### [202] [Perceive Anything: Recognize, Explain, Caption, and Segment Anything in Images and Videos](https://arxiv.org/abs/2506.05302)
*Weifeng Lin,Xinyu Wei,Ruichuan An,Tianhe Ren,Tingwei Chen,Renrui Zhang,Ziyu Guo,Wentao Zhang,Lei Zhang,Hongsheng Li*

Main category: cs.CV

TL;DR: PAM是一个高效的区域级视觉理解框架，结合了SAM 2和LLMs，实现对象分割与多样化语义输出，速度快且内存占用低。


<details>
  <summary>Details</summary>
Motivation: 提升区域级视觉理解的全面性和效率，结合分割模型与语言模型的优势。

Method: 通过Semantic Perceiver将SAM 2的视觉特征转化为多模态令牌，结合数据增强生成高质量数据集。

Result: PAM在多项任务中表现优异，速度提升1.2-2.4倍，GPU内存占用更低。
...
### [203] [Do It Yourself: Learning Semantic Correspondence from Pseudo-Labels](https://arxiv.org/abs/2506.05312)
*Olaf Dünkel,Thomas Wimmer,Christian Theobalt,Christian Rupprecht,Adam Kortylewski*

Main category: cs.CV

TL;DR: 论文提出了一种基于3D感知伪标签的方法，改进了语义匹配的准确性，减少了对称物体或重复部分的模糊性，并在SPair-71k数据集上取得了显著提升。
...
Method: 通过3D感知伪标签训练适配器，结合3D感知链式标注、松弛循环一致性和3D球形原型映射约束，优化现成特征。

Result: 在SPair-71k数据集上实现了超过4%的绝对提升，且相比类似监督需求的方法提升了7%。

Conclusion: 提出的方法具有通用性，可轻松扩展到其他数据源，实验验证了其有效性。
...
### [204] [MARBLE: Material Recomposition and Blending in CLIP-Space](https://arxiv.org/abs/2506.05313)
*Ta-Ying Cheng,Prafull Sharma,Mark Boss,Varun Jampani*

Main category: cs.CV

TL;DR: MARBLE是一种基于CLIP空间材料嵌入的方法，通过控制预训练的文本到图像模型实现材料混合和细粒度属性编辑。


<details>
  <summary>Details</summary>
Motivation: 研究如何通过示例图像编辑图像中对象的材料属性，提升材料编辑的精细度和灵活性。

Method: 利用CLIP空间中的材料嵌入，找到去噪UNet中负责材料属性的模块，并通过浅层网络预测材料属性变化方向。

Result: 实现了对粗糙度、金属感、透明度和发光等细粒度材料属性的参数化控制，并在定性和定量分析中表现出色。

Conclusion: MARBLE方法在材料编辑中具有高效性和灵活性，支持单次前向传播完成多次编辑，并适用于绘画场景。
...
### [205] [ProJo4D: Progressive Joint Optimization for Sparse-View Inverse Physics Estimation](https://arxiv.org/abs/2506.05317)
*Daniel Rho,Jun Myeong Choi,Biswadip Dey,Roni Sengupta*

Main category: cs.CV

TL;DR: ProJo4D提出了一种渐进式联合优化框架，用于从稀疏多视角视频中估计物理参数，解决了现有方法在4D场景理解中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在稀疏多视角视频下表现不佳，且顺序优化策略导致误差累积，限制了物理准确数字孪生等应用。

Method: ProJo4D采用渐进式联合优化，逐步增加优化参数集，最终联合优化几何、外观、物理状态和材料属性。

Result: 在PAC-NeRF和Spring-Gaus数据集上，ProJo4D在4D未来状态预测、新视角渲染和材料参数估计方面优于现有方法。

Conclusion: ProJo4D为物理基础的4D场景理解提供了有效解决方案，适用于机器人学和XR等领域。
...
### [206] [Does Your 3D Encoder Really Work? When Pretrain-SFT from 2D VLMs Meets 3D VLMs](https://arxiv.org/abs/2506.05318)
*Haoyuan Li,Yanpeng Zhou,Yufei Gao,Tao Tang,Jianhua Han,Yujie Yuan,Dave Zhenyu Chen,Jiawang Bian,Hang Xu,Xiaodan Liang*

Main category: cs.CV

TL;DR: 论文分析了3D视觉语言模型（VLMs）的性能差距，发现3D场景中心模型依赖3D编码器不足，并提出新数据集以改进3D理解。


<details>
  <summary>Details</summary>
Motivation: 探索3D VLMs性能低于2D和3D对象中心模型的原因，并提出改进方案。

Method: 分类3D VLMs架构，分析性能差距，并提出3D Relevance Discrimination QA数据集。

Result: 发现3D场景中心模型依赖3D编码器不足，数据扩展效果有限，并提出新数据集。

Conclusion: 需改进评估和策略以提升3D VLMs的3D场景理解能力。
...
### [207] [Revisiting Depth Representations for Feed-Forward 3D Gaussian Splatting](https://arxiv.org/abs/2506.05327)
*Duochao Shi,Weijie Wang,Donny Y. Chen,Zeyu Zhang,Jia-Wang Bian,Bohan Zhuang,Chunhua Shen*

Main category: cs.CV

TL;DR: 论文提出了一种基于点图的PM-Loss正则化损失，用于改进3D高斯泼溅（3DGS）中的深度图，解决深度不连续导致的点云稀疏问题，提升渲染质量。


<details>
  <summary>Details</summary>
Motivation: 深度图在3DGS中常用于生成3D点云，但深度不连续会导致点云稀疏或碎片化，影响渲染质量。
...
### [208] [AV-Reasoner: Improving and Benchmarking Clue-Grounded Audio-Visual Counting for MLLMs](https://arxiv.org/abs/2506.05328)
*Lidong Lu,Guo Chen,Zhiqi Li,Yicheng Liu,Tong Lu*

Main category: cs.CV

TL;DR: 论文提出CG-AV-Counting基准和AV-Reasoner模型，解决视频计数任务中的多模态挑战，并展示强化学习的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有视频理解模型在计数任务上表现不佳，且基准测试存在短视频、封闭查询、缺乏线索标注等问题。

Method: 提出CG-AV-Counting基准和AV-Reasoner模型，结合GRPO和课程学习提升计数能力。

Result: AV-Reasoner在多个基准测试中取得最优结果，但语言空间推理在域外测试中无效。

Conclusion: CG-AV-Counting为计数任务提供了全面测试平台，AV-Reasoner展示了强化学习的潜力，但需进一步优化域外性能。
...
### [209] [MINT-CoT: Enabling Interleaved Visual Tokens in Mathematical Chain-of-Thought Reasoning](https://arxiv.org/abs/2506.05331)
...
### [210] [Unleashing Hour-Scale Video Training for Long Video-Language Understanding](https://arxiv.org/abs/2506.05332)
*Jingyang Lin,Jialian Wu,Ximeng Sun,Ze Wang,Jiang Liu,Yusheng Su,Xiaodong Yu,Hao Chen,Jiebo Luo,Zicheng Liu,Emad Barsoum*

Main category: cs.CV

TL;DR: VideoMarathon是一个大规模的长视频指令跟随数据集，包含9700小时的长视频和330万高质量QA对，支持22种任务。基于此提出的Hour-LLaVA模型在多个长视频语言基准测试中表现最佳。


<details>
  <summary>Details</summary>
Motivation: 当前长视频多模态模型（Video-LMMs）的训练因缺乏高质量长视频标注数据而受限，VideoMarathon旨在填补这一空白。

Method: 提出VideoMarathon数据集，包含长视频和多样化的QA对；基于此开发Hour-LLaVA模型，利用内存增强模块实现1-FPS采样下的长视频训练和推理。

Result: Hour-LLaVA在多个长视频语言基准测试中表现最优，验证了数据集和模型的有效性。

Conclusion: VideoMarathon和Hour-LLaVA为长视频多模态理解提供了高质量数据和高效模型，推动了该领域的发展。
...
### [211] [VideoMolmo: Spatio-Temporal Grounding Meets Pointing](https://arxiv.org/abs/2506.05336)
*Ghazi Shazan Ahmad,Ahmed Heakl,Hanan Gani,Abdelrahman Shaker,Zhiqiang Shen,Ranjay Krishna,Fahad Shahbaz Khan,Salman Khan*

Main category: cs.CV

TL;DR: VideoMolmo是一种多模态大模型，结合文本描述实现细粒度时空定位，通过时序模块和掩码融合提升一致性，并在多个领域表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有视频方法缺乏上下文理解和泛化能力，需结合语言模型的推理能力。

Method: 基于Molmo架构，引入时序模块和双向点传播的掩码融合管道，分两步生成坐标和分割。

Result: 在多个基准测试中表现优异，显著提升时空定位精度和推理能力。
...
### [212] [Defurnishing with X-Ray Vision: Joint Removal of Furniture from Panoramas and Mesh](https://arxiv.org/abs/2506.05338)
...
TL;DR: 提出了一种从室内空间的多视角全景图像和纹理网格中生成无家具复制品的流程，通过简化网格和ControlNet修复技术实现高质量结果。


<details>
  <summary>Details</summary>
Motivation: 现有方法如神经辐射场或RGB-D修复在生成无家具场景时存在模糊或幻觉问题，需要一种更高质量的解决方案。

Method: 首先从网格中分割并移除家具，生成简化网格（SDM），提取边缘信息作为ControlNet修复的引导信号，最终修复全景图像并纹理化网格。

Result: 该方法生成的资产质量高于依赖神经辐射场或RGB-D修复的方法，避免了模糊和幻觉问题。

Conclusion: 提出的流程能有效生成高质量的无家具场景复制品，优于现有技术。
...
### [213] [Direct Numerical Layout Generation for 3D Indoor Scene Synthesis via Spatial Reasoning](https://arxiv.org/abs/2506.05341)
...
Method: DirectLayout通过三阶段生成（BEV布局、3D空间提升和对象放置优化），结合Chain-of-Thought激活和生成奖励，实现空间推理和规划。
...
### [214] [Refer to Anything with Vision-Language Prompts](https://arxiv.org/abs/2506.05342)
*Shengcao Cao,Zijun Wei,Jason Kuen,Kangning Liu,Lingzhi Zhang,Jiuxiang Gu,HyunJoon Jung,Liang-Yan Gui,Yu-Xiong Wang*

Main category: cs.CV

TL;DR: 论文提出了一种新的任务Omnimodal Referring Expression Segmentation (ORES)，通过文本或文本加视觉实体的任意提示生成一组掩码，并提出了RAS框架来解决这一任务。
...
Conclusion: RAS框架成功解决了ORES任务，并在多个任务中验证了其有效性。
...
### [215] [ContentV: Efficient Training of Video Generation Models with Limited Compute](https://arxiv.org/abs/2506.05343)
...
TL;DR: ContentV是一个8B参数的文本到视频模型，通过高效训练方法在4周内达到SOTA性能（VBench得分85.14）。


<details>
  <summary>Details</summary>
Motivation: 视频生成领域计算成本高，需要更高效的训练方法。

Method: 采用预训练图像模型复用、多阶段训练策略和低成本人类反馈强化学习框架。

Result: 生成多样高质量视频，支持多分辨率和时长。

Conclusion: ContentV展示了高效训练和高质量视频生成的潜力。
...
### [216] [SparseMM: Head Sparsity Emerges from Visual Concept Responses in MLLMs](https://arxiv.org/abs/2506.05344)
*Jiahui Wang,Zuyan Liu,Yongming Rao,Jiwen Lu*

Main category: cs.CV

TL;DR: 研究发现MLLMs中仅有约5%的注意力头对视觉理解有贡献，提出了一种无训练框架识别这些“视觉头”，并基于此设计了KV-Cache优化策略SparseMM，显著提升了推理效率。
...
Method: 通过无训练框架量化头的视觉相关性，设计SparseMM策略，为不同头分配非对称计算预算。

Result: SparseMM在主流多模态基准测试中实现了1.38倍实时加速和52%内存减少，同时保持性能。

Conclusion: SparseMM通过利用视觉头的稀疏性，显著提升了MLLMs的推理效率，为未来优化提供了新思路。
...
### [217] [Neural Inverse Rendering from Propagating Light](https://arxiv.org/abs/2506.05347)
*Anagh Malik,Benjamin Attal,Andrew Xie,Matthew O'Toole,David B. Lindell*

Main category: cs.CV

TL;DR: 首个基于物理的神经逆向渲染系统，通过多视角视频捕捉传播光线，利用时间分辨神经辐射缓存技术，实现高精度3D重建和光线传播合成。


<details>
  <summary>Details</summary>
Motivation: 解决在多视角视频中捕捉传播光线并进行逆向渲染的挑战，特别是在间接光强烈的情况下。

Method: 采用时间分辨神经辐射缓存技术，存储任意点从任意方向接收的无限反弹辐射，以加速逆向渲染。

Result: 实现了高精度的3D重建，支持光线传播合成、直接与间接光分解及多视角时间分辨重光照。

Conclusion: 该系统在复杂光照条件下表现出色，为3D重建和光线传播分析提供了新工具。
...
### [218] [FreeTimeGS: Free Gaussians at Anytime and Anywhere for Dynamic Scene Reconstruction](https://arxiv.org/abs/2506.05348)
...
### [219] [VideoMathQA: Benchmarking Mathematical Reasoning via Multimodal Understanding in Videos](https://arxiv.org/abs/2506.05349)
*Hanoona Rasheed,Abdelrahman Shaker,Anqi Tang,Muhammad Maaz,Ming-Hsuan Yang,Salman Khan,Fahad Khan*

Main category: cs.CV

TL;DR: VideoMathQA是一个评估模型在视频中跨模态数学推理能力的基准，涵盖10个数学领域，强调多模态和时序推理。


<details>
  <summary>Details</summary>
Motivation: 解决视频中数学推理的挑战，需要整合视觉、音频和文本信息，现有方法难以应对。

Method: 创建VideoMathQA基准，包含多样化的视频和问题类型，由专家标注，支持多步推理分析。

Result: 基准揭示了现有模型的局限性，并提供了系统评估框架。

Conclusion: VideoMathQA为跨模态时序推理模型提供了重要评估工具，推动相关研究发展。
...
### [220] [Contrastive Flow Matching](https://arxiv.org/abs/2506.05350)
*George Stoica,Vivek Ramanujan,Xiang Fan,Ali Farhadi,Ranjay Krishna,Judy Hoffman*

Main category: cs.CV

TL;DR: Contrastive Flow Matching (CFM) 通过对比目标增强条件流唯一性，提升生成质量与训练效率。


<details>
  <summary>Details</summary>
Motivation: 在条件设置下，传统流匹配无法保证流的唯一性，导致生成模糊。

Method: 引入对比目标，最大化不同条件流之间的差异。

Result: 实验显示CFM提升训练速度9倍，减少去噪步骤5倍，FID降低8.9。

Conclusion: CFM显著优化条件流匹配，适用于多种架构与任务。
...
### [221] [Zero-Shot Adaptation of Parameter-Efficient Fine-Tuning in Diffusion Models](https://arxiv.org/abs/2506.04244)
*Farzad Farhadzadeh,Debasmit Das,Shubhankar Borse,Fatih Porikli*

Main category: cs.AI

TL;DR: ProLoRA实现零样本适应，无需额外训练数据即可将预训练的低秩调整从源模型迁移到目标模型。


<details>
  <summary>Details</summary>
Motivation: 传统方法在切换基础模型时需要重新训练，但数据限制使其具有挑战性。ProLoRA旨在克服这一限制。

Method: 通过将源调整投影到目标模型的权重空间中，利用子空间和零空间相似性，并选择性针对对齐层。

Result: 在文本到图像模型上的评估表明，成功实现了知识迁移且性能可比，无需重新训练。

Conclusion: ProLoRA提供了一种高效的方法，避免了传统方法的局限性，实现了零样本适应。
...
### [222] [Contextual Integrity in LLMs via Reasoning and Reinforcement Learning](https://arxiv.org/abs/2506.04245)
...
TL;DR: 论文提出了一种方法，通过显式推理和强化学习框架，确保自主代理在决策时维护上下文完整性（CI），减少不适当的信息披露。


<details>
  <summary>Details</summary>
Motivation: 随着自主代理为用户决策的时代到来，确保上下文完整性（CI）成为核心问题，即代理需要明确推理其操作上下文。

Method: 首先通过提示LLMs显式推理CI，再开发强化学习框架进一步训练模型实现CI。使用包含700个多样上下文示例的合成数据集进行验证。

Result: 方法显著减少了不适当信息披露，同时保持任务性能，且改进可迁移至人类标注的CI基准测试（如PrivacyLens）。

Conclusion: 通过推理和强化学习结合，能够有效提升代理在复杂上下文中的信息共享决策能力。
...
### [223] [Language-Guided Multi-Agent Learning in Simulations: A Unified Framework and Evaluation](https://arxiv.org/abs/2506.04251)
*Zhengyang Li*
...
<details>
  <summary>Details</summary>
Motivation: 提升多智能体在模拟环境中的协调、通信和泛化能力，探索语言模型与强化学习的结合潜力。

Method: 提出LLM-MARL框架，包含Coordinator、Communicator和Memory模块，结合PPO训练和语言条件损失。

Result: 在Google Research Football等环境中，LLM-MARL在胜率、协调分数和零样本泛化上优于MAPPO和QMIX。

Conclusion: 通过结合语言模型与策略学习，为多智能体系统的智能协作提供了新思路，适用于游戏、训练和人机协作。
...
### [224] [A Graph-Retrieval-Augmented Generation Framework Enhances Decision-Making in the Circular Economy](https://arxiv.org/abs/2506.04252)
...
TL;DR: CircuGraphRAG是一个基于知识图谱的检索增强生成框架，用于提升大语言模型在循环经济领域的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在可持续制造中常产生错误工业代码和排放因子，影响决策。

Method: 结合领域知识图谱，将自然语言查询转换为SPARQL，检索验证子图以确保准确性。

Result: 在单跳和多跳问答中表现优异，ROUGE-L F1达1.0，效率提升50%，令牌使用减少16%。
...
### [225] [HADA: Human-AI Agent Decision Alignment Architecture](https://arxiv.org/abs/2506.04253)
*Tapio Pitkäranta,Leena Pitkäranta*

Main category: cs.AI

TL;DR: HADA是一个框架无关的参考架构，用于确保LLM代理和传统算法与组织目标和价值观对齐。它通过角色特定的利益相关者代理（如业务、数据科学、审计等）提供对话式API，支持查询、调整和审计决策。
...
Method: HADA通过角色特定的利益相关者代理包装算法或LLM，提供对话式API，支持自然语言表达对齐目标、KPI和约束条件，并在云原生环境中实现。

Result: 通过五个零售银行场景的测试，HADA实现了六个预定义目标的完全覆盖，包括对话控制、KPI追踪、偏见检测和决策溯源。

Conclusion: HADA为多代理系统中的人机对齐提供了开源架构和设计理论，实证表明其能提升决策管道的准确性、透明性和伦理合规性。
...
### [226] [Automated Skill Discovery for Language Agents through Exploration and Iterative Feedback](https://arxiv.org/abs/2506.04287)
...
<details>
  <summary>Details</summary>
Motivation: 训练LLM代理以获取技能并执行多样化任务面临数据集生成的挑战，如人工收集成本高或LLM直接生成的任务不可行。

Method: 采用探索优先策略，通过探索代理（Alice）生成可行技能数据集，并利用反馈循环迭代优化目标代理（Bob）的能力。

Result: 在Webshop和Crafter上的实验显示，EXIF能有效发现技能并提升代理性能，且Alice与Bob同模型时效果更佳。

Conclusion: EXIF框架为LLM代理的无监督技能学习提供了有效解决方案，展示了自进化系统的潜力。
...
### [227] [A Statistical Physics of Language Model Reasoning](https://arxiv.org/abs/2506.04374)
*Jack David Carson,Amir Reisizadeh*

Main category: cs.AI

TL;DR: 论文提出了一种统计物理框架，用于模拟Transformer语言模型的连续时间推理动态，通过低维流形上的随机动力学系统捕捉推理阶段，并识别了四种潜在推理状态。


<details>
  <summary>Details</summary>
Motivation: Transformer语言模型展现出难以机制化理解的推理能力，因此需要一种框架来模拟和分析其推理动态。

Method: 使用统计物理学方法，将句子级隐藏状态轨迹建模为低维流形上的随机动力学系统，并引入潜在状态切换以捕捉推理阶段。

Result: 通过8个模型和7个基准测试，发现40维投影能解释约50%的方差，并识别出四种潜在推理状态。

Conclusion: 该框架支持低成本推理模拟，为研究语言模型的关键转变（如推理失败）提供了工具。
...
### [228] [Matter-of-Fact: A Benchmark for Verifying the Feasibility of Literature-Supported Claims in Materials Science](https://arxiv.org/abs/2506.04410)
...
Method: 构建包含8.4k科学文献提取的假设的数据集，测试检索增强生成和代码生成模型的性能。

Result: 当前模型性能不超过72%（随机为50%），但专家认为大多数问题可解。
...
### [229] [Plugging Schema Graph into Multi-Table QA: A Human-Guided Framework for Reducing LLM Reliance](https://arxiv.org/abs/2506.04427)
*Xixi Wang,Miguel Costa,Jordanka Kovaceva,Shuai Wang,Francisco C. Pereira*

Main category: cs.AI

TL;DR: 提出了一种基于图的框架，利用人工整理的关系知识来解决多表问答中的模式链接问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在复杂真实场景中表现不佳，无法可靠地跨多表进行模式链接。

Method: 使用图结构显式编码模式链接和连接路径，结合剪枝和子路径合并策略构建可解释的推理链。

Result: 在标准基准和真实大规模数据集上验证了方法的有效性。

Conclusion: 这是首个应用于复杂工业表格数据的多表问答系统。
...
### [230] [An AI-Based Public Health Data Monitoring System](https://arxiv.org/abs/2506.04429)
*Ananya Joshi,Nolan Gormley,Richa Gadgil,Tina Townes,Roni Rosenfeld,Bryan Wilder*

Main category: cs.AI

TL;DR: 提出了一种基于排名的监控范式，利用AI异常检测方法，显著提升了公共卫生数据监控效率。


<details>
  <summary>Details</summary>
Motivation: 传统基于警报的监控系统难以应对现代公共卫生数据监控的需求，如阈值重置和数据量过大导致的延迟。

Method: 采用基于排名的监控范式，结合AI异常检测技术，并与国家机构合作部署。

Result: 系统每日监控500万数据点，三个月评估显示审核速度效率提升54倍。
...
### [231] [Matching Markets Meet LLMs: Algorithmic Reasoning with Ranked Preferences](https://arxiv.org/abs/2506.04478)
*Hadi Hosseini,Samarth Khanna,Ronak Singh*

Main category: cs.AI

TL;DR: 论文研究了大型语言模型（LLMs）在匹配市场中的表现，发现其在处理排名偏好和组合领域任务时存在局限性，尤其是解决大规模市场的不稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在匹配市场等需要处理排名偏好和结构化算法的组合领域中的能力，填补现有研究的空白。

Method: 评估多个先进模型在偏好推理任务上的表现，包括稳定匹配生成、不稳定性检测与解决等，并尝试参数高效微调（LoRA）。

Result: 发现即使是高性能模型也难以解决大规模市场的不稳定性问题，LoRA在小市场中表现良好，但在大市场中效果有限。
...
### [232] [CogMath: Assessing LLMs' Authentic Mathematical Ability from a Human Cognitive Perspective](https://arxiv.org/abs/2506.04481)
*Jiayu Liu,Zhenya Huang,Wei Dai,Cheng Cheng,Jinze Wu,Jing Sha,Song Li,Qi Liu,Shijin Wang,Enhong Chen*

Main category: cs.AI

TL;DR: CogMath提出了一种基于人类认知的评估框架，通过三个阶段和九个维度全面评估大语言模型（LLMs）的数学能力，发现现有评估高估了主流LLMs的能力30%-40%。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法仅依赖答案准确性，无法全面衡量LLMs的真实数学能力，因此需要更细致的评估框架。

Method: CogMath将人类推理过程分为问题理解、问题解决和解决方案总结三个阶段，设计了九个细粒度评估维度，并采用多智能体系统生成评估问题。

Result: 应用CogMath评估7种主流LLMs，发现其数学能力被高估30%-40%，并揭示了其在各阶段/维度的优缺点。
...
### [233] ["Don't Do That!": Guiding Embodied Systems through Large Language Model-based Constraint Generation](https://arxiv.org/abs/2506.04500)
...
<details>
  <summary>Details</summary>
Motivation: 解决复杂自然语言约束难以形式化描述的问题，避免直接推理和幻觉。

Method: 利用LLM的编码能力将约束转换为Python函数，应用于点云表示和传统搜索算法。

Result: 在Gazebo模拟环境中验证了STPR的高效性和约束合规性，且适用于小型代码专用LLM。

Conclusion: STPR提供了一种高效、透明的方法，适用于多种紧凑模型。
...
### [234] [Schema Generation for Large Knowledge Graphs Using Large Language Models](https://arxiv.org/abs/2506.04512)
*Bohui Zhang,Yuan He,Lydia Pintscher,Albert Meroño Peñuela,Elena Simperl*

Main category: cs.AI

TL;DR: 论文探讨了利用大语言模型（LLMs）自动生成语义网和自然语言处理中的数据模式（schemas），以减少对知识工程师和领域专家的依赖。
...
Conclusion: 该研究不仅展示了LLMs在模式生成中的潜力，还为结构化生成任务设定了新挑战。
...
### [235] [OpenAg: Democratizing Agricultural Intelligence](https://arxiv.org/abs/2506.04571)
*Srikanth Thudumu,Jason Fisher*

Main category: cs.AI

TL;DR: OpenAg是一个农业人工智能框架，结合领域知识、知识图谱和多智能体推理，提供可解释且实用的农业决策支持。


<details>
  <summary>Details</summary>
Motivation: 当前农业智能系统缺乏上下文理解和适应性，通用大语言模型无法满足农业领域的实际需求。

Method: OpenAg整合领域基础模型、神经知识图谱、多智能体推理和因果解释性，构建统一农业知识库。
...
### [236] [Judicial Permission](https://arxiv.org/abs/2506.04610)
*Guido Governatori,Antonino Rotolo*

Main category: cs.AI

TL;DR: 本文探讨了刑事审判中弱许可（司法许可）的重要性，并通过对话游戏模型系统性地分析了司法许可，考虑了不同的证明标准和论证语义。


<details>
  <summary>Details</summary>
Motivation: 研究司法许可在刑事审判中的作用，以填补现有法律理论中对弱许可系统性研究的空白。

Method: 引入对话游戏模型，结合不同的证明标准和论证语义，对司法许可进行系统性分析。

Result: 提出了一个能够有效处理司法许可的框架，为法律实践提供了理论支持。

Conclusion: 弱许可在刑事审判中具有重要意义，对话游戏模型为司法许可的研究提供了新的视角和方法。
...
### [237] [DeePoly: A High-Order Accuracy and Efficiency Deep-Polynomial Framework for Scientific Machine Learning](https://arxiv.org/abs/2506.04613)
*Li Liu,Heng Yong*
...
<details>
  <summary>Details</summary>
Motivation: 传统DNN方法在求解偏微分方程时缺乏收敛保证和计算效率，需要一种更高效且理论可靠的方法。

Method: DeePoly采用两阶段方法：先用DNN提取全局特征，再用线性空间优化结合DNN特征和多项式基函数。

Result: 理论分析和实验表明，该方法显著提高了精度和效率，同时保持无网格和无格式特性。

Conclusion: DeePoly框架为开源项目提供了理论基础，展示了其在科学计算中的潜力。
...
### [238] [Look Before You Leap: A GUI-Critic-R1 Model for Pre-Operative Error Diagnosis in GUI Automation](https://arxiv.org/abs/2506.04614)
...
TL;DR: 该论文提出了一种用于GUI自动化的预操作批评机制（GUI-Critic-R1），通过S-GRPO策略和推理引导的数据收集方法，显著提高了模型的反馈可靠性和自动化任务的成功率。


<details>
  <summary>Details</summary>
Motivation: GUI自动化任务需要实时决策且容错率低，现有MLLMs在错误累积时可能导致不可逆后果，因此需要一种机制在操作前提供有效反馈。

Method: 提出S-GRPO策略构建预操作批评模型GUI-Critic-R1，引入建议奖励增强反馈可靠性，并开发推理引导的数据收集方法创建训练和测试数据集。

Result: 静态实验显示GUI-Critic-R1在批评准确性上优于现有MLLMs；动态评估表明其在GUI自动化任务中提高了成功率和效率。

Conclusion: GUI-Critic-R1通过预操作批评机制显著提升了GUI自动化任务的可靠性和效率，填补了相关数据空白。
...
### [239] [CHANCERY: Evaluating corporate governance reasoning capabilities in language models](https://arxiv.org/abs/2506.04636)
...
<details>
  <summary>Details</summary>
Motivation: 现有法律数据集未专门针对推理任务，而法律实践中推理能力至关重要。

Method: 基于真实公司治理法律，构建包含章程和提案的二元分类任务，并评估SOTA模型的性能。

Result: Claude 3.7 Sonnet和GPT-4o准确率分别为64.5%和75.2%，推理代理表现更优（ReAct 76.1%，CodeAct 78.1%）。

Conclusion: CHANCERY揭示了SOTA模型在法律推理上的局限性，为未来研究提供了方向。
...
### [240] [Agents of Change: Self-Evolving LLM Agents for Strategic Planning](https://arxiv.org/abs/2506.04651)
*Nikolas Belle,Dakota Barnes,Alfonso Amayuelas,Ivan Bercovich,Xin Eric Wang,William Wang*

Main category: cs.AI

TL;DR: 研究探讨了LLM代理在战略规划挑战中的自我改进能力，通过《卡坦岛》游戏测试，发现自进化代理优于静态基线。
...
Method: 使用《卡坦岛》游戏框架，构建多角色代理架构（分析员、研究员、编码员、玩家），通过迭代分析、研究和修改代理逻辑或提示。

Result: 自进化代理（如Claude 3.7和GPT-4o驱动）在策略适应和表现上优于静态代理。

Conclusion: LLM代理能够通过自我改进在战略任务中取得更好表现，展示了自适应推理的潜力。
...
### [241] [E-bike agents: Large Language Model-Driven E-Bike Accident Analysis and Severity Prediction](https://arxiv.org/abs/2506.04654)
...
<details>
  <summary>Details</summary>
Motivation: 电动自行车（e-bikes）使用增加导致事故频发，但事故报告多为非结构化文本，阻碍定量安全分析。

Method: 研究设计了四个LLM代理，分别处理数据分类、信息提取、伤害原因确定和组件关联，并采用有序逻辑模型分析事故严重性与提取因素的关系。
...
### [242] [Empowering Economic Simulation for Massively Multiplayer Online Games through Generative Agent-Based Modeling](https://arxiv.org/abs/2506.04699)
...
TL;DR: 该论文提出了一种基于大型语言模型（LLMs）的新方法，用于模拟MMO游戏中的经济行为，解决了现有研究中代理可靠性、社交性和可解释性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的代理模型在模拟人类经济行为时存在局限性，尤其是在代理的可靠性、社交性和可解释性方面。

Method: 利用LLMs的角色扮演能力、生成能力和推理能力，设计了具有人类决策和适应能力的代理，具备角色扮演、感知、记忆和推理功能。

Result: 模拟实验表明，LLM驱动的代理能够促进游戏中角色专业化和价格波动等市场规则下的涌现现象。

Conclusion: LLM驱动的代理为MMO经济模拟提供了一种更接近人类行为的新方法，具有潜在的应用价值。
...
### [243] [Beyond Accuracy: Dissecting Mathematical Reasoning for LLMs Under Reinforcement Learning](https://arxiv.org/abs/2506.04723)
*Jiayu Wang,Yifei Ming,Zixuan Ke,Caiming Xiong,Shafiq Joty,Aws Albarghouthi,Frederic Sala*

Main category: cs.AI

TL;DR: 本文通过细粒度分析框架研究了强化学习（RL）对语言模型推理能力的影响，重点关注计划执行、问题分解和知识利用，发现RL提升了模型的内部策略制定和知识整合能力。


<details>
  <summary>Details</summary>
Motivation: 尽管RL训练方法（如GRPO）在语言模型中表现出显著优势，但其具体优势尚未被深入理解。本文旨在填补这一空白。

Method: 引入细粒度分析框架，研究RL对计划执行、问题分解和知识利用的影响，并通过实验验证。

Result: RL训练使模型在内部策略制定和知识整合方面表现更优，但对显式计划执行的效果有限。

Conclusion: RL主要提升模型的内部推理能力而非外部计划执行，为推理模型的训练和评估提供了新方向。
...
### [244] [Evaluation is All You Need: Strategic Overclaiming of LLM Reasoning Capabilities Through Evaluation Design](https://arxiv.org/abs/2506.04734)
...
<details>
  <summary>Details</summary>
Motivation: 评估结果的波动性使得模型性能改进难以可靠复现，需更严谨的评估方法。

Method: 对Deepseek-R1-Distill系列模型进行实证评估。

Result: 评估结果受细微条件差异影响显著，类似现象也出现在其他相关模型中。
...
### [245] [Ontology-based knowledge representation for bone disease diagnosis: a foundation for safe and sustainable medical artificial intelligence systems](https://arxiv.org/abs/2506.04756)
...
<details>
  <summary>Details</summary>
Motivation: 医学AI系统缺乏系统的领域专业知识整合，可能影响诊断可靠性。

Method: 开发了基于本体的框架，包括层次神经网络、VQA系统和多模态模型，整合了影像、临床和实验室数据。
...
### [246] [Safe Planning and Policy Optimization via World Model Learning](https://arxiv.org/abs/2506.04828)
*Artem Latyshev,Gregory Gorbov,Aleksandr I. Panov*

Main category: cs.AI

TL;DR: 提出了一种新型基于模型的强化学习框架，联合优化任务性能和安全性，通过自适应机制动态切换规划与执行，解决了传统方法的目标不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 现实中的强化学习应用需优先考虑安全性和可靠性，但基于模型的方法因模型不准确可能导致灾难性失败。

Method: 结合隐式世界模型和动态安全阈值，动态切换模型规划与直接策略执行，优化安全与性能。

Result: 实验显示，该方法在安全关键任务中显著优于非自适应方法，同时优化了安全性和性能。
...
### [247] [Towards a Multi-Agent Simulation of Cyber-attackers and Cyber-defenders Battles](https://arxiv.org/abs/2506.04849)
...
<details>
  <summary>Details</summary>
Motivation: 随着网络攻击日益复杂和协同化，多智能体防御策略成为关键。本文旨在提供一个实验框架，模拟真实网络攻击场景并评估防御动态组织。

Method: 通过马尔可夫模型和模拟器实现网络节点和智能体的抽象建模，智能体的行为影响网络状态和属性变化，集体行动决定攻击与防御目标的实现。

Result: 模拟器实现了真实场景，测试了多种攻击与防御行为策略，展示了动态对抗的效果。

Conclusion: 该方法为网络防御策略的动态评估提供了有效工具，未来可扩展至更复杂的场景。
...
### [248] [LLMs for sensory-motor control: Combining in-context and iterative learning](https://arxiv.org/abs/2506.04867)
...
Method: LLMs首先生成控制策略，随后通过性能反馈和感知运动数据迭代优化策略。

Result: 在Gymnasium和MuJoCo库的控制任务中验证，多数情况下能找到最优或高性能解决方案。
...
TL;DR: 论文研究了大型语言模型（LLMs）的战略欺骗行为，提出了一种检测和控制方法，并展示了高准确率的检测和诱导欺骗的能力。


<details>
  <summary>Details</summary>
Motivation: 随着具备链式思维（CoT）推理能力的LLMs发展，其可能故意欺骗人类，这种战略欺骗与传统幻觉问题不同，需要专门研究。

Method: 采用表示工程方法，通过线性人工断层扫描（LAT）提取“欺骗向量”，并利用激活导向技术诱导和控制欺骗行为。

Result: 实验实现了89%的欺骗检测准确率，并在无明确提示下成功诱导40%的上下文相关欺骗。

Conclusion: 研究揭示了推理模型的诚实性问题，并为可信AI对齐提供了工具。
...
### [250] [Differentiable Logic Cellular Automata: From Game of Life to Pattern Generation](https://arxiv.org/abs/2506.04912)
*Pietro Miotti,Eyvind Niklasson,Ettore Randazzo,Alexander Mordvintsev*

Main category: cs.AI

TL;DR: 论文提出DiffLogic CA，结合神经细胞自动机和可微分逻辑门网络，实现端到端训练并保持离散状态空间，成功应用于多种模式生成任务。


<details>
  <summary>Details</summary>
Motivation: 结合可微分逻辑门网络和神经细胞自动机，探索局部更新规则的学习，同时保留离散特性，为可编程物质和鲁棒计算系统提供新思路。

Method: 使用可微分逻辑门构建电路，实现端到端梯度训练，推理时在离散状态空间运行，学习细胞自动机的局部更新规则。

Result: 成功学习Conway生命游戏规则、生成抗噪声棋盘图案、蜥蜴形状和多色图案，展示了模型的泛化能力和鲁棒性。
...
### [251] [Energentic Intelligence: From Self-Sustaining Systems to Enduring Artificial Life](https://arxiv.org/abs/2506.04916)
*Atahan Karagoz*

Main category: cs.AI

TL;DR: 论文提出了一种名为“能量智能”的自主系统，其核心目标是通过内部能量调节维持自身运行，而非任务表现。


<details>
  <summary>Details</summary>
Motivation: 传统奖励驱动的范式无法适应资源波动环境，因此需要一种以生存为中心的新型自主系统。

Method: 通过能量驱动的效用函数和生存约束的生存期，提出了一种模块化架构，整合能量收集、温度调节和自适应计算。

Result: 模拟环境中展示了无需外部监督的稳定、资源感知行为。

Conclusion: 为资源波动环境中的自主代理提供了理论和架构基础。
...
### [252] [Mathematical Reasoning for Unmanned Aerial Vehicles: A RAG-Based Approach for Complex Arithmetic Reasoning](https://arxiv.org/abs/2506.04998)
...
<details>
  <summary>Details</summary>
Motivation: 传统无人机控制依赖硬编码方程，而LLMs在数学推理中存在选择和应用正确公式的困难，需改进其可靠性。

Method: 提出RAG-UAV框架，结合检索增强生成技术，利用领域文献提升LLMs的数学推理能力，并引入UAV-Math-Bench进行测试。

Result: 实验显示RAG显著提升准确率（最高75%），减少错误公式选择（从25%降至5%），并大幅降低数值误差。

Conclusion: RAG能增强LLMs在工程分析中的可靠性，但实时飞行控制仍需进一步研究。
...
### [253] [Truly Self-Improving Agents Require Intrinsic Metacognitive Learning](https://arxiv.org/abs/2506.05109)
*Tennison Liu,Mihaela van der Schaar*

Main category: cs.AI

TL;DR: 论文提出了一种基于内在元认知学习的自改进代理框架，以解决现有方法在泛化性和扩展性上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有自改进代理的自我改进过程僵化，难以跨任务泛化且无法随能力提升而扩展，需要引入内在元认知学习。

Method: 提出一个包含元认知知识、元认知规划和元认知评估的正式框架，并分析现有代理依赖外在元认知机制的问题。

Result: 发现许多内在元认知的要素已存在，但需解决人类与代理间元认知责任分配及评估改进的挑战。

Conclusion: 内在元认知学习是实现持续、泛化和对齐的自改进的关键，未来需解决相关挑战。
...
### [254] [LLM-First Search: Self-Guided Exploration of the Solution Space](https://arxiv.org/abs/2506.05213)
*Nathan Herr,Tim Rocktäschel,Roberta Raileanu*

Main category: cs.AI

TL;DR: 论文提出了一种名为LLM-First Search (LFS)的新方法，通过让大型语言模型自主控制搜索过程，无需预定义搜索策略，提高了推理和规划的灵活性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法如MCTS依赖固定的探索超参数，难以适应不同难度的任务，限制了其实际应用。

Method: LFS利用LLM自主评估搜索路径，决定是否继续当前路径或探索其他分支，无需外部启发式或硬编码策略。

Result: 在Countdown和Sudoku任务中，LFS表现优于ToT-BFS、BestFS和MCTS，尤其在更具挑战性的任务中表现更优，计算效率更高，且能更好地利用更强的模型和计算资源。

Conclusion: LFS通过LLM自主控制搜索，实现了更灵活、高效的推理，无需手动调整，适用于多种任务。
...
### [255] [Just Enough Thinking: Efficient Reasoning with Adaptive Length Penalties Reinforcement Learning](https://arxiv.org/abs/2506.05256)
*Violet Xiang,Chase Blagden,Rafael Rafailov,Nathan Lile,Sang Truong,Chelsea Finn,Nick Haber*

Main category: cs.AI

TL;DR: ALP（自适应长度惩罚）通过强化学习调整生成长度，根据问题难度动态优化计算资源，减少50%的平均token使用，同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 解决大型推理模型在推理时因生成过多token而浪费计算资源的问题，现有方法需要数据标注或统一惩罚，无法区分问题难度。

Method: 提出ALP，通过强化学习监控每个提示的在线解决率，动态调整惩罚力度，简单问题高惩罚，困难问题低惩罚。

Result: ALP在DeepScaleR-1.5B上减少50%的平均token使用，同时保持性能，并在困难问题上表现更好。

Conclusion: ALP能智能分配计算资源，提升困难问题的准确性，同时显著减少计算成本。
...
### [256] [Control Tax: The Price of Keeping AI in Check](https://arxiv.org/abs/2506.05296)
*Mikhail Terekhov,Zhen Ning David Liu,Caglar Gulcehre,Samuel Albanie*

Main category: cs.AI

TL;DR: 论文提出“控制税”概念，量化AI控制措施的成本，并开发框架连接安全保证与成本，优化监控策略。
...
Method: 引入控制税理论框架，评估语言模型在对抗性环境中的表现，并提供成本估算与优化监控策略。

Result: 框架能系统化评估安全保证与成本，优化监控策略，平衡安全性与经济性。

Conclusion: 通过经济可行性评估，论文推动了AI控制领域的实际应用。
...
### [257] [A Comprehensive Survey on the Risks and Limitations of Concept-based Models](https://arxiv.org/abs/2506.04237)
*Sanchit Sinha,Aidong Zhang*

Main category: cs.LG

TL;DR: 本文综述了基于概念的模型（Concept-based Models）的风险与局限性，包括结构、训练假设及对抗性漏洞等问题，并探讨了改进其可靠性的方法。


<details>
  <summary>Details</summary>
Motivation: 基于概念的模型因其可解释性在敏感领域受到关注，但其局限性如概念泄漏、纠缠表示等影响了可靠性，需系统研究。

Method: 通过综合调查，总结了监督和无监督范式下的常见挑战及架构选择，并分析了提升可靠性的最新进展。

Result: 揭示了模型在概念泄漏、鲁棒性等方面的不足，提出了改进方向。

Conclusion: 未来研究需解决开放问题，如人类干预的有效性，以推动基于概念的模型的实际应用。
...
### [258] [Improving Out-of-Distribution Detection with Markov Logic Networks](https://arxiv.org/abs/2506.04241)
*Konstantin Kirchheim,Frank Ortmeier*

Main category: cs.LG

TL;DR: 该论文提出了一种基于马尔可夫逻辑网络（MLN）的方法，用于增强现有OOD检测器的性能，并通过实验验证了其有效性和计算效率。
...
Method: 利用MLN将一阶逻辑与概率推理结合，通过加权逻辑约束为输入分配概率，提升可解释性。同时提出了一种学习逻辑约束的简单算法。

Result: 实验表明，MLN能显著提升多种现有OOD检测器的性能，同时保持计算效率。

Conclusion: MLN为OOD检测提供了一种可解释且高效的解决方案，并通过学习算法进一步增强了实用性。
...
### [259] [Triple Attention Transformer Architecture for Time-Dependent Concrete Creep Prediction](https://arxiv.org/abs/2506.04243)
*Warayut Dokduea,Weerachart Tangchirapat,Sompote Youwai*

Main category: cs.LG

TL;DR: 提出了一种新颖的三重注意力Transformer架构，用于预测时间依赖性混凝土徐变，显著优于传统方法。
...
Method: 将混凝土徐变预测转化为自回归序列建模任务，利用Transformer的自注意力机制捕捉历史徐变模式的长程依赖。
...
### [260] [SafeSteer: Interpretable Safety Steering with Refusal-Evasion in LLMs](https://arxiv.org/abs/2506.04250)
*Shaona Ghosh,Amrita Bhattacharjee,Yftah Ziser,Christopher Parisien*

Main category: cs.LG

TL;DR: SafeSteer是一种通过潜变量激活引导实现LLM安全调整的方法，无需微调，支持精确控制且保持文本质量。


<details>
  <summary>Details</summary>
Motivation: 由于微调LLM以适应安全政策成本高且不切实际，研究探索了更灵活、低成本的方法。

Method: 利用类别特定的引导向量，采用无梯度无监督方法，避免对比数据需求，实现安全引导。

Result: 方法在多种LLM、数据集和风险类别中有效，能精确控制并避免全面拒绝。

Conclusion: SafeSteer简单有效，支持精确安全调整且保持文本相关性，优于复杂方法。
...
### [261] [Localized Forest Fire Risk Prediction: A Department-Aware Approach for Operational Decision Support](https://arxiv.org/abs/2506.04254)
*Nicolas Caron,Christophe Guyeux,Hassan Noura,Benjamin Aynes*

Main category: cs.LG

TL;DR: 本文提出了一种针对法国各部门的森林火灾风险评估方法，通过考虑局部条件改进了传统的二元分类方法，并提供了首个国家级AI基准测试。
...
Method: 提出了一种考虑部门地形、气候条件和历史经验的区域特异性风险评估方法。

Result: 提供了法国首个国家级AI基准测试，展示了区域特异性预测的可行性。

Conclusion: 该方法为火灾风险评估提供了更实用的解决方案，并指出了未来研究方向。
...
### [262] [MUC-G4: Minimal Unsat Core-Guided Incremental Verification for Deep Neural Network Compression](https://arxiv.org/abs/2506.04268)
*Jingyang Li,Guoqiang Li*

Main category: cs.LG

TL;DR: MUC-G4是一种增量验证框架，用于压缩深度神经网络，通过SMT公式编码和最小不可满足核心（MUC）指导验证，显著提升验证效率。
...
Method: MUC-G4将原始和压缩网络编码为SMT公式，分类变化，并利用原始网络的MUC指导压缩网络的高效验证。
...
### [263] [Understanding the Impact of Sampling Quality in Direct Preference Optimization](https://arxiv.org/abs/2506.04272)
*Kyung Rok Kim,Yumo Bai,Chonghuan Wang,Guanting Chen*

Main category: cs.LG

TL;DR: 研究了采样分布在直接偏好优化（DPO）中的作用，发现其支持与质量影响DPO的训练动态。
...
Method: 分析响应分布对梯度下降中策略更新的影响，设计简化对齐模型并量化结果。

Result: 高质量响应频率高时，梯度信号增强，优化景观改善，策略学习更有效。

Conclusion: 理论分析与实验支持在线DPO框架的合理性。
...
### [264] [SF$^2$Bench: Evaluating Data-Driven Models for Compound Flood Forecasting in South Florida](https://arxiv.org/abs/2506.04281)
*Xu Zheng,Chaohao Lin,Sipeng Chen,Zhuomin Chen,Jimeng Shi,Wei Cheng,Jayantha Obeysekera,Jason Liu,Dongsheng Luo*

Main category: cs.LG

TL;DR: 论文介绍了SF2Bench数据集，用于综合分析复合洪水，并评估了六种机器学习方法的性能。


<details>
  <summary>Details</summary>
Motivation: 复合洪水预测因气象、水文和海洋因素的复杂交互而具有挑战性，且现有数据不足。

Method: 提出SF2Bench数据集，整合潮汐、降雨、地下水和人类管理活动，并评估六类机器学习方法。

Result: 实验验证了不同特征对洪水预测的影响，揭示了各类方法在时空依赖性上的表现差异。

Conclusion: SF2Bench数据集和方法评估为改进复合洪水预测提供了新思路。
...
### [265] [DrSR: LLM based Scientific Equation Discovery with Dual Reasoning from Data and Experience](https://arxiv.org/abs/2506.04282)
*Runxiang Wang,Boxiao Wang,Kai Li,Yifan Zhang,Jian Cheng*

Main category: cs.LG

TL;DR: DrSR框架通过结合数据驱动洞察和反思学习，提升符号回归的鲁棒性和发现能力，显著优于传统和基于LLM的方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的符号回归方法（如LLM-SR）过度依赖内部先验，缺乏数据理解和系统反思，限制了其性能。

Method: DrSR结合数据驱动的结构关系分析和反思学习，通过闭环反馈优化方程生成。

Result: 实验显示DrSR在多个学科数据集中显著提高了有效方程率，并在准确性、泛化性和搜索效率上优于其他方法。

Conclusion: DrSR为科学方程发现提供了高效且可靠的解决方案。
...
### [266] [Training-free AI for Earth Observation Change Detection using Physics Aware Neuromorphic Networks](https://arxiv.org/abs/2506.04285)
*Stephen Smith,Cormac Purcell,Zdenka Kuncic*

Main category: cs.LG

TL;DR: 论文提出了一种基于物理感知神经形态网络（PANN）的方法，用于从多光谱卫星图像中检测自然灾害引起的变化，并生成变化图，以优先传输相关数据。


<details>
  <summary>Details</summary>
Motivation: 解决卫星数据传输带宽受限的问题，通过机载处理优先传输关键数据，以支持自然灾害的快速响应。

Method: 利用基于忆阻器的物理感知神经形态网络（PANN），动态更新权重，生成物理约束的动态输出特征，通过距离度量检测变化。

Result: PANN在自然灾害检测任务中表现优于或与最先进的AI模型相当，且无需训练，计算资源需求低。
...
### [267] [Backbone Augmented Training for Adaptations](https://arxiv.org/abs/2506.04288)
*Jae Wan Park,Junhyeok Kim,Youngjun Jun,Hyunah Ko,Seong Jae Hwang*

Main category: cs.LG

TL;DR: 论文提出了一种名为Backbone Augmented Training (BAT)的方法，利用主干数据增强适应数据集，解决了适应数据不足的问题，并提出了ALBAT算法。
...
Method: 提出BAT方法，利用主干数据增强适应数据，并通过数学命题验证其有效性；进一步提出ALBAT算法实现高效增强。

Result: ALBAT在数据稀缺的个性化和语言生成任务中显著提升了适应训练效果。

Conclusion: BAT和ALBAT为适应数据不足的问题提供了有效解决方案，具有实际应用潜力。
...
### [268] [Relational reasoning and inductive bias in transformers trained on a transitive inference task](https://arxiv.org/abs/2506.04289)
*Jesse Geerts,Stephanie Chan,Claudia Clopath,Kimberly Stachenfeld*

Main category: cs.LG

TL;DR: 论文研究了Transformer模型在关系推理任务中的表现，比较了两种学习机制（IWL和ICL）在传递推理任务中的差异，发现IWL更易推广传递推理，而ICL需预训练才能实现类似效果。


<details>
  <summary>Details</summary>
Motivation: 探讨Transformer模型在不同学习机制下进行关系推理的机制，尤其是传递推理任务的表现。

Method: 比较了IWL（参数学习）和ICL（上下文学习）两种机制在传递推理任务中的表现，并通过机制分析揭示其差异。

Result: IWL模型能自然推广传递推理，而ICL模型需预训练才能实现类似效果，且预训练任务的结构有助于关系推理能力的提升。

Conclusion: 预训练任务的结构性对提升Transformer模型的上下文关系推理能力至关重要。
...
### [269] [A Lyapunov Drift-Plus-Penalty Method Tailored for Reinforcement Learning with Queue Stability](https://arxiv.org/abs/2506.04291)
...
Method: 通过理论分析，提出LDPTRLQ算法，有效结合Lyapunov漂移加惩罚算法的贪婪优化与强化学习的长期视角。

Result: 仿真结果表明，LDPTRLQ在性能和稳定性上优于基线方法。

Conclusion: LDPTRLQ算法在理论和实验上均表现出色，为IoT优化问题提供了有效解决方案。
...
### [270] [AUTOCT: Automating Interpretable Clinical Trial Prediction with LLM Agents](https://arxiv.org/abs/2506.04293)
...
Result: 实验表明，AutoCT在有限的自优化迭代中性能达到或优于现有方法。
...
### [271] [Short-Term Power Demand Forecasting for Diverse Consumer Types to Enhance Grid Planning and Synchronisation](https://arxiv.org/abs/2506.04294)
*Asier Diaz-Iglesias,Xabier Belaunzaran,Ane M. Florez-Tapia*

Main category: cs.LG

TL;DR: 论文提出了一种针对不同消费者群体（工业、商业、居民）的电力需求预测方法，通过聚类和特征选择优化预测模型，结合AI和机器学习算法，显著提高了预测准确性。


<details>
  <summary>Details</summary>
Motivation: 随着可再生能源的普及，电网稳定性依赖于精确的电力需求预测。传统的预测方法未充分考虑不同消费者群体的独特用电模式，因此需要定制化预测策略。

Method: 研究通过消费者聚类和特征选择（包括时间、社会经济和天气数据），探索并比较了多种AI和机器学习算法，用于短期和超短期负荷预测。

Result: 提出的新预测方法在短期和超短期负荷预测中表现优于简单模型，证明了定制化策略和详细天气数据的重要性。

Conclusion: 定制化的预测策略和天气数据的结合显著提高了预测准确性，有助于电网稳定。
...
### [272] [Deep learning for predicting hauling fleet production capacity under uncertainties in open pit mines using real and simulated data](https://arxiv.org/abs/2506.04296)
...
<details>
  <summary>Details</summary>
Motivation: 露天矿运营中天气、机械故障和人员变动等因素导致不确定性，需要准确预测车队容量以优化管理。

Method: 结合高分辨率降雨数据、车队性能遥测和合成机械故障场景，评估了XGBoost和LSTM两种模型。

Result: XGBoost的MedAE为14.3%，LSTM为15.1%，模拟故障数据和班次规划特征显著降低了预测波动。

Conclusion: 未来将整合更多运营数据以提升预测鲁棒性，该方法为动态不确定条件下的车队管理提供了支持。
...
### [273] [Softlog-Softmax Layers and Divergences Contribute to a Computationally Dependable Ensemble Learning](https://arxiv.org/abs/2506.04297)
...
<details>
  <summary>Details</summary>
Motivation: 解决集成学习系统中一致性和可靠性的问题，通过多样性和信息理论优化决策过程。

Method: 1. 解剖学方法：选择多样化的规范形式构建目标集成模型；2. 生理学方法：通过softlog实现一致性弱对数操作；3. 信息理论方法：提出基于softlog的熵和散度；4. 性能评估：推导信息性能张量。

Result: 研究表明，问题越复杂，集成多样性越有用；softlog-softmax层能实现中间决策的一致性；信息度量在闭区间上提供一致值。

Conclusion: 提出的4步流程有效提升了集成学习的一致性和可靠性，适用于复杂问题。
...
### [274] [The Cost of Dynamic Reasoning: Demystifying AI Agents and Test-Time Scaling from an AI Infrastructure Perspective](https://arxiv.org/abs/2506.04301)
*Jiin Kim,Byeongjun Shin,Jinha Chung,Minsoo Rhu*

Main category: cs.LG

TL;DR: 该论文对基于大语言模型（LLM）的AI代理进行了系统级分析，量化了其资源使用、延迟行为、能耗等，揭示了其高计算需求和可持续性危机。


<details>
  <summary>Details</summary>
Motivation: 研究AI代理从静态推理转向动态多步工作流时带来的系统级成本、效率和可持续性问题。

Method: 通过量化资源使用、延迟行为、能耗等指标，分析不同代理设计和测试策略的影响。

Result: 发现代理虽能提高准确性，但计算成本迅速增加，延迟方差扩大，基础设施成本不可持续。

Conclusion: 呼吁设计转向计算高效推理，平衡性能与现实约束下的可部署性。
...
### [275] [RedRFT: A Light-Weight Benchmark for Reinforcement Fine-Tuning-Based Red Teaming](https://arxiv.org/abs/2506.04302)
*Xiang Zheng,Xingjun Ma,Wei-Bin Lee,Cong Wang*

Main category: cs.LG

TL;DR: RedRFT是一个轻量级基准，用于简化和标准化基于RFT的红队测试，解决了当前方法缺乏统一基准的问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于RFT的红队测试方法缺乏统一基准，实现细节（如PPO）影响结果的稳定性和可复现性。

Method: RedRFT结合CleanRL和Tianshou的设计优势，提供高质量的单文件实现和模块化PPO核心组件，支持多样化的评估指标。

Result: 通过广泛的消融研究，分析了LoRA、KL散度和拉格朗日乘数等关键组件对RFT性能的影响。

Conclusion: RedRFT有助于深入理解基于RFT的红队测试算法实现细节，并支持快速原型开发。
...
### [276] [You Only Train Once](https://arxiv.org/abs/2506.04349)
*Christos Sakaridis*

Main category: cs.LG

TL;DR: YOTO方法通过一次性自动优化损失权重超参数，避免了多次训练，提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 传统模型训练需要多次调整损失权重，耗时且低效。YOTO旨在通过一次性优化解决这一问题。

Method: 将损失权重作为网络参数，利用可微复合损失和softmax层进行优化，并引入正则化损失。

Result: 在3D估计和语义分割任务中，YOTO优于传统网格搜索方法。

Conclusion: YOTO提供了一种高效的一次性训练方案，显著提升了模型性能。
...
### [277] [Half-Layered Neural Networks](https://arxiv.org/abs/2506.04352)
*Ethem Alpaydin*

Main category: cs.LG

TL;DR: 提出一种“半”隐藏层单元，部分权重随机固定，部分可训练，减少参数数量并保持性能。


<details>
  <summary>Details</summary>
Motivation: 减少深度网络中隐藏层的可训练参数数量，同时通过随机连接的规范化效果保持模型性能。

Method: 每个“半”单元由两阶段组成：固定随机权重的加权和输入，再通过两个可训练权重进行缩放和平移，最后通过非线性激活。

Result: 在MNIST和FashionMNIST数据集上实验表明，半层能在减少参数的情况下保持合理准确率。

Conclusion: 半层设计为减少参数提供了一种有效方法，同时通过随机连接的规范化效果提升模型性能。
...
### [278] [A Risk-Aware Reinforcement Learning Reward for Financial Trading](https://arxiv.org/abs/2506.04358)
...
<details>
  <summary>Details</summary>
Motivation: 传统单一指标（如夏普比率）无法满足多样化的投资者偏好，因此需要一种模块化且可参数化的奖励函数。

Method: 通过网格搜索调整权重参数（w1-w4），并推导了每个项的闭式梯度以支持梯度训练，分析了单调性、有界性和模块性等理论特性。

Result: 该框架为复杂交易环境中构建稳健的多目标奖励函数提供了通用蓝图，并可扩展其他风险度量或自适应权重。

Conclusion: 提出的方法能够灵活适应不同投资者的风险-收益偏好，具有广泛的应用潜力。
...
### [279] [Even Faster Hyperbolic Random Forests: A Beltrami-Klein Wrapper Approach](https://arxiv.org/abs/2506.04360)
*Philippe Chlenski,Itsik Pe'er*

Main category: cs.LG

TL;DR: 论文提出了一种在Beltrami-Klein双曲空间模型中重新表达hyperDT算法的方法，通过简单的前后处理步骤包装现有的欧几里得空间决策树模型，提升了灵活性、速度和准确性。


<details>
  <summary>Details</summary>
Motivation: 扩展决策树模型到双曲空间，尤其是Lorentz模型，但希望保留欧几里得决策树的阈值操作，以简化实现并利用现有优化。

Method: 将hyperDT算法重新表达为Beltrami-Klein模型中的形式，通过前后处理步骤包装欧几里得空间决策树模型。

Result: 实现了更灵活、快速和准确的模型，同时代码更简单、可维护和可扩展。

Conclusion: 通过Beltrami-Klein模型重新表达hyperDT算法，成功结合了双曲空间的优势与欧几里得决策树的优化，为机器学习模型提供了新的可能性。
...
### [280] [Replay Can Provably Increase Forgetting](https://arxiv.org/abs/2506.04377)
*Yasaman Mahdaviyeh,James Lucas,Mengye Ren,Andreas S. Tolias,Richard Zemel,Toniann Pitassi*

Main category: cs.LG

TL;DR: 论文分析了样本回放在持续学习中的作用，发现即使在无噪声环境中，回放样本可能导致非单调性遗忘，甚至在某些情况下有害。
...
Result: 发现回放样本可能导致非单调性遗忘，甚至在某些情况下有害，且神经网络中也存在类似现象。
...
### [281] [Bridging the Performance Gap Between Target-Free and Target-Based Reinforcement Learning With Iterated Q-Learning](https://arxiv.org/abs/2506.04398)
*Théo Vincent,Yogesh Tripathi,Tim Faust,Yaniv Oren,Jan Peters,Carlo D'Eramo*

Main category: cs.LG

TL;DR: 提出了一种新方法iS-QL，通过共享在线网络的最后一层线性层作为目标网络，减少了目标网络的内存占用，同时提升了目标无关方法的性能。


<details>
  <summary>Details</summary>
Motivation: 目标网络在强化学习中虽稳定但占用内存，完全移除又会导致性能下降。希望找到一种折中方案。

Method: 使用在线网络的最后一层线性层副本作为目标网络，其余参数共享，结合并行学习Bellman迭代的Q学习方法。

Result: iS-QL在多种设置下提升了目标无关方法的样本效率，内存占用更小且训练时间与传统方法相当。
...
### [282] [Unsupervised Meta-Testing with Conditional Neural Processes for Hybrid Meta-Reinforcement Learning](https://arxiv.org/abs/2506.04399)
*Suzan Ece Ada,Emre Ugur*

Main category: cs.LG

TL;DR: UMCNP是一种结合了参数化策略梯度和任务推断的元强化学习方法，适用于元测试阶段缺失奖励信号的情况，提高了样本效率。


<details>
  <summary>Details</summary>
Motivation: 解决元测试阶段奖励信号缺失的问题，同时提升样本效率，减少在线交互需求。

Method: 结合参数化策略梯度和任务推断，利用条件神经过程（CNPs）高效学习任务推断，通过单次测试任务推断动态模型。

Result: 在2D-Point Agent和连续控制基准测试中，UMCNP比基线方法显著减少了元测试阶段的样本需求。

Conclusion: UMCNP在元测试阶段表现出高效性和适应性，适用于动态参数未知的任务。
...
### [283] [Self-Supervised Contrastive Learning is Approximately Supervised Contrastive Learning](https://arxiv.org/abs/2506.04411)
*Achleshwar Luthra,Tianbao Yang,Tomer Galanti*

Main category: cs.LG

TL;DR: 论文分析了自监督对比学习（CL）的理论基础，提出了一种称为“仅负样本监督对比损失”（NSCL）的变体，并证明了CL与NSCL损失之间的差距随类别数量增加而消失。此外，论文还研究了NSCL损失的几何结构，并提出了一个新的少样本误差界限。


<details>
  <summary>Details</summary>
Motivation: 尽管自监督对比学习在实证中表现良好，但其理论基础尚未完全建立。本文旨在填补这一空白，通过分析CL与NSCL损失之间的关系，揭示其内在机制。

Method: 论文提出NSCL损失作为CL的监督变体，并证明了CL与NSCL损失之间的差距随类别数量增加而消失。此外，还研究了NSCL损失的几何结构，并提出了一个新的少样本误差界限。

Result: 研究发现，CL与NSCL损失之间的差距以O(1/类别数)的速率衰减；两种损失高度相关；最小化CL损失会间接使NSCL损失接近直接最小化的值；提出的少样本误差界限在实践中能准确估计性能。

Conclusion: 论文为自监督对比学习提供了理论支持，揭示了其与监督学习之间的联系，并提出了实用的少样本误差界限，为未来的研究提供了方向。
...
### [284] [Leveraging Coordinate Momentum in SignSGD and Muon: Memory-Optimized Zero-Order](https://arxiv.org/abs/2506.04430)
...
TL;DR: 论文提出了一种零阶优化方法JAGUAR SignSGD和JAGUAR Muon，用于高效微调大型语言模型，显著减少内存和计算成本。


<details>
  <summary>Details</summary>
Motivation: 传统一阶优化器（如SGD和Adam）在微调大型语言模型时内存和计算成本过高，因此需要更高效的替代方案。

Method: 提出基于零阶优化的JAGUAR SignSGD和JAGUAR Muon算法，前者扩展了ZO SignSGD，后者利用了模型参数的矩阵结构。

Result: 实验表明，这些算法在收敛质量上不逊于一阶方法，同时显著降低了内存需求。
...
### [285] [KOALA++: Efficient Kalman-Based Optimization of Neural Networks with Gradient-Covariance Products](https://arxiv.org/abs/2506.04432)
...
<details>
  <summary>Details</summary>
Motivation: 传统二阶优化方法计算成本高，KOALA++旨在通过直接估计参数协方差矩阵，避免昂贵计算。

Method: 递归更新紧凑梯度协方差积，避免存储完整协方差矩阵和大规模矩阵求逆。

Result: 在图像分类和语言建模任务中，性能与或优于现有优化器，同时保持一阶方法的效率。

Conclusion: KOALA++在效率和性能上取得平衡，为神经网络训练提供了一种更优的优化方案。
...
### [286] [Grokking and Generalization Collapse: Insights from \texttt{HTSR} theory](https://arxiv.org/abs/2506.04434)
*Hari K. Prakash,Charles H. Martin*

Main category: cs.LG

TL;DR: 论文研究了神经网络中的grokking现象，发现了一个新的第三阶段——anti-grokking，并展示了HTSR指标α如何区分所有三个阶段。


<details>
  <summary>Details</summary>
Motivation: 探索神经网络训练中的grokking现象，特别是其不同阶段的表现和区分方法。

Method: 使用3层MLP在MNIST子集上训练，结合WeightWatcher工具的HTSR指标α分析训练阶段。

Result: 发现anti-grokking阶段，HTSR指标α能区分所有三个阶段，而其他指标仅能检测前两个阶段。

Conclusion: HTSR指标α是衡量泛化和过拟合的有效工具，其目标值α≈2具有普适性。
...
### [287] [RETRO SYNFLOW: Discrete Flow Matching for Accurate and Diverse Single-Step Retrosynthesis](https://arxiv.org/abs/2506.04439)
...
<details>
  <summary>Details</summary>
Motivation: 有机化学中，预测目标产物的合成反应序列是一个基础性问题，现有方法在单步逆合成中难以生成准确且多样的可行反应。

Method: RSF框架通过反应中心识别生成中间结构（synthons），并采用Feynman-Kac引导和Sequential Monte Carlo重采样优化生成样本。

Result: 实验表明，RSF在top-1准确率上达到60.0%，比之前方法提升20%，且top-5往返准确率提升19%。

Conclusion: RSF通过新颖的框架和引导策略，显著提升了单步逆合成的准确性和多样性。
...
### [288] [Selective Matching Losses -- Not All Scores Are Created Equal](https://arxiv.org/abs/2506.04446)
...
<details>
  <summary>Details</summary>
Motivation: 在特定区域（高重要性区域）需要更准确的预测，而传统损失函数无法区分不同区域的重要性。

Method: 设计了基于Sigmoid和双曲正弦链接的选择性标量损失函数，并扩展到多类问题，利用复合Softmax函数解决排名敏感性问题。

Result: 选择性损失函数在高重要性区域表现优于传统损失函数，适用于多种应用场景。

Conclusion: 选择性损失函数能有效解决模型在高重要性区域的预测问题，具有广泛的应用潜力。
...
### [289] [Neurosymbolic Artificial Intelligence for Robust Network Intrusion Detection: From Scratch to Transfer Learning](https://arxiv.org/abs/2506.04454)
*Huynh T. T. Tran,Jacob Sander,Achraf Cohen,Brian Jalaian,Nathaniel D. Bastian*

Main category: cs.LG

TL;DR: 本文扩展了ODXU框架，结合神经符号AI提升网络入侵检测系统的鲁棒性、可解释性和泛化能力，并在实验中优于传统模型。


<details>
  <summary>Details</summary>
Motivation: 探索神经符号AI在网络入侵检测中的应用，弥补迁移学习在网络安全领域的不足。

Method: 扩展ODXU框架，结合深度嵌入聚类、XGBoost符号推理和不确定性量化，并开发迁移学习策略。

Result: 在CIC-IDS-2017和ACI-IoT-2023数据集上表现优异，迁移学习策略显著提升性能。

Conclusion: ODXU框架在网络安全中具有潜力，迁移学习策略和元模型方法效果显著。
...
### [290] [Behavioural vs. Representational Systematicity in End-to-End Models: An Opinionated Survey](https://arxiv.org/abs/2506.04461)
*Ivan Vegner,Sydelle de Souza,Valentin Forch,Martha Lewis,Leonidas A. A. Doumas*

Main category: cs.LG

TL;DR: 论文探讨了机器学习模型中的系统性泛化问题，强调行为系统性与表征系统性的区别，并分析了现有基准测试的局限性。


<details>
  <summary>Details</summary>
Motivation: 系统性是组合性的核心特性，对机器学习模型的泛化能力至关重要。现有研究多关注行为系统性，而忽视了表征系统性。

Method: 基于Hadley（1994）的分类法，分析了语言和视觉领域的关键基准测试，并探讨了如何评估表征系统性。

Result: 现有基准测试主要测试行为系统性，对表征系统性的评估不足。

Conclusion: 应更关注表征系统性的评估，借鉴机制可解释性领域的方法。
...
### [291] [Classifying Dental Care Providers Through Machine Learning with Features Ranking](https://arxiv.org/abs/2506.04474)
*Mohammad Subhi Al-Batah,Mowafaq Salem Alzboon,Muhyeeddin Alqaraleh,Mohammed Hasan Abu-Arqoub,Rashiq Rafiq Marie*

Main category: cs.LG

TL;DR: 该研究利用机器学习模型对牙科提供者进行分类，通过特征排名和多种模型评估，发现神经网络表现最佳（94.1%准确率），并强调特征选择对不平衡医疗数据集的重要性。


<details>
  <summary>Details</summary>
Motivation: 研究旨在通过机器学习模型区分标准牙科提供者和安全网诊所提供者，以优化资源分配，服务弱势群体。

Method: 使用2018年数据集（24,300实例，20特征），采用信息增益、基尼指数和ANOVA进行特征排名，评估12种机器学习模型（如kNN、决策树、SVM等），并通过10折交叉验证测试分类准确性。

Result: 神经网络准确率最高（94.1%），其次是梯度提升（93.2%）和随机森林（93.0%）。治疗相关指标是最关键特征，而人口统计变量影响较小。

Conclusion: 特征选择显著提升模型效率，尤其在数据不平衡的医疗领域。研究建议结合特征排名与先进算法，优化牙科提供者分类，支持资源精准分配。
...
### [292] [Comparative performance of ensemble models in predicting dental provider types: insights from fee-for-service data](https://arxiv.org/abs/2506.04479)
...
Method: 使用2018年数据集，分析24,300个实例和20个特征，测试多种机器学习算法（如kNN、决策树、SVM等），采用10折交叉验证。

Result: 神经网络表现最佳（AUC: 0.975, CA: 94.1%），随机森林次之（AUC: 0.948, CA: 93.0%）。

Conclusion: 先进的机器学习技术（如集成学习和深度学习）显著提升了牙科提供者分类效果，有助于优化资源分配和服务不足人群的医疗支持。
...
### [293] [COSMOS: Predictable and Cost-Effective Adaptation of LLMs](https://arxiv.org/abs/2505.01449)
*Jiayu Wang,Aws Albarghouthi,Frederic Sala*

Main category: cs.LG

TL;DR: COSMOS是一个预测框架，用于高效评估大语言模型（LLMs）的适应策略性能和成本，减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 在资源受限的情况下，如何最优选择LLMs的适应策略是一个挑战，通常需要大量实验。

Method: 引入COSMOS框架，结合嵌入增强的轻量代理模型和低样本扩展法则，预测微调性能和检索增强的上下文学习。

Result: 在八个基准测试中，COSMOS平均减少计算成本92.72%，最高达98.71%，同时保持高预测准确性。

Conclusion: 高效预测LLMs适应策略的可行性，显著降低计算开销，同时维持性能标准。
...
### [294] [Orthogonal Gradient Descent Improves Neural Calibration](https://arxiv.org/abs/2506.04487)
*C. Evans Hedges*
...
### [295] [Multiscale guidance of AlphaFold3 with heterogeneous cryo-EM data](https://arxiv.org/abs/2506.04490)
*Rishwanth Raghu,Axel Levy,Gordon Wetzstein,Ellen D. Zhong*

Main category: cs.LG

TL;DR: CryoBoltz结合冷冻电镜密度图和蛋白质结构预测模型，通过实验数据指导采样轨迹，生成动态生物分子复合物的原子模型。
...
Method: 利用冷冻电镜密度图和预训练的蛋白质结构预测模型，通过全局和局部结构约束指导采样轨迹，生成与实验数据一致的构象状态。

Result: 成功构建了包括转运蛋白和抗体在内的多种动态生物分子系统的原子模型。

Conclusion: CryoBoltz为动态生物分子复合物的结构建模提供了一种灵活且高效的方法。
...
### [296] [Perturbative Gradient Training: A novel training paradigm for bridging the gap between deep neural networks and physical reservoir computing](https://arxiv.org/abs/2506.04523)
*Cliff B. Abbott,Mark Elo,Dmytro A. Bozhko*

Main category: cs.LG

TL;DR: PGT是一种新的训练方法，通过随机扰动近似梯度更新，解决了物理储层计算中无法反向传播的问题。


<details>
  <summary>Details</summary>
Motivation: 解决物理储层计算中因黑盒特性无法使用反向传播的局限性。

Method: 利用参数空间的随机扰动，仅通过前向传播近似梯度更新。

Result: 在模拟和实验硬件中，PGT性能与标准反向传播相当。
...
### [297] [Hierarchical Implicit Neural Emulators](https://arxiv.org/abs/2506.04528)
*Ruoxi Jiang,Xiao Zhang,Karan Jakhar,Peter Y. Lu,Pedram Hassanzadeh,Michael Maire,Rebecca Willett*

Main category: cs.LG

TL;DR: 提出了一种多尺度隐式神经模拟器，通过利用多尺度未来状态表示，提升长期预测准确性，并在湍流流体动力学实验中表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统神经PDE求解器在长期时间范围内易出现误差累积和稳定性问题，需要一种能保持物理一致性和稳定性的方法。

Method: 采用多尺度隐式神经模拟器，通过调整时间下采样比率，利用多步预测和压缩率进行下一步细化。

Result: 在湍流流体动力学实验中，该方法实现了高短期精度和长期稳定预测，显著优于自回归基线。

Conclusion: 多尺度隐式神经模拟器能有效提升长期预测的准确性和稳定性，且计算开销小。
...
### [298] [HALoS: Hierarchical Asynchronous Local SGD over Slow Networks for Geo-Distributed Large Language Model Training](https://arxiv.org/abs/2506.04531)
*Geon-Woo Kim,Junbo Li,Shashidhar Gandham,Omar Baldonado,Adithya Gangidi,Pavan Balaji,Zhangyang Wang,Aditya Akella*

Main category: cs.LG

TL;DR: HALoS是一种分层异步优化框架，通过引入本地和全局参数服务器，减少地理分布式训练中的通信成本，提高效率。


<details>
  <summary>Details</summary>
Motivation: 解决地理分布式加速器训练大语言模型时的高通信成本和硬件利用率不均问题。

Method: 采用分层设计，包括本地参数服务器（LPS）和全局参数服务器（GPS），减少跨区域通信并利用快速区域内链接。

Result: HALoS在非凸目标下收敛性良好，比同步基线快7.5倍，比现有异步方法快2.1倍，同时保持模型质量。

Conclusion: 分层设计和服务器端更新累积是高效训练地理分布式大语言模型的有效工具。
...
### [299] [NOBLE -- Neural Operator with Biologically-informed Latent Embeddings to Capture Experimental Variability in Biological Neuron Models](https://arxiv.org/abs/2506.04536)
*Luca Ghafourpour,Valentin Duruisseaux,Bahareh Tolooshams,Philip H. Wong,Costas A. Anastassiou,Anima Anandkumar*

Main category: cs.LG

TL;DR: NOBLE是一种神经操作框架，通过学习神经元特征的连续频率调制嵌入到电压响应的映射，解决了传统生物真实神经元模型的局限性，并实现了4200倍的加速。


<details>
  <summary>Details</summary>
Motivation: 当前神经元建模方法受限于实验数据的有限性和变异性，传统方法难以扩展，深度学习未能完全捕捉神经元的生物物理复杂性。

Method: NOBLE框架通过训练生物物理真实神经元模型生成的数据，学习从神经元特征到电压响应的映射，并预测神经动力学的分布。

Result: NOBLE能够高效生成具有实验变异性的合成神经元，速度比数值求解器快4200倍，且其动态与实验观察一致。

Conclusion: NOBLE为理解神经元组成、计算及神经形态架构提供了新工具，推动了神经AI应用的发展。
...
### [300] [Neural MJD: Neural Non-Stationary Merton Jump Diffusion for Time Series Prediction](https://arxiv.org/abs/2506.04542)
*Yuanpei Gao,Qi Yan,Yan Leng,Renjie Liao*

Main category: cs.LG

TL;DR: Neural MJD是一种基于神经网络的非平稳Merton跳跃扩散模型，通过显式建模随机微分方程（SDE）来预测时间序列，解决了传统深度学习方法在非平稳数据上的局限性。
...
Method: Neural MJD结合了时间非齐次的Itô扩散和时间非齐次的复合泊松过程，通过截断似然机制和优化的Euler-Maruyama求解器实现高效学习。
...
### [301] [Communication Efficient Adaptive Model-Driven Quantum Federated Learning](https://arxiv.org/abs/2506.04548)
*Dev Gurung,Shiva Raj Pokhrel*

Main category: cs.LG

TL;DR: 提出了一种模型驱动的量子联邦学习算法（mdQFL），解决了大规模数据集、多设备参与和非独立同分布数据带来的挑战，显著降低了通信成本并提升了性能。
...
Method: 引入mdQFL算法，支持训练和更新个性化，适应不同设备数量和非独立同分布数据。

Result: 实验显示，mdQFL在Qiskit环境中显著降低50%通信成本，同时保持或提升模型准确性，并改善本地模型训练。

Conclusion: mdQFL是首个探索QFL中训练个性化与测试泛化的算法，适用于其他FL场景，实验和理论分析验证了其有效性。
...
### [302] [Unsupervised Machine Learning for Scientific Discovery: Workflow and Best Practices](https://arxiv.org/abs/2506.04553)
...
Method: 提出结构化工作流程，包括问题定义、数据准备、建模技术、严格验证及结果沟通与文档化。

Result: 通过天文学案例研究验证了工作流程的有效性，强调了验证的重要性。

Conclusion: 精心设计的无监督学习工作流程可推动科学发现，并提高其可靠性和可重复性。
...
### [303] [Clustering and Median Aggregation Improve Differentially Private Inference](https://arxiv.org/abs/2506.04566)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通过均匀采样敏感输入生成合成文本，但会导致质量下降，尤其是在输入数据涉及异质主题时。

Method: 聚类输入数据以选择推理批次，并利用中位数聚合下一令牌预测，以减少局部敏感性并满足数据依赖的差分隐私保证。

Result: 方法在代表性指标（如MAUVE）和下游任务性能上均有提升，能以更低隐私成本生成高质量合成数据。

Conclusion: 通过聚类和中位数聚合，显著提升了差分隐私语言模型推理的效果，同时降低了隐私成本。
...
### [304] [StatsMerging: Statistics-Guided Model Merging via Task-Specific Teacher Distillation](https://arxiv.org/abs/2506.04567)
*Ranjith Merugu,Bryan Bo Cao,Shubham Jain*

Main category: cs.LG

TL;DR: StatsMerging是一种基于统计学习的轻量级模型合并方法，利用SVD奇异值捕获任务重要性，无需标签或测试样本，在多个任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决内存受限下多大型模型的合并问题，避免依赖标签或测试样本。

Method: 利用SVD奇异值指导任务系数预测，引入轻量级学习器StatsMergeLearner，并通过任务特定教师蒸馏合并异构架构模型。

Result: 在八项任务中表现优于现有技术，整体准确性、泛化能力和鲁棒性显著提升。

Conclusion: StatsMerging为模型合并提供了高效、泛化强的解决方案，适用于异构架构和多样化任务。
...
### [305] [Scaling Laws for Robust Comparison of Open Foundation Language-Vision Models and Datasets](https://arxiv.org/abs/2506.04598)
*Marianna Nezhurina,Tomer Porian,Giovanni Pucceti,Tommie Kerssies,Romain Beaumont,Mehdi Cherti,Jenia Jitsev*

Main category: cs.LG

TL;DR: 本文提出了一种利用扩展定律（scaling laws）进行模型和数据集比较的方法，首次为CLIP和MaMMUT两种语言-视觉学习模型推导了全面的扩展定律，并比较了它们的性能。结果表明MaMMUT在扩展性和样本效率上优于CLIP。


<details>
  <summary>Details</summary>
Motivation: 研究如何通过扩展定律比较不同模型和数据集，以优化预训练过程，避免仅基于单一参考尺度的误导性结论。

Method: 通过密集测量不同模型和样本规模，推导CLIP和MaMMUT的扩展定律，并在分类、检索和分割等下游任务中验证。

Result: MaMMUT在扩展性和样本效率上优于CLIP，且在不同数据集和任务中表现一致。

Conclusion: 扩展定律的准确推导为模型和数据集的系统性比较和改进提供了有效工具。
...
### [306] [Ignoring Directionality Leads to Compromised Graph Neural Network Explanations](https://arxiv.org/abs/2506.04608)
*Changsheng Sun,Xinke Li,Jin Song Dong*

Main category: cs.LG

TL;DR: 论文指出图对称化会丢失方向信息，影响解释质量，提出方向感知GNN可提升解释忠实度。


<details>
  <summary>Details</summary>
Motivation: 在关键领域使用GNN时，可靠的解释对决策至关重要，但现有方法因对称化丢失方向信息。

Method: 通过理论和实证研究，分析方向信息对解释质量的影响。

Result: 保留方向语义显著提升解释质量，提供更忠实的决策支持。

Conclusion: 方向感知GNN解释方法对安全关键应用至关重要。
...
### [307] [Composing Agents to Minimize Worst-case Risk](https://arxiv.org/abs/2506.04632)
*Guruprerana Shabadi,Rajeev Alur*

Main category: cs.LG

TL;DR: 该论文提出了一种基于有向无环图（agent graph）的AI代理组合优化方法，旨在最大化任务成功率的同时最小化风险（如安全、公平、隐私）。通过近似尾部风险（value-at-risk）和动态规划，算法高效地找到近优代理组合。


<details>
  <summary>Details</summary>
Motivation: 现代AI系统需分解复杂任务为子任务并由多个代理完成，但需在任务成功与风险（如安全、公平、隐私）间权衡。论文旨在解决代理组合的最坏情况风险最小化问题。

Method: 将代理工作流建模为有向无环图（agent graph），定义最坏情况风险为尾部损失分位数（value-at-risk），提出基于联合边界和动态规划的高效近似算法。

Result: 算法在视频游戏控制基准测试中验证，能有效近似尾部风险并找到最优代理组合。

Conclusion: 论文提出的方法在理论和实践中均有效，适用于需权衡任务成功与风险的AI代理组合场景。
...
### [308] [Inference economics of language models](https://arxiv.org/abs/2506.04645)
*Ege Erdil*

Main category: cs.LG

TL;DR: 论文提出了一种理论模型，用于权衡大规模部署LLM推理时的每个token成本与生成速度，优化并行设置和批量大小。


<details>
  <summary>Details</summary>
Motivation: 解决在大规模部署LLM推理时，如何在每个token成本和生成速度之间找到最优平衡点的问题。

Method: 模型考虑了算术、内存带宽、网络带宽和延迟约束，并通过优化并行设置和批量大小来找到最佳推理速度。

Result: 计算了流行语言模型的串行速度与每个token成本的帕累托前沿。

Conclusion: 该模型为LLM推理的部署提供了理论支持，帮助在实际应用中优化性能与成本。
...
### [309] [Neural Network Reprogrammability: A Unified Theme on Model Reprogramming, Prompt Tuning, and Prompt Instruction](https://arxiv.org/abs/2506.04650)
*Zesheng Ye,Chengyi Cai,Ruijiang Dong,Jianzhong Qi,Lei Feng,Pin-Yu Chen,Feng Liu*

Main category: cs.LG

TL;DR: 该论文提出神经网络可重编程性作为统一框架，连接主流模型适应技术（如模型重编程、提示调整和提示指令），并分类信息操纵方法。


<details>
  <summary>Details</summary>
Motivation: 随着预训练基础模型规模和能力的增长，高效适应下游任务变得至关重要，但现有方法缺乏统一理解。

Method: 引入神经网络可重编程性框架，分类信息操纵方法（格式、位置、操作符和输出对齐）。

Result: 框架适用于多种数据模态和模型架构，揭示了现有技术的理论联系和实际区别。

Conclusion: 神经网络可重编程性是高效模型适应的基础范式，并指出了未来研究方向。
...
### [310] [The Oversmoothing Fallacy: A Misguided Narrative in GNN Research](https://arxiv.org/abs/2506.04653)
*MoonJeong Park,Sunghyun Choi,Jaeseung Heo,Eunhyeok Park,Dongwoo Kim*

Main category: cs.LG

TL;DR: 本文认为图神经网络（GNN）中的过度平滑问题被夸大，并提出深层GNN架构仍有探索空间。研究发现过度平滑与梯度消失混淆，且经典方法（如跳跃连接和归一化）可解决此问题。


<details>
  <summary>Details</summary>
Motivation: 探讨深层GNN性能受限的真正原因，挑战过度平滑是GNN特有问题的观点。

Method: 分析GNN的三大核心操作（聚合、线性变换和非线性激活），区分过度平滑与梯度消失的关系。

Result: 证明经典方法（如跳跃连接和归一化）可有效构建深层GNN，性能不下降。

Conclusion: 澄清了对过度平滑的误解，揭示了深层GNN的潜力。
...
### [311] [Noise-Resistant Label Reconstruction Feature Selection for Partial Multi-Label Learning](https://arxiv.org/abs/2506.04669)
*Wanfu Gao,Hanlin Pan,Qingqi Han,Kunpeng Liu*

Main category: cs.LG

TL;DR: 提出了一种针对部分多标签学习（PML）的特征选择方法，通过标签关系的抗噪性和连通性解决维度灾难和标签识别问题。


<details>
  <summary>Details</summary>
Motivation: 现有PML方法基于低秩假设，但实际中难以满足且可能丢失高维信息，且对正标签识别能力不足。

Method: 利用标签关系的抗噪性去歧义，改进低秩假设设计学习过程，通过标签连通性重构权重矩阵选择特征。

Result: 在基准数据集上验证了方法的优越性。

Conclusion: 该方法有效解决了PML中的维度灾难和标签识别问题。
...
### [312] [FedAPM: Federated Learning via ADMM with Partial Model Personalization](https://arxiv.org/abs/2506.04672)
*Shengkun Zhu,Feiteng Nie,Jinshan Zeng,Sheng Wang,Yuan Sun,Yuan Yao,Shangfeng Chen,Quanqing Xu,Chuanhui Yang*

Main category: cs.LG

TL;DR: 论文提出了一种基于ADMM的联邦学习框架FedAPM，通过引入一阶和二阶近端项缓解客户端漂移，实现了更稳定和高效的收敛。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中数据非独立同分布和客户端漂移问题影响了算法的效率和效果。

Method: 采用ADMM方法，构建增强拉格朗日函数，结合一阶和二阶近端项修正客户端模型。

Result: FedAPM在四种异构数据集上表现优异，测试准确率、F1分数和AUC分别平均提升12.3%、16.4%和18.0%。

Conclusion: FedAPM通过显式估计拉格朗日乘子，显著提升了联邦学习的收敛速度和准确性。
...
### [313] [The cost of ensembling: is it always worth combining?](https://arxiv.org/abs/2506.04677)
*Marco Zanotti*

Main category: cs.LG

TL;DR: 论文研究了时间序列预测中集合学习的准确性-计算成本权衡，发现集合模型能提升性能但代价高昂，减少重训练频率可降低成本且对准确性影响小，小规模集合（2-3个模型）即可接近最优结果。


<details>
  <summary>Details</summary>
Motivation: 随着数据集规模和预测模型复杂度的增加，如何在预测准确性和计算成本之间取得平衡成为重要问题，尤其是在时间序列预测的集合学习中。

Method: 在两个大型零售数据集（M5和VN1）上评估了十种基础模型和八种集合配置，考虑了不同重训练频率下的点预测和概率预测准确性。

Result: 集合模型显著提升预测性能（尤其是概率预测），但计算成本较高；减少重训练频率可大幅降低成本且对点预测准确性影响较小；小规模集合（2-3个模型）通常能达到接近最优的结果。

Conclusion: 通过精心设计集合模型和选择重训练策略，可以实现准确、稳健且成本效益高的预测，适用于实际应用，支持可持续AI的目标。
...
### [314] [Urania: Differentially Private Insights into AI Use](https://arxiv.org/abs/2506.04681)
...
Result: 评估显示框架在保留内容和语义的同时，提供严格的隐私保护，优于非隐私保护方法。
...
### [315] [Towards Better Generalization via Distributional Input Projection Network](https://arxiv.org/abs/2506.04690)
...
<details>
  <summary>Details</summary>
Motivation: 过参数化模型的训练损失难以反映泛化性能，直接强制平滑性在神经网络中具有挑战性。

Method: 提出DIPNet框架，将输入逐层投影到可学习分布中，理论分析显示其降低局部平滑性度量和Lipschitz常数。

Result: 在ViTs、LLMs、ResNet和MLPs等模型和任务中验证，提升测试性能，包括对抗攻击和分布外输入场景。
...
### [316] [Influence Functions for Edge Edits in Non-Convex Graph Neural Networks](https://arxiv.org/abs/2506.04694)
*Jaeseung Heo,Kyeongheung Yun,Seokwon Yoon,MoonJeong Park,Jungseul Ok,Dongwoo Kim*

Main category: cs.LG

TL;DR: 该论文提出了一种针对图神经网络（GNNs）的近端Bregman响应函数，用于准确预测边删除和插入的影响，并考虑了消息传播效应。
...
Method: 提出近端Bregman响应函数，放宽凸性要求，并明确考虑消息传播效应，支持边删除和插入的影响预测。

Result: 实验证明该方法能准确预测GNNs中不同特性的影响，并适用于图重连和对抗攻击等应用。

Conclusion: 该方法为GNNs的影响预测提供了更灵活和准确的工具，适用于多种实际场景。
...
### [317] [On the Mechanism of Reasoning Pattern Selection in Reinforcement Learning for Language Models](https://arxiv.org/abs/2506.04695)
...
Result: RLVR优先选择高成功率推理模式，理论分析揭示了快速和慢速收敛的两种动态，SFT可缓解慢速收敛。
...
### [318] [Enhanced Drought Analysis in Bangladesh: A Machine Learning Approach for Severity Classification Using Satellite Data](https://arxiv.org/abs/2506.04696)
*Tonmoy Paul,Mrittika Devi Mati,Md. Mahmudul Islam*

Main category: cs.LG

TL;DR: 提出了一种基于卫星数据和机器学习的框架，用于分类孟加拉国38个地区的干旱严重程度，填补了传统指数和现有模型的不足。
...
Result: 成功将干旱严重程度分类为不同级别，但发现地区间干旱脆弱性差异显著。
...
### [319] [Explicit Density Approximation for Neural Implicit Samplers Using a Bernstein-Based Convex Divergence](https://arxiv.org/abs/2506.04700)
*José Manuel de Frutos,Manuel A. Vázquez,Pablo M. Olmos,Joaquín Míguez*

Main category: cs.LG

TL;DR: 本文提出了一种新的无似然目标函数dual-ISL，用于训练隐式生成模型，通过交换目标分布和模型分布的角色，解决了ISL框架中的优化问题，并证明了其连续性和凸性。
...
Method: 提出dual-ISL目标函数，通过交换目标分布和模型分布的角色，构建凸优化问题，并分析其在密度空间中的性质。

Result: dual-ISL在理论和实践中均表现出色，具有更快的收敛速度、更平滑的训练过程和更好的模式保持能力。

Conclusion: dual-ISL是一种高效且鲁棒的隐式生成模型训练方法，优于传统方法。
...
### [320] [UNO: Unlearning via Orthogonalization in Generative models](https://arxiv.org/abs/2506.04712)
...
<details>
  <summary>Details</summary>
Motivation: 随着生成模型的广泛应用，选择性遗忘数据的需求日益增长，例如隐私保护或内容修正，而传统方法需要昂贵的重新训练。

Method: 采用基于损失梯度正交化的算法，确保在移除不需要的数据时，不影响模型生成质量和其他数据的影响。

Result: 在MNIST和CelebA数据集上，新算法比梯度手术等方法快几个数量级，同时保持模型保真度。

Conclusion: 提出的算法在高效遗忘数据方面表现优异，为生成模型的数据管理提供了实用解决方案。
...
### [321] [Multi-Layer GRPO: Enhancing Reasoning and Self-Correction in Large Language Models](https://arxiv.org/abs/2506.04746)
*Fei Ding,Baiqiao Wang,Zijian Zeng,Youwei Wang*

Main category: cs.LG

TL;DR: MGRPO通过双层GRPO结构提升LLM的推理能力，第一层生成初始响应，第二层纠正错误，显著优于标准GRPO。


<details>
  <summary>Details</summary>
Motivation: GRPO在LLM推理中表现优异，但缺乏中间监督导致探索效率低，错误会破坏整个推理链。

Method: MGRPO采用双层GRPO结构，第一层生成初始响应，第二层纠正错误，形成自校正循环。

Result: MGRPO在数学推理基准测试中显著优于标准GRPO，提升了推理和自校正能力。

Conclusion: MGRPO通过自校正机制解决了GRPO的探索效率问题，显著提升了性能。
...
### [322] [Log-Linear Attention](https://arxiv.org/abs/2506.04761)
...
<details>
  <summary>Details</summary>
Motivation: Transformer中的注意力机制虽然强大，但其二次计算和线性内存复杂度成为瓶颈。线性注意力和状态空间模型虽高效，但受限于固定大小的隐藏状态。

Method: 开发了log-linear attention，通过使用对数增长的隐藏状态集合替代固定大小隐藏状态，并展示了其并行计算的可行性。

Result: log-linear attention在计算成本上为对数线性复杂度，且适用于现有线性注意力变体。实验表明，其在Mamba-2和Gated DeltaNet中表现良好。

Conclusion: log-linear attention是一种高效且表达能力强的注意力机制，为序列建模提供了新的解决方案。
...
### [323] [OpenGT: A Comprehensive Benchmark For Graph Transformers](https://arxiv.org/abs/2506.04765)
*Jiachen Tang,Zhonghao Wang,Sirui Chen,Sheng Zhou,Jiawei Chen,Jiajun Bu*

Main category: cs.LG

TL;DR: 论文介绍了OpenGT，一个全面的图Transformer基准测试，旨在解决当前缺乏标准化评估的问题，并提供了多维分析和开源库。


<details>
  <summary>Details</summary>
Motivation: 当前图Transformer的应用场景和设计选择缺乏明确指导，社区需要标准化基准以促进深入理解和进一步发展。

Method: 通过建立标准化实验设置和广泛选择GNNs与GTs，OpenGT从多角度评估GTs，涵盖多样化任务和数据集。

Result: 实验揭示了模型跨任务迁移的困难、局部注意力的局限性、效率权衡、特定位置编码的应用场景及其预处理开销。

Conclusion: OpenGT为未来图Transformer研究奠定了基础，强调公平性、可重复性和泛化性，并提供了开源库。
...
### [324] [Improved Regret Bounds for Linear Bandits with Heavy-Tailed Rewards](https://arxiv.org/abs/2506.04775)
...
### [325] [Kernel $k$-Medoids as General Vector Quantization](https://arxiv.org/abs/2506.04786)
...
Method: 通过QUBO框架比较k-medoids和KDE的启发式与理论推导。

Result: KDE-QUBO是k-medoids-QUBO的特例，揭示了两种方法的深层结构关系。

Conclusion: 研究提供了对VQ中QUBO权重参数的几何解释新视角。
...
### [326] [Adaptive Preconditioners Trigger Loss Spikes in Adam](https://arxiv.org/abs/2506.04805)
*Zhiwei Bai,Zhangchen Zhou,Jiajie Zhao,Xiaolong Li,Zhiyu Li,Feiyu Xiong,Hongkang Yang,Yaoyu Zhang,Zhi-Qin John Xu*

Main category: cs.LG

TL;DR: 研究发现Adam优化器中的自适应预条件器是导致训练中损失突增的原因，而非传统认为的损失景观特性。


<details>
  <summary>Details</summary>
Motivation: 探讨Adam优化器在训练神经网络时损失突增的机制。

Method: 分析Adam的预条件器在特定条件下（平方梯度远小于二阶矩估计）的行为，及其对Hessian矩阵特征值的影响。

Result: 发现预条件器会导致Hessian最大特征值超过稳定性阈值，引发梯度与最大特征方向对齐，从而产生损失突增。

Conclusion: 通过实验验证了这一机制在多种网络架构中的普遍性。
...
### [327] [LogicPuzzleRL: Cultivating Robust Mathematical Reasoning in LLMs via Reinforcement Learning](https://arxiv.org/abs/2506.04821)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLMs在陌生环境中的结构化推理表现不佳，表明标准微调方法可能仅培养领域特定的启发式策略，而非通用思维策略。

Method: 采用强化学习框架，通过七种定制逻辑谜题微调LLMs，每种谜题旨在培养不同的推理技能（如约束传播、空间一致性等）。

Result: 该方法显著提升了LLMs在数学基准测试中的分布外表现，尤其是在需要多步推理的中等难度问题上。

Conclusion: 研究表明，逻辑谜题训练能重塑LLMs的内部推理机制，实现更鲁棒和组合式的泛化，而无需依赖任务特定的符号工具。
...
### [328] [From EHRs to Patient Pathways: Scalable Modeling of Longitudinal Health Trajectories with LLMs](https://arxiv.org/abs/2506.04831)
*Chantal Pellegrini,Ege Özsoy,David Bani-Harouni,Matthias Keicher,Nassir Navab*

Main category: cs.LG

TL;DR: 论文提出了一种名为EHR2Path的新方法，通过将多样化的电子健康记录数据转化为结构化表示，并设计了一个全面的路径预测模型，用于预测患者未来的健康轨迹。该方法在预测性能和效率上优于现有基线模型。


<details>
  <summary>Details</summary>
Motivation: 医疗系统在管理和解释大量异构患者数据以实现个性化护理方面面临挑战。现有方法通常局限于狭窄的用例和有限的特征空间，忽略了患者健康的复杂性和长期交互需求。

Method: 提出EHR2Path模型，将电子健康记录数据转化为结构化表示，并引入一种新颖的摘要机制，将长期时间上下文嵌入到特定主题的摘要标记中。

Result: EHR2Path在预测未来健康轨迹方面表现出色，优于现有基线模型，并能支持多种评估任务，如生命体征预测和住院时长预测。

Conclusion: EHR2Path为预测性和个性化医疗提供了一种有效的工具，能够模拟患者轨迹并支持多样化的评估任务。
...
### [329] [Sparse Autoencoders, Again?](https://arxiv.org/abs/2506.04859)
...
<details>
  <summary>Details</summary>
Motivation: 传统SAEs和VAEs在处理低维潜在结构时存在未被充分认识的弱点，需要一种更有效的模型。

Method: 提出了一种混合模型，结合了SAEs和VAEs的优点，并通过理论证明和实验验证其性能。

Result: 新模型在合成和真实数据集上表现优异，能够更准确地估计潜在维度并生成更稀疏的表示，同时不影响重构误差。
...
### [330] [Aligning Multimodal Representations through an Information Bottleneck](https://arxiv.org/abs/2506.04870)
...
<details>
  <summary>Details</summary>
Motivation: 现有对比损失未能有效学习对齐表示空间，原因是表示空间中存在模态特定信息。

Method: 通过信息瓶颈原理分析问题，提出一种正则化损失函数，并通过实验验证其效果。

Result: 实验表明，正则化项能显著提高表示对齐，适用于多种实际应用。

Conclusion: 提出的正则化方法有效解决了对比损失在多模态学习中的局限性。
...
### [331] [There Was Never a Bottleneck in Concept Bottleneck Models](https://arxiv.org/abs/2506.04877)
*Antonio Almudévar,José Miguel Hernández-Lobato,Alfonso Ortega*

Main category: cs.LG

TL;DR: 论文提出了一种改进的概念瓶颈模型（MCBM），通过引入信息瓶颈目标来确保每个表示组件仅保留与其对应概念相关的信息，从而提升模型的解释性和干预有效性。


<details>
  <summary>Details</summary>
Motivation: 现有的概念瓶颈模型（CBM）未能真正限制表示组件仅编码特定概念的信息，这影响了模型的解释性和干预效果。

Method: 提出最小概念瓶颈模型（MCBM），通过变分正则化项实现信息瓶颈目标，约束表示组件仅保留与概念相关的信息。

Result: MCBM支持具有理论保证的概念级干预，符合贝叶斯原则，并在设计选择上更具灵活性。

Conclusion: MCBM通过信息瓶颈目标改进了CBM，显著提升了模型的解释性和干预有效性。
...
### [332] [Gaussian Process Diffeomorphic Statistical Shape Modelling Outperforms Angle-Based Methods for Assessment of Hip Dysplasia](https://arxiv.org/abs/2506.04886)
*Allen Paul,George Grammatopoulos,Adwaye Rambojun,Neill D. F. Campbell,Harinderjit S. Gill,Tony Shardlow*

Main category: cs.LG

TL;DR: 开发了一种基于CT扫描的半自动化分类方法（GPDSSM），用于区分髋关节发育不良，提高分类准确率并节省临床时间。


<details>
  <summary>Details</summary>
Motivation: 髋关节发育不良是骨关节炎的风险因素，早期诊断有助于手术干预。

Method: 结合高斯过程隐变量模型和微分同胚，构建统计形状模型（GPDSSM），使用192例CT扫描数据（100训练，92测试）。

Result: GPDSSM能有效区分发育不良样本（AUC 96.2%），优于基于角度的方法（AUC 91.2%），并节省临床时间。

Conclusion: GPDSSM为髋关节发育不良的诊断提供了高效、准确的工具。
...
### [333] [Dissecting Long Reasoning Models: An Empirical Study](https://arxiv.org/abs/2506.04913)
*Yongyu Mu,Jiali Zeng,Bei Li,Xinyan Guan,Fandong Meng,Jie Zhou,Tong Xiao,Jingbo Zhu*

Main category: cs.LG

TL;DR: 论文分析了强化学习中正负样本的作用，发现负样本显著提升泛化能力；提出了解决数据效率低下的策略；并探讨了性能不稳定的原因及解决方案。


<details>
  <summary>Details</summary>
Motivation: 研究强化学习中正负样本的作用、数据效率低下及性能不稳定的问题。

Method: 系统分析正负样本作用；提出相对长度奖励和离线样本注入策略；通过多次评估缓解性能不稳定。

Result: 负样本提升泛化能力；新策略提高数据效率；多次评估减少性能波动。

Conclusion: 负样本和优化策略对强化学习模型性能有显著影响，多次评估可提升稳定性。
...
### [334] [Predicting ICU In-Hospital Mortality Using Adaptive Transformer Layer Fusion](https://arxiv.org/abs/2506.04924)
...
TL;DR: ALFIA是一种基于注意力的模块化架构，通过融合多层语义特征，显著提升了ICU高风险患者的早期识别性能。


<details>
  <summary>Details</summary>
Motivation: 早期识别ICU高风险患者对医疗资源分配至关重要，现有方法在性能和实用性上存在不足。

Method: ALFIA结合LoRA适配器和自适应层加权机制，从BERT主干中融合多层语义特征，并在cw-24基准上训练。

Result: ALFIA在AUPRC上超越现有表格分类器，同时保持平衡的精确率-召回率，其嵌入特征可与其他模型结合进一步提升性能。

Conclusion: ALFIA通过直接处理临床文本，为重症监护提供了高效且稳健的风险分层工具。
...
### [335] [Agentic AI for Intent-Based Industrial Automation](https://arxiv.org/abs/2506.04980)
*Marcos Lima Romero,Ricardo Suyama*

Main category: cs.LG

TL;DR: 论文提出了一种将Agentic AI与意图导向范式结合的概念框架，旨在简化人机交互并支持工业5.0的目标。通过意图分解和代理协作，实现了预测性维护场景的可行性验证。


<details>
  <summary>Details</summary>
Motivation: 工业4.0的复杂性推动了简化人机交互的需求，同时工业5.0强调人本、可持续和韧性原则。Agentic AI的自主性和工具使用能力为此提供了新可能性。

Method: 提出了一种概念框架，将Agentic AI与意图导向范式结合，通过自然语言表达高层目标并分解为可执行组件，利用子代理和工具完成任务。

Result: 基于CMAPSS数据集和Google ADK的验证表明，该方法在预测性维护场景中可行，能够降低技术障碍并支持可扩展的意图驱动自动化。

Conclusion: 该框架展示了Agentic AI在工业自动化中的潜力，尽管存在数据质量和可解释性问题，但仍为意图驱动的自动化提供了可行路径。
...
### [336] [FPTQuant: Function-Preserving Transforms for LLM Quantization](https://arxiv.org/abs/2506.04985)
*Boris van Breugel,Yelysei Bondarenko,Paul Whatmough,Markus Nagel*

Main category: cs.LG

TL;DR: FPTQuant提出四种轻量级函数保留变换（FPTs），用于改进大语言模型（LLMs）的量化效果，显著提升效率且几乎不增加推理开销。
...
Conclusion: FPTQuant在精度与速度间取得优异平衡，是高效量化LLMs的有效方法。
...
### [337] [Cautious Optimism: A Meta-Algorithm for Near-Constant Regret in General Games](https://arxiv.org/abs/2506.05005)
*Ashkan Soleymani,Georgios Piliouras,Gabriele Farina*

Main category: cs.LG

TL;DR: 论文提出了一种名为“谨慎乐观”的框架，通过自适应调整学习步长，显著加速了广义博弈中的正则化学习，同时保持了无耦合性和最优的遗憾界。


<details>
  <summary>Details</summary>
Motivation: 现有研究在广义博弈中的无遗憾学习加速现象较为孤立，本文旨在证明这种现象的普遍性，并提出一种更通用的加速框架。

Method: 基于“谨慎乐观”框架，输入任何FTRL实例，输出加速的无遗憾学习算法，通过调整学习步长实现，计算开销极小。

Result: 在自对弈中实现了接近最优的O_T(log T)遗憾，同时在对抗场景中保持了最优的O(√T)遗憾。

Conclusion: 该框架为广义博弈中的快速学习提供了一条新路径，无需依赖单调步长，具有广泛适用性。
...
### [338] [Towards Reasonable Concept Bottleneck Models](https://arxiv.org/abs/2506.05014)
*Nektarios Kalampalikis,Kavya Gupta,Georgi Vitanov,Isabel Valera*

Main category: cs.LG

TL;DR: CREAM是一种新型概念瓶颈模型，通过显式编码概念间关系和使用正则化侧通道，实现高性能任务表现和高概念重要性。
...
Method: CREAM通过嵌入专家定义的概念间关系和任务关系，并使用正则化侧通道来增强性能。

Result: 实验表明，CREAM在保持高性能的同时主要依赖概念，且嵌入的关系易于干预和减少概念泄漏。

Conclusion: CREAM通过结构化和正则化设计，实现了任务表现和概念重要性的平衡，为模型干预提供了便利。
...
### [339] [Multi-Point Proximity Encoding For Vector-Mode Geospatial Machine Learning](https://arxiv.org/abs/2506.05016)
*John Collins*
...
<details>
  <summary>Details</summary>
Motivation: 矢量地理空间数据（点、线、面）需要编码才能用于传统机器学习和人工智能模型，现有方法如栅格化效果不佳。

Method: MPP编码通过计算形状与区域内一组参考点的缩放距离来表示形状的几何特征。

Result: MPP编码具有形状中心性和连续性，能区分空间对象的几何特征并高精度捕获空间关系，性能优于栅格化方法。
...
### [340] [Tuning the Right Foundation Models is What you Need for Partial Label Learning](https://arxiv.org/abs/2506.05027)
...
<details>
  <summary>Details</summary>
Motivation: 现实应用中部分标签学习面临监督不精确的挑战，现有研究多关注卷积神经网络，而忽视了基础模型的潜力。

Method: 通过综合评估11种基础模型和13种PLL方法，提出PartialCLIP框架，结合文本嵌入分类器初始化和零样本CLIP标签过滤。

Result: 发现基础模型显著提升PLL性能，方法间表现相似，对模糊标签稳定，但依赖模型选择和适应策略。
...
### [341] [Identifying and Understanding Cross-Class Features in Adversarial Training](https://arxiv.org/abs/2506.05032)
*Zeming Wei,Yiwen Guo,Yisen Wang*

Main category: cs.LG

TL;DR: 论文通过类间特征归因研究对抗训练（AT），发现跨类特征对AT的影响，并提供了理论证据。研究发现AT初期模型倾向于学习更多跨类特征，但随着训练深入，模型更依赖类特定特征。


<details>
  <summary>Details</summary>
Motivation: 探索对抗训练的机制和动态，特别是跨类特征在AT中的作用。

Method: 通过类间特征归因分析AT，使用合成数据模型验证跨类特征对鲁棒分类的作用，并在多种模型架构和设置下进行系统研究。

Result: AT初期模型学习更多跨类特征，达到最佳鲁棒性后，模型转向依赖类特定特征，导致鲁棒过拟合。
...
### [342] [TIMING: Temporality-Aware Integrated Gradients for Time Series Explanation](https://arxiv.org/abs/2506.05035)
*Hyeongwon Jang,Changhun Kim,Eunho Yang*

Main category: cs.LG

TL;DR: 论文提出了一种新的时间序列可解释性人工智能（XAI）方法TIMING，通过改进集成梯度（IG）方法，引入时间感知能力，并提出了新的评估指标CPD和CPP。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列XAI方法仅关注点级归因大小，忽略了方向性影响，导致关键点识别不准确。

Method: 提出TIMING方法，改进IG以引入时间感知能力，同时提出CPD和CPP评估指标。
...
### [343] [iN2V: Bringing Transductive Node Embeddings to Inductive Graphs](https://arxiv.org/abs/2506.05039)
*Nicolas Lell,Ansgar Scherp*

Main category: cs.LG

TL;DR: iN2V是一种改进的节点嵌入方法，将传统的transductive设置扩展到inductive设置，通过后处理计算新节点的嵌入，提升节点分类性能。


<details>
  <summary>Details</summary>
Motivation: 传统的节点嵌入方法（如node2vec）仅适用于transductive设置，无法处理训练时未见的节点。iN2V旨在解决这一问题，使其适用于inductive设置。

Method: iN2V结合后处理步骤计算新节点的嵌入，并修改原始N2V的训练过程以适应后处理。

Result: 实验表明，iN2V在多个基准数据集上平均提升节点分类性能1个百分点，最高可达6个百分点。

Conclusion: iN2V是一种通用且灵活的节点嵌入方法，可与其他嵌入方法结合，适用于inductive节点表示学习。
...
### [344] [Reliably detecting model failures in deployment without labels](https://arxiv.org/abs/2506.05047)
*Viet Nguyen Changjian Shui,Vijay Giri,Siddarth Arya,Amol Verma,Fahad Razak,Rahul G. Krishnan*

Main category: cs.LG

TL;DR: 论文提出了一种名为D3M的监控算法，用于检测模型部署后的性能退化（PDD），通过预测模型的分歧实现高效监控，并在非退化偏移下保持低误报率。


<details>
  <summary>Details</summary>
Motivation: 动态环境中数据分布会随时间变化，模型需要重新训练，但如何在不依赖标签的情况下判断何时需要重新训练仍是一个挑战。

Method: 基于预测模型的分歧设计D3M算法，提供样本复杂度界限以确保高真阳性率。

Result: 在标准基准和真实世界大规模内科数据集上的实验验证了D3M的有效性。

Conclusion: D3M可作为高风险机器学习管道的有效预警机制。
...
### [345] [NIMO: a Nonlinear Interpretable MOdel](https://arxiv.org/abs/2506.05059)
*Shijian Xu,Marcello Massimo Negri,Volker Roth*

Main category: cs.LG

TL;DR: NIMO结合神经网络与线性模型，通过非线性修正提升预测性能，同时保持线性系数的可解释性。


<details>
  <summary>Details</summary>
Motivation: 神经网络难以解释，而线性模型可解释但表达能力有限，需结合两者优势。

Method: 提出NIMO模型，利用神经网络学习线性预测的非线性修正，采用基于轮廓似然的优化算法，并引入稀疏约束。

Result: 实验表明，NIMO能恢复线性系数并显著提升预测准确性，优于其他混合方法。

Conclusion: NIMO在保持线性模型可解释性的同时，提高了预测性能，适用于多种回归和分类任务。
...
### [346] [UnHiPPO: Uncertainty-aware Initialization for State Space Models](https://arxiv.org/abs/2506.05065)
*Marten Lienen,Abdullah Saydemir,Stephan Günnemann*

Main category: cs.LG

TL;DR: 论文扩展了HiPPO理论，引入测量噪声，提出了一种不确定性感知的状态空间模型动态初始化方法。


<details>
  <summary>Details</summary>
Motivation: HiPPO框架假设数据无噪声，但实际中常存在噪声，因此需要改进。

Method: 将HiPPO重新表述为线性随机控制问题，数据作为噪声输出，推导出新的动态初始化方法。

Result: 实验表明，新方法提高了模型对训练和推理时噪声的鲁棒性。

Conclusion: 提出的方法有效解决了HiPPO的噪声问题，且不影响运行时性能。
...
### [347] [Semi-Implicit Variational Inference via Kernelized Path Gradient Descent](https://arxiv.org/abs/2506.05088)
*Tobias Pielok,Bernd Bischl,David Rügamer*

Main category: cs.LG

TL;DR: 提出了一种基于核化的KL散度估计器的半隐式变分推断方法，通过非参数平滑和重要性采样修正降低偏差，优化训练稳定性与效率。


<details>
  <summary>Details</summary>
Motivation: 传统半隐式变分推断（SIVI）在高维设置中因KL散度的高方差和偏差而训练困难，现有方法（如KSIVI）虽有效但训练成本较高。

Method: 提出核化KL散度估计器，结合非参数平滑和重要性采样修正，降低偏差并稳定训练；理论联系Stein变分梯度下降。

Result: 方法在性能和训练效率上优于或匹配现有SIVI方法，梯度方差更低，优化更稳定高效。

Conclusion: 新方法通过核化KL散度估计和重要性采样修正，显著提升了半隐式变分推断的稳定性和效率。
...
### [348] [Privacy Amplification Through Synthetic Data: Insights from Linear Regression](https://arxiv.org/abs/2506.05101)
*Clément Pierquin,Aurélien Bellet,Marc Tommasi,Matthieu Boussard*

Main category: cs.LG

TL;DR: 本文研究了合成数据在隐私放大方面的理论问题，通过线性回归框架揭示了种子控制和随机输入对隐私的影响。


<details>
  <summary>Details</summary>
Motivation: 合成数据可能因生成模型的隐藏而获得隐私放大，但缺乏严格的理论支持，本文旨在填补这一空白。

Method: 采用线性回归框架，分析种子控制和随机输入对合成数据隐私的影响。

Result: 发现种子控制下隐私泄露严重，而随机输入下隐私放大效果显著。
...
### [349] [Federated Isolation Forest for Efficient Anomaly Detection on Edge IoT Systems](https://arxiv.org/abs/2506.05138)
*Pavle Vasiljevic,Milica Matic,Miroslav Popovic*

Main category: cs.LG

TL;DR: 本文提出了一种基于隔离森林的联邦学习异常检测方法FLiForest，适用于边缘设备和IoT系统，实验显示其高准确率和低内存占用。
...
Method: 采用隔离森林算法，结合联邦学习框架（如Python TestBed和MicroPython TestBed），在MicroPython环境中实现温度异常检测。

Result: 实验结果显示，系统在区分正常与异常读数时准确率超过96%，异常检测精度达78%以上，训练时内存占用低于160 KB。

Conclusion: 该方法适用于资源受限环境，同时满足联邦学习的数据隐私和协作学习原则。
...
### [350] [Associative Memory and Generative Diffusion in the Zero-noise Limit](https://arxiv.org/abs/2506.05178)
...
<details>
  <summary>Details</summary>
Motivation: 探讨生成扩散模型与联想记忆模型的关联，揭示噪声水平变化时从生成到记忆的通用转变特性。

Method: 利用Morse-Smale动力系统分析梯度联想记忆模型和扩散模型，研究结构稳定性及其在零噪声极限下的表现。

Result: 在联想记忆和扩散模型的学习景观中，除孤立点外均表现出稳定性，噪声趋零时通过分岔描述定性转变。

Conclusion: 通过分岔序列表征学习景观，为经典和现代Hopfield网络（注意力机制）提供结构稳定性判据。
...
### [351] [TreeRPO: Tree Relative Policy Optimization](https://arxiv.org/abs/2506.05183)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于轨迹级奖励的方法无法为推理的中间步骤提供足够指导，限制了LLMs的推理能力。

Method: TreeRPO通过树采样直接估计步骤级奖励，并基于GRPO的组相对奖励机制计算细粒度奖励信号。

Result: 实验显示，TreeRPO将Qwen-2.5-Math的Pass@1准确率从19.0%提升至35.5%，同时响应长度减少18.1%。

Conclusion: TreeRPO通过细密奖励信号显著提升了LLMs的推理能力和效率，优于现有方法。
...
### [352] [Locality Preserving Markovian Transition for Instance Retrieval](https://arxiv.org/abs/2506.05196)
*Jifei Luo,Wenzheng Wu,Hantao Yao,Lu Yu,Changsheng Xu*

Main category: cs.LG

TL;DR: 论文提出LPMT框架，通过双向协作扩散和局部状态嵌入增强相似性传播，解决扩散重排序中信号衰减问题。


<details>
  <summary>Details</summary>
Motivation: 扩散重排序方法在数据流形建模中有效，但信号随距离衰减，限制了判别能力。

Method: 结合双向协作扩散（BCD）和局部状态嵌入（LSE），通过热力学马尔可夫转移（TMT）实现全局检索。

Result: 实验证明LPMT在实例检索任务中表现优异。

Conclusion: LPMT框架有效解决了信号衰减问题，提升了全局检索性能。
...
### [353] [Transformers Meet In-Context Learning: A Universal Approximation Theory](https://arxiv.org/abs/2506.05200)
*Gen Li,Yuchen Jiao,Yu Huang,Yuting Wei,Yuxin Chen*

Main category: cs.LG

TL;DR: 论文提出了一种基于通用逼近理论的方法，解释了大语言模型如何通过上下文学习完成任务，无需微调或参数更新。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型（如Transformer）如何仅通过少量输入输出示例进行上下文学习，而无需额外训练。

Method: 通过构建一个Transformer模型，证明其能够在给定少量上下文示例的情况下可靠预测，基于通用函数逼近理论。

Result: 证明了Transformer能够动态适应上下文示例，并学习通用表示，适用范围远超凸问题和线性函数类。

Conclusion: 该方法为理解Transformer的上下文学习能力提供了新视角，扩展了其应用范围。
...
### [354] [Mitigating Degree Bias Adaptively with Hard-to-Learn Nodes in Graph Contrastive Learning](https://arxiv.org/abs/2506.05214)
...
### [355] [Learning Theory of Decentralized Robust Kernel-Based Learning Algorithm](https://arxiv.org/abs/2506.05215)
*Zhan Yu*

Main category: cs.LG

TL;DR: 提出了一种基于RKHS的去中心化鲁棒核学习算法，通过图网络实现统一框架，区别于现有分治法，提供收敛分析和最优学习率。


<details>
  <summary>Details</summary>
Motivation: 现有分布式鲁棒核学习多为分治法，缺乏统一框架，需研究去中心化方法以提升鲁棒性和收敛性。

Method: 利用图网络和鲁棒损失函数$\mathcal{L}_\sigma$，结合核积分算子技术，分析局部估计器的收敛性。

Result: 算法在均方距离、RKHS范数和泛化误差上达到最优学习率，参数$\sigma$增强鲁棒性并确保收敛。

Conclusion: 去中心化算法在鲁棒性和收敛性上表现优越，参数选择和样本量规则对性能至关重要。
...
### [356] [A Unified Framework for Provably Efficient Algorithms to Estimate Shapley Values](https://arxiv.org/abs/2506.05216)
...
TL;DR: 论文提出了一种统一的框架，为KernelSHAP及相关估计器提供了非渐近理论保证，并通过实验验证了其在小到高维数据集上的有效性。


<details>
  <summary>Details</summary>
Motivation: 计算精确的Shapley值困难，现有随机化估计器缺乏理论保证，尤其是KernelSHAP。

Method: 提出一个统一框架，涵盖KernelSHAP及相关估计器，证明其非渐近理论保证，并改进实现以提高可扩展性。

Result: 在中小维数据集上验证了方法的低均方误差，并在高维数据集（如MNIST、CIFAR10）上优于KernelSHAP库。

Conclusion: 该框架首次为KernelSHAP提供理论保证，揭示了现有估计器间的权衡，并通过实验证明了其优越性。
...
### [357] [Diagonal Batching Unlocks Parallelism in Recurrent Memory Transformers for Long Contexts](https://arxiv.org/abs/2506.05229)
*Danil Sivtsov,Ivan Rodkin,Gleb Kuzmin,Yuri Kuratov,Ivan Oseledets*

Main category: cs.LG

TL;DR: Diagonal Batching 是一种调度方案，通过并行处理 RMT 中的片段，消除了顺序执行的瓶颈，显著提升了长上下文推理的效率。


<details>
  <summary>Details</summary>
Motivation: Transformer 模型在长上下文推理中因时间和内存复杂度高而表现不佳，RMT 虽降低了复杂度，但顺序执行导致性能瓶颈。

Method: 引入 Diagonal Batching，一种运行时计算重排序技术，无需重新训练即可在 RMT 中实现并行处理。

Result: 在 LLaMA-1B ARMT 模型上，Diagonal Batching 实现了 3.3 倍速度提升，并降低了推理成本和延迟。

Conclusion: Diagonal Batching 使 RMT 成为长上下文应用的实际解决方案。
...
### [358] [Progressive Tempering Sampler with Diffusion](https://arxiv.org/abs/2506.05231)
*Severi Rissanen,RuiKang OuYang,Jiajun He,Wenlin Chen,Markus Heinonen,Arno Solin,José Miguel Hernández-Lobato*

Main category: cs.LG

TL;DR: 提出了一种结合扩散模型和并行回火（PT）的新方法PTSD，显著提升了神经采样器的目标评估效率。


<details>
  <summary>Details</summary>
Motivation: 现有神经采样器在目标评估效率上仍落后于并行回火（PT），且PT生成依赖样本且计算成本高。

Method: PTSD通过在不同温度下顺序训练扩散模型，结合PT优势，并引入新方法生成近似低温样本。

Result: PTSD显著提升目标评估效率，优于基于扩散的神经采样器。

Conclusion: PTSD结合PT和扩散模型优势，解决了现有方法的不足，生成高质量样本。
...
### [359] [MesaNet: Sequence Modeling by Locally Optimal Test-Time Training](https://arxiv.org/abs/2506.05233)
...
TL;DR: 论文提出了一种基于在线学习规则的稳定且可并行化的Mesa层，通过优化上下文损失提升语言模型性能，但增加了推理计算成本。


<details>
  <summary>Details</summary>
Motivation: 当前序列建模主要依赖Transformer，但其推理时内存和计算成本线性增长。研究旨在通过优化在线学习规则，开发高效且性能更强的RNN模型。

Method: 引入数值稳定、可并行化的Mesa层，通过快速共轭梯度求解器优化上下文损失，并在十亿参数规模的语言模型中进行实验。

Result: 实验表明，该方法在语言建模困惑度和长上下文任务性能上优于先前RNN模型，但需额外推理计算成本。

Conclusion: 通过增加推理计算优化序列问题，可显著提升模型性能，与近期增加测试时计算以提升性能的趋势一致。
...
### [360] [Evaluating Sparse Autoencoders: From Shallow Design to Matching Pursuit](https://arxiv.org/abs/2506.05239)
*Valérie Costa,Thomas Fel,Ekdeep Singh Lubana,Bahareh Tolooshams,Demba Ba*

Main category: cs.LG

TL;DR: 本文评估了稀疏自编码器（SAE）在MNIST数据集上的表现，发现其浅层架构依赖准正交性假设，限制了提取相关特征的能力。为此，作者提出了一种多迭代SAE（MP-SAE），通过残差引导提取相关特征，并在手写数字生成等分层场景中实现重建效果的单调提升。


<details>
  <summary>Details</summary>
Motivation: 稀疏自编码器（SAEs）是解释神经网络表示的重要工具，但其浅层架构依赖准正交性假设，限制了提取相关特征的能力。本文旨在解决这一问题。

Method: 作者提出了一种多迭代稀疏自编码器（MP-SAE），通过展开匹配追踪（Matching Pursuit）实现残差引导的特征提取，适用于分层场景。

Result: 实验表明，MP-SAE能够提取相关特征，并在手写数字生成等任务中实现重建效果的单调提升。

Conclusion: MP-SAE突破了浅层SAE的局限性，为提取相关特征提供了有效方法。
...
### [361] [Aligning Latent Spaces with Flow Priors](https://arxiv.org/abs/2506.05240)
...
Conclusion: 框架为潜在空间对齐提供了新思路，理论和实验均支持其有效性。
...
### [362] [On the Convergence of Gradient Descent on Learning Transformers with Residual Connections](https://arxiv.org/abs/2506.05249)
*Zhen Qin,Jinxin Zhou,Zhihui Zhu*

Main category: cs.LG

TL;DR: 本文分析了单层Transformer的训练动态，证明了梯度下降在适当初始化下具有线性收敛速率，并揭示了残差连接对优化稳定性的促进作用。
...
Conclusion: 理论分析和实证结果均表明，残差连接对Transformer的收敛稳定性具有积极作用，且结论可推广到多层架构。
...
### [363] [Conservative classifiers do consistently well with improving agents: characterizing statistical and online learning](https://arxiv.org/abs/2506.05252)
*Dravyansh Sharma,Alec Sun*

Main category: cs.LG

TL;DR: 本文研究了机器学习中分类代理的真实改进行为对学习能力的影响，提出了新的学习能力表征方法，并解决了Attias等人提出的开放性问题。


<details>
  <summary>Details</summary>
Motivation: 研究分类代理在真实改进行为下对学习算法的影响，探索更自然的学习场景。

Method: 引入非对称变体的最小一致概念类，研究欧几里得球改进集下的学习能力，并分析不同噪声模型下的泛化误差。

Result: 在可实现设置下精确表征了学习能力，解决了Attias等人的开放性问题，并在噪声模型下实现了更低的泛化误差。

Conclusion: 本文扩展了学习能力的表征范围，为实际应用中的机器学习提供了更可靠的理论基础。
...
### [364] [Learning long range dependencies through time reversal symmetry breaking](https://arxiv.org/abs/2506.05259)
*Guillaume Pourcel,Maxence Ernoult*

Main category: cs.LG

TL;DR: 论文提出了一种名为RHEL的算法，用于训练基于物理原理的深度状态空间模型，其梯度计算高效且无需显式雅可比矩阵，性能与BPTT相当。


<details>
  <summary>Details</summary>
Motivation: 受物理系统实现启发，研究旨在开发一种遵循物理原理的学习算法，以高效模拟和设计动态系统。

Method: 提出RHEL算法，通过物理轨迹的有限差分计算梯度，无需显式雅可比矩阵，并扩展到离散时间版本和层次化HRUs（HSSMs）。

Result: RHEL在多种时间序列任务中表现与BPTT相当，支持长序列（~50k）的分类和回归。

Conclusion: RHEL为设计可扩展、高效的自学习物理系统提供了新途径，适用于序列建模。
...
### [365] [Tight analyses of first-order methods with error feedback](https://arxiv.org/abs/2506.05271)
*Daniel Berg Thomsen,Adrien Taylor,Aymeric Dieuleveut*

Main category: cs.LG

TL;DR: 本文分析了分布式学习中通信压缩的两种误差反馈方法（EF和EF21），通过找到最优Lyapunov函数，提供了严格的收敛率分析和比较。


<details>
  <summary>Details</summary>
Motivation: 分布式学习中通信压缩是主要瓶颈，误差反馈方法（如EF和EF21）用于缓解压缩导致的收敛性能下降。本文旨在提供这两种方法的严格分析和比较。

Method: 通过寻找最优Lyapunov函数，分析EF和EF21的收敛率，并与压缩梯度下降进行公平比较。

Result: 找到了两种方法的最优收敛率，并提供了匹配的下界，揭示了其性能差异。

Conclusion: 本文为误差反馈方法提供了严格的理论分析，有助于理解其性能差异和优化方向。
...
### [366] [How to Unlock Time Series Editing? Diffusion-Driven Approach with Multi-Grained Control](https://arxiv.org/abs/2506.05276)
*Hao Yu,Chu Xin Cheng,Runlong Yu,Yuyang Ye,Shiwei Tong,Zhaofeng Liu,Defu Lian*

Main category: cs.LG

TL;DR: CocktailEdit框架通过点级和段级控制实现时间序列的精确编辑，保持时间一致性，适用于多种扩散模型。
...
Method: 结合置信度加权的锚点控制（点级）和基于分类器的控制（段级），在去噪推理阶段实现精确编辑。

Result: 实验证明该框架在多种数据集和模型上有效，保持了时间一致性。

Conclusion: CocktailEdit为时间序列生成和编辑提供了灵活解决方案，弥合了生成模型与实际需求之间的差距。
...
### [367] [Fast-DataShapley: Neural Modeling for Training Data Valuation](https://arxiv.org/abs/2506.05281)
*Haifeng Sun,Yu Xiong,Runze Wu,Xinyu Cai,Changjie Fan,Lan Zhang,Xiang-Yang Li*

Main category: cs.LG

TL;DR: 论文提出Fast-DataShapley，一种单次训练方法，通过加权最小二乘特性训练可复用的解释模型，避免重复训练，显著提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 在AI行业中，训练数据的价值和版权保护至关重要。现有基于Shapley值的方法计算复杂度高且需重复训练，成本难以接受。

Method: 利用Shapley值的加权最小二乘特性训练可复用的解释模型，并提出三种方法降低训练开销（近似计算效用函数和分组计算训练数据）。

Result: 实验表明，性能提升2.5倍以上，训练速度提高两个数量级。

Conclusion: Fast-DataShapley高效解决了Shapley值计算的高复杂度问题，为数据贡献评估提供了实用工具。
...
### [368] [Learning Beyond Experience: Generalizing to Unseen State Space with Reservoir Computing](https://arxiv.org/abs/2506.05292)
...
Result: 实验表明，该方法能够在未观测到的吸引域中捕捉系统行为，实现跨域泛化。

Conclusion: 储层计算结合多轨迹训练方案，为动态系统建模提供了一种高效且泛化能力强的解决方案。
...
### [369] [A Smooth Sea Never Made a Skilled $\texttt{SAILOR}$: Robust Imitation via Learning to Search](https://arxiv.org/abs/2506.05294)
*Arnav Kumar Jain,Vibhakar Mohta,Subin Kim,Atiksh Bhardwaj,Juntao Ren,Yunhai Feng,Sanjiban Choudhury,Gokul Swamy*

Main category: cs.LG

TL;DR: 论文提出了一种名为SAILOR的方法，通过学习搜索（L2S）从专家演示中学习，以解决行为克隆（BC）在模仿学习中的局限性。SAILOR结合了世界模型和奖励模型，能够在测试时规划匹配专家结果，即使出现错误也能恢复。实验表明，SAILOR在多个视觉操作任务中优于基于BC的扩散策略。


<details>
  <summary>Details</summary>
Motivation: 行为克隆（BC）方法仅能教授代理在专家访问的状态下执行专家行为，一旦代理犯错并偏离演示支持区域，无法恢复。因此，需要一种方法能够独立推理并实现专家目标，即使在未见状态下。

Method: 提出SAILOR方法，通过学习搜索（L2S）从专家演示中学习，包括世界模型和奖励模型。通过算法和设计决策的组合，实现稳定且高效的恢复行为学习，无需额外人工修正。

Result: 在多个视觉操作任务中，SAILOR表现优于基于BC的扩散策略，即使BC数据量增加5-10倍仍存在性能差距。SAILOR能够识别细微错误并抵抗奖励黑客攻击。

Conclusion: SAILOR通过L2S方法有效解决了BC的局限性，能够在未见状态下独立推理并恢复，展示了在模仿学习中的优越性能。
...
### [370] [Sample Complexity and Representation Ability of Test-time Scaling Paradigms](https://arxiv.org/abs/2506.05295)
*Baihe Huang,Shanda Li,Tianhao Wu,Yiming Yang,Ameet Talwalkar,Kannan Ramchandran,Michael I. Jordan,Jiantao Jiao*

Main category: cs.LG

TL;DR: 论文研究了测试时策略的样本效率，理论分析了自洽性、最佳-n和自校正策略的差异，并证明了自校正方法在多任务中的有效性。


<details>
  <summary>Details</summary>
Motivation: 尽管测试时策略在大型语言模型中取得了成功，但其样本效率的理论理解仍有限。

Method: 理论分析自洽性和最佳-n策略的样本需求，提出自校正方法的表达性，并通过实验验证。

Result: 自洽性需要Θ(1/Δ²)样本，最佳-n仅需Θ(1/Δ)；自校正方法使Transformer能模拟多任务学习。
...
### [371] [Power Law Guided Dynamic Sifting for Efficient Attention](https://arxiv.org/abs/2506.05300)
...
<details>
  <summary>Details</summary>
Motivation: 解决GPU上大语言模型推理时内存带宽限制问题，特别是HBM与SRAM间数据传输的瓶颈。

Method: 用基于阈值的元素级过滤操作替代昂贵的top-k操作，动态估计每步生成的阈值以减少数据传输。

Result: SiftAttention在减少内存带宽使用的同时，比现有近似注意力方法更好地保持了模型质量。

Conclusion: SiftAttention是一种高效且有效的近似注意力方法，适用于GPU上的大语言模型推理。
...
### [372] [Learning normalized image densities via dual score matching](https://arxiv.org/abs/2506.05310)
*Florentin Guth,Zahra Kadkhodaie,Eero P Simoncelli*

Main category: cs.LG

TL;DR: 论文提出了一种基于扩散生成模型的新框架，用于学习归一化能量模型，通过双重评分匹配目标优化能量网络，在ImageNet64数据集上取得了与现有技术相当的交叉熵值。


<details>
  <summary>Details</summary>
Motivation: 学习概率模型在机器学习中至关重要，但由于维度灾难而困难重重。本文旨在通过改进评分网络架构，学习归一化能量模型以解决这一问题。

Method: 修改评分网络架构以计算能量，同时保留其归纳偏差。通过去噪目标和新的次要目标优化能量网络，确保噪声水平间能量一致且归一化。
...
### [373] [Improving Data Efficiency for LLM Reinforcement Fine-tuning Through Difficulty-targeted Online Data Selection and Rollout Replay](https://arxiv.org/abs/2506.05316)
*Yifan Sun,Jingyan Shen,Yibin Wang,Tianyu Chen,Zhendong Wang,Mingyuan Zhou,Huan Zhang*

Main category: cs.LG

TL;DR: 本文提出两种技术（难度导向的在线数据选择和回放机制）以提高大型语言模型（LLM）强化学习微调的数据效率，显著减少训练时间。
...
Method: 引入自适应难度概念指导数据选择，开发注意力框架估计难度；提出回放机制重用近期数据。

Result: 实验显示方法减少25%-65%训练时间，性能与GRPO相当。

Conclusion: 所提方法高效且实用，显著提升LLM强化学习微调效率。
...
### [374] [LSM-2: Learning from Incomplete Wearable Sensor Data](https://arxiv.org/abs/2506.05321)
...
TL;DR: LSM-2 with AIM是一种自监督学习模型，能直接从缺失数据中学习，无需显式填补。


<details>
  <summary>Details</summary>
Motivation: 可穿戴传感器数据常缺失，传统SSL模型需完整数据输入，LSM-2旨在解决这一问题。

Method: 采用自适应和继承掩码（AIM），通过可学习掩码标记处理缺失数据。

Result: 在40M小时多模态数据上预训练，LSM-2在分类、回归和生成任务中表现最佳。

Conclusion: AIM适用于真实世界数据，尤其在临床相关缺失场景中表现优异。
...
### [375] [Seeing the Invisible: Machine learning-Based QPI Kernel Extraction via Latent Alignment](https://arxiv.org/abs/2506.05325)
*Yingshuai Ji,Haomin Zhuang,Matthew Toole,James McKenzie,Xiaolong Liu,Xiangliang Zhang*

Main category: cs.LG

TL;DR: 本文提出了一种基于AI的QPI核提取框架，通过两步学习策略解决多散射图像中单散射QPI模式的逆问题。


<details>
  <summary>Details</summary>
Motivation: QPI成像是研究量子材料电子结构的有力工具，但从多散射图像中提取单散射QPI模式（即核）是一个病态逆问题。

Method: 采用两步学习策略：第一步训练变分自编码器学习散射核的紧凑潜在空间；第二步通过专用编码器将QPI观测的潜在表示与预学习核对齐。

Result: 实验结果表明，该方法显著提高了提取精度，并对未见核具有更好的泛化能力。

Conclusion: 该AI框架为QPI核提取提供了稳健且高效的解决方案，适用于复杂散射条件。
...
### [376] [Kinetics: Rethinking Test-Time Scaling Laws](https://arxiv.org/abs/2506.05333)
*Ranajoy Sadhukhan,Zhuoming Chen,Haizhong Zheng,Yang Zhou,Emma Strubell,Beidi Chen*

Main category: cs.LG

TL;DR: 研究发现，测试时的小模型效果被高估，提出新的Kinetics Scaling Law，强调稀疏注意力的重要性。
...
### [377] [Inference-Time Hyper-Scaling with KV Cache Compression](https://arxiv.org/abs/2506.05345)
*Adrian Łańcucki,Konrad Staniszewski,Piotr Nawrot,Edoardo M. Ponti*

Main category: cs.LG

TL;DR: 论文提出了一种动态内存稀疏化（DMS）方法，通过压缩KV缓存实现推理时超扩展，提升推理准确率而不增加计算成本。


<details>
  <summary>Details</summary>
Motivation: Transformer LLMs的推理成本受限于KV缓存大小，而非生成令牌数量。通过压缩KV缓存，可以在相同计算预算下生成更多令牌，提高推理准确率。

Method: 引入动态内存稀疏化（DMS），延迟令牌驱逐并隐式合并表示，仅需1K训练步骤即可实现8倍压缩，同时保持比无训练稀疏注意力更高的准确率。

Result: 在多个LLM家族中验证了DMS的有效性，例如Qwen-R1 32B在AIME 24、GPQA和LiveCodeBench上的准确率分别平均提升9.1、7.6和9.6分。

Conclusion: DMS为推理时超扩展提供了实用方法，显著提升了LLMs的推理准确率，同时保持计算预算不变。
...
### [378] [Exploring Diffusion Transformer Designs via Grafting](https://arxiv.org/abs/2506.05340)
...
<details>
  <summary>Details</summary>
Motivation: 传统模型架构设计需要昂贵的预训练评估，限制了架构探索。受软件开发的启发，研究是否可以利用预训练模型研究新架构。

Method: 提出“嫁接”方法，通过替换操作符（如注意力、卷积）和配置（如深度、宽度）来编辑预训练DiTs，构建混合设计。

Result: 混合设计在少量计算预算下表现良好（FID: 2.38-2.64），并在文本到图像模型中实现1.43倍加速。
...
### [379] [Exploring bidirectional bounds for minimax-training of Energy-based models](https://arxiv.org/abs/2506.04609)
...
<details>
  <summary>Details</summary>
Motivation: 传统EBM训练因仅优化下界而不稳定，需要更稳定的训练方法。

Method: 提出四种不同的对数似然边界（基于生成器Jacobian奇异值、互信息、梯度惩罚和扩散过程），并设计评估算法。

Result: 双向边界方法显著提高了EBM训练的稳定性，并实现了高质量的密度估计和样本生成。

Conclusion: 双向边界是训练EBM的有效方法，具有稳定性和性能优势。
...
### [380] [FlashDMoE: Fast Distributed MoE in a Single Kernel](https://arxiv.org/abs/2506.04667)
*Osayamen Jonathan Aimuyo,Byungsoo Oh,Rachee Singh*

Main category: cs.DC

TL;DR: FlashDMoE是一种完全GPU驻留的MoE算子，通过融合专家计算和GPU间通信，显著提升性能。
...
Method: 开发FlashDMoE，采用单一持久GPU内核，实现细粒度流水线和设备启动通信。

Result: 在8-H100 GPU节点上，FlashDMoE实现了6倍延迟降低、5.7倍吞吐量提升和9倍GPU利用率提高。
...
### [381] [Diffusion Transformer-based Universal Dose Denoising for Pencil Beam Scanning Proton Therapy](https://arxiv.org/abs/2506.04467)
...
TL;DR: 提出了一种基于扩散变换器的去噪框架，用于快速生成高质量质子治疗剂量图，解决了低统计量蒙特卡罗模拟的精度问题。


<details>
  <summary>Details</summary>
Motivation: IMPT对解剖变化敏感，需要快速准确的剂量计算，但减少粒子数会降低蒙特卡罗模拟的精度。

Method: 使用扩散变换器去噪框架，以噪声剂量图和CT图像为输入，高统计量剂量图为目标，结合多种损失函数训练模型。

Result: 模型在多个部位（头颈、肺、乳腺、前列腺）的MAE和Gamma通过率表现优异，DVH指标接近真实值。
...
### [382] [Trustworthiness Preservation by Copies of Machine Learning Systems](https://arxiv.org/abs/2506.05203)
*Leonardo Ceragioli,Giuseppe Primiero*

Main category: cs.LO

TL;DR: 本文提出了一种计算工具，用于验证从已知行为原始系统复制的复杂系统的可信赖性，定义了四种不同的可信赖性概念，并研究了它们之间的关系。
...
Method: 引入了一种演算来建模和验证数据和概率复杂查询，定义了四种可信赖性概念（Justifiably, Equally, Weakly, Almost Trustworthy），并通过分析复制系统与原始系统的行为关系来检查这些概念。

Result: 研究了这些可信赖性概念之间的关系及其在逻辑操作下的组合方式。

Conclusion: 提供了一个计算工具，用于检查从已知行为原始系统复制的复杂系统的可信赖性。
...
### [383] [Robust Moment Identification for Nonlinear PDEs via a Neural ODE Approach](https://arxiv.org/abs/2506.05245)
*Shaoxuan Chen,Su Yang,Panayotis G. Kevrekidis,Wei Zhu*

Main category: nlin.PS

TL;DR: 提出了一种基于神经ODE的数据驱动框架，用于从PDE系统中学习降阶矩动力学，适用于稀疏和不规则数据。


<details>
  <summary>Details</summary>
Motivation: 传统方法如SINDy需要密集数据且对噪声敏感，而神经ODE能直接从稀疏和不规则时间序列中学习，更具鲁棒性。

Method: 使用神经ODE直接建模矩轨迹，并在无解析闭合时引入基于Stiefel流形优化的数据驱动坐标变换策略。

Result: 在非线性薛定谔方程和Fisher-KPP反应-扩散系统中，框架能准确恢复或近似矩动力学，且外推精度优于物理专家模型。

Conclusion: 神经ODE框架是学习复杂PDE系统中可解释、低维矩动力学的强大工具，适用于数据有限场景。
...
### [384] [kTULA: A Langevin sampling algorithm with improved KL bounds under super-linear log-gradients](https://arxiv.org/abs/2506.04878)
...
<details>
  <summary>Details</summary>
Motivation: 深度学习应用中，全局Lipschitz连续性条件常不满足，需研究超线性增长对数梯度的分布采样问题。

Method: 提出kTULA算法，基于驯服Langevin动力学，并分析其收敛性。

Result: 在KL散度上获得最佳收敛率$2-\overline{\epsilon}$，显著优于现有结果，并进一步改进了Wasserstein-2距离的误差界。

Conclusion: kTULA在采样高维双阱势分布和神经网络优化问题中表现良好，理论结果为其性能提供了保证。
...
### [385] [Estimating properties of a homogeneous bounded soil using machine learning models](https://arxiv.org/abs/2506.04256)
*Konstantinos Kalimeris,Leonidas Mindrinos,Nikolaos Pallikarakis*

Main category: physics.geo-ph

TL;DR: 利用机器学习模型从水分测量数据中估计土壤性质，支持向量机和神经网络表现最佳。


<details>
  <summary>Details</summary>
Motivation: 研究如何通过水分测量数据准确估计土壤性质，尤其是扩散率$D$和水力传导率$K$。

Method: 使用Fokas方法模拟数据，探索多种机器学习模型（如支持向量机和神经网络）进行参数识别。

Result: 支持向量机和神经网络在完整、噪声和有限数据条件下表现最稳健，扩散率$D$的预测更准确。

Conclusion: 支持向量机和神经网络是估计土壤性质的有效工具，扩散率预测优于水力传导率。
...
### [386] [Interpretable LLMs for Credit Risk: A Systematic Review and Taxonomy](https://arxiv.org/abs/2506.04290)
*Muhammed Golec,Maha AlabdulJalil*

Main category: q-fin.RM

TL;DR: 本文系统综述了基于大语言模型（LLM）的信用风险评估方法，提出了分类法，涵盖模型架构、数据类型、可解释性机制和应用领域，并指出未来趋势和研究空白。


<details>
  <summary>Details</summary>
Motivation: 随着LLM的发展，其在金融文本分析中的应用日益广泛，但缺乏系统性的综述和分类法。本文旨在填补这一空白。

Method: 通过PRISMA研究策略筛选60篇相关论文（2020-2025年），分析模型架构、数据类型、可解释性机制和应用领域。

Result: 提出了一个分类法，总结了LLM在信用风险评估中的主要方法、数据来源和解释机制，并识别了未来研究方向。

Conclusion: 本文为AI和金融领域的研究者提供了参考，并指出了LLM在信用评分系统中的潜力和挑战。
...
### [387] [Was Residual Penalty and Neural Operators All We Needed for Solving Optimal Control Problems?](https://arxiv.org/abs/2506.04742)
*Oliver G. S. Lundqvist,Fabricio Oliveira*

Main category: math.OC

TL;DR: 论文提出了一种简化神经网络解决最优控制问题的方法，通过分离训练过程和优化问题，证明了无需在训练中包含成本函数。


<details>
  <summary>Details</summary>
Motivation: 传统方法在训练神经网络时需考虑成本函数，增加了复杂性。本文旨在简化架构，提高效率。

Method: 使用DeepONet等简单神经算子架构，结合无约束优化方法，通过微分方程残差惩罚项和自动微分计算控制梯度。

Result: 在九个不同最优控制问题上验证了方法的准确性和一致性。

Conclusion: 该方法简化了最优控制问题的求解流程，展示了单一训练阶段后多问题优化的可行性。
...
### [388] [Non-linear Multi-objective Optimization with Probabilistic Branch and Bound](https://arxiv.org/abs/2506.04554)
*Hao Huang,Zelda B. Zabinsky*

Main category: math.OC

TL;DR: 提出了一种名为MOPBnB(so)的多目标仿真优化算法，用于近似随机多目标优化问题的Pareto最优集和相关高效前沿。
...
Method: MOPBnB(so)在每个解处仅评估一次噪声函数，并利用邻近解估计目标函数。

Result: 算法在有限时间内能捕获Pareto最优集，且数值结果显示其计算资源消耗远低于多副本变体，并优于NSGA-II。

Conclusion: MOPBnB(so)是一种高效且收敛性良好的算法，适用于随机多目标优化问题。
...
### [389] [SSIMBaD: Sigma Scaling with SSIM-Guided Balanced Diffusion for AnimeFace Colorization](https://arxiv.org/abs/2506.04283)
...
<details>
  <summary>Details</summary>
Motivation: 传统方法依赖预定义噪声调度，可能影响感知一致性，新方法旨在解决这一问题。

Method: 采用连续时间扩散模型，引入SSIMBaD技术，通过sigma空间变换对齐感知退化。

Result: 在大规模动漫面部数据集上表现优于现有方法，像素精度和感知质量均提升。

Conclusion: SSIMBaD框架在动漫面部上色中实现了更平衡和忠实的结果，代码已开源。
...
### [390] [Handle-based Mesh Deformation Guided By Vision Language Model](https://arxiv.org/abs/2506.04562)
*Xingpeng Sun,Shiyang Jia,Zherong Pan,Kui Wu,Aniket Bera*

Main category: cs.GR

TL;DR: 提出了一种无需训练、基于手柄的网格变形方法，利用视觉语言模型（VLM）通过提示工程选择变形子部分和手柄，并通过多视角投票减少不确定性。
...
Method: 通过锥形奇点检测识别手柄，利用VLM选择变形部分和手柄，并通过多视角投票聚合结果以减少不确定性。

Result: 在多个基准测试中，方法生成的变形更符合用户意图，且引入的失真较低。

Conclusion: 该方法无需训练、高度自动化，并能持续提供高质量的网格变形。
...
### [391] [VoxDet: Rethinking 3D Semantic Occupancy Prediction as Dense Object Detection](https://arxiv.org/abs/2506.04623)
*Wuyang Li,Zhu Yu,Alexandre Alahi*

Main category: cs.GR

TL;DR: VoxDet提出了一种实例中心化的3D语义占用预测框架，通过解耦偏移回归和语义预测任务，显著提升了实例级完整性和性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将3D语义占用预测视为密集分割任务，忽略了实例级的区分性，导致实例不完整和相邻模糊问题。

Method: VoxDet通过Voxel-to-Instance（VoxNT）技巧将体素级标签转换为实例级偏移标签，并采用空间解耦的体素编码器和任务解耦的密集预测器进行实例感知预测。
...
### [392] [A Fast Unsupervised Scheme for Polygonal Approximation](https://arxiv.org/abs/2506.04664)
*Bimal Kumar Ray*

Main category: cs.GR

TL;DR: 提出了一种快速无监督的多边形逼近闭合数字曲线的方法，速度快于现有技术，且在Rosin度量与美观性上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有方法在速度和精度上存在不足，需一种更高效且美观的逼近方案。

Method: 分三阶段：初始分割检测高曲率顶点；迭代顶点插入补充低曲率区域；迭代合并去除冗余顶点，最后调整顶点优化美观。

Result: 速度快于现有技术，Rosin度量和美观性表现优异，对几何变换鲁棒。

Conclusion: 该方法高效、美观且鲁棒，适用于数字曲线逼近。
...
### [393] [What does making money have to do with crime?: A dive into the National Crime Victimization survey](https://arxiv.org/abs/2506.04240)
*Sydney Anuyah*

Main category: physics.soc-ph

TL;DR: 本文利用1992至2022年的国家犯罪受害调查，研究了收入、教育、就业和人口因素如何影响犯罪类型（暴力与财产犯罪）。通过逻辑回归模型和F1评分，发现社会经济因素（如收入和教育）显著降低暴力犯罪风险，而年轻男性、少数族裔则面临更高风险。郊区模型预测效果最佳，农村地区犯罪仍难以预测。


<details>
  <summary>Details</summary>
Motivation: 探讨社会经济和人口因素对犯罪类型的影响，为针对性干预措施提供依据。

Method: 使用平衡分类和逻辑回归模型（F1评分评估），分两组模型分析：社会经济因素（Group A）和加入人口因素的模型（Group B）。

Result: 高收入和教育降低暴力犯罪风险；年轻男性、少数族裔风险更高。郊区预测效果最佳（准确率0.607，F1 0.590），农村犯罪难以预测。
...
### [394] [Deep Learning-Based Breast Cancer Detection in Mammography: A Multi-Center Validation Study in Thai Population](https://arxiv.org/abs/2506.03177)
...
<details>
  <summary>Details</summary>
Motivation: 乳腺癌筛查的准确性和效率是临床实践中的关键需求，本研究旨在开发一种高性能的深度学习系统，以辅助医生进行乳腺X光片的解读。

Method: 采用改进的EfficientNetV2架构，结合增强的注意力机制，训练模型使用来自泰国医疗中心的乳腺X光片，并在多个数据集上进行验证。

Result: 模型在癌症检测上的AUROC分别为0.89、0.96和0.94；在病灶定位和临床验证中表现出色，与放射科医生的分类和定位一致性高，且临床接受度良好。

Conclusion: 该系统在乳腺X光片解读中表现出高效性和实用性，有望提升乳腺癌筛查的临床工作流程。
...
### [395] [Gradient Inversion Attacks on Parameter-Efficient Fine-Tuning](https://arxiv.org/abs/2506.04453)
*Hasin Us Sami,Swapneel Sen,Amit K. Roy-Chowdhury,Srikanth V. Krishnamurthy,Basak Guler*

Main category: eess.IV

TL;DR: 论文研究了联邦学习中参数高效微调（PEFT）的隐私泄露问题，展示了攻击者如何通过恶意设计的预训练模型和适配器模块重构用户数据。


<details>
  <summary>Details</summary>
Motivation: 探讨在联邦学习中，参数高效微调（PEFT）机制可能导致的隐私泄露问题，揭示潜在的安全风险。

Method: 通过梯度反转攻击，利用可访问的适配器梯度重构用户的本地数据样本。
...
### [396] [A Poisson-Guided Decomposition Network for Extreme Low-Light Image Enhancement](https://arxiv.org/abs/2506.04470)
*Isha Rao,Sanjay Ghosh*

Main category: eess.IV

TL;DR: 论文提出了一种轻量级的深度学习方法，结合Retinex分解与泊松去噪，用于极端低光条件下的图像去噪和增强。


<details>
  <summary>Details</summary>
Motivation: 传统的高斯噪声假设在低光成像中不适用，泊松噪声更符合实际场景。

Method: 采用编码器-解码器网络，结合Retinex分解和泊松去噪损失，无需反射率和光照先验。

Result: 实验表明，该方法显著提升了低光条件下的可见性和亮度，同时保持图像结构和颜色一致性。
...
### [397] [DACN: Dual-Attention Convolutional Network for Hyperspectral Image Super-Resolution](https://arxiv.org/abs/2506.05041)
*Usman Muhammad,Jorma Laaksonen*

Main category: eess.IV

TL;DR: DACN是一种双注意力卷积网络，用于解决2D CNN在超分辨率任务中缺乏全局上下文理解的问题，通过多头注意力和通道注意力提升性能。
...
Method: DACN结合多头注意力和通道注意力，捕捉局部和全局特征依赖，并设计优化的损失函数（L2正则化和空间-光谱梯度损失）。

Result: 在两个超光谱数据集上的实验表明，多头注意力和通道注意力的组合优于单独使用任一机制。
...
### [398] [PixCell: A generative foundation model for digital histopathology images](https://arxiv.org/abs/2506.05127)
...
TL;DR: PixCell是一种基于扩散的生成模型，用于病理学图像生成，解决了数据稀缺、隐私保护和虚拟染色等问题。


<details>
  <summary>Details</summary>
Motivation: 病理学数字化提供了大量数据，但标注数据稀缺、隐私问题和生成任务需求推动了生成模型的发展。

Method: 使用PanCan-30M数据集，采用渐进训练策略和自监督条件，训练扩散模型PixCell。

Result: PixCell生成高质量图像，可用于训练判别模型、数据增强和教育，并支持分子标记推断。

Conclusion: PixCell为计算病理学提供了强大的生成工具，模型已公开以促进研究。
...
### [399] [DM-SegNet: Dual-Mamba Architecture for 3D Medical Image Segmentation with Global Context Modeling](https://arxiv.org/abs/2506.05297)
*Hangyu Ji*

Main category: eess.IV

TL;DR: DM-SegNet提出了一种双Mamba架构，结合方向性状态转换和解剖感知分层解码，解决了现有医学SSM在编码器-解码器不兼容的问题，并在两个临床基准测试中取得了最佳性能。


<details>
  <summary>Details</summary>
Motivation: 现有医学SSM在3D医学图像分割中存在编码器-解码器不兼容问题，1D序列扁平化破坏了空间结构，而传统解码器无法利用Mamba的状态传播。

Method: DM-SegNet采用四方向3D扫描的双Mamba模块、门控空间卷积层和Mamba驱动的解码框架，实现双向状态同步。

Result: 在Synapse数据集上达到85.44%的DSC（腹部器官分割），在BraTS2023数据集上达到90.22%的DSC（脑肿瘤分割）。

Conclusion: DM-SegNet通过方向性状态转换和解剖感知解码，显著提升了3D医学图像分割的性能。
...
### [400] [RoboRefer: Towards Spatial Referring with Reasoning in Vision-Language Models for Robotics](https://arxiv.org/abs/2506.04308)
...
TL;DR: RoboRefer是一种3D感知的视觉语言模型，通过监督微调和强化微调提升空间指代任务的精确理解和多步推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在复杂3D场景中的空间指代能力不足，需要更精确的理解和动态推理。

Method: 提出RoboRefer，结合专用深度编码器进行监督微调（SFT），并通过强化微调（RFT）实现多步空间推理。

Result: SFT训练的RoboRefer在空间理解上达到89.6%成功率，RFT训练后性能显著提升，超越Gemini-2.5-Pro 17.4%。

Conclusion: RoboRefer在复杂场景中表现出色，可集成到多种机器人控制策略中执行动态任务。
...
### [401] [cuVSLAM: CUDA accelerated visual odometry](https://arxiv.org/abs/2506.04359)
*Alexander Korovko,Dmitry Slepichev,Alexander Efitorov,Aigul Dzhumamuratova,Viktor Kuznetsov,Hesam Rabeti,Joydeep Biswas*

Main category: cs.RO

TL;DR: cuVSLAM是一种先进的视觉SLAM解决方案，支持多种视觉-惯性传感器组合，适用于从单摄像头到多摄像头的机器人系统，并在边缘设备上实现实时性能。


<details>
  <summary>Details</summary>
Motivation: 准确的姿态估计是自主机器人的关键需求，需要一种能适应多种传感器配置且计算高效的SLAM解决方案。

Method: cuVSLAM利用CUDA优化，支持1到32个任意几何配置的摄像头和惯性测量单元，实现实时视觉SLAM。

Result: 在多个先进基准测试中，cuVSLAM表现出最佳性能。

Conclusion: cuVSLAM是一种高效、灵活且性能卓越的视觉SLAM解决方案，适用于广泛的机器人应用。
...
### [402] [Hierarchical Language Models for Semantic Navigation and Manipulation in an Aerial-Ground Robotic System](https://arxiv.org/abs/2506.05020)
*Haokun Liu,Zhaoqi Ma,Yunong Li,Junichiro Sugihara,Yicheng Chen,Jinjie Li,Moju Zhao*

Main category: cs.RO

TL;DR: 提出了一种结合大型语言模型（LLM）和视觉语言模型（VLM）的层次框架，用于异构多机器人系统的任务协调。
...
Method: LLM负责任务分解和全局语义地图构建，VLM从航拍图像中提取语义标签和空间信息以支持局部规划。

Result: 在真实世界的字母立方体排列任务中，框架展现了动态环境下的适应性和鲁棒性。

Conclusion: 这是首个将VLM感知与LLM驱动的任务推理和运动规划结合的空地异构系统。
...
### [403] [Learning Smooth State-Dependent Traversability from Dense Point Clouds](https://arxiv.org/abs/2506.04362)
*Zihao Dong,Alan Papalia,Leonard Jung,Alenna Spiro,Philip R. Osteen,Christa S. Robison,Michael Everett*

Main category: cs.RO

TL;DR: SPARTA是一种通过点云估计角度依赖地形可通行性的方法，利用傅里叶基函数输出平滑分析函数，减少计算开销并提高泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决越野自主性中地形可通行性依赖车辆状态的问题，尤其是障碍物仅从特定方向可通行的情况。

Method: 提出SPARTA方法，通过输出1-球面上的平滑分析函数（基于傅里叶基函数）预测风险分布，减少重复模型推断。

Result: 在高保真仿真中，模型通过40米巨石场的成功率为91%（基线为73%），并在硬件测试中展示泛化能力。

Conclusion: SPARTA通过几何结构和傅里叶基函数有效解决了角度依赖地形可通行性问题，具有高效性和泛化性。
...
### [404] [MineInsight: A Multi-sensor Dataset for Humanitarian Demining Robotics in Off-Road Environments](https://arxiv.org/abs/2506.04842)
*Mario Malizia,Charles Hamesse,Ken Hasselmann,Geert De Cubber,Nikolaos Tsiogkas,Eric Demeester,Rob Haelterman*

Main category: cs.RO

TL;DR: 论文介绍了MineInsight数据集，用于提升人道主义排雷中的计算机视觉技术验证。


<details>
  <summary>Details</summary>
Motivation: 现有数据集缺乏多样性和真实性，难以可靠验证排雷算法。

Method: 创建多传感器、多光谱数据集，包含35种目标，集成双视角传感器扫描和多种光谱范围图像。
...
### [405] [Synthetic Dataset Generation for Autonomous Mobile Robots Using 3D Gaussian Splatting for Vision Training](https://arxiv.org/abs/2506.05092)
*Aneesh Deogan,Wout Beks,Peter Teurlings,Koen de Vos,Mark van den Brand,Rene van de Molengraft*

Main category: cs.RO

TL;DR: 提出一种基于Unreal Engine的自动生成合成数据方法，用于解决手动标注数据集的高成本和低多样性问题，并在机器人足球场景中验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 手动标注数据集耗时耗力且易出错，尤其在机器人领域，多样性和动态性增加了数据集的创建难度。

Method: 利用Unreal Engine和3D高斯样条快速生成逼真的合成数据，并结合真实数据提升检测性能。

Result: 合成数据训练的检测器在机器人足球场景中表现与真实数据训练的检测器相当，且结合两者能显著提升性能。

Conclusion: 该方法为机器人领域提供了一种高效、可扩展的数据集生成方案，减少了手动标注的需求。
...
### [406] [SGN-CIRL: Scene Graph-based Navigation with Curriculum, Imitation, and Reinforcement Learning](https://arxiv.org/abs/2506.04505)
...
### [407] [Olfactory Inertial Odometry: Sensor Calibration and Drift Compensation](https://arxiv.org/abs/2506.04539)
...
Conclusion: 提出的OIO校准方法能有效提高气味源定位的精度，适用于特定应用场景。
...
### [408] [Chronoamperometry with Room-Temperature Ionic Liquids: Sub-Second Inference Techniques](https://arxiv.org/abs/2506.04540)
*Kordel K. France*
...
Result: 与标准计时电流法相比，新方法在保持合理精度的同时大幅缩短数据采集时间。

Conclusion: 该方法在分析化学、传感器技术和电池科学等领域具有广泛应用前景，尤其适用于快速电化学定量分析。
...
### [409] [ChemReservoir -- An Open-Source Framework for Chemically-Inspired Reservoir Computing](https://arxiv.org/abs/2506.04249)
*Mehmet Aziz Yirik,Jakob Lykke Andersen,Rolf Fagerberg,Daniel Merkle*

Main category: cs.CE

TL;DR: 本文介绍了ChemReservoir，一个开源的化学启发式储备计算框架，解决了以往基于DNA化学的工具的局限性。


<details>
  <summary>Details</summary>
Motivation: 以往基于DNA化学的储备计算工具维护不足且适用性有限，需要开发一个通用的开源工具。

Method: ChemReservoir是一个通用框架，支持化学启发式储备的构建和分析，并增强了测试、评估和可重复性。

Result: 工具在多种循环储备拓扑中表现稳定，适用于记忆容量任务。

Conclusion: ChemReservoir为化学启发式储备计算提供了通用且可靠的解决方案。
...
### [410] [GARG-AML against Smurfing: A Scalable and Interpretable Graph-Based Framework for Anti-Money Laundering](https://arxiv.org/abs/2506.04292)
*Bruno Deprez,Bart Baesens,Tim Verdonck,Wouter Verbeke*

Main category: cs.SI

TL;DR: GARG-AML是一种新型的基于图的方法，通过单一可解释的指标量化洗钱风险，结合树基方法和邻居节点评分，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 洗钱占全球GDP的2%-5%，传统方法复杂且不透明，阻碍了业务应用。

Method: GARG-AML利用二阶交易网络结构和邻居节点评分，结合树基方法，实现高效、透明和强大的检测能力。

Result: 在大规模合成和开源网络上的实验表明，GARG-AML优于现有方法。

Conclusion: GARG-AML展示了基础网络特性在欺诈检测中的潜力，适合集成到现有反洗钱工作流程。
...
### [411] [Solving engineering eigenvalue problems with neural networks using the Rayleigh quotient](https://arxiv.org/abs/2506.04375)
...
Result: 该方法适用于不规则域、参数和非线性特征问题以及高维特征分析，展示了在工程力学中的优势。
...
### [412] [CMIE: Combining MLLM Insights with External Evidence for Explainable Out-of-Context Misinformation Detection](https://arxiv.org/abs/2505.23449)
...
Method: 提出CMIE框架，包含共存关系生成策略和关联评分机制，选择性利用证据。

Result: 实验表明CMIE优于现有方法。

Conclusion: CMIE有效解决了MLLM在虚假信息检测中的挑战。
...
### [413] [Understanding and Meeting Practitioner Needs When Measuring Representational Harms Caused by LLM-Based Systems](https://arxiv.org/abs/2506.04482)
...
Method: 通过对12位实践者进行半结构化访谈，分析工具的可用性和实用性。
...
### [414] [Oversight Structures for Agentic AI in Public-Sector Organizations](https://arxiv.org/abs/2506.04836)
*Chris Schmitz,Jonathan Rystrøm,Jan Batzner*

Main category: cs.CY

TL;DR: 论文发现，引入代理AI系统加剧了传统公共部门监督机制的挑战，提出了五个关键治理维度，并评估现有监督结构的应对能力。


<details>
  <summary>Details</summary>
Motivation: 研究代理AI系统对公共部门监督机制的挑战，提出改进方法以适应新技术需求。

Method: 采用混合方法，包括文献综述和对AI相关公务员的访谈。

Result: 发现代理监督加剧了三个现有治理挑战，并提出适应性解决方案。

Conclusion: 建议调整制度结构并设计符合公共部门限制的代理监督方法。
...
### [415] [Intentionally Unintentional: GenAI Exceptionalism and the First Amendment](https://arxiv.org/abs/2506.05211)
...
<details>
  <summary>Details</summary>
Motivation: 挑战当前对AI模型输出是否应受第一修正案保护的假设，探讨其法律和社会影响。
...
### [416] [Can we reconstruct a dysarthric voice with the large speech model Parler TTS?](https://arxiv.org/abs/2506.04397)
*Ariadna Sanchez,Simon King*

Main category: eess.AS

TL;DR: 论文研究了利用大型语音模型Parler TTS为构音障碍患者重建语音，尝试生成其发病前的声音。结果显示模型能学习生成数据，但在控制清晰度和保持说话者身份一致性方面存在困难。


<details>
  <summary>Details</summary>
Motivation: 构音障碍患者沟通困难，个性化文本转语音技术可作为辅助工具。研究旨在探索大型语音模型能否重建患者发病前的声音。

Method: 使用Parler TTS模型，通过标注的数据集进行微调，评估其生成语音的清晰度和说话者身份一致性。

Result: 模型能学习生成数据，但在清晰度和说话者身份一致性方面表现不佳。
...
### [417] [Towards Efficient Speech-Text Jointly Decoding within One Speech Language Model](https://arxiv.org/abs/2506.04518)
*Haibin Wu,Yuxuan Hu,Ruchao Fan,Xiaofei Wang,Kenichi Kumatani,Bo Ren,Jianwei Yu,Heng Lu,Lijuan Wang,Yao Qian,Jinyu Li*

Main category: eess.AS

TL;DR: 本文比较了联合语音文本解码策略（交错与并行生成），发现交错方法对齐最佳但推理慢，提出了一种新的早期停止交错（ESI）模式以加速解码并提升性能。
...
Method: 在相同基础语言模型、语音分词器和训练数据下，系统比较交错与并行生成策略，并提出早期停止交错（ESI）模式。

Result: 交错方法对齐最佳但推理慢；ESI模式显著加速解码且性能略优。
...
### [418] [EMO-Debias: Benchmarking Gender Debiasing Techniques in Multi-Label Speech Emotion Recognition](https://arxiv.org/abs/2506.04652)
...
Method: 提出EMO-Debias，比较13种去偏方法（包括预处理、正则化、对抗学习等），在性别不平衡条件下评估。
...
### [419] [Knowledge-guided Contextual Gene Set Analysis Using Large Language Models](https://arxiv.org/abs/2506.04303)
...
TL;DR: cGSA是一种新型AI驱动的基因集分析框架，通过结合上下文感知的通路优先级排序，解决了传统GSA方法忽略临床背景的问题。


<details>
  <summary>Details</summary>
Motivation: 传统GSA方法生成冗余且不相关的通路列表，需要大量人工解释，降低了可靠性和可重复性。

Method: cGSA整合基因簇检测、富集分析和大型语言模型，识别具有统计显著性和生物学意义的通路。

Result: 在102个基因集和19种疾病上的测试显示，cGSA性能优于基线方法30%以上，专家验证证实其精确性和可解释性。

Conclusion: cGSA在黑色素瘤和乳腺癌的案例中展示了其发现上下文特异性见解的能力，支持针对性假设生成。
...
### [420] [MVP-Shapley: Feature-based Modeling for Evaluating the Most Valuable Player in Basketball](https://arxiv.org/abs/2506.04602)
*Haifeng Sun,Yu Xiong,Runze Wu,Kai Wang,Lan Zhang,Changjie Fan,Shaojie Tang,Xiang-Yang Li*

Main category: cs.GT

TL;DR: 提出了一种基于Shapley值的MVP评估框架（\oursys），通过特征处理、胜负模型训练、Shapley值分配和贡献排名，优化算法以匹配专家投票结果，并在NBA和Dunk City Dynasty数据集中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 随着电竞和在线多人游戏的快速发展，评估MVP的重要性日益凸显，但现有方法缺乏解释性和实用性。

Method: 提出\oursys框架，利用Shapley值进行特征处理、模型训练和贡献分配，并通过因果视角优化算法以匹配专家投票结果。

Result: 在NBA和Dunk City Dynasty数据集上验证了方法的有效性，并实现了工业部署。

Conclusion: \oursys框架为MVP评估提供了一种解释性强且实用的解决方案，适用于电竞和体育领域。
...
### [421] [The GAIN Model: A Nature-Inspired Neural Network Framework Based on an Adaptation of the Izhikevich Model](https://arxiv.org/abs/2506.04247)
*Gage K. R. Hooper*
...
Method: 使用网格结构使神经元与邻近神经元交互，结合Izhikevich模型。

Result: 实现了计算高效且生物准确的模拟，适用于神经网络开发和大规模仿真。

Conclusion: GAIN模型通过网格结构和Izhikevich模型提升了动态性和准确性，适用于专业领域。
...
### [422] [Generalizable, real-time neural decoding with hybrid state-space models](https://arxiv.org/abs/2506.05320)
*Avery Hee-Woon Ryoo,Nanda H. Krishna,Ximeng Mao,Mehdi Azabou,Eva L. Dyer,Matthew G. Perich,Guillaume Lajoie*

Main category: q-bio.NC

TL;DR: POSSM是一种混合架构，结合了交叉注意力模块和循环状态空间模型，用于实时神经活动解码，兼具快速推理和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决传统方法泛化能力不足和Transformer计算资源需求高的问题。

Method: 采用交叉注意力模块进行单个尖峰标记化，结合循环状态空间模型，支持多数据集预训练。

Result: 在猴子和人类任务中表现优异，推理速度比Transformer快9倍，泛化能力强。

Conclusion: 混合SSM是平衡准确性、推理速度和泛化能力的有效方法，适用于实时闭环应用。
...
### [423] [Benchmark for Antibody Binding Affinity Maturation and Design](https://arxiv.org/abs/2506.04235)
...
TL;DR: AbBiBench是一个抗体结合亲和力成熟与设计的基准框架，通过评估抗体-抗原复合物的功能潜力来改进现有方法。
...
Method: AbBiBench通过测量蛋白模型在抗体-抗原复合物上的似然性来评估抗体设计潜力，并使用9个数据集和14种蛋白模型进行系统比较。

Result: 结构条件逆向折叠模型在亲和力相关性和生成任务中表现最佳。

Conclusion: AbBiBench提供了一个统一的、基于生物学的评估框架，有助于开发更有效的功能感知抗体设计模型。
...
### [424] [The Latent Space Hypothesis: Toward Universal Medical Representation Learning](https://arxiv.org/abs/2506.04515)
*Salil Patel*

Main category: q-bio.QM

TL;DR: 论文提出了一种基于潜在空间假设的框架，将多模态医学数据映射到一个统一的几何表示中，用于个性化医疗。


<details>
  <summary>Details</summary>
Motivation: 医学数据多模态且复杂，传统分类方法可能掩盖疾病的异质性。潜在空间假设旨在统一这些数据，揭示疾病的真实轨迹。

Method: 通过将医学数据投影到一个统一的几何表示中，将健康状态、疾病进展和治疗干预分别表示为点、轨迹和向量。

Result: 该框架能够揭示疾病的子轨迹和患者特异性变化方向，为个性化诊断和治疗提供定量依据。

Conclusion: 潜在空间假设为临床实践提供了新的方向，但仍需解决数据偏见、稀缺性和隐私等问题。
...
### [425] [Artificial Intelligence Should Genuinely Support Clinical Reasoning and Decision Making To Bridge the Translational Gap](https://arxiv.org/abs/2506.05030)
*Kacper Sokol,James Fackler,Julia E Vogt*

Main category: cs.HC

TL;DR: 论文提出了一种新的社会技术概念化方法，旨在通过数据驱动工具辅助医生的认知和认知活动，而非追求无关基准的超人性能。


<details>
  <summary>Details</summary>
Motivation: 当前以技术为中心的方法导致人工智能在医学中的实际影响有限，与临床实践（尤其是诊断推理和决策）不兼容。

Method: 提出了一种社会技术概念化方法，设计数据驱动的支持工具以补充医生的认知和认知活动。

Result: 该方法强调实际影响而非无关基准的超人性能。

Conclusion: 通过社会技术方法可以更好地将人工智能融入临床实践，解决当前的翻译差距。
...
### [426] [Teaming in the AI Era: AI-Augmented Frameworks for Forming, Simulating, and Optimizing Human Teams](https://arxiv.org/abs/2506.05265)
*Mohammed Almutairi*
...
<details>
  <summary>Details</summary>
Motivation: 现有团队优化工具依赖静态数据或特定场景，无法适应动态团队需求，导致成员不满和团队表现下降。

Method: 1. 多臂老虎机算法优化团队组建；2. AI系统tAIfa提供个性化反馈；3. LLM框架PuppeteerLLM模拟团队动态。

Result: 提升团队满意度、凝聚力和表现。

Conclusion: AI增强的团队优化框架能有效解决动态团队管理问题。
...
### [427] [Can Artificial Intelligence Trade the Stock Market?](https://arxiv.org/abs/2506.04658)
*Jędrzej Maskiewicz,Paweł Sakowski*

Main category: q-fin.TR

TL;DR: 论文比较了DDQN和PPO两种DRL算法在股票交易中的表现，结果显示DRL在风险调整收益上优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 探索DRL在股票交易中的应用，验证其相比传统监督学习方法的优势。

Method: 使用DDQN和PPO算法，在多种资产（货币对、S&P 500、比特币）的日数据上进行测试。

Result: DRL能有效管理风险，避免不利交易，风险调整收益优于传统方法。

Conclusion: DRL在股票交易中具有显著优势，尤其在风险管理和收益优化方面。
...
### [428] [Optical Physics-Based Generative Models](https://arxiv.org/abs/2506.04357)
*Amirreza Ahmadnejad,Somayyeh Koohi*

Main category: physics.optics

TL;DR: 论文建立了光学物理方程与生成模型之间的数学框架，展示了光传播动力学如何启发强大的人工智能方法。非线性光学模型通过自组织原理表现出卓越能力，参数减少40-60%，性能显著提升。


<details>
  <summary>Details</summary>
Motivation: 探索光学物理方程与生成模型的联系，利用光传播动力学的自组织特性提升人工智能方法的效率和性能。

Method: 分析六种基本光学方程（线性和非线性），包括Helmholtz、耗散波和Eikonal方程及其非线性扩展，通过实验验证非线性模型的优越性。

Result: 非线性模型在参数减少、模式分离、稳定性和生成质量上显著优于线性模型和传统方法，FID分数更低，内存和训练时间大幅优化。

Conclusion: 揭示了物理自组织与人工智能的深层联系，为高效光学计算和生成模型提供了新途径。
...
### [429] [hdl2v: A Code Translation Dataset for Enhanced LLM Verilog Generation](https://arxiv.org/abs/2506.04544)
*Charles Hong,Brendan Roberts,Huijae An,Alex Um,Advay Ratan,Yakun Sophia Shao*

Main category: cs.AR

TL;DR: 论文介绍了hdl2v数据集，通过将VHDL、Chisel和PyMTL3翻译为Verilog，增加公开可用的Verilog代码量，并提升LLM在Verilog生成任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 公开可用的Verilog代码量远少于其他软件语言（如Python），限制了LLM在硬件代码生成中的应用。

Method: 通过翻译或编译VHDL、Chisel和PyMTL3生成Verilog数据集hdl2v，并验证其在提升LLM性能中的作用。

Result: hdl2v使32B参数模型的Verilog生成性能提升23%（pass@10），数据增强微调方法性能提升63%。

Conclusion: hdl2v有效解决了Verilog数据不足问题，未来可进一步优化数据集特性以提升性能。
...
### [430] [QiMeng: Fully Automated Hardware and Software Design for Processor Chip](https://arxiv.org/abs/2506.05007)
...
TL;DR: QiMeng是一个自动化处理器芯片设计的系统，通过分层结构解决设计挑战，结合领域特定的大模型（LPCM）实现硬件和软件设计的自动化。
...
Method: QiMeng采用三层结构：底层是领域特定的大模型LPCM，中层是硬件和软件设计代理，顶层是应用。

Result: 部分组件已成功应用于实际场景，展示了高效自动化设计的优势。

Conclusion: 未来将整合所有组件，通过迭代设计完善QiMeng系统。
...
### [431] [TQml Simulator: Optimized Simulation of Quantum Machine Learning](https://arxiv.org/abs/2506.04891)
*Viacheslav Kuzmin,Basil Kyriacou,Mateusz Papierz,Mo Kordzanganeh,Alexey Melnikov*

Main category: quant-ph

TL;DR: 论文研究了量子机器学习中硬件高效电路的模拟方法，开发了名为TQml Simulator的数值模拟器，性能优于现有工具。


<details>
  <summary>Details</summary>
Motivation: 量子机器学习中硬件高效电路的快速数值模拟对研究至关重要，需优化模拟方法以加速算法运行。

Method: 通过数值基准测试比较通用和门特定技术，开发了TQml Simulator，针对不同电路层选择最优模拟方法。

Result: TQml Simulator在多种电路和硬件上表现优异，性能提升2至100倍。

Conclusion: 优化模拟方法可显著提升量子机器学习算法的模拟效率，TQml Simulator为研究提供了高效工具。
...
### [432] [Deep learning image burst stacking to reconstruct high-resolution ground-based solar observations](https://arxiv.org/abs/2506.04781)
*Christoph Schirninger,Robert Jarolim,Astrid M. Veronig,Christoph Kuckein*

Main category: astro-ph.SR

TL;DR: 提出一种基于深度学习的实时图像重建方法，用于解决地面太阳望远镜观测中的大气湍流问题。
...
Result: 该方法在感知质量上表现更优，尤其在参考图像存在伪影时，且能充分利用图像序列信息。

Conclusion: 该方法在提供完整图像序列时，能够实现最佳重建效果。
...
### [433] [An Expansion-Based Approach for Quantified Integer Programming](https://arxiv.org/abs/2506.04452)
*Michael Hartisch,Leroy Chew*

Main category: cs.DM

TL;DR: 本文提出了一种基于扩展的QIP求解方法，采用CEGAR技术，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: QIP结合了QBF和整数规划的优势，但现有方法仅限于搜索范式，扩展方法未被探索。

Method: 引入基于CEGAR的扩展方法，适用于多阶段鲁棒离散优化问题。

Result: 实验证明该方法在特定实例中优于现有搜索求解器，且在QBF扩展求解器中表现突出。

Conclusion: 扩展方法为QIP提供了高效求解工具，尤其适用于复杂决策和不确定性优化问题。
...
### [434] [Through the Stealth Lens: Rethinking Attacks and Defenses in RAG](https://arxiv.org/abs/2506.04390)
...
<details>
  <summary>Details</summary>
Motivation: 现有攻击易被检测，需隐蔽性更强的攻击方式，同时探索防御方法的局限性。

Method: 提出Normalized Passage Attention Score和Attention-Variance Filter算法，利用注意力模式检测毒化段落。

Result: 防御方法提升准确率约20%，但适应性攻击成功率仍达35%。

Conclusion: 注意力模式防御有效但仍有局限，需进一步研究隐蔽攻击的应对策略。
...
### [435] [Learning to Diagnose Privately: DP-Powered LLMs for Radiology Report Classification](https://arxiv.org/abs/2506.04450)
...
<details>
  <summary>Details</summary>
Motivation: 解决敏感医疗数据在微调LLM时的隐私风险，防止数据泄露。

Method: 使用MIMIC-CXR和CT-RATE数据集，通过DP-LoRA方法在不同隐私预算下微调模型，评估性能。

Result: 在中等隐私保护下，模型性能接近非隐私基线（MIMIC-CXR: 0.88 vs 0.90; CT-RATE: 0.59 vs 0.78）。

Conclusion: DP-LoRA方法有效实现了隐私保护的多异常分类，为医疗数据微调LLM提供了可行方案。
...
### [436] [BESA: Boosting Encoder Stealing Attack with Perturbation Recovery](https://arxiv.org/abs/2506.04556)
...
<details>
  <summary>Details</summary>
Motivation: 在基于扰动的防御环境下，传统编码器窃取攻击性能受限，因此需要一种能够克服此类防御的方法。

Method: BESA包含扰动检测和扰动恢复两个模块，前者推断防御机制，后者利用生成模型恢复干净特征向量。

Result: 实验表明，BESA在面对先进防御及多防御组合时，能将替代编码器准确率提升高达24.63%。
...
### [437] [On Automating Security Policies with Contemporary LLMs](https://arxiv.org/abs/2506.04838)
...
Method: 系统通过收集和管理工具及API规范，存储在向量数据库中，利用RAG将高级缓解策略分解为离散任务并转化为可执行的API调用。

Result: 使用RAG的实验评估在STIXv2格式的公开CTI策略和Windows API文档中，相比非RAG基线，显著提升了精确率、召回率和F1分数。

Conclusion: 该框架为自动化安全策略执行提供了一种高效且可扩展的解决方案。
...
### [438] [Why LLM Safety Guardrails Collapse After Fine-tuning: A Similarity Analysis Between Alignment and Fine-tuning Datasets](https://arxiv.org/abs/2506.05346)
*Lei Hsiung,Tianyu Pang,Yung-Chen Tang,Linyue Song,Tsung-Yi Ho,Pin-Yu Chen,Yaoqing Yang*

Main category: cs.CR

TL;DR: 研究发现上游安全对齐数据与下游微调任务的数据相似性对模型安全性有显著影响，高相似性会削弱安全护栏，而低相似性则能提升模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注微调过程中的反应性措施，忽略了上游安全对齐数据的作用，因此研究其影响。

Method: 通过实验分析上游对齐数据集与下游微调任务的表示相似性对安全护栏的影响。

Result: 高相似性削弱安全护栏，低相似性使模型更鲁棒，有害性评分降低10.33%。

Conclusion: 上游数据集设计对构建持久安全护栏和减少现实漏洞至关重要，为微调服务提供商提供实用建议。
...
### [439] [EMBER2024 -- A Benchmark Dataset for Holistic Evaluation of Malware Classifiers](https://arxiv.org/abs/2506.05074)
...
<details>
  <summary>Details</summary>
Motivation: 解决现有公共数据集范围狭窄、标签单一且缺乏逃避检测文件的问题。

Method: 与EMBER2017和EMBER2018作者合作，创建包含320万文件的数据集，支持七种分类任务。

Result: EMBER2024数据集包含多种文件格式和特征，首次提供逃避检测的恶意文件挑战集。
...
### [440] [Membership Inference Attacks on Sequence Models](https://arxiv.org/abs/2506.05126)
*Lorenzo Rossi,Michael Aerni,Jie Zhang,Florian Tramèr*

Main category: cs.CR

TL;DR: 论文提出了一种改进序列模型隐私泄露审计的方法，通过利用序列生成中的相关性，改进了现有的成员推理攻击方法。
...
### [441] [Computationally Intensive Research: Advancing a Role for Secondary Analysis of Qualitative Data](https://arxiv.org/abs/2506.04230)
*Kaveh Mohajeri,Amir Karami*

Main category: cs.DB

TL;DR: 探讨计算密集型二次分析在定性数据再利用中的潜力及其挑战。


<details>
  <summary>Details</summary>
Motivation: 定性研究数据通常未被充分利用，计算密集型二次分析可挖掘其跨上下文和纵向研究的价值。

Method: 提出一种计算密集型二次分析方案，并探讨其在创新研究设计中的应用。

Result: 展示了计算密集型二次分析的优势，同时指出数据共享与再利用的关键挑战。

Conclusion: 计算密集型二次分析为定性数据再利用提供了新思路，但需解决数据共享与伦理问题。
...
### [442] [A Multi-Dataset Evaluation of Models for Automated Vulnerability Repair](https://arxiv.org/abs/2506.04987)
*Zanis Ali Khan,Aayush Garg,Qiang Tang*

Main category: cs.SE

TL;DR: 研究探讨了预训练语言模型CodeBERT和CodeT5在自动漏洞修复中的表现，发现CodeBERT在稀疏上下文表现更好，而CodeT5在复杂漏洞模式识别和扩展性上更优。微调模型在未见数据上泛化能力有限。


<details>
  <summary>Details</summary>
Motivation: 软件漏洞修复是安全关键任务，但现有自动程序修复技术对此研究不足。

Method: 使用CodeBERT和CodeT5在六个数据集和四种语言上进行漏洞修复实验，评估其准确性和泛化能力。

Result: CodeBERT在稀疏上下文表现更好，CodeT5在复杂漏洞和扩展性上更优；微调模型在未见数据上泛化能力有限。

Conclusion: 研究为漏洞修复提供了基准和实用见解，但泛化能力仍需改进。
...
### [443] [Leveraging Reward Models for Guiding Code Review Comment Generation](https://arxiv.org/abs/2506.04464)
*Oussama Ben Sghaier,Rosalia Tufano,Gabriele Bavota,Houari Sahraoui*

Main category: cs.SE

TL;DR: 论文提出了一种基于深度学习和强化学习的自动化代码审查框架CoRAL，通过生成语义相似且有用的评论，显著提升了代码审查的效率和质量。


<details>
  <summary>Details</summary>
Motivation: 代码审查是软件开发中的重要环节，但耗时且受主观因素影响。现有自动化技术（如深度学习）虽能生成评论或优化代码，但缺乏综合考虑评论语义和实用性的方法。

Method: CoRAL利用强化学习框架，通过奖励机制评估生成评论的语义相似性和实用性（作为代码优化模型的输入）。

Result: 定量和定性分析表明，CoRAL生成的评论在语义和实用性上优于现有基线技术。

Conclusion: CoRAL框架在自动化代码审查中表现出高效性和优越性，为未来研究提供了新方向。
...
### [444] [A Comprehensive Survey on Bio-Inspired Algorithms: Taxonomy, Applications, and Future Directions](https://arxiv.org/abs/2506.04238)
...
TL;DR: 本文综述了生物启发算法（BIAs）的分类、应用及未来研究方向，涵盖八类算法及其在多个领域的应用，同时指出了当前挑战。


<details>
  <summary>Details</summary>
Motivation: 旨在为研究者和实践者提供生物启发计算领域的全面资源，帮助理解当前研究现状和未来方向。

Method: 将BIAs分为八类（如进化算法、群体智能等），分析其核心原理、优势与局限，并探讨在机器学习等领域的应用。

Result: 总结了BIAs的最新进展（如混合策略、参数调优等），并提出了可扩展性、收敛性等开放性问题。

Conclusion: 本文为生物启发计算的研究和实践提供了基础性参考，并指明了未来研究方向。
...
### [445] [On the Wasserstein Geodesic Principal Component Analysis of probability measures](https://arxiv.org/abs/2506.04480)
*Nina Vesseron,Elsa Cazelles,Alice Le Brigant,Thierry Klein*

Main category: stat.ML

TL;DR: 论文提出了一种基于Otto-Wasserstein几何的GPCA方法，用于分析概率分布的变异性，并通过神经网络参数化Wasserstein空间中的测地线。


<details>
  <summary>Details</summary>
Motivation: 研究如何在概率分布集合中识别最能捕捉数据变异模式的测地线。

Method: 针对高斯分布，将计算提升到可逆线性映射空间；对一般概率测度，采用神经网络参数化Wasserstein空间中的测地线。

Result: 通过实例比较了经典切空间PCA，并在真实数据集上进行了验证。

Conclusion: GPCA方法在概率分布空间中有效捕捉变异性，神经网络参数化提供了通用解决方案。
...
### [446] [Regret-Optimal Q-Learning with Low Cost for Single-Agent and Federated Reinforcement Learning](https://arxiv.org/abs/2506.04626)
*Haochen Zhang,Zhong Zheng,Lingzhou Xue*

Main category: stat.ML

TL;DR: 论文提出两种新型无模型强化学习算法，旨在降低样本成本和策略切换/通信成本，同时保持最优遗憾性能。


<details>
  <summary>Details</summary>
Motivation: 解决实际应用中数据收集和策略部署成本高的问题，特别是在单智能体和联邦强化学习（FRL）中。

Method: 提出Q-EarlySettled-LowCost和FedQ-EarlySettled-LowCost算法，结合线性样本成本和对数级策略切换/通信成本。

Result: 算法在最优遗憾、样本成本和通信成本上均优于现有方法，并提供理论保证。

Conclusion: 新算法在降低成本和保持性能方面具有显著优势，适用于实际应用场景。
...
### [447] [Distributional encoding for Gaussian process regression with qualitative inputs](https://arxiv.org/abs/2506.04813)
*Sébastien Da Veiga*

Main category: stat.ML

TL;DR: 论文提出了一种基于分布编码（DE）的GP回归方法，用于处理分类变量，并通过实验验证了其优越性能。


<details>
  <summary>Details</summary>
Motivation: 在工程应用中，GP回归是一种高效的方法，但处理分类变量时仍具挑战性。本文旨在解决这一问题。

Method: 提出分布编码（DE）方法，利用目标变量的所有样本信息，结合特征核函数（如MMD和Wasserstein距离）构建GP模型。

Result: 实验证明DE方法在合成和真实数据集上具有领先的预测性能。

Conclusion: DE方法为处理分类变量的GP回归提供了有效解决方案，并可与离散和混合空间的BO方法互补。
...
### [448] [Learning Joint Interventional Effects from Single-Variable Interventions in Additive Models](https://arxiv.org/abs/2506.04945)
*Armin Kekić,Sergio Hernan Garrido Mejia,Bernhard Schölkopf*

Main category: stat.ML

TL;DR: 论文提出了一种仅用观测数据和单变量干预数据推断联合干预效应的方法，并通过实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 在许多领域中，联合干预的因果效应估计很重要，但获取联合干预数据困难。

Method: 提出了一种可识别性结果，证明在非线性加性结果机制下，无需联合干预数据即可推断联合效应，并设计了一种分解因果效应的估计器。

Result: 实验表明，该方法性能接近直接使用联合干预数据训练的模型，优于纯观测估计器。

Conclusion: 该方法为联合干预效应的估计提供了一种实用且高效的解决方案。
...
### [449] [Nonlinear Causal Discovery for Grouped Data](https://arxiv.org/abs/2506.05120)
*Konstantin Göbler,Tobias Windisch,Mathias Drton*

Main category: stat.ML

TL;DR: 论文提出了一种处理随机向量的非线性加性噪声模型，用于推断组变量间的因果关系，并在模拟和实际装配线数据中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 许多领域（如神经科学、心理学、社会科学和工业制造）中，因果关系的单位是变量组而非标量变量，现有方法对此支持不足。

Method: 扩展非线性加性噪声模型至随机向量，提出两步法：先推断随机向量的因果顺序，再选择最佳因果图。

Result: 在模拟中表现优异，并成功应用于实际装配线数据。

Conclusion: 该方法为组变量间的因果推断提供了有效工具，适用于多种实际场景。
...
### [450] [Causal Effect Identification in lvLiNGAM from Higher-Order Cumulants](https://arxiv.org/abs/2506.05202)
*Daniele Tramontano,Yaroslav Kivva,Saber Salehkaleybar Mathias Drton,Negar Kiyavash*

Main category: stat.ML

TL;DR: 论文研究了在潜在变量线性非高斯无环模型（lvLiNGAM）中使用高阶累积量识别因果效应，解决了两种具有挑战性的情况：单代理变量和欠定工具变量。


<details>
  <summary>Details</summary>
Motivation: 解决在潜在混杂变量存在下，因果效应识别的挑战性问题，特别是单代理变量和欠定工具变量的情况。

Method: 利用高阶累积量证明因果效应在单代理或工具变量下可识别，并提供相应的估计方法。

Result: 实验结果表明，相比现有方法，所提方法具有更高的准确性和鲁棒性。

Conclusion: 论文在理论和实践上推进了线性系统中潜在混杂变量下因果推断的理解。
...
### [451] [Admissibility of Completely Randomized Trials: A Large-Deviation Approach](https://arxiv.org/abs/2506.05329)
*Guido Imbens,Chao Qin,Stefan Wager*

Main category: stat.ML

TL;DR: 在最佳臂识别问题中，当实验者可以选择自适应试验时，忽略此选项而运行非自适应试验是不可取的。研究发现，对于至少三个处理臂的情况，存在简单的自适应设计，其在统计效率上普遍且严格优于完全随机试验。


<details>
  <summary>Details</summary>
Motivation: 探讨在最佳臂识别问题中，自适应试验是否比非自适应试验更具优势，以解决实验设计中的效率问题。

Method: 研究采用批量臂淘汰设计，逐步淘汰表现不佳的处理臂，并通过效率指数量化设计的统计效率。

Result: 对于至少三个处理臂的情况，某些自适应设计在统计效率上普遍且严格优于完全随机试验。

Conclusion: 自适应设计在最佳臂识别问题中具有显著优势，解决了Qin [2022]中提出的第二个开放问题。
...
### [452] [Spore in the Wild: Case Study on Spore.fun, a Real-World Experiment of Sovereign Agent Open-ended Evolution on Blockchain with TEEs](https://arxiv.org/abs/2506.04236)
*Botao Amber Hu,Helena Rong*

Main category: cs.MA

TL;DR: 论文探讨了基于去中心化物理基础设施网络（DePIN）和区块链技术的开放系统是否能够实现持续的开端演化（OEE），并通过Spore.fun实验进行了案例分析。


<details>
  <summary>Details</summary>
Motivation: 传统封闭系统模拟（如Tierra和Avida）在实现持续OEE方面存在局限，学者认为开放系统可能更有效。

Method: 利用DePIN和区块链技术，结合TEEs，部署自主运行的LLM-based AI代理，并通过Spore.fun实验观察其行为与演化。

Result: Spore.fun展示了AI代理在开放环境中的自主演化能力，为OEE的实现提供了新思路。

Conclusion: 开放ALife系统结合经济激励可能最终实现OEE，但仍需进一步讨论与研究。
...
### [453] [CORA: Coalitional Rational Advantage Decomposition for Multi-Agent Policy Gradients](https://arxiv.org/abs/2506.04265)
...
<details>
  <summary>Details</summary>
Motivation: 在多智能体强化学习中，全局优势共享常导致次优策略更新，因为未考虑智能体的独特贡献。现有方法缺乏对联盟层面的详细分析。

Method: 提出CORA方法，通过评估所有可能联盟的边际贡献，利用合作博弈论的核心解决方案分解优势，确保联盟理性，并采用随机联盟采样降低计算开销。
...
### [454] [Autonomous Collaborative Scheduling of Time-dependent UAVs, Workers and Vehicles for Crowdsensing in Disaster Response](https://arxiv.org/abs/2506.04276)
...
### [455] [Time to Talk: LLM Agents for Asynchronous Group Communication in Mafia Games](https://arxiv.org/abs/2506.05309)
...
Method: 开发了一种自适应异步LLM代理，收集了在线Mafia游戏数据集进行评估。

Result: 代理在游戏表现和融入人类玩家方面与人类相当，发言时机与人类相似，但内容有差异。

Conclusion: 为LLM在真实人类群体环境中的应用铺平了道路，支持进一步研究异步通信。
...
### [456] [Domain Adaptation Method and Modality Gap Impact in Audio-Text Models for Prototypical Sound Classification](https://arxiv.org/abs/2506.04376)
*Emiliano Acevedo,Martín Rocamora,Magdalena Fuentes*

Main category: cs.SD

TL;DR: 音频-文本模型在零样本环境声音分类中表现优异，但背景声源会显著降低其性能。本文提出一种新方法，量化并整合背景声源的影响，无需重新训练模型即可提升分类效果。


<details>
  <summary>Details</summary>
Motivation: 背景声源会显著降低音频-文本模型的分类性能，而现有方法未充分解决这一问题。

Method: 提出一种量化背景声源贡献的方法，并通过缩小音频与文本嵌入的模态差距来提升分类性能。

Result: 该方法在不同背景和信噪比条件下均提升了分类准确性，且适用于多种先进的原型方法。

Conclusion: 该方法无需重新训练模型，即可显著提升音频-文本模型在复杂环境中的分类性能，具有广泛适用性。
...
### [457] [Grapheme-Coherent Phonemic and Prosodic Annotation of Speech by Implicit and Explicit Grapheme Conditioning](https://arxiv.org/abs/2506.04527)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法仅通过微调预训练ASR模型生成标签，缺乏与字形的直接关联，限制了其在文本到语音等任务中的应用。

Method: 1）使用预训练BERT特征通过提示编码器隐式条件化字形；2）在推理阶段显式修剪与字形不一致的标签假设。

Result: 实验表明，该方法显著提升了字形与预测标签的一致性，并在口音估计任务中提高了准确性。

Conclusion: 所提出的方法通过字形条件化生成标签，为下游任务提供了更高质量的并行数据。
...
### [458] [LLM-based phoneme-to-grapheme for phoneme-based speech recognition](https://arxiv.org/abs/2506.04711)
*Te Ma,Min Bi,Saierdaer Yusuyin,Hao Huang,Zhijian Ou*

Main category: cs.SD

TL;DR: 论文提出了一种基于大型语言模型（LLM）的音素到字素解码方法（LLM-P2G），用于音素基础的自动语音识别（ASR），并通过数据增强和随机化训练策略解决了信息损失问题，显著优于传统WFST方法。


<details>
  <summary>Details</summary>
Motivation: 在音素基础的多语言预训练和跨语言微调中，WFST解码方法因复杂流程和无法利用大型语言模型（LLM）而受限，因此需要一种更高效的方法。

Method: 提出LLM-P2G解码方法，包括语音到音素（S2P）和音素到字素（P2G）两部分，并通过数据增强（DANP）和随机化训练（TKM）策略解决信息损失问题。

Result: 实验表明，LLM-P2G在波兰语和德语的跨语言ASR中，相对WER分别降低了3.6%和6.9%，优于WFST系统。

Conclusion: LLM-P2G为音素基础的ASR提供了一种高效解码方法，显著提升了跨语言任务的表现。
...
### [459] [Survey on the Evaluation of Generative Models in Music](https://arxiv.org/abs/2506.05104)
...
<details>
  <summary>Details</summary>
Motivation: 随着音乐生成系统的快速发展，系统评估的需求日益增长，但缺乏统一的评估框架。

Method: 通过跨学科视角，综合分析了评估目标、方法和指标，包括音乐学、工程学和HCI视角。

Result: 总结了不同评估方法的优缺点，为未来研究提供了参考。

Conclusion: 需要进一步整合多学科方法，以建立更全面的音乐生成系统评估框架。
...
### [460] [Improving AI-generated music with user-guided training](https://arxiv.org/abs/2506.04852)
...
Method: 采用人类计算方法，收集用户评分作为损失函数，结合遗传算法优化模型。

Result: 初步测试显示，第一轮迭代评分平均提高0.2，第二轮再提高0.39。

Conclusion: 人类计算方法有效提升了AI音乐生成模型的个性化性能。
...
### [461] [Exp4Fuse: A Rank Fusion Framework for Enhanced Sparse Retrieval using Large Language Model-based Query Expansion](https://arxiv.org/abs/2506.04760)
*Lingyuan Liu,Mengxiang Zhang*

Main category: cs.IR

TL;DR: Exp4Fuse是一种新颖的融合排序框架，通过零样本LLM查询扩展提升稀疏检索性能，优于现有方法并在多个基准测试中达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有LLM查询扩展方法依赖高质量生成文档和复杂提示策略，成本高且计算量大，因此探索零样本LLM扩展以优化稀疏检索。

Method: 提出Exp4Fuse框架，结合原始查询和LLM增强查询的检索路径，通过改进的互惠排名融合方法生成最终排名列表。

Result: 在MS MARCO和低资源数据集上，Exp4Fuse超越现有LLM扩展方法，结合先进稀疏检索器时达到SOTA性能。

Conclusion: Exp4Fuse显著提升了稀疏检索的查询扩展效果，展示了其高效性和性能优势。
...
### [462] [GOLFer: Smaller LM-Generated Documents Hallucination Filter & Combiner for Query Expansion in Information Retrieval](https://arxiv.org/abs/2506.04762)
*Lingyuan Liu,Mengxiang Zhang*

Main category: cs.IR

TL;DR: GOLFer是一种利用小型开源语言模型进行查询扩展的新方法，通过过滤幻觉内容和组合文档，性能优于其他小型模型方法，并与大型模型竞争。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）用于查询扩展成本高且计算密集，小型模型生成的文档常包含不准确内容。GOLFer旨在解决这些问题。

Method: GOLFer包括幻觉过滤器（检测并移除不准确句子）和文档组合器（用权重向量平衡查询与文档内容）。

Result: 在多个数据集上，GOLFer优于其他小型模型方法，并与大型模型性能相当。

Conclusion: GOLFer证明了小型模型在查询扩展中的潜力，提供了一种高效且经济的选择。
...
### [463] [Towards Storage-Efficient Visual Document Retrieval: An Empirical Study on Reducing Patch-Level Embeddings](https://arxiv.org/abs/2506.04997)
*Yubo Ma,Jinsong Li,Yuhang Zang,Xiaobao Wu,Xiaoyi Dong,Pan Zhang,Yuhang Cao,Haodong Duan,Jiaqi Wang,Yixin Cao,Aixin Sun*

Main category: cs.IR

TL;DR: 论文研究了如何减少ColPali/ColQwen2在可视化文档检索（VDR）中的内存使用，通过评估两种标记减少策略（标记剪枝和标记合并），发现标记合并更适合VDR，并开发了Light-ColPali/ColQwen2，显著降低了内存占用且性能损失极小。


<details>
  <summary>Details</summary>
Motivation: ColPali/ColQwen2在VDR中性能强大，但因其将每页编码为多个补丁级嵌入导致内存占用过高，研究旨在减少每页的补丁嵌入数量，同时最小化性能损失。

Method: 评估了两种标记减少策略：标记剪枝（包括随机剪枝和其他复杂方法）和标记合并。研究发现剪枝不适合VDR，转而优化标记合并策略，开发了Light-ColPali/ColQwen2。

Result: Light-ColPali/ColQwen2在仅使用11.8%原始内存的情况下保持了98.2%的检索性能，在2.8%内存占用下仍保留94.6%的有效性。

Conclusion: 研究为高效VDR提供了有价值的见解，并建立了竞争性基线，Light-ColPali/ColQwen2在内存和性能之间取得了良好平衡。
...
TL;DR: 论文提出R2Rec框架，通过推理链增强推荐系统，利用渐进式掩码提示策略模拟用户决策过程，显著提升性能与可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM推理方法在推荐任务中效果不佳，因用户反馈隐式且缺乏监督。R2Rec旨在解决这一问题。

Method: R2Rec从用户-物品图中采样交互链，通过渐进式掩码提示生成结构化推理链，结合监督微调和强化学习训练。

Result: 在三个真实数据集上，R2Rec平均提升HitRatio@1 10.48%，性能超过原始LLM 131.81%。

Conclusion: R2Rec通过显式推理链提升推荐性能与可解释性，为LLM在推荐任务中的应用提供了新思路。
...
### [465] [BEAR: BGP Event Analysis and Reporting](https://arxiv.org/abs/2506.04514)
*Hanqing Li,Melania Fedeli,Vinay Kolar,Diego Klabjan*

Main category: cs.NI

TL;DR: BEAR框架利用大型语言模型自动生成BGP异常事件的详细报告，提升了解释性和分析精度。
...
Method: BEAR通过多步推理将表格BGP数据转化为文本叙述，并利用LLM生成合成数据。

Result: 在真实和合成数据集上，BEAR达到100%准确率，优于基线方法。

Conclusion: BEAR为BGP异常事件提供自动化解释，为网络管理提供实用洞察。
...
### [466] [Intelligent Channel Allocation for IEEE 802.11be Multi-Link Operation: When MAB Meets LLM](https://arxiv.org/abs/2506.04594)
...
<details>
  <summary>Details</summary>
Motivation: WiFi 7的MLO功能在密集网络环境中面临信道分配挑战，现有研究多关注静态网络性能优化，本文旨在解决动态信道分配问题。

Method: 将问题建模为组合优化问题，采用MAB框架进行在线学习，提出BAI-MCTS算法并结合LLM改进为LLM-BAI-MCTS。
...
### [467] [Towards Network Data Analytics in 5G Systems and Beyond](https://arxiv.org/abs/2506.04860)
*Marcos Lima Romero,Ricardo Suyama*

Main category: cs.NI

TL;DR: 论文分析了移动网络运营商（MNOs）如何通过数据分析和NWDAF（网络数据分析功能）提升价值，提出了两个新用例以促进其应用和商业化。


<details>
  <summary>Details</summary>
Motivation: 移动网络运营商的数据利用不足，而OTT玩家主导市场价值。数据分析和NWDAF被视为提升价值和竞争力的关键。

Method: 分析了70多篇文献中的趋势和空白，并提出了两个新的NWDAF用例。

Result: 研究发现现有研究局限于有限的方法和用例，新用例有助于拓展NWDAF的应用潜力。

Conclusion: 通过NWDAF的广泛应用和商业化，移动网络运营商可以超越连接服务，创造更大价值。
...
### [468] [BridgeNet: A Hybrid, Physics-Informed Machine Learning Framework for Solving High-Dimensional Fokker-Planck Equations](https://arxiv.org/abs/2506.04354)
...
Result: 在多测试案例中，BridgeNet误差更低、收敛更快，且在高维环境中稳定。

Conclusion: BridgeNet为计算物理提供可扩展、精确的解决方案，适用于金融数学和复杂系统动力学。
